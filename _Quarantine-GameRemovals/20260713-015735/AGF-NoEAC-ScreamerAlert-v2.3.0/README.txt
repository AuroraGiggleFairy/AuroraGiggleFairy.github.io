...
------------------------------------------------------------------------
  ...
  - /agfsa count: Set COUNT mode. Requires local EnhancedAGF; otherwise
    resolves to ON.

  - Admin Console Commands (F1): agf-sa help: Show admin command usage.
  - agf-sa default <off|on|count>: Set the default mode for new joining
    players.
...
v2.3.0
    ...
    - Updated /agfsa chat command to also display all options.
    - Updated the formatting of F1 command agf-sa list
    - Updating the handshake between server and clients.
    - Updated the Commands-Readme.txt.
