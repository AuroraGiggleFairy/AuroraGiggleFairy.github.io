...
- Chat command for server-side-only users: /sa <off|on>.
...
- Chat command to set mode: /sa <off|on|counts|cycle>.
- Console command (F1) to set mode: agf-sa <off|on|counts|cycle>.
...
- Admin target command (F1): agf-sa admin <entityId> <off|on|counts|cycle>.
