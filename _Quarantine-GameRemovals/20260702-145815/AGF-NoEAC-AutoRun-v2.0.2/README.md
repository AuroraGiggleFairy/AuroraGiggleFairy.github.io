...
7d2d Version 3
**Version:** 2.0.2  
[Download](https://github.com/AuroraGiggleFairy/AuroraGiggleFairy.github.io/raw/main/04_DownloadZips/AGF-NoEAC-AutoRun.zip)

 > Thanks silverviolet9 for the detailed report and testing to find the exact trigger of a major bug.
...
- "AGF-NoEAC-AutoRun" is SAFE to install on an existing game.
- "AGF-NoEAC-AutoRun" is TBD to remove from an existing game.
- Unique Details: May only require client install, ignores server install.
...
- Works standalone.
- Adds Auto-Run for keyboard and controller.
- Keyboard Auto-Run is rebindable (default Z).
- Optional controller Auto-Run support is in Controller settings.
...
- Works standalone.
- Adds Auto-Run on foot and in vehicles for both keyboard and controller.
- Keyboard: use the rebindable Auto-Run key (default Z) to start; press Sprint to toggle Sprint Lock.
- Controller (optional in Controller settings): double-press Sprint on foot or Turbo in a vehicle to start.
- Stop Auto-Run with Forward/Backward/Esc (keyboard) or Backward/Menu (controller).
- Shows an icon next to the sprint one that auto-run (cruise control) is turned on.
...
<!-- CHANGELOG START -->
v2.0.3
- The Auto-Run key is now a toggle, not just "On only."

v2.0.2
- Fixed a bug that would prevent auto run on vehicles.
...
- Default key is now "z" for keyboard.
- Hotkey for keyboard is configurable in the menu.
- Controller support, turn on/off in the options menu.
