...
7d2d Version 2  
Version: 1.2.1-BETA  
Download: https://github.com/AuroraGiggleFairy/AuroraGiggleFairy.github.io/raw/main/04DownloadZips/AGF-NoEAC-ScreamerAlert.zip
...
4. Mod Type
- Hybrid: EAC off required; server install works for all joining players; client install is optional for extra features.
...
- "AGF-NoEAC-ScreamerAlert" is TBD to remove from an existing game.
...
- This is a hybrid server/client mod.
- Server-only install works and players still benefit.
- Installing on both server and client provides the best experience.
- Players are not required to download the mod to join your game.

All Versions:
- Alerts are triggered when live Screamers or Screamer-spawned zombies are within 120 meters.

Server-Side Use:
- Players without the client-side install receive a private chat alert.
- Server-side only users can use off/on only (private message alerts enabled or disabled).
- Server-side alerts are initial notification only (not live-tracked in UI).
- Chat command for server-side-only users: /sa .
- Chat command to check current setting: /sa status.

Client-Side Use:
- Provides a real-time on-screen alert.
- The alert disappears when no matching enemies are alive or in range.
- Can display the number of currently tracked Screamers and horde zombies.
- 3 Modes: Off, On (alert only), Counts (shows Screamer and horde zombie counts).
- Chat command to set mode: /sa .
- Console command (F1) to set mode: agf-sa .
- Status command: /sa status or agf-sa status.
- Can also be toggled via the ESC Window mod (if installed).

Admin Notes:
- Console commands (F1): "agf-screameralert" or "agf-sa".
- Admin target command (F1): agf-sa admin  .
...
v1.2.0
- Attempt to create a Hybrid version of this mod.
- Added chat and console commands
- Ensured correct implementation into the windowESC mod.
