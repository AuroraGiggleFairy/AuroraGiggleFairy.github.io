...
public class ItemActionEntryOpenAll : BaseItemActionEntry
{
    ...
    public override void RefreshEnabled()
    {
        ...
        bool canOpen = liveOpenEntry != null ? liveOpenEntry.Enabled : fallbackOpenEntry != null;
...
public static class OpenAllActionHelpers
{
    ...
    public static void SetActionEntryLabel(BaseItemActionEntry entry, string actionName)
    {
        ...
        try
        {
            PropertyInfo actionNameProperty = typeof(BaseItemActionEntry).GetProperty("ActionName", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
            if (actionNameProperty != null && actionNameProperty.CanWrite)
            {
                actionNameProperty.SetValue(entry, actionName, null);
                return;
            }

            FieldInfo actionNameField = typeof(BaseItemActionEntry).GetField("actionName", BindingFlags.NonPublic | BindingFlags.Instance)
    ...
    public static XUiC_ItemStack GetStackController(BaseItemActionEntry entry)
    {
        ...
        PropertyInfo itemControllerProperty = typeof(BaseItemActionEntry).GetProperty("ItemController", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
        if (itemControllerProperty == null)
        {
            return null;
        }

        return itemControllerProperty.GetValue(entry, null) as XUiC_ItemStack;
    }
    ...
    private static bool HasOpenItemAction(ItemClass itemClass)
    {
        ...
        PropertyInfo actionsProperty = itemClass.GetType().GetProperty("Actions", BindingFlags.Public | BindingFlags.Instance);
        if (actionsProperty == null)
        {
            return false;
        }

        object actionsObject = actionsProperty.GetValue(itemClass, null);
