...
internal class RestDMChannel : RestChannel, IDMChannel, IMessageChannel, IChannel, ISnowflakeEntity, IEntity<ulong>, IPrivateChannel, IRestPrivateChannel, IRestMessageChannel
{
	public RestUser CurrentUser { get; }

	public RestUser Recipient { get; }

	public IReadOnlyCollection<RestUser> Users => System.Collections.Immutable.ImmutableArray.Create(CurrentUser, Recipient);

	private string DebuggerDisplay => $"@{Recipient} ({base.Id}, DM)";

	IUser IDMChannel.Recipient => Recipient;

	IReadOnlyCollection<RestUser> IRestPrivateChannel.Recipients => System.Collections.Immutable.ImmutableArray.Create(Recipient);
	...
	internal RestDMChannel(BaseDiscordClient discord, ulong id, ulong recipientId)
		: base(discord, id)
	{
		Recipient = new RestUser(base.Discord, recipientId);
		CurrentUser = new RestUser(base.Discord, discord.CurrentUser.Id);
	}

	internal new static RestDMChannel Create(BaseDiscordClient discord, Channel model)
	{
		RestDMChannel restDMChannel = new RestDMChannel(discord, model.Id, model.Recipients.Value[0].Id);
		restDMChannel.Update(model);
		return restDMChannel;
	}
