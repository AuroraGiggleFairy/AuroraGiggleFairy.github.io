...
- **`05_GigglePackReleaseData`** now only holds `Discord/` and the GigglePack release-state files (`ModNetwork/` and `NexusMods/` moved into `06_PublishingSupport` - see above). `80_Automation`, the script registry, and a centralized path module (Section 12) still don't exist - those remain needed before the *rest* of the root scripts can move.
