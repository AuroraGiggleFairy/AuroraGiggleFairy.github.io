...
- Reference guide for menu lifecycle/hook details:
  - `_DLL-Projects/DLL_NoEAC-AudioOptionsPlus/MCP-7D2D-Menu-Influence-Guide.md`
...
- 2026-07-15:
  ...
  - Optimized menu responsiveness by:
    - avoiding repeated full-tree scans when binding 31 AOP custom rows (single prebuilt option map per bind pass),
    - throttling update-loop fallback pending checks to once per frame and skipping when `UnsavedChanges` is already true,
    - caching per-frame pending-change computation in `XUiC_AudioOptions.HasPendingChangesAgainstConfig()` to prevent repeated expensive config loads during unsaved-state evaluation.
- 2026-07-16:
  - Aligned custom list preset rows with vanilla localization behavior by adding `localization_prefix` to both `AOPVolumeProfilesOverallVolumePreset` and `AOPSoundSwapAnimalPainDeathSoundPreset` in live mod `windows.xml` and `_x2.6` reference `windows.xml`.
  - Added token-suffix localization key aliases for those two preset lists in live `Localization.csv` and `_x2.6` `Localization.txt` so dropdown entries resolve localized text with vanilla-style `localization_prefix`.
