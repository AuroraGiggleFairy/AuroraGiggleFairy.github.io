...
public class XUiC_AudioOptions : XUiController
{
    ...
    private bool HasPendingChangesAgainstConfig()
    {
        ...
        bool volumeChanged =
            !Nearly(GetValue(_augerChainsaw), AudioOptionsPlusConfig.AugerChainsawMultiplier) ||
            !Nearly(GetValue(_impactHarvestSurface), AudioOptionsPlusConfig.ImpactHarvestSurfaceMultiplier) ||
            !Nearly(GetValue(_gunFire), AudioOptionsPlusConfig.GunFireMultiplier) ||
            !Nearly(GetValue(_explosions), AudioOptionsPlusConfig.ExplosionsMultiplier) ||
            !Nearly(GetValue(_vehiclesNoHorn), AudioOptionsPlusConfig.VehiclesNoHornMultiplier) ||
            !Nearly(GetValue(_electricalBlockLoops), AudioOptionsPlusConfig.ElectricalBlockLoopsMultiplier) ||
            !Nearly(GetValue(_craftingCompleteUi), AudioOptionsPlusConfig.CraftingCompleteUiMultiplier) ||
            !Nearly(GetValue(_spiderZombie), AudioOptionsPlusConfig.SpiderZombieMultiplier) ||
            !Nearly(GetValue(_animalPainDeath), AudioOptionsPlusConfig.AnimalPainDeathMultiplier) ||
            !Nearly(GetValue(_placeUpgradeRepair), AudioOptionsPlusConfig.PlaceUpgradeRepairMultiplier) ||
            !Nearly(GetValue(_protectionDings), AudioOptionsPlusConfig.ProtectionDingsMultiplier) ||
            !Nearly(GetValue(_interactionPrompts), AudioOptionsPlusConfig.InteractionPromptsMultiplier) ||
            !Nearly(GetValue(_twitch), AudioOptionsPlusConfig.TwitchMultiplier) ||
            !Nearly(GetValue(_doorsHatchesVaultsCellarsBridge), AudioOptionsPlusConfig.DoorsHatchesVaultsCellarsBridgeMultiplier) ||
            !Nearly(GetValue(_traderBob), AudioOptionsPlusConfig.TraderBobMultiplier) ||
            !Nearly(GetValue(_traderHugh), AudioOptionsPlusConfig.TraderHughMultiplier) ||
            !Nearly(GetValue(_traderJen), AudioOptionsPlusConfig.TraderJenMultiplier) ||
            !Nearly(GetValue(_traderJoel), AudioOptionsPlusConfig.TraderJoelMultiplier) ||
            !Nearly(GetValue(_traderRekt), AudioOptionsPlusConfig.TraderRektMultiplier) ||
            !Nearly(GetValue(_playerMadeSounds), AudioOptionsPlusConfig.PlayerMadeSoundsMultiplier);
        ...
        return volumeChanged || swapChanged;
    }
