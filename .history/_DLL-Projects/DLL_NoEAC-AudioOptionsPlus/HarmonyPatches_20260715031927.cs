...
internal static class AudioOptionsPlusConfig
{
    ...
    private static readonly Dictionary<string, string> UiSessionSnapshot = new Dictionary<string, string>(StringComparer.Ordinal);
    private static bool UiSessionActive;
    private static bool UiSessionSaved;
    ...
    public static void BeginOptionsUiSession()
    {
        lock (Sync)
        {
            ...
            for (int i = 0; i < UiSessionKeys.Length; i++)
            {
                ...
                {
                    UiSessionSnapshot[key] = PlayerPrefs.GetString(key);
                }
            }

            UiSessionActive = true;
            UiSessionSaved = false;
        }
    }
    ...
    public static void EndOptionsUiSession()
    {
        lock (Sync)
        {
            ...
            if (!UiSessionSaved)
            {
                ...
                {
                    string key = UiSessionKeys[i];
                    if (UiSessionSnapshot.TryGetValue(key, out string value))
                    {
