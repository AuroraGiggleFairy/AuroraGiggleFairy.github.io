...
internal static class Patch_XUiC_OptionsAudio_Init_AudioOptionsPlusCustom
{
    ...
    internal static void BindAllCustomOptions(XUiC_OptionsAudio instance)
    {
        ...
        {
            BindCustomOption(instance, AopCustomOptionNames[i]);
        }
    }

    private static void BindCustomOption(XUiC_OptionsAudio instance, string name)
    {
        ...
        XUiC_OptionEntryCustom option = (controller as XUiC_OptionEntryCustom) ?? controller?.GetChildByType<XUiC_OptionEntryCustom>();
        if (option == null)
        {
            XUiC_OptionEntryCustom[] candidates = instance.windowGroup?.Controller?.GetChildrenByType<XUiC_OptionEntryCustom>() ?? Array.Empty<XUiC_OptionEntryCustom>();
            for (int i = 0; i < candidates.Length; i++)
            {
                string id = candidates[i]?.ViewComponent?.ID;
                if (string.Equals(id, optionId, StringComparison.OrdinalIgnoreCase) || string.Equals(id, name, StringComparison.OrdinalIgnoreCase))
                {
                    option = candidates[i];
                    break;
                }
        ...
        if (option == null)
        {
            Console.WriteLine("[AudioOptionsPlus] OptionEntryCustom bind failed: " + optionId);
            return;
        }

        Console.WriteLine("[AudioOptionsPlus] OptionEntryCustom bound: " + option.ViewComponent?.ID + " (" + name + ")");
        ...
        {
            if (XUiC_AudioOptions.ApplyUsingOptionsAudio(instance) || XUiC_AudioOptions.ApplyAllOpenControllersToConfig())
            {
                AudioOptionsPlusConfig.MarkOptionsUiSaved();
            }
        };
        ...
        {
            if (!XUiC_AudioOptions.ResetUsingOptionsAudio(instance))
            {
                XUiC_AudioOptions.ResetAllOpenControllersToDefaults();
            }
        };
        option.IsChangedDelegate = () =>
        {
            return XUiC_AudioOptions.HasPendingChangesUsingOptionsAudio(instance) || XUiC_AudioOptions.HasAnyOpenControllerPendingChanges();
        };
        option.IsDefaultDelegate = () =>
        {
            return XUiC_AudioOptions.IsAtDefaultsUsingOptionsAudio(instance);
        };
    }
}
