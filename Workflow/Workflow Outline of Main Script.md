...
Steps in this outline are grouped under the numbered script that owns them:
...
## Section A: Lane Model and Script Modes (Reference)
...
## Script 01 — `01_sync_work.py`: Sync Game and Active Mods

> **Mode:** `--mode sync-work`
> Syncs `02_ActiveBuild` with the game Mods folder, handles BackpackPlus and HUDPlus optionals routing.
...
## Script 02 — `02_promote.py`: Promote Tested Mods to Release

> **Mode:** `--mode promote`
> First refreshes folder names and per-mod README files in `02_ActiveBuild`, then promotes to `03_ReleaseSource`. Promotion is version-gated.
...
## Script 03 — `03_package.py`: Package Release Files

> **Mode:** `--mode package`
> First refreshes folder names and per-mod README files in `03_ReleaseSource`, then builds zip outputs, GigglePack release notes, Discord post, and updates the main README.
...
### Step 7: Auto-Versioned GigglePack Release Notes
...
### Step 8: Discord Auto-Post (Update-Only)
...
## Step 10: What If Something Goes Wrong?
...
## Script 05 — `05_pipeline_engine.py`: Full Pipeline Engine

> **Mode:** `--mode full` (and delegated to by scripts 01–03 for all actual work)
> This is the implementation core. Scripts 01, 02, and 03 are thin wrappers that call this engine in the appropriate mode. When running `--mode full`, this script executes Steps 1–9 end-to-end in a single pass.

### Key Functions in This Script

| Function | What it does |
|----------|--------------|
| `sync_workspace_and_game()` | Steps 1–2: find, match, and sync mods by version |
| `move_mods_by_major_version()` | Step 3: route mods to Draft vs. Release lane |
| `sync_staging_and_game()` | Sync `02_ActiveBuild` with game, route optionals |
| `promote_staging_to_publish_ready()` | Steps 4–5: rename, CSV, quotes, README, push back |
| `create_all_zips()` | Step 6: build per-mod and category zips |
| `generate_gigglepack_release_artifacts()` | Step 7: version GigglePack, write release notes |
| `generate_main_readme()` | Step 9: update root README.md from template |
| `run_pipeline()` | Entry: dispatches to the correct steps based on `--mode` |
