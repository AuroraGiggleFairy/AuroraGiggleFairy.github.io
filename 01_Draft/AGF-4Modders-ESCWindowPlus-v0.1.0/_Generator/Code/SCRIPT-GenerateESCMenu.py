...
def build_texts_payload(source: dict) -> dict:
    ...
    ui = source.get("ui", {}) if isinstance(source.get("ui", {}), dict) else {}
    joining_title = str(labels.get("joiningTitle") or labels.get("optionsTitle") or "Joining")
    joining_title_hc = joining_title
    ...
    return {
        ...
        "serverJoin": {
            ...
            "joiningTitleHC": joining_title_hc,
...
def _parse_texts_markdown(text: str) -> dict:
    ...
    joining_title_hc: str | None = None
    ...
    for raw_line in text.splitlines():
        ...
        if section == "serverjoin":
            ...
            if token == "joiningtitle":
                ...
                continue
            if token == "joiningtitlehc":
                joining_title_hc = parsed_value
                continue
        ...
        if section == "adminpages" and stripped.startswith("## "):
            if current_admin_page is not None:
                _trim_page_columns_trailing_blanks(current_admin_page)
                body_lines = current_admin_page.get("bodyLines")
                current_admin_page["body"] = "\n".join(body_lines) if isinstance(body_lines, list) else ""
...
def apply_text_overrides(source: dict, texts_path: Path) -> tuple[dict, int]:
    ...
    # HC joining title intentionally mirrors full-color joiningTitle.
...
def build_localization_rows(source: dict, use_hc_dev_overrides: bool = False) -> list[list[str]]:
    ...
    joining_title_hc = labels.get("joiningTitleHC", joining_title)
    ...
    joining_body_hc = normalize_text(
        _apply_inline_body_styles(
            joining_body,
    ...
    rows: list[list[str]] = [
        ...
        ["windowESCAdmin_mode_highvisibility_tab", "UI", mode_hc_label],
    ]
    ...
    high_rows: list[list[str]] = [
        ...
        ["windowESC_highvisibility_join_title", "UI", joining_title_hc],
        ["windowESC_highvisibility_join_body", "UI", joining_body_hc],
    ...
    if active_links:
        for link_idx, link in enumerate(active_links, start=1):
            ...
            high_rows.append(
                [
                    ...
                    "UI",
                    _colorize_text(link_label, theme_hc["linkButtonText"]),
                ]
            )
    else:
        ...
        high_rows.append(
            [
                ...
                "UI",
                _colorize_text(fallback_label, theme_hc["linkButtonText"]),
            ]
        )
    ...
    def _append_page_rows(tab_list: list[dict], key_prefix: str, page_token: str = "page") -> None:
        for tab in tab_list:
            ...
            high_rows.append([f"{key_prefix}_highvisibility_{page_token}_{idx}_body", "UI", body_columns_hc[0]])
...
def _build_tabs_contents_xml(
    ...
    for tab in tab_models:
        idx = tab["idx"]
        mode_prefix = f"{key_prefix}_highvisibility" if is_hc else f"{key_prefix}_color"
        ...
        page_name = f"{name_prefix}{page_token}_{idx}_hc" if is_hc else f"{name_prefix}{page_token}_{idx}"
        title_name = f"{name_prefix}{page_token}_{idx}_hc_title" if is_hc else f"{name_prefix}{page_token}_{idx}_title"
        body_name = f"{name_prefix}{page_token}_{idx}_hc_body" if is_hc else f"{name_prefix}{page_token}_{idx}_body"
        scrollable_body = False
        body_columns = _get_tab_body_columns(tab, is_hc=is_hc)
        body_layout = _get_body_column_layout(len(body_columns))
        single_body_text = body_columns[0] if body_columns else ""
...
def sync_windows_copied_variants(
    ...
    theme_hc = get_theme_colors(source, "highContrast") if isinstance(source, dict) else dict(DEFAULT_AUTO_HIGH_CONTRAST_COLORS)
    ...
    hc_font_tag = _localization_color_value(str(theme_hc.get("pageBody", DEFAULT_AUTO_HIGH_CONTRAST_COLORS["pageBody"])))
    hc_highlight_tag = _localization_color_value(str(theme_hc.get("textHighlight", DEFAULT_AUTO_HIGH_CONTRAST_COLORS["textHighlight"])))
    ...
    def _convert_options_to_server_join_actions(block: str) -> str:
        ...
        updated = re.sub(
            r'\btext_key="windowESC_color_options_title"',
            'text_key="windowESC_color_join_title"',
            updated,
            flags=re.IGNORECASE,
        )
        updated = re.sub(
            r'\btext_key="windowESC_highvisibility_options_title"',
            'text_key="windowESC_highvisibility_join_title"',
            updated,
            flags=re.IGNORECASE,
        )

        def _replace_joining_block(match: re.Match[str]) -> str:
            indent = match.group("indent")
            block = match.group("block")
            is_hc = "windowESC_highvisibility" in block
            title_color = "141,181,128"
            title_color_match = re.search(
                r'name="joining_title"[^>]*\bcolor="([^"]+)"',
                block,
                re.IGNORECASE,
            )
            if title_color_match:
                title_color = title_color_match.group(1)

            title_key = "windowESC_highvisibility_join_title" if is_hc else "windowESC_color_join_title"
            body_key = "windowESC_highvisibility_join_body" if is_hc else "windowESC_color_join_body"
            font_tag = hc_font_tag if is_hc else color_font_tag
            highlight_tag = hc_highlight_tag if is_hc else color_highlight_tag
            body_color = str(theme_hc.get("pageBody", DEFAULT_AUTO_HIGH_CONTRAST_COLORS["pageBody"])) if is_hc else str(theme_color.get("pageBody", DEFAULT_THEME_COLORS["pageBody"]))
            if is_hc:
            ...
            return (
                ...
                f"{indent}\t<sprite name=\"joiningBackground\" depth=\"2\" pos=\"5,-5\" sprite=\"ui_game_header_fill\" color=\"95,89,128\" width=\"310\" height=\"540\" type=\"sliced\"/>\n"
                f"{indent}\t<sprite name=\"joiningBorder\" depth=\"1\" sprite=\"ui_game_header_fill\" color=\"0,0,0\" width=\"320\" height=\"550\" type=\"sliced\"/>\n"
                ...
                f"{indent}\t<label name=\"dayTimeLabel\" depth=\"10\" pos=\"155,-431\" width=\"250\" height=\"28\" text=\"{font_tag}{{mapdaytimetitle}}:[-] {highlight_tag}{{mapdaytime}}[-]\" justify=\"center\" pivot=\"center\" effect=\"Outline8\" effect_color=\"0,0,0,255\" effect_distance=\"1,1\" font_size=\"26\" controller=\"MapStats\"/>\n"
                f"{indent}</rect>"
            )

        updated = re.sub(
            r'(?ms)^(?P<indent>[ \t]*)<rect\b[^>]*\bname="joining"[^>]*>\s*(?P<block>.*?)^[ \t]*</rect>',
            _replace_joining_block,
            updated,
        )
        updated = re.sub(
        ...
        # Remove stale orphan closing tags left from older options/joining transforms.
        updated = re.sub(
            r'(?ms)(^[ \t]*<rect\b[^>]*\bname="joining"[^>]*>.*?^[ \t]*</rect>\s*\n)[ \t]*</rect>(\s*\n[ \t]*<!--\s*LINKS[^>]*-->)',
            r'\1\2',
            updated,
        )
        updated = re.sub(
    ...
    def _ensure_pages_headers(block: str, target_context: str) -> str:
        ...
        def _replace(match: re.Match[str]) -> str:
            ...
            mode_name = "HIGH VISIBILITY" if re.search(r'_hc\b|highvisibility', first_page_line, re.IGNORECASE) else "FULL COLOR"
            view_suffix = ""
    ...
    def _build_admin_root_tabs(existing_tabs_block: str) -> str:
        ...
        player_hc_range = _extract_first_named_rect_block(player_tabs_inner, "HighVisibility")
        admin_color_range = _extract_first_named_rect_block(admin_tabs_inner, "Color")
        admin_hc_range = _extract_first_named_rect_block(admin_tabs_inner, "HighVisibility")
        if not player_color_range or not player_hc_range or not admin_color_range or not admin_hc_range:
            return existing_tabs_block

        player_color_block = player_color_range[2]
        player_hc_block = player_hc_range[2]
        admin_color_block = admin_color_range[2]
        admin_hc_block = admin_hc_range[2]
        ...
        player_hc_inner = _unwrap_mode_block(player_hc_block)
        admin_color_inner = _unwrap_mode_block(admin_color_block)
        admin_hc_inner = _unwrap_mode_block(admin_hc_block)

        def _replace_admin_inner_tabs(mode_inner_block: str, is_hc: bool) -> str:
        ...
        admin_color_inner = _replace_admin_inner_tabs(admin_color_inner, is_hc=False)
        admin_hc_inner = _replace_admin_inner_tabs(admin_hc_inner, is_hc=True)

        def _build_view_tabs(player_mode_block: str, admin_mode_block: str, mode_label: str) -> str:
            ...
            return "\n".join(
                [
                    ...
                    '\t\t\t<rect controller="TabSelectorButton">',
                    '\t\t\t\t<simplebutton name="tabButton" depth="12" pos="0,0" width="170" height="36" font_size="30" sprite="ui_game_header_fill" bordercolor="0,0,0" defaultcolor="[mediumGrey]" selectedsprite="menu_empty3px" selectedcolor="[lightGrey]" foregroundlayer="false" caption="{tab_name_localized}"/>',
                    '\t\t\t</rect>',
                    '\t\t</grid>',
                    '\t</rect>',
                    '\t<rect name="tabsContents">',
        ...
        color_view_tabs_block = _build_view_tabs(player_color_inner, admin_color_inner, "FULL COLOR")
        hc_view_tabs_block = _build_view_tabs(player_hc_inner, admin_hc_inner, "HIGH VISIBILITY")
        ...
        return "\n".join(
            [
                ...
                _indent_block(hc_view_tabs_block, "\t\t\t"),
                "\t\t</rect>",
                "\t</rect>",
                "</rect>",
            ]
