...
7d2d Version 2  
**Version:** 0.1.1-BETA  
[Download](https://github.com/AuroraGiggleFairy/AuroraGiggleFairy.github.io/raw/main/04_DownloadZips/AGF-NoEAC-VisualEntityTracker.zip)
...
- "AGF-NoEAC-VisualEntityTracker" is UNKNOWN to remove from an existing game.
...
All Versions:
- Compass Visual Entity Indicator when an entity is within 25m of player.

Server-Side Use:
- Works from server XML/buff logic for players who join without local install.
- Chat command for server-side-only users: /vet <off|on>.

Client-Side Use:
- Uses DLL-enhanced local tracking behavior.
- Provides the most responsive and consistent tracking experience.
- Chat command to toggle on/off: /vet <off|on>.
- Console command (F1) to toggle on/off: agf-vet <off|on>.
- Can also be toggled via the ESC Window mod (if installed).

Admin Notes:
- Console commands (F1): "agf-visualentitytracker" or "agf-vet".
- Admin target command (F1): agf-vet admin <entityId> <off|on>.
<!-- FEATURES END -->
...
<!-- CHANGELOG START -->
v0.1.1
- Put in Hybrid version stuff.

v0.1.0
- Created first version (BETA).
