...
BASE_DOWNLOAD_URL = 'https://github.com/AuroraGiggleFairy/AuroraGiggleFairy.github.io/raw/main/_Mods3.zip'
...
def get_base_mod_name(name):
    """Strip trailing -vX.Y.Z version suffix from a folder name."""
    return re.sub(r'-v\d+\.\d+(\.\d+)*$', '', name)

def parse_modinfo(modinfo_path, fallback_name):
    """Return (name, version) from a ModInfo.xml, using fallback values on error."""
    try:
        ...
        mod_name, mod_version = fallback_name, '0.0.0'
    return mod_name, mod_version

def get_modinfo_version(folder_path):
    """Return the version string from ModInfo.xml, or None if missing/unreadable."""
...
def markdown_features_to_html(features_text):
    """Convert a Markdown bullet list to an HTML unordered list."""
...
def extract_readme_block(readme_path, start_marker, end_marker):
    """Return the text between two HTML comment markers in a README file."""
...
def extract_mod_description_from_modinfo(modinfo_path):
    """Return the Description value attribute from ModInfo.xml."""
    if not os.path.exists(modinfo_path):
        return ''
    try:
        tree = ET.parse(modinfo_path)
        ...
        if desc_tag is not None:
            return desc_tag.attrib.get('value', '')
    except Exception:
        pass
    return ''

def load_compat_csv():
    """Load HELPER_ModCompatibility.csv and return (fieldnames_list, rows_list)."""
    ...
    with open(COMPAT_CSV, 'r', encoding='utf-8', newline='') as f:
        ...
        fieldnames = list(reader.fieldnames) if reader.fieldnames else default_fields
        rows       = list(reader)
    return fieldnames, rows

def save_compat_csv(fieldnames, rows):
    """Sort rows alphabetically by MOD_NAME and write to HELPER_ModCompatibility.csv."""
...
def scan_mod_dirs():
    """Return {folder_name: path} dicts for PublishReady, InProgress, and Game."""
    def _scan(base):
        ...
        return {
            ...
            if os.path.isdir(os.path.join(base, f)) and is_agf_mod(f)
        }
    return _scan(PUBLISH_READY), _scan(IN_PROGRESS), _scan(GAME_MODS)
