# AGF 7D2D Mod Management Script Workflow (Beginner Friendly)

Welcome! This guide will help you use the AGF mod management script for 7 Days to Die. Each step is explained simply, with clear instructions. **Always finish one step for all mods before moving to the next.**

---

## Step 1: Find and Match Your Mods

1. **Look for mod folders** in these places:
...
2. **Only use folders that start with `AGF` or `zzzAGF`.**
...
1. For each mod with the same folder name:
  ...
  - If the version in `_Mods1.PublishReady` or `_Mods2.In-Progress` is higher: **This should not happen.**
2. **Note:**
  - If you change the Name in ModInfo.xml but not the folder name, the script still matches by folder name.
  - Folder renaming is handled later.
...
2. This ensures your game is always ready to test the latest version of any mod you worked on, and you never lose changes made in VS Code.
3. Only mods that were originally pulled from the game mods folder during this script run are pushed back—nothing else is overwritten.

---

## Step 6: Create Zip Files for Mods and Categories (After All Other Steps)

**Important:** Only start this step after you have finished Steps 1–5 for all mods. Zipping is always the last stage, so your zips include all updates, renames, and new README files.
...
3. Also generate `ReadableReadMe.txt` from the main README.md, converting Markdown to plain text for easy reading or sharing.
4. This ensures your main documentation is always up to date and ready to publish or distribute with your mod pack.

- The script enforces consistent formatting for all category headers and special sections in README.md, including a single divider (`---`), blank line, `<br>`, blank line, and the section header. This applies to all main categories and the HUDPluszOther table section, ensuring a clean and professional appearance.
