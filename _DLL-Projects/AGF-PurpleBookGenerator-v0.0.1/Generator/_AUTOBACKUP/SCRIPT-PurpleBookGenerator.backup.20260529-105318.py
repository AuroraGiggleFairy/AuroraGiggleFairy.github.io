...
ARMOR_SETBONUS_TOOLTIP_KEY = "agfArmorSetBonusTooltip"
    ...
    try:
        with path.open("r", encoding="utf-8", newline="") as f:
            ...
            for row in reader:
                ...
                target_key = f"armor{armor_name}SetBonusDesc"
    ...
    try:
        with path.open("r", encoding="utf-8", newline="") as f:
            ...
            for row in reader:
                ...
                target_key = f"agf{armor_name}{piece_suffix}Description"
...
def derive_setbonus_rows_from_desc_rows(armor_setbonus_desc_rows: dict[str, str]) -> dict[str, str]:
    ...
    for key, line in armor_setbonus_desc_rows.items():
        m = re.match(r"^armor([A-Za-z0-9_]+)SetBonusDesc$", key)
        if not m:
            continue
        armor_name = m.group(1)
        try:
        ...
        out[f"armor{armor_name}SetBonus1"] = b1 or "none"
        out[f"armor{armor_name}SetBonus2"] = b2 or "none"
    return out
...
def _apply_compact_armor_template_to_armors_rect(armors_rect_xml: str) -> str:
    ...
    for grid in root.findall(".//grid"):
        ...
        setbonus1_key = f"armor{base}SetBonus1"
        setbonus2_key = f"armor{base}SetBonus2"
    ...
    if tabs_contents is not None:
        ...
        def _ensure_zoom_description_panel(tab_el: ET.Element, zoom_grid_el: ET.Element) -> None:
            ...
            base = gname[len("armor"):-len("Outfit")]
            rows = [
                (2, f"agf{base}HelmetDescription"),
                (3, f"agf{base}OutfitDescription"),
                (4, f"agf{base}GloveDescription"),
                (5, f"agf{base}BootsDescription"),
                (6, f"armor{base}SetBonusDesc"),
            ]
        ...
        for tab in list(tabs_contents):
            ...
            if tab.attrib.get("name", "") == "allArmors":
                continue
            if tab.attrib.get("tab_key", "") == "agfOverview":
                continue
...
def _collect_armors_setbonus_localization(armors_rect_xml: str) -> dict[str, str]:
    ...
    for grid in root.findall(".//grid"):
        ...
        if grid.attrib.get("rows") != "7" or grid.attrib.get("cols") != "4":
            continue
        base = gname[len("armor"):-len("Outfit")]
        key1 = f"armor{base}SetBonus1"
        key2 = f"armor{base}SetBonus2"
        ...
        if tabs_contents is not None:
            for content_tab in list(tabs_contents):
                ...
                if content_tab.attrib.get("controller") != "TabSelectorTab":
                    continue
                if content_tab.attrib.get("tab_key") == "lblAll":
                    content_tab.set("tab_key", "agfOverview")
                    break
...
def _merge_localization_text(
    ...
    if existing_text:
        ...
        if len(ex_map) >= 20 or "AGFarmorBikerOutfit" in ex_map or "checklistZoomArmor" in ex_map or "agfHeaderHint" in ex_map:
    ...
    force_generated_keys = {
        "agfUnlockArmorsHelmet",
        "agfUnlockArmorsPlating",
        "agfUnlockDrone",
        "agfUnlockToolsWeaponsClub",
        "agfUnlockToolsWeaponsMotorTool",
        "agfUnlockToolsWeaponsOtherGun",
        "agfUnlockToolsWeaponsOtherMelee",
        "agfUnlockToolsWeaponsShotgun",
        "agfArmorSetBonusTooltip",
    }
    ...
    for k, v in gen_map.items():
        if k.startswith("armor") and k.endswith("SetBonusDesc"):
            seed_map[k] = v
            if k not in seed_order:
                seed_order.append(k)
            continue
        if k.startswith("armor") and re.search(r"SetBonus[12]$", k):
            seed_map[k] = v
            if k not in seed_order:
                seed_order.append(k)
            continue
        if k.startswith("armor") and "SetBonus" in k:
...
def _validate_localization_preservation(existing_text: str, merged_text: str) -> list[str]:
    ...
    changed: list[str] = []
    for key in ex_order:
        if key.startswith("armor") and "SetBonus" in key:
            continue
...
def main() -> int:
    ...
    generated_loc_text = (
        ...
        'agfHeaderHint,,,,,"Hover for details. Use tabs to zoom in."\n'
        'agfArmorSetBonusTooltip,,,,,"Set Bonus"\n'
