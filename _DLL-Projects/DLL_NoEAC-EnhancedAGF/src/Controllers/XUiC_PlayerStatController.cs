...
public class XUiC_PlayerStats : XUiController
{
    ...
    public bool isDirty;
    private float nextChangePollTime;
    private EntityPlayer localPlayer;
    private List<Binding> activeBindings = new List<Binding>();
    private Dictionary<string, Binding> activeBindingLookup = new Dictionary<string, Binding>(StringComparer.OrdinalIgnoreCase);
...
// ...existing code...
	...
	public override void Update(float _dt)
	{
        ...
        bool shouldRefresh = isDirty;
        if (!shouldRefresh && now >= nextChangePollTime)
        {
            shouldRefresh = HasChanged();
            nextChangePollTime = now + ChangePollIntervalSeconds;
        }
    ...
    public override bool GetBindingValueInternal(ref string value, string bindingName)
    {
        if (bindingName != null)
        {
            ...
            {
                if (FalseWhenNoPlayerBindings.Contains(bindingName))
                {
                    value = "false";
                }
                else
    ...
    private bool HasChanged()
    {
        ...
        {
            if (!bindingsLastValue.TryGetValue(stat, out string lastValue))
            {
    ...
    private string GetBindingValue(string bindingName)
    {
        ...
        if (activeBindingLookup.TryGetValue(bindingName, out Binding stat))
        {
        ...
        if (stat == null)
        {
            ...
            activeBindingLookup[bindingName] = stat;
            if (!activeBindingLookup.ContainsKey(stat.Name))
            {
                activeBindingLookup[stat.Name] = stat;
            }

            if (!bindingsLastValue.ContainsKey(stat))
            {
