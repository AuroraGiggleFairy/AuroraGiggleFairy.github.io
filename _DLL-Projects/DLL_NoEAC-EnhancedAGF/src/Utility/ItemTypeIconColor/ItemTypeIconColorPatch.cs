...
namespace ItemTypeIconColor
{
    public static class ItemTypeIconColorPatch
    {
        ...
        private static readonly object ParsedColorCacheLock = new object();
        private static readonly Dictionary<string, Color> ParsedColorCache = new Dictionary<string, Color>(StringComparer.Ordinal);
        ...
        public static Color ParseGameColor(string input)
        {
            ...
            input = input.Trim();
            if (input.Length == 0) return Color.white;

            Color cachedColor;
            lock (ParsedColorCacheLock)
            {
                if (ParsedColorCache.TryGetValue(input, out cachedColor))
                {
                    return cachedColor;
                }
            }
            ...
            if (input.StartsWith("[") && input.EndsWith("]"))
            {
                ...
                if (hex.Length == 6 || hex.Length == 8)
                {
                    ...
                    parsedColor = new Color32(r, g, b, a);
                    return CacheParsedColor(input, parsedColor);
                }
            }
            // R,G,B or R,G,B,A
            ...
            try { parsedColor = StringParsers.ParseHexColor(input); } catch { parsedColor = Color.white; }
            return CacheParsedColor(input, parsedColor);
        }

        private static Color CacheParsedColor(string key, Color color)
        {
            lock (ParsedColorCacheLock)
            {
                ParsedColorCache[key] = color;
            }
