    ...
    internal class StorageManager
    {
        ...
        private static void Distribute(TileEntity source, ITileEntityLootable sourceContainer, Vector3i sourcePos, TileEntity target, ITileEntityLootable targetContainer, Vector3i targetPos)
        {
            ...
            try
            {
                ...
                {
                    // Respect locked slots when sorting after transfer
                    PackedBoolArray lockedSlots = null;
                    if (targetContainer.HasSlotLocksSupport && targetContainer.SlotLocks != null)
                        lockedSlots = targetContainer.SlotLocks;
                    targetContainer.items = StackSortUtil.CombineAndSortStacks(targetContainer.items, 0, lockedSlots);
