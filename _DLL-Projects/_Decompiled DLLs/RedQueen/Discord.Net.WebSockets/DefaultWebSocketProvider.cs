using System;
using System.Net;

namespace Discord.Net.WebSockets;

internal static class DefaultWebSocketProvider
{
	public static readonly WebSocketProvider Instance = Create();

	public static WebSocketProvider Create(IWebProxy proxy = null)
	{
		return delegate
		{
			try
			{
				return new DefaultWebSocketClient(proxy);
			}
			catch (PlatformNotSupportedException inner)
			{
				throw new PlatformNotSupportedException("The default WebSocketProvider is not supported on this platform.", inner);
			}
		};
	}
}
