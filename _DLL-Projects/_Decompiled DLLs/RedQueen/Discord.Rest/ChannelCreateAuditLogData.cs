...
internal class ChannelCreateAuditLogData : IAuditLogData
{
	...
	internal static ChannelCreateAuditLogData Create(BaseDiscordClient discord, AuditLog log, AuditLogEntry entry)
	{
		...
		List<Overwrite> source = (from x in auditLogChange.NewValue.ToObject<Discord.API.Overwrite[]>(discord.ApiClient.Serializer)
			select new Overwrite(x.TargetId, x.TargetType, new OverwritePermissions(x.Allow, x.Deny))).ToList();
		ChannelType type = auditLogChange2.NewValue.ToObject<ChannelType>(discord.ApiClient.Serializer);
		string name = auditLogChange3.NewValue.ToObject<string>(discord.ApiClient.Serializer);
		int? rateLimit = auditLogChange4?.NewValue?.ToObject<int>(discord.ApiClient.Serializer);
		bool? nsfw = auditLogChange5?.NewValue?.ToObject<bool>(discord.ApiClient.Serializer);
		int? bitrate = auditLogChange6?.NewValue?.ToObject<int>(discord.ApiClient.Serializer);
		return new ChannelCreateAuditLogData(entry.TargetId.Value, name, type, rateLimit, nsfw, bitrate, source.ToReadOnlyCollection());
	}
}
