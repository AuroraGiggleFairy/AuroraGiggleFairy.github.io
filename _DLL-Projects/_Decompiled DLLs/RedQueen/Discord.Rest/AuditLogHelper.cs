...
{
	private static readonly Dictionary<ActionType, Func<BaseDiscordClient, AuditLog, AuditLogEntry, IAuditLogData>> CreateMapping = new Dictionary<ActionType, Func<BaseDiscordClient, AuditLog, AuditLogEntry, IAuditLogData>>
	{
	...
	public static IAuditLogData CreateData(BaseDiscordClient discord, AuditLog log, AuditLogEntry entry)
	{
		if (CreateMapping.TryGetValue(entry.Action, out var value))
		{
			return value(discord, log, entry);
		}
		return null;
	}
}
