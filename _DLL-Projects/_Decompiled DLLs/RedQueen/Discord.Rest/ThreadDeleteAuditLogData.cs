...
internal class ThreadDeleteAuditLogData : IAuditLogData
{
	...
	private ThreadDeleteAuditLogData(ulong id, string name, ThreadType type, bool archived, ThreadArchiveDuration autoArchiveDuration, bool locked, int? rateLimit)
	{
		ThreadId = id;
		ThreadName = name;
		ThreadType = type;
	...
	internal static ThreadDeleteAuditLogData Create(BaseDiscordClient discord, AuditLog log, AuditLogEntry entry)
	{
		AuditLogChange[] changes = entry.Changes;
		ulong id = entry.TargetId.Value;
		log.Threads.FirstOrDefault((Channel x) => x.Id == id);
