...
public class EntityMoveHelper
{
	...
	public void UpdateMoveHelper()
	{
		...
		float num4;
		float _desiredyaw = (num4 = MathUtils.NormalizeAxis(entity.rotation.y));
		float _yawOffset = 0f;
		float num5 = 1f;
		if (IsActive)
		{
			...
			else
			{
				...
				bool flag3 = jumping && !entity.isSwimming;
				...
				bool flag4 = entity.IsInElevator();
				isClimb = false;
				if (flag4 && entity.bCanClimbLadders && num8 < 0.10890001f && num9 > 0.1f && !jumping)
				{
					isClimb = true;
				}
				else if (num8 <= 0.0009f && num9 > -0.25f && num9 < Utils.FastMax(0.25f, ccHeight - 0.3f) && !isTempMove)
				{
					StopMove();
					goto IL_0d91;
				}
				...
				{
					entity.SetMoveForwardWithModifiers(moveSpeed, 1f, 0f, _climb: false);
					ResetStuckCheck();
					ClearTempMove();
					ClearBlocked();
					return;
				}
				if ((!flag2 && !isDigging && !avatarController.IsAnimationWithMotionRunning()) || entity.sleepingOrWakingUp || !entity.bodyDamage.CurrentStun.CanMove() || entity.emodel.IsRagdollActive)
				{
					entity.SetMoveForward(0f);
					ResetStuckCheck();
					ClearBlocked();
				...
				{
					num = moveToPos.x - position.x;
					num2 = moveToPos.z - position.z;
					num3 = moveToPos.y - (position.y + 0.05f);
					float num10 = num * num + num2 * num2;
					if (AllowZombieDigging && num3 < -1.1f && num10 <= 0.010000001f && !flag3 && entity.onGround)
					{
						DigStart(20);
					}
					if (isDigging)
					...
					else
					{
						...
						{
							moveToDir = Mathf.MoveTowardsAngle(moveToDir, num4, 13f);
						}
						entity.emodel.ClearLookAt();
						if (hasNextPos || num10 >= 0.0225f)
						{
							if (flag3)
							{
								_desiredyaw = jumpYaw;
							...
							else
							{
								float num11 = num;
								float num12 = num2;
								if (hasNextPos && num10 <= 2.25f)
								{
									float t = Mathf.Sqrt(num10) / 1.5f;
								...
								{
									_desiredyaw = Mathf.Atan2(num11, num12) * 57.29578f;
								}
							}
						}
						float num13 = Utils.FastAbs(Utils.DeltaAngle(num4, moveToDir));
						...
						if (_yawOffset == 0f)
						{
							...
							if (num14 > 0f)
							{
								num5 *= 1f - Utils.FastMin(num14 / 30f, 0.8f);
							}
						}
						if (num5 > 0.5f)
						{
						...
						if (!hasNextPos && !isTempMove && !jumping && num8 < 0.36f && num5 > 0.1f)
						{
							float num15 = num5 * Mathf.Sqrt(num8) / 0.6f;
							if (num15 < 0.1f)
							{
								num15 = 0.1f;
							}
						...
						if (num5 > 0f)
						{
							...
							if (flag4)
							{
								Vector3 normalized = new Vector3(num6, num9, num7).normalized;
								float num16 = Mathf.Pow(moveSpeed, 0.4f);
								if (num9 > 0.1f)
								{
									num16 *= 0.7f;
								}
						...
						else
						{
							...
							if (isBreakingBlocks || num13 > 60f || num5 == 0f)
							{
								moveToTicks = 0;
							}
							...
							{
								moveToTicks = 0;
								float num17 = Mathf.Sqrt(num6 * num6 + num9 * num9 + num7 * num7);
								float num18 = moveToDistance - num17;
								if (num18 < 0.021f)
								{
									if (num18 < -0.01f)
									{
										moveToDistance = num17;
									}
									if (++moveToFailCnt >= 3 && !AIDirector.debugFreezePos)
									{
										bool flag5 = AllowZombieDigging && num3 < -1.1f && num10 <= 0.64000005f;
										if (flag5 && entity.onGround && random.RandomFloat < 0.6f)
										{
											DigStart(80);
											flag = false;
										}
							...
							if (!entity.onGround && !entity.isSwimming && !flag4 && !isClimb && (num3 < -0.5f || num3 > 0.5f))
							{
								BlockedTime = 0f;
								BlockedEntity = null;
							}
							else if (--obstacleCheckTickDelay <= 0)
							{
								...
								else if (num13 < 10f)
								{
									...
									else if ((BlockedFlags > 0 || !hasNextPos) && AllowZombieDigging && num3 < -1.5f && num10 >= 2.25f && entity.onGround)
									{
										float num19 = Mathf.Sqrt(num10 + num3 * num3) + 0.001f;
										if (num3 / num19 < -0.86f)
										{
											DigStart(160);
										}
							...
							if (entity.CanEntityJump() && !isClimb && !flag2)
							{
								float num20 = 0f;
								float heightDiff = 0.9f;
								if (BlockedTime > 0.1f && BlockedFlags == 1)
								{
									num20 = 0.5f + random.RandomFloat * 0.3f;
								}
								else if (num3 > 0.9f && num10 <= 0.16000001f && random.RandomFloat < 0.1f)
								{
									num20 = 0.05f + random.RandomFloat * 0.2f;
									heightDiff = 1f;
								}
								...
								if (num20 > 0f)
								{
									if (!CheckJumpBlocked(position, num10))
									{
										StartJump(calcYaw: true, num20, heightDiff);
		...
		if (flag)
		{
			entity.CalcStrafeYawOffset(num, num2, ref _desiredyaw, ref _yawOffset);
		}
		if (IsActive)
		{
			entity.SetMoveForwardWithModifiers(moveSpeed, num5, _yawOffset, isClimb);
		}
		if (IsActive || !entity.IsSeekYaw())
		{
			entity.SeekYaw(_desiredyaw, 0f, 30f);
		}
	}
	...
	public void CheckForDoorAndOpen()
	{
		...
		Block block = entity.world.GetBlock(blockPos).Block;
		if (block.HasTag(BlockTags.Door) && block is BlockCompositeTileEntity && entity.world.GetTileEntity(blockPos) is TileEntityComposite te && te.TryGetSelfOrFeature<TEFeatureDoor>(out var _typedTe) && !_typedTe.IsOpen())
		{
			bool flag = false;
			if (te.TryGetSelfOrFeature<TEFeatureLockable>(out var _typedTe2))
			{
				flag = _typedTe2.IsLocked();
			}
			if (!flag)
			{
				_typedTe.SetOpen(_open: true, _animate: true);
			}
		}
	}
	...
	public bool CheckJumpBlocked(Vector3 position, float moveToDistXZSq)
	{
		...
		bool flag = block.Block.IsMovementBlocked(entity.world, vector3i, block, BlockFace.None);
		if (flag)
		{
			...
			{
				Vector3 normalized = (moveToPos - position).normalized;
				vector = Vector3.Slerp(vector, normalized, 0.28f);
			}
			origin.y += 0.5f;
			float num = ccRadius - 0.05f;
			float maxDistance = entity.physicsBaseHeight + 0.88f - 0.5f - num;
			flag = Physics.SphereCast(origin, num, vector, out var _, maxDistance, 1082195968);
		}
		return flag;
	}
	...
	public void Push(EntityAlive blockerEntity)
	{
		...
		blockerEntity.DoRagdoll(in damageResponse);
	}
	...
	public WorldRayHitInfo GetAttackHitInfo(out float damageMpy)
	{
		if ((bool)BlockedEntity)
		{
			...
			BlockedEntity.DoRagdoll(in damageResponse);
		}
		damageMpy = 0f;
		return null;
	}
	...
	public void CalcIfUnreachablePos()
	{
		...
		if (num4 > ccHeight + 0.8f && num3 < 25f)
		{
				...
				do
				{
					...
					if (chunk != null)
					{
						...
						for (int k = -2; k <= 2; k++)
						{
							...
							if (num12 < num && (num4 == 0 || num10 < 2) && IsABlockSideOpen(vector3i3, ref chunk))
							{
								num4 += num11;
								num = num12;
								vector3i = vector3i3;
