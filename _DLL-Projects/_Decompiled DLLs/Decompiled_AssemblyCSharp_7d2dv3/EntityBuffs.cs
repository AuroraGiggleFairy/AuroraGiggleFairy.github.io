...
public class EntityBuffs
{
	...
	public void RemoveBuff(string _name, int _instigatorId = -1, bool _netSync = true)
	{
		...
		if (flag && _netSync)
		{
			int instigatorId = ((_instigatorId != -1) ? _instigatorId : parent.entityId);
			RemoveBuffNetwork(_name, instigatorId);
		}
	}
	...
	{
		NetPackageAddRemoveBuff package = NetPackageManager.GetPackage<NetPackageAddRemoveBuff>().Setup(parent.entityId, _name, -1f, _adding: false, _instigatorId, Vector3i.zero);
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendPackage(package, _onlyClientsAttachedToAnEntity: false, -1, -1, parent.entityId);
		}
		...
		for (int i = 0; i < ActiveBuffs.Count; i++)
		{
			...
			if (!_damageTypeTags.Test_AnySet(physicalDamageTypes))
			{
				if (parent.Buffs.GetCustomVar("ETrapHit") == 1f)
				{
					...
					if (value > 0f)
					{
						entityPlayerLocal.AddKillXP(parent, _itemThatKilledMe, value);
						parent.AwardKill(entityPlayerLocal);
					}
				}
				else
				{
					entityPlayerLocal.AddKillXP(parent, _itemThatKilledMe);
					parent.AwardKill(entityPlayerLocal);
				}
			}
	...
	public void RemoveBuffsByTag(FastTags<TagGroup.Global> tags)
	{
		for (int i = 0; i < ActiveBuffs.Count; i++)
		{
			...
			{
				buffValue.Remove = true;
				RemoveBuffNetwork(buffValue.BuffName, parent.entityId);
			}
		}
	}
