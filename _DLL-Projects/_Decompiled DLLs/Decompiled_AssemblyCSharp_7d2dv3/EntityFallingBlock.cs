...
public class EntityFallingBlock : Entity
{
	...
	public override void InitLocation(Vector3 _pos, Vector3 _rot)
	{
		...
		{
			fallingBlocksByChunk[chunkKey] = new List<EntityFallingBlock>();
		}
	...
	public void CreateMesh()
	{
		...
		if (pendingCanvasState != null)
		{
			signCanvas = transform2.GetComponentInChildren<SignCanvas>(includeInactive: true);
			if (signCanvas != null)
			{
				signCanvas.State = pendingCanvasState;
				signCanvas.Initialize();
			}
		}
	}
	...
	public static void ClearFallingBlocksForChunks(HashSetLong chunks)
	{
		...
		foreach (long chunk in chunks)
		{
			...
			foreach (EntityFallingBlock item in fallingBlocksByChunk[chunk])
			{
				list.Add(item);
			}
		}
		foreach (EntityFallingBlock item2 in list)
		{
			item2.SetDead();
		}
	}
