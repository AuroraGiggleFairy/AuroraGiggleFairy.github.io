...
public class WaypointCollection
{
	...
	public void UpdateEntityAnimalWayPoint(EntityAnimal _animal, bool unloaded = false)
	{
		Waypoint waypoint = null;
		foreach (Waypoint item in Collection.list)
		{
			if (item.lastKnownPositionEntityId == _animal.entityId)
			{
				waypoint = item;
				break;
			}
		...
		if (waypoint != null)
		{
			Vector3i vector3i = Vector3i.FromVector3Rounded(_animal.position);
			if (waypoint.pos != vector3i)
			{
				Collection.Remove(waypoint);
				waypoint.pos = vector3i;
				waypoint.navObject.TrackedPosition = _animal.position;
				Collection.Add(waypoint);
			}
			waypoint.navObject.hiddenOnCompass = !unloaded;
			SetWaypointHiddenOnMap(_animal.entityId, !unloaded);
		}
		else
		{
			((XUiC_MapArea)GameManager.Instance.World.GetPrimaryPlayer().PlayerUI.xui.GetWindow("mapArea").Controller).RefreshAnimalPositionWaypoint(_animal, _unloaded: false);
		}
	}
