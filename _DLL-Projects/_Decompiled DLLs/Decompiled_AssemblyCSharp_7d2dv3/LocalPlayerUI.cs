...
public class LocalPlayerUI : MonoBehaviour
{
	...
	public static LocalPlayerUI GetUIForPlayer(EntityPlayerLocal _entityPlayer)
	{
		...
		LocalPlayerUI result = null;
		for (int i = 0; i < playerUIs.Count; i++)
		{
			LocalPlayerUI localPlayerUI = playerUIs[i];
			if (localPlayerUI.entityPlayer == _entityPlayer)
			{
				return localPlayerUI;
			}
			if (!localPlayerUI.isPrimaryUI && localPlayerUI.entityPlayer == null)
			{
				result = localPlayerUI;
			}
		}
		while (playerUIQueueForPendingEntity.Count > 0)
		{
			LocalPlayerUI localPlayerUI2 = playerUIQueueForPendingEntity.Dequeue();
			if (!(localPlayerUI2 == null))
			{
				localPlayerUI2.mEntityPlayer = _entityPlayer;
				return localPlayerUI2;
			}
		}
		return result;
	}
	...
	public static LocalPlayerUI CreateUIForNewLocalPlayer()
	{
		...
		XUi.Instantiate(component2).Load(windowGroupSubset, async);
		return component2;
	}
	...
	public void Start()
	{
		...
		{
			XUi.SetXmlsForUi(_menuUi: true);
			XUi.Instantiate(this).Load(null, _async: true);
			SetupMenuSoftCursor();
		}
	}
	...
	public void Update()
	{
		...
		if (GUIWindowConsole.IsOpen() || (!isPrimaryUI && primaryUI.windowManager.IsModalWindowOpen()))
		{
			return;
		}
		...
		if (!flag6)
		{
			Vector2 vector2 = (flag2 ? new Vector2(navigationTarget.ColliderCenter.x, -Screen.height) : (flag3 ? new Vector2(navigationTarget.ColliderCenter.x, Screen.height) : ((!flag4) ? new Vector2(-Screen.width, navigationTarget.ColliderCenter.y) : new Vector2(Screen.width, navigationTarget.ColliderCenter.y))));
			foreach (XUiView navView2 in navViews)
			{
				if (flag2)
				{
					if (navView2.ColliderCenter.y > navigationTarget.ColliderCenter.y)
					{
						if (!(Vector3.Angle(navigationTarget.UiTransform.up, (navView2.ColliderCenter - navigationTarget.ColliderCenter).normalized) > 78f))
						{
							if (Mathf.Abs(navigationTarget.ColliderCenter.x - navView2.ColliderCenter.x) <= Mathf.Max(navigationTarget.ColliderWidthExtent, navView2.ColliderWidthExtent))
							{
					...
					else if (Mathf.Abs(navigationTarget.ColliderCenter.x - navView2.ColliderCenter.x) <= Mathf.Max(navigationTarget.ColliderWidthExtent, navView2.ColliderWidthExtent))
					{
				...
				{
					if (navView2.ColliderCenter.x < navigationTarget.ColliderCenter.x)
					{
						if (!(Vector3.Angle(-navigationTarget.UiTransform.right, (navView2.ColliderCenter - navigationTarget.ColliderCenter).normalized) > 78f))
						{
							if (Mathf.Abs(navigationTarget.ColliderCenter.y - navView2.ColliderCenter.y) <= Mathf.Max(navigationTarget.ColliderHeightExtent, navView2.ColliderHeightExtent))
							{
								navigationPrimeCandidates.Add(navView2);
							}
							else
					...
					else if (Mathf.Abs(navigationTarget.ColliderCenter.y - navView2.ColliderCenter.y) <= Mathf.Max(navigationTarget.ColliderHeightExtent, navView2.ColliderHeightExtent))
					{
						navigationWrapAroundPrimeCandidates.Add(navView2);
					}
					else
			...
			if (!flag6 && navigationPrimeCandidates.Count > 0)
			{
				...
				{
					float num2 = ((!(flag4 || flag5)) ? Mathf.Abs(navigationTarget.ColliderCenter.y - navigationPrimeCandidate.ColliderCenter.y) : Mathf.Abs(navigationTarget.ColliderCenter.x - navigationPrimeCandidate.ColliderCenter.x));
					if (num2 < num)
					{
						num = num2;
			...
			else if (!flag6 && navigationCandidates.Count > 0)
			{
				...
				{
					float sqrMagnitude2 = (navigationTarget.GetClosestPoint(navigationCandidate.ColliderCenter) - navigationCandidate.GetClosestPoint(navigationTarget.ColliderCenter)).sqrMagnitude;
			...
			else if (!flag6 && navigationWrapAroundPrimeCandidates.Count > 0)
			{
				...
				{
					float num3 = ((!(flag4 || flag5)) ? Mathf.Abs(vector2.y - navigationWrapAroundPrimeCandidate.ColliderCenter.y) : Mathf.Abs(vector2.x - navigationWrapAroundPrimeCandidate.ColliderCenter.x));
					if (num3 < num)
					{
						num = num3;
	...
	{
		activeItemStackGrids.Sort([PublicizedFrom(EAccessModifier.Internal)] (XUiC_ItemStackGrid x, XUiC_ItemStackGrid y) => (x.ViewComponent.ColliderCenter.y + x.ViewComponent.ColliderHeightExtent > y.ViewComponent.ColliderCenter.y + y.ViewComponent.ColliderHeightExtent) ? 1 : (-1));
	}
	...
	public bool TryItemStackGridNavigation()
	{
		...
		while (num != num2)
		{
			if (activeItemStackGrids[num].TryFindFirstNavigableView(out var _))
			{
				if (!xui.DragAndDropWindow.CurrentStack.Equals(ItemStack.Empty))
				{
