...
public class GameStats
{
	...
	{
		propertyList = new PropertyDecl[78]
		{
			...
			new PropertyDecl(EnumGameStats.JarRefund, _bPersistent: true, EnumType.Int, 60),
			new PropertyDecl(EnumGameStats.SandboxPreset, _bPersistent: true, EnumType.String, ""),
			new PropertyDecl(EnumGameStats.SandboxCode, _bPersistent: true, EnumType.String, ""),
			new PropertyDecl(EnumGameStats.DayNightLength, _bPersistent: true, EnumType.Int, 60),
			new PropertyDecl(EnumGameStats.BlockDamageAI, _bPersistent: true, EnumType.Int, 100),
			new PropertyDecl(EnumGameStats.BlockDamageAIBM, _bPersistent: true, EnumType.Int, 100),
			new PropertyDecl(EnumGameStats.LootAbundance, _bPersistent: true, EnumType.Int, 100),
			new PropertyDecl(EnumGameStats.LootRespawnDays, _bPersistent: true, EnumType.Int, 7),
			new PropertyDecl(EnumGameStats.GlobalGSModifier, _bPersistent: true, EnumType.Int, 100),
			new PropertyDecl(EnumGameStats.BiomeGSModifier, _bPersistent: true, EnumType.Int, 100),
			new PropertyDecl(EnumGameStats.GlobalLSModifier, _bPersistent: true, EnumType.Int, 100),
			new PropertyDecl(EnumGameStats.BiomeLSModifier, _bPersistent: true, EnumType.Int, 100)
		};
	}
	...
	public static float GetFloat(EnumGameStats _eProperty)
	{
		try
		{
			...
			{
				BaseSandboxOption baseSandboxOption = Instance.sandboxReferences[(int)_eProperty];
	...
	{
		Instance.sandboxReferences = new BaseSandboxOption[81];
		for (int i = 0; i < Instance.propertyList.Length; i++)
		{
			PropertyDecl propertyDecl = Instance.propertyList[i];
			global::SandboxOptions.SandboxOptions result = global::SandboxOptions.SandboxOptions.Max;
			if (Enum.TryParse<global::SandboxOptions.SandboxOptions>(propertyDecl.name.ToString(), out result))
			{
				Instance.sandboxReferences[(int)propertyDecl.name] = SandboxOptionManager.GetOption(result);
				continue;
			}
			switch (propertyDecl.name)
			{
			case EnumGameStats.BlockDamagePlayer:
				Instance.sandboxReferences[(int)propertyDecl.name] = SandboxOptionManager.GetOption(global::SandboxOptions.SandboxOptions.BlockDamage);
				break;
			case EnumGameStats.LootAbundance:
				Instance.sandboxReferences[(int)propertyDecl.name] = SandboxOptionManager.GetOption(global::SandboxOptions.SandboxOptions.GlobalLootCount);
				break;
			}
		}
	}
