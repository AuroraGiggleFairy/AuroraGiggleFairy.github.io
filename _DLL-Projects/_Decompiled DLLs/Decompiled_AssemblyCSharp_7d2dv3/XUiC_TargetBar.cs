...
public class XUiC_TargetBar : XUiController
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public float checkSandboxOption;

	[PublicizedFrom(EAccessModifier.Private)]
	public bool sandboxForceShow;
	...
	public override void Update(float _dt)
	{
		base.Update(_dt);
		deltaTime = _dt;
		checkSandboxOption -= deltaTime;
		if (checkSandboxOption <= 0f)
		{
			checkSandboxOption = 3f;
			sandboxForceShow = SandboxOptionManager.GetBool(global::SandboxOptions.SandboxOptions.ShowHealthBars);
		}
		if (gameEventManager == null)
		{
			gameEventManager = GameEventManager.Current;
		}
		if (!xui.playerUI.entityPlayer.IsAlive())
		{
			viewComponent.IsVisible = false;
			return;
		}
		...
		else
		{
			...
			if (!sandboxForceShow)
			{
				...
				if (visibility == EVisibility.GodMode && !xui.playerUI.entityPlayer.IsGodMode.Value)
				{
					viewComponent.IsVisible = false;
					return;
				}
			...
			bool flag2 = false;
			WorldRayHitInfo hitInfo = xui.playerUI.entityPlayer.HitInfo;
