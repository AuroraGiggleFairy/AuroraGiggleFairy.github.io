...
public class XUiC_CraftingInfoWindow : XUiC_InfoWindow
{
	...
	{
		if (selectedCraftingTier < XUiM_Recipes.CraftingMaxTier)
		{
			selectedCraftingTier++;
			IsDirty = true;
		}
	...
	public override void OnOpen()
	{
		...
		{
			xui.PlayerInventory.OnBackpackItemsChanged += PlayerInventory_OnItemsChanged;
			xui.PlayerInventory.OnToolbeltItemsChanged += PlayerInventory_OnItemsChanged;
		}
	}
	...
	public override void Update(float _dt)
	{
		...
		{
			itemInfoWindow = (XUiC_ItemInfoWindow)xui.FindWindowGroupByName("backpack").GetChildById("itemInfoPanel");
		}
		...
		if (flag)
		{
			int num = recipe.GetCraftingTier(xui.playerUI.entityPlayer);
			if (num > XUiM_Recipes.CraftingMaxTier)
			{
				num = XUiM_Recipes.CraftingMaxTier;
			}
			if (recipe != lastRecipeSelected || (recipe == lastRecipeSelected && num != selectedMaxCraftingTier))
			{
				selectedCraftingTier = num;
			}
			selectedMaxCraftingTier = Mathf.Min(num, XUiM_Recipes.CraftingMaxTier);
		}
	...
	public override bool GetBindingValueInternal(ref string value, string bindingName)
	{
		...
		case "craftingtime":
			...
			if (recipe != null)
			{
				float recipeCraftTime = XUiM_Recipes.GetRecipeCraftTime(xui, recipe);
		...
		case "itemtypeicon":
			...
			if (recipe != null)
			{
				...
				if (forId5 != null)
				{
					...
					{
						if (forId5.AltItemTypeIcon != null && forId5.Unlocks != "" && XUiM_ItemStack.CheckKnown(xui.playerUI.entityPlayer, forId5))
						{
		...
		case "itemtypeicontint":
			...
			if (recipe != null)
			{
				...
				if (forId != null && forId.Unlocks != "" && XUiM_ItemStack.CheckKnown(xui.playerUI.entityPlayer, forId))
				{
		...
		case "showunlockedbytab":
			value = "false";
			if (recipe != null && !XUiM_Recipes.GetRecipeIsUnlocked(xui, recipe))
			{
		...
		case "enableaddquality":
			if (recipe != null && recipe.GetOutputItemClass().ShowQualityBar && selectedCraftingTier < selectedMaxCraftingTier)
			{
				value = "true";
			}
			else
	...
	public void SetRecipe(XUiC_RecipeEntry _recipeEntry)
	{
		...
		{
			if (XUiM_Recipes.GetRecipeIsUnlocked(xui, selectedEntry.Recipe))
			{
				TabType = TabTypes.Ingredients;
				SetSelectedButtonByType(TabType);
			}
	...
	public override bool ParseAttribute(string attribute, string value)
	{
		if (base.ParseAttribute(attribute, value))
		{
			return true;
		}
		switch (attribute)
