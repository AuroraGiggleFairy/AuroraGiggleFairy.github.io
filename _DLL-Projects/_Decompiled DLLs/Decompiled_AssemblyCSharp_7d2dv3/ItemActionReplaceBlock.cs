...
public class ItemActionReplaceBlock : ItemActionRanged
{
	...
	public IEnumerator fireShotLater(int _shotIdx, ItemActionDataRanged _actionData)
	{
		...
		if (bCopyBlock)
		{
			...
			if (_actionData.invData.actionData[index] != null)
			{
				...
				obj.Block = _bv;
				obj.PaintTextures = chunkCache.GetTextureFullArray(_blockPos);
				obj.Density = chunkCache.GetDensity(_blockPos);
				isHUDDirty = true;
			}
		}
		...
		else
		{
			...
			if (itemActionReplaceBlockData.ReplaceMode == EnumReplaceMode.SingleBlock)
			{
				...
				GameManager.Instance.SetBlocksRPC(new List<BlockChangeInfo> { replaceSingleBlock(chunkCache, _blockPos, itemActionReplaceBlockData) });
			...
			BlockToolSelection.Instance.BeginUndo();
			replace(chunkCache, itemActionReplaceBlockData, _bv, startPos, endPos);
			BlockToolSelection.Instance.EndUndo();
		}
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public BlockChangeInfo replaceSingleBlock(ChunkCluster _cc, Vector3i _blockPos, ItemActionReplaceBlockData _actionData)
	{
		BlockValue block = _cc.GetBlock(_blockPos);
		BlockChangeInfo blockChangeInfo = new BlockChangeInfo
		{
			blockValueRef = _blockPos,
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public void replace(ChunkCluster _cc, ItemActionReplaceBlockData _actionData, BlockValue _srcBlock, Vector3i _startPos, Vector3i _endPos)
	{
		...
		for (int i = _startPos.x; i <= _endPos.x; i++)
		{
			for (int j = _startPos.z; j <= _endPos.z; j++)
			{
				for (int k = _startPos.y; k <= _endPos.y; k++)
				{
					...
					{
						list.Add(replaceSingleBlock(_cc, vector3i, _actionData));
					}
				}
			}
		}
	...
	public bool GetHitBlock(ItemActionDataRanged _actionData, out Vector3i _blockPos, out BlockValue _bv, out WorldRayHitInfo _hitInfo)
	{
		...
		if (_bv.ischild)
		{
			...
			_bv = chunkCache.GetBlock(_blockPos);
		}
		return true;
	}
