...
public class XUiC_PlayersListEntry : XUiController
{
	...
	public AllyStore.AllyStatus AllyStatus
	{
		[PublicizedFrom(EAccessModifier.Private)]
		get
		{
			if (PlayerData != null)
			{
				return PlayersList.AllyCache.GetStatus(PlatformManager.InternalLocalUserIdentifier, PlayerData.PrimaryId);
			}
	...
	{
		if (PlayerData != null && !m_isLocalPlayer)
		{
			switch (AllyStatus)
			{
			...
			case AllyStore.AllyStatus.OutgoingInvite:
				xui.PopupMenuWindow.Setup(new Vector2i(0, -26), Localization.Get("xuiPlayerAllyActions")).AddItem(new XUiC_PopupMenuItem.Entry(Localization.Get("lblRemove"), "ui_game_symbol_x", _isEnabled: true, Array.Empty<object>(), RemoveAlly_ItemClicked)).Show();
				break;
			}
		}
	}
	...
	public void oniconPartyIconPress(XUiController _sender, int _mouseButton)
	{
		...
		case EnumPartyStatus.LocalPlayer_InParty:
			xui.PopupMenuWindow.Setup(new Vector2i(0, -26), Localization.Get("xuiPlayerPartyActions")).AddItem(new XUiC_PopupMenuItem.Entry(Localization.Get("lblLeave"), "ui_game_symbol_x", _isEnabled: true, Array.Empty<object>(), LeaveParty_ItemClicked)).Show();
			break;
		case EnumPartyStatus.LocalPlayer_InPartyAsLead:
			xui.PopupMenuWindow.Setup(new Vector2i(0, -26), Localization.Get("xuiPlayerPartyActions")).AddItem(new XUiC_PopupMenuItem.Entry(Localization.Get("lblLeave"), "ui_game_symbol_x", _isEnabled: true, Array.Empty<object>(), LeaveParty_ItemClicked)).Show();
			break;
		case EnumPartyStatus.OtherPlayer_InPartyAsLead:
			xui.PopupMenuWindow.Setup(new Vector2i(0, -26), Localization.Get("xuiPlayerPartyActions")).AddItem(new XUiC_PopupMenuItem.Entry(Localization.Get("lblKick"), "ui_game_symbol_x", _isEnabled: true, Array.Empty<object>(), KickParty_ItemClicked)).AddItem(new XUiC_PopupMenuItem.Entry(Localization.Get("lblMakeLeader"), "server_favorite", _isEnabled: true, Array.Empty<object>(), MakeLeader_ItemClicked))
				.Show();
			break;
