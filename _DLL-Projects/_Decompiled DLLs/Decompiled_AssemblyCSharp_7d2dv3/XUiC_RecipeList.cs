...
public class XUiC_RecipeList : XUiController
{
	...
	public void GetRecipeData()
	{
		...
		List<Recipe> list = ((xui.QuestTracker.TrackedChallenge != null) ? xui.QuestTracker.TrackedChallenge.CraftedRecipes() : null);
	...
	{
		if (IsDirty && xui.PlayerInventory != null)
		{
			FindShowingWindow();
			if (resortRecipes)
			{
				...
				list.Clear();
				list.AddRange(xui.PlayerInventory.GetBackpackItemStacks());
				list.AddRange(xui.PlayerInventory.GetToolbeltItemStacks());
				XUiC_WorkstationInputGrid childByType = windowGroup.Controller.GetChildByType<XUiC_WorkstationInputGrid>();
				if (childByType != null && (!XUiM_Recipes.DisableSmelter || !(childByType is XUiC_WorkstationMaterialInputGrid)))
				{
					list.Clear();
			...
			if (pageChanged)
			{
				for (int i = 0; i < length; i++)
				{
					...
					else
					{
						...
						if (xUiC_RecipeEntry.IsSelected)
						{
							xUiC_RecipeEntry.IsSelected = false;
						}
					}
		...
		base.Update(_dt);
		if (xui.playerUI.playerInput.GUIActions.Inspect.WasPressed && xui.playerUI.CursorController.navigationTarget != null)
		{
			OnPressRecipe(xui.playerUI.CursorController.navigationTarget.Controller, 0);
		}
	}
	...
	public override void OnOpen()
	{
		...
		xui.playerUI.entityPlayer.QuestChanged += QuestJournal_QuestChanged;
		xui.playerUI.entityPlayer.QuestRemoved += QuestJournal_QuestChanged;
		xui.QuestTracker.OnTrackedChallengeChanged += QuestTracker_OnTrackedChallengeChanged;
		xui.Recipes.OnTrackedRecipeChanged += Recipes_OnTrackedRecipeChanged;
		...
		ClearSelection();
		if (xui.playerUI.entityPlayer.QuestJournal.HasCraftingQuest() && (txtInput == null || txtInput.Text == ""))
		{
			GetRecipeData();
			pageChanged = true;
		}
	...
	public void BuildRecipeInfosList(List<ItemStack> _items)
	{
		...
		for (int i = 0; i < recipes.Count; i++)
		{
			...
			item.unlocked = XUiM_Recipes.GetRecipeIsUnlocked(xui, item.recipe);
