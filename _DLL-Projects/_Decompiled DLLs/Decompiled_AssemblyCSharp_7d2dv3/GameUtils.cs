...
public class GameUtils
{
	public class WorldInfo
	{
		...
		public static WorldInfo LoadWorldInfo(PathAbstractions.AbstractedLocation _worldLocation)
		{
			try
			{
				...
				{
					int.TryParse(dynamicProperties.Values["SchemaVersion"], out result);
				}
				foreach (XElement item in enumerable)
				{
					...
					else if (text2.StartsWith("Generation.", StringComparison.Ordinal))
					{
						...
						string key2 = text2.Substring(num + 1);
						if (!dynamicProperties.Classes.ContainsKey(key))
						{
							dynamicProperties.Classes[key] = new DynamicProperties();
						}
						dynamicProperties.Classes[key].Values[key2] = item.Attribute("value")?.Value;
					}
					else
					{
						dynamicProperties.Add(item);
	...
	public static bool FindMasterBlockForEntityModelBlock(World _world, Vector3 _dirNormalized, string _phsxTag, Vector3 _hitPointPos, Transform _hitTransform, WorldRayHitInfo _hitInfo)
	{
		...
		HitInfoDetails.PropData propData = HitInfoDetails.PropData.GetFrom(chunkCache, transform);
		_hitInfo.hit.propData = propData;
		_hitInfo.fmcHit.propData = propData;
		BlockValue bv;
		if (checkChunk(chunkCache, num3, num4, transform, vector, vector2, _hitInfo, out var _ebcd))
		{
			...
			{
				_hitInfo.lastBlockPos = Voxel.GoBackOnVoxels(chunkCache, new Ray(chunkCache.ToLocalPosition(_hitPointPos + vector2 * 0.01f), -vector2), out bv);
			}
			return true;
		}
		if (checkChunk(chunkCache, (num < 8) ? (num3 - 1) : (num3 + 1), num4, transform, vector, vector2, _hitInfo, out _ebcd))
		{
			_hitInfo.hit.pos = _hitPointPos;
			_hitInfo.lastBlockPos = World.worldToBlockPos(_hitInfo.hit.pos);
			if (!_ebcd.blockValue.Block.isMultiBlock || _ebcd.blockValue.Block.multiBlockPos.ContainsPos(_world, _ebcd.pos, _ebcd.blockValue, _hitInfo.lastBlockPos))
		...
		if (checkChunk(chunkCache, num3, (num2 < 8) ? (num4 - 1) : (num4 + 1), transform, vector, vector2, _hitInfo, out _ebcd))
		{
			_hitInfo.hit.pos = _hitPointPos;
			_hitInfo.lastBlockPos = World.worldToBlockPos(_hitInfo.hit.pos);
			if (!_ebcd.blockValue.Block.isMultiBlock || _ebcd.blockValue.Block.multiBlockPos.ContainsPos(_world, _ebcd.pos, _ebcd.blockValue, _hitInfo.lastBlockPos))
		...
		if (checkChunk(chunkCache, (num < 8) ? (num3 - 1) : (num3 + 1), (num2 < 8) ? (num4 - 1) : (num4 + 1), transform, vector, vector2, _hitInfo, out _ebcd))
		{
			_hitInfo.hit.pos = _hitPointPos;
			_hitInfo.lastBlockPos = World.worldToBlockPos(_hitInfo.hit.pos);
			if (!_ebcd.blockValue.Block.isMultiBlock || _ebcd.blockValue.Block.multiBlockPos.ContainsPos(_world, _ebcd.pos, _ebcd.blockValue, _hitInfo.lastBlockPos))
	...
	public static void CreateEmptyFlatLevel(string _worldName, PathAbstractions.AbstractedLocation _worldLocation, int _worldSize, int _terrainHeight = 60)
	{
		...
		CreateWorldFilesForFlatLevel(_worldName, _worldSize, _worldLocation, _terrainHeight);
	}
	...
	public static void DeleteWorld(PathAbstractions.AbstractedLocation _worldLocation)
	{
		if (!string.IsNullOrEmpty(_worldLocation.Name))
		{
			...
			string saveGameDir = GameIO.GetSaveGameDir(_worldLocation.Name, _worldLocation.StorageType);
	...
	{
		(int Days, int Hours, int Minutes) tuple = WorldTimeToElements(_worldTime);
		int item = tuple.Hours;
		int item2 = tuple.Minutes;
		return $"{item:D2}:{item2:D2}";
	}
	...
	public static (int duskHour, int dawnHour) CalcDuskDawnHours(int _dayLightLength)
	{
		...
		else
		{
			...
			{
				result.Item1 = 12 + _dayLightLength / 2;
			}
	...
	public static void HarvestOnAttack(ItemActionData _actionData, Dictionary<string, ItemActionAttack.Bonuses> ToolBonuses)
	{
		...
		if (_actionData.attackDetails.bKilled)
		{
			...
			else
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					float num = 1f;
					num = (itemValue.ItemClass.HasAnyTags(seedMultiTag) ? (num * XUiM_Recipes.SeedDropOutputModifier) : (block.HasAnyFastTags(cropTag) ? (num * XUiM_Recipes.CropOutputModifier) : ((!block.HasAnyFastTags(oreTag)) ? (num * XUiM_Recipes.HarvestingOutputModifier) : (num * XUiM_Recipes.MiningOutputModifier))));
					if (list[i].prob > 0.999f || random.RandomFloat <= list[i].prob)
					{
						int num2 = (int)((float)random.RandomRange(list[i].minCount, list[i].maxCount + 1) * originalValue);
						if (num2 > 0)
						{
							collectHarvestedItem(_actionData, itemValue, Mathf.Max(1, (int)((float)num2 * num)), 1f, _bScaleCountOnDamage: false);
						}
					}
				}
			}
		...
		if (_actionData.attackDetails.itemsToDrop.ContainsKey(EnumDropEvent.Harvest))
		{
			...
			for (int k = 0; k < list2.Count; k++)
			{
				...
				float num3 = 1f;
				num3 = (itemValue2.ItemClass.HasAnyTags(seedMultiTag) ? (num3 * XUiM_Recipes.SeedDropOutputModifier) : (block.HasAnyFastTags(cropTag) ? (num3 * XUiM_Recipes.CropOutputModifier) : ((!block.HasAnyFastTags(oreTag)) ? (num3 * XUiM_Recipes.HarvestingOutputModifier) : (num3 * XUiM_Recipes.MiningOutputModifier))));
				int num4 = (int)((float)random.RandomRange(list2[k].minCount, list2[k].maxCount + 1) * originalValue2);
				...
				if (num5 > 0)
				{
					collectHarvestedItem(_actionData, itemValue2, Mathf.Max((int)((float)num5 * num3), 1), list2[k].prob);
				}
				if (!_actionData.attackDetails.bKilled)
				{
					continue;
				...
				if (num5 > 0)
				{
					collectHarvestedItem(_actionData, itemValue2, Mathf.Max((int)((float)num5 * num3), 1), num6, _bScaleCountOnDamage: false);
				}
			}
		}
	...
	public static void StartPlaytesting(string _prefabName)
	{
		if (!string.IsNullOrEmpty(_prefabName))
		{
			...
			GameManager.Instance.Disconnect();
			ThreadManager.StartCoroutine(startPlaytestLater(_prefabName));
		}
	}
	...
	{
		StartSinglePrefabEditing([PublicizedFrom(EAccessModifier.Internal)] (NetworkConnectionError _error) =>
		{
			if (_error != NetworkConnectionError.NoError)
			{
				XUiC_MessageBoxWindowGroup.ShowNetworkError(_xui ?? LocalPlayerUI.primaryUI.xui, _error);
			}
		});
	}
	...
	public static void StartSinglePrefabEditing(Action<NetworkConnectionError> _callback = null)
	{
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			GameManager.bHideMainMenuNextTime = true;
			GameManager.Instance.Disconnect();
			ThreadManager.StartCoroutine(startSinglePrefabEditingLater(2f, _callback));
		}
		else
		{
			ThreadManager.StartCoroutine(startSinglePrefabEditingLater(0f, _callback));
		}
	}
	...
	public static IEnumerator startSinglePrefabEditingLater(float _delay, Action<NetworkConnectionError> _callback)
	{
		...
		NetworkConnectionError obj = SingletonMonoBehaviour<ConnectionManager>.Instance.StartServers(GamePrefs.GetString(EnumGamePrefs.ServerPassword), _offline: false);
