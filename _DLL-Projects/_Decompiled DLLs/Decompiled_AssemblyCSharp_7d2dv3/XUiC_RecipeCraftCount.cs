...
public class XUiC_RecipeCraftCount : XUiC_Counter
{
	...
	public int calcMaxCraftable()
	{
		...
		ItemStack[] array = ((childByType == null) ? xui.PlayerInventory.GetAllItemStacks().ToArray() : childByType.GetSlots());
		...
		int craftingTier = ((recipe.craftingTier == -1) ? recipe.GetCraftingTier(xui.playerUI.entityPlayer) : recipe.craftingTier);
		for (int j = 0; j < recipe.ingredients.Count; j++)
		{
			...
			{
				num3 = (int)EffectManager.GetValue(PassiveEffects.CraftingIngredientCount, null, num2, xui.playerUI.entityPlayer, recipe, FastTags<TagGroup.Global>.Parse(itemStack2.itemValue.ItemClass.GetItemName()), calcEquipment: true, calcHoldingItem: true, calcProgression: true, calcBuffs: true, calcChallenges: true, craftingTier);
				if (num3 > 0f)
				{
					num3 = (int)(num3 * XUiM_Recipes.GetCraftingInputModifier(recipe));
				}
			}
			else
			{
