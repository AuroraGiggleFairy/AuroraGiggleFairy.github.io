...
public class XUiC_MapStats : XUiController
{
	...
	public override bool GetBindingValueInternal(ref string value, string bindingName)
	{
		...
		case "mapdaytime":
			value = "";
			if (XUi.IsGameRunning() && xui.playerUI.entityPlayer != null)
			{
		...
		case "mapelevation":
			...
			if (XUi.IsGameRunning() && xui.playerUI.entityPlayer != null)
			{
				int v = Mathf.RoundToInt(xui.playerUI.entityPlayer.GetPosition().y - WeatherManager.SeaLevel());
		...
		case "playercoretemp":
			...
			if (XUi.IsGameRunning() && xui.playerUI.entityPlayer != null)
			{
				value = XUiM_Player.GetCoreTemp(xui.playerUI.entityPlayer);
			}
			return true;
		...
		case "showtime":
			...
			if (XUi.IsGameRunning() && xui.playerUI.entityPlayer != null)
			{
				value = (EffectManager.GetValue(PassiveEffects.NoTimeDisplay, null, 0f, xui.playerUI.entityPlayer) == 0f && SandboxOptionManager.GetBool(global::SandboxOptions.SandboxOptions.ShowDayTime)).ToString();
			}
			return true;
		case "maptradertime":
			value = TraderInfo.GetNextTraderTime();
			return true;
		case "maptradertimetitle":
			value = TraderInfo.GetNextTraderText();
			return true;
		default:
