...
public class XUiC_EquipmentStackGrid : XUiController
{
	...
	public virtual ItemValue[] GetSlots()
	{
		...
		{
			array[i] = equipment.GetSlotItem(i) ?? ItemValue.None;
		}
		return array;
	}
	...
	public virtual void SetStacks(ItemValue[] _stackList)
	{
		...
		XUiC_CharacterFrameWindow childByType2 = xui.GetChildByType<XUiC_CharacterFrameWindow>();
	...
	public void HandleSlotChangedEvent(int _slotNumber, ItemStack _stack)
	{
		...
		if (_stack.IsEmpty())
		{
			xui.PlayerEquipment.Equipment.SetSlotItem(_slotNumber, null);
			xui.PlayerEquipment.RefreshEquipment();
			return;
		}
		items[_slotNumber] = _stack.itemValue.Clone();
		xui.PlayerEquipment.Equipment.SetSlotItem(_slotNumber, _stack.itemValue);
		xui.PlayerEquipment.RefreshEquipment();
