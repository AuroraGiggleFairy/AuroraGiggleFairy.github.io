...
public class EntityFallingTree : Entity
{
	...
	public void CreateMesh()
	{
		...
		Vector3 vector = treeTransform.position;
		SetPosition(vector + Origin.position);
		SetRotation(treeTransform.eulerAngles);
		transform.SetPositionAndRotation(vector, treeTransform.rotation);
		...
		if (Physics.SphereCast(new Ray(transform.position + 3f * Vector3.up, Vector3.down), 0.25f, out var hitInfo, 5f, -538750989))
		{
	...
	public override void OnUpdateEntity()
	{
		...
		if (isMeshCreated && base.transform.position.y + Origin.position.y < 1f)
		{
			DestroyTree();
		}
	}
