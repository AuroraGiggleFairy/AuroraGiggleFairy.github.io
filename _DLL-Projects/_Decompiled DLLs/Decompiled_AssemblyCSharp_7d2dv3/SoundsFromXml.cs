...
public class SoundsFromXml
{
	...
	public static List<string> controllerVibrationAudioSourceExclusions;
	...
	{
		controllerVibrationAudioSourceExclusions = new List<string>();
		foreach (XElement item in root.Elements("ControllerVibrationAudioSourceExclusions"))
		{
			foreach (XElement item2 in item.Elements())
			{
				if (item2.Name == "Source")
				{
					controllerVibrationAudioSourceExclusions.Add(item2.GetAttribute("name"));
				}
			}
		}
		foreach (XElement item3 in root.Elements("SoundDataNode"))
		{
			Parse(item3);
		}
		controllerVibrationAudioSourceExclusions = null;
	}
	...
	public static void Parse(XElement node)
	{
		...
		foreach (XElement item in node.Elements())
		{
			...
			else if (localName.EqualsCaseInsensitive("prioritizeNewNodes"))
			{
				xmlData.prioritizeNewNodes = StringParsers.ParseBool(item.Attributes().First().Value);
			}
		...
		xmlData.vibratesController = xmlData.vibratesController && !controllerVibrationAudioSourceExclusions.Contains(text);
