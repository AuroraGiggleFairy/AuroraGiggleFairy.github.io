...
public class XUiC_ItemActionList : XUiController
{
	...
	{
		bool flag = xui.playerUI.entityPlayer.inventory.GetHoldingPrimary()?.IsActionRunning(xui.playerUI.entityPlayer.inventory.holdingItemData.actionData[0]) ?? false;
		...
		if (base.ViewComponent.IsActiveInHierarchy)
		{
			PlayerActionsGUI gUIActions = xui.playerUI.playerInput.GUIActions;
			if (PlatformManager.NativePlatform.Input.CurrentInputStyle != PlayerInputManager.InputStyle.Keyboard || !xui.playerUI.windowManager.IsInputActive())
			{
	...
	public void SetCraftingActionList(ItemActionListTypes _actionListType, XUiController itemController)
	{
		...
		case ItemActionListTypes.Crafting:
		{
			...
			if (flag)
			{
				if (XUiM_Recipes.GetRecipeIsUnlocked(xui, xUiC_RecipeEntry.Recipe))
				{
			...
			else if (!XUiM_Recipes.GetRecipeIsUnlocked(xui, xUiC_RecipeEntry.Recipe))
			{
				HandleUnlockedBy(itemController, xUiC_RecipeEntry);
			}
			if (xUiC_RecipeEntry.Recipe.IsTrackable && XUiM_Recipes.GetCraftingInputModifier(xUiC_RecipeEntry.Recipe) != 0f)
			{
		...
		case ItemActionListTypes.Part:
		{
			...
			if (xUiC_EquipmentStack == xui.AssembleItem.CurrentEquipmentStackController)
			{
			...
			if (!itemStack3.IsEmpty())
			{
				...
				if ((ItemAction.RepairType == ItemAction.RepairTypes.Both || ItemAction.RepairType == ItemAction.RepairTypes.RepairOnly) && itemValue2.MaxUseTimes > 1 && itemValue2.UseTimes > 0f && itemClass2.RepairTools != null && itemClass2.RepairTools.Length > 0 && itemClass2.RepairTools[0].Value.Length > 0)
				{
		...
		case ItemActionListTypes.Item:
		{
			...
			else
			{
				...
				if ((xUiC_ItemStack.StackLocation == XUiC_ItemStack.StackLocationTypes.Backpack || xUiC_ItemStack.StackLocation == XUiC_ItemStack.StackLocationTypes.ToolBelt) && xUiC_ItemStack == xui.AssembleItem.CurrentItemStackController)
				{
					AddActionListEntry(new ItemActionEntryAssemble(itemController));
					break;
				}
				...
				if (traderData != null)
				{
					...
					if (xui.Trader.Trader is TileEntityVendingMachine tileEntityVendingMachine2)
					{
						if (traderInfo.AllowSell || (!traderInfo.AllowSell && tileEntityVendingMachine2.LocalPlayerIsOwner()) || tileEntityVendingMachine2.IsUserAllowed(PlatformManager.InternalLocalUserIdentifier))
						{
							AddActionListEntry(new ItemActionEntrySell(itemController));
						}
					}
					else if (traderInfo.AllowSell)
					{
						AddActionListEntry(new ItemActionEntrySell(itemController));
					}
					break;
				}
				if (itemClass.CanEquip() && xui.AssembleItem.CurrentItem == null)
				{
				...
				if ((ItemAction.RepairType == ItemAction.RepairTypes.Both || ItemAction.RepairType == ItemAction.RepairTypes.RepairOnly) && itemValue.MaxUseTimes > 1 && itemValue.UseTimes > 0f && itemClass.RepairTools != null && itemClass.RepairTools.Length > 0 && itemClass.RepairTools[0].Value.Length > 0)
				{
					AddActionListEntry(new ItemActionEntryRepair(itemController));
				}
				...
				if (scrapableRecipe != null && XUiM_Recipes.ScrappingOutputModifier > 0f && scrapableRecipe.CanCraft(new ItemStack[1] { itemStack }))
				{
					AddActionListEntry(new ItemActionEntryScrap(itemController));
				}
				if ((xUiC_ItemStack.StackLocation == XUiC_ItemStack.StackLocationTypes.Backpack || xUiC_ItemStack.StackLocation == XUiC_ItemStack.StackLocationTypes.ToolBelt) && xui.AssembleItem.CurrentItemStackController == null && XUiM_Recipes.FilterRecipesByIngredient(itemStack, XUiM_Recipes.GetRecipes()).Count > 0)
				{
					AddActionListEntry(new ItemActionEntryRecipes(itemController));
				}
		...
		case ItemActionListTypes.Trader:
			if (xui.Trader.TraderData.TraderInfo.AllowBuy)
			{
				AddActionListEntry(new ItemActionEntryPurchase(itemController));
			}
			if (xui.Trader.Trader is TileEntityVendingMachine tileEntityVendingMachine && tileEntityVendingMachine.IsUserAllowed(PlatformManager.InternalLocalUserIdentifier))
			{
	...
	public void AddActionActions(ItemValue itemValue, ItemClass itemClass, XUiC_ItemStack stackController, ref bool equipFound)
	{
		for (int i = 0; i < itemClass.Actions.Length; i++)
		{
			...
			{
				ItemValue holdingItemItemValue = xui.playerUI.entityPlayer.inventory.holdingItemItemValue;
	...
	public void HandleUnlockedBy(XUiController itemController, XUiC_RecipeEntry entry)
	{
		...
		if (recipeUnlockData4 != null && recipeUnlockData4.ChallengeGroup.IsVisible(entityPlayer))
		{
			AddActionListEntry(new ItemActionEntryShowChallenge(itemController, recipeUnlockData4));
		}
		if (recipeUnlockData5 != null && recipeUnlockData5.Challenge.ChallengeGroup.IsVisible(entityPlayer))
		{
	...
	public void SetServiceActionList(InGameService service, XUiController itemController)
	{
		...
		{
			AddActionListEntry(new ServiceActionEntryRent(itemController, xui.Trader.Trader as TileEntityVendingMachine));
		}
		IsDirty = true;
	}
}
