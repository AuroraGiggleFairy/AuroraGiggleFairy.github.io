...
public class XUiC_CharacterCosmeticWindow : XUiController
{
	...
	[XuiBindComponent("btnClearAll", true)]
	[PublicizedFrom(EAccessModifier.Private)]
	public readonly XUiC_SimpleButton btnClearAll;
	...
	public override void Init()
	{
		...
		xui.playerUI.OnUIShutdown += HandleUIShutdown;
		xui.OnShutdown += HandleUIShutdown;
	}
	...
	public void PreviewFrame_OnPress(XUiController _sender, int _mouseButton)
	{
		...
		{
			ItemStack itemStack = xui.PlayerEquipment.EquipItem(xui.DragAndDropWindow.CurrentStack);
			if (xui.DragAndDropWindow.CurrentStack != itemStack)
			{
				xui.DragAndDropWindow.CurrentStack = itemStack;
	...
	public override void Update(float _dt)
	{
		...
		{
			PlayerActionsGUI playerActionsGUI = xui?.playerUI?.playerInput?.GUIActions;
			if (playerActionsGUI != null)
			{
				...
				xui.playerUI.CursorController.GetMouseButton(UICamera.MouseButton.LeftButton);
				bool flag = false;
				if (num)
				{
					flag = true;
			...
			bool mouseButton = xui.playerUI.CursorController.GetMouseButton(UICamera.MouseButton.LeftButton);
			bool mouseButton2 = xui.playerUI.CursorController.GetMouseButton(UICamera.MouseButton.RightButton);
	...
	public override void OnOpen()
	{
		...
		xui.CalloutWindow.EnableCallouts(XUiC_GamepadCalloutWindow.CalloutType.CharacterPreview);
	}
	...
	public override void OnClose()
	{
		base.OnClose();
		XUiM_PlayerEquipment.HandleRefreshEquipment -= XUiM_PlayerEquipment_HandleRefreshEquipment1;
		SDCSUtils.UnloadViz(previewSDCSObj);
		...
		xui.CalloutWindow.DisableCallouts(XUiC_GamepadCalloutWindow.CalloutType.CharacterPreview);
	}
	...
	public void BeginCursorLock()
	{
		if (!cursorLockActive)
		{
			...
			lockedCursorPos = xui.playerUI.CursorController.GetScreenPosition();
			prevCursorHidden = xui.playerUI.CursorController.VirtualCursorHidden;
			xui.playerUI.CursorController.VirtualCursorHidden = true;
		}
	}
	...
	{
		xui.playerUI.CursorController.SetScreenPosition(lockedCursorPos);
	}
	...
	public void EndCursorLock()
	{
		if (cursorLockActive)
		{
			...
			xui.playerUI.CursorController.VirtualCursorHidden = prevCursorHidden;
		}
	}

	[XuiBindEvent("OnPressed", "btnClearAll")]
	[PublicizedFrom(EAccessModifier.Private)]
	public void btnClearAll_OnPress(XUiController _sender, int _mouseButton)
	{
		Equipment equipment = xui.PlayerEquipment.Equipment;
		equipment.ClearTempCosmeticSlot();
		equipment.ClearCosmeticSlots();
		if (base.WindowGroup.Controller.TryGetChildController<XUiC_CharacterCosmeticsListWindow>("", out var _child))
		{
			_child.SetEmptyCosmeticForCurrentCategory();
		}
		((XUiC_CharacterCosmeticWindowGroup)base.WindowGroup.Controller).ResetPreview();
		if (xui.playerUI.entityPlayer.emodel is EModelSDCS eModelSDCS)
		{
