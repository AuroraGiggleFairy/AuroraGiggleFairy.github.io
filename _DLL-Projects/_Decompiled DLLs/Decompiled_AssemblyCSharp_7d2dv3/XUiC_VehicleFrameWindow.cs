...
public class XUiC_VehicleFrameWindow : XUiC_AssembleWindow
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public readonly CachedStringFormatter<int> fuelFormatter = new CachedStringFormatter<int>([PublicizedFrom(EAccessModifier.Internal)] (int _i) => _i + "%");
	...
	{
		if (XUiM_Vehicle.RepairVehicle(xui))
		{
			RefreshBindings();
			IsDirty = true;
	...
	{
		if ((!(Vehicle != null) || Vehicle.GetVehicle().HasEnginePart()) && xui.Vehicle.CurrentVehicle.AddFuelFromInventory(xui.playerUI.entityPlayer))
		{
			RefreshBindings();
		}
	}
	...
	public override bool GetBindingValueInternal(ref string value, string bindingName)
	{
		...
		case "fuelfill":
			...
			return true;
		case "showfuel":
			value = (Vehicle != null && Vehicle.GetVehicle().HasEnginePart() && EntityVehicle.VehicleFuelUsageModifier > 0f).ToString();
			return true;
		case "refueltext":
	...
	public override void Update(float _dt)
	{
		if (IsDirty)
		{
			btnRefuel_Background.Enabled = Vehicle != null && Vehicle.GetVehicle().HasEnginePart() && EntityVehicle.VehicleFuelUsageModifier > 0f;
		}
		base.Update(_dt);
	}
