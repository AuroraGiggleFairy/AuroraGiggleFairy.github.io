...
public class XUiC_ShapeStackGrid : XUiController
{
	...
	public void SetShapes(List<ShapeData> shapeIndexList, int newSelectedBlock = -1)
	{
		if (GameStats.GetBool(EnumGameStats.IsCreativeMenuEnabled))
		{
			...
			_ = 0;
		XUiC_ShapeInfoWindow childByType = xui.GetChildByType<XUiC_ShapeInfoWindow>();
		XUiC_ShapeMaterialInfoWindow childByType2 = xui.GetChildByType<XUiC_ShapeMaterialInfoWindow>();
		...
		for (int i = 0; i < Length; i++)
		{
			...
			if (num < count)
			{
				...
				{
					xUiC_ShapeStack.IsSelected = true;
				}
				if (xUiC_ShapeStack.IsSelected && xUiC_ShapeStack.BlockData != selectedBlock)
				{
					xUiC_ShapeStack.IsSelected = false;
				}
			}
			else
			{
		...
		if (selectedBlock == null && newSelectedBlock != -1)
		{
			for (int j = 0; j < shapeControllers.Length; j++)
			{
				...
				if (xUiC_ShapeStack2.BlockData != null && xUiC_ShapeStack2.ShapeIndex == newSelectedBlock)
				{
					...
					xUiC_ShapeStack2.IsSelected = true;
					return;
				}
			}
		}
