...
public class ParticleEffect
{
	...
	public string additionalHitSoundName;
	...
	public ParticleEffect(string _name, Vector3 _pos, Quaternion _rot, float _lightValue, Color _color, string _soundName, Transform _parentTransform, float _volumeScale = 1f, string _additionalHitSound = "")
		: this(_name, _pos, _lightValue, _color, _soundName, _parentTransform, _OLDCreateColliders: false)
	{
		rot = _rot;
		volumeScale = _volumeScale;
		additionalHitSoundName = _additionalHitSound;
	}

	public ParticleEffect(string _name, Vector3 _pos, float _lightValue, Color _color, string _soundName, Transform _parentTransform, bool _OLDCreateColliders, float _volumeScale = 1f, string _additionalHitSound = "")
	...
	public void Read(BinaryReader _br)
	{
		...
		additionalHitSoundName = _br.ReadString();
		if (additionalHitSoundName == string.Empty)
		{
			additionalHitSoundName = null;
		}
		volumeScale = _br.ReadSingle();
	...
	public void Write(BinaryWriter _bw)
	{
		...
		_bw.Write((additionalHitSoundName != null) ? additionalHitSoundName : string.Empty);
		_bw.Write(volumeScale);
	...
	{
		PlaySoundInServer(_entityThatCausedIt, _pe.pos, _pe.soundName, _pe.volumeScale);
		PlaySoundInServer(_entityThatCausedIt, _pe.pos, _pe.additionalHitSoundName, _pe.volumeScale);
		if (GameManager.IsDedicatedServer)
		{
			return null;
		}
		PlaySoundInClient(_entityThatCausedIt, _pe.pos, _pe.soundName, _pe.volumeScale);
		PlaySoundInClient(_entityThatCausedIt, _pe.pos, _pe.additionalHitSoundName, _pe.volumeScale);
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static bool PlaySoundInServer(int entityId, Vector3 position, string soundName, float volumeScale)
	{
		if (!string.IsNullOrEmpty(soundName) && SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			GameManager.Instance.World.aiDirector.OnSoundPlayedAtPosition(entityId, position, soundName, volumeScale);
			return true;
		}
		return false;
	}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static bool PlaySoundInClient(int entityId, Vector3 position, string soundName, float volumeScale)
	{
		if (!string.IsNullOrEmpty(soundName))
		{
			Manager.Play(position, soundName, entityId, wantHandle: false, volumeScale);
			return true;
		}
		return false;
	}
