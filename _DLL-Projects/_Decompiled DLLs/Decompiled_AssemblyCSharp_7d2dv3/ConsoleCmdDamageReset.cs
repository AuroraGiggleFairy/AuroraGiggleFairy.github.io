				...
				for (int i = 0; i < chunkArrayCopySync.Count; i++)
				{
					...
					chunk.LoopOverAllBlocks([PublicizedFrom(EAccessModifier.Internal)] (int _x, int _y, int _z, BlockValue _bv) =>
					{
						...
						if (damage > 0)
						{
							...
							bool flag = block is BlockCompositeTileEntity blockCompositeTileEntity && blockCompositeTileEntity.CompositeData.HasFeature<TEFeatureDoor>();
							if (!(ignoreDoors && flag))
							{
								Vector3i vector3i = chunkWorldPos + new Vector3i(_x, _y, _z);
								block.DamageBlock(world, vector3i, _bv, -damage, -1);
