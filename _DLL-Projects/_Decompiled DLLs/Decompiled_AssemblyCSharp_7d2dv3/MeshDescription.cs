...
public class MeshDescription
{
	...
	{
		if (material.HasTexture("_BumpMap") || material.HasTexture("_DetailNormalMap"))
		{
			SetKeyword(material, "_NORMALMAP", state: true);
		}
		else
		{
			SetKeyword(material, "_NORMALMAP", state: false);
		}
		if (material.HasTexture("_MetallicGlossMap"))
		{
			SetKeyword(material, "_METALLICGLOSSMAP", state: true);
		}
		else
		{
			SetKeyword(material, "_METALLICGLOSSMAP", state: false);
		}
