...
public class GameStartupHelper
{
	...
	public bool LoadConfigFile(string _filename)
	{
		...
		{
			if (dynamicProperties.Values[value.Key] == null)
			{
				ShowConfigError(value.Key, "Value not set");
				return false;
			}
			if (!ParsePref(value.Key, dynamicProperties.Values[value.Key]))
			{
				return false;
			}
		}
