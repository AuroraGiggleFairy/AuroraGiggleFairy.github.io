...
public class XUiC_IngredientEntry : XUiController
{
	...
	public override bool GetBindingValueInternal(ref string value, string bindingName)
	{
		...
		case "itemname":
		{
			...
			{
				text2 = ((!Localization.Exists("lbl" + material)) ? material.UppercaseFirst() : Localization.Get("lbl" + material));
			}
		...
		case "havecount":
		{
			...
			if (childByType3 != null)
			{
				...
				{
					value = (flag ? havecountFormatter.Format(xui.PlayerInventory.GetItemCount(ingredient.itemValue)) : "");
				}
			}
			else
			{
		...
		case "haveneedcount":
		{
			...
			if (childByType != null)
			{
				...
				{
					value = (flag ? (havecountFormatter.Format(xui.PlayerInventory.GetItemCount(ingredient.itemValue)) + "/" + text) : "");
				}
			}
			else
			{
