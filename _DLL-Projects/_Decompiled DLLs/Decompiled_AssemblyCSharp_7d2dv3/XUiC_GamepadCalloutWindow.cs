...
public class XUiC_GamepadCalloutWindow : XUiController
{
	...
	public class Callout : MonoBehaviour
	{
		...
		public void SetAtlas(INGUIAtlas _atlas)
		{
			if (iconSprite != null)
			{
	...
	{
		base.Init();
		xui.CalloutWindow = this;
		Callout.calloutFont = xui.GetUIFontByName("ReferenceFont");
		for (int i = 0; i < 17; i++)
		{
	...
	public void InitContextMenuCallouts()
	{
		...
		xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.LeftTrigger, "igcoCategoryLeft", CalloutType.MenuCategory);
		xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.RightTrigger, "igcoCategoryRight", CalloutType.MenuCategory);
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void InitRWGCallouts()
	{
		xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.RightTrigger, "igco_rwgCameraMode", CalloutType.RWGEditor);
		xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.LeftStick, "igco_moveCamera", CalloutType.RWGCamera);
		xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.RightStick, "igco_pivotCamera", CalloutType.RWGCamera);
		xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.LeftTrigger, "igco_rwgCameraSpeed", CalloutType.RWGCamera);
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void InitCharacterPreviewCallouts()
	{
		xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.RightStickLeftRight, "igco_rwgRotateCharacterPreview", CalloutType.CharacterPreview);
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void InitCameraZoomCallouts()
	{
		xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.RightTrigger, "igcoZoomInCamera", CalloutType.CameraZoom);
		xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.LeftTrigger, "igcoZoomOutCamera", CalloutType.CameraZoom);
	}
	...
	public override void Update(float _dt)
	{
		base.Update(_dt);
		bool flag = false;
		flag = xui.playerUI != null && xui.playerUI.playerInput != null && xui.playerUI.playerInput.Enabled;
	...
	{
		XUiC_DragAndDropWindow dragAndDropWindow = xui.DragAndDropWindow;
		ClearCallouts(CalloutType.MenuHoverItem);
		if (_isHovered)
		{
			...
			if (_itemStack.itemValue.ItemClass != null)
			{
				...
				else if (_itemStack.CanStackWith(dragAndDropWindow.CurrentStack))
				{
			...
			else if (!dragAndDropWindow.CurrentStack.IsEmpty())
			{
				ItemClass itemClass = dragAndDropWindow.CurrentStack.itemValue.ItemClass;
