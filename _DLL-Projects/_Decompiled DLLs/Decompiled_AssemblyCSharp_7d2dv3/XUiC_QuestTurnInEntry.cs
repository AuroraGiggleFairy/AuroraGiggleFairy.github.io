...
public class XUiC_QuestTurnInEntry : XUiC_SelectableEntry
{
	...
	public void RefreshBackground()
	{
		if (background != null)
		{
			background.Color = (base.IsSelected ? new Color32(byte.MaxValue, byte.MaxValue, byte.MaxValue, byte.MaxValue) : new Color32(64, 64, 64, byte.MaxValue));
			background.SpriteName = (base.IsSelected ? "ui_game_select_row" : "menu_empty");
		}
	}
	...
	{
		isHovered = _isOver;
		if (background != null && reward != null && !base.IsSelected)
		{
			if (_isOver)
			{
	...
	public override bool GetBindingValueInternal(ref string value, string bindingName)
	{
		...
		case "itemtypeicontint":
			...
			if (item != null && !item.IsEmpty() && item.itemValue.ItemClass.Unlocks != "" && XUiM_ItemStack.CheckKnown(xui.playerUI.entityPlayer, item.itemValue.ItemClass, item.itemValue))
			{
