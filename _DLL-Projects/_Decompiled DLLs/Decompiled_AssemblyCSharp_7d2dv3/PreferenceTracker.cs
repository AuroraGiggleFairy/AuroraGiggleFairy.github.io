...
public class PreferenceTracker
{
	...
	public PreferenceTracker Clone()
	{
		PreferenceTracker preferenceTracker = new PreferenceTracker(PlayerID);
		preferenceTracker.toolbelt = ((toolbelt != null) ? ItemStack.Clone(toolbelt) : null);
		preferenceTracker.bag = ((bag != null) ? ItemStack.Clone(bag) : null);
		if (equipment != null)
		{
			preferenceTracker.equipment = new ItemValue[equipment.Length];
			for (int i = 0; i < equipment.Length; i++)
			{
				preferenceTracker.equipment[i] = equipment[i].Clone();
			}
		}
