...
public class ChallengeClass
{
	...
	public List<ResourceRequiredTypes> RequiredResourceTypeList;
	...
	public void ParseElement(XElement e)
	{
		...
		string[] array = e.GetAttribute("harvest_required").Split(',');
		for (int i = 0; i < array.Length; i++)
		{
			ResourceRequiredTypes result = ResourceRequiredTypes.Resource;
			if (Enum.TryParse<ResourceRequiredTypes>(array[i], ignoreCase: true, out result))
			{
				if (RequiredResourceTypeList == null)
				{
					RequiredResourceTypeList = new List<ResourceRequiredTypes>();
				}
				RequiredResourceTypeList.Add(result);
			}
		}
	}
	...
	public bool HandleResourceRequirement()
	{
		...
		for (int i = 0; i < RequiredResourceTypeList.Count; i++)
		{
			switch (RequiredResourceTypeList[i])
			{
			case ResourceRequiredTypes.Resource:
				if (XUiM_Recipes.HarvestingOutputModifier == 0f)
				{
					return false;
				}
				break;
			case ResourceRequiredTypes.Crop:
				if (XUiM_Recipes.CropOutputModifier == 0f)
				{
					return false;
				}
				break;
			case ResourceRequiredTypes.Ore:
				if (XUiM_Recipes.MiningOutputModifier == 0f)
				{
					return false;
				}
				break;
			case ResourceRequiredTypes.Seed:
