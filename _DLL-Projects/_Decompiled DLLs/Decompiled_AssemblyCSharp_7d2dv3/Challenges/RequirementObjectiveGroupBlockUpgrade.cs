...
public class RequirementObjectiveGroupBlockUpgrade : BaseRequirementObjectiveGroup
{
	...
	public override bool HasPrerequisiteCondition()
	{
		...
		if (playerInventory.HasItem(new ItemStack(ItemClass.GetItem(NeededResourceID), NeededResourceCount)) && (playerInventory.HasItem(ItemClass.GetItem(ItemID)) || CheckDragDropItem(uIForPlayer.xui.DragAndDropWindow.CurrentStack)))
		{
	...
	public override bool CheckPhaseStatus(int index)
	{
		...
		if (ResourceRecipe == null || (ResourceRecipe != null && ResourceRecipe.ingredients.Count == 0))
		{
			...
			case 0:
				...
				{
					return !CheckResourceDragDropItem(uIForPlayer.xui.DragAndDropWindow.CurrentStack);
				}
				return false;
			case 1:
				if (!playerInventory.HasItem(new ItemStack(ItemClass.GetItem(NeededResourceID), NeededResourceCount)) || playerInventory.HasItem(ItemClass.GetItem(ItemID)) || CheckDragDropItem(uIForPlayer.xui.DragAndDropWindow.CurrentStack))
