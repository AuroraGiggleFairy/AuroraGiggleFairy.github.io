...
public class Challenge
{
	...
	public virtual void Read(BinaryReader _br)
	{
		...
		if (CurrentFileVersion >= 2)
		{
			AutoCompleted = _br.ReadBoolean();
		}
	...
	{
		if (!ChallengeClass.HandleResourceRequirement())
		{
			for (int i = 0; i < ObjectiveList.Count; i++)
			{
				ObjectiveList[i].HandleAutoComplete();
			}
		...
		if (IsActive)
		{
			...
			{
				ObjectiveList[j].HandleAddHooks();
			}
		}
	}
	...
	public Challenge Clone()
	{
		...
		challenge.AutoCompleted = AutoCompleted;
		if (RequirementObjectiveGroup != null)
		{
	...
	public void CompleteChallenge(bool forceRedeem = false, bool giveReward = true)
	{
