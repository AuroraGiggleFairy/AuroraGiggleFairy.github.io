...
public class ChallengeGroup
{
	...
	public bool IsActive
	{
		get
		{
			if (IsIntro && !ChallengeJournal.IntroChallengesEnabled)
			{
				return false;
			}
			return ChallengeCategory.s_ChallengeCategories[Category].IsActive;
		}
	}
	...
	public void ParseElement(XElement e)
	{
		...
		{
			IsIntro = StringParsers.ParseBool(e.GetAttribute("is_intro"));
		}
	}

	public bool IsVisible(EntityPlayer player)
	{
		if (!ChallengeJournal.IntroChallengesEnabled && IsIntro)
		{
			return false;
		}
		if (HiddenBy == "")
		...
		if (!ChallengeCategory.s_ChallengeCategories[Category].CanShow(player))
		{
			return false;
		}
		if (s_ChallengeGroups.ContainsKey(HiddenBy))
		{
			ChallengeGroup challengeGroup = s_ChallengeGroups[HiddenBy];
