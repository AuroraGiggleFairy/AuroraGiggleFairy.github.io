...
public class FloodFillEntityPathGenerator : RaycastEntityPathGenerator
{
	...
	public bool IsValidNeighbor(World world, FloodFillPath path, FloodFillNode current, FloodFillNode adjacent)
	{
		...
		{
			if (adjacent.nodeType == cPathNodeType.Door && RaycastPathUtils.IsPositionBlocked(current.Center, adjacent.Center, 65536) && (!(world.GetTileEntity(adjacent.BlockPos) is TileEntityComposite te) || !te.TryGetSelfOrFeature<TEFeatureDoor>(out var _typedTe) || !_typedTe.IsOpen()))
			{
				return false;
			}
			return true;
