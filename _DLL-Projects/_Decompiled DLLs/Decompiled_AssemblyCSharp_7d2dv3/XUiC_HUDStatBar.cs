...
public class XUiC_HUDStatBar : XUiController
{
	...
	public override void Init()
	{
		base.Init();
		IsDirty = true;
		activeAmmoItemValue = ItemValue.None;
	}
	...
	public override void Update(float _dt)
	{
		...
		if (windowManager.IsFullHUDDisabled() || (!xui.DragAndDropWindow.InMenu && windowManager.IsHUDPartialHidden()))
		{
			hudWasHidden = true;
			RefreshBindings();
			return;
		...
		if (statType == HUDStatTypes.Stamina)
		{
			...
			{
				bool flag = localPlayer.MovementRunning || localPlayer.MoveController.RunToggleActive;
				if (sprinting != flag)
				{
					sprinting = flag;
					IsDirty = true;
	...
	public override bool GetBindingValueInternal(ref string _value, string _bindingName)
	{
		...
		case "statcurrentwithmax":
			...
			{
				Stat currentStat = getCurrentStat();
				_value = statcurrentWMaxFormatterAOfB.Format(Mathf.RoundToInt(currentStat.Value), Mathf.RoundToInt(currentStat.Max));
				break;
			}
			case HUDStatTypes.Stealth:
		...
		case "statmodifiedmax":
		{
			...
			if (currentStat2 != null)
			{
				_value = statmodifiedmaxFormatter.Format(currentStat2.ModifiedMax, currentStat2.Max);
			}
			return true;
		}
		case "statregenrate":
		...
		case "statvisible":
			...
			if (xui.playerUI.windowManager.IsFullHUDDisabled() || (!xui.DragAndDropWindow.InMenu && xui.playerUI.windowManager.IsHUDPartialHidden()))
			{
				_value = "false";
				return true;
			}
			if (statGroup == HUDStatGroups.Vehicle)
			{
				...
				{
					_value = (vehicle != null && vehicle.GetVehicle().HasEnginePart() && EntityVehicle.VehicleFuelUsageModifier > 0f).ToString();
				}
				else
				{
			...
			else if (statType == HUDStatTypes.Stealth)
			{
				...
				{
					xui.BuffPopoutList.SetYOffset(52);
					_value = "true";
				}
				else
				{
					xui.BuffPopoutList.SetYOffset(0);
					_value = "false";
				}
			}
			return true;
	...
	public void setupActiveItemEntry()
	{
		...
		if (itemClass != null)
		{
			...
			if (itemActionAttack != null && itemActionAttack.IsEditingTool())
			{
				...
				xui.CollectedItemList.SetYOffset(46);
				return;
			}
			...
			{
				currentAmmoCount = 0;
				xui.CollectedItemList.SetYOffset((localPlayer.AttachedToEntity is EntityVehicle) ? 100 : 0);
				return;
			}
			attackAction = itemActionAttack;
