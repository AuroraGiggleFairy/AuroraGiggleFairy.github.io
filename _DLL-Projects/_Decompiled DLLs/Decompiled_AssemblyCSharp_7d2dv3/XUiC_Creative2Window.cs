...
public class XUiC_Creative2Window : XUiController
{
	...
	public override void Init()
	{
		...
		foreach (XUiC_CategoryList item in list)
		{
			...
			item.OnVisiblity += [PublicizedFrom(EAccessModifier.Private)] (XUiController _sender, bool _visibleSelf, bool _visibleInScene) =>
			{
				if (!isOpening && (_visibleInScene || currentSubCategory?.CategoryList == _sender))
				{
	...
	public void Refresh()
	{
		...
		filteredItems.Clear();
		ItemClass.GetItemsAndBlocks(filteredItems, -1, -1, currentFilterExcludeFuncs, nameFilter, showDevBlocks, currentCreativeMode, showFavorites, _sortBySortOrder: true, xui);
		length = creativeGrid.Length;
		Page = 0;
		IsDirty = true;
		ItemClass.CreateItemStacks(filteredItems, itemStacks, nameFilter);
