...
public class ItemActionEntryUse : BaseItemActionEntry
{
	...
	public override void OnActivated()
	{
		...
		else if (itemStack.itemValue.ItemClass.Actions[actionIdx] is ItemActionEat { UsePrompt: not false } itemActionEat)
		{
			...
			XUiC_MessageBoxWindowGroup.ShowOkCancel(base.ItemController.xui, Localization.Get(itemActionEat.PromptTitle), Localization.Get(itemActionEat.PromptDescription), UseItemWithAnimation, SwitchBack);
		}
		else
		{
		...
		IEnumerator UseItemWithAnimationCoroutine()
		{
			...
			stackControl.ItemStack = ((originalStack.count > 1) ? new ItemStack(originalStack.itemValue.Clone(), originalStack.count - 1) : ItemStack.Empty);
			...
			{
				base.ItemController.xui.PlayerInventory.AddItem(finalStack, _playCollectSound: false);
			}
			if (!finalStack.IsEmpty())
			{
