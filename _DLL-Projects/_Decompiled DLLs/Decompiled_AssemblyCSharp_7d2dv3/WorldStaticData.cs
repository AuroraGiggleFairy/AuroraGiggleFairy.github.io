...
public static class WorldStaticData
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static readonly XmlLoadInfo[] xmlsToLoad = new XmlLoadInfo[49]
	{
		...
		new XmlLoadInfo("entityclasses", _loadAtStartup: false, _sendToClients: true, EntityClassesFromXml.LoadMain, EntityClass.Cleanup),
		...
		new XmlLoadInfo("XUi_Common/templates", _loadAtStartup: false, _sendToClients: true, XUiFromXml.Load, null),
		new XmlLoadInfo("XUi_InGame/styles", _loadAtStartup: false, _sendToClients: true, XUiFromXml.Load, null),
		new XmlLoadInfo("XUi_InGame/templates", _loadAtStartup: false, _sendToClients: true, XUiFromXml.Load, null),
		new XmlLoadInfo("XUi_InGame/windows", _loadAtStartup: false, _sendToClients: true, XUiFromXml.Load, null),
		new XmlLoadInfo("XUi_InGame/xui", _loadAtStartup: false, _sendToClients: true, XUiFromXml.Load, null),
		...
		new XmlLoadInfo("signs", _loadAtStartup: false, _sendToClients: false, SignDataManager.LoadDefaultLibrary, null),
		new XmlLoadInfo("sandbox_overrides", _loadAtStartup: true, _sendToClients: true, SandboxOverridesFromXml.CreateOverrides, null)
	};
	...
	public static void InitSync(bool _bForce, bool _bDediServer, bool _cleanup = true, string[] _onlyXmls = null)
	{
		if (_cleanup)
		{
			Cleanup();
		...
		ThreadManager.RunCoroutineSync(Init(_bForce, _bDediServer, null, _onlyXmls));
	}

	public static IEnumerator Init(bool _bForce, bool _bDediServer, ProgressDelegate _progressDelegate, string[] _onlyXmls = null)
	{
		...
		yield return LoadAllXmlsCo(!_bForce, _progressDelegate, _onlyXmls);
		bInitDone = true;
		Log.Out($"WorldStaticData.Init() needed {(float)sw.ElapsedMilliseconds / 1000f:F3} s");
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static void ReloadItemsAppend(XmlFile _xmlFile)
	{
		ThreadManager.RunCoroutineSync(LoadItems(_xmlFile));
	}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static IEnumerator LoadItemsAppend(XmlFile _xmlFile)
	{
		yield return ItemClassesFromXml.CreateItems(_xmlFile);
		ItemData.Parser.idOffset = Block.ItemsStartHere;
	}
	...
	{
		if (!ThreadManager.IsMainThread() || !Application.isPlaying || !SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			yield break;
		}
		string outputFileName = GameIO.GetSaveGameDir() + "/ConfigsDump/" + _loadInfo.XmlName + ".xml";
		if (!GameIO.IsRoamingUserDataPath(outputFileName))
		{
			...
			_xmlFile.SerializeToFile(outputFileName);
		}
	}
	...
	public static IEnumerator LoadAllXmlsCo(bool _isStartup, ProgressDelegate _progressDelegate, string[] _onlyXmls = null)
	{
		...
		foreach (XmlLoadInfo xmlLoadInfo in array)
		{
			if (_onlyXmls != null && !_onlyXmls.ContainsCaseInsensitive(xmlLoadInfo.XmlName))
			{
				continue;
			}
			if (!xmlLoadInfo.XmlFileExists())
	...
	public static void UnloadMeshDescriptionTextureArrays(int _meshIndex)
	{
		meshDescCol.UnloadTextureArrays(_meshIndex);
	}
