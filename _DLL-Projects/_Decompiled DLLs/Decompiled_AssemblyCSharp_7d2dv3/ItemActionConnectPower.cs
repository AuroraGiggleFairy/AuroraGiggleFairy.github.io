...
public class ItemActionConnectPower : ItemAction
{
	...
	public override void OnHoldingUpdate(ItemActionData _actionData)
	{
		...
		if (connectPowerData.invData.itemValue.MaxUseTimes > 0 && connectPowerData.invData.itemValue.UseTimes >= (float)connectPowerData.invData.itemValue.MaxUseTimes)
		{
			...
			HandleJamSound(connectPowerData.invData.itemValue, item, localPlayer);
			return;
		}
		...
		{
			if (connectPowerData2.startPoint == hitInfo.hit.blockPos || !connectPowerData2.inRange || Vector3.Distance(connectPowerData.startPoint.ToVector3(), hitInfo.hit.blockPos.ToVector3()) > (float)maxWireLength)
			{
				return;
			}
	...
	{
		WorldRayHitInfo hitInfo = ((EntityPlayerLocal)_actionData.invData.holdingEntity).HitInfo;
		if (hitInfo.bHitValid && (_actionData as ConnectPowerData).isFriendly)
		{
			int num = (int)(Constants.cDigAndBuildDistance * Constants.cDigAndBuildDistance);
			if (hitInfo.hit.distanceSq <= (float)num)
			{
	...
	{
		WorldRayHitInfo hitInfo = ((EntityPlayerLocal)data.holdingEntity).HitInfo;
		Block block = data.world.GetBlock(hitInfo.hit.blockPos).Block;
		if (block is BlockPowered || block is BlockPowerSource)
		{
			Vector3i blockPos = hitInfo.hit.blockPos;
			ChunkCluster chunkCache = data.world.ChunkCache;
			if (chunkCache == null)
			{
				return null;
			}
			Chunk chunk = (Chunk)chunkCache.GetChunkSync(World.toChunkXZ(blockPos.x), blockPos.y, World.toChunkXZ(blockPos.z));
			if (chunk == null)
			{
				return null;
			}
	...
	public bool DisconnectWire(ConnectPowerData _actionData)
	{
		...
		if (!(_actionData.invData.world.GetChunkFromWorldPos(_actionData.startPoint) is Chunk))
		{
			return false;
		}
		if (_actionData.invData.world.GetTileEntity(_actionData.startPoint) is TileEntityPowered tileEntityPowered)
		{
			if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
			{
				SingletonMonoBehaviour<ConnectionManager>.Instance.SendPackage(NetPackageManager.GetPackage<NetPackageWireToolActions>().Setup(NetPackageWireToolActions.WireActions.RemoveWire, Vector3i.zero, _actionData.invData.holdingEntity.entityId));
	...
	public void DecreaseDurability(ConnectPowerData _actionData)
	{
		if (_actionData.invData.itemValue.MaxUseTimes > 0)
		{
			...
			{
				_actionData.invData.itemValue.UseTimes += EffectManager.GetValue(PassiveEffects.DegradationPerUse, _actionData.invData.itemValue, 1f, _actionData.invData.holdingEntity, null, _actionData.invData.itemValue.ItemClass.ItemTags) * ItemAction.ItemDegradationModifier;
				HandleItemBreak(_actionData);
			}
			else
			{
