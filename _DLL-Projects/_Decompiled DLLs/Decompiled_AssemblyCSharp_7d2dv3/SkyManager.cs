...
public class SkyManager : MonoBehaviour
{
	...
	public static bool isAllTimeDay = false;

	public static bool isAllTimeNight = false;
	...
	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public static readonly float[] sMoonPhases = new float[7] { 0.05f, 0.35f, 0.55f, 0.7f, 1.4f, 1.63f, 1.82f };

	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public static readonly float[] sMoonBrights = new float[7] { 1f, 0.65f, 0.45f, 0.25f, 0.4f, 0.6f, 0.9f };
	...
	public static bool WasBloodMoon = false;
	...
	public void UpdateSunMoonAngles()
	{
		...
		{
			bool flag = IsBloodMoonVisible();
			if (isAllTimeNight && !flag)
			{
				worldRotationTarget = 0.75f;
			}
			else if (isAllTimeDay && !flag)
			{
				worldRotationTarget = 0.25f;
			}
			else
			{
		...
		if (!GameManager.IsDedicatedServer && (bool)mainCamera)
		{
			if ((bool)moonSpriteT)
			{
				...
				Vector3 vector = moonLightRot * Vector3.forward * -45000f + transform.position;
				moonSpriteT.SetPositionAndRotation(vector, Quaternion.LookRotation(vector, Vector3.up));
	...
	public void Update()
	{
		...
		int num4 = 0;
		Color value = new Color(1f, 0.14f, 0.05f) * 1.5f;
		if (!num3)
		{
			num4 = (int)(dayCount + 5.5f) % 7;
			value = Color.Lerp(Color.white, moonLightColor, 0.2f);
		}
		float f = sMoonPhases[num4] * MathF.PI;
		Vector3 vector = new Vector3(0f - Mathf.Sin(f), 0f, Mathf.Cos(f));
		moonSpriteMat.SetVector("_LightDir", vector);
		moonSpriteMat.SetColor("_Color", value);
		moonBright = sMoonBrights[num4];
		float cloudThickness = WeatherManager.GetCloudThickness();
		if (cloudThickness > 45f && GamePrefs.GetFloat(EnumGamePrefs.OptionsGfxBrightness) < 0.49f)
		{
			moonBright *= Utils.FastLerp(1f, 0.15f, (cloudThickness - 45f) / 55f);
		}
		float value2 = Mathf.Max(b: ((Mathf.Pow(moonLightColor.grayscale, 0.45f) - 0.5f) * 0.5f + 0.5f) * moonBright, a: sunIntensity);
		...
		if (bUpdateShaders)
		{
			...
			cloudsSphereMtrl.SetFloat("_LightIntensity", value2);
			cloudsSphereMtrl.SetColor("_SkyColor", GetSkyColor());
			cloudsSphereMtrl.SetColor("_SunColor", sunLight.color);
			cloudsSphereMtrl.SetVector("_SunDir", sunDirV);
			cloudsSphereMtrl.SetVector("_SunMoonDir", sunMoonDirV);
			value.a = moonBright * 0.4f;
			cloudsSphereMtrl.SetColor("_MoonColor", value);
		}
