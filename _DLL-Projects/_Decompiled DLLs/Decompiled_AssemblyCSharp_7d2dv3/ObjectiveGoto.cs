	...
	public virtual Vector3 GetPosition(EntityNPC ownerNPC = null, EntityPlayer entityPlayer = null, List<Vector2> usedPOILocations = null, int entityIDforQuests = -1)
	{
		...
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			...
			if (prefabInstance == null)
			{
				...
				prefabInstance = GameManager.Instance.World.ChunkCache.ChunkProvider.GetDynamicPrefabDecorator().GetClosestPOIToWorldPos(fastTags, new Vector2(entityAlive.position.x, entityAlive.position.z), usedPOILocations, -1, !allowCurrentPoi, biomeFilterType, biomeFilter, questKey);
				if (prefabInstance == null)
				{
					prefabInstance = GameManager.Instance.World.ChunkCache.ChunkProvider.GetDynamicPrefabDecorator().GetClosestPOIToWorldPos(fastTags, new Vector2(entityAlive.position.x, entityAlive.position.z), usedPOILocations, -1, !allowCurrentPoi, BiomeFilterTypes.SameBiome, "", questKey);
				}
			}
			if (prefabInstance == null)
			{
