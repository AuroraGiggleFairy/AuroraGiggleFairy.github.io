...
public class VPEngine : VehiclePart
{
	...
	public void startEngine()
	{
		...
		{
			isRunning = true;
			if (vehicle.GetFuelLevel() > 0f || EntityVehicle.VehicleFuelUsageModifier == 0f)
			{
				playSound(properties.GetValue("sound_start"));
				gearIndex = 0;
				updateEngineSounds(0f);
			}
	...
	public void stopEngine(bool _outOfFuel = false)
	{
		if (isRunning)
		{
			...
			if (!_outOfFuel)
			{
				playSound(properties.GetValue("sound_shut_off"));
			}
			else
			{
				playSound(properties.GetValue("sound_no_fuel_shut_off"));
			}
