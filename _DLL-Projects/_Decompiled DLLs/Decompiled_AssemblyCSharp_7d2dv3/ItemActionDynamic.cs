...
public class ItemActionDynamic : ItemAction
{
	public class ItemActionDynamicData : ItemActionAttackData
	{
		...
		public List<PropRef> alreadyHitProps;
			...
			: base(_invData, _indexInEntityOfAction)
		{
			...
			alreadyHitProps = new List<PropRef>();
	...
	public override void ReadFrom(DynamicProperties _props)
	{
		...
		if (_props.Classes.ContainsKey("ToolCategory"))
		{
			...
			{
				ToolBonuses[value.Key] = new ItemActionAttack.Bonuses(StringParsers.ParseFloat(value.Value), dynamicProperties.Params1.ContainsKey(value.Key) ? StringParsers.ParseFloat(dynamicProperties.Params1[value.Key]) : 2f);
			}
		}
		...
		if (_props.Classes.ContainsKey("HitSounds"))
		{
			...
			for (int i = 0; i < 10; i++)
			{
				string key = "Override" + i;
				if (_props.Classes["HitSounds"].Values.ContainsKey(key) && _props.Classes["HitSounds"].Params1.ContainsKey(key))
				{
					HitSoundOverrides[_props.Classes["HitSounds"].Values[key]] = _props.Classes["HitSounds"].Params1[key];
				}
			}
		}
		if (_props.Classes.ContainsKey("GrazeSounds"))
		{
			...
			for (int j = 0; j < 10; j++)
			{
				string key2 = "Override" + j;
				if (_props.Classes["GrazeSounds"].Values.ContainsKey(key2) && _props.Classes["GrazeSounds"].Params1.ContainsKey(key2))
				{
					GrazeSoundOverrides[_props.Classes["GrazeSounds"].Values[key2]] = _props.Classes["GrazeSounds"].Params1[key2];
				}
			}
		}
	...
	public override WorldRayHitInfo GetExecuteActionTarget(ItemActionData _actionData)
	{
		...
		else
		{
			...
			if (entityAlive == null)
			{
				Voxel.Raycast(itemActionDynamicData.invData.world, itemActionDynamicData.ray, num, -538488845, (hitmaskOverride == 0) ? 128 : hitmaskOverride, SphereRadius);
			}
		}
	...
	public WorldRayHitInfo[] GetExecuteActionGrazeTarget(ItemActionData _actionData, float normalizedClipTime = -1f)
	{
		...
		if (Voxel.Raycast(itemActionDynamicData.invData.world, meleeRay2, num3, -538750989, (hitmaskOverride == 0) ? 128 : hitmaskOverride, SphereRadius))
		{
		...
		while (num5 < num4)
		{
			...
			if (Voxel.Raycast(itemActionDynamicData.invData.world, meleeRay, num3, -538750989, (hitmaskOverride == 0) ? 128 : hitmaskOverride, num))
			{
	...
	public virtual void hitTarget(ItemActionData _actionData, WorldRayHitInfo hitInfo, bool _isGrazingHit = false)
	{
		...
		{
			_actionData.invData.itemValue.UseTimes += EffectManager.GetValue(PassiveEffects.DegradationPerUse, _actionData.invData.itemValue, 1f, holdingEntity, null, tmpTag) * ItemAction.ItemDegradationModifier;
		...
		if (hitInfo.tag != null)
		{
			if (GameUtils.IsBlockOrTerrain(hitInfo.tag))
			{
				...
				PropValue propValue = (propValue2 = hitInfo.hit.propValue);
				if (!propValue.IsAir)
				{
					minEventContext.PropRef = hitInfo.hit.propData.PropRef;
					minEventContext.PropValue = propValue2;
					blockValue = (minEventContext.BlockValue = propValue2.blockValue);
					itemActionDynamicData.alreadyHitProps.Add(hitInfo.hit.propData.PropRef);
				}
				else
				{
					BlockValue blockValue3;
					BlockValue blockValue2 = (blockValue3 = hitInfo.fmcHit.blockValue);
					if (!blockValue2.isair)
					{
						minEventContext.BlockValue = blockValue3;
						blockValue = blockValue3;
						minEventContext.PropValue = PropValue.AIR;
		...
		if (blockValue.isair)
		{
			blockValue = ItemActionAttack.GetBlockHit(itemActionDynamicData.invData.world, hitInfo);
		}
		float num = GetDamageBlock(_actionData.invData.itemValue, blockValue, holdingEntity, _actionData.indexInEntityOfAction);
		float num2 = GetDamageEntity(_actionData.invData.itemValue, holdingEntity, _actionData.indexInEntityOfAction);
		if (_isGrazingHit)
		{
			num = 0f;
			num2 *= EffectManager.GetValue(PassiveEffects.GrazeDamageMultiplier, _actionData.invData.itemValue, GrazeDamagePercentage, holdingEntity, null, tmpTag);
		}
		int num3 = 1;
		...
		EnumDamageTypes damageType = DamageType;
		float blockDamage = num;
		float entityDamage = num2;
		float criticalChance = getCriticalChance(_actionData);
		float dismemberChance = ItemAction.GetDismemberChance(_actionData, hitInfo);
		string surfaceCategory = item.MadeOfMaterial.SurfaceCategory;
		DamageMultiplier damageMultiplier = new DamageMultiplier();
		List<string> buffActions = new List<string>();
		ItemActionAttack.AttackHitInfo attackDetails = _actionData.attackDetails;
		int flags = num3;
		int actionExp = ActionExp;
		float actionExpBonusMultiplier = ActionExpBonusMultiplier;
		Dictionary<string, ItemActionAttack.Bonuses> toolBonuses = ToolBonuses;
		int attackMode = (itemActionDynamicData.IsHarvesting ? 1 : 0);
		Dictionary<string, string> hitSoundOverrides = (_isGrazingHit ? GrazeSoundOverrides : HitSoundOverrides);
		bool isGrazingHit = _isGrazingHit;
		bool forceNoDamage = flag;
		ItemActionAttack.Hit(hitInfo, entityId, damageType, blockDamage, entityDamage, 1f, 1f, criticalChance, dismemberChance, surfaceCategory, damageMultiplier, buffActions, attackDetails, flags, actionExp, actionExpBonusMultiplier, null, toolBonuses, (ItemActionAttack.EnumAttackMode)attackMode, hitSoundOverrides, -1, itemActionDynamicData.invData.itemStack.itemValue, isGrazingHit, forceNoDamage);
		if (_isGrazingHit)
		{
			return;
		}
		ItemActionDynamicMelee.ItemActionDynamicMeleeData dmd = _actionData as ItemActionDynamicMelee.ItemActionDynamicMeleeData;
		if (_actionData.invData.itemValue.IsEmpty())
		{
