...
public class NGUIWindowManager : MonoBehaviour
{
	...
	public void Update()
	{
		...
		{
			updateVersionLabelDisplayState();
			ellipsisAnimator?.GetNextAnimatedString(Time.unscaledDeltaTime);
		}
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void updateVersionLabelDisplayState()
	{
		_ = WindowManager;
		bool flag = AlwaysShowVersionUi && LocalPlayerUI.GetUIForPrimaryPlayer() != null;
		if (!flag)
		{
			LocalPlayerUI uIForPrimaryPlayer = LocalPlayerUI.GetUIForPrimaryPlayer();
			if (uIForPrimaryPlayer != null)
			{
				flag |= uIForPrimaryPlayer.windowManager.IsWindowOpen("ingameMenu");
			}
		}
		ShowVersionUi(flag);
	}
