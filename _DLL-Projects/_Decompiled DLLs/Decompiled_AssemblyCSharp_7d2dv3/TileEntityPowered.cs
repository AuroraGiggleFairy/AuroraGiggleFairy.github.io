...
public class TileEntityPowered : TileEntity, IPowered
{
	...
	public override void read(PooledBinaryReader _br, StreamModeRead _eStreamMode)
	{
		...
		{
			bool flag = false;
			if (LocalPlayerUI.GetUIForPrimaryPlayer().windowManager.TryGetWindow(XUiC_PowerCameraWindowGroup.ID, out var _window))
			{
				XUiC_PowerCameraWindowGroup xUiC_PowerCameraWindowGroup = (XUiC_PowerCameraWindowGroup)((XUiWindowGroup)_window).Controller;
	...
	public void DrawWires()
	{
		...
		for (int i = 0; i < wireDataList.Count; i++)
		{
			...
			if (GameManager.Instance.World.GetChunkFromWorldPos(blockPos) is Chunk)
			{
				TileEntityPowered tileEntityPowered = GameManager.Instance.World.GetTileEntity(blockPos) as TileEntityPowered;
				bool flag2 = false;
		...
		for (int j = 0; j < wireDataList.Count; j++)
		{
			...
			if (!(GameManager.Instance.World.GetChunkFromWorldPos(blockPos2) is Chunk))
			{
				continue;
			}
			TileEntityPowered tileEntityPowered2 = GameManager.Instance.World.GetTileEntity(blockPos2) as TileEntityPowered;
			...
			if (tileEntityPowered2 != null)
			{
				if (PowerItemType == PowerItem.PowerItemTypes.ElectricWireRelay && tileEntityPowered2.PowerItemType == PowerItem.PowerItemTypes.ElectricWireRelay)
				{
					...
					electricWireController.Init(chunk.GetBlock(base.localChunkPos).Block.Properties);
	...
	public override void OnLockedLocal(bool _success, ILockContext _context, ushort _channel)
	{
		...
		((XUiC_PoweredGenericWindowGroup)((XUiWindowGroup)uIForPrimaryPlayer.windowManager.GetWindow(WindowGroupToOpen)).Controller).TileEntity = this;
		uIForPrimaryPlayer.windowManager.Open(WindowGroupToOpen, _bModal: true);
	}
}
