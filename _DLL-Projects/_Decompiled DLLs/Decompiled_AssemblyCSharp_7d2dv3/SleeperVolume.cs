...
public class SleeperVolume
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public HashSet<int> pendingSpawnMap = new HashSet<int>();

	[PublicizedFrom(EAccessModifier.Private)]
	public Queue<EntityAsyncManager.EntityCreateHandle> pendingSpawnOps = new Queue<EntityAsyncManager.EntityCreateHandle>();
	...
	public static SleeperVolume Create(PrefabSleeperVolume psv, Vector3i _boxMin, Vector3i _boxMax)
	{
	...
	public bool AddToPrefabInstance()
	{
		...
		prefabInstance = dynamicPrefabDecorator.FindVolumeOwner(PrefabVolumeAbs.EVolumeType.Sleeper, BoxMin, BoxMax);
	...
	public void Tick(World _world)
	{
		if (isSpawning)
		{
			if (minScript != null && minScript.IsRunning())
			{
				...
				{
					if (!pendingSpawnMap.Contains(item.Key) && !_world.GetEntity(item.Key))
					{
						respawnMap.Clear();
						respawnList = null;
		...
		if (isSpawned)
		{
			...
			{
				if (!pendingSpawnMap.Contains(item2.Key) && !_world.GetEntity(item2.Key))
				{
					isSpawned = false;
					break;
				}
	...
	{
		if (pendingSpawnOps.Count > 0)
		{
			EntityAsyncManager.EntityCreateHandle result;
			while (pendingSpawnOps.TryDequeue(out result))
			{
				result.WaitForComplete();
				UnityEngine.Object.Destroy(result.Entity.RootTransform);
			}
		}
		pendingSpawnMap.Clear();
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void CompletePendingSpawns()
	{
		if (pendingSpawnOps.Count > 0)
		{
			EntityAsyncManager.EntityCreateHandle result;
	...
	public void UpdateSpawn(World _world)
	{
		...
		if (spawnsAvailable != null)
		{
			...
			if (respawnList != null && respawnList.Count > 0)
			{
				...
				if (pendingSpawnMap.Contains(num2))
				{
					flag2 = true;
				}
				else if ((bool)entity)
				...
				else
				{
					...
					if (num3 >= 0)
					{
						...
						int num4 = EntityClass.FromString(className);
						EntityClass.list.TryGetValue(num4, out var _value);
						if (_value == null || (_value.bIsEnemyEntity && !EntityFactory.EnemySpawnMode))
						{
						...
						else
						{
							...
							if (Spawn(_world, num4, num3, block))
							{
								respawnMap.Remove(num2);
							}
						}
			...
			else if (flag)
			{
				...
				if (gameStageDefinition != null)
				{
					...
					if (stage != null)
					{
						int num6 = FindSpawnIndex(_world);
						if (num6 >= 0)
						{
							SpawnPoint spawnPoint2 = spawnPointList[num6];
							if (!CheckSpawnPos(_world, spawnPoint2.pos))
							{
								spawnsAvailable.Add(num6);
								return;
							}
							...
							else
							{
								...
								EntityClass.list.TryGetValue(randomFromGroup, out var _value2);
								if (_value2 == null || (_value2.bIsEnemyEntity && !EntityFactory.EnemySpawnMode))
								{
									numSpawned++;
								}
								else
								{
									Log.Out("{0} SleeperVolume {1}: Spawning {2} ({3}), group '{4}', class {5}, count {6}", text, BoxMin, spawnPoint2.pos, World.toChunkXZ(spawnPoint2.pos), spawnGroup, (_value2 != null) ? _value2.entityClassName : "?", num);
									if (Spawn(_world, randomFromGroup, num6, block2))
									{
										numSpawned++;
									}
								}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public bool Spawn(World _world, int entityClassId, int spawnIndex, BlockSleeper block)
	{
		SpawnPoint spawnPoint = spawnPointList[spawnIndex];
		Vector3 spawnPos = spawnPoint.pos.ToVector3();
		spawnPos.x += 0.502f;
		spawnPos.z += 0.501f;
		if (!EntityClass.list.TryGetValue(entityClassId, out var _value))
		{
			Log.Warning("Spawn class {0} is missing", entityClassId);
			entityClassId = EntityClass.FromString("zombieArlene");
		}
		...
		EntityCreationData entityCreationData = EntityFactory.SetupEntityCreationData(entityClassId, spawnPos, new Vector3(0f, spawnPoint.rot, 0f));
		EntityAsyncManager.EntityCreateHandle item = _world.entityAsyncManager.StartCreateEntity(entityCreationData, [PublicizedFrom(EAccessModifier.Internal)] (EntityAsyncManager.EntityCreateHandle handle) =>
		{
			AddEnemyToWorld(_world, handle.Entity as EntityAlive, spawnIndex, spawnPos, block.look, block.pose);
		});
		pendingSpawnOps.Enqueue(item);
		pendingSpawnMap.Add(entityCreationData.id);
		RespawnData value = default(RespawnData);
		value.className = _value.entityClassName;
		value.spawnPointIndex = spawnIndex;
		respawnMap.Add(entityCreationData.id, value);
		TickSpawnCount++;
		return true;
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void AddEnemyToWorld(World _world, EntityAlive zombie, int spawnIndex, Vector3 spawnPos, Vector3 lookDir, int pose)
	{
		if (!zombie)
		{
		...
		zombie.SetSpawnerSource(EnumSpawnerSource.Dynamic);
		zombie.IsSleeperPassive = true;
		zombie.SleeperSpawnPosition = spawnPos;
		zombie.SleeperSpawnLookDir = lookDir;
		zombie.SetSleeper();
		zombie.TriggerSleeperPose(pose);
		_world.SpawnEntityInWorld(zombie);
		hasPassives = true;
		SpawnParticle("sleeperSpawn", zombie);
		if ((bool)playerTouchedTrigger)
		{
			GameManager.Instance.StartCoroutine(WakeAttackLater(zombie, playerTouchedTrigger));
		}
	}
