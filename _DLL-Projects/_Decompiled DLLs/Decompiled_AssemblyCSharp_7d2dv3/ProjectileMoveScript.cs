...
public class ProjectileMoveScript : MonoBehaviour
{
	...
	public const float ReviveSpread = 0.4f;

	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public const float ReviveVelocity = 0.9f;

	public const int InvalidID = -1;

	public static World s_lastWorld;
	...
	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public float? stickyRoll;

	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public Ray? stickyRay;

	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public float stickyRayDistance;

	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public bool stickyForceRevive;

	[PublicizedFrom(EAccessModifier.Private)]
	static ProjectileMoveScript()
	{
		GameManager.Instance.OnWorldChanged += OnWorldChanged;
		OnWorldChanged(GameManager.Instance.World);
	}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static void OnWorldChanged(World _world)
	{
		if (s_lastWorld != _world)
		{
			if (s_lastWorld != null)
			{
				s_lastWorld.EntityUnloadedDelegates -= OnEntityUnloaded;
			}
			if (_world != null)
			{
	...
	public void FixedUpdate()
	{
		...
		else if (state == State.Sticky)
		{
			...
			{
				bool flag = stickyForceRevive;
				if (!flag && stickyRay.HasValue && stickyRay.Value.direction != Vector3.zero && stickyRayDistance > 0f && (!Voxel.Raycast(world, stickyRay.Value, stickyRayDistance, -538751005, hitMask, radius) || (!GameUtils.IsBlockOrTerrain(Voxel.voxelRayHitInfo.tag) && !Voxel.voxelRayHitInfo.tag.StartsWith("E_"))))
				{
					flag = true;
				}
				if (flag)
	...
	public void DoRevive()
	{
		...
		if (gameRandom != null && ProjectileManager.RemoveProjectile(ProjectileID, revive: true))
		{
			Transform obj = base.transform;
			stickyRay = null;
			stickyRayDistance = 0f;
			stickyForceRevive = false;
			Quaternion quaternion = Quaternion.FromToRotation(Vector3.forward, velocity.normalized);
			Vector3 normalized = new Vector3(gameRandom.RandomGaussian * 0.4f, gameRandom.RandomGaussian * 0.4f, 1f).normalized;
			float num = gameRandom.RandomFloat * 0.9f;
			velocity = quaternion * normalized * num;
			if (velocity.y > 0f)
			{
				velocity.y = 0f;
			}
			Utils.SetLayerRecursively(obj.gameObject, 0);
			SetState(State.Active);
		}
	}
	...
	public virtual void checkCollision()
	{
		...
		bool num3 = Voxel.Raycast(world, ray, magnitude + num2, -538751005, hitMask, radius);
		...
		if ((bool)firingEntity && !firingEntity.isEntityRemote)
		{
			...
			float num4 = Utils.FastLerp(1f, itemActionProjectile.GetDamageEntity(itemValueLauncher, entityAlive), actionData.strainPercent);
			bool flag = false;
			if (entityAlive3 is EntityPlayer)
			{
				num4 *= ItemActionAttack.RangedDamagePercent;
				flag = ItemActionAttack.RangedDamagePercent == 0f;
			}
			else
			{
				num4 *= ItemActionAttack.EntityPlayerDamagePercent;
				flag = ItemActionAttack.EntityPlayerDamagePercent == 0f;
			}
			ItemActionAttack.Hit(Voxel.voxelRayHitInfo, ProjectileOwnerID, EnumDamageTypes.Piercing, blockDamage, num4, 1f, 1f, EffectManager.GetValue(PassiveEffects.CriticalChance, itemValueLauncher, itemProjectile.CritChance.Value, entityAlive, null, itemProjectile.ItemTags), ItemAction.GetDismemberChance(actionData, Voxel.voxelRayHitInfo), itemProjectile.MadeOfMaterial.SurfaceCategory, itemActionProjectile.GetDamageMultiplier(), getBuffActions(), attackDetails, 1, itemActionProjectile.ActionExp, itemActionProjectile.ActionExpBonusMultiplier, null, null, ItemActionAttack.EnumAttackMode.RealNoHarvesting, null, -1, itemValueLauncher, _isGrazingHit: false, flag);
			if (!entityAlive2)
			{
			...
			if (itemActionProjectile.Explosion.ParticleIndex > 0 && !explosionDisabled)
			{
				...
				instance.ExplosionServer(hitPos, vector3i, Quaternion.identity, itemActionProjectile.Explosion, ProjectileOwnerID, 0f, _bRemoveBlockAtExplPosition: false, itemValueLauncher);
				SetState(State.Dead);
			}
			else
			{
				...
				if (itemProjectile.IsSticky)
				{
					...
					{
						float valueOrDefault = stickyRoll.GetValueOrDefault();
						float num5;
						if (!stickyRoll.HasValue)
						{
							valueOrDefault = gameRandom.RandomFloat;
							stickyRoll = valueOrDefault;
							num5 = valueOrDefault;
						}
						else
						{
						...
						if (num5 < EffectManager.GetValue(PassiveEffects.ProjectileStickChance, itemValueLauncher, 0.5f, entityAlive, null, itemProjectile.ItemTags | FastTags<TagGroup.Global>.Parse(Voxel.voxelRayHitInfo.fmcHit.blockValue.Block.blockMaterial.SurfaceCategory)))
						{
							...
							stickyRay = ray;
							stickyRayDistance = magnitude + num2;
					...
					else
					{
						...
						if (num6 < EffectManager.GetValue(PassiveEffects.ProjectileStickChance, itemValueLauncher, 0.5f, entityAlive, null, itemProjectile.ItemTags))
						{
							ProjectileID = ProjectileManager.AddProjectileItem(base.transform, -1, Voxel.voxelRayHitInfo.hit.pos, direction.normalized, itemValueProjectile.type);
	...
	public static void OnEntityUnloaded(Entity _entity, EnumRemoveEntityReason _reason)
	{
		List<ProjectileMoveScript> toUnload = new List<ProjectileMoveScript>();
		Search(_entity.RootTransform);
		foreach (ProjectileMoveScript item in toUnload)
		{
			item.OnUnloaded();
		}
		...
		void Search(Transform parent)
		{
			...
			if (parent.TryGetComponent<ProjectileMoveScript>(out var component))
			{
				toUnload.Add(component);
			}
		}
	}
	...
	public void OnUnloaded()
	{
		...
		{
			idealPosition = (previousPosition = base.transform.position + Origin.position);
			base.transform.SetParent(null);
			base.transform.position = previousPosition - Origin.position;
			stickyForceRevive = true;
		}
	}
	...
	public bool TryCollect(EntityPlayerLocal _player)
	{
		if (!itemProjectile.IsSticky)
		{
			return false;
		}
		ItemStack itemStack = new ItemStack(itemValueProjectile, 1);
		if (_player.PlayerUI.xui.PlayerInventory.AddItem(itemStack))
		{
			ProjectileID = -1;
			UnityEngine.Object.Destroy(base.gameObject);
			return true;
