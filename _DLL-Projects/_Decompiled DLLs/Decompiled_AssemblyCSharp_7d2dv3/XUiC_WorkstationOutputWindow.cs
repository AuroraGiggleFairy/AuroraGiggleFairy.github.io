...
public class XUiC_WorkstationOutputWindow : XUiController
{
	...
	public override void Init()
	{
		...
		controls.MoveAllDone = [PublicizedFrom(EAccessModifier.Private)] (bool _allMoved, bool _anyMoved) =>
		{
			if (_anyMoved)
			{
				Manager.BroadcastPlayByLocalPlayer(xui.playerUI.entityPlayer.position + Vector3.one * 0.5f, "UseActions/takeall1");
			}
		};
	}
	...
	{
		base.Update(_dt);
		if (!xui.playerUI.windowManager.IsInputActive() && (xui.playerUI.playerInput.GUIActions.LeftStick.WasPressed || xui.playerUI.playerInput.PermanentActions.Reload.WasPressed))
		{
			controls.MoveAll();
			windowGroup.xui.playerUI.windowManager.Close(windowGroup);
		}
	}
}
