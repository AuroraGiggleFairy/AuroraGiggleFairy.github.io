...
public struct PlayerStealth
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public const int cAttractEmitRate = 2;

	[PublicizedFrom(EAccessModifier.Private)]
	public const float cAttractEmitChance = 0.2f;

	[PublicizedFrom(EAccessModifier.Private)]
	public const float cAttractRadiusMax = 100f;

	[PublicizedFrom(EAccessModifier.Private)]
	public int attractEmitTicks;
	...
	public float ValuePercentUI
	{
		...
		{
			float num = 0f;
			float num2 = smellRadius / 100f;
			return Utils.FastClamp01((lightLevel + noiseVolume * 0.5f + (num + num2) * 50f + (float)(alertEnemy ? 5 : 0)) * 0.01f + 0.005f);
		}
	}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public void AttractTickServer()
	{
		if (--attractEmitTicks > 0)
		{
			return;
		}
		attractEmitTicks = 40;
		...
		float radius = num * (1f + num2);
		player.world.GetEntitiesAround(EntityFlags.AIHearing, player.position, radius, entityTempList);
		for (int i = 0; i < entityTempList.Count; i++)
		{
			...
			if (player.DetectUsScale(entityAlive) >= 1f)
			{
				float distance = player.GetDistance(entityAlive);
				bool flag = true;
				if ((bool)entityAlive.attractPlayer && entityAlive.attractPlayer != player)
				{
					flag = distance < entityAlive.attractPlayerDistance;
				}
				if (flag)
				{
					entityAlive.attractPlayer = player;
					entityAlive.attractPlayerDistance = distance;
					entityAlive.attractPlayerTimeoutTicks = 80;
				}
			}
		}
		entityTempList.Clear();
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public void SmellApplyMode()
	{
		if (GamePrefs.GetInt(EnumGamePrefs.AISmellMode) == 0)
		{
			if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer && player.Buffs.HasCustomVar("smell"))
			{
	...
	public void SmellUpdateItemsAndBlood()
	{
		if (smellWet >= 3f || player.IsDead())
		{
			...
			{
				SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageEntityStealth>().Setup(playerLocal, -1, _eating: false, smellSheltered));
			}
			return;
		}
		...
		float v = SmellCountToRadius(num);
		smellRadiusTarget = Utils.FastMax(v, smellEatRadius);
	...
	public static PlayerStealth Read(EntityPlayer _player, BinaryReader br)
	{
		...
		if (num >= 4)
		{
			result.smellRadiusTarget = br.ReadSingle();
			result.smellRadius = br.ReadSingle();
			result.smellEatRadius = br.ReadSingle();
			result.smellEatTicks = br.ReadInt16();
			result.smellSheltered = br.ReadBoolean();
		}
		result.SmellApplyMode();
		return result;
	}
	...
	public void Write(BinaryWriter bw)
	{
		...
		bw.Write(smellRadiusTarget);
		bw.Write(smellRadius);
		bw.Write(smellEatRadius);
		bw.Write((short)smellEatTicks);
		bw.Write(smellSheltered);
	}
}
