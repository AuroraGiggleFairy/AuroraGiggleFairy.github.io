	...
	public static IEnumerator Load(XmlFile _xmlFile)
	{
		...
		foreach (XElement item in root.Elements("biome"))
		{
			...
			foreach (XElement item2 in item.Elements("spawn"))
			{
				...
				{
					string[] array2 = item2.GetAttribute("respawndelay").Split(',');
					array = new int[array2.Length];
					for (int i = 0; i < array2.Length; i++)
					{
						array[i] = (int)(StringParsers.ParseFloat(array2[i]) * 24000f);
					}
				}
				...
				bool isAnimal = false;
				if (item2.HasAttribute("isanimal"))
				{
					isAnimal = StringParsers.ParseBool(item2.GetAttribute("isanimal"));
				}
				BiomeSpawnEntityGroupData biomeSpawnEntityGroupData = new BiomeSpawnEntityGroupData(hashCode, maxCount, array, daytime, isAnimal);
