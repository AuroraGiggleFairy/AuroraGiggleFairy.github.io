...
public class XUiC_RecipeEntry : XUiC_SelectableEntry
{
	...
	{
		isHovered = _isOver;
		if (background != null && recipe != null && !base.IsSelected)
		{
			if (_isOver)
			{
	...
	public override bool GetBindingValueInternal(ref string value, string bindingName)
	{
		...
		case "isfavorite":
			value = ((recipe != null) ? XUiM_Recipes.GetRecipeIsFavorite(xui, recipe).ToString() : "false");
			return true;
		case "isunlockable":
			value = ((recipe != null) ? (!IsCurrentWorkstation || XUiM_Recipes.GetRecipeIsUnlockable(xui, recipe) || recipe.isQuest || recipe.isChallenge || recipe.IsTracked).ToString() : "false");
			return true;
		case "hasingredients":
		...
		case "hasingredientsstatecolor":
			if (recipe != null)
			{
				...
				if (HasIngredients)
				{
					v = ((!CustomAttributes.ContainsKey("enabled_font_color")) ? ((Color32)Color.white) : ((Color32)StringParsers.ParseColor32(CustomAttributes["enabled_font_color"].ToString())));
				}
				else if (CustomAttributes.ContainsKey("disabled_font_color"))
				{
					v = StringParsers.ParseColor32(CustomAttributes["disabled_font_color"].ToString());
				}
		...
		case "itemtypeicon":
			...
			if (recipe != null)
			{
				...
				if (forId != null)
				{
					...
					{
						if (forId.AltItemTypeIcon != null && forId.Unlocks != "" && XUiM_ItemStack.CheckKnown(xui.playerUI.entityPlayer, forId))
						{
		...
		case "itemtypeicontint":
			...
			if (recipe != null)
			{
				...
				if (forId4 != null && forId4.Unlocks != "" && XUiM_ItemStack.CheckKnown(xui.playerUI.entityPlayer, forId4))
				{
		...
		case "unlockstatecolor":
			if (recipe != null)
			{
				...
				{
					Color32 v2 = (XUiM_Recipes.GetRecipeIsUnlocked(xui, recipe) ? Color.white : Color.gray);
		...
		case "unlockicon":
			if (recipe != null)
			{
				...
				if (XUiM_Recipes.GetRecipeIsUnlockable(xui, recipe) && !XUiM_Recipes.GetRecipeIsUnlocked(xui, recipe))
				{
