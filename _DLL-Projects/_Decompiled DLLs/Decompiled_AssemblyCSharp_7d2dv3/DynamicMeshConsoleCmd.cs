	...
	public override void Execute(List<string> _params, CommandSenderInfo _senderInfo)
	{
		...
		else
		{
			...
			if (text == "traders")
			{
				...
				{
					foreach (PrefabInstance item3 in list2.Where([PublicizedFrom(EAccessModifier.Internal)] (PrefabInstance d) => d.prefab.HasAnyQuestTag(QuestEventManager.traderTag)))
					{
			...
			if (text == "listblocks")
			{
				Block[] list3 = Block.list;
				foreach (Block block in list3)
				{
			...
			if (text == "deco")
			{
				...
				{
					foreach (PrefabInstance item7 in list4.OrderBy([PublicizedFrom(EAccessModifier.Internal)] (PrefabInstance d) => Math.Abs(Vector3.Distance(playerPos, d.boundingBoxPosition.ToVector3()))))
					{
