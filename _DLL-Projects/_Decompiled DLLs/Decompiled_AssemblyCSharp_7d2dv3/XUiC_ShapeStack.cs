...
public class XUiC_ShapeStack : XUiC_SelectableEntry
{
			...
			if (isOver && base.ViewComponent.UiTransformIsHovered && base.ViewComponent.EventOnPress)
			{
				if (gUIActions.LastInputType == BindingSourceType.DeviceBindingSource)
				{
					...
					if (wasReleased && blockData != null)
					{
						...
						if (wasReleased2)
						{
							xui.playerUI.windowManager.Close("shapes");
						}
					}
				}
	...
	{
		if (!base.IsSelected || BlockData == null || xui.playerUI.playerInput == null)
		{
			return;
		}
		PlayerActionsGUI gUIActions = xui.playerUI.playerInput.GUIActions;
	...
	public override bool GetBindingValueInternal(ref string value, string bindingName)
	{
		...
		{
			string favoritesEntryName = GetFavoritesEntryName(BlockData);
			value = (favoritesEntryName != null && xui.playerUI.entityPlayer.favoriteShapes.Contains(favoritesEntryName)).ToString();
			return true;
		}
		default:
