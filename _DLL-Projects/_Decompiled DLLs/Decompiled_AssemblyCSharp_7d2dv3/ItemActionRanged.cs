...
public class ItemActionRanged : ItemActionAttack
{
	public class ItemActionDataRanged : ItemActionAttackData
	{
		...
		public bool burstShotFinished;

		public bool bInitialPress;
	...
	public enum ReloadFlags
	{
		...
		InvalidItem = 1,
		AlreadyReloading = 2,
		PendingInventoryActions = 4,
		Jammed = 8,
		OutOfAmmo = 0x10,
		EmptyClip = 0x20,
		PartialClip = 0x40,
		FullClip = 0x80,
		CompletelyOutOfAmmo = 0x30
	}
	...
	[field: PublicizedFrom(EAccessModifier.Private)]
	public FloatRange aiBurstDelay
	{
		get; [PublicizedFrom(EAccessModifier.Protected)]
		set;
	}
	...
	public override void ReadFrom(DynamicProperties _props)
	{
		...
		aiBurstShot = _props.TryParseRange("aiBurstShot", new IntRange(1, 1));
		aiBurstDelay = _props.TryParseRange("aiBurstDelay", new FloatRange(1f, 2f));
	}
	...
	public override ItemClass.EnumCrosshairType GetCrosshairType(ItemActionData _actionData)
	{
		...
		if (entityPlayerLocal.TPCameraCheckResult == eTPCameraCheckResult.Pass)
		{
		...
		if (entityPlayerLocal.TPCameraCheckResult == eTPCameraCheckResult.LineOfSightCheckFailed)
		{
	...
	public override void StopHolding(ItemActionData _data)
	{
		...
		ResetBurstShot(itemActionDataRanged);
		itemActionDataRanged.bReleased = true;
	...
	public override bool IsActionRunning(ItemActionData _actionData)
	{
		...
		float num = Time.time - itemActionDataRanged.m_LastShotTime;
		if (rapidTrigger && itemActionDataRanged.bReleased && num > 0.25f)
		{
			return false;
		}
		if (num < itemActionDataRanged.Delay)
		{
			return true;
		}
		return false;
	...
	public void CompleteReload(ItemActionDataRanged _adr)
	{
		if (_adr == null)
		{
			return;
		...
		ItemValue itemValue = ItemClass.GetItem(MagazineItemNames[_adr.invData.itemValue.SelectedAmmoTypeIndex]);
		int num = (int)EffectManager.GetValue(PassiveEffects.MagazineSize, _adr.invData.itemValue, BulletsPerMagazine, holdingEntity);
		if (HasInfiniteAmmo(_adr))
		{
			_adr.reloadAmount = (AmmoIsPerMagazine ? num : (num - _adr.invData.itemValue.Meta));
		}
		else if (AmmoIsPerMagazine)
		{
			if (holdingEntity.bag.GetItemCount(itemValue) > 0)
			{
				_adr.reloadAmount = num * holdingEntity.bag.DecItem(itemValue, 1);
			}
			else if (holdingEntity.inventory.GetItemCount(itemValue) > 0)
			{
				_adr.reloadAmount = num * holdingEntity.inventory.DecItem(itemValue, 1);
			}
		}
		else
		{
			int num2 = num - _adr.invData.itemValue.Meta;
			_adr.reloadAmount = holdingEntity.bag.DecItem(itemValue, num2);
			if (_adr.reloadAmount < num2)
			{
				_adr.reloadAmount += holdingEntity.inventory.DecItem(itemValue, num2 - _adr.reloadAmount);
			}
		}
		if (_adr.reloadAmount > 0)
		{
			_adr.invData.itemValue.Meta = Utils.FastMin(_adr.invData.itemValue.Meta + _adr.reloadAmount, num);
	...
	public void CycleAmmoType(ItemActionData _actionData, bool excludeNonUnderwaterAmmoTypes)
	{
		...
		while (selectedAmmoTypeIndex != num)
		{
			...
			if (_actionData.invData.holdingEntity.bag.GetItemCount(itemValue) + _actionData.invData.holdingEntity.inventory.GetItemCount(itemValue) > 0)
			{
				break;
			}
			selectedAmmoTypeIndex--;
	...
	public override void OnHoldingUpdate(ItemActionData _actionData)
	{
		...
		if (itemActionDataRanged.burstShotStarted && !itemActionDataRanged.burstShotFinished)
		{
			TryExecuteAction(_actionData);
		}
		...
		if (itemActionDataRanged.state != ItemActionFiringState.Off && itemActionDataRanged.m_LastShotTime > 0f && Time.time > itemActionDataRanged.m_LastShotTime + itemActionDataRanged.Delay * 2f && !itemActionDataRanged.burstShotStarted)
		{
	...
	public ReloadFlags GetReloadFlags(ItemActionData _actionData)
	{
		ReloadFlags reloadFlags = ReloadFlags.InvalidItem;
		if (_actionData is ItemActionDataRanged itemActionDataRanged && itemActionDataRanged.invData.holdingEntity != null)
		{
			EntityAlive holdingEntity = itemActionDataRanged.invData.holdingEntity;
			...
			int num = (int)EffectManager.GetValue(PassiveEffects.MagazineSize, holdingItemItemValue, BulletsPerMagazine, holdingEntity);
			if (!NotReloading(itemActionDataRanged))
			{
				reloadFlags |= ReloadFlags.AlreadyReloading;
			}
			EntityPlayerLocal entityPlayerLocal = holdingEntity as EntityPlayerLocal;
			if ((bool)entityPlayerLocal && entityPlayerLocal.CancellingInventoryActions)
			{
				reloadFlags |= ReloadFlags.PendingInventoryActions;
			}
			if (isJammed(holdingItemItemValue))
			{
				reloadFlags |= ReloadFlags.Jammed;
			}
			if (_actionData.invData.itemValue.Meta == 0)
			{
				reloadFlags |= ReloadFlags.EmptyClip;
			}
			else if (_actionData.invData.itemValue.Meta > 0)
			{
				reloadFlags |= ReloadFlags.PartialClip;
			}
			else if (_actionData.invData.itemValue.Meta == num)
			{
				reloadFlags |= ReloadFlags.FullClip;
			}
			if (holdingEntity.inventory.GetItemCount(itemValue) == 0)
			{
				Bag bag = holdingEntity.bag;
				if (bag != null && bag.GetItemCount(itemValue) == 0 && !HasInfiniteAmmo(_actionData))
				{
					reloadFlags |= ReloadFlags.OutOfAmmo;
				}
			}
			reloadFlags &= ~ReloadFlags.InvalidItem;
		}
		return reloadFlags;
	}
	...
	public override bool CanReload(ItemActionData _actionData)
	{
		...
		int num = (int)EffectManager.GetValue(PassiveEffects.MagazineSize, holdingItemItemValue, BulletsPerMagazine, holdingEntity);
		EntityPlayerLocal entityPlayerLocal = holdingEntity as EntityPlayerLocal;
		if (NotReloading(actionData) && (!entityPlayerLocal || !entityPlayerLocal.CancellingInventoryActions) && (isJammed(holdingItemItemValue) || _actionData.invData.itemValue.Meta < num))
		{
			if (holdingEntity.inventory.GetItemCount(itemValue) <= 0)
			{
				Bag bag = holdingEntity.bag;
				if (bag == null || bag.GetItemCount(itemValue) <= 0)
				{
	...
	public override bool AllowItemLoopingSound(ItemActionData _actionData)
	{
		...
		if (_actionData.invData.itemValue.Meta > 0 && burstCount > 0 && itemActionDataRanged.curBurstCount <= burstCount && !string.IsNullOrEmpty(soundRepeat))
		{
	...
	{
		if (!myActionData.burstShotStarted || myActionData.burstShotFinished)
		{
	...
	[PublicizedFrom(EAccessModifier.Protected)]
	public void ResetBurstShot(ItemActionDataRanged _actionDataRanged)
	{
		_actionDataRanged.burstShotStarted = false;
		_actionDataRanged.burstShotFinished = false;
		_actionDataRanged.curBurstCount = 0;
	}
	...
	{
		ItemActionDataRanged itemActionDataRanged = (ItemActionDataRanged)_actionData;
		itemActionDataRanged.bInitialPress = !_bReleased && !itemActionDataRanged.bPressed;
		itemActionDataRanged.bPressed = !_bReleased;
		if (_bReleased)
		{
			itemActionDataRanged.bReleased = true;
	...
	[PublicizedFrom(EAccessModifier.Protected)]
	public void TryExecuteAction(ItemActionData _actionData)
	{
		ItemActionDataRanged itemActionDataRanged = (ItemActionDataRanged)_actionData;
		if (_actionData.invData.holdingEntity is EntityPlayerLocal entityPlayerLocal)
		{
			entityPlayerLocal.StartTPCameraLockTimer();
			if (!entityPlayerLocal.TPCameraCheckPassed)
			{
				return;
			}
		}
		if (itemActionDataRanged.burstShotFinished)
		{
			return;
		}
		int burstCount = GetBurstCount(_actionData);
		bool bInitialPress = itemActionDataRanged.bInitialPress;
		bool flag = bInitialPress && (rapidTrigger || burstCount == 1);
		bool flag2 = burstCount == 0 || itemActionDataRanged.curBurstCount < burstCount;
		if (burstCount > 0)
		{
			itemActionDataRanged.burstShotStarted = true;
		}
		if (itemActionDataRanged.burstShotStarted && itemActionDataRanged.curBurstCount >= burstCount)
		{
			itemActionDataRanged.burstShotFinished = true;
			ResetBurstShot(itemActionDataRanged);
			if (burstCount != 1)
			{
				return;
		...
		if (!flag && !flag2 && !itemActionDataRanged.bReleased)
		{
			return;
		}
		...
		if (Reloading(itemActionDataRanged))
		{
			itemActionDataRanged.m_LastShotTime = time;
		}
		else
		{
			if (burstCount != 1 && !flag && time - itemActionDataRanged.m_LastShotTime < itemActionDataRanged.Delay)
			{
				return;
			}
			itemActionDataRanged.m_LastShotTime = time;
			...
			{
				itemActionDataRanged.m_LastShotTime = time + 1f;
				if (bInitialPress)
				{
			...
			{
				itemActionDataRanged.m_LastShotTime = -1f;
				if (bInitialPress)
				{
					EntityPlayerLocal localPlayer = holdingEntity as EntityPlayerLocal;
			...
			{
				_actionData.invData.itemValue.UseTimes += EffectManager.GetValue(PassiveEffects.DegradationPerUse, itemActionDataRanged.invData.itemValue, 1f, itemActionDataRanged.invData.holdingEntity, null, _actionData.invData.itemValue.ItemClass.ItemTags) * ItemAction.ItemDegradationModifier;
				if (itemActionDataRanged.invData.itemValue.PercentUsesLeft == 0f)
				{
					itemActionDataRanged.state = ItemActionFiringState.Off;
				}
			...
			{
				itemActionDataRanged.burstShotFinished = true;
				bool bPressed = itemActionDataRanged.bPressed;
				triggerReleased(itemActionDataRanged, _actionData.indexInEntityOfAction);
				if (!bPressed)
				{
					ResetBurstShot(itemActionDataRanged);
	...
	public virtual float GetIdealRangeForAI(ItemActionData _actionData)
	{
		return EffectManager.GetValue(PassiveEffects.MaxRange, _actionData.invData.itemValue, Range, _actionData.invData.holdingEntity);
	}
	...
	public virtual Vector3 fireShot(int _shotIdx, ItemActionDataRanged _actionData, ref bool hitEntityFound)
	{
		...
		for (int i = 0; i < num; i++)
		{
			if (Voxel.Raycast(_actionData.invData.world, lookRay, range, -538751005, hitMask, 0f))
			{
				...
				float num7 = GetDamageEntity(itemValue, holdingEntity) * num3 * num4;
				bool flag = false;
				if (holdingEntity is EntityPlayer)
				{
					float num8 = (itemValue.ItemClass.ItemTags.Test_AnySet(ItemActionAttack.MeleeTag) ? ItemActionAttack.MeleeDamagePercent : ItemActionAttack.RangedDamagePercent);
					num7 *= num8;
					flag = num8 == 0f;
				}
				else
				{
					num7 *= ItemActionAttack.EntityPlayerDamagePercent;
					flag = ItemActionAttack.EntityPlayerDamagePercent == 0f;
				}
				int entityId = holdingEntity.entityId;
				EnumDamageTypes damageType = ((DamageType == EnumDamageTypes.None) ? EnumDamageTypes.Piercing : DamageType);
				float blockDamage = GetDamageBlock(itemValue, ItemActionAttack.GetBlockHit(_actionData.invData.world, worldRayHitInfo), holdingEntity) * num3 * num4;
				float entityDamage = num7;
				float percentUsesLeft = itemValue.PercentUsesLeft;
				float value2 = _actionData.invData.item.CritChance.Value;
				float dismemberChance = ItemAction.GetDismemberChance(_actionData, worldRayHitInfo);
				string attackingDeviceMadeOf = bulletMaterialName;
				DamageMultiplier obj = damageMultiplier;
				List<string> buffActions = getBuffActions(_actionData);
				AttackHitInfo attackDetails = _actionData.attackDetails;
				int actionExp = ActionExp;
				float actionExpBonusMultiplier = ActionExpBonusMultiplier;
				Dictionary<string, Bonuses> toolBonuses = ToolBonuses;
				int attackMode = (bSupportHarvesting ? 1 : 0);
				bool forceNoDamage = flag;
				Dictionary<string, float> dictionary = soundImpactVolumeScale;
				ItemActionAttack.Hit(worldRayHitInfo, entityId, damageType, blockDamage, entityDamage, 1f, percentUsesLeft, value2, dismemberChance, attackingDeviceMadeOf, obj, buffActions, attackDetails, 0, actionExp, actionExpBonusMultiplier, null, toolBonuses, (EnumAttackMode)attackMode, null, -1, itemValue, _isGrazingHit: false, forceNoDamage, _canHitSpecialBodyParts: true, dictionary);
	...
	public override WorldRayHitInfo GetExecuteActionTarget(ItemActionData _actionData)
	{
		...
		if (holdingEntity is EntityPlayer entityPlayer && itemActionDataRanged.Laser != null)
		{
			...
			{
				if (_actionData.invData.holdingEntity is EntityPlayerLocal { TPCameraCheckPassed: false })
				{
				...
				if (Voxel.Raycast(_actionData.invData.world, ray, num, -538751005, hitMask, 0f))
				{
		...
		bool num2 = Voxel.Raycast(_actionData.invData.world, ray, num, -538751005, hitMask, 0f);
		holdingEntity.SetModelLayer(modelLayer);
		if (num2)
		{
			WorldRayHitInfo updatedHitInfo = _actionData.GetUpdatedHitInfo();
