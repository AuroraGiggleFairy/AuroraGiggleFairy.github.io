...
public class ChunkCacheNeighborBlocks : INeighborBlockCache
{
	...
	public BlockValue Get(int absy)
	{
		...
		try
		{
			BlockValue result = chunks[4].GetBlock(centerBX & 0xF, absy, centerBZ & 0xF);
			if (!GameManager.bShowDecorBlocks && result.Block.IsDecoration)
			{
				result = BlockValue.Air;
			}
			if (!GameManager.bShowLootBlocks && result.Block is BlockCompositeTileEntity blockCompositeTileEntity && blockCompositeTileEntity.CompositeData.HasFeature<ITileEntityLootable>())
			{
				result = BlockValue.Air;
			}
			return result;
		...
		catch (Exception)
		{
			if (firstException)
			{
				Log.Error("ChunkCacheNeighborBlocks.Get: len=" + chunks.Length);
