...
public class ItemActionRepair : ItemActionAttack
{
	...
	public override void OnHoldingUpdate(ItemActionData _actionData)
	{
		...
		else
		{
			...
			if (bUpgradeCountChanged)
			{
				...
				if (int.TryParse(block2.Properties.GetString("UpgradeBlock", "UpgradeHitCount"), out var result))
				{
					...
					if (blockUpgradeCount >= result)
					{
						...
						{
							BlockValue blockValue = Block.GetBlockValue(block2.Properties.GetString(Block.PropUpgradeBlockClass, Block.PropUpgradeBlockToBlock));
							...
							block3.DamageBlock(_actionData.invData.world, blockTargetPos, block, -1, _actionData.invData.holdingEntity.entityId);
							if (int.TryParse(block2.Properties.GetString(Block.PropUpgradeBlockClass, Block.PropUpgradeBlockItemCount), out var result2))
							{
	...
	public override void ExecuteAction(ItemActionData _actionData, bool _bReleased)
	{
		...
		if ((hitInfo.bHitValid && World.SandboxUseTraderArea == TraderAreaStates.Default && _actionData.invData.world.IsWithinTraderArea(hitInfo.hit.blockPos)) || !hitInfo.bHitValid || !GameUtils.IsBlockOrTerrain(hitInfo.tag))
		{
			return;
		}
		blockTargetPos = hitInfo.hit.blockPos;
		...
		{
			HandleJamSound(_actionData.invData.itemValue, item, entityPlayerLocal);
			return;
		}
		InventoryDataRepair inventoryDataRepair = (InventoryDataRepair)_actionData;
		Block block2 = block.Block;
		if (block2.CanRepair(block))
		{
			...
			int num7 = block.Block.DamageBlock(invData.world, blockTargetPos, block, -num, invData.holdingEntity.entityId);
	...
	public bool CanRemoveRequiredResource(ItemInventoryData data, BlockValue blockValue)
	{
		...
		if (!int.TryParse(block.Properties.GetString("UpgradeBlock", "UpgradeHitCount"), out var _))
		{
			return false;
		}
		if (!int.TryParse(block.Properties.GetString(Block.PropUpgradeBlockClass, Block.PropUpgradeBlockItemCount), out var result2) && flag)
		{
			return false;
		}
	...
	public bool RemoveRequiredResource(ItemInventoryData data, BlockValue blockValue)
	{
		...
		if (!int.TryParse(block.Properties.GetString(Block.PropUpgradeBlockClass, Block.PropUpgradeBlockItemCount), out var result))
		{
			return false;
		}
	...
	public override void getOverlayData(ItemActionData _actionData, out float _perc, out string _text)
	{
		...
		if (inventoryDataRepair.repairType == EnumRepairType.None)
		{
			...
			{
				Vector3i parentPos = blockValue.Block.multiBlockPos.GetParentPos(hitInfo.hit.blockPos, blockValue);
