...
public class XUiC_SignWindow : XUiController
{
	...
	public override void OnOpen()
	{
		...
		xui.playerUI.CursorController.SetNavigationLockView(base.ViewComponent);
	}
	...
	public override void OnClose()
	{
		...
		if (GameManager.Instance.World.GetTileEntity(SignTileEntity.ToWorldPos()).GetSelfOrFeature<ITileEntitySignable>() != SignTileEntity)
		{
			FinishClosing();
			return;
		}
		...
		{
			GameManager.ShowTooltip(xui.playerUI.entityPlayer, "uiInvalidCharacters");
			FinishClosing();
			return;
		}
		PersistentPlayerData playerDataFromEntityID = GameManager.Instance.persistentPlayers.GetPlayerDataFromEntityID(xui.playerUI.entityPlayer.entityId);
