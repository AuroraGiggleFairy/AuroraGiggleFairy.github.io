...
public class ObjectiveRallyPoint : BaseObjective
{
	...
	public IEnumerator setRallyPointVisibility(bool visible)
	{
		...
		if (bd != null)
		{
			...
			{
				world.SetBlockRPC(rallyPos, bvRallyType);
			}
		}
		else
		{
	...
	public void RallyPointActivate(Vector3 prefabPos, bool activate, QuestEventManager.POILockoutReasonTypes lockoutReason, ulong extraData)
	{
		...
		if (activate)
		{
			...
			player = base.OwnerQuest.OwnerJournal.OwnerPlayer;
			if (flag)
			{
				...
				if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
				{
					ThreadManager.StartCoroutine(RallyPointActivatedCoroutine());
				}
				else
				{
					OutstandingRallyPoint = this;
					SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageQuestEvent>().Setup(NetPackageQuestEvent.QuestEventTypes.LockPOI, player.entityId, base.OwnerQuest.ID, base.OwnerQuest.QuestTags, prefabPos, base.OwnerQuest.GetSharedWithIDList()));
				}
			}
			else
			{
			...
			if (activateEvent != "")
			{
				GameEventManager.Current.HandleAction(activateEvent, null, player, twitchActivated: false, new Vector3i(prefabPos));
			}
			return;
		}
		EntityPlayer ownerPlayer = base.OwnerQuest.OwnerJournal.OwnerPlayer;
		...
		case QuestEventManager.POILockoutReasonTypes.Bedroll:
			GameManager.ShowTooltip(ownerPlayer as EntityPlayerLocal, Localization.Get("poiLockoutBedroll"));
			break;
		case QuestEventManager.POILockoutReasonTypes.LandClaim:
			GameManager.ShowTooltip(ownerPlayer as EntityPlayerLocal, Localization.Get("poiLockoutLandClaim"));
			break;
		case QuestEventManager.POILockoutReasonTypes.PlayerInside:
			GameManager.ShowTooltip(ownerPlayer as EntityPlayerLocal, Localization.Get("poiLockoutPlayerInside"));
			break;
		case QuestEventManager.POILockoutReasonTypes.QuestLock:
		{
			if (extraData == 0L)
			{
				GameManager.ShowTooltip(ownerPlayer as EntityPlayerLocal, Localization.Get("poiLockoutQuestActiveQuesters"));
				break;
			}
			(int Days, int Hours, int Minutes) tuple = GameUtils.WorldTimeToElements(extraData);
			int item = tuple.Hours;
			int item2 = tuple.Minutes;
			GameManager.ShowTooltip(ownerPlayer as EntityPlayerLocal, Localization.Get("ttQuestLockedUntil"), $"{item:00}:{item2 + 1:00}");
			break;
		}
		}
		[PublicizedFrom(EAccessModifier.Internal)]
		IEnumerator RallyPointActivatedCoroutine()
		{
			yield return QuestEventManager.Current.QuestLockPOI(player.entityId, base.OwnerQuest.QuestClass, prefabPos, base.OwnerQuest.QuestTags, base.OwnerQuest.GetSharedWithIDList());
			RallyPointActivated();
			if (activateEvent != "")
			{
				GameEventManager.Current.HandleAction(activateEvent, null, player, twitchActivated: false, new Vector3i(prefabPos));
	...
	public Vector3 HandleRallyPoint()
	{
			...
			else
			{
				...
				if ((rallyPos = GetRallyPosition(world, prefabPosition, new Vector3i(pos))) != Vector3i.zero)
				{
					...
					if (world.GetChunkFromWorldPos(rallyPos) is Chunk chunk)
					{
						...
						{
							world.SetBlockRPC(rallyPos, blockValue);
							return Vector3.zero;
						}
		...
		else
		{
			...
			else
			{
				...
				{
					world2.SetBlockRPC(rallyPos, blockValue);
					return Vector3.zero;
				}
