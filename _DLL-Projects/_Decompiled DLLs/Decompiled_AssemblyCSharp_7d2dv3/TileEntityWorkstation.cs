...
public class TileEntityWorkstation : TileEntity
{
	...
	public new const int Version = 49;
	...
	public override void OnSetLocalChunkPosition()
	{
		...
		{
			string text = block.Properties.GetString("Workstation", "InputMaterials");
			if (text.Contains(","))
			{
		...
		{
			string text2 = block.Properties.GetString("Workstation", "Modules");
	...
	public void writeItemStackArrayDelta(BinaryWriter bw, ItemStack[] stack, ItemStack[] lastStack)
	{
		...
		if (stack != null)
		{
			for (int i = 0; i < stack.Length; i++)
			{
				stack[i].WriteDelta(bw, (lastStack != null) ? lastStack[i] : ItemStack.Empty);
			}
		}
	}
	...
	public override void OnLockedLocal(bool _success, ILockContext _context, ushort _channel)
	{
		...
		string blockName = GameManager.Instance.World.GetBlock(ToWorldPos()).Block.GetBlockName();
		...
		if (workstationData != null)
		{
			...
			if (blockName == "forge" && XUiM_Recipes.DisableSmelter)
			{
				text += "_nosmelting";
			}
			if (uIForPrimaryPlayer.windowManager.TryGetWindow(text, out var _window))
			{
				((XUiC_WorkstationWindowGroup)((XUiWindowGroup)_window).Controller).SetTileEntity(this);
				uIForPrimaryPlayer.windowManager.Open(text, _bModal: true);
			}
			else
			{
