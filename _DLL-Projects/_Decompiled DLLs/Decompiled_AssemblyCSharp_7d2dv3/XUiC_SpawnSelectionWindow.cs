...
public class XUiC_SpawnSelectionWindow : XUiController
{
	...
	public override bool ParseAttribute(string _name, string _value)
	{
		...
		{
			xui.LoadData(_value, [PublicizedFrom(EAccessModifier.Private)] (AudioClip _clip) =>
			{
	...
	{
		xui.playerUI.xui.PlayMenuConfirmSound();
		spawnMethod = _method;
		if (bEnteringGame)
		{
	...
	public override void Update(float _dt)
	{
		...
		if (IsDirty)
		{
			RefreshButtons();
			RefreshBindings();
			if (xui.playerUI.CursorController.navigationTarget == null && !xui.playerUI.CursorController.CursorModeActive)
			{
	...
	public override void OnOpen()
	{
		...
		lblProgress.Text = Localization.Get("msgBuildingEnvironment");
	...
	public void updateLoadState()
	{
		...
		if (displayedChunkGameObjectsCount < num)
		{
			if (!SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
			{
				lblProgress.Text = Localization.Get("msgStartingGame");
			}
		}
		else if (DistantTerrain.Instance != null && !DistantTerrain.Instance.IsTerrainReady)
		{
			...
			{
				lblProgress.Text = Localization.Get("msgGeneratingDistantTerrain");
			}
		}
		else if (!LocalPlayerUI.GetUIForPrimaryPlayer().xui.IsReady)
		{
			lblProgress.Text = Localization.Get("msgLoadingUI");
		}
	...
	public void showSpawningComponents(ESpawnWindowMode _windowMode)
	{
		...
		if (_windowMode == ESpawnWindowMode.SpawnSelection && AutomationRunner.Instance.IsRunning)
		{
			SpawnButtonPressed(SpawnMethod.Invalid);
		}
		else if (!setCursor)
