...
public class XUiC_ChatOutput : XUiController
{
	...
	public void addMessage(EnumGameMessages _messageType, EChatType _chatType, EChatDirection _chatDirection, string _message, string _senderDisplayName, string _senderHandlerId)
	{
		...
		if (_messageType == EnumGameMessages.Chat)
		{
			...
			string text = (string.IsNullOrEmpty(_senderDisplayName) ? "" : (LabelUrlUtils.BuildUrlFunctionString("Chat", (key: "ChatType", value: _chatType.ToStringCached()), (key: "Sender", value: _senderHandlerId)) + _senderDisplayName + "[/url]: "));
			...
			{
				EChatType.Global => "[" + textColorGlobal + "]" + text + text2 + LabelUrlUtils.BuildUrlFunctionString("Chat", (key: "ChatType", value: _chatType.ToStringCached())) + _message + "[/url]", 
				EChatType.Friends => "[" + textColorFriends + "]" + LabelUrlUtils.BuildUrlFunctionString("Chat", (key: "ChatType", value: _chatType.ToStringCached())) + "(Friends)[/url] " + text + text2 + _message, 
				EChatType.Party => "[" + textColorParty + "]" + LabelUrlUtils.BuildUrlFunctionString("Chat", (key: "ChatType", value: _chatType.ToStringCached())) + "(Party)[/url] " + text + text2 + _message, 
	...
	{
		base.Update(_dt);
		bool flag = xui.playerUI.windowManager.IsWindowOpen(XUiC_Chat.ID);
		collider.enabled = flag;
		if (flag)
		{
			currentWaitTime = fadeoutWaitTime + fadeoutDuration;
		}
		txtOutput.Alpha = Mathf.Lerp(0f, 1f, currentWaitTime / fadeoutDuration);
		currentWaitTime -= Time.deltaTime;
		if (GameManager.Instance == null || GameManager.Instance.World == null || xui.playerUI.entityPlayer == null || xui.playerUI.entityPlayer.IsDead())
		{
			txtOutput.Alpha = 0f;
		}
	}
