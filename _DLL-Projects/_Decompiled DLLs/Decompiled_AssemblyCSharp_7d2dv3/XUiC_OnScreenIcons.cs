...
public class XUiC_OnScreenIcons : XUiController
{
	...
	{
		if (screenIconList.Count == 0 || !ShowIcons)
		{
			return;
		}
		...
		for (int num = screenIconList.Count - 1; num >= 0; num--)
		{
			...
			onScreenIcon.Update(offset, position, forward, xui, ref depth);
	...
	{
		base.OnOpen();
		Camera = xui.playerUI.entityPlayer.playerCamera;
	}
