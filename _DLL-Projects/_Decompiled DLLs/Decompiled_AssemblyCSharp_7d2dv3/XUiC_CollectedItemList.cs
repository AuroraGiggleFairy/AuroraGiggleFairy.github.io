...
public class XUiC_CollectedItemList : XUiController
{
	...
	public override void Init()
	{
		...
		xui.CollectedItemList = this;
	}

	public override void Cleanup()
	{
		base.Cleanup();
		xui.CollectedItemList = null;
	}
	...
	public override void Update(float _dt)
	{
		...
		base.ViewComponent.IsVisible = !localPlayer.IsDead() && xui.playerUI.windowManager.IsHUDEnabled();
	...
	public void AddIconNotification(string iconNotifier, int count = 1, bool _bAddOnlyIfNotExisting = false)
	{
		...
		if (!(gameObject == null))
		{
			...
			if (component != null)
			{
				component.atlas = xui.GetAtlasByName("UIAtlas", iconNotifier);
	...
	public void AddCraftingSkillNotification(ProgressionValue craftingSkill, bool _bAddOnlyIfNotExisting = false)
	{
		...
		if (!(gameObject == null))
		{
			...
			if (component != null)
			{
				component.atlas = xui.GetAtlasByName("UIAtlas", craftingSkill.ProgressionClass.Icon);
	...
	public void AddItemStack(ItemStack _is, bool _bAddOnlyIfNotExisting = false)
	{
				...
				if (items[num].isNegative == flag)
				{
					if (_is.count != 0 && !items[num].isMissingNotifier)
					{
						...
						if (transform != null)
						{
							...
							if (componentInChildren != null)
							{
								componentInChildren.text = string.Format("{0} ({1})", (items[num].ItemStack.count >= 1) ? ("+" + items[num].ItemStack.count) : items[num].ItemStack.count.ToString(), xui.PlayerInventory.GetItemCountWithMods(items[num].ItemStack.itemValue));
							}
						}
						items[num].TimeAdded = Time.time;
						return;
		...
		else
		{
			...
			if (uILabel2 != null)
			{
				uILabel2.text = string.Format("{0} ({1})", (_is.count >= 1) ? ("+" + _is.count) : _is.count.ToString(), xui.PlayerInventory.GetItemCountWithMods(_is.itemValue));
			}
		}
		UISprite component = gameObject.transform.Find("Icon").GetComponent<UISprite>();
		...
		if (component != null && itemClass != null)
		{
			...
			component.atlas = xui.GetAtlasByName("itemIconAtlas", propertyOverride);
	...
	public void RemoveItemStack(ItemStack _is)
	{
		...
		uILabel.text = string.Format("{0} ({1})", (_is.count >= 0) ? ("-" + _is.count) : _is.count.ToString(), xui.PlayerInventory.GetItemCountWithMods(_is.itemValue));
		...
		component.atlas = xui.GetAtlasByName(((Object)component.atlas).name, propertyOverride);
		component.spriteName = propertyOverride;
