...
public class ItemActionTextureBlock : ItemActionRanged
{
	...
	public void floodFill(World _world, ChunkCluster _cc, int _entityId, ItemActionTextureBlockData _actionData, PersistentPlayerData _lpRelative, int _sourcePaint, Vector3 _hitPosition, Vector3 _hitFaceNormal, Vector3 _dir1, Vector3 _dir2, int _channel)
	{
		...
		while (positionsToCheck.Count > 0)
		{
			...
			if (!Voxel.Raycast(_world, new Ray(origin, direction), magnitude, -555528205, 69, 0f))
			{
				continue;
			}
	...
	public IEnumerator fireShotLater(int _shotIdx, ItemActionDataRanged _actionData)
	{
		...
		if (itemActionTextureBlockData.bReplacePaintNextTime)
		{
			...
			for (int i = 0; i < 1; i++)
			{
				...
				int num = chunkCache.GetBlockFaceTexture(blockPos, blockFace, i);
		...
		case EnumPaintMode.Single:
			paintBlock(world, chunkCache, holdingEntity.entityId, itemActionTextureBlockData, blockPos, blockFace, bv, playerDataFromEntityID, itemActionTextureBlockData.channelMask);
			break;
		case EnumPaintMode.Fill:
		{
			...
			for (int j = 0; j < 1; j++)
			{
				...
				int num5 = chunkCache.GetBlockFaceTexture(blockPos, blockFace, j);
				if (itemActionTextureBlockData.idx != num5)
				{
					...
					if (itemActionTextureBlockData.idx != num5)
					{
						floodFill(world, chunkCache, holdingEntity.entityId, itemActionTextureBlockData, playerDataFromEntityID, num5, hitInfo.hit.pos, normalized2, vector3, vector4, j);
					}
				}
			}
			break;
		...
		case EnumPaintMode.Spray:
		{
			...
			for (float num3 = 0f - num2; num3 <= num2; num3 += 0.5f)
			{
				for (float num4 = 0f - num2; num4 <= num2; num4 += 0.5f)
				{
					...
					int hitMask = 69;
					if (Voxel.Raycast(world, new Ray(origin, direction), Range, -555528205, hitMask, 0f))
					{
	...
	public int getHitBlockFace(ItemActionDataRanged _actionData, out Vector3i blockPos, out BlockValue bv, out BlockFace blockFace, out WorldRayHitInfo hitInfo)
	{
		using (s_MarkerGetHitBlockFace.Auto())
		{
			...
			if (bv.ischild)
			{
				...
				bv = chunkCache.GetBlock(blockPos);
			}
			...
			if (_actionData is ItemActionTextureBlockData itemActionTextureBlockData)
			{
				using (s_MarkerAutoChannel.Auto())
				{
					if (itemActionTextureBlockData.bAutoChannel && blockShapeNew != null)
					{
						...
						return chunkCache.GetBlockFaceTexture(blockPos, blockFace, visualMeshChannelFromHitInfo);
					}
				}
				...
				for (int i = 0; i < 1; i++)
				{
					...
					{
						int blockFaceTexture = chunkCache.GetBlockFaceTexture(blockPos, blockFace, i);
			...
			return chunkCache.GetBlockFaceTexture(blockPos, blockFace, 0);
		}
	}
	...
	public void CopyBlockFromWorld(ItemActionDataRanged _actionData)
	{
		...
		if (chunkCache != null)
		{
			...
			if (blockValue.ischild)
			{
				...
				blockValue = chunkCache.GetBlock(vector3i);
			}
			if (blockValue.Block.MeshIndex == 0)
			{
				ItemValue itemValue = executeActionTarget.hit.blockValue.ToItemValue();
				itemValue.TextureFullArray = chunkCache.GetTextureFullArray(vector3i);
	...
	public void replacePaintInCurrentPrefab(Vector3i _blockPos, BlockFace _blockFace, int _searchPaintId, int _replacePaintId, int _channel)
	{
		...
		PrefabInstance prefabFromWorldPosInside = dynamicPrefabDecorator.GetPrefabFromWorldPosInside(_blockPos.x, _blockPos.z);
		if (prefabFromWorldPosInside == null)
		{
			return;
		}
		for (int i = prefabFromWorldPosInside.boundingBoxPosition.x; i <= prefabFromWorldPosInside.boundingBoxPosition.x + prefabFromWorldPosInside.boundingBoxSize.x; i++)
		{
			for (int j = prefabFromWorldPosInside.boundingBoxPosition.z; j <= prefabFromWorldPosInside.boundingBoxPosition.z + prefabFromWorldPosInside.boundingBoxSize.z; j++)
			{
				for (int k = 0; k < 256; k++)
				{
					...
					if (flag)
					{
						world.SetTexture(i, k, j, num, _channel);
					}
				}
			}
		}
