...
public static class ProjectileManager
{
	...
	public static bool RemoveProjectile(int _id, bool revive = false)
	{
		if (projectiles == null)
		{
			init();
		...
		if (!projectiles.dict.TryGetValue(_id, out var value))
		{
			return false;
		}
		...
		value.gameObject.layer = 0;
		value.tag = "Untagged";
		Transform transform = value.Find("Trail");
		if (transform != null)
		{
			transform.gameObject.SetActive(value: true);
		}
		if (value.TryGetComponent<ProjectileMoveScript>(out var component))
		{
			component.ProjectileID = -1;
			component.FinalPosition = Vector3.zero;
		}
		if (value.TryGetComponent<ProjectileMoveScript>(out var component2))
		{
			component2.ProjectileID = -1;
			component2.FinalPosition = Vector3.zero;
		}
		value.localScale = Vector3.one;
		value.SetParent(null);
		if (value.gameObject.TryGetComponent<CapsuleCollider>(out var component3))
		{
