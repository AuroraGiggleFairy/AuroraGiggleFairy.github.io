...
public class ItemActionExchangeItem : ItemAction
{
	...
	public override void ExecuteAction(ItemActionData _actionData, bool _bReleased)
	{
		...
		if (Voxel.Raycast(invData.world, lookRay, Constants.cDigAndBuildDistance, -538480653, 4095, 0f) && Voxel.voxelRayHitInfo.bHitValid && isFocusingBlock(Voxel.voxelRayHitInfo))
		{
	...
	public override void OnHoldingUpdate(ItemActionData _actionData)
	{
		if (_actionData.lastUseTime != 0f && !IsActionRunning(_actionData))
		{
			...
			{
				hitLiquidBlock.Block.DoExchangeAction(_actionData.invData.world, hitLiquidPos, hitLiquidBlock, doBlockAction, _actionData.invData.holdingEntity.inventory.holdingCount);
			}
			if (changeBlockTo != null)
			{
				Vector3i blockPos = ((EntityPlayerLocal)_actionData.invData.holdingEntity).HitInfo.hit.blockPos;
