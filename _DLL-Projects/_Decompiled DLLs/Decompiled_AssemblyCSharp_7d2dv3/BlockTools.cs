...
public class BlockTools
{
	...
	public static void PlaceTerrain(WorldBase _world, Vector3i _targetPos, Brush _brush, List<BlockChangeInfo> _blockChanges)
	{
		...
		for (int i = 0; i < cubesInBrush.Length; i++)
		{
			...
			if (!HasValidNeighbor(vector3i, 1, _world))
			{
				continue;
			}
			...
			{
				_blockChanges.Add(new BlockChangeInfo(vector3i, -1));
			}
	...
	public static void PaintTerrain(WorldBase _world, Vector3i _targetPos, Brush _brush, List<BlockChangeInfo> _blockChanges, BlockValue _paintBlock)
	{
	...
	public static void RemoveTerrain(WorldBase _world, Vector3i _targetPos, Brush _brush, List<BlockChangeInfo> _blockChanges)
	{
		BlockValue block = _world.GetBlock(_targetPos);
		Vector3i[] cubesInBrush = _brush.GetCubesInBrush();
		for (int i = 0; i < cubesInBrush.Length; i++)
	...
	public static bool HasValidNeighbor(Vector3i _pos, sbyte _denThreshold, WorldBase _world)
	{
		if (_denThreshold < 0)
		{
			if (_world.GetDensity(_pos.x - 1, _pos.y, _pos.z) <= _denThreshold)
			{
				return true;
			}
			if (_world.GetDensity(_pos.x + 1, _pos.y, _pos.z) <= _denThreshold)
			...
			if (_world.GetDensity(_pos.x, _pos.y - 1, _pos.z) <= _denThreshold)
			{
				return true;
			}
			if (_world.GetDensity(_pos.x, _pos.y + 1, _pos.z) <= _denThreshold)
			...
			if (_world.GetDensity(_pos.x, _pos.y, _pos.z - 1) <= _denThreshold)
			{
				return true;
			}
			if (_world.GetDensity(_pos.x, _pos.y, _pos.z + 1) <= _denThreshold)
	...
	public static void CubeDensityRPC(GameManager _gm, Vector3i pos1, Vector3i pos2, sbyte _density)
	{
		...
		while (true)
		{
			...
			while (true)
			{
				...
				while (true)
				{
					BlockChangeInfo item = new BlockChangeInfo(new Vector3i(num5, num4, num6), _density);
					list.Add(item);
					if (num6 == pos2.z)
					{
						break;
	...
	{
		byte result = 28;
		if ((_rotations & EBlockRotationClasses.Basic45) == 0)
		{
			result = 24;
		}
		if ((_rotations & EBlockRotationClasses.Sideways) == 0)
		{
			result = 8;
		}
		if ((_rotations & EBlockRotationClasses.Headfirst) == 0)
		{
			result = 4;
		}
		return result;
		...
		while (num8 <= num4)
		{
			...
			while (num10 <= num2)
			{
				...
				while (num12 <= num6)
				{
					...
					sbyte density = _gm.World.GetDensity(num10, num8, num12);
