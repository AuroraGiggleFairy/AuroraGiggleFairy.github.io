...
public class RegionFileManager : WorldChunkCache
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static readonly ProfilerMarker s_AddGroupedChunks = new ProfilerMarker("RegionFileManager.AddGroupedChunks");

	[PublicizedFrom(EAccessModifier.Private)]
	public static readonly ProfilerMarker s_RemoveGroupedChunks = new ProfilerMarker("RegionFileManager.RemoveGroupedChunks");

	[PublicizedFrom(EAccessModifier.Private)]
	public static readonly ProfilerMarker s_UpdateGroupTimestamps = new ProfilerMarker("RegionFileManager.UpdateGrouptimestamps");
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public LongSetGroups chunkGroups = new LongSetGroups();

	[PublicizedFrom(EAccessModifier.Private)]
	public Dictionary<LongSetGroups.Group, uint> groupTimestamps = new Dictionary<LongSetGroups.Group, uint>();

	[PublicizedFrom(EAccessModifier.Private)]
	public bool groupTimestampsDirty;

	[PublicizedFrom(EAccessModifier.Private)]
	public Dictionary<LongSetGroups.Group, ChunkProtectionLevel> groupProtectionLevels = new Dictionary<LongSetGroups.Group, ChunkProtectionLevel>();

	[PublicizedFrom(EAccessModifier.Private)]
	public HashSet<LongSetGroups.Group> processedChunkGroups = new HashSet<LongSetGroups.Group>();
	...
	public RegionFileManager(string _loadDirectory, string _saveDirectory, int _maxChunksInCache, bool _bSaveOnChunkDrop)
	{
		...
		ChunkGroups = chunkGroups.Groups;
	...
	public void SetChunkTimestamp(long chunkKey, uint timeStamp)
	{
		...
		if (!groupTimestampsDirty && chunkGroups.TryGetGroup(chunkKey, out var group) && (!groupTimestamps.TryGetValue(group, out var value) || value < timeStamp))
		{
			groupTimestamps[group] = timeStamp;
		}
	}
	...
	{
		if (chunkGroups.TryGetGroup(chunkKey, out var group) && groupTimestamps.TryGetValue(group, out var value))
		{
			return value;
		}
		if (chunksInSaveDir.TryGetValue(chunkKey, out var value2))
		{
			return value2;
		}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public void UpdateGroupTimestamps()
	{
		using (s_UpdateGroupTimestamps.Auto())
		{
			groupTimestamps.Clear();
			foreach (LongSetGroups.Group group in chunkGroups.Groups)
			{
				uint? num = null;
				foreach (long key in group.Keys)
				{
					if (chunksInSaveDir.TryGetValue(key, out var value) && (!num.HasValue || num.Value < value))
					{
						num = value;
					}
				}
				if (num.HasValue)
				{
					groupTimestamps.Add(group, num.Value);
				}
			}
			groupTimestampsDirty = false;
		}
	}
	...
	{
		if (chunkGroups.TryGetGroup(chunkKey, out var group) && groupProtectionLevels.TryGetValue(group, out var value))
		{
			return value;
		}
	...
	{
		return GetChunkProtectionLevel(WorldChunkCache.MakeChunkKey(chunkX, chunkZ));
	}

	public ChunkProtectionLevel GetChunkProtectionLevel(long chunkKey)
	{
		lock (saveLock)
		{
			lock (chunksInSaveDir)
			{
				...
				if (chunkProtectionLevels.TryGetValue(chunkKey, out var value))
				{
					return value;
				}
				return ChunkProtectionLevel.None;
	...
	public void RebuildChunkGroupsFromPOIs()
	{
		using (s_ComputeChunkGroups.Auto())
		{
			lock (chunksInSaveDir)
			{
				...
				dynamicPrefabsBuffer.Clear();
				dynamicPrefabDecorator.GetAllPrefabs(dynamicPrefabsBuffer);
				chunkGroups.Clear();
				chunksByTraderID.Clear();
				LongSetGroups.Group obj = null;
				foreach (PrefabInstance item in dynamicPrefabsBuffer)
				{
					...
					if (occupiedChunks.Count > 1)
					{
						...
						LongSetGroups.Group obj2 = chunkGroups.MergeOrCreateGroup(occupiedChunks);
						if (obj == null || obj2.Count > obj.Count)
						{
							obj = obj2;
						}
					}
				...
				Log.Out($"Computed {chunkGroups.Count} chunk groups containing a total of {chunkGroups.GroupedLongsCount} chunks. Largest group contains {obj?.Count} chunks.");
			}
		}
	}
	...
	{
		using (s_AddGroupedChunks.Auto())
		{
			lock (chunksInSaveDir)
			{
				HashSetLong longsToGroup = new HashSetLong(chunksToGroup);
				chunkGroups.MergeOrCreateGroup(longsToGroup);
				protectionLevelsDirty = true;
				groupTimestampsDirty = true;
			}
		}
	}

	public void RemoveGroupedChunks(HashSetLong chunks)
	{
		using (s_RemoveGroupedChunks.Auto())
		{
			lock (chunksInSaveDir)
			{
				new List<LongSetGroups.Group>();
				chunkGroups.RemoveGroupedKeys(chunks);
				protectionLevelsDirty = true;
				groupTimestampsDirty = true;
			}
		}
	...
	public void RequestChunkReset(long _chunkKey)
	{
		lock (chunksInSaveDir)
		{
			if (chunkGroups.TryGetGroup(_chunkKey, out var group))
			{
				foreach (long key in group.Keys)
				{
					if (!resetRequestedChunks.Contains(key))
					{
						resetRequestedChunks.Add(key);
					}
				}
				return;
			}
	...
	public void CullExpiredChunks()
	{
		...
		{
			if ((protectionLevel & ~(ChunkProtectionLevel.NearOfflinePlayer | ChunkProtectionLevel.NearQuestObjective | ChunkProtectionLevel.OfflinePlayer | ChunkProtectionLevel.QuestObjective | ChunkProtectionLevel.CurrentlySynced)) != ChunkProtectionLevel.None)
			{
	...
	public void MakeRoomForChunk(long chunkSizeInBytes, long chunkKey)
	{
		lock (saveLock)
		{
			lock (chunksInSaveDir)
			{
				...
				if (!chunkGroups.TryGetGroup(chunkKey, out var group))
				{
					group = null;
				}
				if (protectionLevelsDirty)
				...
				using (s_SortCullingCandidates.Auto())
				{
					...
					foreach (long key in chunksInSaveDir.Keys)
					{
						if (key != chunkKey && (group == null || !group.Contains(chunkKey)))
						{
							ulong num4 = GetChunkTimestamp(key);
							if (chunkProtectionLevels.TryGetValue(key, out var value))
							{
								num4 += (ulong)((long)value << 32);
							}
				...
				using (s_CollectBestCandidates.Auto())
				{
					...
					foreach (KeyValuePair<long, ulong> sortedCullingCandidate in sortedCullingCandidates)
					{
						...
						if (chunkGroups.TryGetGroup(sortedCullingCandidate.Key, out var group2))
						{
							if (processedChunkGroups.Contains(group2))
							{
								continue;
							}
							foreach (long key2 in group2.Keys)
							{
								if (chunksInSaveDir.ContainsKey(key2))
								{
									expiredChunks.Add(key2);
									int chunkByteCount = GetChunkByteCount(key2);
									num5 += chunkByteCount;
								}
							}
							processedChunkGroups.Add(group2);
						}
						else
						{
			...
			lock (chunksInSaveDir)
			{
				...
				using (s_CollectBestCandidates.Auto())
				{
					...
					foreach (KeyValuePair<long, ulong> sortedCullingCandidate in sortedCullingCandidates)
					{
						...
						if (chunkGroups.TryGetGroup(sortedCullingCandidate.Key, out var group))
						{
							if (processedChunkGroups.Contains(group))
							{
								continue;
							}
							foreach (long key2 in group.Keys)
							{
								if (chunksInSaveDir.ContainsKey(key2))
								{
									expiredChunks.Add(key2);
							...
							processedChunkGroups.Add(group);
						}
						else
						{
							expiredChunks.Add(sortedCullingCandidate.Key);
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public void UpdateChunkProtectionLevels(EnumResetUnprotectedChunksGroupingMode _groupingMode = EnumResetUnprotectedChunksGroupingMode.GroupedPOIs)
	{
		using (s_UpdateProtectionLevels.Auto())
		{
			chunkProtectionLevels.Clear();
			...
			{
			case EnumResetUnprotectedChunksGroupingMode.GroupedPOIs:
				foreach (var (obj2, chunkProtectionLevel3) in groupProtectionLevels)
				{
					foreach (long key in obj2.Keys)
					{
						if (chunkProtectionLevels.TryGetValue(key, out var value2) && value2 != (value2 & chunkProtectionLevel3))
						{
							Log.Error($"Error in chunk group protection: member has protections not accounted for by the group. chunkKey: {key}, protectionLevel: {value2}, groupProtectionLevel: {chunkProtectionLevel3})");
						}
						chunkProtectionLevels[key] = chunkProtectionLevel3;
					}
				}
				break;
			case EnumResetUnprotectedChunksGroupingMode.SeparatePOIs:
			{
				List<ChunkProtectionLevel> list = new List<ChunkProtectionLevel>(dynamicPrefabsBuffer.Count);
				for (int i = 0; i < dynamicPrefabsBuffer.Count; i++)
				{
					ChunkProtectionLevel chunkProtectionLevel = ChunkProtectionLevel.None;
					foreach (long occupiedChunk in dynamicPrefabsBuffer[i].GetOccupiedChunks())
					{
						if (chunkProtectionLevels.TryGetValue(occupiedChunk, out var value))
						{
							chunkProtectionLevel |= value;
						}
					}
					list.Add(chunkProtectionLevel);
				}
				for (int j = 0; j < dynamicPrefabsBuffer.Count; j++)
				{
					HashSetLong occupiedChunks = dynamicPrefabsBuffer[j].GetOccupiedChunks();
					if (list[j] == ChunkProtectionLevel.None)
					{
						continue;
					}
					foreach (long item in occupiedChunks)
					{
						AddProtectionLevel(item, list[j]);
					}
				}
				break;
			}
			}
			protectionLevelsDirty = _groupingMode != EnumResetUnprotectedChunksGroupingMode.GroupedPOIs;
		...
		void AddChunksInWorldArea(Vector3i centerPos, int halfSize, ChunkProtectionLevel innerProtectionLevel, int margin, ChunkProtectionLevel marginProtectionLevel)
		{
			...
			for (int k = num - margin; k <= num2 + margin; k++)
			{
				for (int l = num3 - margin; l <= num4 + margin; l++)
				{
					ChunkProtectionLevel protectionLevel = ((k < num || k > num2 || l < num3 || l > num4) ? marginProtectionLevel : innerProtectionLevel);
					long chunkKey = WorldChunkCache.MakeChunkKey(k, l);
		...
		void AddProtectionLevel(long chunkKey, ChunkProtectionLevel protectionLevel)
		{
			...
			{
				if (protectionLevel == (protectionLevel & value3))
				{
					return;
				}
				chunkProtectionLevels[chunkKey] = value3 | protectionLevel;
			}
			else
			{
			...
			if (chunkGroups.TryGetGroup(chunkKey, out var group))
			{
				if (groupProtectionLevels.TryGetValue(group, out var value4))
				{
					groupProtectionLevels[group] = value4 | protectionLevel;
				}
				else
				{
					groupProtectionLevels[group] = protectionLevel;
				}
			}
		}
		[PublicizedFrom(EAccessModifier.Private)]
		void EvaluateSingleChunkProtectionLevels()
		{
			...
			lock (chunksToSave)
			{
				foreach (long key2 in chunksToSave.Keys)
				{
			...
			{
				foreach (long key3 in chunkMemoryStreamsToSave.Keys)
				{
					AddProtectionLevel(key3, ChunkProtectionLevel.CurrentlySynced);
				}
			}
			foreach (HashSetLong value5 in chunksByTraderID.Values)
			{
				using HashSetLong.Enumerator enumerator4 = value5.GetEnumerator();
				if (enumerator4.MoveNext())
				{
					long current7 = enumerator4.Current;
					AddProtectionLevel(current7, ChunkProtectionLevel.Trader);
				}
			}
			lock (ppdPositionCache)
			{
	...
	public bool DoSaveChunks()
	{
		...
		try
		{
			...
			if (regionFileChunkSnapshot != null)
			{
				...
				if (!SaveDataUtils.SaveDataManager.AppliesSaveSizeLimit)
				{
	...
	public void RemoveChunks(ICollection<long> _chunks, bool _resetDecos = true, bool _optimizeFileLayouts = true)
	{
		if (_chunks.Count == 0)
		{
			return;
		...
		lock (saveLock)
		{
			lock (chunksInSaveDir)
			{
				...
				ThreadManager.AddSingleTaskMainThread("RemoveChunks.ProcessChunkDeletionPackages", [PublicizedFrom(EAccessModifier.Private)] () =>
				{
					ProcessChunkDeletionPackages();
				});
				if (_optimizeFileLayouts)
				{
					regionFileAccess.OptimizeLayouts();
				}
			}
	...
	public HashSetLong ResetAllChunks(ChunkProtectionLevel excludedProtectionLevels, EnumResetUnprotectedChunksGroupingMode _groupingMode = EnumResetUnprotectedChunksGroupingMode.GroupedPOIs)
	{
		lock (saveLock)
		{
			lock (chunksInSaveDir)
			{
				UpdateChunkProtectionLevels(_groupingMode);
	...
	public bool TryResetChunks(ICollection<long> _chunks, ChunkProtectionLevel _excludedProtectionLevels, out ChunkProtectionLevel _protectionLevels)
	{
		if (_chunks.Count == 0)
		{
			_protectionLevels = ChunkProtectionLevel.None;
			return true;
		}
		lock (saveLock)
		{
			lock (chunksInSaveDir)
			{
				foreach (long _chunk in _chunks)
				{
					ChunkProtectionLevel chunkProtectionLevel = GetChunkProtectionLevel(_chunk);
					_protectionLevels = chunkProtectionLevel & _excludedProtectionLevels;
					if (_protectionLevels != ChunkProtectionLevel.None)
					{
						return false;
					}
				}
				RemoveChunks(_chunks, _resetDecos: true, _optimizeFileLayouts: false);
				_protectionLevels = ChunkProtectionLevel.None;
				return true;
			}
		}
	...
	public void SaveChunkAgeDebugTexture(float rangeInDays)
	{
		...
		lock (chunksInSaveDir)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				if (chunkGroups.TryGetGroup(key, out var group) && groupTimestamps.TryGetValue(group, out var value2))
				{
					long num7 = num4 - value2;
				...
				if (chunkProtectionLevels.TryGetValue(key, out var value3))
				{
					tuple.Item3 = (float)OrdinalProtectionLevel(value3) * num5;
				}
