...
public class XUiC_WorkstationMaterialInputWindow : XUiController
{
	...
	public override void OnOpen()
	{
		...
		{
			string text = MaterialNames[i].UppercaseFirst();
	...
	public bool HasRequirement(Recipe recipe)
	{
		...
		for (int i = 0; i < weights.Length; i++)
		{
			...
			{
				int num = (int)EffectManager.GetValue(PassiveEffects.CraftingIngredientCount, null, recipe.ingredients[j].count, xui.playerUI.entityPlayer, recipe, FastTags<TagGroup.Global>.Parse(recipe.ingredients[j].itemValue.ItemClass.GetItemName()), calcEquipment: true, calcHoldingItem: true, calcProgression: true, calcBuffs: true, calcChallenges: true, recipe.GetCraftingTier(xui.playerUI.entityPlayer));
				int type = recipe.ingredients[j].itemValue.type;
				if (num > 0)
				{
					num = (int)((float)num * XUiM_Recipes.GetCraftingInputModifier(recipe));
				}
				ItemClass forId = ItemClass.GetForId(type);
				if (forId == null)
				{
					continue;
				}
