...
public class CharacterGazeController : MonoBehaviour
{
	...
	public void UpdateHeadRotation()
	{
		...
		float num = Vector3.Angle(neckTransform.forward, normalized);
		Vector3 normalized2 = neckTransform.InverseTransformDirection(normalized).normalized;
		...
		Quaternion quaternion = Quaternion.AngleAxis(num2, neckTransform.up);
		Vector3 vector = quaternion * neckTransform.forward;
		Vector3 axis = quaternion * neckTransform.right;
		Vector3 normalized3 = (Quaternion.AngleAxis(0f - num4, axis) * vector).normalized;
		if (num < headLookAtTargetAngle && !isDead)
		{
			Quaternion b = Quaternion.FromToRotation(neckTransform.forward, normalized3);
			b *= Quaternion.Euler(0f, 0f, (0f - num2 / headLookAtTargetAngle) * 15f);
			if (shouldSnapHeadNextUpdate)
			{
				currentHeadRotation = b;
				shouldSnapHeadNextUpdate = false;
			}
			else
			{
				currentHeadRotation = Quaternion.Slerp(currentHeadRotation, b, headRotationSpeed * Time.deltaTime);
			}
		}
	...
	public void UpdateEyeGaze()
	{
		...
		eyeMaterial.SetVector("_LeftEyePosition", Vector3.Scale(leftEyeLocalPosition, leftEyeTransform.lossyScale));
		eyeMaterial.SetVector("_RightEyePosition", Vector3.Scale(rightEyeLocalPosition, rightEyeTransform.lossyScale));
