...
public class ItemActionEntryScrap(XUiController _controller) : BaseItemActionEntry(_controller, "lblContextActionScrap", "ui_game_symbol_scrap", GamepadShortCut.DPadRight)
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static FastTags<TagGroup.Global> scrap100Tag = FastTags<TagGroup.Global>.Parse("scrap100");
	...
	public override void OnActivated()
	{
		...
		if (num2 > 0)
		{
			...
			float num4 = ((XUiM_Recipes.DisableSmelter && forId2.HasAnyTags(scrap100Tag)) ? 1f : 0.75f);
			num2 = (int)((float)num2 * (num4 * XUiM_Recipes.ScrappingOutputModifier));
			if (num2 <= 0)
			{
				num2 = 1;
			}
			...
			((XUiC_ItemStack)base.ItemController).ItemStack = ((itemStack.count <= 0) ? ItemStack.Empty : itemStack.Clone());
