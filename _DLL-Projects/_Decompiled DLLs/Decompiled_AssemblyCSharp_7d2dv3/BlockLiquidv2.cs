...
public class BlockLiquidv2 : Block
{
	...
	public override void OnNeighborBlockChange(WorldBase world, Vector3i _myBlockPos, BlockValue _myBlockValue, Vector3i _blockPosThatChanged, BlockValue _newNeighborBlockValue, BlockValue _oldNeighborBlockValue)
	{
		...
		if (_newNeighborBlockValue.isair)
		{
			...
			{
				ChangeThis(world, _myBlockValue, _myBlockPos, UpdateID.Awake, Emissions(_myBlockValue), 0, FlowDirection.West);
			}
			else if (_myBlockPos.x - _blockPosThatChanged.x < 0)
			{
				ChangeThis(world, _myBlockValue, _myBlockPos, UpdateID.Awake, Emissions(_myBlockValue), 0, FlowDirection.East);
			}
			else if (_myBlockPos.z - _blockPosThatChanged.z < 0)
			{
				ChangeThis(world, _myBlockValue, _myBlockPos, UpdateID.Awake, Emissions(_myBlockValue), 0, FlowDirection.North);
			}
			else if (_myBlockPos.z - _blockPosThatChanged.z > 0)
			{
				ChangeThis(world, _myBlockValue, _myBlockPos, UpdateID.Awake, Emissions(_myBlockValue), 0, FlowDirection.South);
			}
			else
			{
				ChangeThis(world, _myBlockValue, _myBlockPos, UpdateID.Awake, Emissions(_myBlockValue), Evap(_myBlockValue), FlowDirection.None);
			}
		}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public void ChangeThis(WorldBase _world, BlockValue _blockValue, Vector3i _blockPos, UpdateID _blockState)
	{
		ChangeThis(_world, _blockValue, _blockPos, _blockState, Emissions(_blockValue), Evap(_blockValue), Flow(_blockValue));
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void ChangeThis(WorldBase _world, BlockValue _blockValue, Vector3i _blockPos, UpdateID _blockState, int _emissions, int _evaporation)
	{
		ChangeThis(_world, _blockValue, _blockPos, _blockState, _emissions, 0, Flow(_blockValue));
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void ChangeThis(WorldBase _world, BlockValue _blockValue, Vector3i _blockPos, UpdateID _blockState, int _emissions, int _evaporation, FlowDirection _flowDirection)
	{
		...
		_world.SetBlockRPC(_blockPos, _blockValue2);
		ulong ticks = 1000uL;
		switch (_blockState)
		{
		case UpdateID.Awake:
		...
		{
			_world.GetWBT().AddScheduledBlockUpdate(_blockPos, blockID, ticks);
		}
	}
	...
	public override void DoExchangeAction(WorldBase _world, Vector3i _blockPos, BlockValue _blockValue, string _action, int _itemCount)
	{
		...
		try
		{
			...
			if (num2 > Emissions(_blockValue))
			{
				...
				if (num3 > 0)
				{
					...
					for (int i = 0; i < 4; i++)
					{
						...
						if (num3 <= num4)
						{
							if (num4 == 0)
							{
								ChangeToAir(_world, zero);
								num3--;
							}
							else
							{
								ChangeThis(_world, block, zero, UpdateID.Awake, Mathf.Clamp(Emissions(block) - num3, 0, MAX_EMISSIONS), Evap(block), Flow(block));
							}
							break;
						}
						ChangeToAir(_world, zero);
				...
				ChangeToAir(_world, _blockPos);
			}
			else
			{
				ChangeThis(_world, _blockValue, _blockPos, UpdateID.Awake, Mathf.Clamp(Emissions(_blockValue) - num2, 0, MAX_EMISSIONS), Evap(_blockValue), Flow(_blockValue));
			}
		}
		catch (Exception ex)
		{
	...
	public override bool UpdateTick(WorldBase world, Vector3i _blockPos, BlockValue _blockValue, bool _bRandomTick, ulong _ticksIfLoaded, GameRandom _rnd)
	{
		...
		if (HasHoles(air))
		{
			...
			if (IsAir(air) || IsPlant(air))
			{
				if (Emissions(_blockValue) > 0 && CheckUpdate())
				{
					...
					{
						ChangeThis(world, _blockValue, _blockPos, UpdateID.Sleep, 0, 0, FlowDirection.None);
					}
					ChangeThis(world, _blockValue, zero2, UpdateID.Awake, Emissions(_blockValue) - 1, 0, FlowDirection.None);
				}
				else
				{
					ChangeToAir(world, _blockPos);
					ChangeThis(world, _blockValue, zero2, UpdateID.Awake, Emissions(_blockValue), 0, FlowDirection.None);
				}
				return true;
			}
			zero2.x = _blockPos.x;
		...
		for (int k = 0; k < 4; k++)
		{
			...
			if (Emissions(_blockValue) > 0)
			{
				if (!CheckDeepWater_Expensive(world, _blockPos))
				{
					ChangeThis(world, _blockValue, _blockPos, UpdateID.Sleep, 0, 0, GetFlowDirection(fallDirsSet[fallSet, k]));
				}
				ChangeThis(world, _blockValue, zero, UpdateID.Awake, Emissions(_blockValue) - 1, 0, GetFlowDirection(fallDirsSet[fallSet, k]));
			}
			else
			{
				ChangeToAir(world, _blockPos);
				ChangeThis(world, _blockValue, zero, UpdateID.Awake, Emissions(_blockValue), 0, GetFlowDirection(fallDirsSet[fallSet, k]));
			}
			return true;
		}
		if (Emissions(_blockValue) > 0)
		{
			if (IsWater(air) && Emissions(air) < MAX_EMISSIONS)
			{
				if (!CheckDeepWater_Expensive(world, _blockPos))
				{
					ChangeThis(world, _blockValue, _blockPos, UpdateID.Awake, Emissions(_blockValue) - 1, 0, FlowDirection.None);
				}
				ChangeThis(world, air, zero2, UpdateID.Awake, Emissions(air) + 1, 0);
				return true;
			}
			for (int l = 0; l < 4; l++)
			{
				...
				if (IsAir(air2) || IsPlant(air2))
				{
					if (!CheckDeepWater_Expensive(world, _blockPos))
					{
						ChangeThis(world, _blockValue, _blockPos, UpdateID.Awake, 0, 0, GetFlowDirection(fallDirsSet[fallSet, l]));
					}
					ChangeThis(world, _blockValue, zero, UpdateID.Awake, Emissions(_blockValue) - 1, 0, GetFlowDirection(fallDirsSet[fallSet, l]));
					return true;
				}
				if (IsWater(air2) && Emissions(_blockValue) - Emissions(air2) - 1 > 0)
				{
					...
					ChangeThis(world, air2, zero, UpdateID.Awake, Emissions(air2) + 1, 0);
					return true;
				}
			}
		}
		...
		for (int m = 0; m < 4; m++)
		{
			...
			{
				ChangeToAir(world, _blockPos);
				ChangeThis(world, air2, zero + Vector3i.down, UpdateID.Awake, Emissions(air2) + 1, 0);
				return true;
			}
		}
		if (Emissions(_blockValue) < MAX_EMISSIONS)
		{
			...
			if (IsWater(air2))
			{
				if (Emissions(air2) == 0)
				{
					ChangeToAir(world, _blockPos + Vector3i.up);
					ChangeThis(world, _blockValue, _blockPos, UpdateID.Awake, Emissions(_blockValue) + 1, 0);
					return true;
				}
				ChangeThis(world, air2, _blockPos + Vector3i.up, UpdateID.Awake, Emissions(air2) - 1, 0);
				ChangeThis(world, _blockValue, _blockPos, UpdateID.Awake, Emissions(_blockValue) + 1, 0);
				return true;
			}
		}
		...
		if (Emissions(_blockValue) < MAX_EMISSIONS - 1)
		{
			for (int num = 0; num < 4; num++)
			{
				...
				if (IsWater(air2) && Emissions(air2) >= 2 - (flag ? 1 : 0) + Emissions(_blockValue))
				{
					...
					if (flag)
					{
						ChangeThis(world, _blockValue, blockPos4, UpdateID.Awake, 0, 0, flowDirection);
					}
					else
					{
						ChangeThis(world, _blockValue, _blockPos, UpdateID.Awake, Emissions(_blockValue) + 1, 0);
		...
		if (Emissions(_blockValue) > 0)
		{
			for (int num2 = 0; num2 < 4; num2++)
			{
				...
				if (HasHoles(air2))
				{
					...
					if (IsAir(air2) || IsPlant(air2))
					{
						ChangeThis(world, _blockValue, _blockPos, UpdateID.Awake, Emissions(_blockValue) - 1, 0, GetFlowDirection(fallDirsSet[fallSet, num2]));
						ChangeThis(world, air2, zero, UpdateID.Awake, 0, 0, GetFlowDirection(fallDirsSet[fallSet, num2]));
						return true;
					}
		...
		if (flag)
		{
			if (State(_blockValue) == UpdateID.Evaporate)
			{
				...
				ChangeThis(world, _blockValue, _blockPos, UpdateID.Sleep);
				return true;
			}
	...
	public override void OnBlockAdded(WorldBase world, Chunk _chunk, Vector3i _blockPos, BlockValue _blockValue, PlatformUserIdentifierAbs _addedByPlayer)
	{
		...
		{
			int count = Count;
			ChangeThis(world, _blockValue, _blockPos, UpdateID.Awake, count, 0, FlowDirection.None);
			if (GameTimer.Instance.elapsedTicks > 0)
			{
				_blockValue.meta2 = (byte)MAX_EMISSIONS;
				_blockValue.rotation = 8;
				UpdateTick(world, _blockPos, _blockValue, _bRandomTick: false, 0uL, null);
			}
		}
		else
		{
			ChangeThis(world, _blockValue, _blockPos, UpdateID.Awake, _blockValue.meta2, 0, FlowDirection.None);
			if (GameTimer.Instance.elapsedTicks > 0)
			{
				UpdateTick(world, _blockPos, _blockValue, _bRandomTick: false, 0uL, null);
			}
