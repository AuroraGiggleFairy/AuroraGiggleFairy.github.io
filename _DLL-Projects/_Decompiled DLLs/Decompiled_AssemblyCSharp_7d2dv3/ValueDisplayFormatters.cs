...
public static class ValueDisplayFormatters
{
	...
	public static string WorldPosLatitude(Vector3 _pos, bool _useLongGeoDirs = false)
	{
		...
		string arg = Localization.Get("geoDirection" + ((_pos.z >= 0f) ? "N" : "S") + (_useLongGeoDirs ? "Long" : ""), _caseInsensitive: false, cardinalDirectionsLanguage);
		string arg2 = Utils.FastAbs(_pos.z).ToCultureInvariantString("0");
		return string.Format(Localization.Get("geoLocationSingleAxis", _caseInsensitive: false, cardinalDirectionsLanguage), arg2, arg);
	}

	public static string WorldPosLongitude(Vector3 _pos, bool _useLongGeoDirs = false)
	{
		string cardinalDirectionsLanguage = CardinalDirectionsLanguage;
		string arg = Localization.Get("geoDirection" + ((_pos.x >= 0f) ? "E" : "W") + (_useLongGeoDirs ? "Long" : ""), _caseInsensitive: false, cardinalDirectionsLanguage);
	...
	{
		string cardinalDirectionsLanguage = CardinalDirectionsLanguage;
		return Localization.Get("geoDirection" + _direction.ToStringCached() + (_useLongGeoDirs ? "Long" : ""), _caseInsensitive: false, cardinalDirectionsLanguage);
	}
	...
	public static string MemoryMiB(long _bytes)
	{
		return _bytes / 1024 / 1024 + " MiB";
	}
}
