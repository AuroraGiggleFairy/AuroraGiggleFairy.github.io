...
public class ChallengeJournal
{
	...
	public void StartChallenges(EntityPlayerLocal player)
	{
		...
		{
			Challenge challenge = Challenges[i];
			if ((IntroChallengesEnabled || !challenge.ChallengeClass.ChallengeGroup.IsIntro) && challenge.ChallengeClass.ChallengeGroup.IsActive)
			{
	...
	public void CompleteIntroChallenges()
	{
		for (int i = 0; i < Challenges.Count; i++)
		{
			...
			if (challenge.ChallengeState != Challenge.ChallengeStates.Redeemed && challenge.ChallengeGroup.IsIntro)
			{
				...
				{
					challenge.CompleteChallenge(forceRedeem: true, giveReward: false);
				}
