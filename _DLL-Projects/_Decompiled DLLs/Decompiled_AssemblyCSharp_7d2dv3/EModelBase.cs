...
public abstract class EModelBase : MonoBehaviour
{
	...
	{
		Vector3 euler = new Vector3(0f, ((!(bipedPelvisTransform == null)) ? bipedPelvisTransform.eulerAngles : entity.rotation).y, 0f);
	...
	public void LookAtUpdate(EntityAlive e)
	{
		...
		else if (!lookAtIsPos)
		{
			...
			if (e.GetHeadLookTarget(out var lookTarget, out var requiresLineOfSight) && (!requiresLineOfSight || e.CanSee(lookTarget)))
			{
	...
	public void DoRagdoll(RagdollMode _mode, float stunTime, EnumBodyPartHit bodyPart, Vector3 forceVec, Vector3 forceWorldPos, bool isRemote)
	{
		...
		{
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageEntityRagdoll>().Setup(entity, (byte)_mode, stunTime, bodyPart, forceVec, forceWorldPos));
			return;
		}
		...
		if (stunTime == 0f)
		{
			...
			if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
			{
				entity.world.entityDistributer.SendPacketToTrackedPlayersAndTrackedEntity(entity.entityId, -1, NetPackageManager.GetPackage<NetPackageEntityRagdoll>().Setup(entity, (byte)_mode, 0f, EnumBodyPartHit.Torso, forceVec, forceWorldPos));
			}
		}
		else
		{
			...
			if (forceVec.sqrMagnitude > 0f && !entity.isEntityRemote)
			{
				...
				else
				{
					...
					if ((bool)hitTransform)
					{
						...
						if ((bool)component)
						{
							if (_mode != RagdollMode.FullForce)
							{
								ClampForceForMass(ref _force);
							}
							if (forceWorldPos.sqrMagnitude > 0f)
							{
								component.AddForceAtPosition(_force, forceWorldPos - Origin.position, ForceMode.Impulse);
							}
							else
							{
								component.AddForce(_force, ForceMode.Impulse);
							}
						}
					}
				}
			...
			if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
			{
				entity.world.entityDistributer.SendPacketToTrackedPlayersAndTrackedEntity(entity.entityId, -1, NetPackageManager.GetPackage<NetPackageEntityRagdoll>().Setup(entity, (byte)_mode, stunTime, bodyPart, forceVec, forceWorldPos));
			}
		}
	}
	...
	{
		float num = EntityClass.list[entity.entityClass].MassKg * 8f;
	...
	public void DoRagdoll(in DamageResponse dr, RagdollMode _mode = RagdollMode.Default, float stunTime = 999999f)
	{
		EntityAlive entityAlive = entity as EntityAlive;
		...
		if (num > 0f && source != null)
		{
			...
			DoRagdoll(_mode, stunTime, dr.HitBodyPart, vector, source.getHitTransformPosition(), isRemote: false);
		}
		else
		{
			DoRagdoll(_mode, stunTime, dr.HitBodyPart, Vector3.zero, Vector3.zero, isRemote: false);
		}
	}
	...
	public void BlendRagdoll()
	{
		if (ragdollAdjustPosDelay > 0f)
		{
			...
			if (ragdollAdjustPosDelay <= 0f)
			{
				...
				for (int i = 0; i < 5; i++)
				{
					if (!Physics.Raycast(vector2, Vector3.down, out var hitInfo, 3f, -538750989))
					{
						break;
					}
	...
	public void FrameUpdateRagdoll()
	{
		...
		case ERagdollState.On:
			...
			if (!entity.isEntityRemote)
			{
				NetPackageEntityRagdoll package = NetPackageManager.GetPackage<NetPackageEntityRagdoll>().Setup(entity, (byte)ragdollState);
				if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
				{
	...
	{
		if (jiggles != null && jiggles.Length != 0)
		{
			Log.Out($"{entity.GetDebugName()}_{entity.entityId} jiggle count: {jiggles.Length}");
			for (int i = 0; i < jiggles.Length; i++)
			{
				Jiggle jiggle = jiggles[i];
				Log.Out($"\t{i} {jiggle.DebugString()}");
			}
		}
	}
	...
	public virtual void createModel(World _world, EntityClass _ec)
	{
		...
		if ((bool)assets.Mesh)
		{
			transform = UnityEngine.Object.Instantiate(assets.Mesh, modelTransformParent, worldPositionStays: false);
			transform.name = assets.Mesh.name;
		...
		bool flag = false;
		AltMaterial = assets.AltMaterial;
		if (AltMaterial != null)
		{
			flag = true;
		}
