...
public class XUiC_OptionsControlsNewBinding : XUiController
{
	...
	[XuiBindComponent("forAction", true)]
	[PublicizedFrom(EAccessModifier.Private)]
	public readonly XUiV_Label lblAction;

	[XuiBindComponent("newBindingAbort", true)]
	[PublicizedFrom(EAccessModifier.Private)]
	public readonly XUiV_Label lblAbort;

	[XuiBindComponent("inUse", true)]
	[PublicizedFrom(EAccessModifier.Private)]
	public readonly XUiV_Rect rectInUse;

	[XuiBindComponent("inUseBy", true)]
	[PublicizedFrom(EAccessModifier.Private)]
	public readonly XUiV_Label lblInUseBy;
	...
	public bool alreadyBound(BindingSource _binding, PlayerAction _selfAction)
	{
		...
		PlayerActionData.ActionUserData obj = (PlayerActionData.ActionUserData)_selfAction.UserData;
		...
		{
			PlayerActionSet playerActionSet = xui.playerUI.playerInput;
			if (!xui.playerUI.playerInput.Actions.Contains(_selfAction) || !xui.playerUI.playerInput.Actions.Contains(playerAction))
			{
				if (xui.playerUI.playerInput.GUIActions.Actions.Contains(_selfAction) && xui.playerUI.playerInput.GUIActions.Actions.Contains(playerAction))
				{
					playerActionSet = xui.playerUI.playerInput.GUIActions;
				}
				else if (xui.playerUI.playerInput.VehicleActions.Actions.Contains(_selfAction) && xui.playerUI.playerInput.VehicleActions.Actions.Contains(playerAction))
				{
					playerActionSet = xui.playerUI.playerInput.VehicleActions;
				}
				else
				{
					if (!xui.playerUI.playerInput.PermanentActions.Actions.Contains(_selfAction) || !xui.playerUI.playerInput.PermanentActions.Actions.Contains(playerAction))
					{
						return false;
					}
					playerActionSet = xui.playerUI.playerInput.PermanentActions;
				}
			}
			bool flag = false;
	...
	public override void OnOpen()
	{
		...
		if (forController)
		{
			string arg = ((inputStyle == PlayerInputManager.InputStyle.PS4) ? "[sp=PS5_Button_Options] / ESC" : "[sp=XB_Button_Back] / ESC");
			text = string.Format(Localization.Get("xuiNewBindingAbort_Controller"), arg);
		}
		else
		{
			string arg2 = ((inputStyle == PlayerInputManager.InputStyle.PS4) ? "ESC / [sp=PS5_Button_Options]" : "ESC / [sp=XB_Button_Back]");
			text = string.Format(Localization.Get("xuiNewBindingAbort_Controller"), arg2);
		}
		lblAbort.Text = text;
		ThreadManager.StartCoroutine(startNextFrame());
	}

	public static void GetNewBinding(XUi _xuiInstance, PlayerAction _action, bool _forController = false)
	{
		...
		_xuiInstance.playerUI.windowManager.Open(childByType.WindowGroup, _bModal: false);
	}
}
