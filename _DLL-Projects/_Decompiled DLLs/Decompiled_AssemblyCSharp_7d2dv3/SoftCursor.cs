...
public class SoftCursor : CursorControllerAbs
{
	...
	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public static INGUIAtlas emptyCursorAtlas;
	...
	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public static INGUIAtlas defaultControllerCursorAtlas;
	...
	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public static INGUIAtlas defaultMouseCursorAtlas;
	...
	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public static INGUIAtlas mapCursorAtlas;
	...
	public override XUiView HoverTarget
	{
		...
		set
		{
			...
			if (hoverTarget != null && !CursorModeActive && LastInputStyle != PlayerInputManager.InputStyle.Keyboard && !movingMouse && hoverTarget.IsNavigatable && !hoverTarget.HasHoverSound)
			{
	...
	public void Update()
	{
		if (!Application.isPlaying || GameManager.Instance.IsQuitting || GUIWindowConsole.IsOpen())
		{
			return;
		}
	...
	public virtual void HandleMovement()
	{
		...
		if (LastInputStyle != PlayerInputManager.InputStyle.Keyboard)
		{
			...
			else
			{
				...
				if (bHasHoverTarget && (HoverTarget == null || !HoverTarget.ColliderEnabled || !HoverTarget.IsActiveInHierarchy))
				{
					HoverTarget = null;
				}
				...
				speedMultiplier = Mathf.MoveTowards(speedMultiplier, (bHasHoverTarget && HoverTarget.IsSnappable) ? CursorControllerAbs.hoverSpeed : 1f, Time.unscaledDeltaTime * (bHasHoverTarget ? 10f : 1f));
	...
	public void Snap()
	{
		if (!GamePrefs.GetBool(EnumGamePrefs.OptionsControllerCursorSnap) || hoverTarget == null || !hoverTarget.IsSnappable || !hoverTarget.ColliderEnabled || !hoverTarget.IsActiveInHierarchy)
		{
			return;
		}
		Bounds colliderBounds = hoverTarget.ColliderBounds;
		if (cursorWorldBounds.extents.x > colliderBounds.extents.x - OffsetSnapBounds)
		{
			Position = colliderBounds.center;
			return;
		}
		Vector3 vector = colliderBounds.ClosestPoint(Position);
		Vector3 vector2 = Vector3.right * cursorWorldBounds.extents.x;
		Vector3 point = vector - vector2;
		Vector3 point2 = vector + vector2;
		if (!colliderBounds.Contains(point))
		{
			vector = colliderBounds.ClosestPoint(point) + vector2;
		}
		else if (!colliderBounds.Contains(point2))
		{
			vector = colliderBounds.ClosestPoint(point2) - vector2;
		}
		vector.y = colliderBounds.center.y;
		Position = vector;
	}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public void SetCursorSprite(INGUIAtlas _atlasMouse, string _spriteMouse, UIWidget.Pivot _pivotMouse, INGUIAtlas _atlasController, string _spriteController, UIWidget.Pivot _pivotController)
	{
	...
	public override void SetNavigationTarget(XUiView _view)
	{
		...
		else if (_view != base.navigationTarget && (base.lockNavigationToView == null || (_view != null && _view.Controller.IsChildOf(base.lockNavigationToView.Controller))))
		{
			...
			{
				_view.Controller.OnCursorSelected(_isActualElement: true);
	...
	public override void RefreshSelection()
	{
		...
		selectionBox.alpha = (base.navigationTarget.UseSelectionBox ? base.navigationTarget.Controller.GetParentWindow().PanelAlpha : 0f);
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void PositionSelectionBox(XUiView _view)
	{
		Position = _view.ColliderCenter;
		if (_view.UseSelectionBox)
		{
			selectionBox.width = _view.Size.x + selectionBoxMargin;
			selectionBox.height = _view.Size.y + selectionBoxMargin;
			selectionBox.depth = 1000000;
			selectionBox.transform.localScale = Vector3.one;
			selectionBox.transform.localPosition = _view.LocalCenter;
			selectionBox.transform.localRotation = Quaternion.identity;
			selectionBox.transform.parent = _view.UiTransform;
			selectionBox.RemoveFromPanel();
		}
	}

	public override void SetNavigationLockView(XUiView _view, XUiView _viewToSelect = null)
	{
		if (_view != null && (!_view.IsVisible || !_view.IsActiveInHierarchy))
		{
	...
	{
		if (base.lockNavigationToView != null)
		{
			return _view.Controller.IsSelfOrChildOf(base.lockNavigationToView.Controller);
		}
		return true;
	}
	...
	{
		base.navigationTargetLater = _view;
		base.Locked = true;
		for (int i = 0; i < 3; i++)
		{
			yield return null;
		...
		base.navigationTargetLater = null;
		lastMousePosition = Input.mousePosition;
	}
	...
	public override void ResetNavigationTarget()
	{
		if (base.navigationTarget != null)
		{
			...
			{
				Position = base.navigationTarget.ColliderCenter;
			}
			else
			{
				SetNavigationTarget(null);
