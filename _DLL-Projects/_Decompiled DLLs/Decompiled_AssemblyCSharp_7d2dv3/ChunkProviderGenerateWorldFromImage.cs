...
public class ChunkProviderGenerateWorldFromImage : ChunkProviderGenerateWorld
{
	...
	public ChunkProviderGenerateWorldFromImage(string _levelName, PathAbstractions.AbstractedLocation _worldLocation, bool _bLoadDTM = true)
		: base(_levelName, _worldLocation)
	{
		bLoadDTM = _bLoadDTM;
	}
	...
	public override IEnumerator Init(World _world)
	{
		...
		m_BiomeProvider = new WorldBiomeProviderFromImage(levelName, worldLocation.FullPath, world.Biomes);
		WorldDecoratorPOIFromImage worldDecoratorPOIFromImage = new WorldDecoratorPOIFromImage(levelName, worldLocation.FullPath, GetDynamicPrefabDecorator(), m_Dtm.DimX, m_Dtm.DimY, null, _bChangeWaterDensity: false, 1, heightMap);
		...
		if (bLoadDTM)
		{
			...
			((TerrainFromDTM)m_TerrainGenerator).Init(m_Dtm, m_BiomeProvider, levelName, worldLocation.FullPath, world.Seed);
	...
	public void loadDTM()
	{
		...
		if (SdFile.Exists(worldLocation.FullPath + "/dtm.png"))
		{
			texture2D = TextureUtils.LoadTexture(worldLocation.FullPath + "/dtm.png");
			Log.Out("Loading local DTM");
		}
		else if (SdFile.Exists(worldLocation.FullPath + "/dtm.tga"))
		{
			texture2D = TextureUtils.LoadTexture(worldLocation.FullPath + "/dtm.tga");
			Log.Out("Loading local DTM");
		}
		else
		{
