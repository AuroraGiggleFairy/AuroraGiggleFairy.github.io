...
public class DynamicMeshManager : MonoBehaviour
{
	...
	{
		if (CONTENT_ENABLED != newvalue && !(GameManager.Instance == null) && GameManager.Instance.World != null)
		{
			OnWorldUnload();
			if (newvalue)
			{
	...
	public static void OnGamePrefChanged(EnumGamePrefs _pref)
	{
		...
		{
			EnabledChanged(GamePrefs.GetBool(_pref));
		}
	}
	...
	public void ForceLoadDataAroundPosition(Vector3i pos, int regionRadius)
	{
		...
		foreach (ChunkGameObject value2 in GameManager.Instance.World.ChunkCache.DisplayedChunkGameObjects.Dict.Values)
		{
	...
	public void CheckPrefabsInRegion(int x, int z)
	{
		...
		List<PrefabInstance> list = (from d in dynamicPrefabsBuffer
	...
	public void CheckPrefabs(string source, bool forceRegen = false)
	{
		...
		dynamicPrefabsBuffer.Clear();
		dynamicPrefabDecorator.GetAllPrefabs(dynamicPrefabsBuffer);
		List<PrefabInstance> list = dynamicPrefabsBuffer.OrderBy([PublicizedFrom(EAccessModifier.Internal)] (PrefabInstance d) => Math.Abs(Vector3.Distance(playerPos, d.boundingBoxPosition.ToVector3()))).ToList();
	...
	public void UpdateDynamicPrefabDecoratorRegion(DynamicMeshRegion region)
	{
		...
		else
		{
			...
			dynamicPrefabsBuffer.Clear();
			dynamicPrefabDecorator.GetAllPrefabs(dynamicPrefabsBuffer);
			if (dynamicPrefabsBuffer.Count == 0)
			{
				LogMsg("prefabs Null or empty");
				return;
			}
			for (int num = dynamicPrefabsBuffer.Count - 1; num >= 0; num--)
			{
				PrefabInstance prefabInstance = dynamicPrefabsBuffer[num];
	...
	public static int GetViewSize(EntityPlayer _player)
	{
		...
		return player.ChunkObserver.viewDim * 16;
	}

	public static int GetBufferSize(EntityPlayer _player)
	{
		return ((_player.ChunkObserver != null) ? player.ChunkObserver.viewDim : GamePrefs.GetInt(EnumGamePrefs.OptionsGfxViewDistance)) * 16 * 3;
	}
	...
	public void ShowOrHidePrefabs()
	{
		...
		int viewSize = GetViewSize(player);
		DictionarySave<long, ChunkGameObject> dictionarySave = GameManager.Instance?.World.ChunkCache.DisplayedChunkGameObjects;
	...
	public DynamicMeshRegion GetRegion(BlockValueRef bvRef)
	{
		...
		case BlockValueRefType.Block:
			return GetRegion(bvRef.BlockPosition);
		case BlockValueRefType.Prop:
		{
			Vector2i vector2i = World.toChunkXZ(bvRef.PropReference.ChunkPos);
			return GetRegion(new Vector3i(vector2i.x, 0, vector2i.y));
		}
		default:
	...
	{
		ChunkChanged(new BlockValueRef(blockPos), entityId, blockType);
	}
	...
	public static void ChunkChanged(BlockValueRef bvRef, int entityId, int blockType)
	{
		...
		{
			DynamicMeshRegion region = Instance.GetRegion(bvRef);
			if (region.IsInItemLoad())
			{
				region.SetVisibleNew(active: false, ChunkChangedInItemLoad);
			}
		}
		if (IsServer && bvRef.TryGetBlockPos(out var pos))
		{
			Instance.AddUpdateData(pos, isUrgent: false, addToThread: true);
			int num = pos.x & 0xF;
			switch (num)
			{
			case 0:
				Instance.AddUpdateData(pos + new Vector3i(-16, 0, 0), isUrgent: false, addToThread: true);
				break;
			case 15:
				Instance.AddUpdateData(pos + new Vector3i(16, 0, 0), isUrgent: false, addToThread: true);
				break;
			...
			if ((pos.z & 0xF) == 0)
			{
				Instance.AddUpdateData(pos + new Vector3i(0, 0, -16), isUrgent: false, addToThread: true);
			}
			else if (num == 15)
			{
				Instance.AddUpdateData(pos + new Vector3i(0, 0, 16), isUrgent: false, addToThread: true);
	...
	public static void ImportVox(string name, Vector3 pos, int blockId)
	{
		...
		using (MemoryStream baseStream = new MemoryStream(buffer))
		{
			...
			while (pooledBinaryReader.BaseStream.Position != pooledBinaryReader.BaseStream.Length)
			{
				...
				int num4 = (int)(pos.x + (float)(int)b);
				int y = (int)(pos.y + (float)(int)b2);
				int num5 = (int)(pos.z + (float)(int)b3);
