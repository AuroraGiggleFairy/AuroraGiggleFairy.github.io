...
public class BlockToolTerrainSmoothing : IBlockTool
{
	...
	public void SmoothTerrain(WorldRayHitInfo _hitInfo)
	{
		...
		for (int i = 0; i < cubesInBrush.Length; i++)
		{
			...
			if (HasValidNeighbor(vector3i, 0, itemInventoryData))
			{
				...
				{
					itemInventoryData.world.SetBlockRPC(vector3i, block2, averageDensity);
				}
			}
		}
	}
	...
	public sbyte GetOutputBasedOnRuleset22(Vector3i _pos, ItemInventoryData _data)
	{
		...
		for (int num2 = 1; num2 >= -1; num2--)
		{
			for (int num3 = 1; num3 >= -1; num3--)
			{
				...
				{
					int density = _data.world.GetDensity(_pos.x + num4, _pos.y + num2, _pos.z + num3);
					if (density >= 0)
					{
						array[num] = 'A';
					}
	...
	{
		Vector3i[] cubesInBrush = brush.GetCubesInBrush();
		int num = _data.world.GetDensity(_pos.x, _pos.y, _pos.z);
		for (int i = 0; i < cubesInBrush.Length; i++)
		{
			Vector3i blockPos = _pos + cubesInBrush[i];
			num += _data.world.GetDensity(blockPos);
		}
	...
	{
		float num = 1f;
		float num2 = 0f + (float)_data.world.GetDensity(_pos.x, _pos.y, _pos.z);
		num += 1f;
		float num3 = num2 + (float)_data.world.GetDensity(_pos.x - 1, _pos.y, _pos.z);
		num += 1f;
		float num4 = num3 + (float)_data.world.GetDensity(_pos.x + 1, _pos.y, _pos.z);
		num += 1f;
		float num5 = num4 + (float)_data.world.GetDensity(_pos.x, _pos.y, _pos.z - 1);
		num += 1f;
		return (sbyte)((num5 + (float)_data.world.GetDensity(_pos.x, _pos.y, _pos.z + 1)) / num);
	}
	...
	public bool HasValidNeighbor(Vector3i _pos, sbyte _denThreshold, ItemInventoryData _data)
	{
		if (_denThreshold < 0)
		{
			if (_data.world.GetDensity(_pos.x - 1, _pos.y, _pos.z) <= _denThreshold)
			{
				return true;
			}
			if (_data.world.GetDensity(_pos.x + 1, _pos.y, _pos.z) <= _denThreshold)
			...
			if (_data.world.GetDensity(_pos.x, _pos.y - 1, _pos.z) <= _denThreshold)
			{
				return true;
			}
			if (_data.world.GetDensity(_pos.x, _pos.y + 1, _pos.z) <= _denThreshold)
			...
			if (_data.world.GetDensity(_pos.x, _pos.y, _pos.z - 1) <= _denThreshold)
			{
				return true;
			}
			if (_data.world.GetDensity(_pos.x, _pos.y, _pos.z + 1) <= _denThreshold)
