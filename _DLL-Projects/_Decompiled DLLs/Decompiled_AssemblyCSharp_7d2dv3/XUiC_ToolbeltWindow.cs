...
public class XUiC_ToolbeltWindow : XUiController
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public string standardXPColor = "255,255,255,255";

	[PublicizedFrom(EAccessModifier.Private)]
	public string updatingXPColor = "255,255,255,255";

	[PublicizedFrom(EAccessModifier.Private)]
	public string expDeficitColor = "255,255,255,255";
	...
	{
		xui.playerUI.entityPlayer.EmptyToolbelt(0, 10);
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void BtnClearInventory2_OnPress(XUiController _sender, int _mouseButton)
	{
		xui.playerUI.entityPlayer.EmptyToolbelt(10, 20);
	}
	...
	public override void Update(float _dt)
	{
		...
		base.ViewComponent.IsVisible = (!(localPlayer.AttachedToEntity != null) || !(localPlayer.AttachedToEntity is EntityVehicle)) && !localPlayer.IsDead() && (windowManager.IsHUDEnabled() || (xui.DragAndDropWindow.InMenu && windowManager.IsHUDPartialHidden()));
		if (CustomAttributes.ContainsKey("standard_xp_color"))
		{
			standardXPColor = CustomAttributes["standard_xp_color"].ToString();
		}
		else
		{
		...
		{
			updatingXPColor = CustomAttributes["updating_xp_color"].ToString();
		}
		else
		{
		...
		{
			expDeficitColor = CustomAttributes["deficit_xp_color"].ToString();
		}
		else
		{
		...
		{
			xpFillSpeed = StringParsers.ParseFloat(CustomAttributes["xp_fill_speed"].ToString());
		}
	}
	...
	public override bool GetBindingValueInternal(ref string value, string bindingName)
	{
		...
		case "xpcolor":
			...
			{
				value = standardXPColor;
			}
			return true;
		case "showxp":
			if (localPlayer != null)
			{
				value = (Progression.ShowXPType == Progression.ShowXPTypes.All || Progression.ShowXPType == Progression.ShowXPTypes.BarOnly).ToString();
				return true;
			}
			value = "false";
			return true;
