...
public class ItemClassTimeBomb : ItemClass
{
	...
	public override void OnHoldingUpdate(ItemInventoryData _data)
	{
		...
		if (itemValue.Meta > 0)
		{
			...
			if (itemValue.Meta == 0)
			{
				...
				_data.gameManager.ExplosionServer(vector, World.worldToBlockPos(vector), Quaternion.identity, explosion, _data.holdingEntity.entityId, 0.1f, _bRemoveBlockAtExplPosition: false, itemValue.Clone());
	...
	public override void OnDroppedUpdate(ItemWorldData _data)
	{
		...
		if (!flag)
		{
			...
			{
				entityItem.SetDead();
				_data.gameManager.ExplosionServer(vector, World.worldToBlockPos(vector), Quaternion.identity, explosion, _data.belongsEntityId, 0f, _bRemoveBlockAtExplPosition: false, itemValue.Clone());
			}
			else
			{
	...
	{
		_data.gameManager.ExplosionServer(_data.entityItem.GetPosition(), World.worldToBlockPos(_data.entityItem.GetPosition()), Quaternion.identity, explosion, _data.belongsEntityId, _data.entityItem.rand.RandomRange(0.1f, 0.3f), _bRemoveBlockAtExplPosition: false, _data.entityItem.itemStack.itemValue.Clone());
	}
