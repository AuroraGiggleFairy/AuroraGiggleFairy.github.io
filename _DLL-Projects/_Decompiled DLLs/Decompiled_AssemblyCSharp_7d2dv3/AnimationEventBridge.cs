...
public class AnimationEventBridge : RootTransformRefEntity
{
	...
	{
		if (entity is EntityPlayer entityPlayer && entityPlayer.inventory.holdingItem.HoldType.Value == ae.intParameter)
		{
			playSound(ae.stringParameter, ae);
		}
	}
	...
	public void ConsumeComplete()
	{
		...
		{
			if (holdingItem.Actions[i] is ItemActionEat itemActionEat && itemActionEat.PercentDone(entityPlayerLocal.inventory.holdingItemData.actionData[i]) > 0f)
			{
