...
public class XUiC_RecipeStack : XUiController
{
	...
	public void HandleOnPress(XUiController _sender, int _mouseButton)
	{
		...
		if (amountToRepair <= 0 && childByType != null)
		{
		...
		else
		{
			...
			for (int j = 0; j < recipe.ingredients.Count; j++)
			{
				...
				bool flag;
				if (amountToRepair <= 0 && childByType2 != null)
				{
					flag = childByType2.AddToItemStackArray(itemStack2) != -1;
				}
				else
				{
					if (itemStack2.itemValue.ItemClass.IsResourceUnit)
					{
						string replaceResourceUnit = itemStack2.itemValue.ItemClass.ReplaceResourceUnit;
						if (replaceResourceUnit != "")
						{
							itemStack2.itemValue = ItemClass.GetItem(replaceResourceUnit);
						}
					}
					flag = xui.PlayerInventory.AddItem(itemStack2, _playCollectSound: true);
				}
				if (flag)
				{
					array[j] = 0;
			...
			if (flag2)
			{
				GameManager.ShowTooltip(xui.playerUI.entityPlayer, inventoryFullDropping);
			}
		}
		isCrafting = false;
		ClearRecipe();
	...
	public bool outputStack()
	{
		...
		else if (amountToRepair > 0)
		{
			...
			ItemAction.HandleDegradation(itemValue);
	...
	{
		EntityPlayerLocal entityPlayer = xui.playerUI.entityPlayer;
		int num = (int)xui.playerUI.entityPlayer.Buffs.GetCustomVar("_craftCount_" + recipe.GetName());
		xui.playerUI.entityPlayer.Buffs.SetCustomVar("_craftCount_" + recipe.GetName(), num + 1);
		xui.playerUI.entityPlayer.Progression.AddLevelExp(recipe.craftExpGain / (num + 1), "_xpFromCrafting", Progression.XPTypes.Crafting);
	...
	public void updateRecipeData()
	{
		...
		{
			Color color = ((XUiV_Sprite)cancel.ViewComponent).Color;
			if (isOver && background.ViewComponent.UiTransformIsHovered)
			{
