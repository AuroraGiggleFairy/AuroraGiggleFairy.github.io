...
public static class SaveDataUtils
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static bool s_isManagementEnabled = false;
	...
	public static IEnumerator InitStaticCoroutine()
	{
		...
		UpdatePaths();
		IPlatformSaveGameProvider platformSaveGameProvider = PlatformManager.MultiPlatform.SaveGameProvider;
		s_isManagementEnabled = platformSaveGameProvider != null;
		if (s_isManagementEnabled)
		{
			while (!platformSaveGameProvider.Status.IsTerminal())
			{
				yield return null;
			}
		}
		if (s_isManagementEnabled)
		{
			s_saveDataManager = new SaveDataManager(platformSaveGameProvider);
		}
		else
		{
		...
		Log.Out("[SaveDataUtils] UserGameDataDir: " + GameIO.GetUserGameDataDir());
		if (LaunchPrefs.PlayerPrefsFile.Value)
		{
			Log.Out("[SaveDataUtils] SdPlayerPrefs -> SdFile");
			SaveDataPrefs = (s_saveDataPrefs = new SaveDataPrefsFile(GameIO.GetNormalizedPath(Path.Join(GameIO.GetUserGameDataDir(), "prefs.cfg"))));
		}
		else
		{
	...
	public static bool IsManaged(string path)
	{
		...
		string normalizedPath = GameIO.GetNormalizedPath(path);
		return s_managedPathRegexWithoutGroups.IsMatch(normalizedPath);
	}
	...
	public static bool TryGetManagedPath(string path, out SaveDataManagedPath managedPath)
	{
		...
		string normalizedPath = GameIO.GetNormalizedPath(path);
		Match match = s_managedPathRegex.Match(normalizedPath);
		if (match.Success)
		{
			GroupCollection groups = match.Groups;
			_ = groups[1].Value;
			string value = groups[2].Value;
			managedPath = new SaveDataManagedPath(value);
			return true;
		}
		managedPath = null;
		return false;
	...
	public static void Destroy()
	{
		...
		RoamingPrefs.Destroy();
		SaveDataManager = (s_saveDataManager = null);
		PlatformManager.NativePlatform.SaveGameProvider?.Destroy();
	}
}
