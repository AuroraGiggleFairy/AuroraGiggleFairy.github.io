...
public class BlockProjectileMoveScript : ProjectileMoveScript
{
	...
	public override void checkCollision()
	{
		...
		hitMask = 32;
		bool num = Voxel.Raycast(instance.World, ray, magnitude, -538750989, hitMask, 0f);
		if (entityAlive != null && entityAlive.emodel != null)
		{
		...
		if (Voxel.voxelRayHitInfo.tag.StartsWith("E_"))
		{
			...
			if (!num2 && component.IsDead())
			{
				...
				if (entityPlayer != null && EntityClass.list.ContainsKey(component.entityClass))
				{
					...
					if (value > 0f)
					{
						entityPlayer.AddKillXP(component as EntityAlive, itemValueProjectile, value);
					}
				}
			}
		}
		if (itemActionProjectile.Explosion.ParticleIndex > 0)
		{
			...
			instance.ExplosionServer(hitPos, vector3i, Quaternion.identity, itemActionProjectile.Explosion, ProjectileOwnerID, 0f, _bRemoveBlockAtExplPosition: false, itemValueProjectile);
		}
	}
