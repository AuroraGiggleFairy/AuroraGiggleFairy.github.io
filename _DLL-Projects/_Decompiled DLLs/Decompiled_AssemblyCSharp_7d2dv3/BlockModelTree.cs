...
public class BlockModelTree : BlockPlantGrowing
{
	...
	public override bool CheckPlantAlive(WorldBase _world, Vector3i _blockPos, BlockValue _blockValue)
	{
		...
		{
			return base.CheckPlantAlive(_world, _blockPos, _blockValue);
		}
		return true;
	}
	...
	{
		startToFall(_world, _bvRef, _blockValue, -1);
	...
	public override void OnBlockStartsToFall(WorldBase _world, Vector3i _blockPos, BlockValue _blockValue)
	{
		if (!_blockValue.ischild)
		{
			if (OnBlockDestroyedBy(_world, _blockPos, _blockValue, -1, _bUseHarvestTool: false) != DestroyedResult.Keep)
			{
	...
	public override void SpawnDestroyParticleEffect(WorldBase _world, BlockValue _blockValue, BlockValueRef _bvRef, float _lightValue, Color _color, int _entityIdThatCaused)
	{
		...
		_world.GetGameManager().PlaySoundAtPositionServer(_bvRef.ToVector3(_world), "trunkbreak", AudioRolloffMode.Logarithmic, 100, -1);
	}
	...
	public override DestroyedResult OnBlockDestroyedBy(WorldBase _world, BlockValueRef _bvRef, BlockValue _blockValue, int _entityId, bool _bUseHarvestTool)
	{
		...
		if (!startToFall(_world, _bvRef, _blockValue, _entityId))
		{
			return DestroyedResult.Downgrade;
		}
		return DestroyedResult.Keep;
