...
public static class MiscFromXml
{
	public static IEnumerator Create(XmlFile _xmlFile)
	{
		...
		foreach (XElement item in root.Elements("animation"))
		{
			...
			foreach (XElement item2 in item.Elements("hold"))
			{
				string attribute = item2.GetAttribute("id");
				if (attribute.Length == 0)
				{
					throw new Exception("Attribute 'id' missing in hold");
				}
				...
				int result2 = 0;
				attribute = item2.GetAttribute("carry");
				if (attribute.Length > 0 && !int.TryParse(attribute, out result2))
				{
					throw new Exception("Invalid hold carry");
				}
				float num = 0f;
				...
				AnimationDelayData.AnimationDelay[result] = new AnimationDelayData.AnimationDelays(result2, num, rayCastMoving, num2, num3, twoHanded);
		...
		foreach (XElement item5 in root.Elements("trigger_effects"))
		{
			foreach (XElement item6 in item5.Elements("trigger_effect"))
			{
				...
				string attribute2 = item6.GetAttribute("name");
				string _result2;
				if (item6.TryGetAttribute("type_ds", out var _result))
				{
					...
					{
						if (!TriggerEffectDualsenseParsers.ParseWeaponEffects(_result, item6, attribute2, out strengths, out effectTypeDualsense, out frequency, out strength, out position2, out endPosition, out amplitude))
						{
							continue;
						}
					}
					else if (_result.ContainsCaseInsensitive("Feedback"))
					{
						if (_result.ContainsCaseInsensitive("MultipointFeedback") || _result.ContainsCaseInsensitive("FeedbackMultipoint"))
						{
							...
							if (!TriggerEffectDualsenseParsers.ParseEffectStrengths(_result, item6, attribute2, out strengths))
							{
								continue;
							}
						}
						else if (_result.ContainsCaseInsensitive("SlopeFeedback") || _result.ContainsCaseInsensitive("FeedbackSlope"))
						{
							...
							if (!TriggerEffectDualsenseParsers.ParseStartEndPosition(_result, item6, attribute2, out position2, out endPosition) || !TriggerEffectDualsenseParsers.ParseStartEndStrengths(_result, item6, attribute2, out strength, out amplitude))
							{
								continue;
							}
						}
						else
						{
							...
							if (!TriggerEffectDualsenseParsers.ParsePosition(_result, item6, attribute2, out position2) || !TriggerEffectDualsenseParsers.ParseStrength(_result, item6, attribute2, out strength))
							{
								continue;
							}
						}
					...
					else
					{
						...
						else
						{
							...
							if (!TriggerEffectDualsenseParsers.ParsePosition(_result, item6, attribute2, out position2) || !TriggerEffectDualsenseParsers.ParseAmplitude(_result, item6, attribute2, out amplitude) || !TriggerEffectDualsenseParsers.ParseFrequency(_result, item6, attribute2, out frequency))
							{
								continue;
							}
						}
				...
				else if (item6.TryGetAttribute("type_xb", out _result2))
				{
					...
					if (_result2.ContainsCaseInsensitive("Feedback"))
					{
						if (_result2.ContainsCaseInsensitive("SlopeFeedback") || _result2.ContainsCaseInsensitive("FeedbackSlope"))
						{
							...
							if (!TriggerEffectXboxParsers.ParseStartEndPosition(_result2, item6, attribute2, out startPosition, out endPosition2) || !TriggerEffectXboxParsers.ParseStartEndStrength(_result2, item6, attribute2, out startStrength, out endStrength))
							{
								continue;
							}
						}
						else
						{
							...
							if (!TriggerEffectXboxParsers.ParseStrength(_result2, item6, attribute2, out startStrength))
							{
								continue;
							}
						}
