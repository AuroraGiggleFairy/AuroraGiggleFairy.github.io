...
public class EntityVehicle : EntityAlive, ILockable
{
	...
	public override void Init(int _entityClass, EntityInstanceAssets _assets, EModelInstanceAssets _eModelAssets)
	{
		...
		bag.SetSlots(ItemStack.CreateArray(size.x * size.y));
	...
	public void EnterVehicle(EntityAlive _entity)
	{
		...
		{
			entityPlayerLocal.MountEvent.Invoke(arg0: true);
		}
		int slot = -1;
	...
	public void DriverRemoved()
	{
		...
		{
			attachedPlayerLocal.MountEvent.Invoke(arg0: false);
		}
		hasDriver = false;
		vehicle.SetColors();
	...
	public override void DetachEntity(Entity _entity)
	{
		...
		entityAlive.SetModelLayer(modelLayer, force: true);
		...
		entityAlive.m_characterController.Enable(isEnabled: true);
		if (!_entity.isEntityRemote && GameManager.Instance.World != null)
		{
			LocalPlayerUI uIForPlayer = LocalPlayerUI.GetUIForPlayer(_entity as EntityPlayerLocal);
			if (uIForPlayer != null)
	...
	public void CameraInit()
	{
		...
		if ((bool)attachedPlayerLocal)
		{
			...
			cameraDist = Mathf.Min(3f, (cameraPos - cameraStartPos).magnitude);
	...
	public override void OnCollisionForward(Transform t, Collision collision, bool isStay)
	{
		...
		if ((bool)entity && entity.IsSpawned())
		{
			...
			if (num9 > 2.1f)
			{
				...
				if (num13 && entity.IsDead())
				{
					...
					if ((bool)entityPlayer2)
					{
						...
						if ((bool)entityAlive)
						{
							entityPlayer2.AddKillXP(entityAlive, vehicle.itemValue, 0.5f);
						}
					}
				}
				else if (num9 >= num14)
			...
			if (flag3)
			{
				entity.emodel.DoRagdoll(EModelBase.RagdollMode.Default, 2.5f, enumBodyPartHit, vector, zero, isRemote: false);
			}
			if (num11 > 2.1f)
			{
		...
		for (int j = 0; j < contactCount2; j++)
		{
			...
			bool flag4 = Voxel.Raycast(world, ray, 0.03f, -555520029, 69, 0f);
			if (!flag4)
			{
				ray.origin += contact2.normal * (0f - contact2.separation);
				ray.direction = -contact2.normal + collisionVelNorm;
				flag4 = Voxel.Raycast(world, ray, 0.03f, -555520029, 69, 0f);
			}
		...
		if (health == 1 || flag)
		{
			if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
			{
				...
				if (explodeHealth <= 0f && (flag || rand.RandomFloat < 0.2f))
				{
					DropItemsAsBackpack();
					Kill();
					GameManager.Instance.ExplosionServer(GetPosition(), World.worldToBlockPos(GetPosition()), base.transform.rotation, EntityClass.list[entityClass].explosionData, entityId, 0f, _bRemoveBlockAtExplPosition: false);
				}
			}
		}
		else
	...
	{
		_addCallback(new EntityActivationCommand("drive", "drive"));
		_addCallback(new EntityActivationCommand("ride", "drive"));
		...
		_addCallback(new EntityActivationCommand("refuel", "gas"));
		_addCallback(new EntityActivationCommand("take", "hand"));
		_addCallback(new EntityActivationCommand("horn", "horn"));
	}
	...
	public override bool AllowActivationCommand(ReadOnlySpan<char> _commandName, EntityPlayerLocal _playerFocusing)
	{
		...
		bool flag3 = CanAttach(_playerFocusing) && isDriveable();
		bool flag4 = IsDriven();
		if (CommandIs(_commandName, "drive"))
		{
			return !flag4 && flag3 && flag2;
		}
		if (CommandIs(_commandName, "ride"))
		{
			return flag4 && flag3 && flag2;
		}
		...
		{
			return vehicle.GetRepairAmountNeeded() > 0;
		}
		if (CommandIs(_commandName, "lock"))
		{
			return hasLock() && !isLocked && !flag4 && flag;
		}
		...
		if (CommandIs(_commandName, "keypad"))
		{
			...
			if (hasLock() && isLocked && !IsUserAllowed(PlatformManager.InternalLocalUserIdentifier))
			{
				return HasPassword();
			}
			return false;
		}
		if (CommandIs(_commandName, "refuel"))
		{
			return hasGasCan(_playerFocusing) && needsFuel() && VehicleFuelUsageModifier > 0f && flag2;
		}
		if (CommandIs(_commandName, "take"))
		{
			return !hasDriver && flag2 && flag;
		}
		if (CommandIs(_commandName, "horn"))
		{
			return vehicle.HasHorn() && flag2;
		}
	...
	public override string GetActivationText()
	{
		...
		string text = string.Format(Localization.Get("tooltipInteract"), arg, Localization.Get(LocalizedEntityName));
		if (IsLockedForLocalPlayer(entityPlayerLocal))
		{
	...
	public override void OnEntityActivated(EntityActivationCommand _command, EntityPlayerLocal _playerFocusing)
	{
		...
		if (CommandIs(_command.commandId, "drive") || CommandIs(_command.commandId, "ride"))
		{
			...
			if (CanAttach(_playerFocusing) && _playerFocusing.AttachedToEntity == null && isDriveable() && (!isLocked || !hasLock() || LocalPlayerIsOwner() || isAllowedUser(PlatformManager.InternalLocalUserIdentifier)))
			{
				if (EffectManager.GetValue(PassiveEffects.NoVehicle, null, 0f, _playerFocusing, null, base.EntityClass.Tags) > 0f)
				{
				...
				for (int i = 0; i < 8; i++)
				{
					...
					{
						flag = true;
						Vector3 vector3 = _playerFocusing.position - Origin.position;
						vector3.y += 1.1f;
				...
				if (!flag)
				{
					EnterVehicle(_playerFocusing);
				}
			}
		}
		if (CommandIs(_command.commandId, "repair") && XUiM_Vehicle.RepairVehicle(uIForPlayer.xui, vehicle))
		{
			PlayOneShot("crafting/craft_repair_item", sound_in_head: true);
			SendSyncData(4);
		}
		if (CommandIs(_command.commandId, "lock"))
		{
			vehicle.SetLocked(isLocked: true, _playerFocusing);
			PlayOneShot("misc/locking", sound_in_head: true);
			SendSyncData(2);
		}
		if (CommandIs(_command.commandId, "unlock"))
		{
			vehicle.SetLocked(isLocked: false, _playerFocusing);
			PlayOneShot("misc/unlocking", sound_in_head: true);
			SendSyncData(2);
		}
		if (CommandIs(_command.commandId, "keypad"))
		{
			PlayOneShot("misc/password_type", sound_in_head: true);
			XUiC_KeypadWindow.Open(uIForPlayer, this);
		}
		if (CommandIs(_command.commandId, "refuel") && AddFuelFromInventory(_playerFocusing))
		{
			SendSyncData(4);
		}
		if (CommandIs(_command.commandId, "take"))
		{
			if (!bag.IsEmpty())
			{
				GameManager.ShowTooltip(_playerFocusing, Localization.Get("ttEmptyVehicleBeforePickup"), string.Empty, "ui_denied");
				return;
			}
			if (!hasDriver)
			{
				...
				{
					GameManager.ShowTooltip(_playerFocusing, Localization.Get("xuiInventoryFullForPickup"), string.Empty, "ui_denied");
				}
			}
		}
		if (CommandIs(_command.commandId, "horn"))
		{
			UseHorn(_playerFocusing);
		}
		if (CommandIs(_command.commandId, "service") || CommandIs(_command.commandId, "storage"))
		{
			LockManager.Instance.LockRequestLocal(this, new EntityLockContext(_command.commandId.ToString(), bag), 0);
		}
	}
	...
	public void StartInteraction(ReadOnlySpan<char> _commandName)
	{
		...
		if ((bool)primaryPlayer)
		{
			...
			{
				XUiC_BagStorageWindowGroup.Open(uIForPlayer.xui, this, bag, LootContainer.GetLootContainer(GetLootList()), Localization.Get("xuiStorage"), SetBagModified, StopUIInteraction, CheckUIInteraction);
			}
			else
			{
				StopInteraction(0);
	...
	public bool CheckUIInteraction()
	{
		...
		float distanceSq = GetDistanceSq(primaryPlayer);
	...
	public void dropLoot(ItemStack[] items, float height)
	{
		...
		pos.y += height;
		GameManager.Instance.DropContentInLootContainerServer(-1, "DroppedVehicleContainer", pos, items, _skipIfEmpty: false, new Vector3(0f, 1f, 0f));
	}
	...
	public void ReadSyncData(BinaryReader _br, ushort syncFlags, int senderId)
	{
		...
		if ((syncFlags & 2) > 0)
		{
			...
			if (b > 1)
			{
				vehicle.PasswordHash = _br.ReadString();
			}
			else
			{
				vehicle.PasswordHash = _br.ReadInt32().ToString();
			}
	...
	public bool SetPasswordHash(string _passwordHash, PlatformUserIdentifierAbs _userIdentifier)
	{
		...
		{
			if (_passwordHash != vehicle.PasswordHash)
			{
				vehicle.PasswordHash = _passwordHash;
				vehicle.AllowedUsers.Clear();
	...
	public bool CheckPasswordHash(string _passwordHash, PlatformUserIdentifierAbs _userIdentifier)
	{
		...
		if (_passwordHash == vehicle.PasswordHash)
		{
			vehicle.AllowedUsers.Add(_userIdentifier);
			SendSyncData(2);
			return true;
		}
		return false;
	...
	{
		return !string.IsNullOrEmpty(vehicle.PasswordHash);
	}

	public string GetPasswordHash()
	{
		return vehicle.PasswordHash;
	}
