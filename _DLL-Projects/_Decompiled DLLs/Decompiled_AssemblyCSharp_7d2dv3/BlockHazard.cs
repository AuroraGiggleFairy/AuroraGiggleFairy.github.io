...
public class BlockHazard : BlockParticle
{
	...
	public override bool OnBlockActivated(string _commandName, WorldBase _world, Vector3i _blockPos, BlockValue _blockValue, EntityPlayerLocal _player)
	{
		...
		else if (_world.IsEditor() && toggleHazardStateForEditor(_world, _blockPos, _blockValue))
		{
			return true;
		}
		return false;
	...
	public override void OnBlockValueChanged(WorldBase _world, Chunk _chunk, Vector3i _blockPos, BlockValue _oldBlockValue, BlockValue _newBlockValue)
	{
		if (!_newBlockValue.ischild)
		{
			...
			updateHazardState(_world, _chunk, _blockPos, _newBlockValue);
	...
	public override void OnBlockEntityTransformAfterActivated(WorldBase _world, Vector3i _blockPos, BlockValue _blockValue, BlockEntityData _ebcd)
	{
		if (!_blockValue.ischild)
		{
			...
			updateHazardState(_world, null, _blockPos, _blockValue);
			checkParticles(_world, _blockPos, _blockValue);
		}
	}
	...
	{
		if (IsHazardOn(_world, _bvRef, _blockValue) && _damagePoints > 0 && buffActions != null && buffActions.Count > 0)
		{
			...
			if (entityAlive != null && entityAlive as EntityTurret == null)
			{
				...
				if ((object)entityAlive != null && (!(itemAction is ItemActionRanged) || (itemAction is ItemActionRanged itemActionRanged && (itemActionRanged.Hitmask & 0x80) != 0)))
				{
					...
					{
						entityAlive.Buffs.AddBuff(buffActions[i], _bvRef, entityAlive.entityId);
					}
				}
			}
		}
