...
public class EntityPlayerLocal : EntityPlayer, IInventoryChangedListener, IGamePrefsChangedListener
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public struct LocationSlotInfo(LocationSlotInfo.LocationTypes location, int slot)
	{
		...
		public LocationTypes Location = location;
	...
	public class AutoMove
	{
		...
		public void Update()
		{
			...
			if (mode == Mode.Orbit)
			{
				...
				Vector3 eulerAngles = ((!orbitLookForward) ? Quaternion.LookRotation(targetPos - entity.position, Vector3.up) : Quaternion.Euler(0f, num3 - 90f, 0f)).eulerAngles;
				eulerAngles.x *= -1f;
	...
	public static DegradeOnDeathTypes DegradeOnDeathType = DegradeOnDeathTypes.None;
	...
	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public List<LocationSlotInfo> DestroyLocationSlotList;
	...
	public static bool PermaDegrationOn
	{
		get
		{
			if (DegradeOnDeathType != DegradeOnDeathTypes.MaxDurability && DegradeOnDeathType != DegradeOnDeathTypes.Both)
			{
				return ItemAction.ItemMaxDegrationAmount > 0f;
			}
			return true;
		}
	}
	...
	public bool TPCameraCheckPassed => TPCameraCheckResult == eTPCameraCheckResult.Pass;
	...
	public void OnGamePrefChanged(EnumGamePrefs _enum)
	{
		...
		{
			cameraDistanceMulti = GamePrefs.GetFloat(_enum);
		}
	}
	...
	public void SetMoveState(MoveState _state, bool _isOverride = false)
	{
		...
		if (m_vp_FPCamera != null)
		{
			m_vp_FPCamera.RotationKneeling = 0.05f;
		}
		switch (_state)
		{
		case MoveState.Off:
		...
		case MoveState.Walk:
			...
			if (m_vp_FPCamera != null)
			{
				m_vp_FPCamera.RotationKneeling = 0.065f;
			}
		...
		case MoveState.Run:
			...
			if (m_vp_FPCamera != null)
			{
				m_vp_FPCamera.RotationKneeling = 0.075f;
			}
			if (flag2)
			{
			...
			else
			{
				...
				else
				{
					...
					if (flag4 && (vp_FPCamera.HasOverheadSpace || IsGodMode.Value))
					{
						...
						m_vp_FPController.MotorJumpForce = Mathf.Max(EffectManager.GetValue(PassiveEffects.JumpStrength, null, m_vp_FPController.originalMotorJumpForce, this, null, CurrentStanceTag | CurrentMovementTag), 0f) * SandboxOptionManager.GetFloat(global::SandboxOptions.SandboxOptions.JumpStrength);
	...
	public new void LateUpdate()
	{
		...
		TPCameraCheckResult = CharacterCameraAngleValid();
		WorldBoundsUpdate();
	}
	...
	public override void Update()
	{
		...
		foreach (NetPackageSharedQuest.SharedQuestData item in sharedQuestsToProcess)
		{
			...
			{
				string arg = "?";
				PersistentPlayerData playerDataFromEntityID = GameManager.Instance.persistentPlayers.GetPlayerDataFromEntityID(item.sharedByEntityID);
	...
	{
		if ((vp_FPCamera != null && vp_FPCamera.Locked3rdPerson) || AttachedToEntity != null || inventory.IsHoldingItemActionRunning())
		{
			return;
		}
	...
	public override void AfterPlayerRespawn(RespawnType _type)
	{
		...
		case RespawnType.EnterMultiplayer:
		{
			...
			current.LoadOptionsFromCode(GameStats.GetString(EnumGameStats.SandboxCode));
			current.UpdateInGameValuesWithSandboxOptions();
		...
		{
			SandboxOptionManager current2 = SandboxOptionManager.Current;
			current2.LoadOptionsFromCode(GameStats.GetString(EnumGameStats.SandboxCode));
			current2.UpdateInGameValuesWithSandboxOptions();
			FireEvent(MinEventTypes.onSelfEnteredGame);
	...
	public override Ray GetLookRay()
	{
		...
		result.direction += playerCamera.transform.up * 0.0001f;
		float num = Vector3.Distance(playerCamera.transform.position, LookPoint);
		float num2 = Vector3.Distance(playerCamera.transform.position, result.origin - Origin.position);
		if (num < num2)
		{
			result.origin = LookPoint;
			result.origin += Origin.position;
		}
		result.origin -= playerCamera.transform.forward * 0.15f;
		return result;
	}
	...
	public override void OnEntityDeath()
	{
		...
		windowManager.Open("death", _bModal: false);
		AimingGun = false;
	...
	public void FrameUpdateCamera()
	{
		...
		if (!(AttachedToEntity is EntityVehicle))
		{
			...
			{
				num3 = ((num2 < 3f) ? 0.5f : ((!(num2 < 6f)) ? 0.8f : 0.65f));
			}
			if (MovementRunning)
			{
				num3 *= 1.5f;
			...
			if (Crouching && !isSwimming)
			{
				if (AimingGun)
				{
					vp_FPCamera2.Position3rdPersonOffset = new Vector3(0.3f, 1.25f, 0.6f);
				}
				else
				{
					vp_FPCamera2.Position3rdPersonOffset = new Vector3(0.3f, 1.25f, 1f);
				}
			}
			else if (AimingGun)
			{
				vp_FPCamera2.Position3rdPersonOffset = new Vector3(0.4f, 1.65f, 0.8f);
			}
			else
			{
				vp_FPCamera2.Position3rdPersonOffset = new Vector3(0.4f, 1.65f, 1.2f);
			}
	...
	public override void Respawn(RespawnType _reason)
	{
		...
		if (_reason == RespawnType.Teleport)
		{
			windowManager.CloseAllOpenModalWindows("map");
		}
		isFallDeath = false;
	}
	...
	{
		return base.GetSpeedModifier() * (IsGodMode.Value ? GodModeSpeedModifier : 1f);
	}
	...
	{
		DropOption dropOption = (DropOption)SandboxOptionManager.GetInt(global::SandboxOptions.SandboxOptions.LoseItemsOnDeathType);
		int num;
		switch (dropOption)
		{
		case DropOption.None:
			return;
		default:
			num = ((dropOption == DropOption.ToolbeltBackpack) ? 1 : 0);
			break;
		case DropOption.All:
		case DropOption.Backpack:
			num = 1;
			break;
		}
		bool flag = (byte)num != 0;
		bool flag2 = dropOption == DropOption.All || dropOption == DropOption.Toolbelt || dropOption == DropOption.ToolbeltBackpack;
		bool flag3 = dropOption == DropOption.All || dropOption == DropOption.Equipment;
		if (DestroyLocationSlotList == null)
		{
			DestroyLocationSlotList = new List<LocationSlotInfo>();
		}
		else
		{
			DestroyLocationSlotList.Clear();
		}
		if (flag)
		{
			...
			{
				if (dropStackCondition(slots[i]))
				{
					DestroyLocationSlotList.Add(new LocationSlotInfo(LocationSlotInfo.LocationTypes.Backpack, i));
				}
			}
		}
		if (flag2)
		{
			for (int j = 0; j < inventory.GetItemCount(); j++)
			{
				if (j != inventory.DUMMY_SLOT_IDX)
				{
					...
					if (dropStackCondition(item))
					{
						DestroyLocationSlotList.Add(new LocationSlotInfo(LocationSlotInfo.LocationTypes.Toolbelt, j));
					}
				}
			}
		}
		...
		if (flag3)
		{
			for (int k = 0; k < equipment.GetSlotCount(); k++)
			{
				if (k < num2 || k > num3)
				{
					ItemValue slotItem = equipment.GetSlotItem(k);
					if (dropValueCondition(slotItem))
					{
						DestroyLocationSlotList.Add(new LocationSlotInfo(LocationSlotInfo.LocationTypes.Equipment, k));
					}
				}
			}
		}
		if (DestroyLocationSlotList.Count != 0)
		{
			int num4 = Mathf.Min(rand.RandomRange(LostItemOnDeathMin, LostItemOnDeathMax + 1), DestroyLocationSlotList.Count);
			DestroyLocationSlotList.Shuffle(rand);
			if (DestroyLocationSlotList.Count > num4)
			{
				DestroyLocationSlotList.RemoveRange(num4, DestroyLocationSlotList.Count - num4);
			}
			if (flag)
			{
				ItemStack[] slots2 = bag.GetSlots();
				HandleRemoveRandomItems(slots2);
				bag.SetSlots(slots2);
			}
			if (flag3)
			{
				HandleRemoveRandomItems(equipment);
			}
			if (flag2)
			{
				HandleRemoveRandomItems(inventory);
			}
		}
	}
	...
	{
		if (DegradeOnDeathType == DegradeOnDeathTypes.None)
		{
			return;
		}
		bool flag = false;
		...
		foreach (ItemStack itemStack in slots)
		{
			...
			if (DegradeOnDeathType == DegradeOnDeathTypes.MaxDurability || DegradeOnDeathType == DegradeOnDeathTypes.Both)
			{
				float maxDurabilityModifier = itemStack.itemValue.MaxDurabilityModifier;
				maxDurabilityModifier -= DegradeAmountOnDeath;
				if (maxDurabilityModifier < DegradeAmountOnDeath)
				{
					maxDurabilityModifier = DegradeAmountOnDeath;
				}
				itemStack.itemValue.MaxDurabilityModifier = maxDurabilityModifier;
			}
			if (DegradeOnDeathType == DegradeOnDeathTypes.Durability || DegradeOnDeathType == DegradeOnDeathTypes.Both)
			{
				int maxUseTimes = itemStack.itemValue.MaxUseTimes;
				int num = (int)((float)maxUseTimes * DegradeAmountOnDeath);
				itemStack.itemValue.UseTimes += num;
				if (itemStack.itemValue.UseTimes > (float)maxUseTimes)
				{
					itemStack.itemValue.UseTimes = maxUseTimes;
				}
			}
		}
		if (flag)
		...
		for (int j = 0; j < equipment.GetSlotCount(); j++)
		{
			ItemValue slotItem = equipment.GetSlotItem(j);
			...
			if (DegradeOnDeathType == DegradeOnDeathTypes.MaxDurability || DegradeOnDeathType == DegradeOnDeathTypes.Both)
			{
				float maxDurabilityModifier2 = slotItem.MaxDurabilityModifier;
				maxDurabilityModifier2 -= DegradeAmountOnDeath;
				if (maxDurabilityModifier2 < DegradeAmountOnDeath)
				{
					maxDurabilityModifier2 = DegradeAmountOnDeath;
				}
				slotItem.MaxDurabilityModifier = maxDurabilityModifier2;
				flag = true;
			}
			if (DegradeOnDeathType == DegradeOnDeathTypes.Durability || DegradeOnDeathType == DegradeOnDeathTypes.Both)
			{
				int maxUseTimes2 = slotItem.MaxUseTimes;
				int num2 = (int)((float)maxUseTimes2 * DegradeAmountOnDeath);
				slotItem.UseTimes += num2;
				if (slotItem.UseTimes > (float)maxUseTimes2)
				{
					slotItem.UseTimes = maxUseTimes2;
				}
				flag = true;
			}
			if (flag)
			{
				equipment.SetSlotItem(j, slotItem);
			}
		}
		flag = false;
		ItemStack[] slots2 = inventory.GetSlots();
		foreach (ItemStack itemStack2 in slots2)
		{
			...
			if (DegradeOnDeathType == DegradeOnDeathTypes.MaxDurability || DegradeOnDeathType == DegradeOnDeathTypes.Both)
			{
				float maxDurabilityModifier3 = itemStack2.itemValue.MaxDurabilityModifier;
				maxDurabilityModifier3 -= DegradeAmountOnDeath;
				if (maxDurabilityModifier3 < DegradeAmountOnDeath)
				{
					maxDurabilityModifier3 = DegradeAmountOnDeath;
				}
				itemStack2.itemValue.MaxDurabilityModifier = maxDurabilityModifier3;
			}
			if (DegradeOnDeathType == DegradeOnDeathTypes.Durability || DegradeOnDeathType == DegradeOnDeathTypes.Both)
			{
				int maxUseTimes3 = itemStack2.itemValue.MaxUseTimes;
				int num3 = (int)((float)maxUseTimes3 * DegradeAmountOnDeath);
				itemStack2.itemValue.UseTimes += num3;
				if (itemStack2.itemValue.UseTimes > (float)maxUseTimes3)
				{
					itemStack2.itemValue.UseTimes = maxUseTimes3;
				}
			}
		}
		if (flag)
	...
	public void dropBackpack(bool _isDying)
	{
		...
		int num2 = 0;
		int num3 = 0;
		bool flag = dropOption == DropOption.All || dropOption == DropOption.Backpack || dropOption == DropOption.ToolbeltBackpack;
		bool flag2 = dropOption == DropOption.All || dropOption == DropOption.Toolbelt || dropOption == DropOption.ToolbeltBackpack;
		bool flag3 = dropOption == DropOption.All || dropOption == DropOption.Equipment;
		ItemStack[] slots = bag.GetSlots();
		for (int i = 0; i < slots.Length; i++)
		...
		if (_isDying && dropOption == DropOption.DeleteAll)
		{
			...
			{
				slots2[l] = ItemStack.Empty;
			}
			bag.SetSlots(slots2);
			for (int m = 0; m < inventory.GetItemCount(); m++)
			{
				inventory.SetItem(m, ItemStack.Empty);
			}
			for (int n = 0; n < 12; n++)
			{
				ItemValue slotItem2 = equipment.GetSlotItem(n);
				if (slotItem2 != null && !slotItem2.ItemClassOrMissing.KeepOnDeath())
				{
					equipment.SetSlotItem(n, null);
				}
			}
			return;
		}
		...
		if (flag)
		{
			...
			{
				if (dropStackCondition(slots3[num6]))
				{
		...
		if (flag3)
		{
			...
			{
				ItemValue itemValue = equipment.GetSlotItem(num7)?.Clone();
				if (dropValueCondition(itemValue))
				{
					...
					if (!_isDying)
					{
						equipment.SetSlotItem(num7, null);
					}
				}
			}
		...
		if (flag2)
		{
			...
			{
				if (num8 != inventory.DUMMY_SLOT_IDX)
				{
					ItemStack itemStack2 = (array2[num8] = inventory.GetItem(num8));
					if (dropStackCondition(itemStack2))
					{
						entityBackpack.bag.AddItem(itemStack2);
						inventory.SetItem(num8, ItemStack.Empty);
					}
				}
			}
	...
	{
		for (int num = DestroyLocationSlotList.Count - 1; num >= 0; num--)
		{
			if (DestroyLocationSlotList[num].Location == LocationSlotInfo.LocationTypes.Backpack)
			{
				items[DestroyLocationSlotList[num].Slot].Clear();
				DestroyLocationSlotList.RemoveAt(num);
			}
		}
	}
	...
	public void HandleRemoveRandomItems(Inventory inventory)
	{
		for (int num = DestroyLocationSlotList.Count - 1; num >= 0; num--)
		{
			if (DestroyLocationSlotList[num].Location == LocationSlotInfo.LocationTypes.Toolbelt)
			{
				inventory.SetItem(DestroyLocationSlotList[num].Slot, ItemStack.Empty);
				DestroyLocationSlotList.RemoveAt(num);
			}
		}
	}
	...
	public void HandleRemoveRandomItems(Equipment equipment)
	{
		for (int num = DestroyLocationSlotList.Count - 1; num >= 0; num--)
		{
			if (DestroyLocationSlotList[num].Location == LocationSlotInfo.LocationTypes.Equipment)
			{
				equipment.SetSlotItem(DestroyLocationSlotList[num].Slot, null);
				DestroyLocationSlotList.RemoveAt(num);
			}
		}
	}
	...
	public override void OnHUD()
	{
		...
		windowManager.Open(playerUI.xui.CalloutWindow.WindowGroup, _bModal: false);
		...
		windowManager.Open(playerUI.xui.ToolTipWindow.WindowGroup, _bModal: false);
		windowManager.Open(XUiC_ChatOutput.ID, _bModal: false);
		if (Event.current.type == EventType.Repaint)
		{
	...
	{
		if (!_guiInGame.showCrosshair || Event.current.type != EventType.Repaint || IsDead() || emodel.IsRagdollActive || AttachedToEntity != null || !GamePrefs.GetBool(EnumGamePrefs.OptionsCrosshairEnabled))
		{
			return;
		}
		ItemClass.EnumCrosshairType enumCrosshairType = inventory.holdingItem.GetCrosshairType(inventory.holdingItemData);
		if (enumCrosshairType == ItemClass.EnumCrosshairType.None)
		{
			if (bFirstPersonView || !GamePrefs.GetBool(EnumGamePrefs.OptionsCrosshairDot3P))
			{
				return;
			}
		...
		if ((!GamePrefs.GetBool(EnumGamePrefs.OptionsCrosshairADS) && ((bFirstPersonView && AimingGun) || isAimingScoped)) || bModalWindowOpen || inventory == null)
		{
			return;
		}
		...
		int num3 = GamePrefs.GetInt(EnumGamePrefs.OptionsCrosshairColor);
		if (num3 < 0 || num3 >= crosshairColors.Count)
		{
			num3 = 0;
		}
		Color crosshairColor = crosshairColors[num3];
		Vector2 crosshairPosition2D = GetCrosshairPosition2D();
		crosshairPosition2D.y = (float)Screen.height - crosshairPosition2D.y;
		float num4 = (float)Screen.height * 0.059f;
		float num5 = num4 * num;
		...
		case ItemClass.EnumCrosshairType.CrosshairOnAiming:
		{
			...
			value *= (float)Screen.width / cameraTransform.GetComponent<Camera>().fieldOfView;
			...
			value2 *= (float)Screen.width / cameraTransform.GetComponent<Camera>().fieldOfView;
			float x = crosshairPosition2D.x;
			float y = crosshairPosition2D.y;
			float length = num5 / 3f;
			DrawDynamicCrosshair(x, y, value, value2, length, GamePrefs.GetFloat(EnumGamePrefs.OptionsCrosshairThickness), num2, crosshairColor);
			break;
		}
		case ItemClass.EnumCrosshairType.Plus:
			GUI.color = new Color(crosshairColor.r, crosshairColor.g, crosshairColor.b, num2);
			GUI.DrawTexture(new Rect(crosshairPosition2D.x - num5 / 2f, crosshairPosition2D.y - num5 / 2f, num5, num5), _guiInGame.CrosshairTexture, ScaleMode.StretchToFill);
			GUI.color = color;
			break;
		case ItemClass.EnumCrosshairType.Damage:
			...
			{
				GUI.color = new Color(crosshairColor.r, crosshairColor.g, crosshairColor.b, CrosshairAlpha(_guiInGame));
			}
			GUI.DrawTexture(new Rect(crosshairPosition2D.x - num4 / 2f, crosshairPosition2D.y - num4 / 2f, num4, num4), _guiInGame.CrosshairDamage, ScaleMode.StretchToFill);
			GUI.color = color;
			break;
		case ItemClass.EnumCrosshairType.Repair:
			...
			GUI.DrawTexture(new Rect(crosshairPosition2D.x - num4 / 2f, crosshairPosition2D.y - num4 / 2f, num4, num4), _guiInGame.CrosshairRepair, ScaleMode.StretchToFill);
			GUI.color = color;
			break;
		case ItemClass.EnumCrosshairType.Upgrade:
			...
			GUI.DrawTexture(new Rect(crosshairPosition2D.x - num4 / 2f, crosshairPosition2D.y - num4 / 2f, num4, num4), _guiInGame.CrosshairUpgrade, ScaleMode.StretchToFill);
			GUI.color = color;
			break;
		...
		case ItemClass.EnumCrosshairType.PowerItem:
			...
			GUI.DrawTexture(new Rect(crosshairPosition2D.x - num4 / 2f, crosshairPosition2D.y - num4 / 2f, num4, num4), _guiInGame.CrosshairPowerItem, ScaleMode.StretchToFill);
			GUI.color = color;
			break;
		case ItemClass.EnumCrosshairType.PowerSource:
			...
			GUI.DrawTexture(new Rect(crosshairPosition2D.x - num4 / 2f, crosshairPosition2D.y - num4 / 2f, num4, num4), _guiInGame.CrosshairPowerSource, ScaleMode.StretchToFill);
			GUI.color = color;
			break;
		case ItemClass.EnumCrosshairType.Blocked:
			GUI.color = new Color(1f, 0.5f, 0f, num2 * 0.5f);
			GUI.DrawTexture(new Rect(crosshairPosition2D.x - num4 / 4f, crosshairPosition2D.y - num4 / 4f, num4 / 2f, num4 / 2f), _guiInGame.CrosshairBlocked, ScaleMode.StretchToFill);
			GUI.color = color;
			break;
		case ItemClass.EnumCrosshairType.Dot:
			GUI.color = new Color(crosshairColor.r, crosshairColor.g, crosshairColor.b, num2);
			GUI.DrawTexture(new Rect(crosshairPosition2D.x - num4 / 4f, crosshairPosition2D.y - num4 / 4f, num4 / 2f, num4 / 2f), _guiInGame.CrosshairDot, ScaleMode.StretchToFill);
			GUI.color = color;
			break;
		}
	}
	...
	public void WeatherStatusFrameUpdate()
	{
		...
		if (!StormWarning || biomeStandingOn == null)
		{
			return;
		}
	...
	public eTPCameraCheckResult CharacterCameraAngleValid()
	{
		...
		if (Vector3.Angle(to: new Vector3(cameraTransform.forward.x, 0f, cameraTransform.forward.z), from: base.transform.forward) > 20f)
		{
	...
	public void LogFellThroughWorldDebugInfo()
	{
		...
		else
		{
			...
			Log.Out($"[FELLTHROUGHWORLD] Time since chunk {key} selected for data generation: {ChunkManager.SecondsSinceChunkSelectedForGeneration(chunk.Key)}");
			(int, double) tuple = ChunkCluster.SecondsSinceChunkStartedRegeneration(chunk.Key);
			if (tuple.Item1 == -1)
			{
				Log.Out($"[FELLTHROUGHWORLD] Chunk {key} has never started mesh regeneration.");
			}
			else
			{
				Log.Out($"[FELLTHROUGHWORLD] Time since a layer on chunk {key} started mesh regeneration: Layer {tuple.Item1}, {tuple.Item2}");
			}
			tuple = ChunkCluster.SecondsSinceChunkEndedRegeneration(chunk.Key);
			if (tuple.Item1 == -1)
			{
				Log.Out($"[FELLTHROUGHWORLD] Chunk {key} has never finished mesh regeneration.");
			}
			else
			{
				Log.Out($"[FELLTHROUGHWORLD] Time since a layer on chunk {key} finished mesh regeneration: Layer {tuple.Item1}, {tuple.Item2}");
			}
		}
		...
		world?.m_ChunkManager?.LogCurrentGenerationState();
		ChunkCluster.LogCurrentChunkRegenerationState(chunk.Key);
		moveController.LogCurrentRespawnState();
	}
