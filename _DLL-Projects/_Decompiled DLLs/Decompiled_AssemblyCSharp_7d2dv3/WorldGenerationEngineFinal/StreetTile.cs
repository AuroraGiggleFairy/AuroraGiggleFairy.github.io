...
public class StreetTile
{
	...
	{
		if (neighborsDiagonal == null)
		{
			neighborsDiagonal = new StreetTile[4];
			for (int i = 0; i < 4; i++)
			{
				neighborsDiagonal[i] = GetNeighbor(worldBuilder.StreetTileShared.dirDiagonal[i]);
			}
		}
	...
	{
		int num = 0;
		CalcConnectedDistrictTiles(tempTiles);
		for (int i = 0; i < tempTiles.Count; i++)
		{
			num += tempTiles[i].CountDistrictExits();
		}
		tempTiles.Clear();
		return num;
	}
	...
	public int CountDistrictExitsBetween(StreetTile _neighbor)
	{
		int num = 0;
		CalcConnectedDistrictTiles(tempTiles);
		_neighbor.CalcConnectedDistrictTiles(tempTiles2);
		for (int i = 0; i < tempTiles.Count; i++)
		{
			StreetTile streetTile = tempTiles[i];
			for (int j = 0; j < tempTiles2.Count; j++)
			{
				StreetTile otherTile = tempTiles2[j];
	...
	{
		_tiles.Add(this);
		CalcConnectedDistrictNeighbors(_tiles);
	}
	...
	public void CalcConnectedDistrictNeighbors(List<StreetTile> _tiles)
	{
		for (int i = 0; i < 4; i++)
		{
			if ((ConnectedExits & (1 << i)) > 0)
			{
				StreetTile neighbor = GetNeighbor(i);
				if (neighbor != null && neighbor.District == District && neighbor.Township == Township && !_tiles.Contains(neighbor))
				{
					_tiles.Add(neighbor);
	...
	public int CountDistrictExits()
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			{
				StreetTile neighbor = GetNeighbor(i);
				if (neighbor != null && neighbor.District != District && neighbor.Township == Township)
				{
					num++;
				}
			}
	...
	{
		if (ConnectedExits != 3 && ConnectedExits != 6 && ConnectedExits != 12 && ConnectedExits != 9)
		{
			return false;
		}
		int num = CalcConnectedDistrictTilesCount(tempTiles);
		for (int i = 0; i < 4; i++)
		{
			if ((ConnectedExits & (1 << i)) > 0)
			{
				...
				if (neighbor != null && neighbor.District == District && neighbor.Township == Township)
				{
					tempTiles.Add(neighbor);
				}
			}
		}
		bool result = false;
		if (tempTiles.Count == 2)
		{
			int index = _rand.RandomRange(0, tempTiles.Count);
			int neighborDir = GetNeighborDir(tempTiles[index]);
			ConnectedExits &= ~(1 << neighborDir);
			int num2 = CalcConnectedDistrictTilesCount(tempTiles2);
			if (num == num2)
			{
				SetExitUnUsedAndFromNeighbor(neighborDir);
				result = true;
			}
			else
			{
	...
	public void SetExitsToMyTownship()
	{
		...
		{
			StreetTile neighbor = GetNeighbor(i);
			if (neighbor != null && neighbor.Township == Township)
			{
				num |= 1 << i;
			}
		}
	...
	{
		SetExitUnUsed(_dir);
		GetNeighbor(_dir)?.SetExitUnUsed((_dir + 2) & 3);
	}
	...
	public bool spawnStreetTile(Vector2i tileMinPositionWorld, string streetPrefabName, int baseRotations)
	{
		...
		int num2 = (baseRotations + streetTile.rotationToFaceNorth) & 3;
		switch (num2)
		{
		case 1:
			num2 = 3;
	...
	{
		int num = (prefab.rotationToFaceNorth + rndm.RandomRange(0, 4)) & 3;
		...
		{
			List<Marker> list2 = prefab.RotatePOIMarkers(_bLeft: true, num);
			for (int num6 = list2.Count - 1; num6 >= 0; num6--)
			{
				if (list2[num6].MarkerType != Marker.MarkerTypes.RoadExit)
				{
					list2.RemoveAt(num6);
				}
			}
	...
	{
		List<Marker> list = _parentPrefab.RotatePOIMarkers(_bLeft: true, _parentRotations);
		if (list.Count == 0)
		{
			return;
		}
		...
		List<Marker> list2 = list.FindAll([PublicizedFrom(EAccessModifier.Internal)] (Marker m) => m.MarkerType == Marker.MarkerTypes.POISpawn);
		if (_depth < 5 && list2.Count > 0)
		{
			list2.Sort([PublicizedFrom(EAccessModifier.Internal)] (Marker m1, Marker m2) => (m2.size.x + m2.size.y + m2.size.z).CompareTo(m1.size.x + m1.size.y + m1.size.z));
			...
			foreach (string groupName in list3)
			{
				...
				{
					Marker marker = list4[num2];
					Vector2i vector2i = new Vector2i(marker.size.x, marker.size.z);
					Vector2i vector2i2 = new Vector2i(marker.startPos.x, marker.startPos.z);
					...
					{
						int num3;
						if (District.name != "gateway" && (num3 = Marker.MarkerSizes.IndexOf(new Vector3i(vector2i.x, 0, vector2i.y))) >= 0)
						{
							if (num3 > 0)
							{
								vector2i4 = new Vector2i(Marker.MarkerSizes[num3 - 1].x + 1, Marker.MarkerSizes[num3 - 1].z + 1);
							}
						}
						else
						{
					...
					int num6 = _parentPosition.x + marker.startPos.x;
					int num7 = _parentPosition.z + marker.startPos.z;
					...
					if (prefabWithDistrict.Tags.Test_AnySet(worldBuilder.StreetTileShared.traderTag) || prefabWithDistrict.Name.Contains("trader"))
					{
						...
						HasTrader = true;
						Log.Out("Trader {0}, {1}, {2}, at {3}", prefabWithDistrict.Name, BiomeType, District.name, vector2i5);
					}
					int num8 = marker.Rotations;
					byte b = (byte)((_parentRotations + prefabWithDistrict.rotationToFaceNorth + num8) & 3);
					...
					Vector3i position = new Vector3i(num6, _parentPosition.y + marker.startPos.y + prefabWithDistrict.yOffset, num7) + worldBuilder.PrefabWorldOffset;
					...
					SpawnMarkerPartsAndPrefabs(prefabWithDistrict, new Vector3i(num6, _parentPosition.y + marker.startPos.y + prefabWithDistrict.yOffset, num7), b, _depth + 1, totalDensityLeft);
					break;
				}
			}
		}
		List<Marker> list5 = list.FindAll([PublicizedFrom(EAccessModifier.Internal)] (Marker m) => m.MarkerType == Marker.MarkerTypes.PartSpawn);
		if (_depth < 20 && list5.Count > 0)
		{
			...
			foreach (string groupName2 in list6)
			{
				...
				{
					num13 = 0f;
					foreach (Marker item in list7)
					{
						num13 += item.PartChanceToSpawn;
					}
				}
				float num14 = 0f;
				foreach (Marker item2 in list7)
				{
					...
					Vector3i vector3i = new Vector3i(_parentPosition.x + item2.startPos.x - worldBuilder.WorldSize / 2, _parentPosition.y + item2.startPos.y, _parentPosition.z + item2.startPos.z - worldBuilder.WorldSize / 2);
					if (vector3i.y <= 0)
					{
						continue;
					}
					...
					byte b3 = (byte)((_parentRotations + prefabByName.rotationToFaceNorth + b2) % 4);
					...
					SpawnMarkerPartsAndPrefabs(prefabByName, _parentPosition + item2.startPos, b3, _depth + 1, totalDensityLeft);
					break;
					IL_0cb4:;
				}
			}
		...
		list5 = list.FindAll([PublicizedFrom(EAccessModifier.Internal)] (Marker m) => m.PartToSpawn.Contains("highway_transition"));
		if (list5.Count <= 0)
		{
			return;
		}
		foreach (Marker item3 in list5)
		{
			Vector2 vector = new Vector2(item3.startPos.x, item3.startPos.z) - new Vector2(size.x / 2, size.z / 2);
			...
			Vector3i position2 = new Vector3i(_parentPosition.x + item3.startPos.x - worldBuilder.WorldSize / 2, _parentPosition.y + item3.startPos.y, _parentPosition.z + item3.startPos.z - worldBuilder.WorldSize / 2);
			if (position2.y > 0)
			{
				...
				byte b5 = (byte)((_parentRotations + prefabByName2.rotationToFaceNorth + b4) % 4);
	...
	public void SpawnMarkerPartsAndPrefabsWilderness(PrefabData _parentPrefab, Vector3i _parentPosition, int _parentRotations)
	{
		...
		if (list2.Count > 0)
		{
			for (int num = 0; num < list2.Count; num++)
			{
				...
				{
					int num2 = _parentPosition.x + marker.startPos.x;
					int num3 = _parentPosition.z + marker.startPos.z;
					int num4 = marker.Rotations;
					byte b = (byte)((_parentRotations + wildernessPrefab.rotationToFaceNorth + num4) & 3);
					...
					PrefabDataInstance pdi = new PrefabDataInstance(_position: new Vector3i(num2 - worldBuilder.WorldSize / 2, _parentPosition.y + marker.startPos.y + wildernessPrefab.yOffset, num3 - worldBuilder.WorldSize / 2), _id: worldBuilder.PrefabManager.PrefabInstanceId++, _rotation: b, _prefabData: wildernessPrefab);
					AddPrefab(pdi);
					worldBuilder.WildernessPrefabCount++;
					wildernessPrefab.RotatePOIMarkers(_bLeft: true, b);
					SpawnMarkerPartsAndPrefabsWilderness(wildernessPrefab, new Vector3i(num2, _parentPosition.y + marker.startPos.y + wildernessPrefab.yOffset, num3), b);
				}
			}
		}
		List<Marker> list3 = list.FindAll([PublicizedFrom(EAccessModifier.Internal)] (Marker m) => m.MarkerType == Marker.MarkerTypes.PartSpawn);
		if (list3.Count > 0)
		{
			...
			{
				List<Marker> list5 = list3.FindAll([PublicizedFrom(EAccessModifier.Internal)] (Marker m) => m.GroupName == groupName);
				float num9 = 1f;
				if (list5.Count > 1)
				{
					num9 = 0f;
					foreach (Marker item in list5)
					{
						num9 += item.PartChanceToSpawn;
					}
				}
				float num10 = 0f;
				foreach (Marker item2 in list5)
				{
					...
					Vector3i position = new Vector3i(_parentPosition.x + item2.startPos.x - worldBuilder.WorldSize / 2, _parentPosition.y + item2.startPos.y, _parentPosition.z + item2.startPos.z - worldBuilder.WorldSize / 2);
					byte b2 = item2.Rotations;
					switch (b2)
					{
					case 1:
					...
					AddPrefab(pdi2);
					worldBuilder.WildernessPrefabCount++;
					SpawnMarkerPartsAndPrefabsWilderness(prefabByName, _parentPosition + item2.startPos, b3);
					break;
				}
			}
		}
