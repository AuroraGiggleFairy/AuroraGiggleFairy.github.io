...
public class BlockVendingMachine : Block
{
	...
	public override void PlaceBlock(WorldBase _world, BlockPlacement.Result _result, EntityAlive _ea)
	{
		...
		if (_world.GetTileEntity(_result.blockPos) is TileEntityVendingMachine && _ea != null && _ea.entityType == EntityType.Player && TraderInfo.traderInfoList[traderID].PlayerOwned)
		{
	...
	{
		if (!(_world.GetTileEntity(_blockPos) is TileEntityVendingMachine tileEntityVendingMachine))
		{
		...
		if (!flag)
		{
			_ = playerData?.IsAlly(internalLocalUserIdentifier) ?? false;
		}
		else
			_ = 0;
		bool playerOwned = TraderInfo.traderInfoList[traderID].PlayerOwned;
		cmds[0].enabled = TraderManager.VendingEnabled || playerOwned;
	...
	public override void OnBlockAdded(WorldBase world, Chunk _chunk, Vector3i _blockPos, BlockValue _blockValue, PlatformUserIdentifierAbs _addedByPlayer)
	{
		...
		if (tileEntityVendingMachine == null)
		{
			tileEntityVendingMachine = new TileEntityVendingMachine(_chunk);
			tileEntityVendingMachine.SetDisableModifiedCheck(_b: true);
			...
			tileEntityVendingMachine.SetDisableModifiedCheck(_b: false);
			if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
			{
	...
	public override void OnBlockRemoved(WorldBase world, Chunk _chunk, Vector3i _blockPos, BlockValue _blockValue)
	{
		...
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer && world.GetTileEntity(_blockPos) is TileEntityVendingMachine tileEntityVendingMachine)
		{
	...
	{
		_ = _world.GetTileEntity(_bvRef) is TileEntityVendingMachine;
	...
	public override bool OnBlockActivated(string _commandName, WorldBase _world, Vector3i _blockPos, BlockValue _blockValue, EntityPlayerLocal _player)
	{
		...
		if (null != uIForPlayer)
		{
			...
			{
			case "trade":
				_player.PlayOneShot("interact_vending");
		...
		if (_damagePoints > 0 && base.Properties.Values.ContainsKey("Buff"))
		{
			...
			if (entityAlive != null && entityAlive as EntityTurret == null)
			{
				...
				if (!flag)
				{
					...
					{
						entityAlive.Buffs.AddBuff(array[i].Trim(), _bvRef, entityAlive.entityId);
					}
				}
			}
		}
