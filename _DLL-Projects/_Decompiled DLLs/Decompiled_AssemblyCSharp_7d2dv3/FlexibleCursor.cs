...
public class FlexibleCursor : CursorControllerAbs
{
	...
	{
		if (hoverTarget != null && hoverTarget.IsActiveInHierarchy)
		{
			if (cursorWorldBounds.extents.x > hoverTarget.ColliderBounds.extents.x - OffsetSnapBounds)
			{
				return uiCamera.cachedCamera.WorldToScreenPoint(hoverTarget.ColliderBounds.center);
			}
			Vector3 vector = hoverTarget.ColliderBounds.ClosestPoint(uiCamera.cachedCamera.ScreenToWorldPoint(_newPos));
			Vector3 vector2 = Vector3.right * cursorWorldBounds.extents.x;
			Vector3 point = vector - vector2;
			Vector3 point2 = vector + vector2;
			if (!hoverTarget.ColliderBounds.Contains(point))
			{
				vector = hoverTarget.ColliderBounds.ClosestPoint(point) + vector2;
			}
			else if (!hoverTarget.ColliderBounds.Contains(point2))
			{
				vector = hoverTarget.ColliderBounds.ClosestPoint(point2) - vector2;
			}
			vector.y = hoverTarget.ColliderBounds.center.y;
