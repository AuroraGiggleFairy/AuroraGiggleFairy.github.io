...
public class XUiC_SkillSkillMilestone : XUiController
{
	...
	public ProgressionValue CurrentSkill
	{
		...
		get
		{
			if (xui.SelectedSkill == null || !xui.SelectedSkill.ProgressionClass.IsSkill)
			{
				return null;
			}
	...
	public void UpdateSkill()
	{
		...
		foreach (MinEffectGroup effectGroup in CurrentSkill.ProgressionClass.Effects.EffectGroups)
		{
			...
			foreach (PassiveEffect passiveEffect in effectGroup.PassiveEffects)
			{
				...
				int entityClass = xui.playerUI.entityPlayer.entityClass;
				if (EntityClass.list.ContainsKey(entityClass) && EntityClass.list[entityClass].Effects != null)
				{
					EntityClass.list[entityClass].Effects.ModifyValue(xui.playerUI.entityPlayer, passiveEffect.Type, ref _base_value, ref _perc_value, 0f, EntityClass.list[entityClass].Tags);
				}
				float num = _base_value;
				passiveEffect.ModifyValue(xui.playerUI.entityPlayer, levelGoal, ref _base_value, ref _perc_value, passiveEffect.Tags);
