...
public class vp_FPWeapon : vp_Weapon
{
	...
	public Vector3 RotationFallSwayAbsoluteSpringLimit = new Vector3(0f, 0f, 3f);
	...
	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Protected)]
	public Vector3 m_FallSwaySpringLimit = Vector3.zero;
	...
	public virtual void UpdateRetraction(bool firstIteration = true)
	{
		if (RetractionDistance != 0f)
		{
			...
			if (Physics.Linecast(vector, end, out var hitInfo, 1084850184) && !hitInfo.collider.isTrigger)
			{
	...
	public virtual void UpdateSwaying()
	{
		...
		m_FallSwaySpringLimit.x = Mathf.Abs(RotationFallSwayAbsoluteSpringLimit.x);
		m_FallSwaySpringLimit.y = Mathf.Abs(RotationFallSwayAbsoluteSpringLimit.y);
		m_FallSwaySpringLimit.z = Mathf.Abs(RotationFallSwayAbsoluteSpringLimit.z);
		if (m_FallSwaySpringLimit.x > 0f)
		{
			m_RotationSpring.State.x = Utils.FastClamp(m_RotationSpring.State.x, 0f - m_FallSwaySpringLimit.x, m_FallSwaySpringLimit.x);
		}
		if (m_FallSwaySpringLimit.y > 0f)
		{
			m_RotationSpring.State.y = Utils.FastClamp(m_RotationSpring.State.y, 0f - m_FallSwaySpringLimit.y, m_FallSwaySpringLimit.y);
		}
		if (m_FallSwaySpringLimit.z > 0f)
		{
			m_RotationSpring.State.z = Utils.FastClamp(m_RotationSpring.State.z, 0f - m_FallSwaySpringLimit.z, m_FallSwaySpringLimit.z);
		}
