...
public class DecoChunk
{
	...
	{
		Vector3i vector3i = _boundsPosition;
		Vector3i vector3i2 = _boundsPosition + _boundsSize;
		...
		for (int i = num; i <= num3; i++)
		{
			for (int j = num2; j <= num4; j++)
			{
				...
				{
					DecoObject decoObject = value[num5];
					Vector3i pos = decoObject.pos;
					if (pos.x >= vector3i.x && pos.x < vector3i2.x && pos.y >= vector3i.y && pos.y < vector3i2.y && pos.z >= vector3i.z && pos.z < vector3i2.z)
					{
						if (OcclusionManager.Instance.cullDecorations && (bool)decoObject.go)
						{
				...
				if (value.Count == 0)
				{
					decosPerSmallChunks.Remove(key);
				}
			}
		}
	}
