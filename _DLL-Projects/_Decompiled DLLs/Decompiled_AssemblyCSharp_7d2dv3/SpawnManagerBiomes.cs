...
public class SpawnManagerBiomes : SpawnManagerAbstract
{
	...
	public void SpawnUpdate(string _spawnerName, bool _isSpawnEnemy, ChunkAreaBiomeSpawnData _spawnData)
	{
		...
		if (!world.GetRandomSpawnPositionInAreaMinMaxToPlayers(_spawnData.area, minDistance, uNUSED_maxDistance, _checkBedrolls: true, out var _position, out var _chunk))
		{
			return;
		}
		...
		int num3 = Utils.FastMin(5, biomeSpawnEntityGroupList.list.Count);
		int num4 = 0;
		while (num4 < num3)
		{
			BiomeSpawnEntityGroupData biomeSpawnEntityGroupData2 = biomeSpawnEntityGroupList.list[num2];
			if ((_spawnData.groupsEnabledFlags & (1 << num2)) != 0 && (biomeSpawnEntityGroupData2.daytime == EDaytime.Any || biomeSpawnEntityGroupData2.daytime == eDaytime))
			{
				...
				if (!flag2 || _isSpawnEnemy)
				{
					...
					if (world.worldTime > delayWorldTime)
					{
						...
						_spawnData.ResetRespawn(idHash, world, num5);
					}
		...
		int randomFromGroup = EntityGroups.GetRandomFromGroup(biomeSpawnEntityGroupList.list[num].entityGroupName, ref lastClassId);
		if (randomFromGroup == 0)
		{
			_spawnData.DecMaxCount(idHash);
			return;
		}
		if (!EntityClass.list.TryGetValue(randomFromGroup, out var _))
		{
			Log.Error($"SpawnManagerBiomes invalid entity id {randomFromGroup}");
			return;
		}
		_spawnData.IncCount(idHash);
		EntityCreationData ecd = EntityFactory.SetupEntityCreationData(randomFromGroup, _position);
		_chunk.SpawnEntityAsync(world, ecd, [PublicizedFrom(EAccessModifier.Internal)] (Entity entity) =>
		{
			OnEntitySpawned(entity, _spawnData.chunk.Key, idHash);
		});
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void OnEntitySpawned(Entity _entity, long _masterChunkKey, int _biomeIdHash)
	{
		_entity.SetSpawnerSource(EnumSpawnerSource.Biome, _masterChunkKey, _biomeIdHash);
		world.DebugAddSpawnedEntity(_entity);
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void LogSpawn(string format, params object[] args)
	{
		format = $"{GameManager.frameCount} SpawnManagerBiomes {format}";
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public void LogSpawn(EntityCreationData _ecd, string format, params object[] args)
	{
		format = $"{GameManager.frameCount} SpawnManagerBiomes {_ecd.id} {EntityClass.GetEntityClass(_ecd.entityClass).entityClassName}, {format}";
		Log.Warning(format, args);
	}
}
