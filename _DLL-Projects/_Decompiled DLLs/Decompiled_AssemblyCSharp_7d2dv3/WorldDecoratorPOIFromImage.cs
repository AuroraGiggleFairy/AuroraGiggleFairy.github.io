...
public class WorldDecoratorPOIFromImage : IWorldDecorator
{
	...
	public WorldDecoratorPOIFromImage(string _levelName, string _worldPath, DynamicPrefabDecorator _prefabDecorator, int _worldX, int _worldZ, Texture2D _splat3Tex = null, bool _bChangeWaterDensity = true, int _worldScale = 1, HeightMap _heightMap = null, Texture2D _splat4Tex = null)
	{
	...
	public IEnumerator InitData()
	{
		...
		yield return null;
		List<WaterInfo> waterSources = LoadWaterInfo(worldPath + "/water_info.xml");
		...
		if (splat3Tex == null)
		{
			localSplat3 = (SdFile.Exists(worldPath + "/splat3.png") ? TextureUtils.LoadTexture(worldPath + "/splat3.png") : TextureUtils.LoadTexture(worldPath + " / splat3.tga"));
			...
			for (int j = 0; j < 16; j++)
			{
				...
				if (!poiForColor.m_BlockValue.isair)
				{
					...
					if (DecoUtils.CanPlaceDeco(_chunk, _c10, _c01, _c11, blockPos, blockValue))
					{
						...
						blockValue = block2.OnBlockPlaced(_world, blockPos, blockValue, gameRandom);
				...
				else if (poiForColor.m_sModelName != null && poiForColor.m_sModelName.Length > 0 && !decoAllowedAt.IsNothing())
				{
					_world.m_PrefabCache.GetPrefab(poiForColor.m_sModelName).CopyIntoLocal(_world.ChunkCache, new Vector3i(blockWorldPosX, num, blockWorldPosZ), _bOverwriteExistingBlocks: false, _bSetChunkToRegenerate: false, FastTags<TagGroup.Global>.none);
				}
			}
		}
