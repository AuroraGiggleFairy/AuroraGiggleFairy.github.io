...
public class XUiC_QuestTurnInRewardsWindow : XUiController
{
	...
	{
		float value = EffectManager.GetValue(PassiveEffects.QuestRewardChoiceCount, null, 1f, xui.playerUI.entityPlayer);
		if (selectedEntry != null)
		{
		...
		if (optionCount <= 1 || (float)selectedEntryList.Count == value)
		{
			...
			{
				GameManager.ShowTooltip(xui.playerUI.entityPlayer, Localization.Get("ttClearInventoryForTurnIn"), string.Empty, "ui_denied");
				selectedEntry.Chosen = false;
				selectedEntryList.Remove(selectedEntry);
			}
		}
	...
	public override void OnOpen()
	{
		base.OnOpen();
		SelectedEntry = null;
		CurrentQuest = xui.Dialog.QuestTurnIn;
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void SetupOptions()
	{
		float value = EffectManager.GetValue(PassiveEffects.QuestRewardOptionCount, null, currentQuest.QuestClass.RewardChoicesCount, xui.playerUI.entityPlayer);
		optionCount = 0;
		int num = 0;
		if (selectedEntry != null)
		{
	...
	public override bool GetBindingValueInternal(ref string value, string bindingName)
	{
		...
		case "questxpreward":
			...
			else
			{
				...
				for (int j = 0; j < currentQuest.Rewards.Count; j++)
				{
					...
					{
						int num3 = Mathf.FloorToInt((float)Convert.ToInt32(currentQuest.Rewards[j].Value) * Progression.XPGain);
						num2 += Mathf.FloorToInt(EffectManager.GetValue(PassiveEffects.PlayerExpGain, null, num3, xui.playerUI.entityPlayer, null, XUiM_Quest.QuestTag));
					}
				}
		...
		case "questitemrewards":
			value = XUiM_Quest.GetQuestItemRewards(currentQuest, xui.playerUI.entityPlayer, rewardItemFormat, rewardItemBonusFormat);
			return true;
		...
		case "chosentitle":
			...
			if (XUiM_Quest.HasQuestRewards(currentQuest, xui.playerUI.entityPlayer, _isChosen: true))
			{
				float value3 = EffectManager.GetValue(PassiveEffects.QuestRewardChoiceCount, null, 1f, xui.playerUI.entityPlayer);
		...
		case "acceptbuttontext":
			...
			if (SelectedEntry != null)
			{
				...
				{
					float value2 = EffectManager.GetValue(PassiveEffects.QuestRewardChoiceCount, null, 1f, xui.playerUI.entityPlayer);
