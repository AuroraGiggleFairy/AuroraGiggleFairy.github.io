...
public class ActionFillSafeZone : BaseAction
{
	...
	public override ActionCompleteStates OnPerformAction()
	{
		...
		case FillSafeZoneStates.HandleChunks:
		{
			...
			for (int num17 = -num15; num17 <= num15; num17++)
			{
				...
				for (int num18 = -num16; num18 <= num16; num18++)
				{
					...
					Chunk chunk3 = (Chunk)world.GetChunkFromWorldPos(new Vector3i(x5, claimPos.y, z5));
					if (chunk3 != null && !chunkList.Contains(chunk3))
					{
				...
				for (int num10 = -num2; num10 <= num2; num10++)
				{
					for (int num11 = -num2; num11 <= num2; num11++)
					{
						for (int num12 = -num2; num12 <= num2; num12++)
						{
							...
							BlockValue blockNoDamage4 = chunk2.GetBlockNoDamage(x4, y4, z4);
							if ((blockNoDamage4.isair || blockNoDamage4.Block.Tags.Test_AnySet(other)) && !blockedPositions.Contains(vector3i5))
							{
								Chunk item5 = (Chunk)chunk2;
								if (!chunkList.Contains(item5))
								{
									chunkList.Add(item5);
								}
								blockChanges.Add(new BlockChangeInfo(vector3i5, blockValue2));
							}
						}
					}
				}
				...
				for (int n = -num2; n <= num2; n++)
				{
					for (int num5 = -num2; num5 <= num2; num5++)
					{
						for (int num6 = -num2; num6 <= num2; num6++)
						{
							...
							if (Vector3.SqrMagnitude(claimPos - vector3i3) >= (float)distanceSq || !world.GetChunkFromWorldPos(vector3i3, ref chunk2))
							{
								continue;
							}
							...
							BlockValue blockNoDamage2 = chunk2.GetBlockNoDamage(x2, y2, z2);
							if ((blockNoDamage2.isair || blockNoDamage2.Block.Tags.Test_AnySet(other)) && !blockedPositions.Contains(vector3i3))
							{
								...
								if (!chunkList.Contains(item3))
								{
									chunkList.Add(item3);
								}
								blockChanges.Add(new BlockChangeInfo(vector3i3, blockValue2));
							}
						}
					}
				}
				...
				for (int num7 = -num2; num7 <= num2; num7++)
				{
					for (int num8 = -num2; num8 <= num2; num8++)
					{
						for (int num9 = -num2; num9 <= num2; num9++)
						{
							...
							if (Vector3.SqrMagnitude(new Vector3i(claimPos.x, vector3i4.y, claimPos.z) - vector3i4) >= (float)distanceSq || !world.GetChunkFromWorldPos(vector3i4, ref chunk2))
							{
								continue;
							}
							...
							BlockValue blockNoDamage3 = chunk2.GetBlockNoDamage(x3, y3, z3);
							if ((blockNoDamage3.isair || blockNoDamage3.Block.Tags.Test_AnySet(other)) && !blockedPositions.Contains(vector3i4))
							{
								...
								if (!chunkList.Contains(item4))
								{
									chunkList.Add(item4);
								}
								blockChanges.Add(new BlockChangeInfo(vector3i4, blockValue2));
							}
						}
					}
				}
				...
				for (int k = -num2; k <= num2; k++)
				{
					...
					for (int l = -num4; l <= num4; l++)
					{
						for (int m = -num4; m <= num4; m++)
						{
							...
							BlockValue blockNoDamage = chunk2.GetBlockNoDamage(x, y, z);
							if ((blockNoDamage.isair || blockNoDamage.Block.Tags.Test_AnySet(other)) && !blockedPositions.Contains(vector3i2))
							{
								...
								if (!chunkList.Contains(item2))
								{
									chunkList.Add(item2);
								}
								blockChanges.Add(new BlockChangeInfo(vector3i2, blockValue2));
							}
						}
					}
					num3++;
		...
		case FillSafeZoneStates.AddSigns:
		{
			...
			blockChanges.Add(new BlockChangeInfo(vector3i, blockValue));
		...
		case FillSafeZoneStates.ResetChunks:
		{
			...
			if (base.Owner.Target != null)
			{
				...
				{
					if (world.GetTileEntity(signPositions[j]).TryGetSelfOrFeature<TEFeatureSignable>(out var _typedTe))
					{
						_typedTe.SetText(ModifiedName, _syncData: true, playerDataFromEntityID?.PrimaryId);
					}
				}
			}
