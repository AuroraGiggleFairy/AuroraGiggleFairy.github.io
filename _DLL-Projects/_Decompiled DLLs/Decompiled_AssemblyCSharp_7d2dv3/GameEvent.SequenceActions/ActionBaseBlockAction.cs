		...
		for (int i = minOffset.y; i <= maxOffset.y; i++)
		{
			for (int j = minOffset.z; j <= maxOffset.z; j += spacing + 1)
			{
				...
				for (int k = minOffset.x; k <= maxOffset.x; k += spacing + 1)
				{
					...
					if (vector3i.y < 0 || !world.GetChunkFromWorldPos(vector3i, ref chunk) || world.GetTraderAreaAt(vector3i) != null || (checkSafe && (safeAllowed || !world.CanPlaceBlockAt(vector3i, null))))
					{
						continue;
					}
					int x = World.toBlockXZ(vector3i.x);
					int z = World.toBlockXZ(vector3i.z);
					BlockValue blockValue = (NeedsDamage() ? chunk.GetBlock(x, vector3i.y, z) : chunk.GetBlockNoDamage(x, vector3i.y, z));
