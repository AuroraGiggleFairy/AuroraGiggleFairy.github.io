		...
		if (ent is EntityVehicle)
		{
			...
			for (int i = 0; i < entityVehicle.GetAttachMaxCount(); i++)
			{
				...
				if (entityPlayer != null)
				{
					...
					{
						(entityPlayer as EntityPlayerLocal).PlayerUI.windowManager.CloseAllOpenModalWindows();
					}
		...
		else
		{
			...
			for (int num = 0; num < entityTurret.GetAttachMaxCount(); num++)
			{
				...
				if (entityPlayer2 != null)
				{
					...
					{
						(entityPlayer2 as EntityPlayerLocal).PlayerUI.windowManager.CloseAllOpenModalWindows();
					}
