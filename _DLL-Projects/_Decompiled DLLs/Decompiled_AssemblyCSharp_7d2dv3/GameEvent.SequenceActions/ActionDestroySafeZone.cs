...
public class ActionDestroySafeZone : BaseAction
{
	...
	public override ActionCompleteStates OnPerformAction()
	{
		...
		case DestroySafeZoneStates.FindClaim:
		{
			...
			TEFeatureLandClaim selfOrFeature = world.GetTileEntity(World.toBlock(claimPos)).GetSelfOrFeature<TEFeatureLandClaim>();
		...
		case DestroySafeZoneStates.HandleChunks:
		{
			...
			for (int num12 = -num10; num12 <= num10; num12++)
			{
				...
				for (int num13 = -num11; num13 <= num11; num13++)
				{
					...
					Chunk chunk6 = (Chunk)world.GetChunkFromWorldPos(new Vector3i(x4, claimPos.y, z4));
					if (chunk6 != null && !chunkList.Contains(chunk6))
					{
						chunkList.Add(chunk6);
						chunk6.StopStabilityCalculation = true;
					}
				}
			}
		...
		case DestroySafeZoneStates.SetupChanges:
		{
				...
				for (int l = -num; l <= num; l++)
				{
					for (int m = -num; m <= num; m++)
					{
						for (int n = -num; n <= num; n++)
						{
							...
							if (!chunk.IsAir(x2, y2, z2) && chunk.GetBlock(x2, y2, z2).Block.blockMaterial.CanDestroy)
							{
								Chunk chunk3 = (Chunk)chunk;
								if (!chunkList.Contains(chunk3))
								{
									chunkList.Add(chunk3);
									chunk3.StopStabilityCalculation = true;
								}
								if (chunk3.GetTileEntity(World.toBlock(vector3i2)).TryGetSelfOrFeature<ITileEntityLootable>(out var _typedTe2))
								{
									_typedTe2.SetEmpty();
									_typedTe2.SetModified();
								}
								blockChanges.Add(new BlockChangeInfo(vector3i2, BlockValue.Air));
							}
						}
					}
				}
				...
				for (int num2 = -num; num2 <= num; num2++)
				{
					for (int num3 = -num; num3 <= num; num3++)
					{
						for (int num4 = -num; num4 <= num; num4++)
						{
							...
							if (Vector3.SqrMagnitude(claimPos - vector3i3) >= (float)distanceSq || !world.GetChunkFromWorldPos(vector3i3, ref chunk))
							{
								continue;
							}
							...
							if (!chunk.IsAir(x3, y3, z3) && chunk.GetBlock(x3, y3, z3).Block.blockMaterial.CanDestroy)
							{
								Chunk chunk4 = (Chunk)chunk;
								if (!chunkList.Contains(chunk4))
								{
									chunkList.Add(chunk4);
									chunk4.StopStabilityCalculation = true;
								}
								if (chunk4.GetTileEntity(World.toBlock(vector3i3)).TryGetSelfOrFeature<ITileEntityLootable>(out var _typedTe3))
								{
									_typedTe3.SetEmpty();
									_typedTe3.SetModified();
								}
								blockChanges.Add(new BlockChangeInfo(vector3i3, BlockValue.Air));
							}
						}
					}
				}
				...
				for (int i = -num; i <= num; i++)
				{
					for (int j = -num; j <= num; j++)
					{
						for (int k = -num; k <= num; k++)
						{
							...
							if (Vector3.SqrMagnitude(new Vector3i(claimPos.x, vector3i.y, claimPos.z) - vector3i) >= (float)distanceSq || !world.GetChunkFromWorldPos(vector3i, ref chunk))
							{
								continue;
							}
							...
							if (!chunk.IsAir(x, y, z) && chunk.GetBlock(x, y, z).Block.blockMaterial.CanDestroy)
							{
								...
								if (chunk2.GetTileEntity(World.toBlock(vector3i)).TryGetSelfOrFeature<ITileEntityLootable>(out var _typedTe))
								{
									_typedTe.SetEmpty();
									_typedTe.SetModified();
								}
								blockChanges.Add(new BlockChangeInfo(vector3i, BlockValue.Air));
							}
						}
					}
				}
			...
			case DestructionTypes.LandClaimOnly:
				blockChanges.Add(new BlockChangeInfo(claimPos, BlockValue.Air));
				break;
			}
		...
		case DestroySafeZoneStates.AddSigns:
		{
			...
			blockChanges.Add(new BlockChangeInfo(vector3i4, blockValue));
		...
		case DestroySafeZoneStates.ResetChunks:
		{
			...
			{
				Chunk chunk5 = chunkList[num5];
				long item = WorldChunkCache.MakeChunkKey(chunk5.X, chunk5.Z);
				chunk5.StopStabilityCalculation = false;
				chunkHash.Add(item);
			}
			if (signPositions != null && base.Owner.Target != null)
			{
				...
				{
					if (world.GetTileEntity(signPositions[num6]).TryGetSelfOrFeature<TEFeatureSignable>(out var _typedTe4))
					{
						_typedTe4.SetText(ModifiedName, _syncData: true, playerDataFromEntityID?.PrimaryId);
					}
				}
			}
