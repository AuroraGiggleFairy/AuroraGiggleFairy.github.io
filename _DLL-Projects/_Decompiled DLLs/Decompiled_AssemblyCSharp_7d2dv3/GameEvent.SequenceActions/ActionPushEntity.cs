...
public class ActionPushEntity : ActionBaseTargetAction
{
	...
	public override ActionCompleteStates PerformTargetAction(Entity target)
	{
		...
		if (entityAlive != null)
		{
			...
			_dmResponse.StunDuration = 1f;
			_dmResponse.Strength = (int)(force * 100f);
			...
			_dmResponse.Source = new DamageSource(EnumDamageSource.External, EnumDamageTypes.Bashing, vector);
			entityAlive.DoRagdoll(in _dmResponse);
		}
	...
	public bool CheckValidPosition(ref Vector3 newPoint, EntityAlive target)
	{
		...
		if (Voxel.Raycast(world, ray, force, -538750989, 67, 0f))
		{
