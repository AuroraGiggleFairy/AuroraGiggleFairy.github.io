...
public class EAISetNearestEntityAsTarget : EAITarget
{
	...
	public void FindTarget()
	{
		...
		for (int i = 0; i < targetClasses.Count; i++)
		{
			...
			if (targetClass.type == typeof(EntityPlayer))
			{
				...
				if ((bool)theEntity.attractPlayer && !closeTargetEntity && !theEntity.IsSleeping && !theEntity.HasInvestigatePosition)
				{
					SeekBreadcrumb(theEntity.attractPlayer, 15f);
				}
				...
				{
					SeekBreadcrumb(theEntity.smellPlayer, 24f);
				}
			}
			else
			{
	...
	public void PlaySoundSense()
	{
		...
		if (senseSoundTime - time < 0f)
		{
			...
			theEntity.PlayOneShot(theEntity.GetSoundSense());
		}
	}
	...
	public void FindTargetPlayer(float seeDist)
	{
		...
		theEntity.world.GetPlayersAround(theEntity.position, seeDist, playerList);
		if (theEntity.IsSleeping)
		{
			...
			{
				EntityPlayer entityPlayer2 = playerList[i];
		...
		else
		{
			...
			{
				EntityPlayer entityPlayer3 = playerList[j];
