...
public abstract class ItemAction : XMLData.Item.ItemActionData
{
	...
	public enum CanRepairValues
	{
		Allowed,
		NoLongerRepairable,
		DurabilityWouldBeReduced
	}
	...
	public static float ItemDegradationModifier = 1f;

	public static float ItemMaxDegrationAmount = 0f;

	public static RepairTypes RepairType = RepairTypes.Both;
	...
	public static void HandleDegradation(ItemValue _outItem)
	{
		if (ItemMaxDegrationAmount != 0f)
		{
			if (!_outItem.TryGetMetadata("DurabilityModifier", out float value))
			{
				value = 1f;
				_outItem.SetMetadata("DurabilityModifier", value);
			}
			else
			{
				value -= ItemMaxDegrationAmount;
	...
	public void HandleJamSound(ItemValue itemValue, ItemClass itemClass, EntityPlayerLocal localPlayer)
	{
		switch (CanRepair(itemValue))
		{
		case CanRepairValues.Allowed:
			if (itemClass.Properties.Values.ContainsKey(ItemClass.PropSoundJammed))
			{
				Manager.PlayInsidePlayerHead(itemClass.Properties.Values[ItemClass.PropSoundJammed]);
			}
			GameManager.ShowTooltip(localPlayer, "ttItemNeedsRepair");
			break;
		case CanRepairValues.NoLongerRepairable:
			if (itemClass.Properties.Values.ContainsKey(ItemClass.PropSoundJammed))
			{
				Manager.PlayInsidePlayerHead(itemClass.Properties.Values[ItemClass.PropSoundJammed]);
			}
			GameManager.ShowTooltip(localPlayer, "ttItemNoLongerRepairable");
			break;
		}
	}

	public static CanRepairValues CanRepair(ItemValue _itemToBeRepaired)
	{
		if (ItemMaxDegrationAmount == 0f)
		{
			return CanRepairValues.Allowed;
		}
		if (!_itemToBeRepaired.TryGetMetadata("DurabilityModifier", out float value))
		{
			return CanRepairValues.Allowed;
		}
		float num = value - ItemMaxDegrationAmount;
		if (num < ItemMaxDegrationAmount)
		{
			return CanRepairValues.NoLongerRepairable;
		}
		if (num * (float)_itemToBeRepaired.MaxUseTimes < (float)_itemToBeRepaired.MaxUseTimes - _itemToBeRepaired.UseTimes)
		{
			return CanRepairValues.DurabilityWouldBeReduced;
		}
		return CanRepairValues.Allowed;
	}
