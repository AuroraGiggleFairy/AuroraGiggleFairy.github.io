...
public class XUiC_LightEditor : XUiController
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public XUiView panelAngle;

	[PublicizedFrom(EAccessModifier.Private)]
	public XUiView panelRate;

	[PublicizedFrom(EAccessModifier.Private)]
	public XUiView panelDelay;
	...
	public static void Open(LocalPlayerUI _playerUi, TileEntityLight _te, Vector3i _blockPos, World _world, BlockLight _block)
	{
		...
		childByType.chunk = (Chunk)chunkCache.GetChunkSync(World.toChunkXZ(_blockPos.x), _blockPos.y, World.toChunkXZ(_blockPos.z));
	...
	public override void Init()
	{
		...
		panelAngle = GetChildById("panelAngle").ViewComponent;
		panelRate = GetChildById("panelRate").ViewComponent;
		panelDelay = GetChildById("panelDelay").ViewComponent;
	...
	public void BtnSave_OnPressed(XUiController _sender, int _mouseButton)
	{
		...
		xui.playerUI.windowManager.Close("lightProperties");
	}
	...
	{
		Transform transform = DataLoader.LoadAsset<GameObject>(block.Properties.GetValue("Model")).transform.Find("MainLight");
		if (!transform)
		{
			Log.Error("MainLight missing for {0}", block.Properties.GetValue("Model"));
		}
		else
		{
