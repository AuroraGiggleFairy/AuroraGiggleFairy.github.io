...
public class RenderDisplacedCube
{
	...
	public void Cleanup()
	{
		...
		if (transformParent != null)
		{
			...
			UnityEngine.Object.Destroy(transformParent.gameObject);
	...
	public void rebuildCubeOnBounds(BlockValue _blockValue)
	{
		...
		if (!(shape is BlockShapeModelEntity) && !(shape is BlockShapeNew))
		{
	...
	public bool update0(World _world, WorldRayHitInfo _hitInfo, Vector3i _focusBlockPos, EntityAlive _player, PersistentPlayerData _ppLocal, bool _bAlternativeBlockPos, BlockValue _holdingBlockValue, TextureFullArray _texture)
	{
		...
		if (_player.inventory.holdingItemData is ItemClassBlock.ItemBlockInventoryData itemBlockInventoryData)
		{
		...
		else
		{
			...
			mode = BlockPlacement.EnumRotationMode.Advanced;
			localRot = 0;
			propTransform = PropTransform.identity;
		}
		HitInfoDetails hitInfo = _hitInfo.hit.Clone();
		hitInfo.blockPos = _hitInfo.lastBlockPos;
		BlockPlacement.Result result = _holdingBlockValue.Block.BlockPlacementHelper.OnPlaceBlock(placement, mode, localRot, _world, _holdingBlockValue, propTransform, hitInfo, _player.GetPosition());
		_holdingBlockValue.rotation = result.blockValue.rotation;
		Vector3 zero = Vector3.zero;
		_world.GetBlock(_focusBlockPos);
		...
		{
			sbyte density = _world.GetDensity(_focusBlockPos);
			sbyte density2 = _world.GetDensity(_focusBlockPos - Vector3i.up);
			zero.y += MarchingCubes.GetDecorationOffsetY(density, density2);
		}
		if (!_bAlternativeBlockPos)
		{
			_focusBlockPos = _holdingBlockValue.Block.GetFreePlacementPosition(_world, _focusBlockPos, _holdingBlockValue, _player);
		}
		Vector3 vector = result.placement switch
		{
			BlockPlacement.EnumPlacement.Voxel => new Vector3(0.5f, 0.5f, 0.5f), 
			BlockPlacement.EnumPlacement.Free => result.pos - result.blockPos + result.blockFace.GetNormal() * 0.5f, 
			_ => throw new ArgumentOutOfRangeException(), 
		};
		Vector3 vector2 = _focusBlockPos + vector;
		Vector3 vector3 = vector2 + zero;
		if ((vector3 - transformLastPos).sqrMagnitude > 64f)
		{
			transformLastPos = vector3;
		}
		Vector3 current = Vector3.LerpUnclamped(transformLastPos, vector3, 0.3f);
		current = (transformLastPos = Vector3.MoveTowards(current, vector3, 5f * Time.deltaTime));
		...
		if (!_bAlternativeBlockPos && ItemClass.GetForId(_holdingBlockValue.type) != null && ItemClass.GetForId(_holdingBlockValue.type).IsBlock())
		{
			...
			if (num != 0)
			{
				...
				if (transformBlockPreview != null)
				{
					...
					else if (block.shape is BlockShapeModelEntity)
					{
						...
						transformBlockPreview.GetComponentsInChildren(renderers);
						...
						transformBlockPreview.GetComponentsInChildren(signRenderers);
						float previewCanvasRotation = (float)(int)TEFeatureCanvas.GetUprightRotation(focusTransformBlockValue.rotation) * MathF.PI * 0.5f;
						SignDataManager.Instance.TryApplyRenderingData(SignData.defaultSignDataID, [PublicizedFrom(EAccessModifier.Internal)] (MaterialPropertyBlock mpb) =>
						{
							mpb.SetFloat(SignShaderIDs._CanvasRotation, previewCanvasRotation);
						}, signRenderers);
			...
			{
				bool flag2 = focusTransformBlockValue.Block.CanPlaceBlockAt(_world, _focusBlockPos, _holdingBlockValue);
				...
				{
					UnityEngine.Object.DestroyImmediate(cachedLandClaimBoundary);
				}
				...
				float num4 = num2;
				float magnitude = (_player.getHeadPosition() - vector2).magnitude;
				if (magnitude < 1f)
				{
					num4 *= magnitude / 1f;
				}
				...
				if ((bool)transformFocusCubePrefab)
				{
					...
					componentsInChildren = transformFocusCubePrefab.GetComponentsInChildren<Renderer>();
					for (int num5 = 0; num5 < componentsInChildren.Length; num5++)
					{
						componentsInChildren[num5].material.SetColor("_Color", value2);
					}
				}
				if (cachedLandClaimBoundary != null)
				{
				...
				Renderer[] componentsInChildren2 = transformBlockPreview.GetComponentsInChildren<Renderer>();
				bool flag5 = false;
				componentsInChildren = componentsInChildren2;
				for (int num5 = 0; num5 < componentsInChildren.Length; num5++)
				{
					if (componentsInChildren[num5].shadowCastingMode == ShadowCastingMode.ShadowsOnly)
					{
						flag5 = true;
						break;
					}
	...
	public void DestroyPreview()
	{
		if ((bool)transformBlockPreview)
		{
			...
			UnityEngine.Object.Destroy(transformBlockPreview.gameObject);
			transformBlockPreview = null;
		}
		for (int j = 0; j < blockPreviewMats.Count; j++)
		{
			UnityEngine.Object.Destroy(blockPreviewMats[j]);
		}
