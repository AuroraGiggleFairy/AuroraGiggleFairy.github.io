...
public class AIDirectorBloodMoonComponent : AIDirectorComponent
{
	...
	public override void Read(BinaryReader _stream, int _version)
	{
		...
		if (_version >= 8)
		{
			...
			int bloodMoonFrequency = BloodMoonFrequency;
			int bloodMoonRange = BloodMoonRange;
	...
	public override void Write(BinaryWriter _stream)
	{
		...
		_stream.Write(bmDay);
		_stream.Write((short)BloodMoonFrequency);
		_stream.Write((short)BloodMoonRange);
	}
	...
	{
		parties.Add(new AIDirectorBloodMoonParty(_player, this, BloodMoonEnemyCount));
	}
	...
	public void CalcNextDay(bool isSeek = false)
	{
		...
		else
		{
			...
			int num3 = GameUtils.WorldTimeToDays(Director.World.worldTime);
			while (num3 <= bmDayLast)
			{
				bmDayLast -= num2;
			}
			if (bmDayLast < 0)
			...
			while (num < num3);
			bmDayLast = num - num2;
			if (isSeek && bmDay > bmDayLast && bmDay <= bmDayLast + bloodMoonFrequency + bloodMoonRange)
			{
				num = bmDay;
			}
		}
	...
	public void SetDay(int day)
	{
		...
		{
			bmDay = day;
			Log.Out("BloodMoon SetDay: day {0}, last day {1}, freq {2}, range {3}", bmDay, bmDayLast, BloodMoonFrequency, BloodMoonRange);
		}
	}
