...
public class XUiC_ItemInfoWindow : XUiC_InfoWindow
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public readonly CachedStringFormatterXuiRgbaColor removeddurabilitycolorFormatter = new CachedStringFormatterXuiRgbaColor();
	...
	public bool isOpenAsTrader
	{
		...
		get
		{
			if (xui.Trader != null)
			{
				return xui.Trader.TraderData != null;
			}
			return false;
		}
	}
	...
	public ItemStack EquippedStack
	{
		get
		{
			...
			{
				return xui.PlayerEquipment.GetStackFromSlot(itemClassArmor.EquipSlot);
			}
			return compareStack;
		}
	}
	...
	{
		base.Update(_dt);
		bool flag = extraStatsShown;
		extraStatsShown = InputUtils.ShiftKeyPressed || LocalPlayerUI.GetUIForPrimaryPlayer().playerInput.GUIActions.Inspect.IsPressed;
		IsDirty = extraStatsShown != flag;
		if (IsDirty && base.ViewComponent.IsVisible)
		{
			if (emptyInfoWindow == null)
			{
				emptyInfoWindow = (XUiC_InfoWindow)xui.FindWindowGroupByName("backpack").GetChildById("emptyInfoPanel");
			}
	...
	public override bool GetBindingValueInternal(ref string value, string bindingName)
	{
		...
		case "itemammoname":
			...
			if (this.itemClass != null)
			{
				if (this.itemClass.Actions[0] is ItemActionRanged itemActionRanged)
				{
					...
					{
						ItemClass itemClass2 = ItemClass.GetItemClass(itemActionRanged.MagazineItemNames[itemStack.itemValue.SelectedAmmoTypeIndex]);
				...
				{
					ItemClass itemClass3 = ItemClass.GetItemClass(itemActionLauncher.MagazineItemNames[itemStack.itemValue.SelectedAmmoTypeIndex]);
					value = itemClass3.GetLocalizedItemName();
				}
			}
			return true;
		case "itemicon":
		...
		case "itemcost":
			...
			if (this.itemClass != null)
			{
				...
				if (isBuying)
				{
					if (useCustomMarkup)
					{
						value = itemcostFormatter.Format(XUiM_Trader.GetBuyPrice(xui, itemStack.itemValue, count, this.itemClass, selectedTraderItemStack?.SlotIndex ?? (-1)));
						return true;
					}
					value = itemcostFormatter.Format(XUiM_Trader.GetBuyPrice(xui, itemStack.itemValue, count, this.itemClass));
				}
				else
				{
					int sellPrice = XUiM_Trader.GetSellPrice(xui, itemStack.itemValue, count, this.itemClass);
		...
		case "pricelabel":
			...
			if (this.itemClass != null)
			{
				...
				if (isBuying)
				{
					value = ((XUiM_Trader.GetBuyPrice(xui, itemStack.itemValue, count2, this.itemClass) > 0) ? Localization.Get("xuiBuyPrice") : "");
				}
				else
				{
					value = ((XUiM_Trader.GetSellPrice(xui, itemStack.itemValue, count2, this.itemClass) > 0) ? Localization.Get("xuiSellPrice") : "");
				}
			}
			return true;
		case "markup":
			...
			{
				int v2 = 0;
				if (selectedTraderItemStack != null && selectedTraderItemStack.SlotIndex != -1)
				{
					v2 = xui.Trader.TraderData.PrimaryInventory[selectedTraderItemStack.SlotIndex].Markup * 20;
				}
				value = markupFormatter.Format(v2);
			}
			return true;
		case "itemicontint":
		{
		...
		case "haspermadurability":
			if (itemStack?.itemValue == null || !this.itemClass.ShowQualityBar || !EntityPlayerLocal.PermaDegrationOn)
			{
				value = "false";
				return true;
			}
			value = (itemStack.itemValue.MaxDurabilityModifier < 1f).ToString();
			return true;
		case "durabilitycolor":
		{
			...
			{
				v3 = QualityInfo.GetTierColor(itemStack.itemValue.Quality);
			}
			value = durabilitycolorFormatter.Format(v3);
			return true;
		}
		case "durabilityfill":
			value = ((itemStack?.itemValue == null) ? "0.0" : ((itemStack.itemValue.MaxUseTimes == 0) ? "1" : durabilityfillFormatter.Format(((float)itemStack.itemValue.MaxUseTimes - itemStack.itemValue.UseTimes) / (float)itemStack.itemValue.MaxUseTimesUI)));
			return true;
		case "removeddurabilityfill":
			...
			value = removeddurabilityfillFormatter.Format(itemStack.itemValue.MaxDurabilityModifier);
			return true;
		...
		case "itemtypeicon":
			...
			if (!itemStack.IsEmpty() && this.itemClass != null)
			{
				...
				{
					if (this.itemClass.AltItemTypeIcon != null && this.itemClass.Unlocks != "" && XUiM_ItemStack.CheckKnown(xui.playerUI.entityPlayer, this.itemClass, itemStack.itemValue))
					{
		...
		case "itemtypeicontint":
			...
			if (!itemStack.IsEmpty() && this.itemClass != null && this.itemClass.Unlocks != "" && XUiM_ItemStack.CheckKnown(xui.playerUI.entityPlayer, this.itemClass, itemStack.itemValue))
			{
	...
	public string GetStatTitle(int index)
	{
		...
		string text = displayInfoEntry.TitleOverride ?? UIDisplayInfoManager.Current.GetLocalizedName(displayInfoEntry.StatType);
		if (itemStack.itemValue.GetStatPercent(displayInfoEntry.StatType, _onlyBoosted: true) != 1f)
		{
			text = "[FFAF0B]" + text;
		}
		return text;
	...
	public string GetStatValue(int index)
	{
		...
		{
			return XUiM_ItemStack.GetStatItemValueTextWithCompareInfo(itemStack.itemValue, CompareStack.itemValue, xui.playerUI.entityPlayer, infoEntry, flipCompare: false, useMods: false);
		}
		if (!EquippedStack.IsEmpty())
		{
			return XUiM_ItemStack.GetStatItemValueTextWithCompareInfo(itemStack.itemValue, EquippedStack.itemValue, xui.playerUI.entityPlayer, infoEntry, flipCompare: true, useMods: false);
		}
		return XUiM_ItemStack.GetStatItemValueTextWithCompareInfo(itemStack.itemValue, CompareStack.itemValue, xui.playerUI.entityPlayer, infoEntry);
	}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public void ClearSelectedStacks()
	{
	...
	public void ShowEmptyInfo()
	{
		...
		mainActionItemList.SetCraftingActionList(XUiC_ItemActionList.ItemActionListTypes.None, null);
		traderActionItemList.SetCraftingActionList(XUiC_ItemActionList.ItemActionListTypes.None, null);
	...
	public void SetInfo(ItemStack stack, XUiController controller, XUiC_ItemActionList.ItemActionListTypes actionListType)
	{
		...
		if (isOpenAsTrader)
		{
			...
			useCustomMarkup = selectedTraderItemStack != null && xui.Trader.Trader is TileEntityVendingMachine && (xui.Trader.TraderData.TraderInfo.PlayerOwned || xui.Trader.TraderData.TraderInfo.Rentable);
			...
			int num2 = (isBuying ? Math.Min(itemStack.count, xui.PlayerInventory.CountAvailableSpaceForItem(itemStack.itemValue)) : itemStack.count);
