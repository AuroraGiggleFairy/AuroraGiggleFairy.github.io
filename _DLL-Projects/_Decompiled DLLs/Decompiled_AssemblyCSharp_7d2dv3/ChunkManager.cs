...
public class ChunkManager : IChunkProviderIndicator
{
	...
	public Chunk FindNextChunkToCopy()
	{
		...
		Chunk chunkSync = chunkCache.GetChunkSync(num);
		if (chunkSync == null)
		{
			return null;
		}
	...
	public void freeChunkGameObjects()
	{
		lock (chunksToFreeArr)
		{
			...
			for (int i = 0; i < m_ChunksToFree.Count; i++)
			{
				...
				{
					FreeChunkGameObject(chunkCache, chunkSync);
	...
	public bool doCopyChunksToUnity()
	{
		...
		if (!isContinueCopying && currentCopiedChunk != null)
		{
			...
			if (chunkCache == null || chunkCache.GetChunkSync(currentCopiedChunk.Key) != currentCopiedChunk)
			{
				if (chunkCache.DisplayedChunkGameObjects.ContainsKey(currentCopiedChunkGameObject.chunk.Key))
				{
					FreeChunkGameObject(chunkCache, currentCopiedChunk);
				}
				else
				{
			...
			if (!currentCopiedChunk.NeedsCopying)
			{
				...
				if (currentCopiedChunk.displayState == Chunk.DisplayState.BlockEntities && chunkCache != null)
				{
					currentCopiedChunk.OnDisplayBlockEntities(m_World, currentCopiedChunkGameObject.blockEntitiesParentT, chunkCache);
				}
		...
		if (currentCopiedChunk == null)
		{
			...
			if (!chunkCache.DisplayedChunkGameObjects.TryGetValue(key, out currentCopiedChunkGameObject))
			{
				currentCopiedChunkGameObject = GetNextFreeChunkGameObject();
				currentCopiedChunkGameObject.SetChunk(currentCopiedChunk, chunkCache);
				currentCopiedChunkGameObject.gameObject.SetActive(value: true);
				chunkCache.SetDisplayedChunkGameObject(key, currentCopiedChunkGameObject);
			}
	...
	public void task_Lighting(ThreadManager.TaskInfo _taskInfo)
	{
		...
		for (int i = 0; i < m_World.Players.list.Count * 2; i++)
		{
			...
			while (true)
			{
				...
				chunk.ExitWriteLock();
				if (!chunkCache.GetNeighborChunks(chunk, neighborsLightingThread))
				{
				...
				chunkCache.LightChunk(chunk, neighborsLightingThread);
				chunk.InProgressLighting = false;
	...
	public void task_CalcChunkPositions(ThreadManager.ThreadInfo _threadInfo)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (chunkCache2 != null)
			{
				long key = chunkCache2.ToLocalKey(collisionChunkPositionsCopy[num4]);
				...
				{
					if (!chunkSync2.hasEntities && chunkCache2.GetNeighborChunks(chunkSync2, neighborsGenerationThread2) && !hasAnyChunkEntities(neighborsGenerationThread2))
					{
	...
	public int thread_Regenerating(ThreadManager.ThreadInfo _threadInfo)
	{
		if (!_threadInfo.TerminationRequested())
		{
			...
			if (num > MaxQueuedMeshLayers)
			{
				...
				{
					Log.Warning($"ChunkManager mesh regeneration thread blocked for {timeSpan.TotalSeconds:F1}s - VML max queued exceeded by {num2}, (Instance: {instanceCount}, Pooled: {poolSize}, MaxQueued: {MaxQueuedMeshLayers})");
			...
			if (vmlExhaustionStartTime != DateTime.MinValue)
			{
				...
				{
					Log.Out($"ChunkManager mesh regeneration thread resumed after {timeSpan2.TotalSeconds:F1}s blocked");
				}
	...
	public Chunk RegenerateNextChunk(List<long> _chunkPositions, bool _isOnlyColliders)
	{
		for (int i = 0; i < _chunkPositions.Count; i++)
		{
			...
			chunkSync.ExitWriteLock();
			if (!chunkCache.GetNeighborChunks(chunkSync, regenerateNextChunkNeighbors))
			{
			...
			chunkCache.RegenerateChunk(chunkSync, regenerateNextChunkNeighbors);
			chunkSync.InProgressRegeneration = false;
	...
	public void DetermineChunksToLoad()
	{
		...
		if (flag)
		{
			lock (lockObject)
			{
				...
				m_ViewingChunkPositions.Clear();
				m_AllChunkPositions.Clear();
				for (int m = 0; m <= m_AllChunkPositions.buckets.Count; m++)
				{
					for (int n = 0; n < m_ObservedEntities.Count; n++)
					{
						ChunkObserver chunkObserver2 = m_ObservedEntities[n];
						if (m < chunkObserver2.viewDim + 2)
						{
							m_AllChunkPositions.Add(m, chunkObserver2.chunksAround.buckets.array[m]);
							if (chunkObserver2.bBuildVisualMeshAround)
							{
								m_ViewingChunkPositions.buckets.array[m].UnionWithHashSetLong(chunkObserver2.chunksAround.buckets.array[m]);
							}
						}
					}
				}
				...
				if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
				{
					...
					m_CollisionChunkPositions.Clear();
					for (int num2 = 0; num2 < m_CollisionChunkPositions.buckets.Count; num2++)
					{
						m_CollisionChunkPositions.buckets.array[num2].UnionWithHashSetLong(m_AllChunkPositions.buckets.array[num2]);
						m_CollisionChunkPositions.buckets.array[num2].ExceptWithHashSetLong(m_ViewingChunkPositions.buckets.array[num2]);
					}
					...
					if (chunkCache != null && !chunkCache.IsFixedSize)
					{
						lock (chunksToUnload)
						{
							...
							for (int num3 = 0; num3 < chunksToUnload.list.Count; num3++)
							{
								chunkCache.RemoveChunk(chunksToUnload.list[num3]);
							}
						}
					}
				}
		...
		lock (chunksToUnload)
		{
			...
			for (int num2 = chunksToUnload.list.Count - 1; num2 >= 0; num2--)
			{
				...
				if (flag)
				{
					...
					{
						FreeChunkGameObject(chunkCache, chunk);
						num++;
					}
	...
	public Chunk checkChunkNeedsCopying(long key)
	{
		...
		{
			flag = chunkCache.DisplayedChunkGameObjects.ContainsKey(chunkSync.Key);
		}
	...
	public IList<ChunkGameObject> GetDisplayedChunkGameObjects()
	{
		...
		if (chunkCache != null)
		{
			tempDisplayedCGOs.AddRange(chunkCache.DisplayedChunkGameObjects.Dict.Values);
		}
		return tempDisplayedCGOs;
	}
	...
	{
		return (m_World.ChunkCache?.DisplayedChunkGameObjects?.Count).GetValueOrDefault();
	}
	...
	{
		format = $"{GameManager.frameCount} {format}";
	...
	public void LogCurrentGenerationState()
	{
		...
		if (num > MaxQueuedMeshLayers)
		{
			...
			Log.Warning($"[FELLTHROUGHWORLD] ChunkManager mesh regeneration thread blocked for {timeSpan.TotalSeconds:F1}s - VML max queued exceeded by {num2}, (Instance: {instanceCount}, Pooled: {poolSize}, MaxQueued: {MaxQueuedMeshLayers})");
		}
		else
		{
			Log.Out($"[FELLTHROUGHWORLD] ChunkManager mesh regeneration thread running - VML pool (Instance: {instanceCount}, Pooled: {poolSize}, Queued: {num})");
		}
		if (threadInfoRegenerating.HasTerminated())
		{
		...
		if (threadInfoCalc.HasTerminated())
		{
