...
public static class SDCSUtils
{
	...
	{
		[PublicizedFrom(EAccessModifier.Private)]
		public List<LoadManager.IAssetHandle> trackedAssetHandles = new List<LoadManager.IAssetHandle>();

		public AssetRefs()
		{
		}

		public AssetRefs(AssetRefs other)
		{
			foreach (LoadManager.IAssetHandle trackedAssetHandle in other.trackedAssetHandles)
			{
				trackedAssetHandles.Add(trackedAssetHandle.Copy());
			}
		}

		public void AddAssetHandle(LoadManager.IAssetHandle handle)
		{
			trackedAssetHandles.Add(handle);
		}

		public void ReleaseAssets()
		{
			foreach (LoadManager.IAssetHandle trackedAssetHandle in trackedAssetHandles)
			{
				trackedAssetHandle.Release();
			}
			trackedAssetHandles.Clear();
		}
		...
		public void LogAssets()
		{
			foreach (LoadManager.IAssetHandle trackedAssetHandle in trackedAssetHandles)
			{
				Log.Out($"LogAssets {trackedAssetHandle}");
			}
		}
	}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static Dictionary<int, AssetRefs> rigOwnedAssets = new Dictionary<int, AssetRefs>();

	[PublicizedFrom(EAccessModifier.Private)]
	public static AssetRefs tmpAssetsOwner;
	...
	public static GameObject Stitch(GameObject sourceObj, GameObject parentObj, TransformCatalog boneCatalog, EModelSDCS emodel = null, bool isFPV = false, bool isUI = false, Material eyeMat = null, bool isGear = false)
	{
		...
		foreach (SkinnedMeshRenderer tempSMR in tempSMRs)
		{
			...
			if (tempSMR.gameObject.name == "eyes" && (bool)eyeMat)
			{
				sharedMaterials[0] = eyeMat;
			}
			for (int j = 0; j < sharedMaterials.Length; j++)
			{
				sharedMaterials[j] = new Material(sharedMaterials[j]);
			}
			tempSMR.sharedMaterials = sharedMaterials;
			bool flag = gameObject.name.StartsWith("body");
			for (int k = 0; k < sharedMaterials.Length; k++)
			{
				if (sharedMaterials[k].HasFloat("_ClipFPV") && flag && (bool)emodel && !isUI && isFPV && isGear)
				{
					sharedMaterials[k].SetFloat("_ClipFPV", 1f);
					RemoveFPViewObstructingGearPolygons(tempSMR, k);
				}
			}
		}
		tempSMRs.Clear();
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static void RemoveFPViewObstructingGearPolygons(SkinnedMeshRenderer smr, int submeshIndex)
	{
		...
		mesh.SetTriangles(array, submeshIndex);
	}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static AssetRefs RemoveAssetOwnerForRig(GameObject rig)
	{
		if (rig == null)
		{
			return null;
		}
		int instanceID = rig.GetInstanceID();
		rigOwnedAssets.Remove(instanceID, out var value);
		return value;
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public static AssetRefs GetAssetOwnerForRig(GameObject rig)
	{
		int instanceID = rig.GetInstanceID();
		if (!rigOwnedAssets.TryGetValue(instanceID, out var value))
		{
			value = new AssetRefs();
			rigOwnedAssets.Add(instanceID, value);
		}
		return value;
	}
	...
	{
		AssetRefs assetRefs = RemoveAssetOwnerForRig(baseRig);
	...
	{
		AssetRefs assetRefs = RemoveAssetOwnerForRig(baseRigFP);
		...
		tmpAssetsOwner = GetAssetOwnerForRig(baseRigFP);
		if (isFPV)
		{
	...
	public static void UnloadViz(GameObject _baseRig)
	{
		if (!(_baseRig == null))
		{
			AssetRefs assetRefs = RemoveAssetOwnerForRig(_baseRig);
			DestroyViz(_baseRig);
			assetRefs.ReleaseAssets();
		}
	}
	...
	public static void setupEquipment(GameObject _rig, TransformCatalog _boneCatalog, string[] ignoredParts, bool isUI, List<SlotData> slotData, bool _ignoreDlcEntitlements)
	{
		...
		foreach (SlotData slotDatum2 in slotData)
		{
			...
			{
				gameObject = setupHeadgearMorph(_rig, _boneCatalog, null, _isFPV: false, isUI, slotDatum2, _ignoreDlcEntitlements);
			}
			else
			{
				...
				if ((bool)transform2)
				{
					gameObject = Stitch(transform2.gameObject, _rig, _boneCatalog, null, isFPV: false, isUI);
					if (gameObject != null)
					{
			...
			if (gameObject != null)
			{
				...
				{
					componentInChildren2.ApplyColorSwatch(tmpArchetype.HairColor);
				}
			}
		}
	...
	public static void setupEquipment(GameObject _rig, TransformCatalog _boneCatalog, string[] ignoredParts, EntityAlive entity, bool isUI, bool _ignoreDlcEntitlements, bool useTempCosmetics = false)
	{
		...
		foreach (SlotData item in tmpArchetype.Equipment)
		{
			...
			{
				gameObject = setupHeadgearMorph(_rig, _boneCatalog, eModelSDCS, eModelSDCS.IsFPV, isUI, item, _ignoreDlcEntitlements);
			}
			else if (!"head".Equals(item.PartName, StringComparison.OrdinalIgnoreCase) || !eModelSDCS.IsFPV || isUI)
			{
				Transform transform2 = setupEquipmentSlot(_rig, _boneCatalog, ignoredParts, item, allGears, _ignoreDlcEntitlements);
				if ((bool)transform2)
				{
					gameObject = Stitch(transform2.gameObject, _rig, _boneCatalog, eModelSDCS, eModelSDCS.IsFPV, isUI, null, isGear: true);
					if (gameObject != null)
					{
						Morphable componentInChildren = gameObject.GetComponentInChildren<Morphable>();
						if ((bool)componentInChildren)
	...
	public static Transform setupEquipmentSlot(GameObject _rig, TransformCatalog _boneCatalog, string[] ignoredParts, SlotData wornItem, List<Transform> allGears, bool _ignoreDlcEntitlements)
	{
		...
		LoadManager.AssetRequestTask<GameObject> assetRequestTask = LoadManager.LoadAsset<GameObject>(pathForSlotData, null, null, _deferLoading: false, _loadSync: true, _ignoreDlcEntitlements);
		if (assetRequestTask.Asset == null && wornItem.PartName == "head")
		{
			pathForSlotData = GetPathForSlotData(wornItem);
			assetRequestTask = LoadManager.LoadAsset<GameObject>(pathForSlotData, null, null, _deferLoading: false, _loadSync: true, _ignoreDlcEntitlements);
		}
		...
		GameObject asset = assetRequestTask.Asset;
		tmpAssetsOwner.AddAssetHandle(assetRequestTask.GetHandle());
		...
		Transform clothingPartWithName = getClothingPartWithName(asset, getPartNameWithVariant(wornItem.PartName, pathForSlotData, targetBodyPath));
		if ((bool)clothingPartWithName)
		{
			HashSet<string> allowedBones = CollectRequiredNamesForSlot(asset.transform, clothingPartWithName);
			...
			MatchRigs(asset.transform, _rig.transform, _boneCatalog, allowedBones);
		}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static GameObject setupHeadgearMorph(GameObject _rig, TransformCatalog _boneCatalog, EModelSDCS _emodel, bool _isFPV, bool isUI, SlotData _slotData, bool _ignoreDlcEntitlements)
	{
		...
		LoadManager.AssetRequestTask<MeshMorph> assetRequestTask = LoadManager.LoadAsset<MeshMorph>(pathForSlotData, null, null, _deferLoading: false, _loadSync: true, _ignoreDlcEntitlements);
		if (assetRequestTask.Asset == null)
		{
			pathForSlotData = GetPathForSlotData(_slotData);
			assetRequestTask = LoadManager.LoadAsset<MeshMorph>(pathForSlotData, null, null, _deferLoading: false, _loadSync: true, _ignoreDlcEntitlements);
		}
		...
		GameObject morphedSkinnedMesh = asset.GetMorphedSkinnedMesh();
		if (morphedSkinnedMesh == null)
		{
			Log.Warning("SDCSUtils::" + pathForSlotData + " no morphed skinned mesh for " + _slotData.PrefabName + "!");
			assetRequestTask.Release();
			return null;
		}
		tmpAssetsOwner.AddAssetHandle(assetRequestTask.GetHandle());
		assetRequestTask.Release();
		if (!morphedSkinnedMesh.gameObject.activeSelf)
		{
			morphedSkinnedMesh.gameObject.SetActive(value: true);
		}
		GameObject result = Stitch(morphedSkinnedMesh.gameObject, _rig, _boneCatalog, _emodel, _isFPV, isUI);
		UnityEngine.Object.Destroy(morphedSkinnedMesh);
		return result;
	}
