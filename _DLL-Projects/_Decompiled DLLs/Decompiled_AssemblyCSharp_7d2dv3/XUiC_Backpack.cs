...
public class XUiC_Backpack : XUiC_ItemStackGrid
{
	...
	{
		return xui.PlayerInventory.GetBackpackItemStacks();
	}

	[PublicizedFrom(EAccessModifier.Protected)]
	public override void UpdateBackend(ItemStack[] stackList)
	{
		xui.PlayerInventory.SetBackpackItemStacks(stackList);
	}

	public override void OnOpen()
	{
		base.OnOpen();
		wasHoldingItem = !xui.DragAndDropWindow.CurrentStack.IsEmpty();
		UpdateCallouts(wasHoldingItem);
		xui.CalloutWindow.EnableCallouts(XUiC_GamepadCalloutWindow.CalloutType.Menu);
		if (EffectManager.GetValue(PassiveEffects.ShuffledBackpack, null, 0f, xui.playerUI.entityPlayer) > 0f)
		{
			ItemStack[] slots = GetSlots();
			GameRandom rand = xui.playerUI.entityPlayer.rand;
	...
	public override void Update(float _dt)
	{
		...
		{
			bool flag = !xui.DragAndDropWindow.CurrentStack.IsEmpty();
		...
		if (maxSlotCount != xui.playerUI.entityPlayer.CarryCapacity)
		{
			RefreshBackpackSlots();
		}
	}
