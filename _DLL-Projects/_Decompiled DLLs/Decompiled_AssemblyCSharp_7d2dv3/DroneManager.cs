...
public class DroneManager
{
	...
	public void Update()
	{
		...
		_ = 0;
		for (int j = 0; j < dronesActive.Count; j++)
		{
			EntityDrone entityDrone2 = dronesActive[j];
			if ((bool)entityDrone2.Owner && (entityDrone2.position - entityDrone2.Owner.getChestPosition()).sqrMagnitude > 1024f && entityDrone2.GetState() != EntityDrone.State.Shutdown && entityDrone2.OrderState != EntityDrone.Orders.Stay)
			{
	...
	{
		EntityDrone drone = GetActiveDronesWithId(entityId);
		if ((bool)drone)
		{
			World world = GameManager.Instance.World;
			EntityPlayer entityPlayer = world.Players.list.Find([PublicizedFrom(EAccessModifier.Internal)] (EntityPlayer p) => p == drone.Owner);
