...
public class Block
{
	...
	public class MultiBlockArray
	{
		...
		public BoundsInt GetBlockBounds()
		{
			...
			{
				vector3Int = Vector3Int.Min(vector3Int, vector3i);
				vector3Int2 = Vector3Int.Max(vector3Int2, vector3i);
			}
		...
		public bool ToParentLocalPos(out Vector3i localOffset, WorldBase _world, Vector3i _parentPos, BlockValue _blockValue, Vector3i _posToCheck)
		{
			...
			{
				if (_parentPos + Get(i, _blockValue.type, rotation) == _posToCheck)
				{
					localOffset = pos[i];
					return true;
				}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public BlockActivationCommand[] cmds = new BlockActivationCommand[1]
	{
				...
				if (num > 0)
				{
					for (int j = 1; j <= num; j++)
					{
						if (Properties.Values.ContainsKey($"{PropCustomCommandName}{j}"))
						{
							...
							string key = $"{PropCustomCommandIconColor}{j}";
							if (Properties.Values.ContainsKey(key))
							{
								blockActivationCommand.iconColor = StringParsers.ParseHexColor(Properties.Values[key]);
							}
							else
							{
								blockActivationCommand.iconColor = Color.white;
							}
							key = $"{PropCustomCommandActivateTime}{j}";
							if (Properties.Values.ContainsKey(key))
							{
								blockActivationCommand.activateTime = StringParsers.ParseFloat(Properties.Values[key]);
							}
							else
							{
	...
	public static Dictionary<string, int> BlockIdsByName()
	{
		...
		{
			dictionary[item.Key] = item.Value.blockID;
		}
		return dictionary;
	}
	...
	public virtual void Init()
	{
		...
		{
			int.TryParse(Properties.GetValue(PropLightOpacity), out lightOpacity);
		}
		else
		{
		...
		StringParsers.TryParseBool(Properties.GetValue(PropCanPickup), out CanPickup);
		...
		if (Properties.Values.ContainsKey(PropIsProp))
		{
			IsProp = StringParsers.ParseBool(Properties.Values[PropIsProp]);
		}
		else
		{
			IsProp = shape is BlockShapeModelEntity;
		}
		...
		if (Properties.Values.ContainsKey(PropGroupName))
		{
			...
			if (!groupNameStringToGroupNames.TryGetValue(Properties.Values[PropGroupName], out GroupNames))
			{
				if (array3.Length != 0)
				{
		...
		if (Properties.Values.ContainsKey(PropMultiBlockDim))
		{
			...
			if (Properties.Values.ContainsKey(PropMultiBlockLayer0))
			{
				...
				while (Properties.Values.ContainsKey(PropMultiBlockLayer + num))
				{
					...
					for (int l = 0; l < array4.Length; l++)
					{
						...
						if (array4[l].Length > dim.x)
						{
							throw new Exception("Multi block layer entry " + l + " too long for block " + blockName);
						}
			...
			else
			{
				...
				{
					for (int num6 = 0; num6 < dim.y; num6++)
					{
		...
		if (isMultiBlock && multiBlockPos != null)
		{
			if (multiBlockPos.dim.x > 16)
			{
				Log.Error($"MultiBlock '{blockName}' dim.x={multiBlockPos.dim.x} exceeds XZ encoding range (16). Reauthor with smaller X dimension.");
			}
			if (multiBlockPos.dim.z > 16)
			{
				Log.Error($"MultiBlock '{blockName}' dim.z={multiBlockPos.dim.z} exceeds XZ encoding range (16). Reauthor with smaller Z dimension.");
			}
			if (multiBlockPos.dim.y > 64)
			{
				Log.Error($"MultiBlock '{blockName}' dim.y={multiBlockPos.dim.y} exceeds Y encoding range (64).");
			}
			if ((AllowedRotations & EBlockRotationClasses.Advanced) != EBlockRotationClasses.None && multiBlockPos.dim.y > 8)
			{
				Log.Error($"MultiBlock '{blockName}' dim.y={multiBlockPos.dim.y} is > 8 and allows Advanced rotations which would overflow XZ encoding. Reauthor dimensions (e.g. swap Y with X/Z) or restrict to OnlySimpleRotations.");
			}
		}
		...
		IsExplosionAffected = true;
		Properties.ParseBool(PropExplosionAffected, ref IsExplosionAffected);
		...
		if (Properties.Values.ContainsKey("CopyPaintOnDowngrade"))
		{
			...
			for (int num9 = 0; num9 < 6; num9++)
			{
				...
				{
					RemovePaintOnDowngrade.Add((BlockFace)num9);
				}
			}
		}
		for (int num10 = 0; num10 < 1; num10++)
		{
			...
			if (!text3.Contains(","))
			{
				...
				for (int num11 = 0; num11 < cUVModeSides; num11++)
				{
					UVModesPerSide[num10] |= (uint)uVMode << num11 * cUVModeBits;
				}
				continue;
			}
			int num12 = 0;
			foreach (char c in text3)
			{
				if (c != ',')
				{
					UVModesPerSide[num10] |= (uint)(c switch
					{
						'L' => 2, 
						'G' => 1, 
						_ => 0, 
		...
		if (text4.Length > 0)
		{
			...
			foreach (string text5 in array2)
			{
				...
				if (num14 >= 0)
				{
					float num15 = StringParsers.ParseFloat(text5, num14 + 1);
					item.radiusSq = num15 * num15;
					item.variable = text5.Substring(0, num14);
				}
				list2.Add(item);
			}
		...
		Properties.ParseString(PropSoundHitAdditional, ref SoundHitAdditional);
	}
	...
	public virtual void LateInit()
	{
		...
		if (Properties.TryGetValue(PropUpgradeBlockClass, PropUpgradeBlockToBlock, out var _value))
		{
			UpgradeBlock = GetBlockValue(_value);
			if (UpgradeBlock.isair)
			{
				throw new Exception("Block with name '" + _value + "' not found in block " + blockName);
			}
		}
		else
		{
	...
	public virtual void OnBlockLoaded(WorldBase _world, Vector3i _blockPos, BlockValue _blockValue)
	{
		...
		{
			shape.OnBlockLoaded(_world, _blockPos, _blockValue);
		}
	}
	...
	public virtual void OnBlockUnloaded(WorldBase _world, Vector3i _blockPos, BlockValue _blockValue)
	{
		if (!_blockValue.ischild)
		{
			shape.OnBlockUnloaded(_world, _blockPos, _blockValue);
		}
		if (RefundOnUnload)
		{
	...
	public virtual void OnBlockValueChanged(WorldBase _world, Chunk _chunk, Vector3i _blockPos, BlockValue _oldBlockValue, BlockValue _newBlockValue)
	{
		...
		{
			shape.OnBlockValueChanged(_world, _blockPos, _oldBlockValue, _newBlockValue);
	...
	{
		return OnBlockDamaged(_world, _blockValueRef, _blockValue, _damagePoints, _entityIdThatDamaged, _attackHitInfo, _bUseHarvestTool, _bBypassMaxDamage);
	}
	...
	public virtual int OnBlockDamaged(WorldBase _world, BlockValueRef _bvRef, BlockValue _blockValue, int _damagePoints, int _entityIdThatDamaged, ItemActionAttack.AttackHitInfo _attackHitInfo, bool _bUseHarvestTool, bool _bBypassMaxDamage, int _recDepth = 0)
	{
		...
		if (isMultiBlock && _blockValue.ischild && _bvRef.Type == BlockValueRefType.Block)
		{
			Vector3i parentPos = multiBlockPos.GetParentPos(_bvRef.BlockPosition, _blockValue);
			BlockValue block = chunkCache.GetBlock(parentPos);
			if (block.ischild)
			{
		...
		if (num < 0)
		{
			if (!UpgradeBlock.isair)
			{
				...
				upgradeBlock = BlockPlaceholderMap.Instance.Replace(_bvRef, upgradeBlock, _world.GetGameRandom());
				...
				{
					_world.SetBlockRPC(_bvRef, upgradeBlock);
					if (chunkCache.GetTextureFull(_bvRef) != 0L)
					{
						GameManager.Instance.SetBlockTextureServer(_bvRef, BlockFace.None, 0, _entityIdThatDamaged);
					}
				}
				else
				{
					_world.SetBlockRPC(_bvRef, upgradeBlock, block3.Density);
				}
		...
		if (!flag && num >= block2.MaxDamage)
		{
			...
			DynamicMeshManager.ChunkChanged(_bvRef, _entityIdThatDamaged, _blockValue.type);
			DestroyedResult destroyedResult = OnBlockDestroyedBy(_world, _bvRef, _blockValue, _entityIdThatDamaged, _bUseHarvestTool);
			if (destroyedResult != DestroyedResult.Keep)
			{
				if (!DowngradeBlock.isair && destroyedResult == DestroyedResult.Downgrade)
				{
					if (_recDepth == 0)
					{
						SpawnDowngradeFX(_world, _blockValue, _bvRef, block2.tintColor, _entityIdThatDamaged);
					}
					BlockValue downgradeBlock = DowngradeBlock;
					downgradeBlock = BlockPlaceholderMap.Instance.Replace(_bvRef, downgradeBlock, _world.GetGameRandom());
					...
					{
						_world.SetBlockRPC(_bvRef, downgradeBlock);
						if (chunkCache.GetTextureFull(_bvRef) != 0L)
						{
							...
							else
							{
								...
								{
									GameManager.Instance.SetBlockTextureServer(_bvRef, RemovePaintOnDowngrade[i], 0, _entityIdThatDamaged);
								}
							}
						}
						...
						if (!string.IsNullOrEmpty(blockDowngradeEvent))
						{
							...
							GameEventManager.Current.HandleAction(blockDowngradeEvent, null, entity as EntityPlayer, twitchActivated: false, _bvRef);
						}
					}
					else
					{
						_world.SetBlockRPC(_bvRef, downgradeBlock, block4.Density);
					}
					if ((num3 > 0 && EnablePassThroughDamage) || _bBypassMaxDamage)
					{
						block4.OnBlockDamaged(_world, _bvRef, downgradeBlock, num3, _entityIdThatDamaged, _attackHitInfo, _bUseHarvestTool, _bBypassMaxDamage, _recDepth + 1);
					}
				}
				else
				{
					...
					SpawnDestroyFX(_world, _blockValue, _bvRef, GetColorForSide(_blockValue, BlockFace.Top), _entityIdThatDamaged);
					...
					if (!string.IsNullOrEmpty(blockDestroyedEvent))
					{
						...
						GameEventManager.Current.HandleAction(blockDestroyedEvent, null, entity3 as EntityPlayer, twitchActivated: false, _bvRef);
					}
				}
			}
		...
		if (_blockValue.damage != num)
		{
			...
			if (!block2.shape.IsTerrain())
			{
				...
				{
					new BlockChangeInfo(_bvRef, _blockValue, _updateLight: false, _bOnlyDamage: true)
				});
			}
			else
			{
	...
	public virtual DestroyedResult OnBlockDestroyedByExplosion(WorldBase _world, BlockValueRef _bvRef, BlockValue _blockValue, int _playerThatStartedExpl)
	{
		_world.ChunkCache?.InvokeOnBlockDamagedDelegates(_bvRef, _blockValue, _blockValue.Block.MaxDamage, _playerThatStartedExpl);
	...
	public virtual bool CanPlaceBlockAt(WorldBase _world, Vector3i _blockPos, BlockValue _blockValue, bool _bOmitCollideCheck = false)
	{
		...
		{
			return !overlapsWithOtherBlock(_world, _blockPos, _blockValue);
		}
		return true;
	}
	...
	public virtual void PlaceBlock(WorldBase _world, BlockPlacement.Result _result, EntityAlive _ea)
	{
		...
		{
			_world.SetBlockRPC(_result.blockPos, _result.blockValue, Density, changingEntityId);
		}
	...
	{
		Vector3 vector = _result.placement switch
		{
			BlockPlacement.EnumPlacement.Voxel => new Vector3(0.5f, 0.5f, 0.5f), 
			BlockPlacement.EnumPlacement.Free => _result.pos - _result.blockPos + _result.blockFace.GetNormal() * 0.5f, 
			_ => throw new ArgumentOutOfRangeException(), 
		};
		Vector3 vector2 = _result.blockPos + vector + _result.propTransform.position - new Vector3(0.5f, 0.5f, 0.5f);
		Vector3i v = new Vector3i(0, 0, 0);
		v.FloorToInt(vector2);
		int chunkX = World.toChunkXZ(v.x);
		int chunkZ = World.toChunkXZ(v.z);
		_world.SetPropRPC(new PropChangeInfo.Builder(chunkX, chunkZ).SetPosition(vector2 - World.toChunkXyzWorldPos(v)).SetRotation(_result.propTransform.rotation).SetScale(_result.propTransform.scale)
			.SetBlockValue(_result.blockValue)
			.Build());
	}

	public virtual DestroyedResult OnBlockDestroyedBy(WorldBase _world, BlockValueRef _bvRef, BlockValue _blockValue, int _entityId, bool _bUseHarvestTool)
	{
		return DestroyedResult.Downgrade;
	}
	...
	public virtual ItemStack OnBlockPickedUp(WorldBase _world, Vector3i _blockPos, BlockValue _blockValue, int _entityId)
	{
		...
		{
			itemValue = new ItemValue(ItemClass.GetItem(PickupTarget).type);
		}
		else if (PickedUpItemValue != null)
		{
			if (PickedUpItemValue.Contains(","))
			{
				string[] array = PickedUpItemValue.Split(',');
				itemValue = ItemClass.CreateItemValue(array[0], (array.Length < 2) ? 1 : int.Parse(array[1]));
			}
			else
			{
				itemValue = ItemClass.GetItem(PickedUpItemValue);
			}
		}
		else
		{
	...
	public virtual bool OnBlockActivated(WorldBase _world, Vector3i _blockPos, BlockValue _blockValue, EntityPlayerLocal _player)
	{
		...
		_world.GetGameManager().PickupBlockServer(_blockPos, _blockValue, _player.entityId);
		return false;
	}
	...
	public virtual string GetActivationText(WorldBase _world, BlockValue _blockValue, Vector3i _blockPos, EntityAlive _entityFocusing)
	{
		...
		return string.Format(Localization.Get("pickupPrompt"), Localization.Get(text));
	}
	...
	public void SpawnDowngradeFX(WorldBase _world, BlockValue _blockValue, BlockValueRef _bvRef, Color _color, int _entityIdThatCaused)
	{
		...
		{
			SpawnFX(_world, _bvRef, 1f, _color, _entityIdThatCaused, block.DowngradeFX);
		}
		else
		{
			SpawnDestroyParticleEffect(_world, _blockValue, _bvRef, 1f, _color, _entityIdThatCaused);
		}
	}
	...
	public void SpawnDestroyFX(WorldBase _world, BlockValue _blockValue, BlockValueRef _bvRef, Color _color, int _entityIdThatCaused)
	{
		...
		{
			SpawnFX(_world, _bvRef, 1f, _color, _entityIdThatCaused, block.DestroyFX);
		}
		else
		{
			SpawnDestroyParticleEffect(_world, _blockValue, _bvRef, 1f, _color, _entityIdThatCaused);
	...
	public virtual void SpawnDestroyParticleEffect(WorldBase _world, BlockValue _blockValue, BlockValueRef _bvRef, float _lightValue, Color _color, int _entityIdThatCaused)
	{
		...
		{
			_world.GetGameManager().SpawnParticleEffectServer(new ParticleEffect(deathParticleName, World.blockToTransformPos(_world, _bvRef) + new Vector3(0f, 0.5f, 0f), _lightValue, _color, blockMaterial.SurfaceCategory + "destroy", null, _OLDCreateColliders: true), _entityIdThatCaused, _forceCreation: false, _worldSpawn: true);
			return;
		}
		...
		{
			_world.GetGameManager().SpawnParticleEffectServer(new ParticleEffect("blockdestroy_" + destroyParticle, World.blockToTransformPos(_world, _bvRef) + new Vector3(0f, 0.5f, 0f), _lightValue, _color, blockMaterial.SurfaceCategory + "destroy", null, _OLDCreateColliders: true), _entityIdThatCaused, _forceCreation: false, _worldSpawn: true);
		}
	}
	...
	public void SpawnFX(WorldBase _world, BlockValueRef _bvRef, float _lightValue, Color _color, int _entityIdThatCaused, string _fxName)
	{
		...
		_world.GetGameManager().SpawnParticleEffectServer(new ParticleEffect(array[0], World.blockToTransformPos(_world, _bvRef) + new Vector3(0f, 0.5f, 0f), _lightValue, _color, array[1], null, _OLDCreateColliders: true), _entityIdThatCaused, _forceCreation: false, _worldSpawn: true);
	}
	...
	public virtual bool OnBlockActivated(string _commandName, WorldBase _world, Vector3i _blockPos, BlockValue _blockValue, EntityPlayerLocal _player)
	{
		...
		{
			OnBlockActivated(_world, _blockPos, _blockValue, _player);
			return true;
		}
		return false;
	}
	...
	public void HandleTrigger(EntityPlayer _player, World _world, Vector3i _blockPos, BlockValue _blockValue)
	{
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsClient)
		{
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageBlockTrigger>().Setup(_blockPos, _blockValue));
			return;
		}
	...
	{
		if (this is BlockCompositeTileEntity blockCompositeTileEntity && blockCompositeTileEntity.CompositeData.HasFeature<TEFeatureDoor>())
		{
	...
	public bool SupportsRotationFromMask(int rotationMask)
	{
		for (int i = 0; i < 28; i++)
		{
			if ((rotationMask & (1 << i)) != 0 && SupportsRotation((byte)i))
			{
				return true;
			}
		}
	...
	public void RotateHoldingBlock(ItemClassBlock.ItemBlockInventoryData _blockInventoryData, bool _increaseRotation, bool _playSoundOnRotation = true)
	{
		...
		BlockValue bv = _blockInventoryData.itemValue.ToBlockValue(_allowAlternates: true);
		bv.rotation = _blockInventoryData.rotation;
		bv = BlockPlacementHelper.OnPlaceBlock(_blockInventoryData.Placement, _blockInventoryData.mode, _blockInventoryData.localRot, _blockInventoryData.world, bv, _blockInventoryData.propTransform, ((EntityPlayerLocal)_blockInventoryData.holdingEntity).HitInfo.hit, _blockInventoryData.holdingEntity.position).blockValue;
		int rotation = _blockInventoryData.rotation;
		_blockInventoryData.rotation = BlockPlacementHelper.LimitRotation(_blockInventoryData.mode, ref _blockInventoryData.localRot, ((EntityPlayerLocal)_blockInventoryData.holdingEntity).HitInfo.hit, _increaseRotation, bv, bv.rotation);
	...
	public void GetCollisionCollisionHash(IncrementalHash calcHash)
	{
		...
		calcHash.AppendDataNoAlloc((byte)AllowedRotations);
		calcHash.AppendDataNoAlloc(BlockingType);
		calcHash.AppendDataNoAlloc(isMultiBlock);
		calcHash.AppendDataNoAlloc(isOversized);
		oversizedBounds.CalculatePersistableHash(calcHash);
		shape.CalculateCollisionHash(calcHash);
	}

	[PublicizedFrom(EAccessModifier.Protected)]
	public void takeItemWithTimer(Vector3i _blockPos, BlockValue _blockValue, EntityPlayerLocal _player, float _takeDelay)
	{
		TakeItemWithTimer(_blockPos, _blockValue, _player, _takeDelay, takeItemWithTimerCanTake);
	}
	...
	public static void TakeItemWithTimer(Vector3i _blockPos, BlockValue _blockValue, EntityPlayerLocal _player, float _takeDelay, Func<Vector3i, EntityPlayerLocal, bool> _canTakeCallback = null)
	{
		...
		else if (_canTakeCallback == null || _canTakeCallback(_blockPos, _player))
		{
			LocalPlayerUI playerUI = _player.PlayerUI;
			TimerEventData timerEventData = new TimerEventData();
			timerEventData.Data = new object[3] { _blockValue, _blockPos, _player };
			timerEventData.FullTimeFinishEvent += TakeItemWithTimerDone;
			XUiC_Timer.OpenTimer(playerUI.xui, _takeDelay, timerEventData);
		}
	}
	...
	public static void TakeItemWithTimerDone(TimerEventData _timerData)
	{
		...
		object[] obj = (object[])_timerData.Data;
		BlockValue blockValue = (BlockValue)obj[0];
