...
public class VersionInformation : IComparable<VersionInformation>
{
	...
	public VersionInformation(EGameReleaseType _releaseType, int _major, int _minor, int _build)
	{
		...
		if (_releaseType == EGameReleaseType.Alpha || (_releaseType == EGameReleaseType.V && _major < 3))
		{
			ShortString = ((Major < 1) ? "Unk" : $"{ReleaseType.ToStringCached()[0]}{Major}.{_minor}");
			LongStringNoBuild = ((Major < 1) ? "Unknown" : $"{ReleaseType.ToStringCached()} {Major}.{_minor}");
			LongString = ((Major < 1) ? "Unknown" : $"{ReleaseType.ToStringCached()} {Major}.{_minor} (b{Build})");
		}
		else
		{
			int num = _minor / 10;
			int num2 = _minor % 10;
			ShortString = ((Major < 1) ? "Unk" : $"{ReleaseType.ToStringCached()[0]}{Major}.{num}.{num2}");
			LongStringNoBuild = ((Major < 1) ? "Unknown" : $"{ReleaseType.ToStringCached()} {Major}.{num}.{num2}");
			LongString = ((Major < 1) ? "Unknown" : $"{ReleaseType.ToStringCached()} {Major}.{num}.{num2} (b{Build})");
		}
