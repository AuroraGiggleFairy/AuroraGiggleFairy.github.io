...
public class EntityHuman : EntityEnemy
{
	...
	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public static float[] superRageChances = new float[10] { 0f, 0.01f, 0.03f, 0.05f, 0.08f, 0.15f, 0.18f, 0.2f, 0.25f, 0.3f };
	...
	public override bool ExecuteFallBehavior(FallBehavior behavior, float _distance, Vector3 _fallMotion)
	{
		...
		case FallBehavior.Op.Ragdoll:
			emodel.DoRagdoll(EModelBase.RagdollMode.Default, rand.RandomFloat * 2f, EnumBodyPartHit.None, _fallMotion * 20f, Vector3.zero, isRemote: false);
			break;
		}
	...
	{
		rageChance = chance;
		superRageChance = superRageChances[index];
	}
	...
	public override void ProcessDamageResponseLocal(DamageResponse _dmResponse)
	{
		...
		float num2 = rageChance * num;
		if (rand.RandomFloat < num2)
		{
			if (rand.RandomFloat < superRageChance)
			{
				StartRage(2f, 30f);
