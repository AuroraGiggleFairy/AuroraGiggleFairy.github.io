...
public class BlocksFromXml
{
	...
	public static void LoadProperties(DynamicProperties properties, XElement elementBlock)
	{
		...
		foreach (XElement item2 in elementBlock.Elements(XNames.array))
		{
			properties.AddArray(item2);
		}
	}
	...
	public static Block CreateBlock(bool _bEditMode, string blockName, DynamicProperties properties)
	{
		...
		if (properties.TryGetValue(Block.PropDisableCover, out var _value))
		{
			block.DisableCover = StringParsers.ParseBool(_value);
		}
		else
		{
			block.DisableCover = false;
		}
		if (properties.Values.TryGetValue("Path", out var value))
		{
			if (value.EqualsCaseInsensitive("solid"))
			{
				block.PathType = 1;
			}
			if (value.EqualsCaseInsensitive("scan"))
			{
				block.PathType = -1;
			}
		}
		else if (properties.Values.TryGetValue("Model", out value) && (value.EqualsCaseInsensitive("@:Shapes/cube.fbx") || value.EqualsCaseInsensitive("@:Shapes/cube_glass.fbx") || value.EqualsCaseInsensitive("@:Shapes/cube_frame.fbx")))
		{
			block.PathType = 1;
		}
		if (properties.Values.TryGetValue("WaterFlow", out var value2))
		{
			block.WaterFlowMask = StringParsers.ParseWaterFlowMask(value2);
		}
		if (properties.Values.TryGetValue("WaterClipPlane", out var value3))
		{
			block.WaterClipPlane = StringParsers.ParsePlane(value3);
		...
		DynamicProperties dynamicProperties = properties.GetClass("RepairItems");
		if (dynamicProperties != null)
		{
			block.RepairItems = new List<Block.SItemNameCount>();
			foreach (KeyValuePair<string, string> value4 in dynamicProperties.Values)
			{
				Block.SItemNameCount item = new Block.SItemNameCount
				{
					ItemName = value4.Key,
					Count = int.Parse(dynamicProperties.Values[value4.Key])
				};
				block.RepairItems.Add(item);
			}
		}
		DynamicProperties dynamicProperties2 = properties.GetClass("RepairItemsMeshDamage");
		if (dynamicProperties2 != null)
		{
			...
			foreach (KeyValuePair<string, string> value5 in dynamicProperties2.Values)
			{
				item2.ItemName = value5.Key;
				item2.Count = int.Parse(dynamicProperties2.Values[value5.Key]);
	...
	public static HashSet<int> GetTextureIdsForMesh(XmlFile _xmlFile, string _meshName)
	{
		...
		try
		{
			...
			foreach (XElement item3 in root.Elements(XNames.block))
			{
				...
				for (int i = 0; i < 1; i++)
				{
					if (!dynamicProperties3.Values.TryGetValue(ShapesFromXml.TextureLabelsByChannel[i].Texture, out var value))
					{
						continue;
					}
					try
					...
					{
						throw new Exception("Error parsing texture id '" + value + "' on layer '" + (i + 1) + "' in block " + item3.GetAttribute(XNames.name));
					}
				}
			}
		}
