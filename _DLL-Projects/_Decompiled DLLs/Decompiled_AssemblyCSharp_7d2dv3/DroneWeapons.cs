...
public class DroneWeapons
{
	...
	public class Weapon
	{
		...
		public Weapon(EntityAlive _entity)
		{
			...
			entityProperties = _entity.EntityClass.Properties;
		}
		...
		{
			modItem = itemValue;
			DynamicProperties properties = modItem.ItemClass.Properties;
			properties.ParseString("SoundWeaponCharged", ref soundWeaponCharged);
			properties.ParseString("SoundWeaponUseBark", ref soundWeaponUseBark);
			float value = EffectManager.GetValue(PassiveEffects.MaxRange, itemValue);
			if (value != 0f)
			{
				range = value;
			}
			float value2 = EffectManager.GetValue(PassiveEffects.JunkDroneModCooldown, itemValue);
			if (value2 != 0f)
			{
				cooldown = value2;
			}
		...
		{
			if (!string.IsNullOrEmpty(soundWeaponCharged))
			{
				PlaySound(entity, soundWeaponCharged);
			}
		}
		...
		public virtual void Equip(ItemValue itemValue)
		{
			...
			SetModProps(itemValue);
			if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
			{
				entity.aiManager.AddItemTasks(itemValue.ItemClass);
			}
		}

		public void Unequip()
		{
			modItem.FireEvent(MinEventTypes.onSelfEquipStop, entity.MinEventContext);
			if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
			{
				entity.aiManager.RemoveItemTasks(modItem.ItemClass.Name);
			}
		}

		[PublicizedFrom(EAccessModifier.Protected)]
		public void PlaySound(Entity _entity, string soundGroupKey)
		{
			Manager.Play(_entity, soundGroupKey);
		}

		[PublicizedFrom(EAccessModifier.Protected)]
		public void PlaySound(string soundGroupKey)
		{
			PlaySound(entity, soundGroupKey);
		}

		[PublicizedFrom(EAccessModifier.Protected)]
		public void TargetApplyBuff(string _buff, bool _fromElectrical)
		{
			target.Buffs.AddBuff(_buff, World.worldToBlockPos(entity.position), (belongsPlayerId != -1) ? belongsPlayerId : entity.entityId, _netSync: true, _fromElectrical);
		}
	...
	public class HealBeamWeapon(EntityAlive _entity) : Weapon(_entity)
	{
		...
		public override void SetModProps(ItemValue itemValue)
		{
			...
			properties.ParseFloat("HealActionTime", ref actionTime);
			properties.ParseFloat("HealDamageThreshold", ref HealDamageThreshold);
		...
		public override void Equip(ItemValue itemValue)
		{
			...
			if ((bool)entityAlive && (bool)entityDrone && entityDrone.TimeSinceCreation != 0f)
			{
				entityAlive.Buffs.AddBuff("buffJunkDroneHealCooldownEffect");
			}
		}
		...
		{
			if (_target.IsAlive() && !_target.Buffs.HasBuff("buffHealHealth") && _target.GetCVar("medicalRegHealthAmount") == 0f && (float)_target.Health < _target.Stats.Health.ModifiedMax)
			{
				return true;
			}
			return false;
		...
		public bool isTargetInNeedOfMedical(EntityAlive _target)
		{
			...
			if ((num == modifiedMax && (float)_target.Health < num - HealDamageThreshold) || (float)_target.Health < modifiedMax * 0.67f)
			{
				return _target.GetCVar("medicalRegHealthAmount") == 0f;
			}
			return false;
		}
		...
		public ItemStack getHealingItemStack(HealItemType healType)
		{
			...
			{
				if (itemStack != null && isItem(itemStack.itemValue, healType))
				{
					ItemStack result = new ItemStack(itemStack.itemValue.Clone(), 1);
					bag.DecItem(itemStack.itemValue, 1);
					return result;
				}
			}
			return null;
		...
		public bool hasItem(string itemGroupOrName)
		{
			...
			foreach (ItemStack itemStack in slots)
			{
				if (itemStack != null && isItem(itemStack.itemValue, itemGroupOrName) && bag.CanTakeItem(itemStack))
				{
					return true;
				}
			}
		...
		{
			if (iv.ItemClass != null && iv.ItemClass.Name.Equals(itemGroupOrName))
			{
				return true;
			}
			return false;
	...
	public class StunBeamWeapon : Weapon
	{
		...
		{
			base.Fire(_target);
			target.SetCVar("_droneStunDamage", (int)modItem.Quality);
			TargetApplyBuff("buffShocked", _fromElectrical: true);
			PlaySound(entity, "drone_attackeffect");
			FireControllerUtils.SpawnParticleEffect(new ParticleEffect("nozzleflashuzi", WeaponJoint.position + Origin.position, Quaternion.Euler(0f, 180f, 0f), 1f, Color.white, "Electricity/Turret/turret_fire", WeaponJoint), -1);
			float lightValue = GameManager.Instance.World.GetLightBrightness(World.worldToBlockPos(entity.position)) / 2f;
			FireControllerUtils.SpawnParticleEffect(new ParticleEffect("nozzlesmokeuzi", WeaponJoint.position + Origin.position, lightValue, new Color(1f, 1f, 1f, 0.3f), null, WeaponJoint, _OLDCreateColliders: false), -1);
		}
	}
	...
	public class MachineGunWeapon(EntityAlive _entity) : Weapon(_entity)
	{
		...
		public override void Init()
		{
			...
			{
				string[] collection = entityProperties.Values["Buff"].Replace(" ", "").Split(',');
	...
	public class NetPackageDroneParticleEffect : NetPackage
	{
		...
		public override void ProcessPackage(World _world, GameManager _callbacks)
		{
			...
			if (entityDrone != null)
			{
				...
				if (actionType == cActionType.Attack && entityDrone.activeWeapon != null)
				{
