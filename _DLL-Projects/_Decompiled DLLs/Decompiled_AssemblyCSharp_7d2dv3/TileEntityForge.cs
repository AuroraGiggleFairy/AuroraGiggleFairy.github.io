...
public class TileEntityForge : TileEntity
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public ItemStack lastServerMold = ItemStack.Empty;

	[PublicizedFrom(EAccessModifier.Private)]
	public ItemStack lastServerOutput = ItemStack.Empty;
		...
		: base(_chunk)
	{
		...
		outputItem = ItemValue.None;
		burningItemValue = ItemValue.None;
	...
	public override void read(PooledBinaryReader _br, StreamModeRead _eStreamMode)
	{
		...
		case StreamModeRead.Persistency:
		{
			...
			if (fuel == null || fuel.Length != num4)
			{
				fuel = ItemStack.CreateArray(num4);
			}
			if (num < 3)
			{
			...
			if (input == null || input.Length != num5)
			{
				input = ItemStack.CreateArray(num5);
			}
			if (num < 3)
			{
		...
		case StreamModeRead.FromClient:
		{
			...
			if (fuel == null || fuel.Length != num8)
			{
				fuel = ItemStack.CreateArray(num8);
			}
			for (int num9 = 0; num9 < num8; num9++)
			{
				fuel[num9].ReadDelta(_br, fuel[num9]);
			}
			int num10 = _br.ReadByte();
			if (input == null || input.Length != num10)
			{
				input = ItemStack.CreateArray(num10);
			}
			for (int num11 = 0; num11 < num10; num11++)
			{
				input[num11].ReadDelta(_br, input[num11]);
			}
		...
		case StreamModeRead.FromServer:
		{
			...
			if (fuel == null || fuel.Length != num2)
			{
				fuel = ItemStack.CreateArray(num2);
			}
			if (!base.bWaitingForServerResponse)
			{
			...
			if (input == null || input.Length != num3)
			{
				input = ItemStack.CreateArray(num3);
			}
			if (!base.bWaitingForServerResponse)
			{
	...
	public override void write(PooledBinaryWriter _bw, StreamModeWrite _eStreamMode)
	{
		...
		case StreamModeWrite.ToServer:
		{
			...
			{
				fuel[k].WriteDelta(_bw, (lastServerFuel != null) ? lastServerFuel[k] : ItemStack.Empty);
			}
			_bw.Write((byte)input.Length);
			for (int l = 0; l < input.Length; l++)
			{
				input[l].WriteDelta(_bw, (lastServerInput != null) ? lastServerInput[l] : ItemStack.Empty);
			}
