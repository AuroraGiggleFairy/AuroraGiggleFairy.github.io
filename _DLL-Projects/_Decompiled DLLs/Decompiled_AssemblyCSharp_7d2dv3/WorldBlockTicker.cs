...
public class WorldBlockTicker
{
	...
	public void AddScheduledBlockUpdate(Vector3i _pos, int _blockId, ulong _ticks)
	{
		if (_blockId == 0)
		{
			return;
		}
		WorldBlockTickerEntry worldBlockTickerEntry = new WorldBlockTickerEntry(_pos, _blockId, _ticks + GameTimer.Instance.ticks);
		lock (lockObject)
		{
			if (scheduledTicksDict.ContainsKey(worldBlockTickerEntry.GetHashCode()))
			{
				remove(_pos, _blockId);
			}
	...
	{
		BlockValue block = world.GetBlock(_wbte.worldPos);
		if (block.type == _wbte.blockID)
		{
			block.Block.UpdateTick(world, _wbte.worldPos, block, _bRandomTick: false, _ticksIfLoaded, _rnd);
		}
	}
	...
	public bool tickScheduled(GameRandom _rnd)
	{
		...
		for (int num2 = 0; num2 < num; num2++)
		{
			...
			if (!world.IsChunkAreaLoaded(worldBlockTickerEntry.worldPos.x, worldBlockTickerEntry.worldPos.y, worldBlockTickerEntry.worldPos.z))
			{
				...
				{
					AddScheduledBlockUpdate(worldBlockTickerEntry.worldPos, worldBlockTickerEntry.blockID, (uint)(30 + _rnd.RandomRange(0, 15)));
				}
			}
			else
			{
	...
	public void tickChunkRandom(Chunk chunk, GameRandom _rnd)
	{
		...
		lock (tickedBlocks)
		{
			for (int num = tickedBlocks.list.Count - 1; num >= 0; num--)
			{
				...
				if (scheduledTicksDict.Count == 0 || !scheduledTicksDict.ContainsKey(WorldBlockTickerEntry.ToHashCode(vector3i, block.type)))
				{
					block.Block.UpdateTick(world, vector3i, block, _bRandomTick: true, ticksIfLoaded, _rnd);
				}
			}
		}
	}
	...
	public void InvalidateScheduledBlockUpdate(Vector3i _pos, int _blockID)
	{
		if (_blockID != 0)
		{
			remove(_pos, _blockID);
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public void remove(Vector3i _pos, int _blockID)
	{
		lock (lockObject)
		{
			int key = WorldBlockTickerEntry.ToHashCode(_pos, _blockID);
			...
			long v = WorldChunkCache.MakeChunkKey(World.toChunkXZ(_pos.x), World.toChunkXZ(_pos.z));
