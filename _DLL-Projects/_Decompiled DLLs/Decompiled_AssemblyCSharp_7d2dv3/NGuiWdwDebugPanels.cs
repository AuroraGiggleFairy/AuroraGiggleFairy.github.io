...
public class NGuiWdwDebugPanels : MonoBehaviour
{
	...
	public void debugShowChunkCache()
	{
		...
		GameManager.Instance.World.ChunkCache?.DebugOnGUI(middleX, middleY, 8f);
	...
	public int showDebugPanel_Player(int x, int y)
	{
		...
		for (EGuiState eGuiState = EGuiState.CalcSize; eGuiState < EGuiState.Count; eGuiState++)
		{
			...
			if (eGuiState == EGuiState.Draw)
			{
				PrefabInstance pOIAtPosition = entityPlayer.world.GetPOIAtPosition(entityPlayer.position, FastTags<TagGroup.Poi>.none);
	...
	public int showDebugPanel_PlayerEffectInfo(int x, int y)
	{
		...
		if (InputUtils.ShiftKeyPressed)
		{
			...
			int hitMask = 69;
			if (Voxel.Raycast(GameManager.Instance.World, ray, distance, -555528221, hitMask, 0f))
			{
	...
	public int showDebugPanel_General(int x, int y)
	{
		...
		for (EGuiState eGuiState = EGuiState.CalcSize; eGuiState < EGuiState.Count; eGuiState++)
		{
			...
			if (eGuiState == EGuiState.Draw)
			{
				LabelWithOutline(x, y, "Save/World Name='" + (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer ? GamePrefs.GetString(EnumGamePrefs.GameName) : GamePrefs.GetString(EnumGamePrefs.GameNameClient)) + "'/'" + GamePrefs.GetString(EnumGamePrefs.GameWorld) + "'");
			}
			y += 16;
			if (eGuiState == EGuiState.Draw)
			{
				LabelWithOutline(x, y, $"Seed={world.Seed}");
			}
			y += 16;
			if (eGuiState == EGuiState.Draw)
			{
	...
	{
		if (playerEntity == null || playerEntity.inventory.holdingItemData == null || !playerEntity.HitInfo.bHitValid)
		{
			return y;
		}
		WorldRayHitInfo hitInfo = playerEntity.HitInfo;
		...
		Chunk chunk = (Chunk)chunkCache.GetChunkFromWorldPos(vector3i);
		if (chunk == null)
		{
			return y;
		}
		...
		for (int i = 0; i < 1; i++)
		{
			array2[i] = GameManager.Instance.World.ChunkCache.GetBlockFaceTexture(vector3i, rotatedBlockFace, i);
			if (array2[i] == 0)
			{
		...
		for (EGuiState eGuiState = EGuiState.CalcSize; eGuiState < EGuiState.Count; eGuiState++)
		{
			...
			if (eGuiState == EGuiState.Draw)
			{
				if (block.isair && playerEntity.inventory.holdingItemData is ItemClassBlock.ItemBlockInventoryData itemBlockInventoryData && !itemBlockInventoryData.itemValue.ToBlockValue().Block.shape.IsTerrain())
				{
			...
			if (eGuiState == EGuiState.Draw)
			{
				...
				{
					if (chunk.GetTileEntity(vector3i2).TryGetSelfOrFeature<ITileEntityLootable>(out var _typedTe))
					{
						LabelWithOutline(x, y, "LootStage: " + playerEntity.GetLootStage(_typedTe.LootStageMod, _typedTe.LootStageBonus));
					}
					else
					{
						LabelWithOutline(x, y, "LootStage: " + playerEntity.GetLootStage(0f, 0f));
					}
				}
				else
				{
			...
			if (eGuiState == EGuiState.Draw)
			{
				AstarVoxelGrid.VoxelNode voxelNode = (AstarPath.active ? ((AstarVoxelGrid.VoxelNode)AstarPath.active.GetNearest(World.blockToTransformPos(vector3i), NNConstraint.Default).node) : null);
				if (voxelNode != null)
				{
					GridGraph gridGraph = GridNode.GetGridGraph(voxelNode.GraphIndex);
					...
					Vector3 vector = (Vector3)voxelNode.position - Origin.position;
					Vector3 vector2 = new Vector3(0.4f, 0f, 0.4f);
					Vector3 vector3 = new Vector3(0f, 0.1f, 0f);
					Utils.DrawRectLinesHorzontal(vector + vector3 - vector2, vector + vector3 + vector2, Color.green, 0.2f);
					LabelWithOutline(x, y, $"Penalty: {voxelNode.Penalty}, H: {voxelNode.PenaltyHigh}, L: {voxelNode.PenaltyLow}");
					y += 16;
					LabelWithOutline(x, y, $"Tag: {voxelNode.Tag}");
					y += 16;
					for (int l = 0; l < 4; l++)
					{
						string empty = string.Empty;
						AstarVoxelGrid.VoxelNode voxelNode2 = (AstarVoxelGrid.VoxelNode)voxelNode.GetNeighbourAlongDirection(l);
						if (voxelNode2 != null)
						{
							empty = $"Dir[{l} {gridGraph.neighbourXOffsets[l]},{gridGraph.neighbourZOffsets[l]}]: BlockerFlags[{voxelNode2.BlockerFlags:X}] Tag[{voxelNode2.Tag}]";
							Vector3 vector4 = (Vector3)voxelNode2.position;
							Utils.DrawRectLinesHorzontal(vector4 + vector3 - vector2, vector4 + vector3 + vector2, array3[l], 0.2f);
							Debug.DrawLine(vector, vector4, array3[l], 0.2f);
						}
						else
						{
							empty = $"Dir[{l} {gridGraph.neighbourXOffsets[l]},{gridGraph.neighbourZOffsets[l]}]: -";
						}
						Utils.DrawOutline(new Rect(x + 5, y, 200f, 25f), empty, guiStyleDebug, Color.black, array3[l]);
						y += 16;
					}
					if (voxelNode.connections != null)
					{
						Connection[] connections = voxelNode.connections;
						for (int m = 0; m < connections.Length; m++)
						{
							Connection connection = connections[m];
							if (connection.cost != uint.MaxValue)
							{
								Vector3 vector5 = (Vector3)connection.node.position - Origin.position;
								Utils.DrawRectLinesHorzontal(vector5 - vector2 * 0.5f, vector5 + vector2 * 0.5f, Color.white, 0.2f);
								Debug.DrawLine(vector, vector5, Color.gray, 0.2f);
							}
						}
					}
				}
		...
		for (EGuiState eGuiState = EGuiState.CalcSize; eGuiState < EGuiState.Count; eGuiState++)
		{
			...
			if (eGuiState == EGuiState.Draw)
			{
				if (XUiC_WoPropsSleeperVolume.GetSelectedVolumeStats(out var _stats))
				{
					...
					y += 16;
					LabelWithOutline(x, y, $"Pos: {_stats.WorldPos}");
					y += 16;
	...
	public void panelManager()
	{
		...
		int num = (GameManager.Instance.IsEditMode() ? 365 : 247);
		num += 55;
		float num2 = _targetScale / _actualScale;
		num = (int)((float)num * num2);
		num = showDebugPanel_EnablePanels(18, num);
		for (int i = 0; i < Panels.Count; i++)
		{
			PanelDefinition panelDefinition = Panels[i];
			if (panelDefinition.Enabled && panelDefinition.Active)
			{
				num = panelDefinition.GuiHandler(18, num);
			}
		}
