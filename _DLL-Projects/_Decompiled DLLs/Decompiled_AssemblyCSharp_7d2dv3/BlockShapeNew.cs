...
public class BlockShapeNew : BlockShape
{
	...
	public class MySimpleMesh
	{
		...
		public override int GetHashCode()
		{
			...
			{
				num = HashCode.Combine(num, index);
			}
			foreach (Vector2 uv in Uvs)
			{
				num = HashCode.Combine(num, uv);
			}
			foreach (Vector3 vertex in Vertices)
			{
				num = HashCode.Combine(num, vertex);
			}
			foreach (Vector3 normal in Normals)
			{
				num = HashCode.Combine(num, normal);
			}
			return num;
		}
	}
	...
	public struct ThreadData
	{
		...
		public void Init()
		{
			trisXPos = new List<int>(128);
			trisXNeg = new List<int>(128);
			trisYPos = new List<int>(128);
			trisYNeg = new List<int>(128);
			trisZPos = new List<int>(128);
			trisZNeg = new List<int>(128);
			subsetUsed = new Dictionary<int, int>(256);
			subsetUsedList = new List<int>(256);
		}
	}
	...
	{
		ShapeName = _block.Properties.GetValue("Model");
		if (ShapeName == null)
		{
	...
	public void AppendSubsetTriPlanar(VoxelMesh targetMesh, MySimpleMesh src, List<int> triIdx, Quaternion q, Vector3 sum1, Vector3 sum2, int texArrayIndex, int blockW, int blockH, int xTiling, int yTiling, Vector3 drawPos, LightingAround lighting, Rect cracksRect, bool imposterActive)
	{
		...
		threadData.subsetUsed.Clear();
		threadData.subsetUsedList.Clear();
		for (int i = 0; i < triIdx.Count; i++)
		{
			int index = triIdx[i];
			int num = src.Indices[index];
			if (!threadData.subsetUsed.ContainsKey(num))
			{
				threadData.subsetUsed[num] = threadData.subsetUsedList.Count;
				threadData.subsetUsedList.Add(num);
			}
		}
		int count = threadData.subsetUsedList.Count;
		...
		{
			int index2 = threadData.subsetUsedList[j];
			int idx = num2 + j;
		...
		for (int k = 0; k < count2; k++)
		{
			...
			int num5 = threadData.subsetUsed[key];
		...
		{
			int index4 = threadData.subsetUsedList[m];
	...
	public override void renderFace(Vector3i _chunkPos, BlockValue _blockValue, Vector3 _drawPos, BlockFace _face, Vector3[] _vertices, LightingAround _lightingAround, TextureFullArray _textureFullArray, VoxelMesh[] _meshes, MeshPurpose _purpose = MeshPurpose.World)
	{
		...
		if (voxelMesh.m_CollVertices != null)
		{
			...
			else
			{
				...
				{
					int num3 = i * 7;
					MySimpleMesh mySimpleMesh = visualMeshes[num3 + num];
					if (mySimpleMesh != null)
					{
		...
		if (_purpose == MeshPurpose.SimplifiedCollisionOnly)
		{
			return;
		}
		Color value = default(Color);
		...
		{
			int num4 = j * 7;
			MySimpleMesh visualMesh = getVisualMesh(num + num4);
			if (visualMesh == null)
			{
				continue;
			}
			...
			if (num2 == 0)
			{
				...
				if (blockTextureData == null)
				{
					if (!DynamicMeshBlockSwap.InvalidPaintIds.Contains(num6))
					{
						...
						Log.Out("Missing paint ID XML entry: " + num6 + " for block '" + block.GetBlockName() + "'");
					}
				}
				else
				{
					num5 = blockTextureData.TextureID;
				}
			}
			int num7 = ((num5 == 0) ? block.GetSideTextureId(_blockValue, (BlockFace)num, j) : num5);
			if ((uint)num7 >= meshDescription.textureAtlas.uvMapping.Length)
			{
				continue;
			}
			UVRectTiling uvTexRect = meshDescription.textureAtlas.uvMapping[num7];
			if (uvTexRect.blockW == 0 || uvTexRect.blockH == 0)
			{
				Log.Error("Block with name '{0}' uses a texture id {1} that is not in the atlas!", block.GetBlockName(), num7);
				continue;
			}
			int num8 = uvTexRect.blockW;
			int num9 = uvTexRect.blockH;
			bool flag = uvTexRect.bGlobalUV;
			switch (block.GetUVMode(num, j))
			{
			...
			int num12 = (int)uvTexRect.uv.width;
			int num13 = (int)uvTexRect.uv.height;
			int num14 = 0;
			if (!bTextureArray)
			{
				vector4.x = ((num10 > 0f) ? (uvTexRect.uv.x + (float)FloorModFloat(x, num8) * uvTexRect.uv.width) : (uvTexRect.uv.x + uvTexRect.uv.width * (float)(num8 - 1) - (float)FloorModFloat(x, num8) * uvTexRect.uv.width));
				vector4.y = ((num11 > 0f) ? (uvTexRect.uv.y + (float)FloorModFloat(x2, num9) * uvTexRect.uv.height) : (uvTexRect.uv.y + uvTexRect.uv.height * (float)(num9 - 1) - (float)FloorModFloat(x2, num9) * uvTexRect.uv.height));
			}
			else if (!flag)
			{
				num8 = Utils.FastMax(num8, num12);
				num9 = Utils.FastMax(num9, num13);
				vector4.x = ((num10 > 0f) ? FloorModFloat(x, num8) : (num8 - 1 - FloorModFloat(x, num8)));
				vector4.y = ((num11 > 0f) ? FloorModFloat(x2, num9) : (num9 - 1 - FloorModFloat(x2, num9)));
				if (num12 > 1)
				{
					num14 += FloorModInt((int)vector4.x, num12);
				}
				if (num13 > 1)
				{
					num14 += FloorModInt((int)vector4.y, num13) * num12;
				}
				vector4.x -= (int)vector4.x;
				vector4.y -= (int)vector4.y;
			}
			int num15 = uvTexRect.index + num14;
			if (bTextureArray)
			{
				value.g = num15;
			}
			...
			if (bTextureArray && flag)
			{
				...
				{
					PlanePick planePick = ClassifyTriPlane(visualMesh, k, quaternion);
					object obj = planePick.axis switch
					{
						PlaneAxis.X => planePick.neg ? threadData.trisXNeg : threadData.trisXPos, 
						PlaneAxis.Y => planePick.neg ? threadData.trisYNeg : threadData.trisYPos, 
						_ => planePick.neg ? threadData.trisZNeg : threadData.trisZPos, 
					};
					((List<int>)obj).Add(k);
					((List<int>)obj).Add(k + 1);
					((List<int>)obj).Add(k + 2);
				}
				int blockW = Mathf.Max(uvTexRect.blockW, num12);
				int blockH = Mathf.Max(uvTexRect.blockH, num13);
				cracksRect = WorldConstants.MapDamageToUVRect(_blockValue);
				Vector3 uvPos = _drawPos + _chunkPos + new Vector3(0.5f, 0.5f, 0.5f);
				if (threadData.trisXPos.Count > 0)
				{
					int texArrayIndex = uvTexRect.index + ComputeTaIndexOffsFor(PlaneAxis.X, uvTexRect, uvPos, flipU: false);
					AppendSubsetTriPlanar(voxelMesh, visualMesh, threadData.trisXPos, quaternion, vector2, vector3, texArrayIndex, blockW, blockH, num12, num13, _drawPos, _lightingAround, cracksRect, bImposterGenerationActive);
					threadData.trisXPos.Clear();
				}
				if (threadData.trisXNeg.Count > 0)
				{
					int texArrayIndex2 = uvTexRect.index + ComputeTaIndexOffsFor(PlaneAxis.X, uvTexRect, uvPos, flipU: true);
					AppendSubsetTriPlanar(voxelMesh, visualMesh, threadData.trisXNeg, quaternion, vector2, vector3, texArrayIndex2, blockW, blockH, num12, num13, _drawPos, _lightingAround, cracksRect, bImposterGenerationActive);
					threadData.trisXNeg.Clear();
				}
				if (threadData.trisYPos.Count > 0)
				{
					int texArrayIndex3 = uvTexRect.index + ComputeTaIndexOffsFor(PlaneAxis.Y, uvTexRect, uvPos, flipU: false);
					AppendSubsetTriPlanar(voxelMesh, visualMesh, threadData.trisYPos, quaternion, vector2, vector3, texArrayIndex3, blockW, blockH, num12, num13, _drawPos, _lightingAround, cracksRect, bImposterGenerationActive);
					threadData.trisYPos.Clear();
				}
				if (threadData.trisYNeg.Count > 0)
				{
					int texArrayIndex4 = uvTexRect.index + ComputeTaIndexOffsFor(PlaneAxis.Y, uvTexRect, uvPos, flipU: false);
					AppendSubsetTriPlanar(voxelMesh, visualMesh, threadData.trisYNeg, quaternion, vector2, vector3, texArrayIndex4, blockW, blockH, num12, num13, _drawPos, _lightingAround, cracksRect, bImposterGenerationActive);
					threadData.trisYNeg.Clear();
				}
				if (threadData.trisZPos.Count > 0)
				{
					int texArrayIndex5 = uvTexRect.index + ComputeTaIndexOffsFor(PlaneAxis.Z, uvTexRect, uvPos, flipU: true);
					AppendSubsetTriPlanar(voxelMesh, visualMesh, threadData.trisZPos, quaternion, vector2, vector3, texArrayIndex5, blockW, blockH, num12, num13, _drawPos, _lightingAround, cracksRect, bImposterGenerationActive);
					threadData.trisZPos.Clear();
				}
				if (threadData.trisZNeg.Count > 0)
				{
					int texArrayIndex6 = uvTexRect.index + ComputeTaIndexOffsFor(PlaneAxis.Z, uvTexRect, uvPos, flipU: false);
					AppendSubsetTriPlanar(voxelMesh, visualMesh, threadData.trisZNeg, quaternion, vector2, vector3, texArrayIndex6, blockW, blockH, num12, num13, _drawPos, _lightingAround, cracksRect, bImposterGenerationActive);
					threadData.trisZNeg.Clear();
				}
				continue;
			}
			...
			{
				int idx = num16 + l;
				Vector3 value2 = quaternion * (visualMesh.Vertices[l] + vector2) + vector3;
				voxelMesh.m_Vertices[idx] = value2;
				Vector3 value3 = quaternion * visualMesh.Normals[l];
				voxelMesh.m_Normals[idx] = value3;
				value2 -= _drawPos;
				float num17 = (float)(int)_lightingAround[LightingAround.Pos.X0Y0Z0].sun * (1f - value2.x) * (1f - value2.y) * (1f - value2.z) + (float)(int)_lightingAround[LightingAround.Pos.X1Y0Z0].sun * value2.x * (1f - value2.y) * (1f - value2.z) + (float)(int)_lightingAround[LightingAround.Pos.X0Y0Z1].sun * (1f - value2.x) * (1f - value2.y) * value2.z + (float)(int)_lightingAround[LightingAround.Pos.X1Y0Z1].sun * value2.x * (1f - value2.y) * value2.z + (float)(int)_lightingAround[LightingAround.Pos.X0Y1Z0].sun * (1f - value2.x) * value2.y * (1f - value2.z) + (float)(int)_lightingAround[LightingAround.Pos.X0Y1Z1].sun * (1f - value2.x) * value2.y * value2.z + (float)(int)_lightingAround[LightingAround.Pos.X1Y1Z0].sun * value2.x * value2.y * (1f - value2.z) + (float)(int)_lightingAround[LightingAround.Pos.X1Y1Z1].sun * value2.x * value2.y * value2.z;
				value.r = num17 / 15f;
			...
			{
				voxelMesh.m_Indices[num18 + m] = voxelMesh.CurTriangleIndex + visualMesh.Indices[m];
			}
			...
			if (bTextureArray)
			{
				num18 = voxelMesh.m_Uvs.Alloc(count3);
				for (int n = 0; n < count3; n++)
				{
					int idx2 = num18 + n;
					if (flag)
					{
						vector5.x = num8 / num12;
						vector5.y = num9 / num13;
					}
					else
					{
						vector5.x = vector4.x + visualMesh.Uvs[n].x;
						vector5.y = vector4.y + visualMesh.Uvs[n].y;
					}
				...
				for (int num19 = 0; num19 < count3; num19++)
				{
					...
					if (!bImposterGenerationActive)
					{
						vector5.x = cracksRect.x + visualMesh.Uvs[num19].x * cracksRect.width;
						vector5.y = cracksRect.y + visualMesh.Uvs[num19].y * cracksRect.height;
					}
					else
					{
						vector5.x = value.g;
			...
			if (!flag)
			{
				...
				{
					vector5.x = vector4.x + visualMesh.Uvs[num20].x * uvTexRect.uv.width;
					vector5.y = vector4.y + visualMesh.Uvs[num20].y * uvTexRect.uv.height;
					voxelMesh.m_Uvs.Add(vector5);
					if (voxelMesh.UvsCrack != null)
					{
						vector5.x = cracksRect.x + visualMesh.Uvs[num20].x * cracksRect.width;
						vector5.y = cracksRect.y + visualMesh.Uvs[num20].y * cracksRect.height;
			...
			{
				MySimpleMesh visualMesh2 = getVisualMesh(num21);
			...
			for (int num22 = 0; num22 < count3; num22++)
			{
				...
				bool num23 = Mathf.Abs(vector7.y) > Mathf.Abs(vector7.x) && Mathf.Abs(vector7.y) > Mathf.Abs(vector7.z);
				...
				vector7 = voxelMesh.m_Normals[count2 + num22];
				if (!flag2)
				{
					...
					Vector3 vector8 = voxelMesh.m_Vertices[count2 + num22] - _drawPos;
					...
					float num24 = (((int)Mathf.Abs(vector9.x) != vector3i.x) ? (Mathf.Abs(vector9.x) % (float)vector3i.x) : Mathf.Abs(vector9.x));
	...
	{
		Action<IncrementalHash, MySimpleMesh> action = [PublicizedFrom(EAccessModifier.Internal)] (IncrementalHash hash, MySimpleMesh mesh) =>
		{
			foreach (ushort index in mesh.Indices)
			{
				hash.AppendDataNoAlloc(index);
			}
			foreach (Vector2 uv in mesh.Uvs)
			{
				hash.AppendDataNoAlloc(uv.x);
				hash.AppendDataNoAlloc(uv.y);
			}
			foreach (Vector3 vertex in mesh.Vertices)
			{
				hash.AppendDataNoAlloc(vertex.x);
				hash.AppendDataNoAlloc(vertex.y);
				hash.AppendDataNoAlloc(vertex.z);
			}
			foreach (Vector3 normal in mesh.Normals)
			{
				hash.AppendDataNoAlloc(normal.x);
				hash.AppendDataNoAlloc(normal.y);
				hash.AppendDataNoAlloc(normal.z);
			}
		};
		int value = 1;
		...
		{
			for (int num = 0; num < colliderMeshes.Length; num++)
			{
				if (colliderMeshes[num] != null)
				{
					action(calcHash, colliderMeshes[num]);
				}
			}
		}
		...
		for (int num2 = 0; num2 < visualMeshes.Length; num2++)
		{
			if (visualMeshes[num2] != null)
			{
				action(calcHash, visualMeshes[num2]);
			}
		}
	}
