	...
	public void CheckKeys(ItemInventoryData _data, WorldRayHitInfo _hitInfo, PlayerActionsLocal playerActions)
	{
		...
		if (Input.GetMouseButton(0) && isButtonDown)
		{
			if (!(Time.time - _data.actionData[0].lastUseTime < 0.1f))
			{
				...
				{
					blockChanges.Clear();
					BlockTools.PlaceTerrain(_data.world, _hitInfo.hit.blockPos + new Vector3i(parentWindow.lastDirection.x * 2f, parentWindow.lastDirection.y * 2f, parentWindow.lastDirection.z * 2f), brush, blockChanges);
		...
		else if (Input.GetMouseButton(1) && isButtonDown)
		{
			if (!(Time.time - _data.actionData[0].lastUseTime < 0.1f))
			{
				...
				{
					blockChanges.Clear();
					BlockTools.RemoveTerrain(_data.world, _hitInfo.hit.blockPos, brush, blockChanges);
					_data.world.SetBlocksRPC(blockChanges);
				}
			}
		}
