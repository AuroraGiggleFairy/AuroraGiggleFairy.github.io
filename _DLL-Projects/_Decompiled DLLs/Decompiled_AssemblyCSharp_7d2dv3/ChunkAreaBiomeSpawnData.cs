...
public class ChunkAreaBiomeSpawnData
{
	...
	public static int RespawnDelayIndexEnemies;

	public static int RespawnDelayIndexAnimals;

	public static int RespawnCountOverride;
	...
	public void ResetRespawn(int _idHash, World _world, int _maxCount)
	{
		...
		{
			entitesSpawned.TryGetValue(_idHash, out var value);
			int num = ((biomeSpawnEntityGroupData.respawnDelayInWorldTime.Length != 1) ? (biomeSpawnEntityGroupData.isAnimal ? RespawnDelayIndexAnimals : RespawnDelayIndexEnemies) : 0);
			if (_maxCount > 0 && RespawnCountOverride > 0 && EntityGroups.IsEnemyGroup(biomeSpawnEntityGroupData.entityGroupName))
			{
				_maxCount = RespawnCountOverride;
			}
