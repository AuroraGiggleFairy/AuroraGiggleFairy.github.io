...
public static class PrefabHelpers
{
	...
	{
		List<PathAbstractions.AbstractedLocation> paths = PathAbstractions.PrefabsSearchPaths.GetAvailablePathsList();
		MicroStopwatch msw = new MicroStopwatch(_bStart: true);
		for (int i = 0; i < paths.Count; i++)
		{
			if (i % 20 == 0 && i > 0)
			{
				TimeSpan elapsed = msw.Elapsed;
				TimeSpan timeSpan = elapsed / i * (paths.Count - i);
				Log.Out($"Processed {i} of {paths.Count} prefabs in {elapsed.TotalMinutes:0}:{elapsed.Seconds:00}. Expected time left: {timeSpan.TotalMinutes:0}:{timeSpan.Seconds:00}");
			}
			PathAbstractions.AbstractedLocation abstractedLocation = paths[i];
			if ((_prefabPathFilter == null || _prefabPathFilter(abstractedLocation)) && PrefabEditModeManager.Instance.LoadVoxelPrefab(abstractedLocation, _bBulk: true, _ignoreExcludeImposterCheck) && (_prefabContentFilter == null || _prefabContentFilter(PrefabEditModeManager.Instance.VoxelPrefab)))
			{
				yield return processPrefab(_onPrefabLoaded, _onChunksBuilt, PrefabEditModeManager.Instance.VoxelPrefab, abstractedLocation);
			}
		}
	...
	{
		PrefabEditModeManager.Instance.ImposterSignData = CollectImposterSignData();
		SimplifyPrefab();
		ThreadManager.StartCoroutine(convertWaitForAllChunksBuilt(_callbackOnDone));
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public static List<ImposterCanvas> CollectImposterSignData()
	{
		List<ImposterCanvas> list = null;
		foreach (Chunk item2 in GameManager.Instance.World.ChunkCache.GetChunkArrayCopySync())
		{
			foreach (TileEntity item3 in item2.GetTileEntities().list)
			{
				if (!item3.TryGetSelfOrFeature<TEFeatureCanvas>(out var _typedTe))
				{
					continue;
				}
				SignCanvas canvas = _typedTe.Canvas;
				if (canvas == null || !canvas.AllowedOnImposter || !canvas.ShowOnImposter || !canvas.SignId.IsValid)
				{
					continue;
				}
				Vector3i vector3i = item3.ToWorldPos();
				BlockEntityData blockEntityData = item3.GetChunk()?.GetBlockEntity(vector3i);
				if (blockEntityData == null || !blockEntityData.bHasTransform)
				{
					Log.Warning($"[PrefabHelpers] No BlockEntityData transform for sign at {vector3i}. Skipping.");
					continue;
				}
				BoxCollider componentInChildren = blockEntityData.transform.GetComponentInChildren<BoxCollider>();
				Vector3 vector;
				Vector2 size;
				if (componentInChildren != null)
				{
					vector = componentInChildren.bounds.center;
					size = new Vector2(componentInChildren.size.x, componentInChildren.size.y);
				}
				else
				{
					vector = blockEntityData.transform.position;
					size = Vector2.one;
				}
				Vector3 worldPosition = vector + new Vector3(0f, 3f, 0f);
				Quaternion worldRotation = blockEntityData.transform.rotation * Quaternion.Euler(0f, 180f, 0f);
				ImposterCanvas item = new ImposterCanvas
				{
					WorldPosition = worldPosition,
					WorldRotation = worldRotation,
					Size = size,
					CanvasAspect = canvas.CanvasAspect,
					IsDecal = canvas.IsDecal,
					State = canvas.State
				};
				if (list == null)
				{
					list = new List<ImposterCanvas>();
				}
				list.Add(item);
			}
		}
	...
	public static IEnumerator convertWaitForAllChunksBuilt(Action _callbackOnDone)
	{
		...
		if (prefab != null)
		{
			...
			if (combine(_bCombineSliced: true))
			{
				...
				PrefabEditModeManager.Instance.ImposterPrefab = SimpleMeshFile.ReadGameObject(prefab.location.FullPathNoExtension + ".mesh", out var _meshInfo, 0f, null, bTextureArray);
				PrefabEditModeManager.Instance.ImposterPrefab.transform.name = prefab.PrefabName;
				PrefabEditModeManager.Instance.LoadEditorImposterSigns(_meshInfo);
	...
	public static void simplifyPrefab2()
	{
		for (int i = 0; i < blockIds.Length; i++)
		{
			...
			{
				Block block = blockValue.Block;
				if (block.shape is BlockShapeModelEntity || block.MeshIndex != 0 || block.bImposterExcludeAndStop || block.blockMaterial.IsLiquid)
				{
					blockIds[i] = BlockValue.Air;
					bTouched[i] = true;
				}
	...
	public static void export()
	{
		...
		SimpleMeshFile.WriteGameObject(pooledBinaryWriter, PrefabEditModeManager.Instance.ImposterPrefab, PrefabEditModeManager.Instance.ImposterSignData);
	}
