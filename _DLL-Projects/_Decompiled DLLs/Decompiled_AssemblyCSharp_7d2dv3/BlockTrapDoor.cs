...
public class BlockTrapDoor : Block
{
	...
	{
		XUiC_TriggerProperties.Show(_player.PlayerUI.xui, _blockPos, _showTriggers: true, _showTriggeredBy: true);
		return true;
	}
	...
	public override bool OnEntityCollidedWithBlock(WorldBase _world, Vector3i _blockPos, BlockValue _blockValue, Entity _targetEntity)
	{
		...
		if (value > 0f)
		{
			...
			GameManager.Instance.StartCoroutine(damageBlock(value, _world, _blockPos, _blockValue, _targetEntity as EntityPlayer, _targetEntity.entityId));
		}
		return true;
	}
	...
	public override DestroyedResult OnBlockDestroyedBy(WorldBase _world, BlockValueRef _bvRef, BlockValue _blockValue, int _entityId, bool _bUseHarvestTool)
	{
		...
		{
			HandleTrigger(player, (World)_world, _bvRef, _blockValue);
		}
	...
	public IEnumerator damageBlock(float time, WorldBase _world, Vector3i _blockPos, BlockValue _blockValue, EntityPlayer _player, int _entityId)
	{
		...
		DamageBlock(_world, _blockPos, _blockValue, _blockValue.Block.MaxDamage - _blockValue.damage, _entityId);
	}
