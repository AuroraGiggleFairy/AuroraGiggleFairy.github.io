...
public class XUiC_RecipeTrackerWindow : XUiController
{
	...
	public override void Update(float _dt)
	{
		...
		{
			int craftingTier = currentRecipe.GetCraftingTier(xui.playerUI.entityPlayer);
			if (currentRecipe.GetOutputItemClass().HasQuality)
			{
				currentRecipe.craftingTier = xui.Recipes.TrackedRecipeQuality;
			}
			else
			{
	...
	{
		base.OnOpen();
		CurrentRecipe = xui.Recipes.TrackedRecipe;
		xui.Recipes.OnTrackedRecipeChanged += RecipeTracker_OnTrackedRecipeChanged;
		IsDirty = true;
	}
	...
	{
		CurrentRecipe = xui.Recipes.TrackedRecipe;
		Count = xui.Recipes.TrackedRecipeCount;
		IsDirty = true;
	}
