...
public class XUiC_EnteringArea : XUiController
{
	...
	public override void Update(float _dt)
	{
		...
		if (!LocalPlayer.IsAlive() || !xui.playerUI.windowManager.IsHUDEnabled())
		{
		...
		if (prefab != null && prefab != this.prefab && prefab != lastPrefab)
		{
			...
			if (!prefab.Tags.Test_AnySet(prefabIgnoreTags) && ShowLocation)
			{
				this.prefab = prefab;
		...
		if (this.prefab == null && ShowLocation)
		{
		...
		else if (LocalPlayer.AreaMessage != null)
		{
			showTime = Utils.FastMin(2f, showTime);
		}
		showTime -= _dt;
