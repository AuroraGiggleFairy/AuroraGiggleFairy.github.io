...
public class XUiC_SkillEntry : XUiController
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public readonly CachedStringFormatter<int, int> craftingGroupLevelFormatter = new CachedStringFormatter<int, int>([PublicizedFrom(EAccessModifier.Internal)] (int _i1, int _i2) => _i1 + "/" + _i2);
	...
	public ProgressionValue Skill
	{
		...
		set
		{
			...
			bool isNavigatable = (base.ViewComponent.IsSnappable = value != null);
			xUiView.IsNavigatable = isNavigatable;
		}
	}
	...
	{
		if (displayType == ProgressionClass.DisplayTypes.Standard && currentSkill != null)
		{
			if (currentSkill.ProgressionClass.Type == ProgressionType.Crafting || currentSkill.ProgressionClass.IsPerk)
			{
				return craftingGroupLevelFormatter.Format(currentSkill.CalculatedLevel(xui.playerUI.entityPlayer), currentSkill.ProgressionClass.MaxLevel);
			}
			if (currentSkill.ProgressionClass.Type != ProgressionType.Skill)
			{
	...
	public override bool GetBindingValueInternal(ref string value, string bindingName)
	{
		...
		case "canpurchase":
			...
			{
				value = ((currentSkill != null && currentSkill.ProgressionClass.Type != ProgressionType.Skill) ? currentSkill.CanPurchase(xui.playerUI.entityPlayer, currentSkill.Level + 1).ToString() : "false");
			}
			return true;
		case "cannotpurchase":
			value = ((currentSkill != null && currentSkill.ProgressionClass.Type != ProgressionType.Skill && currentSkill.ProgressionClass.Type != ProgressionType.BookGroup && currentSkill.ProgressionClass.Type != ProgressionType.Crafting) ? (!currentSkill.CanPurchase(xui.playerUI.entityPlayer, currentSkill.Level + 1)).ToString() : "false");
			return true;
		case "requiredskill":
		{
			string text = "NA";
		...
		case "statuscolor":
			value = ((currentSkill == null) ? disabledColor : ((currentSkill.CalculatedMaxLevel(xui.playerUI.entityPlayer) == 0 && currentSkill.ProgressionClass.Type != ProgressionType.Crafting) ? disabledColor : enabledColor));
			return true;
		case "hasskill":
