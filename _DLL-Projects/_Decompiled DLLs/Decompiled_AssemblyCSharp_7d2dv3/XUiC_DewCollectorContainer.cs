...
[Preserve]
public class XUiC_DewCollectorContainer : XUiC_VariableHeightGrid, ITileEntityChangedListener
{
	...
	public void SetTileEntity(TileEntityCollector _te)
	{
		if (_te != null)
		{
			SetHeight(_te.OutputWindowHeight);
		}
	}
	...
	public override void Init()
	{
		...
		InitVariableHeightGrids(new XUiV_Grid[1] { xUiV_Grid });
		for (int i = 0; i < itemControllers.Length; i++)
		{
	...
	{
		TileEntityCollector tileEntityCollector = _te as TileEntityCollector;
		SetHeight(tileEntityCollector.OutputWindowHeight);
	...
	public void SetItemInSlot(int i, ItemStack stack, bool onTEChanged)
	{
		...
		{
			Manager.PlayInsidePlayerHead(localTileEntity.GetSlotOutputType(i).ConvertSound);
		}
		XUiC_DewCollectorStack xUiC_DewCollectorStack = (XUiC_DewCollectorStack)xUiC_ItemStack;
		if (xUiC_DewCollectorStack != null)
		{
			TileEntityCollector.FillData slotFillData = localTileEntity.GetSlotFillData(i);
			if (slotFillData != null)
			{
				xUiC_DewCollectorStack.FillAmount = slotFillData.fillTime - slotFillData.fillTimeLeft;
				xUiC_DewCollectorStack.MaxFill = slotFillData.fillTime;
			}
			xUiC_DewCollectorStack.IsCurrentStack = localTileEntity.IsCurrentStack(i);
			xUiC_DewCollectorStack.IsBlocked = localTileEntity.IsSlotDisabled(i);
			xUiC_DewCollectorStack.IsModded = localTileEntity.HasModConvert;
