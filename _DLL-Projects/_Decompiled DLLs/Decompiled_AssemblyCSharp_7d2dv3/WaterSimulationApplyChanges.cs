...
public class WaterSimulationApplyChanges
{
	...
	public void ApplyChanges(Chunk _chunk, Dictionary<int, WaterValue> changedVoxels)
	{
		...
		foreach (KeyValuePair<int, WaterValue> changedVoxel in changedVoxels)
		{
			...
			{
				int num7 = voxelCoords.y >> 4;
				int num8 = 1 << num7;
				if ((voxelCoords.y & 3) == 3 && voxelCoords.y < 255)
				{
					num8 |= 2 << num7;
				}
				num2 |= num8;
		...
		if (num2 != 0)
		{
			_chunk.NeedsRegenerationOrBits(num2);
		}
		if (num3 != 0)
		{
			chunks.GetChunkSync(_chunk.X - 1, _chunk.Z)?.NeedsRegenerationOrBits(num3);
		}
		if (num4 != 0)
		{
			chunks.GetChunkSync(_chunk.X + 1, _chunk.Z)?.NeedsRegenerationOrBits(num4);
		}
		if (num5 != 0)
		{
			chunks.GetChunkSync(_chunk.X, _chunk.Z - 1)?.NeedsRegenerationOrBits(num5);
		}
		if (num6 != 0)
		{
			chunks.GetChunkSync(_chunk.X, _chunk.Z + 1)?.NeedsRegenerationOrBits(num6);
		}
		if (num <= 0)
		{
			return;
