...
public abstract class ChunkProviderGenerateWorld : ChunkProviderAbstract
{
	...
	public IReadOnlyCollection<LongSetGroups.Group> ChunkGroups => m_RegionFileManager.ChunkGroups;
	...
	{
		world = _world;
		prefabDecorator = new DynamicPrefabDecorator(_world.m_PrefabCache);
	...
	public override void SaveAll()
	{
		...
		if (world.IsEditor())
		{
			GetDynamicPrefabDecorator().Save(worldLocation.FullPath);
			spawnPointManager.Save(worldLocation.FullPath);
			return;
		}
		...
		GetEventPrefabs()?.Save(waitForComplete: true);
	}
	...
	{
		return m_RegionFileManager.ResetAllChunks(excludedProtectionLevels, _groupingMode);
	}
	...
	{
		return m_RegionFileManager.ResetRegion(_regionX, _regionZ, excludedProtectionLevels, _groupingMode);
	}
	...
	public override void RebuildTerrain(HashSetLong _chunks, Vector3i _areaStart, Vector3i _areaSize, bool _isStopStabilityCalc, bool _isRegenChunk, bool _isFillEmptyBlocks, bool _isReset)
	{
		...
		{
			Chunk chunkSync = chunkCache.GetChunkSync(_chunk);
			if (chunkSync != null)
			{
	...
	{
		Log.Error($"ChunkProvider Currently Generating Data for Chunk - Key: {currentGeneratingChunk}, Pos: {WorldChunkCache.extractX(currentGeneratingChunk) << 4}/{WorldChunkCache.extractZ(currentGeneratingChunk) << 4}, Duration {chunkGenerationTimer.Elapsed.TotalSeconds}");
	}
}
