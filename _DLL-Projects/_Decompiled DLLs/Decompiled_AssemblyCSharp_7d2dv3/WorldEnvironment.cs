...
public class WorldEnvironment : MonoBehaviour
{
	...
	public static GameObject CreateUnityTerrainOld(string _worldPath, int _sliceAtWidth, int _heightMapDataWidth, int _heightMapDataHeight, List<float[,]> _heightMapDataList, int _worldScale = 1, float _yOffsetTerrain = 1f, int _sliceAt = 2048, bool _bEditMode = true, Action<float> _progressCallback = null)
	{
		...
		TerrainLayer[] array3 = new TerrainLayer[array.Length];
		for (int i = 0; i < array.Length; i++)
		{
			TerrainLayer terrainLayer = new TerrainLayer();
			terrainLayer.diffuseTexture = array[i];
			terrainLayer.normalMapTexture = array2[i];
			terrainLayer.tileSize = new Vector2(10f, 10f);
			array3[i] = terrainLayer;
		}
		int num = 4;
		int _width;
		int _height;
		Color32[] array4 = TextureUtils.LoadTexturePixels(_worldPath + "/splat1", out _width, out _height);
		float num2 = (float)_width / (float)_heightMapDataWidth;
		_progressCallback?.Invoke(0.05f);
		string pathNoExtension = _worldPath + "/splat2";
		float num3 = 0f;
		int _width2;
		int _height2;
		Color32[] array5 = TextureUtils.LoadTexturePixels(pathNoExtension, out _width2, out _height2);
		if (array5 != null)
		{
			num3 = (float)_width2 / (float)_heightMapDataWidth;
			num += 4;
		}
		_progressCallback?.Invoke(0.1f);
		string pathNoExtension2 = _worldPath + "/splat3";
		float num4 = 0f;
		int _width3;
		int _height3;
		Color32[] array6 = TextureUtils.LoadTexturePixels(pathNoExtension2, out _width3, out _height3);
		if (array6 != null)
		{
			num4 = (float)_width3 / (float)_heightMapDataWidth;
			num += 4;
		}
		_progressCallback?.Invoke(0.15f);
		microStopwatch2.ResetAndRestart();
		for (int j = 0; j < array4.Length; j++)
		{
			Color32 color = array4[j];
			int num5 = color.r;
			int num6 = color.g;
			int num7 = color.b;
			int num8 = color.a;
			...
			{
				array4[j] = new Color32((byte)(array4[j].r + (255 - (num5 + num6 + num7 + num8))), array4[j].g, array4[j].b, array4[j].a);
			}
		}
		_progressCallback?.Invoke(0.2f);
		...
		Terrain[,] array7 = new Terrain[num11, num12];
		float[,,] array8 = new float[_sliceAtWidth, _sliceAtWidth, num];
		for (int k = 0; k < num12; k++)
		{
			for (int l = 0; l < num11; l++)
			{
				...
				terrainData.terrainLayers = array3;
				for (int m = 0; m < _sliceAtWidth; m++)
				{
					int num13 = k * _sliceAtWidth + m;
					int num14 = (int)((float)num13 * num2);
					for (int n = 0; n < _sliceAtWidth; n++)
					{
						int num15 = l * _sliceAtWidth + n;
						int num16 = (int)((float)num15 * num2 + (float)(num14 * _width)) % array4.Length;
						Color32 color2 = array4[num16];
						array8[m, n, 0] = (float)(int)color2.r * 0.003921569f;
						array8[m, n, 1] = (float)(int)color2.g * 0.003921569f;
						array8[m, n, 2] = (float)(int)color2.b * 0.003921569f;
						array8[m, n, 3] = (float)(int)color2.a * 0.003921569f;
						if (num > 4)
						{
							num16 = (int)((float)num15 * num3 + (float)((int)((float)num13 * num3) * _width2)) % array5.Length;
							color2 = array5[num16];
							array8[m, n, 4] = (float)(int)color2.r * 0.003921569f;
							array8[m, n, 5] = (float)(int)color2.g * 0.003921569f;
							array8[m, n, 6] = (float)(int)color2.b * 0.003921569f;
							array8[m, n, 7] = (float)(int)color2.a * 0.003921569f;
						}
						if (num > 8)
						{
							num16 = (int)((float)num15 * num4 + (float)((int)((float)num13 * num4) * _width3)) % array6.Length;
							color2 = array6[num16];
							array8[m, n, 8] = (float)(int)color2.r * 0.003921569f;
							array8[m, n, 9] = (float)(int)color2.g * 0.003921569f;
							array8[m, n, 10] = (float)(int)color2.b * 0.003921569f;
							array8[m, n, 11] = (float)(int)color2.a * 0.003921569f;
						}
					}
				}
				terrainData.SetAlphamaps(0, 0, array8);
				...
				array7[l, k] = component;
				obj.layer = 16;
				obj.tag = "T_Mesh";
				obj.transform.parent = gameObject.transform;
				obj.transform.localPosition = new Vector3(l * _sliceAtWidth * _worldScale, _yOffsetTerrain, k * _sliceAtWidth * _worldScale);
				float num17 = (float)(k * num11 + l + 1) / (float)(num11 * num12);
				_progressCallback?.Invoke(0.2f + num17 * 0.8f);
			}
		}
		for (int num18 = 0; num18 < num12; num18++)
		{
			...
			{
				array7[num19, num18].SetNeighbors((num19 > 0) ? array7[num19 - 1, num18] : null, (num18 > 0) ? array7[num19, num18 - 1] : null, (num19 < num11 - 1) ? array7[num19 + 1, num18] : null, (num18 < num12 - 1) ? array7[num19, num18 + 1] : null);
			}
		}
	...
	{
		if (!GameManager.IsDedicatedServer && !world.ChunkCache.IsFixedSize)
		{
			chunkClusterVisibleDelegate = OnChunkDisplayed;
			world.ChunkCache.OnChunkVisibleDelegates += chunkClusterVisibleDelegate;
		}
	}
	...
	{
		if (_wcd.Properties.Contains("WorldEnvironment", "Prefab"))
		{
			UnityEngine.Object.Instantiate(Resources.Load<GameObject>(_wcd.Properties.GetString("WorldEnvironment", "Prefab"))).transform.parent = base.transform;
		}
	}
