...
public class XUiC_ColorPicker : XUiController
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public XUiV_Sprite selectedColorSatValPointerSprite;
	...
	public Color SelectedColor
	{
		...
		set
		{
			...
			selectedColorSatValPointerSprite.IsVisible = true;
			selectedColorSatValPointerSprite.Position = new Vector2i((int)num, -(int)num2);
			selectedColorSatValPointerSprite.UiTransform.localPosition = new Vector3(num, 0f - num2);
	...
	public override void Init()
	{
		...
		if (childById != null && childById.ViewComponent is XUiV_Texture xUiV_Texture)
		{
			svPicker = xUiV_Texture;
			childById.OnPress += SvPickerC_OnPress;
			childById.OnDrag += SvPickerC_OnDrag;
		}
		if (childById2 != null && childById2.ViewComponent is XUiV_Texture xUiV_Texture2)
		{
			hPicker = xUiV_Texture2;
			childById2.OnPress += HPickerC_OnPress;
			childById2.OnDrag += HPickerC_OnDrag;
		}
		XUiController childById4 = GetChildById("selectedColor");
		if (childById4 != null && childById4.ViewComponent is XUiV_Sprite xUiV_Sprite)
		{
			selectedColorSprite = xUiV_Sprite;
		}
		if (childById3 != null && childById3.ViewComponent is XUiV_Sprite xUiV_Sprite2)
		{
			selectedColorSatValPointerSprite = xUiV_Sprite2;
		}
		setupHueTexture();
		setupSaturationVibranceTexture();
	}
	...
	{
		base.Update(_dt);
		if (xui.playerUI.CursorController.navigationTarget == svPicker && PlatformManager.NativePlatform.Input.CurrentInputStyle != PlayerInputManager.InputStyle.Keyboard)
		{
			if (xui.playerUI.playerInput.GUIActions.Submit.IsPressed)
			{
				showCallouts(_held: true);
				xui.playerUI.CursorController.Locked = true;
				Vector2 value = xui.playerUI.playerInput.GUIActions.Nav.Value;
				...
				float b = hue;
				hue = Mathf.Clamp(hue + xui.playerUI.playerInput.GUIActions.PageUp.Value, 0f, 255f);
				hue = Mathf.Clamp(hue - xui.playerUI.playerInput.GUIActions.PageDown.Value, 0f, 255f);
				if (!Mathf.Approximately(hue, b))
				{
					isLocalDirty = true;
				}
			}
	...
	public void showCallouts(bool _held)
	{
		...
		{
			xui.CalloutWindow.ClearCallouts(XUiC_GamepadCalloutWindow.CalloutType.ColorPicker);
			if (_held)
			{
				xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.LeftStick, "igcoColorPickerSaturationVibrance", XUiC_GamepadCalloutWindow.CalloutType.ColorPicker);
				xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.LeftTrigger, "igcoColorPickerHueMinus", XUiC_GamepadCalloutWindow.CalloutType.ColorPicker);
				xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.RightTrigger, "igcoColorPickerHuePlus", XUiC_GamepadCalloutWindow.CalloutType.ColorPicker);
			}
			else
			{
				xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.FaceButtonSouth, "igcoSelectColorPicker", XUiC_GamepadCalloutWindow.CalloutType.ColorPicker);
			}
			xui.CalloutWindow.EnableCallouts(XUiC_GamepadCalloutWindow.CalloutType.ColorPicker);
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public IEnumerator setSatVal()
	{
		Texture2D tex = new Texture2D(1, 1, TextureFormat.ARGB32, mipChain: false);
		selectedColorSatValPointerSprite.IsVisible = false;
	...
	{
		GameManager.Instance.StartCoroutine(setSatVal());
	}
