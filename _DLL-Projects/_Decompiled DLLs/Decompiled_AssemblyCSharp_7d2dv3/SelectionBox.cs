...
public class SelectionBox : MonoBehaviour
{
	...
	public SelectionCategory Category => ownerCategory;
	...
	public object UserData
	{
		...
		{
			userData = value;
			ownerCategory.BoxCallbacks?.OnSelectionBoxUserDataChanged(this);
		}
	}
	...
	public float FacingDirection
	{
		...
		set
		{
			if (!Mathf.Approximately(facingDirection, value))
			{
	...
	public bool IsActive => SelectionBoxManager.Instance.Selection == this;
	...
	public void SetPositionAndSize(Vector3i _pos, Vector3i _size)
	{
	...
	public bool WorldPosWithinBox(Vector3i _pos)
	{
		Vector3i position = Position;
		Vector3i vector3i = position + size;
		if (_pos.x >= position.x && _pos.x < vector3i.x && _pos.y >= position.y && _pos.y < vector3i.y && _pos.z >= position.z)
		{
			return _pos.z < vector3i.z;
		}
		return false;
	}
	...
	public void camPostRender(Camera _cam)
	{
		...
		if ((!isActive && sqrMagnitude > 62500f) || !base.gameObject.activeInHierarchy)
		{
			return;
		}
		GUIUtils.SetupLines(_cam, 3f);
		if (bDrawDirection && (isActive || bAlwaysDrawDirection))
		{
		...
		if (!isActive || GamePrefs.GetInt(EnumGamePrefs.SelectionOperationMode) == 0)
		{
			return;
		}
		...
		{
			if (ownerCategory.BoxCallbacks.OnSelectionBoxIsAvailable(EnumSelectionBoxAvailabilities.CanResize))
			{
				Axises.Add(vector3 + num2 * Vector3.right);
				Axises.Add(vector3 - num2 * Vector3.right);
				Axises.Add(vector3 + num2 * Vector3.up);
		...
		else if (GamePrefs.GetInt(EnumGamePrefs.SelectionOperationMode) == 3 && ownerCategory.BoxCallbacks.OnSelectionBoxIsAvailable(EnumSelectionBoxAvailabilities.CanMirror))
		{
			Axises.Add(vector3 + num2 * Vector3.right);
			Axises.Add(vector3 - num2 * Vector3.right);
			Axises.Add(vector3 + num2 * Vector3.up);
