...
public class QuestsFromXml
{
	...
	public static void ParseNode(XElement root)
	{
		...
		if (root.HasAttribute("starter_quest"))
		{
			Quest.StarterQuest = root.GetAttribute("starter_quest");
		}
	}
	...
	public static void ParseQuestList(XElement e)
	{
		...
		foreach (XElement item2 in e.Elements("quest"))
		{
			...
			string cvar = "";
			if (item2.HasAttribute("cvar"))
			{
				cvar = item2.GetAttribute("cvar");
			}
			if (item2.HasAttribute("id"))
			{
				QuestEntry item = new QuestEntry(item2.GetAttribute("id"), _result, startStage, endStage, cvar);
