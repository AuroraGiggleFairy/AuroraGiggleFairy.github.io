...
public class Inventory
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public Coroutine delayedShowHideHeldItemRoutine;
	...
	public Inventory(IGameManager _gameManager, EntityAlive _entity)
	{
		...
		emptyItem = new ItemInventoryData(null, ItemStack.Empty, _gameManager, _entity, 0);
		Clear();
		m_HoldingItemIdx = 0;
	...
	public bool UsingBareHand()
	{
		return bareHandItem == holdingItem;
	}
	...
	{
		if (delayedShowHideHeldItemRoutine != null)
		{
			GameManager.Instance.StopCoroutine(delayedShowHideHeldItemRoutine);
			delayedShowHideHeldItemRoutine = null;
		}
		delayedShowHideHeldItemRoutine = GameManager.Instance.StartCoroutine(delayedShowHideHeldItem(hideFirst, waitTime));
	}
	...
	public void clearSlotByIndex(int _idx)
	{
		...
		{
			slots[_idx].itemStack = ItemStack.Empty;
		}
	...
	{
		return ItemClass.GetForId(_itemValue.type).CreateInventoryData(ItemStack.Empty, gameManager, entity, _idx);
	}
	...
	public IEnumerator SimulateActionExecution(int _actionIdx, ItemStack _itemStack, Action<ItemStack> onComplete)
	{
		...
		void HandleComplete()
		{
			...
			SetItem(DUMMY_SLOT_IDX, ItemStack.Empty);
			onComplete(obj);
		}
		[PublicizedFrom(EAccessModifier.Private)]
	...
	public bool CanStackNoEmpty(ItemStack _itemStack)
	{
		for (int i = 0; i < PUBLIC_SLOTS; i++)
		{
			if (slots[i].itemStack.CanStackPartlyWith(_itemStack, out var _))
			{
				return true;
			}
		}
