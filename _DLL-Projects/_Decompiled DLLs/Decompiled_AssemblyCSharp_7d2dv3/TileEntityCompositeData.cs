...
public class TileEntityCompositeData
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public readonly Dictionary<int, int> featureIndexByHash = new Dictionary<int, int>();
	...
	{
		if (!_block.Properties.Classes.TryGetValue("CompositeFeatures", out var value))
		{
		...
		return new TileEntityCompositeData(_block, value);
	}
	...
	public TileEntityCompositeData(BlockCompositeTileEntity _block, DynamicProperties _compositeProps)
	{
		...
		int num = 0;
		foreach (var (text2, props) in _compositeProps.Classes)
		{
		...
		for (int i = 0; i < Features.Count; i++)
		{
			...
			featureIndexByName[tileEntityFeatureData.Name.AsMemory()] = i;
			int stableHashCode = tileEntityFeatureData.Name.GetStableHashCode();
			if (featureIndexByHash.TryGetValue(stableHashCode, out var value2))
			{
				TileEntityFeatureData tileEntityFeatureData2 = Features[value2];
				Log.Error($"Composite block \"{_block.GetBlockName()}\": Feature hash collision 0x{stableHashCode:X8} for \"{tileEntityFeatureData2.Name}\" vs \"{tileEntityFeatureData.Name}\". " + "This hash will be ambiguous during load.");
			}
			else
			{
				featureIndexByHash.Add(stableHashCode, i);
			}
		}
	...
	public int GetFeatureIndex(int _featureNameHash)
	{
		return featureIndexByHash.GetValueOrDefault(_featureNameHash, -1);
	}
	...
	public bool HasFeature(int _featureNameHash)
	{
		return GetFeatureIndex(_featureNameHash) >= 0;
	}
	...
	public bool TryGetFeatureData<T>(out TileEntityFeatureData featureData) where T : class, ITileEntityFeature
	{
		int featureIndex = GetFeatureIndex<T>();
		if (featureIndex >= 0)
		{
			featureData = Features[featureIndex];
			return true;
		}
		featureData = null;
		return false;
	...
	public void PrintConfig()
	{
		...
		string value;
		string key;
		foreach (KeyValuePair<string, string> value2 in CompositeProps.Values)
		{
			value2.Deconstruct(out value, out key);
			string text = value;
			string text2 = key;
			Log.Out("    " + text + "=" + text2);
		}
		foreach (TileEntityFeatureData feature in Features)
		{
			...
			foreach (KeyValuePair<string, string> value3 in feature.Props.Values)
			{
