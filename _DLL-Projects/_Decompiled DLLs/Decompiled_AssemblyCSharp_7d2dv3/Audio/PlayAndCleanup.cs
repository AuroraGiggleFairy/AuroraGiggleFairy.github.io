...
public class PlayAndCleanup
{
	...
	public PlayAndCleanup(LoopingPair _lp, string _soundGroupName)
	{
		...
		{
			Manager.AddPlayAndCleanupAudioSource(_soundGroupName, lp.sgoBegin.src);
			Manager.AddPlayAndCleanupAudioSource(_soundGroupName, lp.sgoLoop.src);
		}
		GameManager.Instance.StartCoroutine(StopBeginWhenDone(lp.sgoBegin.src.clip.length, _soundGroupName));
	}
	...
	public IEnumerator StopBeginWhenDone(float waitTime, string soundGroupName)
	{
		...
		if (!(lp.sgoBegin.src == null))
		{
			...
			{
				Manager.RemovePlayAndCleanupAudioSource(soundGroupName, lp.sgoBegin.src);
			}
	...
	public PlayAndCleanup(GameObject _go, AudioSource _source, float _occlusion = 0f, float delay = 0f, bool isLooping = false, bool hasLoopingAnalog = false, string soundGroupName = "")
	{
		...
		if (!string.IsNullOrEmpty(soundGroupName))
		{
			Manager.AddPlayAndCleanupAudioSource(soundGroupName, src);
		}
		if (!isLooping)
		{
			float waitTime = _source.clip.length * (1f + Utils.FastClamp01(1f - _source.pitch)) + delay;
			GameManager.Instance.StartCoroutine(StopWhenDone(waitTime, soundGroupName));
		}
	}
	...
	public IEnumerator StopWhenDone(float waitTime, string soundGroupName)
	{
		...
		if (!string.IsNullOrEmpty(soundGroupName))
		{
			Manager.RemovePlayAndCleanupAudioSource(soundGroupName, src);
		}
		if ((bool)go)
		{
			Object.Destroy(go);
