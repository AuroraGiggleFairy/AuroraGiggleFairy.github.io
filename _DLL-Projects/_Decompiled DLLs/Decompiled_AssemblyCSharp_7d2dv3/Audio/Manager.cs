...
public class Manager : IDisposable
{
	...
	{
		audioData = new Dictionary<string, XmlData>(StringComparer.OrdinalIgnoreCase);
		...
		playAndCleanupSources = new DictionaryList<string, List<AudioSource>>();
	...
	public static void BroadcastPlay(Entity entity, string soundGroupName, bool signalOnly = false, float volumeScale = 1f)
	{
		...
		if (ServerAudio != null)
		{
			if (!GameManager.IsDedicatedServer)
			{
				Play(entity, soundGroupName, volumeScale);
			}
	...
	public static Handle Play(Vector3 position, string soundGroupName, int entityId = -1, bool wantHandle = false, float _volumeScale = 1f)
	{
		...
		SignalAI(entity, position, soundGroupName, _volumeScale);
		...
		if (!value.CanAddANewVoice())
		{
			return null;
		}
		if (value.stopOldestNode)
		{
			Stop(entityId, soundGroupName);
		}
		...
		if (flag2)
		{
			...
			value.lastClipLenght = audioSource.clip.length;
			...
			if (entity != null && entity is EntityAlive)
			{
				...
				num = (((EntityAlive)entity).IsCrouching ? (num * value.localCrouchVolumeScale) : num);
				audioSource.volume = (((EntityAlive)entity).MovementRunning ? (num * value.runningVolumeScale) : num);
			}
		}
		...
		if (flag)
		{
			...
			value.lastClipLenght = audioSource2.clip.length;
			...
			if (entity != null && entity is EntityAlive)
			{
				float volume = audioSource2.volume;
				volume = (((EntityAlive)entity).IsCrouching ? (volume * value.localCrouchVolumeScale) : volume);
				audioSource2.volume = (((EntityAlive)entity).MovementRunning ? (volume * value.runningVolumeScale) : volume);
			}
		}
		...
		else
		{
			...
			{
				SetPitch(audioSource, value, 0f);
				new PlayAndCleanup(gameObject, audioSource, (entity == null) ? 0f : entity.CalculateAudioOcclusion(), 0f, isLooping: false, hasLoopingAnalog: false, soundGroupName);
			...
			{
				SetPitch(audioSource2, value, 0f);
				new PlayAndCleanup(gameObject2, audioSource2, (entity == null) ? 0f : entity.CalculateAudioOcclusion(), 0f, isLooping: false, hasLoopingAnalog: false, soundGroupName);
			}
		}
		if (wantHandle)
		{
	...
	public static Handle Play(Entity _entity, string soundGroupName, float volumeScale = 1f, bool wantHandle = false)
	{
		...
		if (value.stopOldestNode)
		{
			Stop(_entity.entityId, soundGroupName);
		}
		bool flag = false;
		...
		if (flag2)
		{
			...
			value.lastClipLenght = audioSource3.clip.length;
		...
		else
		{
			...
			if (audioSource2 != null)
			{
				new PlayAndCleanup(gameObject, audioSource2, _entity.CalculateAudioOcclusion(), 0f, isLooping: false, hasLoopingAnalog: false, soundGroupName);
			}
			if (audioSource3 != null)
			{
				new PlayAndCleanup(gameObject2, audioSource3, _entity.CalculateAudioOcclusion(), 0f, isLooping: false, hasLoopingAnalog: false, soundGroupName);
			}
		}
		Handle result = null;
		if (wantHandle)
	...
	{
		lock (playAndCleanupSources)
		{
			List<AudioSource> list = playAndCleanupSources.Get(soundGroupName);
			if (list == null)
			{
				list = new List<AudioSource> { _src };
			}
			else
			{
				list.Add(_src);
			}
			playAndCleanupSources.Set(soundGroupName, list);
		}
	}
	...
	public static void RemovePlayAndCleanupAudioSource(string soundGroupName, AudioSource _src)
	{
		lock (playAndCleanupSources)
		{
			...
			if (list != null)
			{
				...
				if (list.Count == 0)
				{
					playAndCleanupSources.Remove(soundGroupName);
				}
				else
				{
					playAndCleanupSources.Set(soundGroupName, list);
	...
	public static void Stop(int entityId, string soundGroupName)
	{
		...
		StopPlayAndCleanupSource(soundGroupName);
	}
	...
	public static void StopPlayAndCleanupSource(string soundGroupName)
	{
		lock (playAndCleanupSources)
		{
			...
			{
				StopSource(audioSource);
				RemovePlayAndCleanupAudioSource(soundGroupName, audioSource);
	...
	public static void PlayInsidePlayerHead(string soundGroupNameBegin, int entityID)
	{
		...
		ConvertName(ref soundGroupName);
		if (!audioData.TryGetValue(soundGroupNameBegin, out var value) || !value.CanAddANewVoice())
		{
			return;
		}
		if (value.stopOldestNode)
		{
			Stop(entityID, soundGroupNameBegin);
		}
		if (!audioData.TryGetValue(soundGroupName, out var value2) || !value2.CanAddANewVoice())
		{
			return;
		}
		if (value2.stopOldestNode)
		{
			Stop(entityID, soundGroupName);
		}
		ClipSourceMap randomClip = value.GetRandomClip();
		...
		if (!(audioSource2 == null))
		{
			...
			new PlayAndCleanup(lp, soundGroupName);
		}
	}
	...
	public static void PlayInsidePlayerHead(string soundGroupName, int entityID = -1, float delay = 0f, bool isLooping = false, bool isUnique = false)
	{
		...
		if (!audioData.TryGetValue(soundGroupName, out var value) || !value.CanAddANewVoice())
		{
			return;
		}
		if (value.stopOldestNode)
		...
		if (isLooping)
		{
			...
			new PlayAndCleanup(gameObject, audioSource, 0f, delay, isLooping: true, hasLoopingAnalog: false, soundGroupName);
		}
		else
		{
			if (isUnique)
			...
			new PlayAndCleanup(gameObject, audioSource, 0f, delay, isLooping: false, hasLoopingAnalog: false, soundGroupName);
		}
		if (GamePrefs.GetBool(EnumGamePrefs.OptionsSubtitlesEnabled) && randomClip.hasSubtitle)
		{
	...
	public static void PlaySequence(Entity entity, string soundGroupName)
	{
		...
		if (!CheckGlobalPlayRequirements(soundGroupName) || entity == null || !audioData.TryGetValue(soundGroupName, out var value) || !value.CanAddANewVoice())
		{
			return;
		}
		if (value.stopOldestNode)
		{
			Stop(entity.entityId, soundGroupName);
		}
