...
public class XmlData
{
	...
	public XmlData()
	{
		...
		noiseScale = 1f;
		maxRepeatRate = 0.1f;
		prioritizeNewNodes = false;
		stopOldestNode = false;
		voicesPlaying = 0;
	...
	public bool CanAddANewVoice()
	{
		if (maxRepeatRate > 0f)
		{
			...
			float num2 = maxRepeatRate;
			if (prioritizeNewNodes && lastClipLenght > 0f)
			{
				num2 = lastClipLenght;
			}
			voicesPlaying = Utils.FastClamp(voicesPlaying - (int)(num / num2), 0, 999);
			if (voicesPlaying >= maxVoices)
			{
				if (prioritizeNewNodes)
				{
					stopOldestNode = true;
					return true;
				}
