...
public class EntityTurret : EntityAlive
{
	...
	public override void updateTransform()
	{
		...
		if (GameManager.Instance.World.GetChunkFromWorldPos((int)vector.x, (int)vector.y, (int)vector.z) is Chunk chunk && chunk.IsCollisionMeshGenerated)
		{
			if (posCheckTimer <= 0f)
			{
				...
				float y = fallPos.y;
				fallPos = vector;
				if (Voxel.Raycast(ray: new Ray(vector + Vector3.up * 0.375f, Vector3.down), _world: GameManager.Instance.World, distance: 255f, _layerMask: 1082195969, _hitMask: 128, _sphereRadius: 0.25f))
				{
	...
	public override string GetActivationText()
	{
		...
		return string.Format(Localization.Get("turretPickUp"), arg, LocalizedEntityName);
	}
	...
	public override bool AllowActivationCommand(ReadOnlySpan<char> _commandName, EntityPlayerLocal _playerFocusing)
	{
		...
		{
			return CanInteract(_playerFocusing.entityId);
		}
