...
public static class ShapesFromXml
{
	...
	public static void CreateShapeMaterialCombination(bool _bEditMode, XElement _elementBlock, string _blockBaseName, KeyValuePair<string, XElement> _shapeKvp)
	{
		...
		if (dynamicProperties.TryGetValue(Block.PropDowngradeBlock, out var _value))
		{
			_value = AppendShapeName(_value, _blockBaseName, key);
			dynamicProperties.SetValue(Block.PropDowngradeBlock, _value);
		}
		if (dynamicProperties.TryGetValue(Block.PropUpgradeBlockClass, Block.PropUpgradeBlockToBlock, out var _value2))
		{
			_value2 = AppendShapeName(_value2, _blockBaseName, key);
			dynamicProperties.SetValue("UpgradeBlock", "ToBlock", _value2);
		}
		if (dynamicProperties.TryGetValue(Block.PropSiblingBlock, out var _value3))
		{
			_value3 = PrependBlockBaseName(_value3, _blockBaseName);
			dynamicProperties.SetValue(Block.PropSiblingBlock, _value3);
		}
		if (dynamicProperties.TryGetValue("MirrorSibling", out var _value4))
		{
			_value4 = PrependBlockBaseName(_value4, _blockBaseName);
			dynamicProperties.SetValue("MirrorSibling", _value4);
		}
