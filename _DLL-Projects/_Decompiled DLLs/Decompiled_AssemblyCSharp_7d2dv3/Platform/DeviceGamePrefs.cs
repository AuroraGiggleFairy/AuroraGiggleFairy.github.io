...
public static class DeviceGamePrefs
{
	public static void Apply()
	{
		...
		{
			GameOptionsPlatforms.GfxPreset gfxPreset = (GameOptionsPlatforms.GfxPreset)GamePrefs.GetInt(EnumGamePrefs.OptionsGfxQualityPreset);
			if (gfxPreset != GameOptionsPlatforms.GfxPreset.ConsolePerformance && gfxPreset != GameOptionsPlatforms.GfxPreset.ConsoleQuality)
			{
				Log.Out($"[DeviceGamePrefs] Quality preset \"{gfxPreset}\" is unsupported on this platform; defaulting to ConsolePerformance.");
				GamePrefs.Set(EnumGamePrefs.OptionsGfxQualityPreset, 6);
			}
			int num = GamePrefs.GetInt(EnumGamePrefs.OptionsGfxUpscalerMode);
			if (num != 2 && num != 4)
			{
				Log.Out($"[DeviceGamePrefs] Upscaler mode \"{num}\" is unsupported on this platform; defaulting to \"{GameOptionsPlatforms.DefaultUpscalerMode}\".");
	...
	{
		foreach (string key in properties.Values.Keys)
		{
