...
public static class PlatformApplicationManager
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static EPlatformLoadSaveGameState loadSaveGameState = EPlatformLoadSaveGameState.Init;
	...
	{
		isRestartRequired = RestartIsSupported;
	...
	public static EPlatformLoadSaveGameState GetLoadSaveGameState()
	{
		...
		{
			UserDataStorageType saveStorage = (UserDataStorageType)GamePrefs.GetInt(EnumGamePrefs.GameSaveStorageType);
			PathAbstractions.Contextual.FindActiveWorld(out var worldName, out var _location);
			if (_location.Type != PathAbstractions.EAbstractedLocationType.None)
			{
				Log.Warning("[LoadSaveGame] World " + worldName + " could not be found. GameWorld: " + GamePrefs.GetString(EnumGamePrefs.GameWorld) + ", GameWorldLocationType: " + GamePrefs.GetString(EnumGamePrefs.GameWorldLocationType) + ", " + $"UserWorldStorageType: {(UserDataStorageType)GamePrefs.GetInt(EnumGamePrefs.UserWorldStorageType)}");
				return loadSaveGameState = EPlatformLoadSaveGameState.Done;
			}
			string gameName = GamePrefs.GetString(EnumGamePrefs.GameName);
			bool found = false;
			bool isArchived = false;
			GameIO.GetPlayerSaves([PublicizedFrom(EAccessModifier.Internal)] (UserDataStorageType foundStorage, string foundSaveName, string foundWorldName, DateTime _, WorldState _, bool foundIsArchived) =>
			{
				if (foundStorage == saveStorage && foundSaveName.EqualsCaseInsensitive(gameName) && foundWorldName.EqualsCaseInsensitive(worldName))
				{
					found = true;
