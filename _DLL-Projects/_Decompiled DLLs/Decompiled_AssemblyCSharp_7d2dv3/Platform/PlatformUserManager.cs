...
public static class PlatformUserManager
{
	...
	public static void Init()
	{
		...
		s_nativeIdToPrimaryIds = new OneToManyDictionary<PlatformUserIdentifierAbs, PlatformUserIdentifierAbs>();
	...
	public static void UpdateUserDetails()
	{
		...
		using (s_userDetailsToUpdateLock.UpgradableReadLockScope())
		{
			...
			{
				if (!(item.NextDetailRetryTime > realtimeSinceStartup))
				{
	...
	public static IEnumerator ResolveUserDetailsCoroutine()
	{
		try
		{
			...
			using (s_userDetailsToUpdateLock.WriteLockScope())
			{
				...
				{
					PlatformUserData userData = item2.UserData;
					if (!string.IsNullOrEmpty(item2.Name))
					{
						userData.DetailAttempts = 0;
						userData.NextDetailRetryTime = 0f;
						s_userDetailsToUpdate.Remove(userData);
						continue;
					}
					userData.DetailAttempts++;
					if (userData.DetailAttempts >= 5)
					{
						LogWarning($"[PLATFORM] User details lookup gave up after {userData.DetailAttempts} attempts.");
						userData.DetailAttempts = 0;
						userData.NextDetailRetryTime = 0f;
						s_userDetailsToUpdate.Remove(userData);
						continue;
