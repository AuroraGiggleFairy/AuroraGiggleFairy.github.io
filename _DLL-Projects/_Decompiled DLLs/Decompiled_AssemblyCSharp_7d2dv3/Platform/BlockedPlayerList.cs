...
public class BlockedPlayerList : IRemotePlayerStorageObject
{
	...
	public void Update()
	{
		if (PlatformManager.MultiPlatform?.RemotePlayerFileStorage == null)
		{
			return;
		}
		if (writeRequestTime.HasValue)
		{
			...
			if (utcNow - dateTime >= WriteRequestDelay)
			{
				goto IL_0093;
			}
		}
		if (!(DateTime.UtcNow - lastWriteTime >= WriteThreshold))
		{
			return;
		}
		goto IL_0093;
	...
	public void MarkForWrite()
	{
		if (!writeRequestTime.HasValue)
		{
			writeRequestTime = DateTime.UtcNow;
		}
	}
