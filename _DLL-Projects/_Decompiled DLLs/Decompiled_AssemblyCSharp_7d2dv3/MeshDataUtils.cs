...
public static class MeshDataUtils
{
	...
	public static void CalculateNormals(ReadOnlySpan<Vector3> vertices, ReadOnlySpan<int> indices, Span<Vector3> normals)
	{
		...
		Allocator allocator = (ThreadManager.IsMainThread() ? Allocator.Temp : Allocator.Persistent);
		using NativeParallelMultiHashMap<Vector3, int> vertexToIndices = new NativeParallelMultiHashMap<Vector3, int>(num, allocator);
		for (int j = 0; j < num; j++)
		{
			vertexToIndices.Add(vertices[j], j);
		}
		using NativeArray<int> normalsAdded = new NativeArray<int>(num, allocator);
		for (int k = 0; k + 2 < indices.Length; k += 3)
		{
			int index = indices[k];
			int index2 = indices[k + 1];
			int index3 = indices[k + 2];
