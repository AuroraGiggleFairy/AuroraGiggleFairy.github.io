...
public class XUiC_Chat : XUiController
{
	...
	public void registerRegularChatMessageHandlers()
	{
		...
		void SendMessageRegular(EChatType _chatType, string _targetId, string _message)
		{
			...
			GameManager.Instance.ChatMessageServer(null, _chatType, xui.playerUI.entityPlayer.entityId, _message, recipientEntityIds, EMessageSender.SenderIdAsPlayer);
		}
	}
	...
	public void updateTargets()
	{
		...
		entityIdsFriends.Add(xui.playerUI.entityPlayer.entityId);
		...
		entityIdsParty.Add(xui.playerUI.entityPlayer.entityId);
		if (xui.playerUI.entityPlayer.Party != null)
		{
			foreach (EntityPlayer member in xui.playerUI.entityPlayer.Party.MemberList)
			{
				if (member != xui.playerUI.entityPlayer)
				{
		...
		for (int num = allTargets.Count - 1; num >= 0; num--)
		{
			...
			else if (!chatTarget.IsValid(xui))
			{
	...
	public void UpdateInput()
	{
		...
		if (xui.playerUI.playerInput.PermanentActions.Cancel.WasPressed)
		{
	...
	public static bool GetChatTargetTooltip(XUi _xuiInstance, EChatType _chatType, string _targetId, out string _targetTooltip)
	{
		_targetTooltip = null;
		if (_xuiInstance.GetWindowByType<XUiC_Chat>() == null)
		{
			Log.Error("No chat window found!");
			return false;
		}
		if ((int)_chatType >= messagingHandlers.Length || messagingHandlers[(int)_chatType] == null)
		{
			Log.Warning($"Can not handle chat messaging, invalid chat type '{(int)_chatType}' or no handler defined");
			return false;
		}
		if (!messagingHandlers[(int)_chatType].IsValidTargetDelegate(_chatType, _targetId))
		{
			return false;
		}
		_targetTooltip = messagingHandlers[(int)_chatType].GetTargetDisplayNameDelegate(_chatType, _targetId);
		return true;
	}
