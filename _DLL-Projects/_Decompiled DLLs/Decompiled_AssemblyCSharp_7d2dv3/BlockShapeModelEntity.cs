...
public class BlockShapeModelEntity : BlockShapeInvisible
{
	...
	{
		base.Init(_block);
		modelNameWithPath = _block.Properties.GetValue("Model");
		...
		if (_block.Properties.Values.TryGetValue(PropDamagedMesh, out var value))
		{
	...
	{
		Transform transform = DataLoader.LoadAsset<Transform>(modelNameWithPath);
		if (transform == null)
		{
			Log.Error("Model '{0}' not found on block with name {1}", modelNameWithPath, block.GetBlockName());
			transform = DataLoader.LoadAsset<Transform>("@:Entities/Misc/block_missingPrefab.prefab");
			if (transform == null)
			{
				return null;
			}
	...
	public void registerSleepers(Vector3i _blockPos, BlockValue _blockValue)
	{
		...
		{
			Prefab.TransientSleeperBlockIncrement(_blockPos, 1);
			ThreadManager.AddSingleTaskMainThread("OnBlockAddedOrLoaded.RegisterSleeperBlock", [PublicizedFrom(EAccessModifier.Internal)] () =>
			{
	...
	public bool IsObstructionForDamageState(BlockValue _blockValue)
	{
		...
		{
			GetDamageStateIndex(_blockValue);
			List<DamageState> list = damageStates;
