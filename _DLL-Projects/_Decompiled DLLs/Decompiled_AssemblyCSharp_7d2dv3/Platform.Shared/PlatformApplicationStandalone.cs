...
public class PlatformApplicationStandalone : IPlatformApplication
{
	...
	public (int width, int height, FullScreenMode fullScreenMode) ScreenOptions
	{
		get
		{
			FullScreenMode item = (FullScreenMode)PlayerPrefs.GetInt("Screenmanager Fullscreen mode", 3);
			if (PlayerPrefs.HasKey("Screenmanager Resolution Width") && PlayerPrefs.HasKey("Screenmanager Resolution Height"))
			{
				int item2 = PlayerPrefs.GetInt("Screenmanager Resolution Width");
				int item3 = PlayerPrefs.GetInt("Screenmanager Resolution Height");
