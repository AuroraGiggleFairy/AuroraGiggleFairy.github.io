...
public class XUiC_MapEnterWaypoint : XUiController
{
	...
	{
		((XUiC_MapArea)xui.GetWindow("mapArea").Controller).closeAllPopups();
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void waypointOnSubmitHandler(XUiController _sender, string _text)
	{
		((XUiC_MapArea)xui.GetWindow("mapArea").Controller).OnWaypointCreated(_text);
	}

	public void Show(Vector2i _position)
	{
		XUiV_Window window = xui.GetWindow("mapAreaEnterWaypointName");
