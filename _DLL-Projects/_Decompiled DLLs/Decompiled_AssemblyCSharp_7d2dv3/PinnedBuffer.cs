...
public abstract class PinnedBuffer : IDisposable
{
	...
	public static PinnedBuffer<T> Create<T>(ArrayListMP<T> list, Ownership ownership) where T : unmanaged
	{
		...
		switch (ownership)
		{
		case Ownership.Steal:
		{
			T[] items = list.Items;
			int count = list.Count;
			list.Items = null;
			list.Count = 0;
			return new PinnedBuffer<T>(list.pool, items, count);
		}
		case Ownership.Clone:
		{
		...
		default:
			throw new ArgumentOutOfRangeException("ownership", ownership, null);
		}
	}
...
public sealed class PinnedBuffer<T> : PinnedBuffer where T : unmanaged
{
	...
	public PinnedBuffer(MemoryPooledArray<T> pool, T[] poolArray, int length)
	{
		m_pool = pool;
		m_poolArray = poolArray;
		InitBuffer(poolArray, length);
	}
