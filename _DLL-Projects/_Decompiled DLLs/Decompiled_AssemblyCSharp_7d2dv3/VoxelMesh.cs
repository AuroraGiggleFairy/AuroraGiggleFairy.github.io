...
public class VoxelMesh
{
	...
	public class MeshFilterState
	{
		public int NextGeneration;

		public int AppliedGeneration = -1;
	}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static readonly ConditionalWeakTable<MeshFilter, MeshFilterState> m_meshFilterStates = new ConditionalWeakTable<MeshFilter, MeshFilterState>();
	...
	{
		CreateMeshFilter(_meshIndex, _yOffset, _gameObject, _meshTag, _bAllowLOD, out _mf, out _mr);
		CreateMeshCollider(_meshIndex, _gameObject, _meshTag, out _mc);
	}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static void CreateMeshCollider(int _meshIndex, GameObject _gameObject, string _meshTag, out MeshCollider _mc)
	{
		_mc = null;
		MeshDescription meshDescription = MeshDescription.meshes[_meshIndex];
	...
	{
		MeshFilterState filterState = m_meshFilterStates.GetOrCreateValue(_mf);
		int count = m_Vertices.Count;
		if (count == 0)
		{
			filterState.AppliedGeneration = filterState.NextGeneration++;
			Mesh sharedMesh = _mf.sharedMesh;
			if ((bool)sharedMesh)
			{
				_mf.sharedMesh = null;
				AddPooledMesh(sharedMesh);
			}
		...
		int generation = filterState.NextGeneration++;
		Mesh newMesh = GetPooledMesh();
		copyToMesh(newMesh, m_Vertices, m_Indices, m_Uvs, UvsCrack, m_Normals, m_Tangents, m_ColorVertices, [PublicizedFrom(EAccessModifier.Internal)] () =>
		{
			if (!_mf || generation <= filterState.AppliedGeneration)
			{
				AddPooledMesh(newMesh);
				_onCopyComplete?.Invoke();
			}
			else
			{
				filterState.AppliedGeneration = generation;
				Mesh sharedMesh2 = _mf.sharedMesh;
				_mf.sharedMesh = newMesh;
				AddPooledMesh(sharedMesh2);
				_onCopyComplete?.Invoke();
			}
		});
		return m_Triangles;
	...
	public void copyToMesh(Mesh _mesh, ArrayListMP<Vector3> _vertices, ArrayListMP<int> _indices, ArrayListMP<Vector2> _uvs, ArrayListMP<Vector2> _uvCracks, ArrayListMP<Vector3> _normals, ArrayListMP<Vector4> _tangents, ArrayListMP<Color> _colorVertices, Action _onCopyComplete = null)
	{
		...
		{
			MeshDataManager.Instance.Add(_mesh, _vertices, _indices, _normals, _tangents, _colorVertices, _uvs, _uvCracks, PinnedBuffer.Ownership.Steal, generateNormals: true, generateTangents: true, recalculateUvDistributionMetrics: false, sameFrameUpload: false, _onCopyComplete);
	...
	public virtual int CopyToColliders(MeshCollider _meshCollider, out Mesh mesh)
	{
		if (m_CollIndices == null)
		{
			mesh = null;
			return 0;
		}
