...
public class ItemClass : ItemData
{
	...
	public static string PropSoundImpactHit = "SoundImpactHit";

	public static string PropSoundImpactGraze = "SoundImpactGraze";
	...
	public static string PropReplaceResourceUnit = "ReplaceResourceUnit";
	...
	public static string PropQualityMin = "QualityMin";
	...
	public static string PropRestrictedMove = "RestrictedMove";

	public static string PropPreviousItemType = "PreviousTechItem";
	...
	public string ReplaceResourceUnit = "";
	...
	public string SoundImpactGraze;
	...
	[PublicizedFrom(EAccessModifier.Protected)]
	public bool bRestrictedMove;

	[PublicizedFrom(EAccessModifier.Private)]
	public List<XUiC_ItemStack.StackLocationTypes> restrictedTo = new List<XUiC_ItemStack.StackLocationTypes>();

	public ItemTechTypes ItemTechType;

	[PublicizedFrom(EAccessModifier.Private)]
	public string previousTechTypeName = "";

	[PublicizedFrom(EAccessModifier.Private)]
	public ItemClass[] previousTechItems;

	public static ItemTechTypes MaxTechType = ItemTechTypes.T3;
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public FastTags<TagGroup.Global> T0Tag = FastTags<TagGroup.Global>.Parse("T0");

	[PublicizedFrom(EAccessModifier.Private)]
	public FastTags<TagGroup.Global> T1Tag = FastTags<TagGroup.Global>.Parse("T1");

	[PublicizedFrom(EAccessModifier.Private)]
	public FastTags<TagGroup.Global> T2Tag = FastTags<TagGroup.Global>.Parse("T2");

	[PublicizedFrom(EAccessModifier.Private)]
	public FastTags<TagGroup.Global> T3Tag = FastTags<TagGroup.Global>.Parse("T3");
	...
	public float TraderQualityMaxMod;

	public float TraderQualityMinMod;

	[PublicizedFrom(EAccessModifier.Private)]
	public const string cFilterQualityPrefix = "#";
	...
	public const float cGSStatScale = 0.005f;

	public const float cGSStatMax = 163.83499f;

	public Dictionary<PassiveEffects, List<GSStat>> Stats;
	...
	public virtual bool IsLimitedItem => IsLimited;
	...
	public bool CanMoveToLocation(XUiC_ItemStack.StackLocationTypes locationType)
	{
		bool result = true;
		if (bRestrictedMove)
		{
			result = false;
			foreach (XUiC_ItemStack.StackLocationTypes item in restrictedTo)
			{
	...
	public virtual void Init()
	{
		...
		Properties.ParseString(PropSoundImpactHit, ref SoundImpactHit);
		Properties.ParseString(PropSoundImpactGraze, ref SoundImpactGraze);
		...
		Properties.ParseString(PropReplaceResourceUnit, ref ReplaceResourceUnit);
		...
		Properties.ParseBool(PropShowQuality, ref ShowQualityBar);
		Properties.ParseByte(PropQualityMin, ref QualityMin);
		...
		if (Properties.Classes.ContainsKey("SDCS"))
		{
			...
			if (Properties.Contains("SDCS", "Prefab"))
			{
				SDCSData.PrefabName = Properties.GetString("SDCS", "Prefab");
			}
			if (Properties.Contains("SDCS", "TransformName"))
			{
				SDCSData.PartName = Properties.GetString("SDCS", "TransformName");
			}
			if (Properties.Contains("SDCS", "Excludes"))
			{
				SDCSData.BaseToTurnOff = Properties.GetString("SDCS", "Excludes");
			}
			if (Properties.Contains("SDCS", "HairMaskType"))
			{
				SDCSData.HairMaskType = (SDCSUtils.SlotData.HairMaskTypes)Enum.Parse(typeof(SDCSUtils.SlotData.HairMaskTypes), Properties.GetString("SDCS", "HairMaskType"));
			}
			if (Properties.Contains("SDCS", "FacialHairMaskType"))
			{
				SDCSData.FacialHairMaskType = (SDCSUtils.SlotData.HairMaskTypes)Enum.Parse(typeof(SDCSUtils.SlotData.HairMaskTypes), Properties.GetString("SDCS", "FacialHairMaskType"));
			}
		}
		...
		Properties.ParseString(PropRestrictedMove, ref optionalValue);
		string[] array3 = optionalValue.Split(',');
		if (optionalValue.Length > 0)
		{
			if (!bool.TryParse(array3[0], out bRestrictedMove))
			{
				bRestrictedMove = false;
			}
			if (bRestrictedMove)
			{
				for (int j = 1; j < array3.Length; j++)
				{
					if (Enum.TryParse<XUiC_ItemStack.StackLocationTypes>(array3[j], out var result3))
					{
						restrictedTo.Add(result3);
					}
				}
			}
		}
		if (Properties.Values.ContainsKey("TraderQualityMod"))
		{
			float _minCount = 1f;
			float _maxCount = 1f;
			StringParsers.ParseMinMaxCount(Properties.Values["TraderQualityMod"], out _minCount, out _maxCount);
			TraderQualityMinMod = _minCount;
			TraderQualityMaxMod = _maxCount;
		}
		else
		{
			TraderQualityMaxMod = 0f;
			TraderQualityMinMod = 0f;
		}
		Properties.ParseString(PropPreviousItemType, ref previousTechTypeName);
		CalculateTechType();
	}
	...
	public static ItemValue CreateItemValue(string _itemName, int _quality, bool _caseInsensitive = false)
	{
		ItemClass itemClass = GetItemClass(_itemName, _caseInsensitive);
		if (itemClass != null)
		{
			return new ItemValue(itemClass.Id, _quality, _quality);
		}
		return ItemValue.None;
	}
	...
	public static void GetItemsAndBlocks(List<ItemClass> _targetList, int _idStart = -1, int _idEndExcl = -1, FilterItem[] _filterExprs = null, string _nameFilter = null, bool _bShowUserHidden = false, EnumCreativeMode _currentCreativeMode = EnumCreativeMode.Player, bool _showFavorites = false, bool _sortBySortOrder = false, XUi _xui = null)
	{
		...
		if (_nameFilter != null)
		{
			if (_nameFilter.Length >= 2 && _nameFilter.StartsWith("#", StringComparison.OrdinalIgnoreCase))
			{
				_nameFilter = _nameFilter.Substring(2);
			}
	...
	public static void CreateItemStacks(IEnumerable<ItemClass> _itemClassList, List<ItemStack> _targetList, string _nameFilter = null)
	{
		_targetList.Clear();
		GameRandom gameRandom = GameRandomManager.Instance.CreateGameRandom(GameManager.frameCount);
		int minQuality = 1;
		int maxQuality = 6;
		if (_nameFilter != null && _nameFilter.Length >= 2 && _nameFilter.IndexOf("#", StringComparison.OrdinalIgnoreCase) == 0)
		{
			int num = (int)char.GetNumericValue(_nameFilter[1]);
			if (num >= 1 && num <= 6)
			{
				minQuality = num;
				maxQuality = num;
		...
		{
			ItemValue itemValue = new ItemValue(_itemClass.Id, minQuality, maxQuality, _bCreateDefaultModItems: true);
			int stage = ((itemValue.Quality != 0) ? ((itemValue.Quality - 1) * 20) : 0);
			_itemClass.AddGSStats(itemValue, stage, gameRandom);
	...
	public virtual bool RestrictedMove(ItemValue _iv = null)
	{
		return bRestrictedMove;
	}
		...
		if (!_bReleased)
		{
			...
			else
			{
				...
				if (curAction is ItemActionEat itemActionEat)
				{
					...
					{
						XUiC_MessageBoxWindowGroup.ShowOkCancel(xui, Localization.Get(itemActionEat.PromptTitle), Localization.Get(itemActionEat.PromptDescription), [PublicizedFrom(EAccessModifier.Internal)] () =>
						{
	...
	public virtual void OnPlacedAsCatalyst(ItemInventoryData _data)
	{
	}
	...
	{
		if (Properties.GetValue(PropSoundIdle) != null && _data.holdingEntitySoundID == 0)
		{
			Manager.BroadcastStop(_data.holdingEntity.entityId, Properties.GetValue(PropSoundIdle));
		}
	...
	public void GSStatsParseXml(XElement _element)
	{
		GSStat item = default(GSStat);
		foreach (XElement item2 in _element.Elements("stats"))
		{
			foreach (XElement item3 in item2.Elements())
			{
				...
				string attribute2 = item3.GetAttribute("value");
				...
				if (array.Length >= 5)
				{
					PassiveEffects passiveEffects = EnumUtils.Parse<PassiveEffects>(attribute);
					if (Stats == null)
					{
						Stats = new Dictionary<PassiveEffects, List<GSStat>>();
					}
					if (!Stats.TryGetValue(passiveEffects, out var value))
					{
						value = new List<GSStat>();
						Stats[passiveEffects] = value;
					}
					item.quality = short.Parse(array[0]);
					item.gameStage = short.Parse(array[1]);
					item.chance = float.Parse(array[2]);
					float num = float.Parse(array[3]);
					float num2 = float.Parse(array[4]);
					if (Utils.FastAbs(num) < 163.83499f && Utils.FastAbs(num2) < 163.83499f)
					{
						item.min = (short)(num / 0.005f);
						item.max = (short)(num2 / 0.005f);
						value.Add(item);
						continue;
					}
					Log.Warning("GSStat {0}, qual {1}, gs {2}, chance {3}, {4} to {5} out of range", passiveEffects, item.quality, item.gameStage, item.chance, num, num2);
				}
			}
		}
	}

	public void AddGSStats(ItemValue _iv, int _stage, GameRandom _rand)
	{
		if (Stats == null)
		{
			return;
		}
		_iv.InitStats(Stats.Count);
		int quality = _iv.Quality;
		int num = 0;
		foreach (KeyValuePair<PassiveEffects, List<GSStat>> stat in Stats)
		{
			stat.Deconstruct(out var key, out var value);
			PassiveEffects type = key;
			List<GSStat> list = value;
			int num2 = 0;
			int num3 = 0;
			GSStat gSStat = FindGSStat(list, 0, 0, _rand);
			if (gSStat.quality >= 0)
			{
				num2 = (short)_rand.RandomRange(gSStat.min, gSStat.max + 1);
			}
			GSStat gSStat2 = FindGSStat(list, quality, _stage, _rand);
			if (gSStat2.quality > 0)
			{
				num3 = (short)_rand.RandomRange(gSStat2.min, gSStat2.max + 1);
			}
			if (num2 + num3 != 0)
			{
				_iv.SetStat(num++, type, num2, num3);
			}
		}
		_iv.RemoveUnusedStats();
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public GSStat FindGSStat(List<GSStat> _list, int _quality, int _gameStage, GameRandom _rand)
	{
		...
		if (_gameStage < 0)
		{
			...
			{
				if (_list[i].quality == _quality)
				{
					_gameStage = _list[i].gameStage;
					break;
				}
			}
		}
		...
		{
			if (_list[num2].quality == _quality)
			{
				int gameStage = _list[num2].gameStage;
				if (gameStage <= _gameStage)
				{
					if (num < 0)
					{
						num = gameStage;
					}
					else if (gameStage < num)
					{
						break;
					}
					float chance = _list[num2].chance;
					if (chance >= 1f || chance > _rand.RandomFloat)
					{
						return _list[num2];
					}
				}
		...
		GSStat result = _list[0];
		result.quality = -1;
		return result;
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void CalculateTechType()
	{
		FastTags<TagGroup.Global> fastTags = (IsBlock() ? GetBlock().Tags : ItemTags);
		if (fastTags.Test_AnySet(T3Tag))
		{
			ItemTechType = ItemTechTypes.T3;
		}
		else if (fastTags.Test_AnySet(T2Tag))
		{
			ItemTechType = ItemTechTypes.T2;
		}
		else if (fastTags.Test_AnySet(T1Tag))
		{
			ItemTechType = ItemTechTypes.T1;
		}
		else if (fastTags.Test_AnySet(T0Tag))
		{
			ItemTechType = ItemTechTypes.T0;
		}
		else
		{
			ItemTechType = ItemTechTypes.None;
		}
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public ItemClass GetPreviousTechItem()
	{
		if (previousTechItems == null && previousTechTypeName != "")
		{
			string[] array = previousTechTypeName.Split(',');
			previousTechItems = new ItemClass[array.Length];
			for (int i = 0; i < array.Length; i++)
			{
				previousTechItems[i] = GetItemClass(array[i]);
			}
		}
		if (previousTechItems != null && previousTechItems.Length != 0)
		{
			if (previousTechItems.Length > 1)
			{
				return previousTechItems[GameManager.Instance.World.GetGameRandom().RandomRange(previousTechItems.Length)];
			}
			return previousTechItems[0];
		}
		return null;
	}

	public static ItemClass HandleSandboxTechType(ItemClass itemClass)
	{
		if (itemClass.ItemTechType > MaxTechType)
		{
			ItemClass previousTechItem;
			for (previousTechItem = itemClass.GetPreviousTechItem(); previousTechItem != null; previousTechItem = previousTechItem.GetPreviousTechItem())
			{
				if (previousTechItem == null)
				{
					return null;
				}
				if (previousTechItem.ItemTechType <= MaxTechType)
				{
					break;
				}
				itemClass = previousTechItem;
