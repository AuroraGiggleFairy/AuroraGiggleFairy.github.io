...
public class ChunkCluster : WorldChunkCache, IBlockAccess
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static readonly ConcurrentDictionary<long, StateHistory<ChunkState>> chunkRegenerationHistories = new ConcurrentDictionary<long, StateHistory<ChunkState>>();
	...
	public IEnumerator Init(EnumChunkProviderId _providerId, PathAbstractions.AbstractedLocation _worldLocation)
	{
		...
		case EnumChunkProviderId.Disc:
			ChunkProvider = new ChunkProviderDisc(this, Name, _worldLocation);
			break;
		case EnumChunkProviderId.NetworkClient:
			if (!IsFixedSize)
			{
				ChunkProvider = new ChunkProviderGenerateWorldFromRaw(Name, _worldLocation, _bClientMode: true, _bFixedWaterLevel: true);
			}
			else
			{
		...
		case EnumChunkProviderId.GenerateFromDtm:
			ChunkProvider = new ChunkProviderGenerateWorldFromImage(Name, _worldLocation);
			break;
		case EnumChunkProviderId.ChunkDataDriven:
			ChunkProvider = new ChunkProviderGenerateWorldFromRaw(Name, _worldLocation);
			break;
		case EnumChunkProviderId.FlatWorld:
			ChunkProvider = new ChunkProviderGenerateFlat(this, _worldLocation.FullPath);
			break;
		}
		yield return null;
	...
	public void Cleanup()
	{
		...
		chunkRegenerationHistories.Clear();
	}
	...
	public override bool AddChunkSync(Chunk _chunk, bool _bOmitCallbacks = false)
	{
		...
		UpdateRegenerationState(_chunk.Key, ChunkState.Loaded);
	...
	public void RemoveChunk(Chunk _chunk)
	{
		...
		UpdateRegenerationState(_chunk.Key, ChunkState.Removed);
	}
	...
	public void UnloadChunk(Chunk _chunk)
	{
		if (ChunkProvider != null)
		{
			...
			UpdateRegenerationState(_chunk.Key, ChunkState.Unloaded);
		}
	}
	...
	public void RegenerateChunk(Chunk _chunk, Chunk[] _neighbours)
	{
		...
		while (_chunk.NeedsRegeneration)
		{
			...
			UpdateRegenerationState(_chunk.Key, ChunkState.Starting);
			...
			UpdateRegenerationState(_chunk.Key, ChunkState.GeneratingMeshes);
			...
			UpdateRegenerationState(_chunk.Key, ChunkState.AddingMeshLayer);
			_chunk.AddMeshLayer(voxelMeshLayer);
			UpdateRegenerationState(_chunk.Key, ChunkState.Regenerated);
	...
	{
		IChunk chunkSync = GetChunkSync(World.toChunkXZ(_worldPos.x), World.toChunkXZ(_worldPos.z));
		if (chunkSync == null)
		{
	...
	public BlockValue GetBlock(int x, int y, int z)
	{
		if ((uint)y < 256u)
		{
			IChunk chunkFromWorldPos = GetChunkFromWorldPos(x, y, z);
			if (chunkFromWorldPos != null)
			{
	...
	{
		return SetBlock(_bvRef, _isChangeBV: true, _bv, _isChangeDensity: false, 0, _isNotify, _isUpdateLight);
	}
	...
	public BlockValue SetBlock(BlockValueRef _bvRef, bool _isChangeBV, BlockValue _bv, bool _isChangeDensity, sbyte _density, bool _isNotify, bool _isUpdateLight, bool _isForceDensity = false, bool _wasChild = false, int _changedByEntityId = -1)
	{
		...
		case BlockValueRefType.Block:
			return SetBlock(_bvRef.BlockPosition, _isChangeBV, _bv, _isChangeDensity, _density, _isNotify, _isUpdateLight, _isForceDensity, _wasChild, _changedByEntityId);
		case BlockValueRefType.Prop:
		{
			PropRef propReference = _bvRef.PropReference;
			BlockValue? blockValue = _bv;
			return SetProp(propReference, null, null, null, blockValue).blockValue;
		}
		default:
	...
	public BlockValue SetBlock(Vector3i _pos, bool _isChangeBV, BlockValue _bv, bool _isChangeDensity, sbyte _density, bool _isNotify, bool _isUpdateLight, bool _isForceDensity = false, bool _wasChild = false, int _changedByEntityId = -1)
	{
		...
		{
			if (!isInNotify && !world.IsRemote())
			{
				isInNotify = true;
				notifyBlocksOfNeighborChange(_pos, _bv, blockValue);
				isInNotify = false;
			}
			bool flag = !chunkSync.StopStabilityCalculation;
			if (flag && blockValue.Block.isMultiBlock)
			{
				...
				{
					Vector2i vector2i = World.toChunkXZ(_pos + multiBlockPos.Get(num7, blockValue.type, blockValue.rotation));
					...
					if (num8 != chunkSync.Key)
					{
						...
						if (!flag2)
						{
							Chunk chunkSync2 = GetChunkSync(num8);
			...
			if (GameManager.bPhysicsActive && flag && !blockValue.Equals(_bv))
			{
				...
				if (!isair && !block.blockMaterial.IsLiquid)
				{
					...
					if (block.isMultiBlock)
					{
						...
						{
							Vector3i pos = _pos + block.multiBlockPos.Get(num11, _bv.type, _bv.rotation);
				...
				else if (isair && !blockValue.Block.blockMaterial.IsLiquid)
				{
					...
					if (block2.isMultiBlock)
					{
						...
						{
							Vector3i vector3i = block2.multiBlockPos.Get(num12, blockValue.type, blockValue.rotation);
							if (vector3i.x != 0 || vector3i.y != 0 || vector3i.z != 0)
							{
				...
				if (MeshDescription.bDebugStability)
				{
					for (int num13 = num2 - 1; num13 <= num2 + 1; num13++)
					{
						...
						{
							Chunk chunkSync3 = GetChunkSync(num14, num13);
							if (chunkSync3 != null)
							{
	...
	{
		return GetChunkSync(chunkX, chunkZ)?.GetProp(chunkX, chunkZ, propId) ?? PropValue.AIR;
	}

	public PropValue GetProp(long chunkKey, int propId)
	{
		return GetChunkSync(chunkKey)?.GetProp(chunkKey, propId) ?? PropValue.AIR;
	}
	...
	{
		return SetProp(propRef.ChunkPos, propRef.PropId, position, rotation, scale, blockValue);
	}
	...
	{
		Chunk chunk = (Chunk)GetChunkSync(chunkPos);
		PropValue obj = (propId.HasValue ? chunk.GetProp(chunkPos, propId.Value) : PropValue.AIR);
		...
		if (obj != propValue)
		{
			...
			chunkPosNeedsRegeneration(chunk, vector3i.x, vector3i.y, vector3i.z, _bTerrainBlockChanged: false);
		}
		if (!propValue.IsAir)
		{
			return propValue;
	...
	public void SetBlockValue(BlockValueRef blockValueRef, BlockValue blockValue)
	{
		...
		{
		case BlockValueRefType.Block:
			SetBlockRaw(blockValueRef.BlockPosition, blockValue);
			break;
		case BlockValueRefType.Prop:
			SetProp(blockValueRef.PropReference, null, null, null, blockValue);
			break;
		default:
			throw new ArgumentOutOfRangeException();
		case BlockValueRefType.None:
	...
	public void ChunkPosNeedsRegeneration_DelayedStop()
	{
		...
		lock (delayedRegenChunks)
		{
			...
			{
				delayedRegenChunk.Key.NeedsRegenerationOrBits(delayedRegenChunk.Value);
			}
			delayedRegenChunks.Clear();
		}
	}
	...
	public void chunkRegenerateAt(Chunk _c, int _yPos)
	{
		...
		lock (delayedRegenChunks)
		{
			...
			delayedRegenChunks[_c] = value | (1 << (_yPos >> 4));
		}
	}
	...
	public void notifyBlockOfNeighborChange(Vector3i _myBlockPos, BlockValue _newBlockValue, BlockValue _oldBlockValue, Vector3i _blockPosThatChanged)
	{
		if (!world.IsRemote())
		{
			...
			if (!block.isair)
			{
				block.Block.OnNeighborBlockChange(world, _myBlockPos, block, _blockPosThatChanged, _newBlockValue, _oldBlockValue);
			}
		}
	}
			...
			for (int j = num3; j <= num6; j++)
			{
				...
				for (int k = num2; k <= num5; k++)
				{
					...
					if (block2.IsCheckCollideWithEntity)
					{
						...
						{
							block2.OnEntityCollidedWithBlock(world, vector3i, block, _entity);
						}
					}
				}
			}
	...
	public void SetTextureFull(BlockValueRef _bvRef, long _textureFull, int channel)
	{
		...
		case BlockValueRefType.None:
			break;
		case BlockValueRefType.Block:
			SetTextureFull(_bvRef.BlockPosition, _textureFull, channel);
			break;
		case BlockValueRefType.Prop:
			break;
		default:
	...
	public void SetTextureFullArray(BlockValueRef _bvRef, TextureFullArray _textureFull)
	{
		...
		case BlockValueRefType.None:
			break;
		case BlockValueRefType.Block:
			SetTextureFullArray(_bvRef.BlockPosition, _textureFull);
			break;
		case BlockValueRefType.Prop:
			break;
		default:
	...
	public long GetTextureFull(BlockValueRef _bvRef)
	{
		...
		{
			BlockValueRefType.None => 0L, 
			BlockValueRefType.Block => GetTextureFull(_bvRef.BlockPosition), 
			BlockValueRefType.Prop => 0L, 
	...
	public TextureFullArray GetTextureFullArray(BlockValueRef _bvRef)
	{
		return _bvRef.Type switch
		{
			BlockValueRefType.None => TextureFullArray.Default, 
			BlockValueRefType.Block => GetTextureFullArray(_bvRef.BlockPosition), 
			BlockValueRefType.Prop => TextureFullArray.Default, 
			_ => throw new ArgumentOutOfRangeException(), 
		};
	}
	...
	{
		Log.Out(string.Format("[FELLTHROUGHWORLD] ChunkCluster Current Mesh Regeneration State - Chunk: {0}, VML Index: {1}, Regeneration State: {2}", currentChunkKey, currentChunkVMLIndex, (chunkRegenerationHistories.TryGetValue(currentChunkKey, out var value) && value.Length > 0) ? ((object)value.Current) : "N/A"));
		List<(double, long, StateHistory<ChunkState>)> list = new List<(double, long, StateHistory<ChunkState>)>(chunkRegenerationHistories.Count);
		foreach (var (num2, item) in chunkRegenerationHistories)
		{
			list.Add((ChunkManager.SecondsSinceChunkSelectedForGeneration(num2), num2, item));
		}
		list.Sort([PublicizedFrom(EAccessModifier.Internal)] ((double seconds, long chunkKey, StateHistory<ChunkState> history) a, (double seconds, long chunkKey, StateHistory<ChunkState> history) b) => a.seconds.CompareTo(b.seconds));
		int num3 = 0;
		foreach (var (num4, num5, stateHistory2) in list)
		{
			if (num3 < 16 || num5 == importantChunkKey || num5 == currentChunkKey)
			{
				lock (stateHistory2)
				{
					Log.Out(string.Format("[FELLTHROUGHWORLD] Chunk {0} ({1}, {2}), Last Regeneration Requested: {3} State History: {4}", num5, WorldChunkCache.extractX(num5), WorldChunkCache.extractZ(num5), (num4 < 0.0) ? "Never" : $"{num4:F3} s", stateHistory2));
				}
				num3++;
			}
		}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public void UpdateRegenerationState(long chunkKey, ChunkState state)
	{
		if (GameManager.IsDedicatedServer)
		{
			return;
		}
		StateHistory<ChunkState> orAdd = chunkRegenerationHistories.GetOrAdd(chunkKey, [PublicizedFrom(EAccessModifier.Internal)] (long _) => new StateHistory<ChunkState>(16, preventConsecutiveDuplicates: true));
		lock (orAdd)
		{
			orAdd.Add(state);
		}
	...
	{
		stabilityCalcMainThread.ClearChunkStabilityQueues(chunks);
		world.ClearFallingBlocksForChunks(chunks);
	}
}
