...
public class Explosion
{
	...
	public void AttackBlocks(int _entityThatCausedExplosion, ItemValue _itemValueExplosionSource)
	{
		...
		if (chunkCache.GetBlock(blockPos).Block.shape.IsTerrain() && !chunkCache.GetBlock(blockPos + Vector3i.up).Block.shape.IsTerrain())
		{
			blockPos.y++;
		}
			...
			for (int j = -num2; j <= num2; j++)
			{
				for (int k = -num2; k <= num2; k++)
				{
					...
					for (; num3 >= -0.1f; num3 -= 0.51f, vector2 += vector3)
					{
						...
						{
							value = new BlockChangeInfo(vector3i, block);
						...
						if (value.blockValue.isair || (World.SandboxUseTraderArea == TraderAreaStates.Default && world.IsWithinTraderArea(vector3i)) || world.InBoundsForPlayersPercent(vector3i.ToVector3CenterXZ()) < 0.5f)
						{
							continue;
						}
						...
						float landProtectionHardnessModifier = world.GetLandProtectionHardnessModifier(vector3i, entityAlive, playerDataFromEntityID);
						int num7 = 0;
						if (hardness > 0f)
						{
							...
							{
								num8 *= explosionData.damageMultiplier.Get(block3.blockMaterial.DamageCategory);
							}
						...
						if (num7 + value.blockValue.damage >= block2.MaxDamage)
						{
							num7 = num7 + value.blockValue.damage - block2.MaxDamage;
							value.bChangeDamage = false;
							Block.DestroyedResult destroyedResult = block3.OnBlockDestroyedByExplosion(world, vector3i, value.blockValue, entityId);
							if (!block3.DowngradeBlock.isair && destroyedResult == Block.DestroyedResult.Downgrade)
							{
								do
								{
									...
									value.blockValue.damage = num7;
									num7 -= value.blockValue.Block.MaxDamage;
								}
								while (num7 > 0 && !value.blockValue.Block.DowngradeBlock.isair);
								if (num7 >= 0)
								{
						...
						{
							value.blockValue.damage = num7 + value.blockValue.damage;
						}
						if (!value.blockValue.isair)
						{
							break;
	...
	public void AttackEntites(int _entityThatCausedExplosion, ItemValue _itemValueExplosionSource, EnumDamageTypes damageType)
	{
		...
		Collider[] array = Physics.OverlapSphere(worldPos - Origin.position, value2, -538480653);
		bool flag = false;
		...
		foreach (KeyValuePair<int, DamageRecord> hitEntity in hitEntities)
		{
			...
			{
				entityPlayer.AddKillXP(entity, _itemValueExplosionSource);
			}
