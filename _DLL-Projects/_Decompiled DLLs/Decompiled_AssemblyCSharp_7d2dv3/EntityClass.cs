...
public class EntityClass
{
	...
	public static readonly int fallingBlocksClass = FromString("fallingBlocks");
	...
	public EntityTierTypes EntityTier;
	...
	public EntityClass Init()
	{
		...
		{
			text = Properties.GetString(PropCensor);
		}
		...
		if (Properties.Values.TryGetValue(PropPrefabCombined, out var value) && bool.TryParse(value, out result) && result)
		{
		...
		if (Properties.Values.TryGetValue(PropMesh, out var value2) && value2.Length > 0)
		{
			if (censorMode != 0 && (censorType == 1 || censorType == 3) && (bool)GameManager.Instance && GameManager.Instance.IsGoreCensored())
			{
				value2 = value2.Replace(".", "_CGore.");
			}
			if (DataLoader.IsInResources(value2))
			{
				value2 = "Entities/" + value2;
			}
			meshPath = value2;
		}
		...
		Properties.ParseString(PropPreviousTierZombie, ref PreviousTierZombieName);
		if (Properties.Array.TryGetValue(PropTokenManager, out var value3))
		{
			TokenManagerConfig = new Dictionary<AITokenType, AITokenConfig>();
			for (int m = 0; m < value3.Count; m++)
			{
				if (value3[m].TryGetValue("type", out var value4) && Enum.TryParse<AITokenType>(value4, ignoreCase: true, out var result2))
				{
					AITokenConfig value5 = default(AITokenConfig);
					if (value3[m].TryGetValue("max", out var value6))
					{
						int.TryParse(value6, out value5.MaxClaims);
					}
					TokenManagerConfig[result2] = value5;
				}
			}
		}
		CalculateEntityTier();
	...
	{
		foreach (KeyValuePair<string, string> value in _other.Properties.Values)
		{
			if (_exclude == null || !_exclude.Contains(value.Key))
			{
				Properties.Values[value.Key] = _other.Properties.Values[value.Key];
			}
		}
		foreach (KeyValuePair<string, string> item in _other.Properties.Params1)
		{
			if (_exclude == null || !_exclude.Contains(item.Key))
			{
				Properties.Params1[item.Key] = item.Value;
			}
		}
		foreach (KeyValuePair<string, string> item2 in _other.Properties.Params2)
		{
			if (_exclude == null || !_exclude.Contains(item2.Key))
			{
				Properties.Params2[item2.Key] = item2.Value;
			}
		}
		foreach (KeyValuePair<string, string> datum in _other.Properties.Data)
		{
			if (_exclude == null || !_exclude.Contains(datum.Key))
			{
				Properties.Data[datum.Key] = datum.Value;
			}
		}
		foreach (KeyValuePair<string, DynamicProperties> @class in _other.Properties.Classes)
		{
			if (_exclude == null || !_exclude.Contains(@class.Key))
			{
				DynamicProperties dynamicProperties = new DynamicProperties();
				dynamicProperties.CopyFrom(@class.Value);
				Properties.Classes[@class.Key] = dynamicProperties;
			}
		}
	}
	...
	{
		if (Tags.Test_AnySet(eliteTag))
		{
			EntityTier = EntityTierTypes.Elite;
		}
		else if (Tags.Test_AnySet(radiatedTag))
		{
			EntityTier = EntityTierTypes.Radiated;
		}
		else if (Tags.Test_AnySet(feralTag))
		{
			EntityTier = EntityTierTypes.Feral;
		}
		else if (Tags.Test_AnySet(specialTag))
		{
			EntityTier = EntityTierTypes.Special;
		}
		else if (Tags.Test_AnySet(strongTag))
		{
			EntityTier = EntityTierTypes.Strong;
		}
		else
		{
			EntityTier = EntityTierTypes.Normal;
		}
	}

	public EntityClass GetPreviousTierEntity()
	{
		if (previousTierEntities == null && PreviousTierZombieName != "")
		{
			string[] array = PreviousTierZombieName.Split(',');
			previousTierEntities = new EntityClass[array.Length];
			for (int i = 0; i < array.Length; i++)
			{
				previousTierEntities[i] = GetEntityClass(GetId(array[i]));
			}
		}
		if (previousTierEntities != null && previousTierEntities.Length != 0)
		{
			if (previousTierEntities.Length > 1)
			{
				return previousTierEntities[GameManager.Instance.World.GetGameRandom().RandomRange(previousTierEntities.Length)];
			}
			return previousTierEntities[0];
		}
		return null;
	}
}
