...
public class ItemActionDynamicMelee : ItemActionDynamic
{
	...
	{
		WorldRayHitInfo executeActionTarget = GetExecuteActionTarget(_actionData);
		ItemValue itemValue = _actionData.invData.itemStack.itemValue;
		bool flag = false;
		myAttackHitInfo = new ItemActionAttack.AttackHitInfo
		{
			WeaponTypeTag = ItemActionAttack.MeleeTag
		};
		float damageEntity = GetDamageEntity(_actionData.invData.itemValue, _actionData.invData.holdingEntity, _actionData.indexInEntityOfAction);
		if (_actionData.invData.holdingEntity is EntityPlayer)
		{
			damageEntity *= ItemActionAttack.MeleeDamagePercent;
			flag = ItemActionAttack.MeleeDamagePercent == 0f;
		}
		else
		{
			damageEntity *= ItemActionAttack.EntityPlayerDamagePercent;
			flag = ItemActionAttack.EntityPlayerDamagePercent == 0f;
		}
		int entityId = _actionData.invData.holdingEntity.entityId;
		EnumDamageTypes damageType = ((DamageType == EnumDamageTypes.None) ? EnumDamageTypes.Bashing : DamageType);
		float damageBlock = GetDamageBlock(_actionData.invData.itemValue, ItemActionAttack.GetBlockHit(_actionData.invData.world, executeActionTarget), _actionData.invData.holdingEntity, _actionData.indexInEntityOfAction);
		float entityDamage = damageEntity;
		float dismemberChance = ItemAction.GetDismemberChance(_actionData, executeActionTarget);
		string id = item.MadeOfMaterial.id;
		DamageMultiplier damageMultiplier = new DamageMultiplier();
		List<string> buffActions = new List<string>();
		ItemActionAttack.AttackHitInfo attackDetails = myAttackHitInfo;
		...
		ItemActionAttack.Hit(executeActionTarget, entityId, damageType, damageBlock, entityDamage, 1f, 1f, 0f, dismemberChance, id, damageMultiplier, buffActions, attackDetails, 1, actionExp, actionExpBonusMultiplier, null, toolBonuses, ItemActionAttack.EnumAttackMode.Simulate, null, -1, itemValue, _isGrazingHit: false, forceNoDamage);
	...
	public bool canStartAttack(ItemActionDynamicMeleeData _actionData)
	{
		...
		if (_actionData.invData.holdingEntity is EntityPlayerLocal { bFirstPersonView: false } entityPlayerLocal)
		{
			...
			if (entityPlayerLocal.TPCameraCheckResult == eTPCameraCheckResult.AngleCheckFailed)
			{
				return false;
			}
		}
		...
		{
			_actionData.StaminaUsage = EffectManager.GetValue(PassiveEffects.StaminaLoss, _actionData.invData.itemValue, 2f, holdingEntity, null, _actionData.ActionTags) * ItemActionAttack.StaminaUsageMultiplier;
			if (holdingEntity.Stats.Stamina.Value < _actionData.StaminaUsage)
			{
				if (_actionData.HasReleased)
				{
					...
					GameManager.ShowTooltip(holdingEntity as EntityPlayerLocal, "ttOutOfStamina", _showImmediately: true);
				}
				return false;
			}
		}
