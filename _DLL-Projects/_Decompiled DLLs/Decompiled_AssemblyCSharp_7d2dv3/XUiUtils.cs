...
public static class XUiUtils
{
	...
	public static bool HotkeysAllowedFor(XUiView _view, bool _ignoreViewLock = false)
	{
		...
		if (!windowManager.IsKeyShortcutsAllowed())
		{
			return false;
		}
		if (!_ignoreViewLock && !cursorController.IsWithinNavigationLockView(_view))
		{
			return false;
		}
		if (XUiC_FullScreenCollider.IsBlocked(_view))
		{
			return false;
		}
		return true;
	...
	public static void ApplyFillPaddedSprite(UISprite _uiSprite, string _origSpriteName)
	{
		...
		if (atlas != null)
		{
			modifyAtlas(_uiSprite, atlas, _origSpriteName);
		}
	}
	...
	public static void modifyAtlas(UISprite _uiSprite, INGUIAtlas _atlas, string _origSpriteName)
	{
		...
		{
			modifyAtlas(_uiSprite, _atlas.replacement, _origSpriteName);
			return;
		}
		string filledSpriteName = _origSpriteName + "##filled";
		List<UISpriteData> spriteList = _atlas.spriteList;
		if (spriteList.FindIndex([PublicizedFrom(EAccessModifier.Internal)] (UISpriteData _data) => _data.name == filledSpriteName) >= 0)
		{
			_uiSprite.spriteName = filledSpriteName;
			return;
		}
		int num = spriteList.FindIndex([PublicizedFrom(EAccessModifier.Internal)] (UISpriteData _data) => _data.name == _origSpriteName);
		if (num >= 0)
		{
			...
			setAtlasSpriteList(_atlas, spriteList);
			_uiSprite.spriteName = filledSpriteName;
		}
	}
	...
	{
		if (_atlas is UIAtlas uIAtlas)
		{
			uIAtlas.spriteList = _list;
			uIAtlas.MarkSpriteListAsChanged();
			return;
		}
		if (_atlas is NGUIAtlas nGUIAtlas)
		{
