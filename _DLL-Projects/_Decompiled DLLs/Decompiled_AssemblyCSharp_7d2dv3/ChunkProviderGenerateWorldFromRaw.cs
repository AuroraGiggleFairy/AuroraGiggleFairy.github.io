...
public class ChunkProviderGenerateWorldFromRaw : ChunkProviderGenerateWorld
{
	...
	public delegate void RoadSmooth_00005653_0024PostfixBurstDelegate(in NativeArray<Color32> _colors, ref NativeArray<Color32> _colorsOut, int width, int height);

	[PublicizedFrom(EAccessModifier.Internal)]
	public static class RoadSmooth_00005653_0024BurstDirectCall
	{
		[PublicizedFrom(EAccessModifier.Private)]
		...
		public unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
		{
			if (Pointer == (IntPtr)0)
			{
				Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(RoadSmooth_00005653_0024PostfixBurstDelegate).TypeHandle);
			}
			P_0 = Pointer;
		}
		...
		[PublicizedFrom(EAccessModifier.Private)]
		static RoadSmooth_00005653_0024BurstDirectCall()
		{
			Constructor();
		}

		public unsafe static void Invoke(in NativeArray<Color32> _colors, ref NativeArray<Color32> _colorsOut, int width, int height)
		{
			if (BurstCompiler.IsEnabled)
			{
				...
				{
					((delegate* unmanaged[Cdecl]<ref NativeArray<Color32>, ref NativeArray<Color32>, int, int, void>)functionPointer)(ref _colors, ref _colorsOut, width, height);
					return;
				}
			}
			RoadSmooth_0024BurstManaged(in _colors, ref _colorsOut, width, height);
		}
	}
	...
	public ChunkProviderGenerateWorldFromRaw(string _levelName, PathAbstractions.AbstractedLocation _worldLocation, bool _bClientMode = false, bool _bFixedWaterLevel = false)
		: base(_levelName, _worldLocation, _bClientMode)
	{
	...
	public override IEnumerator Init(World _world)
	{
		...
		world = _world;
		string worldPath = worldLocation.FullPath;
		base.WorldInfo = GameUtils.WorldInfo.LoadWorldInfo(worldLocation);
		...
		if (!SdFile.Exists(dtmFilename + ".raw"))
		{
			...
			else
			{
				...
				{
					throw new FileNotFoundException($"No height data found for world '{worldLocation}'");
				}
				...
				HeightMapUtils.SaveHeightMapRAW(dtmFilename + ".raw", heightMapWidth, heightMapHeight, heightDataFromImageFile);
		...
		m_BiomeProvider = new WorldBiomeProviderFromImage(levelName, worldLocation.FullPath, world.Biomes, heightMapWidth * heightMapScale);
		...
		{
			worldDecoratorPoiFromImage = new WorldDecoratorPOIFromImage(levelName, worldLocation.FullPath, GetDynamicPrefabDecorator(), heightMapWidth, heightMapHeight, splat3Tex, _splat4Tex: splat4Tex, _bChangeWaterDensity: false, _worldScale: heightMapScale, _heightMap: heightMap);
		...
		((TerrainFromRaw)m_TerrainGenerator).Init(heightMap, m_BiomeProvider, world.Seed);
		yield return null;
		...
		if (!bClientMode)
		{
			...
			eventPrefabs = new EventPrefabs(_world, prefabDecorator, m_RegionFileManager);
		}
		...
		yield return null;
		Texture2D tex = splats[0];
		if (!(DeviceFlag.XBoxSeriesS | DeviceFlag.XBoxSeriesX | DeviceFlag.PS5).IsCurrent())
		{
			Texture2D tex16 = new Texture2D(tex.width, tex.height, TextureFormat.ARGB4444, mipChain: false);
			yield return RoadSmooth(tex, tex16);
			splats[0] = tex16;
		...
		yield return null;
		splats[1].Compress(highQuality: true);
	...
	public static IEnumerator RoadSmooth(Texture2D _tex, Texture2D _smoothedTex)
	{
		...
		for (int i = 0; i < num; i++)
		{
			colorsNA[i] = ARGB32ToColor(pixelData[i]);
		}
		...
		while (pass > 0)
		{
			RoadSmooth(in colorsNA, ref smoothColorsNA, width, height);
			if (pass > 1)
			{
	...
	{
		RoadSmooth_00005653_0024BurstDirectCall.Invoke(in _colors, ref _colorsOut, width, height);
	}
	...
	public IEnumerator processFiles(string _worldPath, ProcessFilesCallback _callback)
	{
		...
		yield return GetDynamicPrefabDecorator().CopyWorldPrefabHeightsIntoHeightMap(heightMapWidth, heightMapHeight, heightData, heightMapScale, topTexMap);
		...
		string fullPath = worldLocation.FullPath;
		...
		WorldDecoratorPOIFromImage worldDecoratorPoiFromImage = new WorldDecoratorPOIFromImage(text3, fullPath, dynamicPrefabDecorator, worldX, worldZ, splat3Tex2, _bChangeWaterDensity: false, heightMapScale, heightMap, splat4Tex2);
		m_Decorators.Add(worldDecoratorPoiFromImage);
		yield return worldDecoratorPoiFromImage.InitData();
	...
	public Texture2D generateHalfResTexture(Texture2D _tex)
	{
		...
		Texture2D texture2D = new Texture2D(_tex.width >> 1, _tex.height >> 1, TextureFormat.ARGB32, mipChain: false);
	...
	[BurstCompile(CompileSynchronously = true)]
	[PublicizedFrom(EAccessModifier.Internal)]
	public static void RoadSmooth_0024BurstManaged(in NativeArray<Color32> _colors, ref NativeArray<Color32> _colorsOut, int width, int height)
	{
		int num = width - 1;
		int num2 = 0;
		for (int i = 0; i < height; i++)
		{
			...
			{
				Color32 value = _colors[num2];
				if (j > 0 && j < num && i > 0 && i < num && (value.b == 0 || value.g > 0))
				{
					float num3 = (int)value.g;
					num3 += (float)(int)_colors[num2 - width].g * 1f;
					num3 += (float)(int)_colors[num2 - 1].g * 1f;
					num3 += (float)(int)_colors[num2 + 1].g * 1f;
					num3 += (float)(int)_colors[num2 + width].g * 1f;
					num3 *= 0.2f;
					if (value.r == 0)
					{
						...
						value.g = (byte)Utils.FastMax((int)value.g, num3);
					}
					else
					{
						value.g = (byte)(num3 * 0.5f);
					}
				}
				if (value.r + value.g > 0)
				{
					value.a = 0;
				}
				_colorsOut[num2++] = value;
			}
		}
	}
}
