...
public class MiniTurretFireController : MonoBehaviour
{
	...
	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public bool burstFireStarted;

	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public bool burstFireFinished;
	...
	public void Init(DynamicProperties _properties, EntityTurret _entity)
	{
		...
		GameManager.Instance.World.GetEntity(entityTurret.belongsPlayerId);
		...
		{
			fireRateMax = StringParsers.ParseFloat(_properties.Values["BurstFireRate"]);
		}
		...
		damageMultiplier = new DamageMultiplier(_properties);
	...
	public virtual int GetBurstRoundCount(EntityAlive owner)
	{
		return (int)EffectManager.GetValue(PassiveEffects.BurstRoundCount, entityTurret.OriginalItemValue, 0f, owner, null, default(FastTags<TagGroup.Global>), calcEquipment: true, calcHoldingItem: false);
	}
	...
	public void Update()
	{
		...
		if (hasTarget)
		{
			...
			if (trackTarget(currentEntityTarget, ref _yaw, ref _pitch, out var _))
			{
				...
				EntityAlive entityAlive = GameManager.Instance.World.GetEntity(entityTurret.belongsPlayerId) as EntityAlive;
				burstRoundCountMax = GetBurstRoundCount(entityAlive);
				if (burstRoundCountMax == 0 || !burstFireStarted || (burstFireStarted && !burstFireFinished))
				{
					FastTags<TagGroup.Global> tags = entityTurret.OriginalItemValue.ItemClass.ItemTags | entityTurret.EntityClass.Tags;
					fireRateMax = 60f / (EffectManager.GetValue(PassiveEffects.RoundsPerMinute, entityTurret.OriginalItemValue, fireRateMax, entityAlive, null, tags, calcEquipment: true, calcHoldingItem: false) + 1E-05f);
					if (fireRateTimer >= fireRateMax)
					{
						Fire();
						fireRateTimer = 0f;
						if (burstRoundCountMax > 0)
						{
							burstFireStarted = true;
							burstRoundCount++;
							if (burstRoundCount >= burstRoundCountMax)
							{
								burstFireFinished = true;
							}
						}
					}
				}
	...
	public void findTarget()
	{
		...
		for (int i = 0; i < entitiesInBounds.Count; i++)
		{
			...
			if (!Voxel.Raycast(ray: new Ray(position + Origin.position - normalized * 0.05f, normalized), _world: GameManager.Instance.World, distance: maxDistance + 0.05f, _layerMask: -538750989, _hitMask: 8, _sphereRadius: 0.05f) || !Voxel.voxelRayHitInfo.tag.StartsWith("E_"))
			{
				continue;
			}
		...
		if (_target is EntityPlayer)
		{
			...
			if (!flag && persistentPlayerList.EntityToPlayerMap.ContainsKey(entityTurret.belongsPlayerId))
			{
				...
				if (persistentPlayerData != null && persistentPlayerList.EntityToPlayerMap.ContainsKey(_target.entityId))
				{
					...
					if (persistentPlayerData.IsAlly(other))
					{
						flag2 = true;
					}
				}
	...
	public bool canHitEntity(Entity _targetEntity, ref float _yaw, ref float _pitch, out Vector3 targetPos)
	{
		...
		if (Voxel.Raycast(GameManager.Instance.World, ray, maxDistance, -538750989, 8, 0f) && Voxel.voxelRayHitInfo.tag.StartsWith("E_"))
		{
			Transform hitRootTransform = GameUtils.GetHitRootTransform(Voxel.voxelRayHitInfo.tag, Voxel.voxelRayHitInfo.transform);
			if (hitRootTransform == null)
			{
	...
	public virtual void Fire()
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			{
				if (!Voxel.Raycast(GameManager.Instance.World, ray, maxDistance, -538750989, 8, 0f))
				{
					continue;
				}
		...
		entityTurret.OriginalItemValue.UseTimes += EffectManager.GetValue(PassiveEffects.DegradationPerUse, entityTurret.OriginalItemValue, 1f, entity, null, entityTurret.OriginalItemValue.ItemClass.ItemTags, calcEquipment: true, calcHoldingItem: false) * ItemAction.ItemDegradationModifier;
	}
