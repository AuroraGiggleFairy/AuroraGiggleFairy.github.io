...
public class XUiC_PowerSourceStats : XUiController
{
	...
	public void BtnRefuel_OnPress(XUiController _sender, int _mouseButton)
	{
		...
		if (num4 == 0)
		{
			...
			GameManager.ShowTooltip(xui.playerUI.localPlayer.entityPlayerLocal, Localization.Get("ttNotEnoughFuel"));
			return;
		}
		ItemStack itemStack = new ItemStack(itemValue, num4);
		xui.CollectedItemList.RemoveItemStack(itemStack);
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
	...
	public void btnOn_OnPress(XUiController _sender, int _mouseButton)
	{
		...
		{
			Manager.PlayInsidePlayerHead("ui_denied");
			GameManager.ShowTooltip(xui.playerUI.localPlayer.entityPlayerLocal, Localization.Get("ttRequiresOneComponent"));
			return;
		}
		...
		{
			Manager.PlayInsidePlayerHead("ui_denied");
			GameManager.ShowTooltip(xui.playerUI.localPlayer.entityPlayerLocal, Localization.Get("ttGeneratorRequiresFuel"));
			return;
		}
		if (!flag)
		{
		...
		GameManager.Instance.World.SetBlockRPC(TileEntity.ToWorldPos(), block);
		RefreshIsOn(flag);
		Owner.SetOn(flag);
	}
	...
	public override void Update(float _dt)
	{
		if ((!(GameManager.Instance == null) || GameManager.Instance.World != null) && tileEntity != null)
		{
			...
			if (xui.playerUI.playerInput.GUIActions.WindowPagingRight.WasPressed && PlatformManager.NativePlatform.Input.CurrentInputStyle != PlayerInputManager.InputStyle.Keyboard)
			{
	...
	public override void OnOpen()
	{
		...
		RefreshBindings();
		tileEntity.SetModified();
		xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.RightBumper, "igcoWorkstationTurnOnOff", XUiC_GamepadCalloutWindow.CalloutType.MenuShortcuts);
