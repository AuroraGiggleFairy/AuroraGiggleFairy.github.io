...
public class ChunkProviderDisc : ChunkProviderAbstract
{
	...
	public override IEnumerator Init(World _world)
	{
		...
		string text = worldLocation.FullPath + "/Region";
		m_RegionFileManager = new RegionFileManager(text, (!_world.IsEditor()) ? GameIO.GetSaveGameRegionDir() : text, 0, _bSaveOnChunkDrop: false);
		cc.IsFixedSize = true;
		...
		if (GameUtils.IsPlaytesting())
		{
			...
			prefabDecorator.AddWorldPrefab(pi, _isPOI: true);
			prefabDecorator.CopyAllPrefabsIntoWorld(world);
			int num5 = -prefab.size.z / 2 - 11;
			world.SetBlock(new Vector3i(2, world.GetHeight(0, num5) + 1, num5), Block.GetBlockValue("cntQuestTestLoot"), bNotify: false, updateLight: false);
			...
			foreach (Chunk item in world.ChunkCache.GetChunkArrayCopySync())
			{
				item.ResetStability();
				item.ResetLights(0);
	...
	public override void SaveAll()
	{
		if (world.IsEditor())
		{
			PathAbstractions.AbstractedLocation location = worldLocation;
	...
	{
		ThreadManager.RunCoroutineSync(prefabDecorator.Load(worldLocation.FullPath));
