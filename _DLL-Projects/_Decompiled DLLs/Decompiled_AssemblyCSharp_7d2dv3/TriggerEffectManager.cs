...
public class TriggerEffectManager : IDisposable
{
	...
	public TriggerEffectManager()
	{
		...
		Init();
		InputManager.OnDeviceAttached += OnControllerConnected;
	}
	...
	public void OnGamePrefChanged(EnumGamePrefs _pref)
	{
		switch (_pref)
		{
		case EnumGamePrefs.OptionsControllerTriggerEffects:
			SetEnabled(GamePrefs.GetBool(_pref));
			break;
		case EnumGamePrefs.OptionsControllerVibrationStrength:
	...
	{
		StopGamepadVibration(clearSources: true);
		LibShutdown();
		TriggerEffectDualsense.ResetControllerIdentification();
		TriggerEffectDualsensePC.ResetControllerIdentification();
		for (int i = 1; i < 5; i++)
	...
	public void StopGamepadVibration(bool clearSources = false)
	{
		if (clearSources)
		{
			AudioGamepadRumbleSource[] array = vibrationAudioSources;
	...
	public static ControllerTriggerEffect GetTriggerEffect((string, string) triggerEffectNames)
	{
		...
		{
			Log.Warning("Failed to find trigger effect DS: " + triggerEffectNames.Item1);
		}
		...
		{
			Log.Warning("Failed to find trigger effect XB: " + triggerEffectNames.Item2);
		}
		return result;
	}
	...
	public static ControllerTriggerEffect GetTriggerEffect(string dualsenseTrigger, string impulseTrigger)
	{
		...
		{
			Log.Warning("Failed to find trigger effect DS: " + dualsenseTrigger);
		}
		...
		{
			Log.Warning("Failed to find trigger effect XB: " + impulseTrigger);
		}
		return result;
	}
	...
	public static void UpdateDualSenseLightFromWeather(WeatherManager.BiomeWeather weather)
	{
		if (!(lightbarGradients == null) && weather != null)
		{
			...
			if (!flag && SkyManager.isAllTimeNight)
			{
				flag = true;
			}
			if (flag && SkyManager.isAllTimeDay)
			{
				flag = false;
			}
			float time;
			if (flag)
			{
				...
				time = (SkyManager.isAllTimeNight ? 0.5f : ((!(num >= num3) || !(num < 24000f)) ? (num4 / num5 + num / (num2 + num4)) : ((num - num3) / num5)));
			}
			else if (SkyManager.isAllTimeDay)
			{
				time = 0.5f;
			}
			else
