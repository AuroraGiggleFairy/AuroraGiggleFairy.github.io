...
public class ConsoleCmdSmoothWorldAll : ConsoleCmdAbstract
{
	...
	public override void Execute(List<string> _params, CommandSenderInfo _senderInfo)
	{
		...
		if (!HeightMapUtils.TryGetHeightDataFromResources(text, out var heightData))
		{
			heightData = HeightMapUtils.GetHeightDataFromWorld(location.FullPath);
		}
		...
		heightData = HeightMapUtils.SmoothTerrain(_result, heightData);
		...
		writeHeightDataBinary(location, heightData);
		if (flag)
		{
			...
			for (int i = 0; i < length2; i++)
			{
				...
				{
					array[j + i * length] = heightData[j, i];
				}
			}
			HeightMapUtils.SaveHeightMapRAW(location.FullPath + "/dtm.raw", length, length2, array);
		}
		else
		{
			writeRegionFiles(location.FullPath, heightData);
		}
	...
	public void writeRegionFiles(string worldPath, float[,] heightData)
	{
		...
		RegionFileManager regionFileManager = new RegionFileManager(text, text, 0, _bSaveOnChunkDrop: true);
		for (int i = -length / 2; i < length / 2; i += 16)
		{
			...
			if (i % 64 == 0)
			{
				...
				regionFileManager = new RegionFileManager(text, text, 0, _bSaveOnChunkDrop: true);
			}
		}
		regionFileManager.MakePersistent(null, _bSaveEvenIfUnchanged: true);
		regionFileManager.WaitSaveDone();
