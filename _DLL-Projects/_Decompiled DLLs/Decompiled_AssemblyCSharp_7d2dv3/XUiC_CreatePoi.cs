...
public class XUiC_CreatePoi : XUiController
{
	...
	public override void Init()
	{
		...
		if (GetChildById("folders") is XUiC_PrefabFolderList xUiC_PrefabFolderList)
		{
			...
			folderList.SelectionChanged += [PublicizedFrom(EAccessModifier.Private)] (XUiC_List<XUiC_PrefabFolderList.PrefabFolderEntry> _, XUiC_PrefabFolderList.PrefabFolderEntry _, XUiC_PrefabFolderList.PrefabFolderEntry _) =>
			{
				validateName();
			};
	...
	{
		bool flag = name != null && name.Length > 0 && !name.Contains(" ") && GameUtils.ValidateGameName(name) && !name.EndsWith("_signs", StringComparison.OrdinalIgnoreCase);
		...
		if (flag)
		{
			...
			List<PathAbstractions.AbstractedLocation> availablePathsList = PathAbstractions.PrefabsSearchPaths.GetAvailablePathsList(nameMatch, _ignoreDuplicateNames: false, SelectedLocation);
		...
		validFolder = !string.IsNullOrEmpty(folderList.SelectedEntryData?.AbsolutePath);
		IsDirty = true;
	}
	...
	public void BtnOk_OnPressed(XUiController _sender, int _mouseButton)
	{
		...
		block.OnBlockPlaceBefore(GameManager.Instance.World, ref _bpResult, xui.playerUI.entityPlayer, GameManager.Instance.World.GetGameRandom());
		blockValue = _bpResult.blockValue;
		blockValue.rotation = blockValue.rotation;
		BlockTools.CubeRPC(GameManager.Instance, vector3i, pos, blockValue, blockValue.Block.shape.IsTerrain() ? MarchingCubes.DensityTerrain : MarchingCubes.DensityAir, 0, TextureFullArray.Default);
		PrefabEditModeManager.Instance.SetGroundLevel(-SelectedSize.y);
		string subFolder = folderList.SelectedEntryData?.RelativePath;
		...
		{
			GameManager.ShowTooltip(xui.playerUI.entityPlayer, string.Format(Localization.Get("xuiPrefabsPrefabSaved"), PrefabEditModeManager.Instance.LoadedPrefab.Name));
		}
		else
		{
			GameManager.ShowTooltip(xui.playerUI.entityPlayer, Localization.Get("xuiPrefabsPrefabSavingError"));
		}
		xui.playerUI.windowManager.Open(XUiC_InGameMenuWindow.ID, _bModal: true);
	}
