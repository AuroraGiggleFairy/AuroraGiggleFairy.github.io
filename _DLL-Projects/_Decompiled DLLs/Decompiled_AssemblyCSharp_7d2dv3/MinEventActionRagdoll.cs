...
public class MinEventActionRagdoll : MinEventActionTargetedBase
{
	...
	{
		DamageResponse _dmResponse = DamageResponse.New(_fatal: false);
		_dmResponse.StunDuration = duration;
		_dmResponse.Strength = (int)force;
		if (cvarRef && targets.Count > 0)
		{
			_dmResponse.StunDuration = targets[0].Buffs.GetCustomVar(refCvarName);
		}
		...
		for (int i = 0; i < targets.Count; i++)
		{
			...
			EModelBase.RagdollMode mode = EModelBase.RagdollMode.Default;
			Vector3 vector2 = entityAlive.position - vector;
			if (scaleY == 0f)
			{
				vector2.y = 0f;
				_dmResponse.Source = new DamageSource(EnumDamageSource.External, EnumDamageTypes.Bashing, vector2.normalized);
			}
			else
			{
				mode = EModelBase.RagdollMode.FullForce;
				...
				_dmResponse.Source = new DamageSource(EnumDamageSource.External, EnumDamageTypes.Falling, vector2.normalized * num);
			}
			entityAlive.DoRagdoll(in _dmResponse, mode);
		}
	}
