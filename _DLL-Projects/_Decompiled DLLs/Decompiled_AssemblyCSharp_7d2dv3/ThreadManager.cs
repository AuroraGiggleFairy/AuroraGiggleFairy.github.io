...
public static class ThreadManager
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static readonly ManualTaskScheduler MainThreadScheduler;
	...
	public static TaskFactory MainThreadFactory => MainThreadScheduler.Factory;
	...
	{
		MainThreadScheduler = new ManualTaskScheduler();
	...
	public static void AddSingleTaskMainThread(string _name, Action _func)
	{
		AddSingleTaskMainThread(_name, [PublicizedFrom(EAccessModifier.Internal)] (object _) =>
		{
			_func();
		});
	}

	public static void RunTaskAfterFrames(Action _func, int _inFrames = 1)
	{
		StartCoroutine(runAfterFrames(_func, _inFrames));
	}
	...
	public static void UpdateMainThreadTasks()
	{
		...
		try
		{
			MainThreadScheduler.ProcessTasks();
		}
		catch (Exception e)
		{
			Log.Exception(e);
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static IEnumerator runAfterFrames(Action _func, int _inFrames)
	{
		for (int i = 0; i < _inFrames; i++)
		{
			yield return null;
		}
		_func?.Invoke();
