...
public class DynamicPropertiesCache
{
	...
	{
		Log.Out($"[BLOCKPROPERTIES] Creating DynamicProperties Cache, max cache size {1000}");
	...
	public void Stats()
	{
		...
		Log.Out($"[BLOCKPROPERTIES] Cache Size: {m_cache.Count}");
		Log.Out($"[BLOCKPROPERTIES] Hits: {m_cacheHits}, Misses: {m_cacheMisses}, Rate: {(float)m_cacheHits / (float)(m_cacheHits + m_cacheMisses) * 100f}%");
	}
}
