...
public class EAIApproachAndAttackTarget : EAIBase
{
	...
	public override void Update()
	{
		...
		if (!isTargetToEat)
		{
			...
			int num = 0;
			ItemAction itemAction = holdingItemItemValue.ItemClass.Actions[num];
			num2 = 1.095f;
			if (itemAction != null)
			{
				...
				if (num2 == 0f)
				{
					num2 = EffectManager.GetItemValue(PassiveEffects.MaxRange, holdingItemItemValue);
				}
			}
			num3 = num2;
			if (num2 >= 2.5f)
			...
			num3 = Utils.FastMax(0.7f, num3 - 0.35f);
		}
		else
		{
		...
		if (targetXZDistanceSq > num10 || num6 < -1.25f || num6 - theEntity.GetHeight() > 0.95f)
		{
			return;
		}
		...
		if (manager.groupCircle > 0f)
		{
			...
			if (targetIfAttackedNow != entityTarget && (!entityTarget.AttachedToEntity || entityTarget.AttachedToEntity != targetIfAttackedNow))
			{
				...
				{
					relocateTicks = 46;
					Vector3 vector4 = (theEntity.position - vector2).normalized * (num9 + 1.1f);
					float num11 = base.RandomFloat * 28f + 18f;
					if (base.RandomFloat < 0.5f)
					{
						num11 = 0f - num11;
					}
					vector4 = Quaternion.Euler(0f, num11, 0f) * vector4;
	...
	public Vector3 GetMoveToLocation(float maxDist)
	{
		...
		if (!entityTarget.onGround && !entityTarget.IsInElevator())
		{
			if (Physics.Raycast(vector - Origin.position, Vector3.down, out var hitInfo, 3.9f, 1082195968))
			{
				vector.y -= hitInfo.distance;
			}
			else
			{
				vector.y -= 3.9f;
		...
		if (maxDist > 0f)
		{
			...
			if (magnitude < 3f)
			{
				...
				{
					if (Physics.Raycast(vector4 - Origin.position, Vector3.down, out var hitInfo2, 1.02f, 1082195968))
					{
						vector4.y = hitInfo2.point.y + Origin.position.y;
						return vector4;
					}
