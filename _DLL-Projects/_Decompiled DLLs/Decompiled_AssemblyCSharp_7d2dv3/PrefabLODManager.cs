...
public class PrefabLODManager
{
	public class PrefabGameObject
	{
		...
		public List<ImposterCanvas> signDatas;
	}
	...
	public void FrameUpdate()
	{
		try
		{
			...
			if (world.ChunkCache.IsFixedSize)
			{
				return;
			}
	...
	public void removePrefabs(List<int> toRemove)
	{
		try
		{
			foreach (int item in toRemove)
			{
				...
				if (displayedPrefabs[item].signDatas != null)
				{
					foreach (ImposterCanvas signData in displayedPrefabs[item].signDatas)
					{
						ImposterSignFactory.Destroy(signData);
					}
				}
	...
	public void BuildGameObjectFromMeshInfo(SimpleMeshInfo _meshInfo, PrefabGameObject _pgo)
	{
		try
		{
			...
			LoadImposterSigns(_pgo, go, _meshInfo);
		}
		finally
		{
		}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public void LoadImposterSigns(PrefabGameObject _pgo, GameObject _go, SimpleMeshInfo _meshInfo)
	{
		if (_meshInfo.signs == null || _meshInfo.signs.Count == 0)
		{
			return;
		}
		_pgo.signDatas = new List<ImposterCanvas>();
		foreach (ImposterCanvas sign in _meshInfo.signs)
		{
			ImposterCanvas imposterCanvas = sign.Clone();
			ImposterSignFactory.Create(_go, imposterCanvas);
			if (imposterCanvas.Canvas != null)
			{
				_pgo.signDatas.Add(imposterCanvas);
			}
		}
	}
