...
public class vp_Placement
{
	...
	{
		if (Physics.CheckSphere(Position, physicsRadius, 2261000))
		{
			return true;
		}
		return false;
	...
	public static void SnapToGround(vp_Placement p, float radius, float snapDistance)
	{
		if (snapDistance != 0f)
		{
			Physics.SphereCast(new Ray(p.Position + Vector3.up * snapDistance, Vector3.down), radius, out var hitInfo, snapDistance * 2f, 1084850184);
