...
public class ChunkGameObject : MonoBehaviour
{
	...
	public void SetStatic(bool _bStatic)
	{
		for (int i = 0; i < layers.Length; i++)
		{
			...
			for (int k = 0; k < layers[i].m_MeshCollider.Length; k++)
			{
				...
				{
					layers[i].m_MeshCollider[k].gameObject.isStatic = _bStatic;
				}
			}
		}
	...
	public int copyToMesh(int _meshIdx)
	{
		...
		int num = vml.CopyToMesh(_meshIdx, meshFilter, chunkGameObjectLayer.m_MeshRenderer[_meshIdx], 0);
		bool active = num != 0;
	...
	public int copyToColliders(int _meshIdx)
	{
		...
		Mesh mesh;
		int num = vml.meshes[_meshIdx].CopyToColliders(meshCollider, out mesh);
		if (num != 0)
		{
