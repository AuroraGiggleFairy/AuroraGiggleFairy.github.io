...
public class ThrownWeaponMoveScript : MonoBehaviour
{
	...
	public virtual void checkCollision(Vector3 _amountToMove)
	{
		...
		bool num = Voxel.Raycast(gameManager.World, ray, magnitude, -538750989, (hmOverride == 0) ? 8 : hmOverride, 0f);
		if (entityAlive != null && entityAlive.emodel != null)
		{
		...
		if (firingEntity != null && !firingEntity.isEntityRemote)
		{
			...
			{
				itemValueWeapon.UseTimes += EffectManager.GetValue(PassiveEffects.DegradationPerUse, itemValueWeapon, 1f, firingEntity as EntityAlive, null, itemValueWeapon.ItemClass.ItemTags | FastTags<TagGroup.Global>.Parse("Secondary")) * ItemAction.ItemDegradationModifier;
			}
	...
	{
		if (!itemWeapon.IsSticky)
		{
			return false;
		}
		ItemStack itemStack = new ItemStack(itemValueWeapon, 1);
