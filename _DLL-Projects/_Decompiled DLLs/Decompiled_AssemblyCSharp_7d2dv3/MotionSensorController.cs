...
public class MotionSensorController : MonoBehaviour, IPowerSystemCamera
{
	...
	public void Update()
	{
		...
		if (!TileEntity.IsPowered || UserAccessingId != -1)
		{
			if (UserAccessingId != -1)
			{
	...
	public bool hasTarget()
	{
		...
		if (entitiesInBounds.Count > 0)
		{
			for (int i = 0; i < entitiesInBounds.Count; i++)
			{
				...
				if (!Voxel.Raycast(GameManager.Instance.World, ray, maxDistance, -538750989, 8, 0.1f) || !Voxel.voxelRayHitInfo.tag.StartsWith("E_"))
				{
					continue;
				}
