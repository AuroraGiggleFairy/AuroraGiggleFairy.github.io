...
public class WorldChunkCache : IChunkAccess
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	IChunk IChunkAccess.GetChunkSync(int chunkX, int chunkZ)
	{
		return GetChunkSync(chunkX, chunkZ);
	}

	[PublicizedFrom(EAccessModifier.Private)]
	IChunk IChunkAccess.GetChunkSync(long chunkKey)
	{
		return GetChunkSync(chunkKey);
	}
	...
	public void GetNeighborChunkKeys(long _chunkKey, bool _includeDiagonals, ref long[] _neighbours)
	{
		if (_includeDiagonals && _neighbours.Length != 8)
		{
			_neighbours = new long[8];
		}
		else if (_neighbours.Length != 8)
		{
			_neighbours = new long[4];
		}
		int num = extractX(_chunkKey);
		int num2 = extractZ(_chunkKey);
		_neighbours[0] = MakeChunkKey(num, num2 - 1);
		_neighbours[1] = MakeChunkKey(num, num2 + 1);
		_neighbours[2] = MakeChunkKey(num - 1, num2);
		_neighbours[3] = MakeChunkKey(num + 1, num2);
		if (_includeDiagonals)
		{
			_neighbours[4] = MakeChunkKey(num - 1, num2 - 1);
			_neighbours[5] = MakeChunkKey(num - 1, num2 + 1);
			_neighbours[6] = MakeChunkKey(num + 1, num2 - 1);
			_neighbours[7] = MakeChunkKey(num + 1, num2 + 1);
		}
	}
	...
	public bool HasNeighborChunks(Chunk _chunk)
	{
		...
		int x = _chunk.X;
		int z = _chunk.Z;
		bool result = GetChunk(x + 1, z) != null && GetChunk(x - 1, z) != null && GetChunk(x, z + 1) != null && GetChunk(x, z - 1) != null && GetChunk(x + 1, z + 1) != null && GetChunk(x - 1, z - 1) != null && GetChunk(x - 1, z + 1) != null && GetChunk(x + 1, z - 1) != null;
		sync.ExitReadLock();
		return result;
	}
