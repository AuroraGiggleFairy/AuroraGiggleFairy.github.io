...
public class Voxel
{
	...
	public const int HM_IgnoreFragile = 4096;
	...
	{
		return Raycast(_worldData, ray, distance, -538488845, _hitmask, _sphereSize);
	}
	...
	{
		return Raycast(_worldData, ray, distance, -538488845, 0x42 | (bHitTransparentBlocks ? 1 : 0) | (bHitNotCollidableBlocks ? 4 : 0), 0f);
	}
	...
	public static void terrainMeshHit(World _world, Vector3 dirNormalized, string phsxTag, Vector3 phsxHitpPoint, HitInfoDetails.VoxelData lastHitData, int _layerMask, int _hitMask)
	{
		...
		Vector3 vector = chunkCache.ToLocalPosition(worldPos);
		dirNormalized = chunkCache.ToLocalVector(dirNormalized);
		voxelRayHitInfo.hit.blockPos = World.worldToBlockPos(vector);
		voxelRayHitInfo.hit.voxelData = HitInfoDetails.VoxelData.GetFrom(chunkCache, voxelRayHitInfo.hit.blockPos);
		...
		if (flag2 || voxelRayHitInfo.hit.voxelData.Equals(lastHitData) || ((_hitMask & 2) == 0 && voxelRayHitInfo.hit.voxelData.IsOnlyWater()))
		{
			...
			{
				voxelData = (voxelRayHitInfo.hit.voxelData = HitInfoDetails.VoxelData.GetFrom(chunkCache, voxelRayHitInfo.hit.blockPos - Vector3i.up));
			...
			int num = calcBestNormalToRaycastHit(chunkCache);
			voxelRayHitInfo.hit.blockPos = voxelRayHitInfo.hit.blockPos - normalsI[num];
			voxelRayHitInfo.hit.voxelData = HitInfoDetails.VoxelData.GetFrom(chunkCache, voxelRayHitInfo.hit.blockPos);
			voxelRayHitInfo.hit.blockFace = normalToFaces[num];
			return;
		}
		int num2 = calcBestNormalToRaycastHit(chunkCache);
		...
		do
		{
			...
			voxelData = (voxelData2 = HitInfoDetails.VoxelData.GetFrom(chunkCache, voxelRayHitInfo.lastBlockPos));
		}
		while (!voxelData.IsOnlyAir() && voxelData2.BlockValue.Block.isMultiBlock && !voxelData2.IsOnlyWater() && num3++ < 3);
		if (!(phsxTag == "T_Mesh_B") || !(MeshDescription.meshes[voxelRayHitInfo.hit.blockValue.Block.MeshIndex].Tag != "T_Mesh_B"))
		{
			return;
		}
		voxelData = (voxelData2 = HitInfoDetails.VoxelData.GetFrom(chunkCache, voxelRayHitInfo.hit.blockPos + Vector3i.up));
	...
	public static bool raycastNew(World _world, Ray ray, float distance, int _layerMask, int _hitMask, float _sphereRadius)
	{
		...
		while (num++ < 10 && distance > 0f)
		{
			...
			Block block2 = voxelRayHitInfo.hit.propValue.blockValue.Block;
			...
			bool flag11 = block.IsSeeThrough(_world, voxelRayHitInfo.hit.blockPos, voxelRayHitInfo.hit.blockValue);
			bool flag12 = block2.IsSeeThrough(_world, voxelRayHitInfo.hit.blockPos, voxelRayHitInfo.hit.blockValue);
			if (flag10 && block.MaxDamage <= 5)
			{
			...
			{
				if ((flag3 && (block.IsCollideMovement || block2.IsCollideMovement) && (flag2 || !flag11)) || (flag4 && !block.IsCollideMovement && !block2.IsCollideMovement && !voxelRayHitInfo.hit.voxelData.IsOnlyWater()) || (flag6 && (block.IsCollideBullets || block2.IsCollideBullets)) || (flag7 && (block.IsCollideRockets || block2.IsCollideRockets)) || (flag9 && (block.IsCollideArrows || block2.IsCollideArrows)) || (flag8 && (block.IsCollideMelee || block2.IsCollideMelee)) || (flag5 && voxelRayHitInfo.hit.voxelData.IsOnlyWater()) || (flag2 && (flag11 || flag12)))
				{
					voxelRayHitInfo.tag = text;
					voxelRayHitInfo.bHitValid = true;
					voxelRayHitInfo.hit.pos = vector;
	...
	public static bool GetNextBlockHit(World _worldData, Ray ray, float _distance, int _hitMask, bool bCastSphere)
	{
		...
		while (true)
		{
			...
			if (!voxelData.IsOnlyAir())
			{
				...
				bool flag10 = false;
				flag10 = block.IsSeeThrough(_worldData, new Vector3i(num, num2, num3), voxelData.BlockValue);
