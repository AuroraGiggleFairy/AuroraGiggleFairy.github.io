...
public class XUiC_CategoryList : XUiController
{
	...
	public bool TrySetCategory(string _category)
	{
		int _index;
		XUiC_CategoryEntry categoryByName = GetCategoryByName(_category, out _index);
	...
	public bool SetupCategoriesByWorkstation(string _workstation)
	{
		if (currentWorkstation != _workstation)
		{
			...
			if (_workstation == "skills")
			{
				...
				{
					if (progressionClass.Value.IsAttribute && (XUiM_Recipes.CraftingProgression || !(progressionClass.Value.Name == "attcrafting")))
					{
	...
	public bool SetupCategoriesBasedOnItems(List<TraderData.Entry> _entries, int _traderStage)
	{
		if (_entries == null)
		{
			return false;
		...
		{
			ItemClass itemClass = _entries[i].Item.itemValue.ItemClass;
			if (itemClass == null)
			{
				Log.Error("TraderData.Entry #{0} null", i);
				continue;
			}
			...
			if (traderStageTemplateGroup != null && !traderStageTemplateGroup.IsWithin(_traderStage, _entries[i].Item.itemValue.Quality) && !_entries[i].AddedByPlayer)
			{
				continue;
			}
			...
			{
				array = Block.list[_entries[i].Item.itemValue.type].GroupNames;
			}
