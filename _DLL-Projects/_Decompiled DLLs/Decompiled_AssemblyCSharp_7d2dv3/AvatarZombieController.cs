...
public class AvatarZombieController : AvatarHumanController
{
	...
	public override void RemoveLimb(BodyDamage _bodyDamage, bool restoreState)
	{
		...
		if (!headDismembered && (bodyPartHit & EnumBodyPartHit.Head) > EnumBodyPartHit.None)
		{
			...
			else if (entity.GetHeadState() == EModelBase.HeadStates.BigHead || entity.GetHeadState() == EModelBase.HeadStates.Growing)
			{
	...
	public void ActivateDetachableLimbs(uint bodyDamageFlag, EnumDamageTypes damageType, Transform partT, DismemberedPart part)
	{
		...
		bool flag = entity.GetHeadState() == EModelBase.HeadStates.BigHead || entity.GetHeadState() == EModelBase.HeadStates.Growing;
		if ((entity.OverrideHeadSize != 1f || flag) && headDismembered && bodyDamageFlag == 1)
		{
			float headBigSize = entity.emodel.HeadBigSize;
			if (!flag)
			{
		...
		if ((bool)componentInChildren)
		{
			...
			float num = Vector3.Angle(entity.GetForwardVector(), entity.lastHitImpactDir);
			componentInChildren.AddTorque(Quaternion.FromToRotation(entity.GetForwardVector(), entity.lastHitImpactDir).eulerAngles * (1f + num / 90f), ForceMode.Impulse);
			...
			if (damageTag == "blade")
			{
				float num2 = Vector3.Dot(entity.GetForwardVector(), entity.lastHitImpactDir);
				float num3 = Vector3.Dot(entity.GetForwardVector(), entity.lastHitEntityFwd);
				componentInChildren.AddForce((num2 < num3) ? (-entity.transform.right * entity.lastHitForce + vector) : (entity.transform.right * entity.lastHitForce + vector), ForceMode.Impulse);
				componentInChildren.AddTorque(Quaternion.FromToRotation(entity.GetForwardVector(), entity.lastHitImpactDir).eulerAngles * (1f + num / 90f) * entity.lastHitForce, ForceMode.Impulse);
			}
			if (damageType == EnumDamageTypes.Heat)
			{
				float num4 = 2.67f;
				componentInChildren.AddForce(entity.lastHitImpactDir * num4 + Vector3.up * num4 * 0.67f, ForceMode.Impulse);
			}
		}
