...
public class ObjectiveClosestPOIGoto : ObjectiveGoto
{
	...
	public override Vector3 GetPosition(EntityNPC ownerNPC = null, EntityPlayer entityPlayer = null, List<Vector2> usedPOILocations = null, int entityIDforQuests = -1)
	{
		...
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			...
			PrefabInstance closestPOIToWorldPos = GameManager.Instance.World.ChunkCache.ChunkProvider.GetDynamicPrefabDecorator().GetClosestPOIToWorldPos(base.OwnerQuest.QuestTags, new Vector2(entityAlive.position.x, entityAlive.position.z), null, -1, ignoreCurrentPOI: false, BiomeFilterTypes.SameBiome, "", questKey);
