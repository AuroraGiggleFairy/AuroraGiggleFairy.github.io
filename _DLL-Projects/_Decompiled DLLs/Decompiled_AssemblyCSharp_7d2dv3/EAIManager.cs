...
public class EAIManager
{
	...
	public void AddItemTasks(ItemClass ic)
	{
		...
		string text = ic.Properties.GetString("AITask");
		if (text.Length > 0)
		{
			ParseTasks(text, eAITaskList);
		}
		for (int i = 0; i < eAITaskList.Tasks.Count; i++)
		{
			EAITaskEntry eAITaskEntry = eAITaskList.Tasks[i];
			if (eAITaskEntry.action is EAIItemTask eAIItemTask)
			{
		...
		tasks.AddTaskList(eAITaskList);
	}

	public void RemoveItemTasks(string itemKey)
	{
		List<EAITaskEntry> list = tasks.Tasks.FindAll([PublicizedFrom(EAccessModifier.Internal)] (EAITaskEntry t) => t.action is EAIItemTask && (t.action as EAIItemTask).ItemKey.Equals(itemKey));
		if (list != null)
		{
			for (int num = 0; num < list.Count; num++)
			{
				tasks.RemoveTask(list[num]);
			}
		}
	}
	...
	public void CopyPropertiesFromEntityClass(EntityClass ec)
	{
		...
		else
		{
			...
			while (true)
			{
				...
				if (!ec.Properties.Values.TryGetValue(key, out var value) || value.Length == 0)
				{
					break;
				}
				EAIBase eAIBase = CreateInstance(value);
				if (eAIBase == null)
				{
				...
				if (dictionary != null)
				{
					...
					{
						Log.Error("EAIManager {0} SetData error {1}", value, ex);
					}
				}
		...
		while (true)
		{
			...
			if (!ec.Properties.Values.TryGetValue(key2, out var value2) || value2.Length == 0)
			{
				break;
			}
			EAIBase eAIBase2 = CreateInstance(value2);
			if (eAIBase2 == null)
			{
				throw new Exception("Class '" + value2 + "' not found!");
			}
			...
			if (dictionary2 != null)
			{
				...
				{
					Log.Error("EAIManager {0} SetData error {1}", value2, ex2);
				}
			}
	...
	public static Type GetType(string _className)
	{
		...
		case "DroneItemModStunWeapon":
			return typeof(EAIDroneItemModStunWeapon);
		case "DroneItemModHealWeapon":
	...
	public string MakeDebugName(EntityPlayer player)
	{
		...
		string text = string.Format("\n{0}{1}{2}{3}", entity.IsAlert ? string.Format("Alert {0}, ", ((float)entity.GetAlertTicks() * 0.05f).ToCultureInvariantString("0.00")) : "", entity.HasInvestigatePosition ? string.Format("Investigate {0}, ", ((float)entity.GetInvestigatePositionTicks() * 0.05f).ToCultureInvariantString("0.00")) : "", entity.attractPlayer ? string.Format("Attract {0}, Dist {1}, {2}", entity.attractPlayer.EntityName, entity.attractPlayerDistance.ToCultureInvariantString("0.00"), ((float)entity.attractPlayerTimeoutTicks * 0.05f).ToCultureInvariantString("0.00")) : "", entity.smellPlayer ? string.Format("Smell {0}, Dist {1}, {2}", entity.smellPlayer.EntityName, entity.smellPlayerDistance.ToCultureInvariantString("0.00"), ((float)entity.smellPlayerTimeoutTicks * 0.05f).ToCultureInvariantString("0.00")) : "");
		if (text.Length > 1)
		{
	...
	{
		return getTaskType<T>(targetTasks);
	}
