...
public class XUiC_ItemPartStack : XUiC_BasePartStack
{
	...
	public override bool CanSwap(ItemStack stack)
	{
		...
		ItemValue itemValue = xui.AssembleItem.CurrentItem.itemValue;
		...
		if (itemClassModifier.ModifierTags.Test_AnySet(ItemClassModifier.CosmeticModTypes) && itemValue.CosmeticMods.Length != 0)
		{
		...
		{
			if (itemValue.Modifications[j].ItemClass != null && itemValue.Modifications[j].ItemClass is ItemClassModifier itemClassModifier2 && !itemValue.Modifications[j].Equals(this.itemValue) && itemClassModifier.ModifierTags.Test_AnySet(itemClassModifier2.ModifierTags))
			{
				num++;
			}
		}
	...
	public override bool CanRemove()
	{
		...
		if (itemClass is ItemClassModifier itemClassModifier)
		{
			...
			if (itemClassModifier.ModifierTags.Test_AnySet(EntityVehicle.StorageModifierTags) && base.WindowGroup.Controller is XUiC_VehicleWindowGroup xUiC_VehicleWindowGroup)
			{
	...
	public override void SwapItem()
	{
		...
		base.SwapItem();
		if (itemClassModifier != null && itemClassModifier.ModifierTags.Test_AnySet(ItemClassModifier.CosmeticModTypes))
		{
