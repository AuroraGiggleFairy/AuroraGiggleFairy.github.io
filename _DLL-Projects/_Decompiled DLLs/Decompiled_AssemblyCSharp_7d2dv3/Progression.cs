...
public class Progression
{
	...
	public static int SkillPointsPerLevelXML;

	public static int SkillPointsGainRate;
	...
	public static ShowXPTypes ShowXPType = ShowXPTypes.All;
	...
	public int AddLevelExp(int _exp, string _cvarXPName = "_xpOther", XPTypes _xpType = XPTypes.Other, bool useBonus = true, bool notifyUI = true, int _instigatorID = -1, ItemValue _itemValue = null)
	{
		...
		if (useBonus)
		{
			...
			num *= XPGain;
			float originalValue = num;
			EntityAlive entity = parent;
			FastTags<TagGroup.Global> tags = xpFastTags[(int)_xpType];
			num = EffectManager.GetValue(PassiveEffects.PlayerExpGain, _itemValue, originalValue, entity, null, tags);
		}
		if (num > 214748370f)
		{
			num = 214748370f;
		...
		if ((bool)entityPlayerLocal && (ShowXPType == ShowXPTypes.All || ShowXPType == ShowXPTypes.NotificationsOnly))
		{
	...
	public void AddLevelExpRecursive(int exp, string _cvarXPName, bool notifyUI = true)
	{
		...
		if (ExpDeficit <= 0)
		{
			...
			{
				Level++;
				if (Level % SkillPointsGainRate == 0)
				{
	...
	public void UpdateForSandbox()
	{
		foreach (ProgressionClass value in ProgressionClasses.Values)
		{
			if (value.DisplayDataList != null)
			{
				value.UpdateForSandbox();
			}
		}
	}
}
