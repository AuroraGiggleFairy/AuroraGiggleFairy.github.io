...
public class ItemActionOpenLootBundle : ItemAction
{
	...
	public override void ExecuteAction(ItemActionData _actionData, bool _bReleased)
	{
		...
		if (ConditionBlockTypes != null)
		{
			...
			if (air.isair || !ConditionBlockTypes.Contains(air.type))
			{
				...
				if (!Voxel.Raycast(myInventoryData.invData.world, lookRay, 2.5f, -538480653, 4095, 0f))
				{
					return;
				}
				_ = voxelRayHitInfo.hit;
	...
	public override bool ExecuteInstantAction(EntityAlive ent, ItemStack stack, bool isHeldItem, XUiC_ItemStack stackController)
	{
		...
		{
			IList<ItemStack> list = lootContainer.Spawn(ent.rand, 100, entityPlayer.GetHighestPartyLootStage(0f, 0f), 0f, entityPlayer, FastTags<TagGroup.Global>.none, uniqueItems: true, ignoreLootProb: false, ignoreLootAbundance: true);
			for (int i = 0; i < list.Count; i++)
			{
				...
				{
					itemStack.itemValue.MaxDurabilityModifier = 1f;
				}
