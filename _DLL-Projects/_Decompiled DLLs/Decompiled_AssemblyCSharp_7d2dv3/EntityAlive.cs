...
public abstract class EntityAlive : Entity
{
	...
	public static HeadShotOnlyModes HeadshotMode = HeadShotOnlyModes.None;

	public static CelebrateModes CelebrateMode = CelebrateModes.Disabled;
	...
	public static bool IsHeadshotOnly => HeadshotMode == HeadShotOnlyModes.HeadshotOnly;

	public static bool IsHeadshotFinisher => HeadshotMode == HeadShotOnlyModes.HeadshotFinisher;

	public static bool IsCelebrate => CelebrateMode == CelebrateModes.Enabled;

	public static bool IsCelebrateHeadshot => CelebrateMode == CelebrateModes.HeadshotOnly;
	...
	public void ActivateDynamicRagdoll()
	{
		...
		emodel.DoRagdoll(EModelBase.RagdollMode.Default, _dynamicRagdollStunTime, EnumBodyPartHit.None, forceVec, Vector3.zero, isRemote: true);
	...
	public override void CopyPropertiesFromEntityClass()
	{
		...
		{
			float num2 = rand.RandomRange(optionalValue3.x, optionalValue3.y);
		...
		entityClass.Properties.ParseString(EntityClass.PropSoundPlayerLandThump, ref soundLandThump);
		...
		if (gameModeForId != null)
		{
			...
			{
				text4 = entityClass.Properties.Classes[EntityClass.PropItemsOnEnterGame].GetString(gameModeForId.GetTypeName());
			}
			if (text4.Length > 0)
			{
				...
				{
					ItemStack itemStack = ItemStack.FromString(text5.Trim());
					if (itemStack.itemValue.IsEmpty())
					{
						throw new Exception("Item with name '" + text5 + "' not found in class " + EntityClass.list[base.entityClass].entityClassName);
					}
		...
		DynamicProperties dynamicProperties = entityClass.Properties.GetClass(EntityClass.PropFallLandBehavior);
		if (dynamicProperties != null)
		{
			foreach (KeyValuePair<string, string> datum in dynamicProperties.Data)
			{
				string key = datum.Key;
				Dictionary<string, string> dictionary = dynamicProperties.ParseKeyData(key);
				if (dictionary == null)
				{
					continue;
				}
				...
				if (!dictionary.TryGetValue("anim", out var value) || !Enum.TryParse<FallBehavior.Op>(value, out var result))
				{
					Log.Error("Expected 'anim' parameter as float for FallBehavior " + key + ", skipping");
					continue;
				}
				float _result = 0f;
				if (!dictionary.TryGetValue("weight", out value) || !StringParsers.TryParseFloat(value, out _result))
				{
					Log.Error("Expected 'weight' parameter as float for FallBehavior " + key + ", skipping");
				}
				else if (dictionary.TryGetValue("height", out value))
				{
					if (StringParsers.TryParseRange(value, out var _result2, float.MaxValue))
					{
						floatRange = _result2;
						if (dictionary.TryGetValue("ragePer", out value))
						{
							if (!StringParsers.TryParseRange(value, out FloatRange _result3, (float?)null, ','))
							{
						...
						if (dictionary.TryGetValue("rageTime", out value))
						{
							if (!StringParsers.TryParseRange(value, out FloatRange _result4, (float?)null, ','))
							{
						...
						if (dictionary.TryGetValue("difficulty", out value))
						{
							if (!StringParsers.TryParseRange(value, out var _result5, null, ','))
							{
		...
		DynamicProperties dynamicProperties2 = entityClass.Properties.GetClass(EntityClass.PropDestroyBlockBehavior);
		if (dynamicProperties2 != null)
		{
			...
			{
				string text6 = array2[j].ToStringCached();
				Dictionary<string, string> dictionary2 = dynamicProperties2.ParseKeyData(array2[j].ToStringCached());
				if (dictionary2 == null)
				{
					continue;
				}
				...
				if (!dictionary2.TryGetValue("weight", out var value2) || !StringParsers.TryParseFloat(value2, out var _result6))
				{
					Log.Error($"Expected 'weight' parameter as float for FallBehavior {array2[j]}, skipping");
					continue;
				}
				if (dictionary2.TryGetValue("ragePer", out value2))
				{
					if (!StringParsers.TryParseRange(value2, out FloatRange _result7, (float?)null, ','))
					{
				...
				if (dictionary2.TryGetValue("rageTime", out value2))
				{
					if (!StringParsers.TryParseRange(value2, out FloatRange _result8, (float?)null, ','))
					{
				...
				if (dictionary2.TryGetValue("difficulty", out value2))
				{
					if (!StringParsers.TryParseRange(value2, out var _result9, null, ','))
					{
						Log.Error("Expected 'difficulty' parameter as range(min,min-max) " + text6 + ", skipping");
						continue;
					}
					difficulty2 = _result9;
				}
				_destroyBlockBehaviors.Add(new DestroyBlockBehavior(text6, array2[j], _result6, ragePer2, rageTime2, difficulty2));
			}
		}
	...
	public void SetInventorySlots(string _handItemName)
	{
		...
		string[] array = _handItemName.Split(",");
		if (inventory == null)
		{
			return;
		}
	...
	public virtual void InitStats()
	{
		...
		startOfFrameStats = new EntityStats(this);
	}
	...
	public override void OnUpdateEntity()
	{
		...
		OnUpdateLive();
		if (!IsSleeping && (!isEntityRemote || !(this is EntityPlayer)) && inventory != null)
		{
			inventory.OnUpdate();
		}
	...
	public virtual void ClientKill(DamageResponse _dmResponse)
	{
		...
		if (!IsDead())
		{
			...
			if (Buffs != null)
			{
				Buffs.OnDeath(entityThatKilledMe, (_dmResponse.Source == null) ? null : _dmResponse.Source.AttackingItem, _dmResponse.Source != null && _dmResponse.Source.damageType == EnumDamageTypes.Crushing, (_dmResponse.Source == null) ? FastTags<TagGroup.Global>.Parse("crushing") : _dmResponse.Source.DamageTypeTag);
			}
			if (Progression != null)
			{
				Progression.OnDeath();
			...
			if (obj == null && base.EntityClass.bIsEnemyEntity && entityThatKilledMe is EntityPlayer && (EffectManager.GetValue(PassiveEffects.CelebrationKill, null, 0f, entityThatKilledMe) > 0f || IsCelebrate || (IsCelebrateHeadshot && _dmResponse.Source != null && _dmResponse.HitBodyPart == EnumBodyPartHit.Head)))
			{
			...
			else
			{
				HandleClientDeath((_dmResponse.Source != null) ? _dmResponse.Source.BlockPosition : GetBlockPosition());
				OnEntityDeath();
				emodel.OnDeath(_dmResponse, world.ChunkCache);
			}
		}
	}
	...
	public virtual void OnUpdateLive()
	{
		...
		if (!isEntityRemote && !IsDead())
		{
			...
			if ((bool)attractPlayer && --attractPlayerTimeoutTicks <= 0)
			{
				attractPlayer = null;
			}
		}
	...
	public void checkForTeleportOutOfTraderArea()
	{
		...
		if (traderAreaAt != null && traderAreaAt.IsInitialized)
		{
			...
			int num = 0;
			PrefabTeleportVolume tpVolume;
			if (TraderInfo.TraderHoursPreset != TraderInfo.TraderHourPresets.AlwaysOpen && world.IsWorldEvent(World.WorldEvent.BloodMoon) && traderAreaAt.IsWithinProtectArea(vector))
			{
	...
	public override void Update()
	{
		...
		{
			float num = Utils.FastAbs(speedForward);
			float num2 = 0.06935714f;
			if (num > 0.01942f)
			{
				num2 = speedForwardTargetStep;
			}
			float x = Utils.FastMoveTowards(speedForward, speedForwardTarget, num2 * Time.deltaTime);
			if (num > 0.01942f && Utils.FastAbs(x) <= 0.01942f)
			{
				x = 0.01942f;
			}
			speedForward = x;
	...
	{
		startOfFrameStats.CopyFrom(entityStats);
	}
	...
	public virtual void OnDeathUpdate()
	{
		...
		if (deathUpdateTime >= timeStayAfterDeath && !isEntityRemote && !markedForUnload && particleOnDestroy != null && particleOnDestroy.Length > 0)
		{
			float lightBrightness = world.GetLightBrightness(GetBlockPosition());
	...
	public virtual void MoveEntityHeaded(Vector3 _direction, bool _isDirAbsolute)
	{
		...
		if (_isDirAbsolute)
		{
			vector = base.transform.InverseTransformDirection(moveDirection);
		}
	...
	public bool CanEntityBeSeen(Entity _other, bool checkViewCone = true)
	{
		...
		if (checkViewCone && !IsInViewCone(headPosition2))
		{
			return false;
		}
		bool result = false;
		...
		int modelLayer = GetModelLayer();
		SetModelLayer(2);
		if (Voxel.Raycast(world, ray, num, -1612492829, 64, 0f))
		{
	...
	public bool NotifyDestroyedBlock(ItemActionAttack.AttackHitInfo attackHitInfo)
	{
		...
		{
			if (moveHelper.HitInfo.hit.blockValueRef == attackHitInfo.hitRef)
			{
	...
	public virtual void PlayHitGroundSound(float impactSpeed = 1f)
	{
		...
		if (!string.IsNullOrEmpty(soundLand))
		{
			PlayOneShot(soundLand, sound_in_head: false, serverSignalOnly: false, isUnique: false, null, volumeScale);
		}
		else if (!string.IsNullOrEmpty(soundLandThump))
		{
			PlayOneShot(soundLandThump, sound_in_head: false, serverSignalOnly: false, isUnique: false, null, volumeScale);
		}
		else
		{
			PlayOneShot("entityhitsground", sound_in_head: false, serverSignalOnly: false, isUnique: false, null, volumeScale);
		}
	}
	...
	public override int DamageEntity(DamageSource _damageSource, int _strength, bool _criticalHit, float _impulseScale = 1f)
	{
		...
		if (!IsDead() && (bool)entityAlive)
		{
			...
			float value = EffectManager.GetValue(PassiveEffects.DamageBonus, _damageSource.AttackingItem, 0f, entity);
			if (value > 0f)
			{
		...
		else
		{
			...
			if (!flag && _damageSource.CreatorEntityId != -2)
			{
				...
				{
					Entity entity2 = world.GetEntity(_damageSource.CreatorEntityId);
					if ((bool)entity2 && !entity2.isEntityRemote)
					{
						excludePlayer = -1;
					}
				}
	...
	public override bool CanLockOnServer(int _lockingPlayerID, ILockContext _context, ushort _channel)
	{
		if (!IsDead())
		{
			return base.CanLockOnServer(_lockingPlayerID, _context, _channel);
		}
		return false;
	}
	...
	public override bool CanLockLocally(ILockContext _context, ushort _channel)
	{
		if (!IsDead())
		{
			return base.CanLockLocally(_context, _channel);
		}
		return false;
	}
	...
	public virtual void ProcessDamageResponseLocal(DamageResponse _dmResponse)
	{
		...
		if (equipment != null && _dmResponse.ArmorDamage > 0)
		{
			...
			if (armor.Count > 0)
			{
				...
				{
					armor[i].UseTimes += EffectManager.GetValue(PassiveEffects.DegradationPerUse, armor[i], num, this, null, armor[i].ItemClass.ItemTags) * ItemAction.ItemDegradationModifier;
				}
			}
		}
		...
		{
			if (!flag4 && ((IsHeadshotOnly && entityAlive is EntityPlayer) || EffectManager.GetValue(PassiveEffects.HeadShotOnly, null, 0f, entityAlive) > 0f) && !HasAnyTags(noheadTag) && (_dmResponse.HitBodyPart & EnumBodyPartHit.Head) == 0)
			{
				num2 = 0;
				_dmResponse.Fatal = false;
			}
			if (!flag4 && IsHeadshotFinisher && entityAlive is EntityPlayer && _dmResponse.Fatal && !HasAnyTags(noheadTag) && (_dmResponse.HitBodyPart & EnumBodyPartHit.Head) == 0)
			{
				num2 = Health - 1;
				_dmResponse.Fatal = false;
			}
		...
		else if (_dmResponse.PainHit && !flag3 && (bool)emodel.avatarController)
		{
			...
			if (num3 >= 0f)
			{
				float num4 = GetMaxHealth();
				if ((float)Health / num4 < entityClass.PainResistPerHitLowHealthPercent)
				{
					num3 = entityClass.PainResistPerHitLowHealth;
				}
				painResistPercent = Utils.FastMin(painResistPercent + num3, 3f);
				float duration = float.MaxValue;
				if (painResistPercent >= 3f && num3 >= 1f)
				{
					duration = 0f;
					painHitsFelt += 0.15f;
				}
				...
				emodel.avatarController.StartAnimationHit(_dmResponse.HitBodyPart, (int)_dmResponse.HitDirection, (int)((float)_dmResponse.Strength * 100f / num4), _dmResponse.Critical, _dmResponse.MovementState, _dmResponse.Random, duration);
			}
		}
		if (bodyDamage.CurrentStun == EnumEntityStunType.None)
		{
	...
	public string GetArmorImpactSound(EquipmentSlots _slot, bool _grazingHit, bool _includeCosmetic = true)
	{
		...
		if (_includeCosmetic)
		{
			itemClass = equipment.GetCosmeticSlot((int)_slot, useTemporary: false);
		}
		if (itemClass == null)
		{
			itemClass = equipment.GetSlotItem((int)_slot)?.ItemClass;
		}
		if (itemClass == null)
		{
			return string.Empty;
	...
	{
		emodel.DoRagdoll(in _dmResponse, EModelBase.RagdollMode.Default, _dmResponse.StunDuration);
	}

	public void DoRagdoll(in DamageResponse _dmResponse, EModelBase.RagdollMode _mode)
	{
		emodel.DoRagdoll(in _dmResponse, _mode, _dmResponse.StunDuration);
	}
	...
	public virtual void dropItemOnDeath()
	{
		if (!hasAI)
		{
			for (int i = 0; i < inventory.GetItemCount(); i++)
			{
				...
				if (forId != null && forId.CanDrop())
				{
					...
					inventory.SetItem(i, ItemValue.None, 0);
				}
			}
		...
		{
			lootDropProb = EffectManager.GetValue(PassiveEffects.LootDropProb, entityThatKilledMe.inventory.holdingItemItemValue, lootDropProb, entityThatKilledMe) * LootContainer.LootBagChance;
		}
		if (!LootContainer.NoLoot && lootDropProb > rand.RandomFloat)
		{
			DropBagServer();
		}
	}
	...
	public virtual void updateTasks()
	{
		...
		{
			SetMoveForwardWithModifiers(0f, 0f, 0f, _climb: false);
			if (aiManager != null)
			{
	...
	public Entity GetTargetIfAttackedNow()
	{
		...
		if (holdingItem != null)
		{
			...
			if (itemAction != null)
			{
				WorldRayHitInfo executeActionTarget = itemAction.GetExecuteActionTarget(inventory.holdingItemData.actionData[0]);
	...
	public void updateCurrentBlockPosAndValue()
	{
		...
		{
			bool flag = true;
			if (world.GetTileEntity(blockPosStandingOn) is TileEntityWorkstation tileEntityWorkstation)
			{
	...
	public void ExecuteDismember(bool restoreState)
	{
		if (!(emodel == null) && !(emodel.avatarController == null))
		{
			if (bodyDamage.IsCrippled && bodyDamage.bodyPartHit.IsLeg() && IsAlive() && walkType != 5 && walkType < 20)
			{
				SetWalkType(5);
			}
	...
	public void CrouchHeightFixedUpdate()
	{
		...
		if (Physics.SphereCast(origin, num2, Vector3.up, out var hitInfo, maxDistance, 1083277320))
		{
		...
		if (num < physicsHeight)
		{
			...
			num = Utils.FastMoveTowards(physicsHeight, num, 0.099999994f);
		}
		else
		{
		...
		if (num <= num4)
		{
			...
			if (walkType != 8 && walkType != 21)
			{
				int num5 = walkType;
				SetWalkType(8);
				walkTypeBeforeCrouch = num5;
			}
		}
		else
		{
	...
	public void SetWalkType(int _walkType)
	{
		if (walkType != 21)
		{
			...
			{
				walkType = _walkType;
				emodel.avatarController.TurnIntoCrawler();
				walkTypeBeforeCrouch = 0;
			}
			else if (walkTypeBeforeCrouch != 0)
			{
				walkTypeBeforeCrouch = _walkType;
			}
			else
			{
				walkType = _walkType;
	...
	public void ForceBigHead()
	{
		if ((this is EntityAnimal || this is EntityEnemy || this is EntityTrader) && base.EntityClass.CanBigHead && emodel.HeadState == EModelBase.HeadStates.Standard)
		{
			emodel.HeadState = EModelBase.HeadStates.BigHead;
		}
	}
	...
	public void ForceResetHead()
	{
		if ((this is EntityAnimal || this is EntityEnemy || this is EntityTrader) && base.EntityClass.CanBigHead && (emodel.HeadState == EModelBase.HeadStates.BigHead || emodel.HeadState == EModelBase.HeadStates.Growing))
		{
			emodel.HeadState = EModelBase.HeadStates.Standard;
		}
	}
	...
	{
		EntityAlive attackTargetLocal = GetAttackTargetLocal();
	...
	public virtual bool GetHeadLookTarget(out Vector3 lookTarget, out bool requiresLineOfSight)
	{
