...
public class XUiC_DialogResponseEntry : XUiController
{
	...
	public DialogResponse CurrentResponse
	{
		...
		set
		{
			...
			if (currentResponse != null && currentResponse.RequirementList.Count > 0)
			{
				...
				{
					if (!currentResponse.RequirementList[i].CheckRequirement(xui.playerUI.entityPlayer, xui.Dialog.Respondent))
					{
	...
	public override bool GetBindingValueInternal(ref string value, string bindingName)
	{
		...
		case "response":
			...
			if (flag)
			{
				value = (HasRequirement ? currentResponse.Text : currentResponse.GetRequiredDescription(xui.playerUI.entityPlayer));
			}
			return true;
		case "textstatecolor":
