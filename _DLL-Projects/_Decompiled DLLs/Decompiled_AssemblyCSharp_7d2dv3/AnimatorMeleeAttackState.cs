...
public class AnimatorMeleeAttackState : StateMachineBehaviour
{
	...
	public override void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
	{
		...
		holdingItem.Properties.ParseFloat((actionIndex == 0) ? "Action0" : "Action1", "RaycastTime", ref RaycastTime);
		float optionalValue = -1f;
		holdingItem.Properties.ParseFloat((actionIndex == 0) ? "Action0" : "Action1", "ImpactDuration", ref optionalValue);
		if (optionalValue >= 0f)
		{
			ImpactDuration = optionalValue * originalMeleeAttackSpeed;
		}
		holdingItem.Properties.ParseFloat((actionIndex == 0) ? "Action0" : "Action1", "ImpactPlaybackSpeed", ref ImpactPlaybackSpeed);
		...
		iData = entity.inventory.holdingItemData.actionData[actionIndex] as ItemActionDynamicMelee.ItemActionDynamicMeleeData;
		if (iData != null)
		{
			iData.HasFinished = false;
		}
	...
	public IEnumerator impactStart(Animator animator, AnimatorStateInfo stateInfo, AnimationClip clip, int layerIndex)
	{
		...
		{
			hasFired = true;
			if (entity != null && !entity.isEntityRemote && actionIndex >= 0 && entity.inventory.holdingItemData.actionData[actionIndex] is ItemActionDynamicMelee.ItemActionDynamicMeleeData itemActionDynamicMeleeData && itemActionDynamicMeleeData.Equals(iData) && (entity.inventory.holdingItem.Actions[actionIndex] as ItemActionDynamicMelee).Raycast(itemActionDynamicMeleeData))
			{
