...
public class NetPackageEntityStealth : NetPackage
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public const int cSmellRadiusMin = -1;
	...
	{
		id = player.entityId;
		data = (ushort)(2 | (Utils.FastMin(_smellRadius - -1, 255) << 8));
		if (_eating)
		{
			data |= 4;
		}
	...
	public override void ProcessPackage(World _world, GameManager _callbacks)
	{
		...
		else if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			if ((data & 2) > 0)
			{
				entityPlayer.Stealth.SetSmellRadiusTarget((data >> 8) + -1, (data & 4) > 0, (data & 8) > 0);
			}
			else
			{
