...
public class BlockLimitTracker
{
	...
	public bool CanAddBlock(BlockValue _blockValue, Vector3i _blockPosition, out eSetBlockResponse _response)
	{
		...
		else if (_blockValue.Block is BlockCompositeTileEntity blockCompositeTileEntity && blockCompositeTileEntity.CompositeData.HasFeature<ITileEntityLootable>() && !playerStorageTracker.CanAdd(_blockPosition))
		{
	...
	public void TryAddTrackedBlock(BlockValue _blockValue, Vector3i _blockPosition, int _entityId)
	{
		...
		else if (_blockValue.Block is BlockCompositeTileEntity blockCompositeTileEntity && blockCompositeTileEntity.CompositeData.HasFeature<ITileEntityLootable>() && playerStorageTracker.TryAddBlock(_blockPosition))
		{
			TriggerSave();
	...
	public void TryRemoveOrReplaceBlock(BlockValue _oldBlockValue, BlockValue _newBlockValue, Vector3i _blockPosition)
	{
		...
		else if (_oldBlockValue.Block is BlockCompositeTileEntity blockCompositeTileEntity && blockCompositeTileEntity.CompositeData.HasFeature<ITileEntityLootable>() && (!(_newBlockValue.Block is BlockCompositeTileEntity blockCompositeTileEntity2) || !blockCompositeTileEntity2.CompositeData.HasFeature<ITileEntityLootable>()) && playerStorageTracker.RemoveBlock(_blockPosition))
		{
			TriggerSave();
			Log.Out("{0}/{1} Storage Blocks", playerStorageTracker.blockLocations.Count, playerStorageTracker.limit);
		}
