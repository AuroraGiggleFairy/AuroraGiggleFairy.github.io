...
public class EntityPlayer : EntityAlive
{
	...
	public PersistentPlayerData PersistentPlayerData => GameManager.Instance.persistentPlayers?.GetPlayerDataFromEntityID(entityId);
	...
	public string PlayerDisplayName
	{
		get
		{
			...
			cachedPlayerName = PersistentPlayerData.PlayerName;
			return cachedPlayerName.DisplayName;
		}
	}
	...
	public bool IsFriendOfLocalPlayer
	{
		get
		{
			...
			{
				return persistentLocalPlayer.IsAlly(playerDataFromEntityID);
			}
			return false;
		}
	}
	...
	public int unModifiedGameStage
	{
		get
		{
			...
			return Mathf.FloorToInt(EffectManager.GetValue(PassiveEffects.GameStage, null, ((float)Progression.Level + num) * difficultyBonus, this));
		}
	}
	...
	public int gameStage
	{
		get
		{
			...
			if (biomeStandingOn != null)
			{
				...
				float num4 = biomeStandingOn.GameStageMod * BiomeGameStageModifier;
				float num5 = biomeStandingOn.GameStageBonus * BiomeGameStageModifier;
				return Mathf.FloorToInt(EffectManager.GetValue(PassiveEffects.GameStage, null, ((float)Progression.Level * (1f + num4 + num2) + num + num5 + num3) * difficultyBonus, this) * GlobalGameStageModifier);
			}
			return Mathf.FloorToInt(EffectManager.GetValue(PassiveEffects.GameStage, null, ((float)Progression.Level + num) * difficultyBonus, this) * GlobalGameStageModifier);
		}
	}
	...
	{
		base.Awake();
		CarryCapacity = (int)EffectManager.GetValue(PassiveEffects.BagSize, null, 45f, this, null, default(FastTags<TagGroup.Global>), calcEquipment: false, calcHoldingItem: false);
	...
	public override void InitStats()
	{
		...
		startOfFrameStats = new PlayerEntityStats(this);
	}
	...
	public int GetTraderStage(int tier)
	{
		...
		return Mathf.FloorToInt(EffectManager.GetValue(PassiveEffects.TraderStage, null, (float)Progression.Level * (1f + num), this) * GlobalTraderStageModifier);
	}
	...
	public int GetLootStage(float containerMod, float containerBonus)
	{
		...
		if (prefab != null && prefab.prefab.DifficultyTier > 0)
		{
			...
			num = LootManager.POITierMod[Mathf.Min(a, LootManager.POITierMod.Length - 1)] * POITierLootStageModifier;
			num2 = LootManager.POITierBonus[Mathf.Min(a, LootManager.POITierBonus.Length - 1)] * POITierLootStageModifier;
		}
		if (biomeStandingOn != null)
		{
			float num3 = biomeStandingOn.LootStageMod * BiomeLootStageModifier;
			float num4 = biomeStandingOn.LootStageBonus * BiomeLootStageModifier;
			int num5 = Mathf.FloorToInt(EffectManager.GetValue(PassiveEffects.LootStage, null, (float)Progression.Level * (1f + num + num3 + containerMod) + (num2 + num4 + containerBonus), this));
			if (biomeStandingOn.LootStageMin != -1)
			{
				num5 = Utils.FastMax(num5, biomeStandingOn.LootStageMin);
			}
			if (GameStats.GetBool(EnumGameStats.BiomeProgression) && biomeStandingOn.LootStageMax != -1)
			{
				num5 = Utils.FastMin(num5, Mathf.FloorToInt(EffectManager.GetValue(PassiveEffects.LootStageMax, null, biomeStandingOn.LootStageMax, this)));
			}
			return num5;
		}
	...
	public override float GetSpeedModifier()
	{
		...
		if (base.IsCrouching)
		{
			if (MovementRunning)
			{
				...
				value = EffectManager.GetValue(PassiveEffects.WalkSpeed, null, num, this);
				value *= SandboxOptionManager.GetFloat(global::SandboxOptions.SandboxOptions.WalkSpeed);
			}
			else
			{
				num = Constants.cPlayerSpeedModifierCrouching;
				value = EffectManager.GetValue(PassiveEffects.CrouchSpeed, null, num, this);
				value *= SandboxOptionManager.GetFloat(global::SandboxOptions.SandboxOptions.CrouchSpeed);
			}
		}
		else if (MovementRunning)
		{
			num = Constants.cPlayerSpeedModifierRunning;
			value = EffectManager.GetValue(PassiveEffects.RunSpeed, null, num, this);
			value *= SandboxOptionManager.GetFloat(global::SandboxOptions.SandboxOptions.RunSpeed);
		}
		else
		{
			num = Constants.cPlayerSpeedModifierWalking;
	...
	public override void StartJumpMotion()
	{
		...
		motion.y = EffectManager.GetValue(PassiveEffects.JumpStrength, null, jumpStrength, this) * base.Stats.Stamina.ValuePercent * SandboxOptionManager.GetFloat(global::SandboxOptions.SandboxOptions.JumpStrength);
	}
	...
	public bool IsFriendsWith(EntityPlayer _other)
	{
		...
		if (playerDataFromEntityID2 != null && playerDataFromEntityID2.IsAlly(playerDataFromEntityID))
		{
			return true;
		}
		return false;
	...
	public override void OnUpdateEntity()
	{
		...
		if (bag != null)
		{
			CarryCapacity = (int)EffectManager.GetValue(PassiveEffects.CarryCapacity, null, 0f, this);
			int num = bag.GetUsedSlotCount() - CarryCapacity;
			Buffs.SetCustomVar("_carrycapacity", CarryCapacity);
			Buffs.SetCustomVar("_encumbrance", Mathf.Max(num, 0f) / (float)(bag.GetSlots().Length - CarryCapacity));
			Buffs.SetCustomVar("_encumberedslots", Mathf.Max(num, 0f));
		}
		PrefabTick();
	}
	...
	public void PrefabTick()
	{
		...
		if (prefab != null && !prefabInfoEntered && !world.IsEditor())
		{
			if (this is EntityPlayerLocal)
			{
				...
				{
					if (prefab.prefab.InfoVolumeList.Count > 0 || prefab.prefab.DifficultyTier >= 0)
					{
		...
		try
		{
			...
			if (entityPlayer != null)
			{
				...
				case 2:
				{
					...
					{
						bool num2 = playerDataFromEntityID2.IsAlly(playerDataFromEntityID);
	...
	public void AddKillXP(EntityAlive killedEntity, ItemValue itemUsed, float xpModifier = 1f)
	{
		...
		if (!isEntityRemote)
		{
			Progression.AddLevelExp(experienceValue, "_xpFromKill", Progression.XPTypes.Kill, useBonus: true, notifyUI: true, entityId, itemUsed);
			bPlayerStatsChanged = true;
		}
		else
		{
			NetPackageEntityAddExpClient package = NetPackageManager.GetPackage<NetPackageEntityAddExpClient>().Setup(entityId, experienceValue, Progression.XPTypes.Kill, itemUsed);
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendPackage(package, _onlyClientsAttachedToAnEntity: false, entityId);
		}
		if (xpModifier == 1f)
		{
