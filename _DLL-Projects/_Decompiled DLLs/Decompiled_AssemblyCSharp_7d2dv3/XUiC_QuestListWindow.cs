...
public class XUiC_QuestListWindow : XUiController
{
	...
	public void showOnMapBtn_OnPress(XUiController _sender, int _mouseButton)
	{
		if (questList.SelectedEntry != null)
		{
			...
			if (quest.HasPosition)
			{
				...
				((XUiC_MapArea)xui.GetWindow("mapArea").Controller).PositionMapAt(quest.Position);
			}
			else
			{
				GameManager.ShowTooltip(xui.playerUI.entityPlayer, Localization.Get("ttQuestNoLocation"));
			}
		}
	}
	...
	public void trackBtn_OnPress(XUiController _sender, int _mouseButton)
	{
		...
		if (questList.SelectedEntry != null)
		{
			...
			if (quest.Active)
			{
				...
				xui.playerUI.entityPlayer.QuestJournal.TrackedQuest = (quest.Tracked ? quest : null);
	...
	public void questRemoveBtn_OnPress(XUiController _sender, int _mouseButton)
	{
		if (questList.SelectedEntry != null)
		{
			xui.playerUI.entityPlayer.QuestJournal.RemoveQuest(questList.SelectedEntry.Quest);
	...
	{
		IsDirty = true;
		if (q == xui.QuestTracker.TrackedQuest)
		{
			xui.QuestTracker.TrackedQuest = null;
		}
	}
