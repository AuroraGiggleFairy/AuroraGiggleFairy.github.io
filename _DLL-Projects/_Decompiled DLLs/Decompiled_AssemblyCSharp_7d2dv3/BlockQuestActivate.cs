...
public class BlockQuestActivate : Block
{
	...
	public override bool OnBlockActivated(string _commandName, WorldBase _world, Vector3i _blockPos, BlockValue _blockValue, EntityPlayerLocal _player)
	{
		...
		if (!(_commandName == "activate"))
		{
			...
			{
				XUiC_TriggerProperties.Show(_player.PlayerUI.xui, _blockPos, _showTriggers: true, _showTriggeredBy: false);
				return true;
			}
			return false;
		}
		...
		if (activateTime > 0f)
		{
			...
			XUiC_Timer.OpenTimer(playerUI.xui, activateTime, timerEventData);
			Manager.BroadcastPlay(_blockPos.ToVector3(), "generator_start");
			_blockValue.meta2 = 2;
			(_world as World).SetBlockRPC(_blockPos, _blockValue);
			setControllerState(_world, null, _blockPos, QuestGeneratorController.GeneratorStates.RebootState);
		}
		else
		{
			QuestEventManager.Current.BlockActivated(_blockValue.Block.GetBlockName(), _blockPos);
			HandleTrigger(_player, (World)_world, _blockPos, _blockValue);
			Manager.BroadcastPlay(_blockPos.ToVector3(), "generator_start");
			_blockValue.meta2 = 4;
			(_world as World).SetBlockRPC(_blockPos, _blockValue);
		}
	...
	public void EventData_AlternateEvent(TimerEventData timerData)
	{
		...
		EntityPlayer entityPlayer = (EntityPlayer)obj[2];
	...
	public void EventData_CloseEvent(TimerEventData timerData)
	{
		...
		blockValue.meta2 = 1;
		world.SetBlockRPC(vector3i, blockValue);
		setControllerState(world, null, vector3i, QuestGeneratorController.GeneratorStates.Off);
	}
	...
	public void EventData_Event(TimerEventData timerData)
	{
		...
		BlockValue blockValue = (BlockValue)obj[0];
		Vector3i vector3i = (Vector3i)obj[1];
		EntityPlayerLocal player = obj[2] as EntityPlayerLocal;
		World world = GameManager.Instance.World;
		QuestEventManager.Current.BlockActivated(blockValue.Block.GetBlockName(), vector3i);
		HandleTrigger(player, world, vector3i, blockValue);
		blockValue.meta2 = 4;
		world.SetBlockRPC(vector3i, blockValue);
		setControllerState(world, null, vector3i, QuestGeneratorController.GeneratorStates.On);
	}
	...
	public override void OnBlockValueChanged(WorldBase _world, Chunk _chunk, Vector3i _blockPos, BlockValue _oldBlockValue, BlockValue _newBlockValue)
	{
		...
		setControllerState(_world, _chunk, _blockPos, meta);
	}
	...
	public override void OnTriggerAddedFromPrefab(BlockTrigger _trigger, Vector3i _blockPos, BlockValue _blockValue, FastTags<TagGroup.Global> _questTags)
	{
		...
		if (!ValidQuestTags.IsEmpty)
		{
			...
			{
				_blockValue.meta2 = 0;
				world.SetBlock(_trigger.ToWorldPos(), _blockValue, bNotify: true, updateLight: false);
				setControllerState(world, _trigger.Chunk, _trigger.LocalChunkPos, QuestGeneratorController.GeneratorStates.OnNoQuest);
			}
	...
	public override void OnBlockLoaded(WorldBase _world, Vector3i _blockPos, BlockValue _blockValue)
	{
		...
		if (chunkCache != null && chunkCache.GetChunkSync(World.toChunkXZ(_blockPos.x), World.toChunkY(_blockPos.y), World.toChunkXZ(_blockPos.z)) is Chunk chunk)
		{
	...
	public override void OnBlockEntityTransformAfterActivated(WorldBase _world, Vector3i _blockPos, BlockValue _blockValue, BlockEntityData _ebcd)
	{
		...
		if (chunkCache != null && chunkCache.GetChunkSync(World.toChunkXZ(_blockPos.x), World.toChunkY(_blockPos.y), World.toChunkXZ(_blockPos.z)) is Chunk chunk)
		{
			...
			setControllerState(_world, chunk, _blockPos, meta, isInit: true);
		}
	}
	...
	public void SetupForQuest(WorldBase _world, Chunk _chunk, Vector3i _blockPos, BlockValue _blockValue, List<BlockChangeInfo> blockChanges)
	{
		if (!_world.IsEditor() && !_blockValue.ischild)
		{
			...
			setControllerState(_world, _chunk, _blockPos, QuestGeneratorController.GeneratorStates.Off);
		}
	}
