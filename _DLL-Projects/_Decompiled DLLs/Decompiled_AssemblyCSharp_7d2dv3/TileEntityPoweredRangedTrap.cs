...
public class TileEntityPoweredRangedTrap : TileEntityPoweredBlock
{
	...
	{
		if (!_itemStack.CanMoveTo(XUiC_ItemStack.StackLocationTypes.Workstation))
		{
			return false;
		}
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			return (PowerItem as PowerRangedTrap).AddItem(_itemStack);
		}
		for (int i = 0; i < ItemSlots.Length; i++)
		{
			if (ItemSlots[i].IsEmpty())
			{
				ItemSlots[i] = _itemStack;
				return true;
			}
		}
		return false;
	...
	public override void ClientUpdate()
	{
		if (Time.time > updateTime)
		{
			...
			if (block is BlockLauncher blockLauncher)
			{
				blockLauncher.InstantiateProjectile(world, ToWorldPos());
			}
			else if (block is BlockRanged)
			{
				block.ActivateBlock(world, ToWorldPos(), blockValue, base.IsPowered, base.IsPowered);
			}
		}
	}
	...
	public override void OnLockedLocal(bool _success, ILockContext _context, ushort _channel)
	{
		...
		((XUiC_PowerRangedTrapWindowGroup)((XUiWindowGroup)uIForPrimaryPlayer.windowManager.GetWindow("powerrangedtrap")).Controller).TileEntity = this;
		uIForPrimaryPlayer.windowManager.Open("powerrangedtrap", _bModal: true);
	}
}
