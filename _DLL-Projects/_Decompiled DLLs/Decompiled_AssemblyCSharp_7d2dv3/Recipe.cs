...
public class Recipe
{
	...
	{
		craftingTier = GetCraftingTier((EntityPlayer)_ea);
		...
		for (int i = 0; i < ingredients.Count; i++)
		{
			...
			if (UseIngredientModifier)
			{
				...
				if (num > 0)
				{
					num = (int)((float)num * XUiM_Recipes.GetCraftingInputModifier(this));
				}
			}
			if (num == 0)
			{
	...
	{
		for (int num = GetCraftingTier((EntityPlayer)_ea); num >= 0; num--)
		{
			bool flag = true;
			for (int i = 0; i < ingredients.Count; i++)
			{
				...
				if (UseIngredientModifier)
				{
					...
					if (num2 > 0)
					{
						num2 = (int)((float)num2 * XUiM_Recipes.GetCraftingInputModifier(this));
					}
				}
				if (num2 == 0)
				{
