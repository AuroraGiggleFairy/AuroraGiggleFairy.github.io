...
public class XUiC_Radial : XUiController
{
	...
	public class RadialContextBlock : RadialContextAbs
	{
		...
		public RadialContextBlock(Vector3i _blockPos, BlockValue _blockValue, EntityPlayerLocal _entityFocusing, BlockActivationCommand[] _commands, BlockActivationCommand[] _customCommands)
		{
			BlockPos = _blockPos;
	...
	public class RadialContextEntity : RadialContextAbs
	{
		...
		public RadialContextEntity(Vector3i _blockPos, EntityAlive _entityFocusing, Entity _entityFocused, EntityActivationCommand[] _commands)
		{
			BlockPos = _blockPos;
			EntityFocusing = _entityFocusing;
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public const int BLOCK_SHAPE_COMMAND_ALL_BLOCKS = 0;

	[PublicizedFrom(EAccessModifier.Private)]
	public const int BLOCK_SHAPE_COMMAND_COPY_SHAPE = 1;

	[PublicizedFrom(EAccessModifier.Private)]
	public const int BLOCK_SHAPE_COMMAND_ROTATE_SIMPLE = 2;

	[PublicizedFrom(EAccessModifier.Private)]
	public const int BLOCK_SHAPE_COMMAND_ROTATE_ADVANCED = 3;

	[PublicizedFrom(EAccessModifier.Private)]
	public const int BLOCK_SHAPE_COMMAND_ROTATE_ON_FACE = 4;

	[PublicizedFrom(EAccessModifier.Private)]
	public const int BLOCK_SHAPE_COMMAND_ROTATE_AUTO = 5;

	[PublicizedFrom(EAccessModifier.Private)]
	public const int BLOCK_SHAPE_COMMAND_PAINT_COPY_BLOCK = 6;

	[PublicizedFrom(EAccessModifier.Private)]
	public const int BLOCK_SHAPE_COMMAND_PAINT_MATERIALS = 7;

	[PublicizedFrom(EAccessModifier.Private)]
	public const int BLOCK_SHAPE_COMMAND_COPY_SHAPE_AND_ROTATION = 8;

	[PublicizedFrom(EAccessModifier.Private)]
	public const int BLOCK_SHAPE_COMMAND_PLACEMENT_VOXEL = 9;

	[PublicizedFrom(EAccessModifier.Private)]
	public const int BLOCK_SHAPE_COMMAND_PLACEMENT_FREE = 10;
	...
	public override void Init()
	{
		...
		xui.RadialWindow = this;
	}

	public override void Cleanup()
	{
		base.Cleanup();
		xui.RadialWindow = null;
	}
	...
	public override void OnOpen()
	{
		...
		if (!showingCallouts)
		{
			...
			UIUtils.ButtonIcon button = (radialButtonIsLeftSide(xui.playerUI.playerInput) ? UIUtils.ButtonIcon.RightStick : UIUtils.ButtonIcon.LeftStick);
			xui.CalloutWindow.ClearCallouts(XUiC_GamepadCalloutWindow.CalloutType.Menu);
			xui.CalloutWindow.AddCallout(button, "igcoRadialHighlight", XUiC_GamepadCalloutWindow.CalloutType.Menu);
			xui.CalloutWindow.AddCallout(button, "igcoRadialSelect", XUiC_GamepadCalloutWindow.CalloutType.Menu);
			xui.CalloutWindow.EnableCallouts(XUiC_GamepadCalloutWindow.CalloutType.Menu);
		}
		if (PlatformManager.NativePlatform.Input.CurrentInputStyle != PlayerInputManager.InputStyle.Keyboard)
		{
			xui.playerUI.entityPlayer.SetControllable(_b: false);
			xui.playerUI.CursorController.SetCursorHidden(_hidden: true);
		}
		xui.playerUI.entityPlayer.ClearMovementInputs();
	}
	...
	{
		bool flag = xui.playerUI.entityPlayer != null && xui.playerUI.entityPlayer.AttachedToEntity is EntityVehicle;
	...
	public void SetCurrentBlockData(WorldBase _world, Vector3i _blockPos, BlockValue _blockValue, EntityPlayerLocal _entityFocusing)
	{
		ResetRadialEntries();
		BlockActivationCommand[] blockActivationCommands = _blockValue.Block.GetBlockActivationCommands(_world, _blockValue, _blockPos, _entityFocusing);
		...
		SetCommonData(UIUtils.GetButtonIconForAction(xui.playerUI.playerInput.Activate), handleBlockCommand, new RadialContextBlock(_blockPos, _blockValue, _entityFocusing, blockActivationCommands, customCmds));
	}
	...
	public void handleBlockCommand(XUiC_Radial _sender, int _commandIndex, RadialContextAbs _context)
	{
		...
		{
			radialContextBlock.BlockValue.Block.OnBlockActivated(radialContextBlock.Commands[_commandIndex].text, GameManager.Instance.World, radialContextBlock.BlockPos, radialContextBlock.BlockValue, radialContextBlock.EntityFocusing);
			return;
		}
		...
		if (blockActivationCommand.activateTime > 0f)
		{
			...
			XUiC_Timer.OpenTimer(playerUI.xui, blockActivationCommand.activateTime, timerEventData);
		}
		else
		{
	...
	public void SetCurrentEntityData(Entity _entity, EntityAlive _entityFocusing)
	{
		ResetRadialEntries();
		EntityPlayerLocal playerFocusing = _entityFocusing as EntityPlayerLocal;
		_entity.UpdateActivationCommands(playerFocusing);
		EntityActivationCommand[] activationCommands = _entity.GetActivationCommands();
		List<EntityActivationCommand> list = new List<EntityActivationCommand>(activationCommands.Length);
		for (int i = 0; i < activationCommands.Length; i++)
		{
			if (activationCommands[i].enabled)
			{
				int count = list.Count;
				list.Add(activationCommands[i]);
				CreateRadialEntry(count, $"ui_game_symbol_{activationCommands[i].icon}", "UIAtlas", "", activationCommands[i].commandText);
			}
		}
		if (list.Count != 0)
		{
			SetCommonData(UIUtils.GetButtonIconForAction(xui.playerUI.playerInput.Activate), handleEntityCommand, new RadialContextEntity(default(Vector3i), _entityFocusing, _entity, list.ToArray()));
		}
	}
	...
	{
		if (_context is RadialContextEntity radialContextEntity && _commandIndex >= 0 && _commandIndex < radialContextEntity.Commands.Length)
		{
			EntityPlayerLocal playerFocusing = radialContextEntity.EntityFocusing as EntityPlayerLocal;
			EntityActivationCommand command = radialContextEntity.Commands[_commandIndex];
			if (radialContextEntity.EntityFocused.AllowActivationCommand(command.commandId, playerFocusing))
			{
				radialContextEntity.EntityFocused.ActivateEntityCommand(command, playerFocusing);
			}
		}
	}
	...
	public void SetupMenuData()
	{
		...
		if (ChallengeJournal.AllowChallenges || World.BiomeProgressionEnabled)
		{
		...
		SetCommonData(UIUtils.GetButtonIconForAction(xui.playerUI.playerInput.Inventory), handleMenuCommand);
	}
	...
	public void handleMenuCommand(XUiC_Radial _sender, int _commandIndex, RadialContextAbs _context)
	{
		...
		case 8:
			...
			break;
		case 9:
			xui.playerUI.windowManager.Open(XUiC_Chat.ID, _bModal: true);
			break;
		default:
			XUiC_WindowSelector.OpenSelectorAndWindow(entityPlayer, "crafting");
			break;
	...
	public void SetupToolbeltMenu(int _direction)
	{
		...
		SetCommonData(UIUtils.GetButtonIconForAction(xui.playerUI.playerInput.Swap), HandleToolbeltCommand);
	}
	...
	public void HandleToolbeltCommand(XUiC_Radial _sender, int _commandIndex, RadialContextAbs _context)
	{
		...
		else if (_commandIndex >= 0 && xui.playerUI.entityPlayer.inventory.holdingItemIdx != _commandIndex)
		{
			xui.playerUI.entityPlayer.inventory.SetHoldingItemIdx(_commandIndex);
		}
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public bool getBasicBlockInfo(out EntityPlayerLocal _epl, out ItemClassBlock.ItemBlockInventoryData _ibid, out Block _blockHolding, out Block _blockSelectedShape, out bool _hasAutoRotation, out bool _onlySimpleRotations, out bool _hasCopyRotation, out bool _allowShapes, out bool _hasCopyAutoShape, out bool _hasCopyShapeLegacy, out bool _allowPainting, out bool _allowProp, out BlockPlacement.EnumPlacement _blockPlacement)
	{
		...
		_epl = xui.playerUI.entityPlayer;
		...
		_allowProp = false;
		_blockPlacement = BlockPlacement.EnumPlacement.Voxel;
		...
		if (_allowShapes && _epl.HitInfo.bHitValid)
		{
			...
			{
				_hasCopyAutoShape = !block.Properties.TryGetValue("ShapeMenu", out var _value) || !_value.EqualsCaseInsensitive("false") || GameManager.Instance.IsEditMode() || _epl.IsGodMode.Value;
			}
		...
		_allowProp = LaunchPrefs.EnableProps.Value && _ibid.item.IsProp();
		_blockPlacement = _ibid.Placement;
		return true;
	}
	...
	{
		ResetRadialEntries();
		if (!getBasicBlockInfo(out var _epl, out var _, out var _, out var _, out var _hasAutoRotation, out var _onlySimpleRotations, out var _hasCopyRotation, out var _allowShapes, out var _hasCopyAutoShape, out var _hasCopyShapeLegacy, out var _allowPainting, out var _allowProp, out var _))
		{
			SetCommonData(UIUtils.GetButtonIconForAction(xui.playerUI.playerInput.Activate), handleBlockShapeCommand);
			return;
		}
		if (_allowShapes)
		{
		...
		if (_allowProp)
		{
			CreateRadialEntry(9, "ui_game_symbol_placement_voxel", "UIAtlas", "", Localization.Get("xuiPlacementVoxel"));
			CreateRadialEntry(10, "ui_game_symbol_placement_free", "UIAtlas", "", Localization.Get("xuiPlacementFree"));
		}
		SetCommonData(UIUtils.GetButtonIconForAction(xui.playerUI.playerInput.Activate), handleBlockShapeCommand, new RadialContextHoldingSlotIndex(_epl.inventory.holdingItemIdx), -1, _hasSpecialActionPriorToRadialVisibility: true, RadialValidSameHoldingSlotIndex);
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void handleBlockShapeCommand(XUiC_Radial _sender, int _commandIndex, RadialContextAbs _context)
	{
		if (!getBasicBlockInfo(out var _epl, out var _ibid, out var _blockHolding, out var _blockSelectedShape, out var _hasAutoRotation, out var _onlySimpleRotations, out var _hasCopyRotation, out var _allowShapes, out var _hasCopyAutoShape, out var _hasCopyShapeLegacy, out var _, out var _, out var _))
		{
			return;
		}
		switch (_commandIndex)
		...
		case 0:
			if (_allowShapes)
			{
				xui.GetChildByType<XUiC_ShapesWindow>().ItemValue = _epl.inventory.holdingItemItemValue.Clone();
				xui.playerUI.windowManager.Open("shapes", _bModal: true);
			}
			break;
		case -1:
		...
		case 6:
			...
			break;
		case 7:
			xui.playerUI.windowManager.Open("materials", _bModal: true);
			break;
		case 8:
			...
			break;
		case 9:
			_ibid.Placement = BlockPlacement.EnumPlacement.Voxel;
			break;
		case 10:
			_ibid.Placement = BlockPlacement.EnumPlacement.Free;
			break;
		}
		...
		void copyShape(EntityPlayerLocal entityPlayerLocal, bool flag, Block _block, Block _targetedBlock, ItemClassBlock.ItemBlockInventoryData itemBlockInventoryData)
		{
			...
			if (num >= 0)
			{
				...
				XUiC_Toolbelt childByType = ((XUiWindowGroup)xui.playerUI.windowManager.GetWindow("toolbelt")).Controller.GetChildByType<XUiC_Toolbelt>();
				if (childByType != null)
				{
	...
	public void SetupQuickActionsMenu(EntityPlayerLocal _epl)
	{
		...
		SetCommonData(UIUtils.GetButtonIconForAction(xui.playerUI.playerInput.PermanentActions.QuickMenu), HandleQuickActionsCommand);
	}
