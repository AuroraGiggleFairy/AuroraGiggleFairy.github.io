...
public class Chunk : IChunk, IBlockAccess, IMemoryPoolableObject
{
	...
	public bool StopStabilityCalculation
	{
		get
		{
			return stopStabilityCalculation;
		}
		set
		{
			if (value)
			{
				GameManager.Instance.World?.m_ChunkManager?.OnChunkStabilityCalculationEnabled?.Invoke(this);
			}
			stopStabilityCalculation = value;
		}
	}
	...
	public void read(PooledBinaryReader _br, uint _version, bool _bNetworkRead)
	{
		...
		if (_version < 32)
		{
			throw new Exception("Chunk version " + _version + " not supported!");
		}
		for (int i = 0; i < 64; i++)
		{
			if (_br.ReadBoolean())
		...
		default:
		{
			for (int k = 0; k < 1; k++)
			{
				chnTextures[k].Read(_br, _version, _bNetworkRead);
			}
			break;
		}
		case 35u:
		...
		if (!_bNetworkRead && _version > 37)
		{
			...
			for (int num6 = 0; num6 < num5; num6++)
			{
				...
				if (num7 < 0)
				{
					Log.Error("chunk sleeper volumeId invalid {0}", num7);
				}
				else
				{
					AddSleeperVolumeId(num7);
				}
			}
		}
		...
		{
			triggerVolumes.Clear();
			int num8 = _br.ReadByte();
			for (int num9 = 0; num9 < num8; num9++)
			{
				int num10 = _br.ReadInt32();
				if (num10 < 0)
				{
					Log.Error("chunk trigger volumeId invalid {0}", num10);
				}
				else
				{
					AddTriggerVolumeId(num10);
				}
			}
		}
		if (_version >= 45)
		{
			wallVolumes.Clear();
			int num11 = _br.ReadByte();
			for (int num12 = 0; num12 < num11; num12++)
			{
				int num13 = _br.ReadInt32();
				if (num13 < 0)
				{
					Log.Error("chunk wall volumeId invalid {0}", num13);
				}
				else
				{
					AddWallVolumeId(num13);
				}
			}
		}
		if (_bNetworkRead)
		...
		lock (tickedBlocks)
		{
			...
			{
				ChunkBlockLayer chunkBlockLayer2 = m_BlockLayers[num14];
		...
		if (_version > 39)
		{
			int num16 = _br.ReadInt16();
			insideDevices.Capacity = num16;
			byte x2 = 0;
			byte z2 = 0;
			int num17 = 0;
	...
	public BlockValue SetBlock(WorldBase _world, int x, int y, int z, BlockValue _blockValue, bool _notifyAddChange = true, bool _notifyRemove = true, bool _fromReset = false, bool _poiOwned = false, int _changedByEntityId = -1)
	{
		...
		if (_notifyAddChange)
		{
			...
			if (block != null)
			{
				...
				{
					block.OnBlockValueChanged(_world, this, vector3i, blockValue, _blockValue);
					if (_fromReset)
					{
	...
	public PropValue AddProp(Vector3 position, Quaternion rotation, Vector3 scale, BlockValue blockValue)
	{
		if (blockValue.isair)
		{
			Log.Warning($"Can not create a prop out of an air block. BlockValue({blockValue})");
			return PropValue.AIR;
		}
		if (!IsInChunk(position))
		{
			Log.Warning($"Prop being created is not contained within this chunk. Position: {position}");
		}
	...
	public PropValue GetProp(int chunkX, int chunkZ, int propId)
	{
		...
		if (!props.TryGetValue(propId, out var value))
		{
			return PropValue.AIR;
		}
		return value;
	...
	public PropValue SetProp(int? propId = null, Vector3? position = null, Quaternion? rotation = null, Vector3? scale = null, BlockValue? blockValue = null)
	{
		if (!propId.HasValue)
		{
			if (!position.HasValue || !rotation.HasValue || !scale.HasValue || !blockValue.HasValue)
			{
				Log.Error($"Creating a new prop requires valid position, rotation, scale and block values. Position{position}, Rotation{rotation}, Scale{scale}, BlockValue({blockValue})");
				return PropValue.AIR;
			}
			return AddProp(position.Value, rotation.Value, scale.Value, blockValue.Value);
		}
		if (!props.TryGetValue(propId.Value, out var value))
		{
			Log.Error($"Could not find prop with id {propId.Value}.");
			return PropValue.AIR;
		}
		bool flag = false;
		if (blockValue.HasValue)
		{
			...
			{
				props.Remove(propId.Value);
				return PropValue.AIR;
			}
			if (value.blockValue.rawData != blockValue.Value.rawData)
			{
				value.blockValue.rawData = blockValue.Value.rawData;
				flag = true;
			}
			if (value.blockValue.damage != blockValue.Value.damage)
			{
				value.blockValue.damage = blockValue.Value.damage;
				flag = true;
			}
		}
		if (position.HasValue)
		{
			if (!IsInChunk(position.Value))
			{
				Log.Warning($"Prop being updated is not contained within this chunk. Position: {position.Value}");
			}
		...
		if (rotation.HasValue)
		{
			...
			if (value.rotation != normalized)
			{
				value.rotation = normalized;
				flag = true;
			}
		}
		if (scale.HasValue)
		{
			Vector3 value2 = scale.Value;
			if (value.scale != value2)
			{
				value.scale = value2;
				flag = true;
			}
	...
	public void OnLoad(World _world)
	{
		if (!_world.IsRemote())
		{
			for (int i = 0; i < entityStubs.Count; i++)
			{
				...
				{
					SpawnEntityAsync(_world, entityCreationData);
				}
			}
		...
		for (int j = 0; j < m_BlockLayers.Length; j++)
		{
			...
			{
				m_BlockLayers[j].OnLoad(_world, X * 16, j * 4, Z * 16);
			}
		}
		for (int k = 0; k < tileEntities.list.Count; k++)
		{
			tileEntities.list[k].OnLoad();
		}
	}
	...
	public void OnUnload(WorldBase _world)
	{
		...
		if (!_world.IsRemote())
		{
			...
			if (pendingEntityCreateOps.Count > 0)
			{
				EntityAsyncManager.EntityCreateHandle[] array = new EntityAsyncManager.EntityCreateHandle[pendingEntityCreateOps.Count];
				pendingEntityCreateOps.CopyTo(array);
			...
			for (int k = 0; k < 16; k++)
			{
				if (entityLists[k].Count != 0)
				{
					_world.UnloadEntities(entityLists[k]);
				}
			}
			ProfilerEnd();
			removeExpiredCustomChunkDataEntries(_world.GetWorldTime());
		...
		for (int l = 0; l < tileEntities.list.Count; l++)
		{
			tileEntities.list[l].OnUnload(GameManager.Instance.World);
		}
		ProfilerEnd();
		RemoveBlockEntityTransforms();
		ProfilerBegin("block layers");
		for (int m = 0; m < m_BlockLayers.Length; m++)
		{
			if (m_BlockLayers[m] != null)
			{
				m_BlockLayers[m].OnUnload(_world, X * 16, m * 4, Z * 16);
			}
		}
		ProfilerEnd();
		ProfilerBegin("water");
	...
	{
		if (InProgressUnloading)
		{
			Log.Error($"Spawning entity onto chunk ({X},{Z}) which is unloading");
			return;
		}
		EntityAsyncManager.EntityCreateHandle item = _world.entityAsyncManager.StartCreateEntity(_ecd, [PublicizedFrom(EAccessModifier.Internal)] (EntityAsyncManager.EntityCreateHandle handle) =>
		{
			pendingEntityCreateOps.Remove(handle);
			_onEntityCreated?.Invoke(handle.Entity);
			_world.SpawnEntityInWorld(handle.Entity);
		});
		pendingEntityCreateOps.Add(item);
	}
	...
	public void OnDisplayBlockEntities(World _world, Transform _entityBlocksParentT, ChunkCluster _chunkCluster)
	{
				...
				if (!IsInternalBlocksCulled || block.type == blockEntityData.blockValue.type)
				{
					...
					else
					{
						...
						if ((bool)objectForType)
						{
							...
							block2.OnBlockEntityTransformAfterActivated(_world, blockEntityData.pos, GetBlock(vector3i.x, vector3i.y, vector3i.z), blockEntityData);
							ProfilerEnd();
		...
		HashSet<int> hashSet = new HashSet<int>(propEntities.Keys);
		List<(int, PropValue)> list = new List<(int, PropValue)>();
		foreach (KeyValuePair<int, PropValue> prop in props)
		{
			prop.Deconstruct(out var key, out var value);
			int num6 = key;
			PropValue item = value;
			BlockValue blockValue = item.blockValue;
		...
		{
			if (propEntities.Remove(item4, out var value2))
			{
				propEntitiesToRemove.Add(value2);
			}
		}
		foreach (var item5 in list)
		{
			int item2 = item5.Item1;
			PropValue item3 = item5.Item2;
			BlockValue blockValue2 = item3.blockValue;
			Block block3 = blockValue2.Block;
			if (!(block3.shape is BlockShapeModelEntity blockShapeModelEntity2))
			{
				continue;
			}
			if (!propEntities.TryGetValue(item2, out var value3))
			{
				value3 = (propEntities[item2] = new PropEntityData());
			}
			PropValue propValue = value3.propValue;
			if (item3 == propValue)
			{
				continue;
			}
			value3.propValue = item3;
			if (value3.bHasTransform)
			{
				occlusionTs.Add(value3.transform);
				if (OcclusionManager.Instance.cullChunkEntities)
				{
					OcclusionManager.Instance.RemoveChunkTransforms(this, occlusionTs);
				}
				occlusionTs.Clear();
				GameObjectPool.Instance.PoolObject(value3.transform.gameObject);
				value3.bHasTransform = false;
				value3.transform = null;
			}
			float num7 = 0f;
			...
			{
				num7 = _world.GetDecorationOffsetY(vector3i2);
			}
			...
			ProfilerBegin("BE setup");
			Transform transform2 = (value3.transform = objectForType2.transform);
			value3.bHasTransform = true;
			transform2.SetParent(_entityBlocksParentT, worldPositionStays: false);
			transform2.localScale = Vector3.one;
			transform2.SetLocalPositionAndRotation(localPosition, rotation2);
			if (!objectForType2.TryGetComponent<PropReference>(out var component))
			{
				component = objectForType2.AddComponent<PropReference>();
			}
			component.ChunkPos = new Vector2i(X, Z);
			component.PropId = item2;
			bool isCollideMovement2 = block3.IsCollideMovement;
			int newLayer2 = num;
			if (isCollideMovement2)
			{
				switch (objectForType2.layer)
				{
				case 30:
	...
	public void removeBlockEntitesMarkedForRemoval()
	{
		if (OcclusionManager.Instance.cullChunkEntities)
		{
			...
			foreach (PropEntityData item in propEntitiesToRemove)
			{
				if (item.bHasTransform)
				{
					occlusionTs.Add(item.transform);
				}
			}
			if (occlusionTs.Count > 0)
			{
		...
		foreach (PropEntityData item2 in propEntitiesToRemove)
		{
			if (!item2.bHasTransform)
			{
				continue;
			}
			if ((bool)item2.transform)
			{
				if (item2.transform.TryGetComponent<PropReference>(out var component))
				{
					UnityEngine.Object.Destroy(component);
				}
				GameObjectPool.Instance.PoolObject(item2.transform.gameObject);
			}
			item2.bHasTransform = false;
			item2.transform = null;
		}
		propEntitiesToRemove.Clear();
	}
