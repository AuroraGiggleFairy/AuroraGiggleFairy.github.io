...
public class EntityBackpack : EntityItem
{
	...
	public override void OnUpdateEntity()
	{
		...
		try
		{
			...
			deathUpdateTicks++;
			if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer && !bRemoved && !isEntityRemote && base.transform.position.y + Origin.position.y < 1f)
			{
			...
			if (!SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer || bRemoved || isEntityRemote || RefPlayerId == -1)
			{
				return;
			}
	...
	public override string GetActivationText()
	{
		...
		return string.Format(Localization.Get("lootTooltipTouched"), arg, LocalizedEntityName);
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void RemoveBackpack(string reason)
	{
		LogBackpack("RemoveBackpack empty {0}, reason {1}", bag == null || bag.IsEmpty(), reason);
