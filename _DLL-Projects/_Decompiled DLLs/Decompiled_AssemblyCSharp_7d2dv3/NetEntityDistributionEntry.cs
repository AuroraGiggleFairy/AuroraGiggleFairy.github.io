...
public class NetEntityDistributionEntry
{
		...
		if (num * num + num2 * num2 <= (float)(trackingDistanceThreshold * trackingDistanceThreshold))
		{
			...
			if ((bool)entityAlive)
			{
				...
				if (entityAlive is EntityPlayer)
				{
					...
					SingletonMonoBehaviour<ConnectionManager>.Instance.SendPackage(NetPackageManager.GetPackage<NetPackagePlayerEquipment>().Setup(entityAlive), _onlyClientsAttachedToAnEntity: false, _ep.entityId);
				}
			}
	...
	public void updatePlayerList(List<EntityPlayer> _playerList)
	{
		...
		if (flag)
		{
			...
			if (entityPlayer != null && entityPlayer.bPlayerEquipmentChanged)
			{
				SendToPlayers(NetPackageManager.GetPackage<NetPackagePlayerEquipment>().Setup(entityAlive), trackedEntity.entityId);
				entityAlive.bPlayerEquipmentChanged = false;
			}
			if (flag2)
			{
				encodedPos = vector3i;
