...
public class PersistentPlayerData
{
	...
	public bool IsAlly(PlatformUserIdentifierAbs other)
	{
		return GameManager.Instance.persistentPlayers.Allies.IsAlly(PrimaryId, other);
	}

	public bool IsAlly(PersistentPlayerData other)
	{
		return GameManager.Instance.persistentPlayers.Allies.IsAlly(PrimaryId, other?.PrimaryId);
	}
	...
	public void Write(BinaryWriter stream)
	{
		...
		foreach (KeyValuePair<int, ProtectedBackpack> item in backpacksByID)
		{
			...
			Vector3i position = item.Value.Position;
			stream.Write(position.x);
			stream.Write(position.y);
			stream.Write(position.z);
			stream.Write(item.Value.Timestamp);
		}
	...
	public void Write(XmlElement _root)
	{
		...
		if ((LPBlocks == null || LPBlocks.Count == 0) && backpacksByID.Count == 0 && BedrollPos.y == int.MaxValue && QuestPositions.Count == 0 && OwnedVendingMachinePositions.Count == 0)
		{
			return;
		}
		if (LPBlocks != null)
		...
		foreach (KeyValuePair<int, ProtectedBackpack> item in backpacksByID)
		{
			int key = item.Key;
			Vector3i position = item.Value.Position;
			uint timestamp = item.Value.Timestamp;
		...
		if (QuestPositions != null && QuestPositions.Count > 0)
		{
			...
			foreach (QuestPositionData questPosition in QuestPositions)
			{
				XmlElement xmlElement2 = node.AddXmlElement("position");
				xmlElement2.SetAttribute("id", questPosition.questCode.ToString());
				int positionDataType = (int)questPosition.positionDataType;
				xmlElement2.SetAttribute("positiondatatype", positionDataType.ToString());
				xmlElement2.SetAttrib("pos", $"{questPosition.blockPosition.x},{questPosition.blockPosition.y},{questPosition.blockPosition.z}");
			}
		}
	...
	public static PersistentPlayerData ReadXML(XmlElement root, int readVersion, out HashSet<PlatformUserIdentifierAbs> legacyACL)
	{
		legacyACL = null;
			...
			if (root.HasAttribute("position"))
			{
				...
				persistentPlayerData.Position.z = int.Parse(array[2].Trim());
				{
					foreach (XmlNode childNode in root.ChildNodes)
					{
						...
						if (readVersion == 0 && childNode.Name == "acl")
						{
							if (legacyACL == null)
							{
								legacyACL = new HashSet<PlatformUserIdentifierAbs>();
							}
							...
							{
								legacyACL.Add(platformUserIdentifierAbs2);
							}
						}
