...
public class ConnectionManager : SingletonMonoBehaviour<ConnectionManager>
{
	...
	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public CountdownTimer countdownAnalyticsHeartbeat = new CountdownTimer(300f, _start: false);
	...
	{
		countdownAnalyticsHeartbeat.ResetAndRestart();
	}
	...
	public void Update()
	{
		...
		if (!GameManager.IsDedicatedServer && countdownAnalyticsHeartbeat.HasPassed())
		{
			countdownAnalyticsHeartbeat.ResetAndRestart();
			HeartbeatEventData analyticsEventData = new HeartbeatEventData
			{
	...
	public void PlayerAllowed(string _gameInfo, PlatformLobbyId _platformLobbyId, (PlatformUserIdentifierAbs userId, string token) _platformUserAndToken, (PlatformUserIdentifierAbs userId, string token) _crossplatformUserAndToken)
	{
		...
		void LobbyHostDisconnect(string _message)
		{
			...
			if (_message != null)
			{
				XUiC_MessageBoxWindowGroup.ShowOk(windowManager.playerUI.xui, Localization.Get("mmLblErrorConnectionLost"), _message);
			}
		}
		[PublicizedFrom(EAccessModifier.Internal)]
	...
	public void PlayerDenied(string _reason)
	{
		if (IsClient)
		{
			...
			XUiC_MessageBoxWindowGroup.ShowOk(windowManager.playerUI.xui, Localization.Get("mmLblErrorConnectionDeniedTitle"), _reason);
		}
	}
	...
	public void Net_ConnectionFailed(string _message)
	{
		...
		XUiC_MessageBoxWindowGroup.ShowOk(windowManager.playerUI.xui, Localization.Get("mmLblErrorConnectionFailed"), _message);
		IsConnected = false;
		PlatformManager.MultiPlatform.AntiCheatClient?.DisconnectFromServer();
	}
	...
	public void Net_DisconnectedFromServer(string _reason)
	{
		...
		XUiC_MessageBoxWindowGroup.ShowOk(windowManager.playerUI.xui, Localization.Get("mmLblErrorConnectionLost"), _reason);
	}
