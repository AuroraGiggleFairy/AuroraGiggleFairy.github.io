...
public class ItemActionEat : ItemAction
{
	...
	public override bool ExecuteInstantAction(EntityAlive ent, ItemStack stack, bool isHeldItem, XUiC_ItemStack stackController)
	{
		...
		if (Consume)
		{
			...
			{
				stack.itemValue.UseTimes += EffectManager.GetValue(PassiveEffects.DegradationPerUse, stack.itemValue, 1f, ent, null, stack.itemValue.ItemClass.ItemTags) * ItemAction.ItemDegradationModifier;
				return true;
			}
			if (isHeldItem)
			{
			...
			if (stackController != null)
			{
				...
				{
					stackController.ItemStack = ItemStack.Empty;
				}
		...
		if (CreateItem != null && CreateItemCount > 0 && (!UseJarRefund || SandboxOptionManager.GetFloat(global::SandboxOptions.SandboxOptions.JarRefund) > entityPlayer.rand.RandomRange(1f)))
		{
	...
	{
		if (!(_actionData is MyInventoryData { bEatingStarted: not false } myInventoryData))
		{
			return;
		}
		...
		if (Consume)
		{
			...
			{
				_actionData.invData.itemValue.UseTimes += EffectManager.GetValue(PassiveEffects.DegradationPerUse, myInventoryData.invData.itemValue, 1f, holdingEntity, null, myInventoryData.invData.itemValue.ItemClass.ItemTags) * ItemAction.ItemDegradationModifier;
				return;
			}
		...
		if (CreateItem != null && CreateItemCount > 0 && (!UseJarRefund || SandboxOptionManager.GetFloat(global::SandboxOptions.SandboxOptions.JarRefund) > holdingEntity.rand.RandomRange(1f)))
		{
			ItemStack itemStack = new ItemStack(ItemClass.GetItem(CreateItem), CreateItemCount);
	...
	public override void OnHoldingUpdate(ItemActionData _actionData)
	{
		...
		if ((double)PercentDone(_actionData) >= 1.0)
		{
			consume(actionData);
		}
	}
