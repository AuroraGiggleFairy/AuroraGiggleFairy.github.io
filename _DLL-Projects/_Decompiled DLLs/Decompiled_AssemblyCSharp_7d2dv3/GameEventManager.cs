...
public class GameEventManager
{
	...
	public class SpawnedBlocksEntry
	{
		...
		public bool TryRemoveBlocks()
		{
			...
			for (int num = BlockList.Count - 1; num >= 0; num--)
			{
				if (world.GetChunkFromWorldPos(BlockList[num], ref chunk))
				{
					if (list == null)
					{
						list = new List<BlockChangeInfo>();
					}
					list.Add(new BlockChangeInfo(BlockList[num], BlockValue.Air, _updateLight: false));
					BlockList.RemoveAt(num);
				}
			}
			if (list != null)
	...
	{
		return HandleAction(name, requester, entity, twitchActivated, ActionTarget.None, extraData, tag, crateShare, allowRefunds, sequenceLink, ownerSeq);
	}
	...
	public bool HandleAction(string name, EntityPlayer requester, Entity entity, bool twitchActivated, ActionTarget target, string extraData = "", string tag = "", bool crateShare = false, bool allowRefunds = true, string sequenceLink = "", GameEventActionSequence ownerSeq = null, List<Tuple<string, string>> variables = null)
	{
		...
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsClient)
		{
			return HandleActionClient(name, entity, twitchActivated, target, variables, extraData, tag, crateShare, allowRefunds, sequenceLink);
		}
	...
	{
		SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageGameEventRequest>().Setup(name, entity ? entity.entityId : (-1), twitchActivated, target, variables, extraData, tag, crateShare, allowRefunds, sequenceLink));
		return true;
	}
	...
	public void SetGameEventFlag(GameEventFlagTypes flag, bool value, float duration, bool isPermanent)
	{
		if (value)
		{
			bool flag2 = false;
			...
			{
				GameEventFlag gameEventFlag = GameEventFlags[i];
	...
	public void HandleFlagBuffUpdates(GameEventFlagTypes flag, float deltaTime)
	{
		...
		{
			if (text != "" && !value.Buffs.HasBuff(text))
			{
				value.Buffs.AddBuff(text);
			}
		}
	...
	public void HandleFlagChanged(GameEventFlagTypes flag, bool oldValue, bool newValue)
	{
		...
		switch (flag)
		{
		case GameEventFlagTypes.BigHeadSandbox:
		{
			foreach (Entity value in GameManager.Instance.World.Entities.dict.Values)
			{
				EntityAlive entityAlive3 = value as EntityAlive;
				if (entityAlive3 != null && !(entityAlive3 is EntityPlayer))
				{
					if (newValue)
					{
						entityAlive3.Buffs.AddBuff("sandbox_bighead");
					}
					else
					{
						entityAlive3.Buffs.RemoveBuff("sandbox_bighead");
					}
				}
			}
			break;
		...
		case GameEventFlagTypes.BigHead:
			foreach (Entity value2 in GameManager.Instance.World.Entities.dict.Values)
			{
				EntityAlive entityAlive5 = value2 as EntityAlive;
				if (entityAlive5 != null && !(entityAlive5 is EntityPlayer))
				{
					if (newValue)
					{
						entityAlive5.Buffs.AddBuff("twitch_bighead");
					}
					else
					{
						entityAlive5.Buffs.RemoveBuff("twitch_bighead");
					}
				}
			}
			{
				foreach (EntityPlayer value3 in GameManager.Instance.World.Players.dict.Values)
				{
					if (newValue)
					{
						value3.Buffs.AddBuff("twitch_buffBigHead");
					}
					else
					{
						value3.Buffs.RemoveBuff("twitch_buffBigHead");
					}
				}
				break;
			}
		case GameEventFlagTypes.Dancing:
			foreach (Entity value4 in GameManager.Instance.World.Entities.dict.Values)
			{
				EntityAlive entityAlive6 = value4 as EntityAlive;
				if (entityAlive6 != null && !(entityAlive6 is EntityPlayer))
				{
					if (newValue)
					{
						entityAlive6.Buffs.AddBuff("twitch_dance");
					}
					else
					{
						entityAlive6.Buffs.RemoveBuff("twitch_dance");
					}
				}
			}
			{
				foreach (EntityPlayer value5 in GameManager.Instance.World.Players.dict.Values)
				{
					if (newValue)
					{
						value5.Buffs.AddBuff("twitch_buffDance");
					}
					else
					{
						value5.Buffs.RemoveBuff("twitch_buffDance");
					}
				}
				break;
			}
		case GameEventFlagTypes.BucketHead:
			foreach (Entity value6 in GameManager.Instance.World.Entities.dict.Values)
			{
				EntityAlive entityAlive4 = value6 as EntityAlive;
				if (entityAlive4 != null && !(entityAlive4 is EntityPlayer) && !(entityAlive4 is EntityVehicle))
				{
					if (newValue)
					{
						entityAlive4.Buffs.AddBuff("twitch_buckethead");
					}
					else
					{
						entityAlive4.Buffs.RemoveBuff("twitch_buckethead");
					}
				}
			}
			{
				foreach (EntityPlayer value7 in GameManager.Instance.World.Players.dict.Values)
				{
					if (newValue)
					{
						value7.Buffs.AddBuff("twitch_buffBucketHead");
					}
					else
					{
						value7.Buffs.RemoveBuff("twitch_buffBucketHead");
					}
				}
				break;
			}
		case GameEventFlagTypes.TinyZombies:
			foreach (Entity value8 in GameManager.Instance.World.Entities.dict.Values)
			{
				EntityAlive entityAlive2 = value8 as EntityAlive;
				if (entityAlive2 != null && entityAlive2 is EntityZombie)
				{
					if (newValue)
					{
						entityAlive2.Buffs.AddBuff("twitch_tiny");
					}
					else
					{
						entityAlive2.Buffs.RemoveBuff("twitch_tiny");
					}
				}
			}
			{
				foreach (EntityPlayer value9 in GameManager.Instance.World.Players.dict.Values)
				{
					if (newValue)
					{
						value9.Buffs.AddBuff("twitch_buffTinyZombies");
					}
					else
					{
						value9.Buffs.RemoveBuff("twitch_buffTinyZombies");
					}
				}
				break;
			}
		case GameEventFlagTypes.TinyZombiesSandbox:
		{
			foreach (Entity value10 in GameManager.Instance.World.Entities.dict.Values)
			{
				EntityAlive entityAlive = value10 as EntityAlive;
				if (entityAlive != null && entityAlive is EntityZombie)
				{
					if (newValue)
					{
						entityAlive.Buffs.AddBuff("sandbox_tiny");
					}
					else
					{
						entityAlive.Buffs.RemoveBuff("sandbox_tiny");
					}
				}
			}
			break;
	...
	public void HandleSpawnModifier(EntityAlive alive)
	{
		for (int i = 0; i < GameEventFlags.Count; i++)
		{
			...
			case GameEventFlagTypes.BigHeadSandbox:
				...
				{
					alive.Buffs.AddBuff("sandbox_bighead");
				}
				break;
			case GameEventFlagTypes.BigHead:
				if (alive != null && !(alive is EntityPlayer))
			...
			case GameEventFlagTypes.TinyZombiesSandbox:
				if (alive != null && alive is EntityZombie)
				{
					alive.Buffs.AddBuff("sandbox_tiny");
				}
				break;
			}
		}
	...
	public static int GetIntValue(EntityAlive alive, string value, int defaultValue = 0)
	{
		...
		{
			value.IndexOf('{');
			string[] array = value.Substring(1, value.Length - 2).ToLower().Split('.');
			if (array.Length == 2 && array[0] == "sandbox" && Enum.TryParse<global::SandboxOptions.SandboxOptions>(array[1], ignoreCase: true, out var result))
			{
				return SandboxOptionManager.GetInt(result);
			}
			return 0;
		}
		if (value.Contains("-"))
		{
			...
			int min = StringParsers.ParseSInt32(array2[0]);
			int maxExclusive = StringParsers.ParseSInt32(array2[1]) + 1;
	...
	public static float GetFloatValue(EntityAlive alive, string value, float defaultValue = 0f)
	{
		...
		if (value.StartsWith("{") && value.EndsWith("}"))
		{
			...
			if (array.Length == 2 && array[0] == "sandbox" && Enum.TryParse<global::SandboxOptions.SandboxOptions>(array[1], ignoreCase: true, out var result))
			{
				return SandboxOptionManager.GetFloat(result);
			}
			return 0f;
		}
		if (value.Contains("-"))
		{
			string[] array2 = value.Split('-');
			float min = StringParsers.ParseSInt32(array2[0]);
			float maxExclusive = StringParsers.ParseSInt32(array2[1]) + 1;
			return instance.Random.RandomRange(min, maxExclusive);
		}
		float _result = 0f;
