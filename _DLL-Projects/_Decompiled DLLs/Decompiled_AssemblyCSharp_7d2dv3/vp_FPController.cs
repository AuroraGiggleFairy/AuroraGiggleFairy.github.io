...
public class vp_FPController : vp_Component
{
	...
	public virtual void FixedMove()
	{
		...
		if (flag)
		{
			...
			if (moveDirection != Vector3.zero)
			{
				...
				moveDirection = moveDirection.normalized;
				Vector3 origin = m_Transform.position - moveDirection * m_CharacterController.radius;
				origin.y += m_CharacterController.height + 0.18f;
				float maxDistance = m_CharacterController.radius * 2f + 0.5f;
				if (Physics.SphereCast(origin, 0.05f, moveDirection, out var _, maxDistance, 1073807360))
				{
					flag = false;
				}
			}
		...
		Physics.SphereCast(new Ray(base.Transform.position + Vector3.up * m_CharacterController.radius, Vector3.down), m_CharacterController.radius, out m_GroundHit, m_SkinWidth + 0.001f, 1084850184);
		m_Grounded = m_GroundHit.collider != null;
		if (!m_Grounded && Physics.SphereCast(new Ray(base.Transform.position + Vector3.up * m_CharacterController.radius, Vector3.down), m_CharacterController.radius, out var hitInfo2, (m_CharacterController.skinWidth + 0.001f) * 4f, 1084850184) && hitInfo2.collider is CharacterController)
		{
			m_Grounded = true;
			m_GroundHit = hitInfo2;
		}
		if (!m_Grounded && Player.Velocity.Get().y > 0f)
		{
			Physics.SphereCast(new Ray(base.Transform.position, Vector3.up), m_CharacterController.radius, out m_CeilingHit, m_CharacterController.height - (m_CharacterController.radius - m_CharacterController.skinWidth) + 0.01f, 1084850184);
	...
	public virtual void DeflectHorizontalForce()
	{
		...
		if (!Physics.CapsuleCast(CapsuleBottom, CapsuleTop, Player.Radius.Get(), m_PrevDir, out m_WallHit, Vector3.Distance(m_PrevPos, m_PredictedPos) + 0.07f, 1084850184))
		{
			return;
		}
	...
	{
		if (Physics.SphereCast(new Ray(base.Transform.position, Vector3.up), Player.Radius.Get(), m_NormalHeight - Player.Radius.Get() + 0.01f, 1084850184))
		{
