...
public class AvatarBanditController : AvatarHumanController
{
	...
	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public CoverMode _currentCoverMode;

	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public CoverType _currentCoverType;

	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public CoverEngageType _currentEngageType;
	...
	public override void SwitchModelAndView(string _modelName, bool _bFPV, bool _bMale)
	{
		...
		locomotionLayerIndex = anim.GetLayerIndex("Locomotion");
		jumpLayerIndex = anim.GetLayerIndex("Jump");
		hitLayerIndex = anim.GetLayerIndex("Hit");
		coverLayerIndex = anim.GetLayerIndex("Cover");
	}
	...
	public override CoverMode CurrentCoverMode()
	{
		return _currentCoverMode;
	}

	public override CoverType CurrentCoverType()
	{
		return _currentCoverType;
	}

	public override CoverEngageType CurrentCoverEngageType()
	{
		return _currentEngageType;
	}

	public override void SetCoverMode(CoverMode _coverMode)
	{
		idleTime = 0f;
		_currentCoverMode = _coverMode;
		_setInt(AvatarController.CoverModeHash, (int)_coverMode);
	}

	public override void SetCoverType(CoverType _coverType)
	{
		idleTime = 0f;
		_currentCoverType = _coverType;
		_setInt(AvatarController.CoverTypeHash, (int)_coverType);
	}
	...
	public override void SetCoverHeight(float coverHeight)
	{
		...
		_setFloat(AvatarController.CoverHeightHash, _coverHeight);
	}

	public override void SetCoverEngageType(CoverEngageType _engage)
	{
		idleTime = 0f;
		_currentEngageType = _engage;
		_setInt(AvatarController.CoverEngageHash, (int)_engage);
	}
	...
	public override void Update()
	{
		...
		_setInt(AvatarController.weaponCarryHash, carry);
		float num = entity.speedForward * 20f;
		float num2 = entity.speedStrafe * 20f;
		bool flag = num * num + num2 * num2 > 0.0001f;
		_setFloat(AvatarController.forwardHash, num, _netSync: false);
		_setFloat(AvatarController.strafeHash, num2, _netSync: false);
		if (!isMoving)
		{
			if (flag)
			{
		...
		if (!entity.IsDead())
		{
			...
			{
				float num3 = num * num;
				_setInt(AvatarController.movementStateHash, (num3 > entity.moveSpeedAggro * entity.moveSpeedAggro) ? 3 : ((num3 > entity.moveSpeed * entity.moveSpeed) ? 2 : ((num3 > 0.001f) ? 1 : 0)), _netsync: false);
			}
		}
		if (entity.GetAimTarget(out var aimTarget))
		{
			Vector3 chestPosition = entity.emodel.GetChestPosition();
			Matrix4x4 matrix4x = Matrix4x4.TRS(chestPosition, entity.emodel.GetFlatHipRotation(), Vector3.one);
			Vector3 direction = matrix4x.inverse.MultiplyPoint(aimTarget);
			float x = Mathf.Sqrt(direction.x * direction.x + direction.z * direction.z);
			float value2 = Mathf.Atan2(direction.y, x) * 57.29578f / 90f;
			float value3 = MathUtils.NormalizeAxis(MathUtils.YawForDirection(direction)) / 90f;
			if (GameManager.Instance.DebugAIAim)
			{
				Vector3 vector = chestPosition - Origin.position;
				UnityEngine.Debug.DrawLine(vector, vector + (Vector3)matrix4x.GetColumn(0), Color.red, 0.1f);
				UnityEngine.Debug.DrawLine(vector, vector + (Vector3)matrix4x.GetColumn(1), Color.green, 0.1f);
				UnityEngine.Debug.DrawLine(vector, vector + (Vector3)matrix4x.GetColumn(2), Color.blue, 0.1f);
				UnityEngine.Debug.DrawLine(vector, aimTarget - Origin.position, Color.cyan, 0.1f);
			}
			_setFloat(AvatarController.aimPitchHash, Mathf.Clamp(value2, -1f, 1f));
			_setFloat(AvatarController.aimYawHash, Mathf.Clamp(value3, -1f, 1f));
		}
		else
		{
			_setFloat(AvatarController.aimPitchHash, 0f);
			_setFloat(AvatarController.aimYawHash, 0f);
		}
	...
	public void UpdateLayerStateInfo()
	{
		...
		coverStateInfo = anim.GetCurrentAnimatorStateInfo(coverLayerIndex);
		if (hitLayerIndex >= 0)
		{
			hitStateInfo = anim.GetCurrentAnimatorStateInfo(hitLayerIndex);
		}
		if (locomotionLayerIndex >= 0)
		{
			locomotionStateInfo = anim.GetCurrentAnimatorStateInfo(locomotionLayerIndex);
		}
		if (jumpLayerIndex >= 0)
		{
			jumpStateInfo = anim.GetCurrentAnimatorStateInfo(jumpLayerIndex);
		}
	}
	...
	{
		if (anim.IsInTransition(coverLayerIndex))
		{
			return true;
		}
		if (anim.GetLayerWeight(coverLayerIndex) > 0f)
		{
			return AvatarController.readyToFireHash != coverStateInfo.tagHash;
		}
		return false;
	}
	...
	public override bool IsAnimationJumpRunning()
	{
		...
		if (jumpLayerIndex >= 0 && jumpStateInfo.tagHash == AvatarController.jumpHash)
		{
			return true;
		}
		return false;
	...
	public Transform SpawnLimbGore(Transform parent, string path, bool restoreState)
	{
		if ((bool)parent && !string.IsNullOrEmpty(path))
		{
			...
			if (isCensored)
			{
				...
				assetRequestTask = LoadManager.LoadAsset<GameObject>(text2, null, null, false, true);
			...
			{
				assetRequestTask = LoadManager.LoadAsset<GameObject>(text, null, null, false, true);
			}
			if (!assetRequestTask.Asset)
			{
				return null;
			}
			GameObject gameObject = UnityEngine.Object.Instantiate(assetRequestTask.Asset, parent);
			GorePrefab component = gameObject.GetComponent<GorePrefab>();
			if ((bool)component)
			{
				component.restoreState = restoreState;
			}
			if (!gameObject.TryGetComponent<ReleaseAssetsOnDestroy>(out var component2))
			{
				component2 = gameObject.AddComponent<ReleaseAssetsOnDestroy>();
			}
			component2.AddAssetHandle(assetRequestTask.GetHandle());
	...
	public void ProcDismemberedPart(Transform t, Transform partT, DismemberedPartData part, uint bodyDamageFlag)
	{
		...
		if (!string.IsNullOrEmpty(part.dismemberMatPath))
		{
			...
			assetRequestTask = LoadManager.LoadAsset<Material>(text4, null, null, false, true);
			if ((bool)assetRequestTask.Asset)
			{
				text3 = text4;
			}
		...
		if (isCensored)
		{
			string text5 = text3;
			text3 += "_CGore.mat";
			assetRequestTask = LoadManager.LoadAsset<Material>(text3, null, null, false, true);
			if (!assetRequestTask.Asset)
			{
				text3 = text5;
			}
		...
		dismemberMat = UnityEngine.Object.Instantiate(assetRequestTask.Asset);
		...
		if (!t.TryGetComponent<ReleaseAssetsOnDestroy>(out var component))
		{
			component = t.gameObject.AddComponent<ReleaseAssetsOnDestroy>();
		}
		component.AddAssetHandle(assetRequestTask.GetHandle());
		assetRequestTask.Release();
	}
	...
	public void ActivateDetachableLimbs(uint bodyDamageFlag, EnumDamageTypes damageType, Transform partT, DismemberedPart part)
	{
		...
		if (partT.TryGetComponent<ReleaseAssetsOnDestroy>(out var component2))
		{
			ReleaseAssetsOnDestroy other = transform2.gameObject.AddComponent<ReleaseAssetsOnDestroy>();
			component2.CopyTo(other);
		}
	}
