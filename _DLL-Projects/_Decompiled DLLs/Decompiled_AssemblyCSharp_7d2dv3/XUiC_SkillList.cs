...
public class XUiC_SkillList : XUiController
{
	...
	public void updateFilteredList()
	{
		...
		foreach (ProgressionValue skill in skills)
		{
			...
			if (progressionClass == null || !progressionClass.ValidDisplay(DisplayType) || progressionClass.Name == null || progressionClass.IsBook || progressionClass.Hidden || !progressionClass.Enabled || (flag && !progressionClass.NameKey.ContainsCaseInsensitive(filterText) && !Localization.Get(progressionClass.NameKey).ContainsCaseInsensitive(filterText)))
			{
				continue;
			}
			if (!flag && (text == "" || text.EqualsCaseInsensitive(progressionClass.Name)))
			{
				currentSkills.Add(skill);
				continue;
			}
			ProgressionClass progressionClass2 = progressionClass.Parent;
			if (progressionClass2 == null || progressionClass2 == progressionClass)
			{
				continue;
			}
			if (flag)
			{
				if ((!progressionClass.IsSkill && !progressionClass.IsPerk) || !progressionClass2.IsAttribute)
				{
					currentSkills.Add(skill);
				}
			}
			else if (((progressionClass.IsSkill || progressionClass.IsCrafting) && text.EqualsCaseInsensitive(progressionClass2.Name)) || ((progressionClass.IsPerk || progressionClass.IsBookGroup) && text.EqualsCaseInsensitive(progressionClass2.Parent.Name)))
			{
				currentSkills.Add(skill);
			}
		}
	...
	public void listSkills(int page)
	{
		...
		for (int i = 0; i < skillEntries.Length; i++)
		{
			...
			if (num2 < currentSkills.Count && currentSkills[num2] != null && Progression.ProgressionClasses.ContainsKey(currentSkills[num2].Name))
			{
				...
				{
					SelectedEntry = xUiC_SkillEntry;
					xui.SelectedSkill = xUiC_SkillEntry.Skill;
				...
				{
					xUiC_SkillEntry.IsSelected = xUiC_SkillEntry.Skill.Name == xui.SelectedSkill.Name;
					xUiC_SkillEntry.RefreshBindings();
				}
				else
				{
				...
				{
					XUiView navRightTarget = xui.GetWindow("windowSkillAttributeInfo").Controller.GetChildById("0").ViewComponent;
					xUiC_SkillEntry.ViewComponent.NavRightTarget = navRightTarget;
				}
				else if (xUiC_SkillEntry.Skill.ProgressionClass.IsPerk)
				{
					XUiView navRightTarget2 = xui.GetWindow("windowSkillPerkInfo").Controller.GetChildById("0").ViewComponent;
					xUiC_SkillEntry.ViewComponent.NavRightTarget = navRightTarget2;
				}
				else if (xUiC_SkillEntry.Skill.ProgressionClass.IsBookGroup)
				{
					XUiView navRightTarget3 = xui.GetWindow("windowSkillBookInfo").Controller.GetChildById("0").ViewComponent;
					xUiC_SkillEntry.ViewComponent.NavRightTarget = navRightTarget3;
				}
				else if (xUiC_SkillEntry.Skill.ProgressionClass.IsCrafting)
				{
					XUiView navRightTarget4 = xui.GetWindow("windowSkillCraftingInfo").Controller.GetChildById("0").ViewComponent;
