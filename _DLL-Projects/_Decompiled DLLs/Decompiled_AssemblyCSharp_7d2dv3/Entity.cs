...
public class Entity : MonoBehaviour, ILockTarget
{
	...
	public class EntityLockContext : ILockContext
	{
		...
		{
			_bw.Write(Command ?? string.Empty);
			_bw.Write(FirstTimeTouched);
			bool flag = Bag != null;
			_bw.Write(flag);
			if (flag)
			{
		...
		public void Read(PooledBinaryReader _br)
		{
			...
			FirstTimeTouched = _br.ReadBoolean();
			if (_br.ReadBoolean())
			{
				Bag = Bag.Read(_br);
			}
	...
	public LockTargetType LockTargetType => LockTargetType.Entity;
	...
	public void InitializeBagFromLootList()
	{
		...
		string value = GetLootList();
		if (string.IsNullOrEmpty(value))
		{
			return;
		}
		LootContainer lootContainer = LootContainer.GetLootContainer(value, _errorOnMiss: false);
		if (lootContainer != null)
		{
			Vector2i size = lootContainer.size;
			if (size.x > 0 && size.y > 0)
			{
				bag = new Bag(size.x * size.y);
			}
		}
	}
	...
	public void InitEModel(EModelInstanceAssets _assets)
	{
		...
		emodel.Init(world, this, _assets);
	}
	...
	public virtual void PostInit()
	{
		...
		if (physicsRB != null)
		{
			fallLastY = physicsRB.position.y + Origin.position.y;
		}
	}
	...
	{
		if (yawSeekTimeMax > float.Epsilon)
		{
			yawSeekTime += Time.deltaTime;
			float t = Mathf.Clamp01(yawSeekTime / yawSeekTimeMax);
			if (yawSeekTime < yawSeekTimeMax)
			{
				rotation.y = Mathf.Lerp(yawSeekAngle, yawSeekAngleEnd, t);
				return;
			}
			yawSeekTimeMax = 0f;
			rotation.y = yawSeekAngleEnd;
		}
		else if (yawSeekTimeMax > 0f)
		{
			rotation.y = yawSeekAngleEnd;
			yawSeekTimeMax = 0f;
		}
	...
	public void ccEntityCollisionResults()
	{
		...
		Vector2 rhs = new Vector2(motion.x, motion.z);
		projectedMove = 0f;
		float sqrMagnitude = rhs.sqrMagnitude;
		if (sqrMagnitude > 1E-15f)
		{
			Vector2 lhs = new Vector2(vector2.x, vector2.z);
			projectedMove = Utils.FastClamp01(Vector2.Dot(lhs, rhs) / sqrMagnitude);
			rhs *= projectedMove;
		}
		motion.x = rhs.x;
		motion.z = rhs.y;
		...
		if (!onGround && wasOnGround && (bool)physicsRB)
		{
			fallLastY = physicsRB.position.y + Origin.position.y;
		}
		if (onGround)
	...
	public virtual void SetAlive()
	{
		...
		if (PhysicsTransform != null)
		{
			...
			else if (ConsoleCmdCCPhysics.EnableCCPhysicsChanges && this is EntityPlayer)
			{
				PhysicsTransform.gameObject.layer = 3;
			}
			else
			{
	...
	public void UpdateFall(float mY)
	{
		...
		else if (mY < 0f && physicsRB != null)
		{
			float num = physicsRB.position.y + Origin.position.y;
			float num2 = fallLastY - num;
			fallLastY = num;
	...
	public virtual void CopyPropertiesFromEntityClass()
	{
		...
		entityClass.Properties.ParseString(EntityClass.PropLootList, ref lootList);
		...
		if (num > 0)
		{
			for (int j = 1; j <= num; j++)
			{
				if (entityClass.Properties.Values.ContainsKey($"{EntityClass.PropCustomCommandName}{j}"))
				{
					...
					{
						commandId = entityClass.Properties.Values[$"{EntityClass.PropCustomCommandName}{j}"],
					...
					string key = $"{EntityClass.PropCustomCommandIconColor}{j}";
					if (EntityClass.Properties.Values.ContainsKey(key))
					{
						entityActivationCommand.iconColor = StringParsers.ParseHexColor(EntityClass.Properties.Values[key]);
					}
					else
					{
						entityActivationCommand.iconColor = Color.white;
					}
					key = $"{EntityClass.PropCustomCommandActivateTime}{j}";
					if (entityClass.Properties.Values.ContainsKey(key))
					{
						entityActivationCommand.activateTime = StringParsers.ParseFloat(entityClass.Properties.Values[key]);
					}
					else
					{
	...
	public virtual void PlayOneShot(string clipName, bool sound_in_head = false, bool serverSignalOnly = false, bool isUnique = false, AnimationEvent _animEvent = null, float volumeScale = 1f)
	{
		if (!sound_in_head)
		{
			if (serverSignalOnly)
			{
				Handle handle = Manager.Play(this, clipName, volumeScale, wantHandle: true);
	...
	{
		return UpdateActivationCommands(_playerFocusing);
	}
	...
	public EntityActivationCommand[] GetActivationCommands()
	{
		...
		InitLocalActivationCommands([PublicizedFrom(EAccessModifier.Internal)] (EntityActivationCommand command) =>
		{
			commands.Add(command);
		});
		if (customCmds != null)
		{
			for (int num = 0; num < customCmds.Length; num++)
			{
				commands.Add(customCmds[num]);
			}
		}
		ReorderActivationCommands(commands);
		activationCommands = commands.ToArray();
		return activationCommands;
	}
	...
	public bool UpdateActivationCommands(EntityPlayerLocal _playerFocusing)
	{
		...
		if (frameCount == lastUpdateFrameOfActivationCommands && num == lastUpdateActivationCommandsPlayerId)
		{
			return lastUpdateHadEnabledActivationCommands;
		}
		EntityActivationCommand[] array = GetActivationCommands();
		if (array == null || array.Length == 0)
		{
			lastUpdateFrameOfActivationCommands = frameCount;
			lastUpdateActivationCommandsPlayerId = num;
			lastUpdateHadEnabledActivationCommands = false;
			return false;
		}
		if (_playerFocusing == null)
		{
			...
			lastUpdateFrameOfActivationCommands = frameCount;
			lastUpdateActivationCommandsPlayerId = num;
			lastUpdateHadEnabledActivationCommands = true;
			return true;
		}
		bool flag = false;
		for (int j = 0; j < array.Length; j++)
		{
			...
			entityActivationCommand2.enabled = AllowActivationCommand(entityActivationCommand2.commandId, _playerFocusing);
			array[j] = entityActivationCommand2;
			flag |= entityActivationCommand2.enabled;
		}
		lastUpdateFrameOfActivationCommands = frameCount;
		lastUpdateActivationCommandsPlayerId = num;
		lastUpdateHadEnabledActivationCommands = flag;
		return flag;
	}
	...
	{
		OnEntityActivated(_command, _playerFocusing);
		...
		if (_command.activateTime > 0f)
		{
			...
			timerEventData.FullTimeFinishEvent += [PublicizedFrom(EAccessModifier.Internal)] (TimerEventData timerEventData2) =>
			{
				GameEventManager.Current.HandleAction(ec.onActivateEvent, _playerFocusing, this, twitchActivated: false);
			};
			XUiC_Timer.OpenTimer(_playerFocusing.PlayerUI.xui, _command.activateTime, timerEventData);
		}
		else if (ec.onActivateEvent != "")
		{
			GameEventManager.Current.HandleAction(ec.onActivateEvent, _playerFocusing, this, twitchActivated: false);
		}
	}
	...
	public void Collect(int _playerId)
	{
		...
		{
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageEntityCollect>().Setup(entityId, _playerId));
			return;
		}
		OnCollectLocal(_playerId);
		OnCollectServer(_playerId);
	}
	...
	public virtual bool CanLockOnServer(int _lockingPlayerID, ILockContext _context, ushort _channel)
	{
		...
		if (spawnById > 0 && spawnById != _lockingPlayerID)
		{
			if (TwitchManager.Current.StealingCrateEvent != "" && GameManager.Instance.World.GetEntity(_lockingPlayerID) is EntityPlayer entityPlayer)
			{
				GameEventManager.Current.HandleAction(TwitchManager.Current.StealingCrateEvent, entityPlayer, entityPlayer, twitchActivated: false);
			}
			if (!spawnByAllowShare)
			{
				if (TwitchManager.Current.DeniedCrateEvent != "" && GameManager.Instance.World.GetEntity(_lockingPlayerID) is EntityPlayer entityPlayer2)
				{
					GameEventManager.Current.HandleAction(TwitchManager.Current.DeniedCrateEvent, entityPlayer2, entityPlayer2, twitchActivated: false);
				}
				return false;
			}
		}
	...
	{
		if (bag != null || _context is EntityLockContext { Bag: not null })
		{
	...
	{
		if (_success && bag != null)
		{
			if (_context is EntityLockContext entityLockContext)
			{
				entityLockContext.FirstTimeTouched = !bag.Touched;
				entityLockContext.Bag = bag;
			}
			if (!bag.Touched)
			{
				GameManager.Instance.lootManager.LootBagOpened(bag, this, _lockingPlayerID);
			}
			bag.Touched = true;
		}
	}
	...
	{
		if (_context is EntityLockContext { Bag: not null } entityLockContext)
		{
			bag = entityLockContext.Bag.Clone();
		}
		LocalPlayerUI playerUI = LocalPlayerUI.GetUIForPrimaryPlayer();
		if (bag == null || !_success || playerUI == null)
		{
			if (playerUI != null)
			{
		...
		XUiC_BagStorageWindowGroup.Open(playerUI.xui, this, bag, lootContainer, Localization.Get(LocalizedEntityName), OnBagModified, [PublicizedFrom(EAccessModifier.Internal)] () =>
		{
			LockManager.Instance.UnlockRequestLocal();
		}, [PublicizedFrom(EAccessModifier.Internal)] () => GetDistanceSq(playerUI.entityPlayer) <= Constants.cPlayerInteractDistance * Constants.cPlayerInteractDistance, _context is EntityLockContext entityLockContext2 && entityLockContext2.FirstTimeTouched);
		if (lootContainer != null && playerUI.entityPlayer != null)
		{
			lootContainer.ExecuteBuffActions(entityId, playerUI.entityPlayer);
		}
		bag.Touched = true;
	}
	...
	public virtual void OnUnlockedServer(int _unlockingPlayerId, ushort _channel)
	{
		...
		LootContainer lootContainer = LootContainer.GetLootContainer(GetLootList());
		if (lootContainer != null && lootContainer.destroyOnClose switch
		{
			LootContainer.DestroyOnClose.True => true, 
			LootContainer.DestroyOnClose.Empty => bag.IsEmpty(), 
			_ => false, 
		})
		{
			if (!bag.IsEmpty())
	...
	{
		if (!SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer || this is EntityLootContainer)
		{
			return;
		}
		Vector3 transformPos = GetPosition();
		transformPos.y += 0.9f;
		if (lootDropProb != 0f)
		{
			EntityClass entityClass = EntityClass.list[this.entityClass];
			...
			{
				EntityLootContainer entityLootContainer = EntityFactory.CreateEntity(entityClass.LootDropPick(rand), transformPos, Vector3.zero) as EntityLootContainer;
				...
				Manager.BroadcastPlay(transformPos, "zpack_spawn");
				return;
			}
		}
		if (bag != null && !bag.IsEmpty())
		{
			...
			{
				entityLootContainer2.OverrideLootList = GetLootList();
				entityLootContainer2.SetContent(ItemStack.Clone(bag.GetSlots()), bag.SlotCount);
				entityLootContainer2.bag.Touched = bag.Touched;
	...
	public void OnBagModified()
	{
		if (!SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			NetPackageBag package = NetPackageManager.GetPackage<NetPackageBag>().Setup(entityId, bag);
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(package);
		}
	}
