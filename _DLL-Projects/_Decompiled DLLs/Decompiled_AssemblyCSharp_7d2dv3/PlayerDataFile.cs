...
public class PlayerDataFile
{
	...
	public const int cFileVersion = 58;
	...
	public void FromPlayer(EntityPlayer _player)
	{
		...
		inventory = ((_player.AttachedToEntity != null && _player.saveInventory != null) ? ItemStack.Clone(_player.saveInventory.GetSlots()) : ItemStack.Clone(_player.inventory.GetSlots()));
		bag = _player.bag.Clone();
	...
	public void ReadNetwork(PooledBinaryReader _br)
	{
		Read(_br, uint.MaxValue);
		metadata = PlayerMetaInfo.FromStream(_br);
	}
	...
	public void Read(PooledBinaryReader _br, uint _version)
	{
		...
		else
		{
			...
			PackedBoolArray packedBoolArray = null;
			switch (_version)
			{
			default:
				if (_br.ReadBoolean())
				{
					packedBoolArray = new PackedBoolArray();
					packedBoolArray.Read(_br);
				}
				else
				{
					packedBoolArray = null;
				}
				break;
			case 55u:
			case 56u:
			{
				...
				packedBoolArray = new PackedBoolArray(num2);
				for (int j = 0; j < num2; j++)
				{
					packedBoolArray[j] = _br.ReadBoolean();
				}
				break;
			}
			case 53u:
			case 54u:
			{
				...
				packedBoolArray = new PackedBoolArray(num);
				for (int i = 0; i < num; i++)
				{
					packedBoolArray[i] = true;
				}
				break;
			}
			case 0u:
			...
			bag = new Bag(array.Length);
			bag.SetSlots(array);
			bag.LockedSlots = packedBoolArray;
		}
		if (_version >= 52)
		{
			ItemStack[] array2 = GameUtils.ReadItemStack(_br);
	...
	public void Save(string _dir, string _playerId)
	{
		try
		{
			...
			using (Stream baseStream = SdFile.Open(text + ".tmp", FileMode.CreateNew, FileAccess.Write, FileShare.Read))
			{
				...
				pooledBinaryWriter.Write((byte)58);
	...
	public void WriteNetwork(PooledBinaryWriter _bw)
	{
		Write(_bw);
		metadata.Write(_bw);
	}
