	...
	public static IEnumerator Load(XmlFile _xmlFile)
	{
		...
		foreach (XElement item in root.Elements())
		{
			...
			foreach (XElement item2 in item.Elements("block"))
			{
				...
				if (item2.HasAttribute("sandboxoption"))
				{
					sandboxOption = item2.GetAttribute("sandboxoption");
				}
				...
				{
					BlockPlaceholderMap.Instance.AddPlaceholder(blockValue, targetValue, _result, biome, randomRotation, sandboxOption);
				}
			}
