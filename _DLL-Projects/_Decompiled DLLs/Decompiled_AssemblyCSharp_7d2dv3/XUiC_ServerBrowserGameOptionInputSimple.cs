...
public class XUiC_ServerBrowserGameOptionInputSimple : XUiController
{
	...
	public void ComparisonLabel_OnPress(XUiController _sender, int _mouseButton)
	{
		...
		XUiV_Label xUiV_Label = comparisonLabel;
		xUiV_Label.Text = currentComparison switch
		{
			EComparisonType.Smaller => "<", 
			EComparisonType.SmallerEquals => "<=", 
			EComparisonType.Equals => "=", 
			EComparisonType.LargerEquals => ">=", 
			EComparisonType.Larger => ">", 
			_ => comparisonLabel.Text, 
		};
	...
	public XUiC_ServersList.UiServerFilter GetFilter()
	{
		...
		return new XUiC_ServersList.UiServerFilter(name, XUiC_ServersList.EnumServerLists.Regular, func, type, intMinValue, intMaxValue);
	}
}
