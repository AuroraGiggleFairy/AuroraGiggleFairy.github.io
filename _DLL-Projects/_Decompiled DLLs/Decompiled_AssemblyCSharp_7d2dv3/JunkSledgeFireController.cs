...
public class JunkSledgeFireController : MiniTurretFireController
{
	...
	public new void Update()
	{
		...
		{
			float num = Mathf.Clamp01(timeCounter / (fireRateMax * 0.5f * 0.25f));
		...
		{
			float num2 = Mathf.Clamp01(timeCounter / (fireRateMax * 0.5f * 0.75f));
	...
	public void hitTarget()
	{
		...
		forward *= -1f;
		Voxel.Raycast(ray: new Ray(position + Origin.position, forward), _world: GameManager.Instance.World, distance: distance, _layerMask: -538750989, _hitMask: 128, _sphereRadius: 0.15f);
