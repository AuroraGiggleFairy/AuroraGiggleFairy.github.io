...
public class InviteManager
{
	...
	public void Update()
	{
		...
		else
		{
			...
			if (XUiC_WorldGenerationWindow.WorldBuilderIsGenerating)
			{
				XUiC_WorldGenerationWindow.CancelGeneration();
			}
			else if (!profileReady)
			{
				...
				if (ProfileSDF.CurrentProfileName().Length == 0)
				{
					...
					XUiC_PlayerProfile.Open(LocalPlayerUI.primaryUI.xui, [PublicizedFrom(EAccessModifier.Private)] () =>
					{
						profileReady = true;
	...
	public IEnumerator StartJoinIntentCoroutine()
	{
		...
		IEnumerator CheckMultiplayerPrivileges()
		{
			...
			if (XUiC_MultiplayerPrivilegeNotification.ResolvePrivilegesWithDialog(EUserPerms.Multiplayer, [PublicizedFrom(EAccessModifier.Internal)] (bool _result) =>
			{
		...
		IEnumerator RequestPlayerWarning()
		{
			if (SingletonMonoBehaviour<ConnectionManager>.Instance.ClientCount() > 0)
			{
				...
				XUiC_MessageBoxWindowGroup.ShowOkCancel(LocalPlayerUI.primaryUI.xui, Localization.Get("lblPrivilegesCloseServerHeader"), string.Format(Localization.Get("lblPrivilegesCloseServer"), SingletonMonoBehaviour<ConnectionManager>.Instance.ClientCount()), [PublicizedFrom(EAccessModifier.Internal)] () =>
				{
	...
	public static IEnumerator HandleSessionIdInvite(string _sessionId, string _password, Action<bool> _onFinished)
	{
		...
		if (serverInfo.AllowsCrossplay)
		{
			...
			XUiC_MultiplayerPrivilegeNotification.ResolvePrivilegesWithDialog(EUserPerms.Crossplay, [PublicizedFrom(EAccessModifier.Internal)] (bool result) =>
			{
		...
		else if (!serverInfo.PlayGroup.IsCurrent())
		{
			...
			XUiC_MessageBoxWindowGroup.ShowOk(LocalPlayerUI.primaryUI.xui, title, text);
			yield break;
		}
