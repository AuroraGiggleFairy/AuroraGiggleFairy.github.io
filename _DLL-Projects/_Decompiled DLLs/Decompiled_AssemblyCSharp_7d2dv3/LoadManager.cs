...
public static class LoadManager
{
	...
	public abstract class AssetRequestTask<T> : LoadTask where T : UnityEngine.Object
	{
		...
		public virtual IAssetHandle GetHandle()
		{
	...
	public class ResourceRequestTask<T> : AssetRequestTask<T> where T : UnityEngine.Object
	{
		...
		public override void CompleteNow()
		{
			if (loadAsync)
			{
				if (assetRetrieved)
				{
					throw new Exception("ResourceRequestTask async already completed.");
				}
				if (request != null)
				{
					Log.Warning("ResourceRequestTask async WaitForComplete is not supported, asset " + assetPath + " will be loaded again sync");
				}
				asyncCancelled = true;
				LoadSync();
			}
	...
	public class AssetBundleRequestTask<T> : AssetRequestTask<T> where T : UnityEngine.Object
	{
		...
		public override void CompleteNow()
		{
			if (loadAsync)
			{
				if (assetRetrieved)
				{
					throw new Exception("AssetBundleRequestTask async already completed.");
				}
				if (request != null)
				{
					Log.Warning("AssetBundleRequestTask async WaitForComplete is not supported, asset " + identifier.AssetName + " from " + identifier.BundlePath + " will be loaded again sync");
				}
				asyncCancelled = true;
				LoadSync();
			}
	...
	public class AddressableRequestTask<T> : AssetRequestTask<T> where T : UnityEngine.Object
	{
		...
		public override void Complete()
		{
			if (INTERNAL_IsPending)
			{
				throw new Exception("AddressableRequestTask still pending.");
			}
			if (isGameObject)
			{
				GameObject result = gameObjectRequest.Result;
		...
		public override void CompleteNow()
		{
			if (loadAsync)
			{
				if (assetRetrieved)
				{
					throw new Exception("AddressableRequestTask async already completed.");
				}
				LoadSync();
			}
		}

		public override IAssetHandle GetHandle()
		{
			if (isGameObject)
			{
				return new AddressableAssetHandle<GameObject>(gameObjectRequest);
			}
			return new AddressableAssetHandle<T>(request);
		}
	...
	public class AddressableAssetsRequestTask<T> : AssetsRequestTask<T> where T : UnityEngine.Object
	{
		...
		public override void Complete()
		{
			if (INTERNAL_IsPending)
			{
				throw new Exception("AddressableAssetsRequestTask still pending.");
			}
		...
		public override void CompleteNow()
		{
			if (loadAsync)
			{
				if (assetsRetrieved)
				{
					throw new Exception("AddressableAssetsRequestTask async already completed.");
				}
				LoadSync();
			}
		}
	...
	public class FileLoadTask : LoadTask
	{
		...
		[PublicizedFrom(EAccessModifier.Private)]
		public ThreadManager.TaskInfo asyncLoadTask;
		...
		public override bool Load()
		{
			asyncLoadTask = ThreadManager.AddSingleTask([PublicizedFrom(EAccessModifier.Private)] (ThreadManager.TaskInfo _threadInfo) =>
			{
				try
				{
		...
		public override void CompleteNow()
		{
			if (loadAsync)
			{
				...
				{
					throw new Exception("AssetBundleRequestTask async has not started.");
				}
				asyncLoadTask.WaitForEnd();
				Complete();
			}
		}
	}
	...
	public class AddressableAssetHandle<T> : IAssetHandle where T : UnityEngine.Object
	{
		[PublicizedFrom(EAccessModifier.Private)]
		public AsyncOperationHandle<T> handle;

		public AddressableAssetHandle(AsyncOperationHandle<T> _handle)
		{
			handle = Addressables.ResourceManager.Acquire(_handle);
		}

		public IAssetHandle Copy()
		{
			return new AddressableAssetHandle<T>(handle);
		}
	...
	public class DummyHandle : IAssetHandle
	{
		public static DummyHandle Instance = new DummyHandle();
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static void RemoveRequest(LoadTask _request)
	{
		if (loadRequests.Remove(_request))
		{
			_request.Group.DecrementPending();
		}
	}
	...
	public static AssetRequestTask<T> LoadAsset<T>(string _uri, Action<T> _callback = null, LoadGroup _lg = null, bool _deferLoading = false, bool _loadSync = false, bool _ignoreDlcEntitlements = false) where T : UnityEngine.Object
	{
		return LoadAsset(DataLoader.ParseDataPathIdentifier(_uri), _callback, _lg, _deferLoading, _loadSync, _ignoreDlcEntitlements);
	}
