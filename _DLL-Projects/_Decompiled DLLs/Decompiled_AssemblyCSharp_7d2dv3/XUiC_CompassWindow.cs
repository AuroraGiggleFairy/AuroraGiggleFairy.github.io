...
public class XUiC_CompassWindow : XUiController
{
	...
	public override void Init()
	{
		...
		for (int i = 0; i < 50; i++)
		{
			...
			uISprite.atlas = xui.GetAtlasByName("UIAtlas", "menu_empty");
	...
	public override void Update(float _dt)
	{
		...
		base.ViewComponent.IsVisible = !localPlayer.IsDead() && xui.playerUI.windowManager.IsHUDEnabled() && ShowCompass;
		if (localPlayer.playerCamera != null && ShowCompass)
		{
	...
	public override bool GetBindingValueInternal(ref string value, string bindingName)
	{
		...
		case "showtime":
			if (localPlayer != null)
			{
				value = (EffectManager.GetValue(PassiveEffects.NoTimeDisplay, null, 0f, localPlayer) == 0f && SandboxOptionManager.GetBool(global::SandboxOptions.SandboxOptions.ShowDayTime)).ToString();
			}
			else
			{
				value = "true";
		...
		case "daycolor":
			...
			if (XUi.IsGameRunning())
			{
				...
				var (num, num2, _) = GameUtils.WorldTimeToElements(worldTime);
				if (bloodMoonWarningHour != -1 && GameStats.GetInt(EnumGameStats.BloodMoonDay) == num && bloodMoonWarningHour <= num2)
				{
					value = "FF0000";
				}
			}
		...
		case "compass_language":
			...
			return true;
		case "playercoretemp":
			value = (((bool)localPlayer && World.TemperatureSurvival) ? XUiM_Player.GetCoreTemp(localPlayer) : "");
			return true;
		case "coretempcolor":
			value = "FFFFFF";
			...
			return true;
		case "outsidetemp":
			value = (((bool)localPlayer && World.TemperatureSurvival) ? XUiM_Player.GetOutsideTemp(localPlayer) : "");
			return true;
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (mapObject.type == EnumMapObjectType.TreasureChest)
			{
				...
				if (magnitude < value)
				{
					...
					waypointSpriteList[waypointSpriteIndex].atlas = xui.GetAtlasByName("UIAtlas", mapObject.GetMapIcon());
			...
			if (mapObject.type == EnumMapObjectType.HiddenCache)
			{
				...
				waypointSpriteList[waypointSpriteIndex].atlas = xui.GetAtlasByName("UIAtlas", spriteName);
		...
		for (int i = 0; i < navObjectList.Count; i++)
		{
			...
			if (currentCompassSettings.HotZone != null)
			{
				...
				if (magnitude < num3)
				{
					float num4 = Mathf.PingPong(Time.time, 0.25f);
					float num5 = 1.25f + num4;
					waypointSpriteList[waypointSpriteIndex].atlas = xui.GetAtlasByName("UIAtlas", currentCompassSettings.HotZone.SpriteName);
