...
public class AIDirectorAirDropComponent : AIDirectorComponent
{
	...
	public override void InitNewGame()
	{
		...
		nextAirDropTime = calcNextAirdrop(lastAirdropCheckTime);
		supplyCrates.Clear();
	}
	...
	{
		UlongAsUshorts ulongAsUshorts = new UlongAsUshorts
		{
			MinDays = (ushort)MinDayCount,
			MaxDays = (ushort)MaxDayCount,
			MinTime = (ushort)MinTimeOfDay,
			MaxTime = (ushort)MaxTimeOfDay
		};
		return ulongAsUshorts.Value;
	}
	...
	public override void Tick(double _dt)
	{
		...
		ulong num = packDropFrequency();
		ulong worldTime = Director.World.worldTime;
		if (GameUtils.IsPlaytesting() || Director.World.IsEditor() || num == 0 || LootContainer.NoLoot || MaxDayCount == 0)
		{
			return;
		}
		if (worldTime >= nextAirDropTime)
		{
			if (SpawnAirDrop())
			{
				...
				nextAirDropTime = calcNextAirdrop(worldTime);
			}
		}
	...
	public override void Read(BinaryReader _stream, int _version)
	{
		...
		{
			lastFrequency = packDropFrequency();
		}
		supplyCrates.Clear();
	...
	{
		bool result = false;
		if (LootContainer.NoLoot || MaxDayCount == 0)
		{
			return true;
		}
		if (activeAirDrop == null)
	...
	{
		int num = GameUtils.WorldTimeToDays(currentTime) + base.Random.RandomRange(MinDayCount, MaxDayCount + 1) - 1;
		ulong num2 = (ulong)base.Random.RandomRange(MinTimeOfDay, MaxTimeOfDay + 1);
		ulong num3 = (ulong)(num * 24000) + num2;
