...
public class AstarManager : MonoBehaviour
{
	...
	public void OnBlockChanged(Vector3i pos, BlockValue bvOld, sbyte densOld, TextureFullArray texOld, BlockValue bvNew)
	{
		...
		if (block.isMultiBlock)
		{
			...
			for (int i = 0; i < length; i++)
			{
				...
				blockPos += pos;
				UpdateBlock(blockPos, isSlowUpdate: false);
			}
		}
		else if (block.isOversized)
		{
			OversizedBlockUtils.GetWorldAlignedBoundsExtents(pos, block.shape.GetRotation(bvOld), block.oversizedBounds, out var min, out var max);
			for (int j = min.x; j <= max.x; j++)
			{
				for (int k = min.y; k <= max.y; k++)
				{
					...
					{
						UpdateBlock(new Vector3i(j, k, l), isSlowUpdate: false);
					}
				}
			}
		}
		Block block2 = bvNew.Block;
		bool isSlowUpdate = block2 is BlockCompositeTileEntity blockCompositeTileEntity && blockCompositeTileEntity.CompositeData.HasFeature<TEFeatureDoor>();
		if (block2.isMultiBlock)
		{
			int rotation2 = bvNew.rotation;
			int length2 = block2.multiBlockPos.Length;
			for (int m = 0; m < length2; m++)
			{
				Vector3i blockPos2 = block2.multiBlockPos.Get(m, bvNew.type, rotation2);
				blockPos2 += pos;
				UpdateBlock(blockPos2, isSlowUpdate);
			}
		}
		else if (block2.isOversized)
		{
			OversizedBlockUtils.GetWorldAlignedBoundsExtents(pos, block2.shape.GetRotation(bvNew), block2.oversizedBounds, out var min2, out var max2);
			for (int n = min2.x; n <= max2.x; n++)
			{
				for (int num = min2.y; num <= max2.y; num++)
				{
					for (int num2 = min2.z; num2 <= max2.z; num2++)
					{
						UpdateBlock(new Vector3i(n, num, num2), isSlowUpdate);
					}
				}
			}
		}
	...
	public void OnBlockDamaged(BlockValueRef _bvRef, BlockValue _blockValue, int _damage, int _attackerEntityId)
	{
		...
		case BlockValueRefType.Block:
			UpdateBlock(_bvRef.BlockPosition, isSlowUpdate: false);
			break;
		default:
