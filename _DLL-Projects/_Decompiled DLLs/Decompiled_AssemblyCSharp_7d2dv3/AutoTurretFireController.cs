...
public class AutoTurretFireController : MonoBehaviour
{
	...
	public void Update()
	{
		...
		if (!IsOn || atc.UserAccessingId != -1 || TileEntity.IsUserAccessing())
		{
			if (atc.UserAccessingId != -1)
			{
	...
	public void findTarget()
	{
		...
		for (int j = 0; j < entitiesInBounds.Count; j++)
		{
			...
			if (!Voxel.Raycast(ray: new Ray(position + Origin.position - normalized * 0.05f, normalized), _world: GameManager.Instance.World, distance: maxDistance + 0.05f, _layerMask: -538750989, _hitMask: 8, _sphereRadius: 0f) || !Voxel.voxelRayHitInfo.tag.StartsWith("E_"))
			{
				continue;
			}
	...
	public void Fire()
	{
		...
		if (TileEntity != null)
		{
			...
			if ((SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer && !TileEntity.IsUserAccessing()) || (atc != null && atc.UserAccessingId != -1))
			{
		...
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer || (atc != null && atc.UserAccessingId != -1))
		{
		...
		Vector3 fireSingleDirectionBullet(GameRandom _random, ItemValue _ammo, Ray ray2, Vector3 _turretLookDirection, Vector2 _localSpread, float num3, int penCount, int num4)
		{
			...
			{
				if (Voxel.Raycast(GameManager.Instance.World, ray2, num3, -538751005, hitMask, 0f))
				{
