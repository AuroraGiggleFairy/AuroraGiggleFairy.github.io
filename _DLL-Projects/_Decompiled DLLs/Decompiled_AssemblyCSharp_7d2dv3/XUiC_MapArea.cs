...
public class XUiC_MapArea : XUiController
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public List<PrefabInstance> dynamicPrefabBuffer = new List<PrefabInstance>();
	...
	public override void Init()
	{
		...
		zoomScale = 1f;
		targetZoomScale = 1f;
		xui.LoadData("Prefabs/MapSpriteEntity", [PublicizedFrom(EAccessModifier.Private)] (GameObject o) =>
		{
			prefabMapSprite = o;
		});
		xui.LoadData("Prefabs/MapSpriteStartPoint", [PublicizedFrom(EAccessModifier.Private)] (GameObject o) =>
		{
			prefabMapSpriteStartPoint = o;
		});
		xui.LoadData("Prefabs/MapSpritePrefab", [PublicizedFrom(EAccessModifier.Private)] (GameObject o) =>
		{
			prefabMapSpritePrefab = o;
		});
		xui.LoadData("Prefabs/MapSpriteEntitySpawner", [PublicizedFrom(EAccessModifier.Private)] (GameObject o) =>
		{
	...
	{
		xui.LoadData("@:Textures/UI/fow_chunkMask.png", [PublicizedFrom(EAccessModifier.Private)] (Texture2D o) =>
		{
	...
	public override void OnOpen()
	{
		...
		if (!isOpen)
		{
			...
			xui.playerUI.GetComponentInParent<LocalPlayerCamera>().PreRender += OnPreRender;
			xui.CalloutWindow.ClearCallouts(XUiC_GamepadCalloutWindow.CalloutType.Menu);
			xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.RightStick, "igcoMapMoveNoHold", XUiC_GamepadCalloutWindow.CalloutType.Menu);
			xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.FaceButtonSouth, "igcoMapWaypoint", XUiC_GamepadCalloutWindow.CalloutType.Menu);
			xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.RightTrigger, "igcoMapZoomIn", XUiC_GamepadCalloutWindow.CalloutType.Menu);
			xui.CalloutWindow.AddCallout(UIUtils.ButtonIcon.LeftTrigger, "igcoMapZoomOut", XUiC_GamepadCalloutWindow.CalloutType.Menu);
			...
			xui.FindWindowGroupByName("windowpaging").GetChildByType<XUiC_WindowSelector>()?.SetSelected("map");
	...
	public override void OnClose()
	{
		...
		if (isOpen)
		{
			...
			if (bMapCursorSet)
			{
				...
				xui.ToolTipWindow.ToolTip = string.Empty;
			}
			xui.playerUI.GetComponentInParent<LocalPlayerCamera>().PreRender -= OnPreRender;
	...
	public override void Update(float _dt)
	{
		...
		if (xui.playerUI.playerInput.GUIActions.LastDeviceClass == InputDeviceClass.Controller && !xui.GetWindow("mapAreaSetWaypoint").IsVisible && bMouseOverMap)
		{
		...
		uiLblPlayerPos.Text = ValueDisplayFormatters.WorldPos(xui.playerUI.entityPlayer.GetPosition());
		Vector3 pos = screenPosToWorldPos(xui.playerUI.CursorController.GetScreenPosition());
		uiLblCursorPos.Text = ValueDisplayFormatters.WorldPos(pos);
		uiLblCursorPos.IsVisible = bMouseOverMap;
		...
		float num = 5f * xui.playerUI.playerInput.GUIActions.TriggerAxis.Value;
		if (num != 0f)
		{
		...
		if (xui.playerUI.playerInput.GUIActions.Cancel.WasPressed || xui.playerUI.playerInput.PermanentActions.Cancel.WasPressed)
		{
			XUiV_Window window = xui.GetWindow("mapAreaSetWaypoint");
			if (window.IsVisible)
			{
				window.IsVisible = false;
				xui.GetWindow("mapAreaChooseWaypoint").IsVisible = false;
				xui.GetWindow("mapAreaEnterWaypointName").IsVisible = false;
				xui.playerUI.CursorController.SetNavigationTargetLater(GetChildById("MapView").ViewComponent);
			}
			else
			{
	...
	public void onMapDragged(XUiController _sender, EDragType _dragType, Vector2 _mousePositionDelta)
	{
		...
		{
			if (xui.playerUI.playerInput.GUIActions.LastDeviceClass != InputDeviceClass.Controller)
			{
	...
	{
		float pixelRatioFactor = xui.GetPixelRatioFactor();
	...
	public void updateMapObjectList(List<MapObject> _mapObjectList, bool _bConsiderInOnMouseOverCursor = false)
	{
		...
		for (int i = 0; i < _mapObjectList.Count; i++)
		{
			...
			if (!keyToMapSprite.ContainsKey(num))
			{
				...
				uISprite.atlas = xui.GetAtlasByName(((UnityEngine.Object)uISprite.atlas).name, mapIcon);
			...
			if (_bConsiderInOnMouseOverCursor)
			{
				...
				Vector3 vector3 = mousePosToWindowPos(xui.playerUI.CursorController.GetScreenPosition());
				if (vector2.x > 0f && vector2.x < 712f && vector2.y > 0f && vector2.y < 712f && Utils.FastAbs((vector2 - vector3).magnitude) < 30f)
				{
					...
					if (xui.ToolTipWindow.ToolTip != mapObject.GetName() && !string.IsNullOrEmpty(mapObject.GetName()))
					{
						xui.ToolTipWindow.ToolTip = mapObject.GetName();
		...
		for (int i = 0; i < navObjectList.Count; i++)
		{
			...
			if (currentMapSettings != null)
			{
				...
				if (!keyToNavObject.ContainsKey(key))
				{
					...
					uISprite.atlas = xui.GetAtlasByName(((UnityEngine.Object)uISprite.atlas).name, spriteName);
	...
	public virtual void updateMapObjects()
	{
		...
		if (world.IsEditor() || showStaticData)
		{
			...
			GameManager.Instance.GetDynamicPrefabDecorator()?.GetAllPrefabs(dynamicPrefabBuffer);
			if (dynamicPrefabBuffer.Count > 0)
			{
				for (int j = 0; j < dynamicPrefabBuffer.Count; j++)
				{
					PrefabInstance prefabInstance = dynamicPrefabBuffer[j];
	...
	public void UpdateWaypointSelection()
	{
		...
		foreach (NavObject navObject in NavObjectManager.Instance.NavObjectList)
		{
			...
			if (gameObject2 != null)
			{
				Vector3 b = xui.playerUI.uiCamera.cachedCamera.WorldToScreenPoint(gameObject2.transform.position);
	...
	public Vector3 screenPosToWorldPos(Vector3 _mousePos, bool needY = false)
	{
		...
		Vector3 vector2 = xui.playerUI.camera.WorldToScreenPoint(xuiTexture.UiTransform.position);
		vector.x -= vector2.x;
		vector.y -= vector2.y;
		vector.y *= -1f;
		Bounds xUiWindowScreenBounds = xui.GetXUiWindowScreenBounds(xuiTexture.UiTransform);
	...
	public void onMapPressedLeft(XUiController _sender, int _mouseButton)
	{
		...
		if (closestMouseOverNavObject != null)
		{
			...
			{
				xui.playerUI.entityPlayer.navMarkerHidden = closestMouseOverNavObject.hiddenOnCompass;
				return;
			}
			Waypoint waypointForNavObject = xui.playerUI.entityPlayer.Waypoints.GetWaypointForNavObject(closestMouseOverNavObject);
	...
	public void onMapPressed(XUiController _sender, int _mouseButton)
	{
		...
		if (InputUtils.ControlKeyPressed)
		{
			...
			{
				teleportPlayerOnMap(xui.playerUI.CursorController.GetScreenPosition());
			}
		}
		else
		{
	...
	{
		nextMarkerMousePosition = xui.playerUI.CursorController.GetScreenPosition();
		xui.GetWindow("mapAreaChooseWaypoint").IsVisible = false;
		XUiV_Window window = xui.GetWindow("mapAreaSetWaypoint");
		window.Position = xui.GetMouseXUiPosition();
		window.IsVisible = true;
		xui.playerUI.CursorController.SetNavigationTargetLater(window.Controller.GetChildById("opt1").ViewComponent);
	}
	...
	public void onMapHover(XUiController _sender, bool _isOver)
	{
		...
		XUiC_Paging.IsOverPagingOverrideElement = _isOver;
	}
	...
	public void OnWaypointCreated(string _name)
	{
		...
		xui.playerUI.entityPlayer.Waypoints.Collection.Add(w);
		closeAllPopups();
	...
	public void RefreshAnimalPositionWaypoint(EntityAnimal _animal, bool _unloaded)
	{
		Log.Out("Refresh animal Position Waypoint {0} {1}", _animal.entityId, _unloaded);
		Waypoint waypoint = new Waypoint();
		waypoint.pos = World.worldToBlockPos(_animal.position);
		waypoint.icon = _animal.GetMapIcon();
		waypoint.ownerId = PlatformManager.InternalLocalUserIdentifier;
		waypoint.name.Update(Localization.Get(_animal.EntityName), PlatformManager.MultiPlatform.User.PlatformUserId);
		waypoint.lastKnownPositionEntityType = eLastKnownPositionEntityType.Animal;
		waypoint.lastKnownPositionEntityId = _animal.entityId;
		EntityPlayer entityPlayer = xui.playerUI.entityPlayer;
		if (!entityPlayer.Waypoints.ContainsLastKnownPositionWaypoint(_animal.entityId))
		{
			entityPlayer.Waypoints.Collection.Add(waypoint);
			if (waypoint.CanBeViewedBy(PlatformManager.InternalLocalUserIdentifier))
			{
				...
				waypoint.navObject = NavObjectManager.Instance.RegisterNavObject("waypoint", waypoint.pos.ToVector3(), waypoint.icon);
				waypoint.navObject.IsActive = false;
				waypoint.navObject.OverrideSpriteName = _animal.GetMapIcon();
				waypoint.navObject.name = waypoint.name.Text;
				waypoint.navObject.usingLocalizationId = waypoint.bUsingLocalizationId;
			}
			else
			{
				RemoveAnimalLastKnownWaypoint(_animal);
			}
		}
		else if (waypoint.CanBeViewedBy(PlatformManager.InternalLocalUserIdentifier))
		{
			entityPlayer.Waypoints.UpdateEntityAnimalWayPoint(_animal, _unloaded);
		}
		else
		{
			RemoveAnimalLastKnownWaypoint(_animal);
	...
	{
		if (xui.playerUI.entityPlayer.Waypoints.TryRemoveLastKnownPositionWaypoint(_vehicle.entityId))
		{
			((XUiC_MapWaypointList)base.Parent.GetChildById("waypointList")).UpdateWaypointsList();
		}
	}

	public void RemoveAnimalLastKnownWaypoint(EntityAnimal _animal)
	{
		if (xui.playerUI.entityPlayer.Waypoints.TryRemoveLastKnownPositionWaypoint(_animal.entityId))
		{
			((XUiC_MapWaypointList)base.Parent.GetChildById("waypointList")).UpdateWaypointsList();
		}
	}
	...
	{
		if (xui.playerUI.entityPlayer.Waypoints.TryRemoveLastKnownPositionWaypoint(_entityId))
		{
			((XUiC_MapWaypointList)base.Parent.GetChildById("waypointList")).UpdateWaypointsList();
		}
	}
	...
	{
		xui.GetWindow("mapAreaSetWaypoint").IsVisible = false;
		xui.GetWindow("mapAreaChooseWaypoint").IsVisible = false;
		xui.GetWindow("mapAreaEnterWaypointName").IsVisible = false;
		xui.GetWindow("mapTrackingPopup").IsVisible = false;
