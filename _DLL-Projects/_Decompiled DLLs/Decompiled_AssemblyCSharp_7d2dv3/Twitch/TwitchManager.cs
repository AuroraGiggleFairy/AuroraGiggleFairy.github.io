...
public class TwitchManager
{
		...
		case InitStates.CheckingForExtension:
			...
			else
			{
				...
				ExtensionManager.CheckExtensionInstalled([PublicizedFrom(EAccessModifier.Private)] (bool IsInstalled) =>
				{
					...
					if (!IsInstalled)
					{
						XUiC_MessageBoxWindowGroup.ShowOk(LocalPlayerXUi, Localization.Get("xuiTwitchPopup_ExtensionNeededHeader"), Localization.Get("xuiTwitchPopup_ExtensionNeeded"));
	...
	public void HandleExtensionMessage(int userId, string message, bool isRerun, int creditUsed, int bitsUsed)
	{
		...
		if (!VotingManager.VotingIsActive && twitchActive && (flag2 || (VoteLockedLevel != TwitchVoteLockTypes.ActionsLocked && AllowActions)))
		{
			...
			if (flag2 || isRerun || twitchAction.IgnoreCooldown || !twitchAction.CooldownBlocked || !OnCooldown || ((CooldownType == CooldownTypes.MaxReachedWaiting || CooldownType == CooldownTypes.SafeCooldown) && !twitchAction.WaitingBlocked))
			{
				...
				if ((isRerun || twitchAction.IsReady(this)) && ViewerData.HandleInitialActionEntrySetup(value, twitchAction, isRerun, flag2, bitsUsed, out actionEntry))
				{
					...
					if (flag2)
					{
						...
						int creditsUsed = actionEntry.CreditsUsed;
						int num = Utils.FastMax(0, currentCost - creditsUsed);
						if (bitsUsed < num)
						{
							Debug.LogWarning("[Bits] Insufficient bits for '" + text + "' by " + value + ". " + $"Cost={currentCost} CreditsSpent={creditsUsed} BitsRequired={num} BitsUsed={bitsUsed}. " + "Rejecting + reimbursing credits + resyncing.");
							if (creditsUsed > 0)
							{
								ViewerData.ReimburseAction(value, creditsUsed, twitchAction);
							}
							ViewerEntry viewerEntry3 = ViewerData.GetViewerEntry(value);
							if (viewerEntry3 != null)
							{
								PushBalanceToExtensionQueue(viewerEntry3.UserID.ToString(), viewerEntry3.BitCredits);
							}
							return;
						}
						if (creditsUsed != creditUsed)
						{
							Debug.LogWarning("[Bits] Credit split mismatch for '" + text + "' by " + value + ". " + $"ClientCreditsSpent={creditsUsed} ExtCreditUsed={creditUsed}. Proceeding (client-authoritative).");
						}
						int num2 = bitsUsed - num;
						if (num2 > 0)
						{
							ViewerData.AddCredit(value, num2, displayNewTotal: false);
						}
					}
		...
		if (!flag && flag2)
		{
			Debug.LogWarning("[Bits] Bit action '" + text + "' did not start for " + value + ". Rejecting + resyncing balance (no crediting). " + $"ExtCreditUsed={creditUsed} BitsUsed={bitsUsed} Cost={twitchAction.CurrentCost}");
			ViewerEntry viewerEntry5 = ViewerData.GetViewerEntry(value);
			if (viewerEntry5 != null)
			{
				PushBalanceToExtensionQueue(viewerEntry5.UserID.ToString(), viewerEntry5.BitCredits);
			}
		}
	}
	...
	{
		ViewerData.WriteExport(_filePath);
	}

	public void LoadExportViewerData(string _filePath)
	{
		using StreamReader tr = File.OpenText(_filePath);
	...
	public bool LoadLatestMainViewerData(UserDataStorageType _userDataStorage)
	{
		...
		{
			if (LoadLatestMainViewerData(_userDataStorage, num))
			{
				return true;
			}
		}
	...
	{
		string text = GameIO.GetUserGameDataDir(_userDataStorage) + "/Twitch/" + version;
	...
	public void LoadMainViewerData()
	{
		...
		if (!LoadViewerDataFromStorage(defaultSaveStorage))
		{
			UserDataStorageType userDataStorageType = UserDataStorageType.DeviceLocal;
			if (defaultSaveStorage != userDataStorageType && LoadViewerDataFromStorage(userDataStorageType))
			{
				Log.Out($"Twitch main viewer data found in {userDataStorageType}");
			}
		}
		[PublicizedFrom(EAccessModifier.Private)]
		bool LoadViewerDataFromStorage(UserDataStorageType storageType)
		{
			if (LoadLatestMainViewerData(storageType))
			{
				return true;
			}
			if (LoadMainViewerData(storageType))
			{
				return true;
			}
			if (LoadSpecialViewerData(storageType))
			{
				return true;
			}
			return false;
