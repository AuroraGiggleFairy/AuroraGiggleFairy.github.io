...
public class TwitchAction
{
	...
	public virtual bool ParseProperties(DynamicProperties properties)
	{
		...
		if (properties.Values.ContainsKey(PropSpecialRequirement))
		{
			...
			foreach (string text in array2)
			{
				...
				{
					Log.Error("TwitchAction " + Title + " has unknown SpecialRequirement " + text);
				}
			}
		}
	...
	{
		if (TwitchRequirements == null)
		{
			TwitchRequirements = new List<BaseTwitchRequirement>();
		}
		requirement.OwnerAction = this;
		requirement.OwnerType = BaseTwitchRequirement.OwnerTypes.Action;
		TwitchRequirements.Add(requirement);
	}
	...
	public bool CheckAllowed()
	{
		if (TwitchRequirements != null)
		{
			...
			for (int i = 0; i < TwitchRequirements.Count; i++)
			{
				BaseTwitchRequirement baseTwitchRequirement = TwitchRequirements[i];
	...
	public virtual bool IsReady(TwitchManager twitchManager)
	{
		...
		{
			for (int j = 0; j < SpecialRequirementList.Count; j++)
			{
				switch (SpecialRequirementList[j])
				{
				...
				case SpecialRequirements.Night:
					...
					break;
				case SpecialRequirements.NotAllDaySetting:
