...
public class EosUserIdMapper : IUserIdentifierMappingService
{
	...
	public bool TryValidateUser(out ProductUserId loggedInUser, out MappedAccountQueryResult failureResult)
	{
		...
		if (!(platformUserId is UserIdentifierEos userIdentifierEos))
		{
			Log.Error($"[EOS] Cannot query mapped account details. EosUserIdMapper got non-EOS id type: {platformUserId}");
			return false;
		}
		loggedInUser = userIdentifierEos.ProductUserId;
		if (loggedInUser == null)
		{
			Log.Out($"[EOS] Cannot query mapped account details - EOS does not have info yet for {userIdentifierEos}");
			failureResult = MappedAccountQueryResult.NotReady;
			return false;
		}
		return true;
	...
	{
		ProductUserId _puid;
		if (!TryValidateUser(out var loggedInUser, out var failureResult))
		{
			_callback(failureResult, _platform, null, null);
		}
		else if (!TryValidateRequest(_id, _platform, out _puid))
		{
	...
	public void QueryMappedAccountsDetails(IReadOnlyList<MappedAccountRequest> _requests, MappedAccountsQueryCallback _callback)
	{
		...
		MappedAccountQueryResult failureResult;
		if (_requests.Count == 0)
		{
			_callback(_requests);
		}
		else if (!TryValidateUser(out loggedInUser, out failureResult))
		{
			foreach (MappedAccountRequest _request in _requests)
			{
				_request.Result = failureResult;
			}
			_callback(_requests);
		}
		else
