...
public class Api : IPlatformApi
{
	...
	public PlatformInterface createPlatformInterface(EosCreds _creds, bool _serverMode)
	{
		...
		options.CacheDirectory = GameIO.GetDeviceLocalUserGameDataDir();
	...
	public void logCallback(ref LogMessage _message)
	{
		...
		if (DebugLevel != EDebugLevel.Off || _message.Level != LogLevel.Warning || !(_message.Category == (Utf8String)"LogEOSRTC") || !((string)_message.Message).StartsWith("TickTracker Ticks have been delayed.", StringComparison.Ordinal))
		{
