...
public class XUiC_SkillCraftingInfoWindow : XUiC_InfoWindow
{
	...
	public ProgressionValue CurrentSkill
	{
		...
		get
		{
			if (xui.SelectedSkill == null || !xui.SelectedSkill.ProgressionClass.IsCrafting)
			{
				return null;
			}
	...
	public void UpdateSkill()
	{
		...
		if (progressionClass != null && progressionClass.DisplayDataList != null)
		{
			XUiC_SkillEntry entryForSkill = windowGroup.Controller.GetChildByType<XUiC_SkillList>().GetEntryForSkill(CurrentSkill);
			{
				foreach (XUiC_SkillCraftingInfoEntry levelEntry in levelEntries)
				{
					ProgressionClass.DisplayData displayData = ((progressionClass.DisplayDataList.Count > num) ? progressionClass.DisplayDataList[num] : null);
					levelEntry.Data = ((displayData != null && displayData.Enabled) ? displayData : null);
