...
public class WorldStats
{
	public class MeshStats
	{
		...
		public DynamicProperties ToProperties()
		{
			...
			Dictionary<string, string> values = dynamicProperties.Values;
			int vertices = Vertices;
			values["Vertices"] = vertices.ToString();
			Dictionary<string, string> values2 = dynamicProperties.Values;
			vertices = Triangles;
	...
	public static WorldStats FromProperties(DynamicProperties _props)
	{
		...
		if (_props.Classes.TryGetValue("BlockEntities", out var value))
		{
			blockEntities = MeshStats.FromProperties(value);
		}
		MeshStats[] array = new MeshStats[(MeshDescription.meshes.Length != 0) ? MeshDescription.meshes.Length : 20];
		for (int i = 0; i < array.Length; i++)
		{
			if (_props.Classes.TryGetValue("ChunkMeshes" + i, out value))
			{
				array[i] = MeshStats.FromProperties(value);
			}
		}
		float _result = 0f;
		if (_props.Values.TryGetValue("LightsVolume", out var value2) && !StringParsers.TryParseFloat(value2, out _result))
		{
			Log.Warning("Could not parse LightsVolume string '" + value2 + "'");
		}
	...
	public static WorldStats CaptureWorldStats()
	{
		...
		foreach (ChunkGameObject item in displayedChunkGameObjects)
		{
			...
			for (int i = 0; i < 16; i++)
			{
				...
				{
					MeshFilter meshFilter = layer.m_MeshFilter[j];
					if ((bool)meshFilter)
					{
		...
		MeshStats[] array3 = new MeshStats[array.Length];
		for (int k = 0; k < array.Length; k++)
		{
			array3[k] = new MeshStats(array[k], array2[k]);
		}
		WorldStats result = new WorldStats(new MeshStats(_verts, _tris), array3, _lightsVolume);
	...
	public static IEnumerator CaptureCameraStatsCo(XUi _xui)
	{
		...
		GameObject uiRootObj = Object.FindAnyObjectByType<UIRoot>().gameObject;
