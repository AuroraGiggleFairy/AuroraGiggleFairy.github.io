...
public class ItemClassArmor : ItemClass
{
	...
	{
		if (!World.TemperatureSurvival && World.StormFrequency == 0f && EquipSlot >= EquipmentSlots.ClothingHead && EquipSlot <= EquipmentSlots.ClothingFeet)
		{
			return false;
		}
		if (!World.BiomeProgressionEnabled && EquipSlot >= EquipmentSlots.BiomeBadge && EquipSlot <= EquipmentSlots.BiomeBadge4)
		{
			return false;
		}
		return true;
