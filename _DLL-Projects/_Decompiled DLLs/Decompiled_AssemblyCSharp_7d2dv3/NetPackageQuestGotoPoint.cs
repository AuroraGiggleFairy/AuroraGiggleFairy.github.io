	...
	public override void ProcessPackage(World _world, GameManager _callbacks)
	{
		...
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			for (int i = 0; i < 5; i++)
			{
				...
				{
					prefabInstance = GameManager.Instance.World.ChunkCache.ChunkProvider.GetDynamicPrefabDecorator().GetClosestPOIToWorldPos(questTags, new Vector2(entityAlive.position.x, entityAlive.position.z), null, -1, ignoreCurrentPOI: false, biomeFilterType, biomeFilter);
					if (prefabInstance == null)
					{
						prefabInstance = GameManager.Instance.World.ChunkCache.ChunkProvider.GetDynamicPrefabDecorator().GetClosestPOIToWorldPos(questTags, new Vector2(entityAlive.position.x, entityAlive.position.z));
					}
				}
				...
				{
					prefabInstance = DynamicPrefabDecorator.GetRandomPOINearWorldPos(new Vector2(entityAlive.position.x, entityAlive.position.z), 100, 50000000, questTags, difficulty);
				}
