...
public class StabilityCalculator
{
	...
	public void BlockRemovedAt(Vector3i _pos)
	{
		...
		foreach (Vector3i vector3i in allDirections)
		{
			...
			int stability = chunk.GetStability(x, y, z);
			if (stability == 0)
			{
	...
	public void physicsIsolation(Vector3i _pos)
	{
		...
		while (queueIsolation.Count > 0)
		{
			...
			for (int i = 0; i < allDirections.Length; i++)
			{
				...
				if (!world.GetChunkFromWorldPos(zero, ref chunk))
				{
					continue;
				}
		...
		if (block.isMultiBlock)
		{
			...
			for (int i = v.z; i <= v2.z; i++)
			{
				...
				for (int j = v.x; j <= v2.x; j++)
				{
					...
					if (chunk != null && chunk.GetStability(j & 0xF, v.y - 1, i & 0xF) == 15)
					{
	...
	public static float GetBlockStability(Vector3i _pos, BlockValue _newBV)
	{
		...
		while (num4 < 25 && posToCheck.Count > 0)
		{
			...
			for (int i = 0; i < posToCheck.Count; i++)
			{
				...
				{
					world.GetChunkFromWorldPos(vector3i.x, vector3i.z, ref chunk);
					if (chunk == null)
					{
						continue;
					}
					blockValue = chunk.GetBlockNoDamage(vector3i.x & 0xF, vector3i.y, vector3i.z & 0xF);
				}
				num3 += 7;
				...
				for (int j = 0; j < allDirectionsShuffled.Length; j++)
				{
					...
					{
						world.GetChunkFromWorldPos(vector3i2.x, vector3i2.z, ref chunk);
						if (chunk == null)
						{
							continue;
						}
						...
						value2 = chunk.GetStability(x, vector3i2.y, z);
					}
					if (value2 > 0)
					{
						if (value2 == 15)
	...
	public List<Vector3i> CalcPhysicsStabilityToFall(Vector3i _pos, int maxBlocksToCheck, out float calculatedStability)
	{
		...
		for (int i = 0; i < maxBlocksToCheck; i++)
		{
			...
			{
				world.GetChunkFromWorldPos(item, ref chunk);
				BlockValue blockValue = chunk?.GetBlockNoDamage(World.toBlockXZ(item.x), item.y, World.toBlockXZ(item.z)) ?? BlockValue.Air;
				...
				for (int j = 0; j < allDirectionsShuffled.Length; j++)
				{
					...
					if (chunk == null || chunk.X != World.toChunkXZ(vector3i2.x) || chunk.Z != World.toChunkXZ(vector3i2.z))
					{
						chunk = world.GetChunkFromWorldPos(vector3i2);
					}
					int x = World.toBlockXZ(vector3i2.x);
					int z = World.toBlockXZ(vector3i2.z);
					BlockValue other = chunk?.GetBlockNoDamage(x, vector3i2.y, z) ?? BlockValue.Air;
					int num4 = ((!other.isair && chunk != null) ? chunk.GetStability(x, vector3i2.y, z) : 0);
					if (num4 == 15)
					{
						int forceToOtherBlock = blockValue.GetForceToOtherBlock(other);
	...
	public void ClearChunkStabilityQueues(HashSetLong chunks)
	{
		resetTempPositions.Clear();
		while (queueStabilityEmpty.Count > 0)
		{
			Vector3i item = queueStabilityEmpty.Dequeue();
			long item2 = WorldChunkCache.MakeChunkKey(World.toChunkXZ(item.x), World.toChunkXZ(item.z));
			if (!chunks.Contains(item2))
			{
				resetTempPositions.Add(item);
			}
		}
		queueStabilityEmpty = new Queue<Vector3i>(resetTempPositions);
		resetTempPositions.Clear();
		while (queueStabilityAvail.Count > 0)
		{
			Vector3i item3 = queueStabilityAvail.Dequeue();
			long item4 = WorldChunkCache.MakeChunkKey(World.toChunkXZ(item3.x), World.toChunkXZ(item3.z));
			if (!chunks.Contains(item4))
			{
				resetTempPositions.Add(item3);
			}
		}
		queueStabilityAvail = new Queue<Vector3i>(resetTempPositions);
		resetTempPositions.Clear();
	}
}
