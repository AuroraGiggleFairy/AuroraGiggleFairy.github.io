...
public class TextEllipsisAnimator
{
	...
	{
		uiLabel = _uiLabel;
		isChinese = Localization.ActiveLanguage == "schinese" || Localization.ActiveLanguage == "tchinese";
		lastEllipsisState = (isChinese ? ellipsisEndingsChinese[2] : ellipsisEndings[3]);
		Init(_input);
	}

	public TextEllipsisAnimator(string _input, XUiV_Label _label, UILabel _uiLabel)
	{
		uiLabel = _uiLabel;
		xuiLabel = _label;
		isChinese = Localization.ActiveLanguage == "schinese" || Localization.ActiveLanguage == "tchinese";
	...
	public void SetBaseString(string _input, AnimationMode _mode = AnimationMode.All)
	{
		...
		xuiLabel?.SetTextImmediately(baseString);
		int processedMatches = 0;
		for (int l = 0; l < totalEllipsis; l++)
		{
			if (xuiLabel != null && xuiLabel.Text == uiLabel.processedText)
			{
				break;
			}
	...
	public void UpdateLabel()
	{
		if (totalEllipsis > 0)
		{
			...
			if (xuiLabel != null)
			{
				xuiLabel.SetTextImmediately(animatedStrings[currentEllipsisState]);
			}
			else
			{
