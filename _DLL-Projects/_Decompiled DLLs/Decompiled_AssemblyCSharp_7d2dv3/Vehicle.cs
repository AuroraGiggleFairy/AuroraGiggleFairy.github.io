...
public class Vehicle
{
	...
	public void SetSeats()
	{
		...
		if (Properties != null)
		{
			...
			for (int i = 0; i < 99; i++)
			{
				if (!Properties.Classes.TryGetValue("seat" + i, out var value))
				{
					break;
				}
				if (value.GetString("mod").Length > 0)
				{
					if (num3 >= num)
					{
						break;
	...
	{
		if (Properties != null && Properties.Classes.TryGetValue("seat" + _seatIndex, out var value))
		{
			string text = value.GetString("pose");
			if (text.Length > 0)
			{
	...
	public DynamicProperties GetPropertiesForClass(string className)
	{
		...
		Properties.Classes.TryGetValue(className, out var value);
		return value;
	}
	...
	public void OnXMLChanged()
	{
		...
		foreach (KeyValuePair<string, DynamicProperties> @class in properties.Classes)
		{
			VehiclePart vehiclePart = FindPart(@class.Key);
			if (vehiclePart != null)
			{
				DynamicProperties value = @class.Value;
	...
	public void CalcEffects()
	{
		...
		EffectEntityDamagePer = EffectManager.GetValue(PassiveEffects.VehicleEntityDamage, itemValue, 1f, entityAlive, null, entityTags) * EntityVehicle.VehicleEntityDamageModifier;
		EffectBlockDamagePer = EffectManager.GetValue(PassiveEffects.VehicleBlockDamage, itemValue, 1f, entityAlive, null, entityTags) * EntityVehicle.VehicleBlockDamageModifier;
		EffectSelfDamagePer = EffectManager.GetValue(PassiveEffects.VehicleSelfDamage, itemValue, 1f, entityAlive, null, entityTags) * EntityVehicle.VehicleSelfDamageModifier;
		EffectStrongSelfDamagePer = EffectManager.GetValue(PassiveEffects.VehicleStrongSelfDamage, itemValue, 1f, entityAlive, null, entityTags) * EntityVehicle.VehicleSelfDamageModifier;
		...
		EffectFuelUsePer = EffectManager.GetValue(PassiveEffects.VehicleFuelUsePer, itemValue, 1f, entityAlive, null, entityTags) * EntityVehicle.VehicleFuelUsageModifier;
	...
	public void CalcMods()
	{
		...
		if (modifications != null)
		{
			foreach (ItemValue itemValue in modifications)
			{
				...
				{
					ModTags |= itemClassModifier.ModifierTags;
					if (itemClassModifier.ModifierTags.Test_AnySet(EntityVehicle.StorageModifierTags))
					{
						num++;
					}
				}
