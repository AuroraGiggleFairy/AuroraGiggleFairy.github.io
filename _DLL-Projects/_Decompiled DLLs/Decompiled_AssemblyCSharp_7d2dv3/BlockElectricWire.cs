...
public class BlockElectricWire : BlockPowered
{
	...
	public override void OnBlockAdded(WorldBase _world, Chunk _chunk, Vector3i _blockPos, BlockValue _blockValue, PlatformUserIdentifierAbs _addedByPlayer)
	{
		...
		TileEntityPoweredMeleeTrap tileEntityPoweredMeleeTrap = _world.GetTileEntity(_blockPos) as TileEntityPoweredMeleeTrap;
		if (tileEntityPoweredMeleeTrap == null)
		{
			tileEntityPoweredMeleeTrap = CreateTileEntity(_chunk) as TileEntityPoweredMeleeTrap;
			tileEntityPoweredMeleeTrap.SetDisableModifiedCheck(_b: true);
			...
			tileEntityPoweredMeleeTrap.SetDisableModifiedCheck(_b: false);
	...
	public override int OnBlockDamaged(WorldBase _world, BlockValueRef _bvRef, BlockValue _blockValue, int _damagePoints, int _entityIdThatDamaged, ItemActionAttack.AttackHitInfo _attackHitInfo, bool _bUseHarvestTool, bool _bBypassMaxDamage, int _recDepth = 0)
	{
		if ((_blockValue.meta & 2) != 0 && 1f - (float)_blockValue.damage / (float)_blockValue.Block.MaxDamage > brokenPercentage)
		{
			...
			if (buffActions != null && _world.GetTileEntity(_bvRef) is TileEntityPoweredMeleeTrap { IsPowered: not false } tileEntityPoweredMeleeTrap && _world.GetEntity(_entityIdThatDamaged) is EntityAlive entityAlive)
			{
