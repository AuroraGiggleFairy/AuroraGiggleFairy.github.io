...
public class BlockGameEvent : Block
{
	...
	public override void OnBlockAdded(WorldBase _world, Chunk _chunk, Vector3i _blockPos, BlockValue _blockValue, PlatformUserIdentifierAbs _addedByPlayer)
	{
		...
		if (destroyOnEvent)
		{
			DamageBlock(_world, _blockPos, _blockValue, _blockValue.Block.MaxDamage - _blockValue.damage, -1);
		}
	}
	...
	public override int OnBlockDamaged(WorldBase _world, BlockValueRef _bvRef, BlockValue _blockValue, int _damagePoints, int _entityIdThatDamaged, ItemActionAttack.AttackHitInfo _attackHitInfo, bool _bUseHarvestTool, bool _bBypassMaxDamage, int _recDepth = 0)
	{
		if (onDamageEvent != "")
		{
			...
			else
			{
				...
				GameEventManager.Current.HandleAction(onDamageEvent, null, entity as EntityPlayer, twitchActivated: false, _bvRef);
				if (destroyOnEvent)
				{
					DamageBlock(_world, _bvRef, _blockValue, _blockValue.Block.MaxDamage - _blockValue.damage, -1);
				}
			}
		}
		...
		if (num > 0 && sendDamageUpdate && GameManager.Instance.World.GetEntity(_entityIdThatDamaged) is EntityPlayer)
		{
			...
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageGameEventResponse>().Setup(NetPackageGameEventResponse.ResponseTypes.BlockDamaged, _bvRef));
		}
		return num;
	}
