...
public class EntityItem : Entity
{
	...
	public override void InitLocalActivationCommands(Action<EntityActivationCommand> _addCallback)
	{
		...
		_addCallback(new EntityActivationCommand("search", "search"));
	}
	...
	public override void OnEntityActivated(EntityActivationCommand _command, EntityPlayerLocal _playerFocusing)
	{
		...
		else if (CommandIs(_command.commandId, "search"))
		{
	...
	public override string GetActivationText()
	{
		...
		PlayerActionsLocal playerInput = entityPlayerLocal.playerInput;
		...
		if (CanCollect() && onGround)
		{
			string localizedItemName = ItemClass.GetForId(itemStack.itemValue.type).GetLocalizedItemName();
			if (itemStack.count > 1)
			{
				return string.Format(Localization.Get("itemTooltipFocusedSeveral"), arg, localizedItemName, itemStack.count);
			}
			return string.Format(Localization.Get("itemTooltipFocusedOne"), arg, localizedItemName);
		}
		if (bag != null)
		{
			string arg2 = Localization.Get(LocalizedEntityName);
			if (!bag.Touched)
			{
				return string.Format(Localization.Get("lootTooltipNew"), arg, arg2);
			}
			if (bag.IsEmpty())
			{
				return string.Format(Localization.Get("lootTooltipEmpty"), arg, arg2);
			}
			return string.Format(Localization.Get("lootTooltipTouched"), arg, arg2);
		}
		return string.Empty;
	}
	...
	{
		EntityPlayerLocal obj = world.GetEntity(_playerId) as EntityPlayerLocal;
		LocalPlayerUI uIForPlayer = LocalPlayerUI.GetUIForPlayer(obj);
		bool num = obj.inventory.IsHoldingItemActionRunning();
		int num2 = (num ? uIForPlayer.xui.PlayerInventory.CountAvailableSpaceForItem(itemStack.itemValue, _limitToOneStack: false) : (-1));
		if ((num && num2 - itemStack.itemValue.ItemClass.Stacknumber.Value <= itemStack.count) || !uIForPlayer.xui.PlayerInventory.AddItem(itemStack))
		{
			GameManager.Instance.ItemDropServer(itemStack, GetPosition(), Vector3.zero, _playerId);
		}
	}
	...
	public void OnCollisionEnter(Collision collision)
	{
		...
		for (int num = collision.GetContacts(contactPoints) - 1; num >= 0; num--)
		{
			...
			if ((object)hitRootEntity != null)
			{
				...
				if (!string.IsNullOrEmpty(value) && itemClass != null && itemClass.MadeOfMaterial != null)
				{
					playThrowSound(itemClass.MadeOfMaterial.id + "hit" + value);
				}
				break;
			}
