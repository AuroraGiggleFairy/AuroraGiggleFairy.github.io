	...
	public override bool GetBindingValueInternal(ref string value, string bindingName)
	{
		...
		case "questicon":
			...
			if (flag)
			{
				...
				else if (quest.CurrentState == Quest.QuestState.InProgress || quest.CurrentState == Quest.QuestState.ReadyForTurnIn)
				{
					...
					else if (quest.QuestClass.AddsToTierComplete && !xui.playerUI.entityPlayer.QuestJournal.CanAddProgression)
					{
		...
		case "distance":
			if (flag && (quest.Active || SharedQuestEntry != null) && quest.HasPosition)
			{
				...
				Vector3 position2 = xui.playerUI.entityPlayer.GetPosition();
				position.y = 0f;
				position2.y = 0f;
				...
				if (quest.MapObject is MapObjectTreasureChest)
				{
					...
					if (num2 < num3)
					{
						num3 = EffectManager.GetValue(PassiveEffects.TreasureRadius, null, num3, xui.playerUI.entityPlayer);
