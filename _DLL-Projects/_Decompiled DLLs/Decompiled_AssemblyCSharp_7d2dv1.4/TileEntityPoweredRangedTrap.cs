...
public class TileEntityPoweredRangedTrap : TileEntityPoweredBlock
{
	...
	public ItemClass AmmoItem
	{
		get
		{
			if (ammoItem == null)
			{
				...
				{
					ammoItem = ItemClass.GetItemClass(blockLauncher.AmmoItemName);
				}
				else if (block is BlockRanged blockRanged)
				{
					ammoItem = ItemClass.GetItemClass(blockRanged.AmmoItemName);
				}
			}
			return ammoItem;
		}
	...
	public override void ClientUpdate()
	{
		if (Time.time > updateTime)
		{
			...
			if (block2 is BlockLauncher blockLauncher)
			{
				blockLauncher.InstantiateProjectile(world, GetClrIdx(), ToWorldPos());
			}
			else if (block2 is BlockRanged)
			{
