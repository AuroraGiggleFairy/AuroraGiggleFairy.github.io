...
public class EntityDrone : EntityNPC, ILockable
{
	...
	public override void OnEntityUnload()
	{
		...
		if (!isBeingPickedUp)
		{
			...
			{
				NavObjectManager.Instance.RegisterNavObject(entityClass.NavObject, position);
			}
		}
	...
	public override int DamageEntity(DamageSource _damageSource, int _strength, bool _criticalHit, float _impulseScale)
	{
		...
		{
			Buffs.RemoveBuff("buffInjuryBleeding");
			strength = 0;
		}
		EntityAlive entityAlive = (EntityAlive)world.GetEntity(_damageSource.getEntityId());
		if ((bool)Owner && (bool)entityAlive)
		{
			if (!debugFriendlyFire && (bool)entityAlive && entityAlive.factionId == Owner.factionId)
			{
				strength = 0;
			}
		}
	...
	public override void OnUpdateEntity()
	{
		...
		UpdateAnimStates();
		UpdateShutdownState();
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer && (bool)Owner && isOutOfRange(Owner.position, MaxDistFromOwner) && state != State.Shutdown && state != State.Sentry)
		{
			teleportState();
		}
		if (!isQuietMode && idleLoop == null && state == State.Idle && !GameManager.IsDedicatedServer)
		{
			idleLoop = PlaySoundLoop("drone_idle_hover", 0.2f);
		}
		if (!isQuietMode && idleLoop != null && state == State.Idle && !GameManager.IsDedicatedServer)
		{
			notifySoundNoise(0.2f, 5f);
		}
		...
		if ((bool)Owner)
		{
			if ((bool)currentTarget)
			{
				if (!steering.IsInRange(currentTarget.position, FollowDistance * 2f))
				{
					...
					currentSpeedFlying = Mathf.Lerp(currentSpeedFlying, 15f, Mathf.Clamp01(accelerationTime / SpeedFlying));
				}
				else
				{
	...
	public void UpdatePartyBuffs()
	{
		if ((bool)Owner && isSupportModAttached && !isEntityRemote)
		{
			...
			if (!partyEventsSet && (bool)entityPlayer && entityPlayer.Party != null)
			{
				registerPartyEvents(entityPlayer, value: true);
			}
		}
	}
	...
	{
		IsEntityUpdatedInUnloadedChunk = value;
		bWillRespawn = value;
	}
	...
	public void registerPartyEvents(EntityPlayer player, bool value)
	{
		if (value)
		{
			player.Party.PartyMemberRemoved += OnPartyMemberRemoved;
		}
		else
		{
			player.Party.PartyMemberRemoved -= OnPartyMemberRemoved;
		}
		partyEventsSet = value;
	}
	...
	public void pickup(Entity _entityFocusing)
	{
		...
		if (entityPlayer.inventory.CanTakeItem(itemStack) || entityPlayer.bag.CanTakeItem(itemStack))
		{
			...
			if (entityPlayer.Party != null)
			{
				registerPartyEvents(entityPlayer, value: false);
			}
			UnRegsiterMovingLights();
		}
		else
	...
	public void move(Vector3 dir, float speedFlying)
	{
		...
		if (!RaycastPathUtils.IsPositionBlocked(position, end, 1073807360))
		{
	...
	{
		IsHealingAllies = !IsHealingAllies;
		allyHealMode = (IsHealingAllies ? AllyHealMode.HealAllies : AllyHealMode.DoNotHeal);
	}
	...
	public void Heal()
	{
		...
		if (!IsHealingAllies && state != State.Heal && healWeapon.canFire() && healWeapon.targetNeedsHealing(Owner))
		{
			HealOwnerServer();
		}
	}
	...
	{
		if (state != State.Heal && healWeapon.canFire())
		{
			if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
			{
				currentTarget = Owner;
	...
	{
		if (state != State.Heal && healWeapon.canFire() && SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			currentTarget = Owner;
			healTarget(Owner);
		}
	...
	public void followState()
	{
		...
		if (isTargetUnderWater(currentTarget.getHeadPosition()))
		{
			...
			Vector3 dir = steering.Seek(position, findOpenBlockAbove(currentTarget.getHeadPosition()), 0.2f);
			rotateTo(dir);
			move(dir);
			return;
		}
		float magnitude = (currentTarget.getHeadPosition() - position).magnitude;
		findOpenFollowPoints(debugDraw: true);
		...
		if (currentPath.Count > 0)
		{
			...
			Vector3 dir2 = steering.Seek(position, currentPathTarget, 1f);
		...
		if (state == State.Follow && steering.IsInRange(followTargetPos, 0.5f))
		{
			debugPathDelay = 2f;
			setState(State.Idle);
		}
	...
	public virtual void healState()
	{
		...
		if (magnitude <= FollowDistance && healWeapon.canFire())
		{
			healWeapon.RegisterOnFireComplete(onHealDone);
			healWeapon.Fire(currentTarget);
		}
		else if (healWeapon.hasActionCompleted())
		{
			setState(State.Idle);
			SendSyncData(32768);
		}
	...
	public virtual void teleportState()
	{
		if (!isTeleporting)
		{
			...
			Vector3 vector = Owner.getHeadPosition() - new Vector3(Owner.GetLookVector().x, 0f, Owner.GetLookVector().z) * FollowDistance;
			isTeleporting = true;
			clearCurrentPath();
			motion = Vector3.zero;
			SetPosition(vector);
			orderState = Orders.Follow;
	...
	public void UpdateDroneSystems()
	{
		...
		{
			if (state != State.Shutdown && state != State.Sentry && state != State.Heal && healWeapon.targetNeedsHealing(Owner))
			{
				Heal();
			}
			healWeapon.Update();
	...
	public void updateState()
	{
		...
		{
		case State.Idle:
			if (isTargetUnderWater(Owner.getHeadPosition()))
			{
				currentTarget = Owner;
				setState(State.Follow);
			}
