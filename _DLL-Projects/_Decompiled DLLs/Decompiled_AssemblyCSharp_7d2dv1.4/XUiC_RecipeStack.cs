...
public class XUiC_RecipeStack : XUiController
{
	...
	public void CopyTo(XUiC_RecipeStack _recipeStack)
	{
		...
		_recipeStack.destinationToolbeltSlot = destinationToolbeltSlot;
	}
	...
	public bool outputStack()
	{
		...
		if (originalItem == null || originalItem.Equals(ItemValue.None))
		{
			...
			{
				xUiC_WorkstationWindowGroup.WorkstationData.TileEntity.AddCraftComplete(startingEntityId, outputItemValue, recipe.GetName(), recipe.craftExpGain, recipe.count);
			}
		...
		else
		{
			...
			if (destinationToolbeltSlot >= 0 && base.xui.PlayerInventory.AddItemToPreferredToolbeltSlot(itemStack2, destinationToolbeltSlot))
			{
				return true;
			}
	...
	public bool SetRepairRecipe(float _repairTimeLeft, ItemValue _itemToRepair, int _amountToRepair, int _sourceToolbeltSlot = -1)
	{
		...
		destinationToolbeltSlot = _sourceToolbeltSlot;
