...
public class PassiveEffect
{
	...
	public static PassiveEffect ParsePassiveEffect(XElement _element)
	{
		...
		{
			passiveEffect.CVarValues = new string[1] { attribute3.Trim().Remove(0, 1) };
			passiveEffect.Values = new float[1];
		}
		else
		{
			passiveEffect.Values = new float[1] { StringParsers.ParseFloat(attribute3) };
		}
		if (passiveEffect.CVarValues != null)
		{
		...
		if (attribute5.Length > 0)
		{
			...
			{
				string[] array2 = attribute5.Split(',');
			...
			{
				passiveEffect.Levels = new float[1] { StringParsers.ParseFloat(attribute5) };
			}
		}
		else if ((attribute5 = _element.GetAttribute("tier")).Length > 0)
		{
			if (attribute5.Contains(","))
			{
				string[] array3 = attribute5.Split(',');
		...
		else if ((attribute5 = _element.GetAttribute("duration")).Length > 0)
		{
			if (attribute5.Contains(","))
			{
				string[] array4 = attribute5.Split(',');
