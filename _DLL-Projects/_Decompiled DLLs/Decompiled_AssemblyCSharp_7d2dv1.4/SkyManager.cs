...
public class SkyManager : MonoBehaviour
{
	...
	[NonSerialized]
	[PublicizedFrom(EAccessModifier.Private)]
	public static float[] lightningTimes = new float[5] { 0f, 1f, 2f, 3f, 4f };
	...
	public static float BloodMoonVisiblePercent()
	{
		...
		return (num > 1 && num == num2 + 1 && num3 <= 6f) ? 1 : 0;
	}
	...
	{
		return dawnTime / 24f * (float)GamePrefs.GetInt(EnumGamePrefs.DayNightLength);
	}
	...
	{
		return duskTime / 24f * (float)GamePrefs.GetInt(EnumGamePrefs.DayNightLength);
	}
	...
	public static void TriggerLightning(Vector3 _position)
	{
		if (!triggerLightning)
		{
			lightningFlashes = random.RandomRange(3, 5);
			float num = 0f;
			for (int i = 1; i < lightningFlashes; i++)
			{
				float num2 = random.RandomRange(0.07f, 0.2f);
				num += num2;
				lightningTimes[i] = num;
			}
			lightningIndex = 0;
			lightningStartTime = Time.time;
			lightningPosition = _position;
			lightningDir = random.RandomFloat * 360f;
			lightningFrameCount = 2;
	...
	public void UpdateSunMoonAngles()
	{
		...
		if (num > 22)
		{
			duskTime = Mathf.Clamp(num, 0f, 23.999f);
		}
		dawnTime = Mathf.Clamp(duskTime - (float)num, 0f, 23.999f);
		if (Time.time - worldRotationTime >= 0.2f || bUpdateSunMoonNow)
		{
			...
			if (num2 >= dawnTime && num2 < duskTime)
			{
				worldRotationTarget = (num2 - dawnTime) / (duskTime - dawnTime);
			}
			else
			{
				float num3 = 24f - duskTime;
				float num4 = num3 + dawnTime;
				if (num2 < dawnTime)
				{
					worldRotationTarget = (num3 + num2) / num4;
				}
				else
				{
					worldRotationTarget = (num2 - duskTime) / num4;
				}
		...
		float num5 = worldRotationTarget - worldRotation;
		float num6 = worldRotationTarget;
		if (num5 < -0.5f)
		{
			num6 += 1f;
		}
		...
		worldRotation = Mathf.Lerp(worldRotation, num6, 0.05f);
		if (worldRotation < 0f)
		{
			worldRotation += 1f;
		}
		...
		if (sunIntensity >= 0.001f)
		{
			...
			Vector3 eulerAngles = Quaternion.LookRotation(Quaternion.AngleAxis(num7, sunAxis) * sunStartV).eulerAngles;
		...
		else if (moonLightColor.grayscale > 0f)
		{
			...
			Vector3 eulerAngles2 = Quaternion.LookRotation(Quaternion.AngleAxis(num7, sunAxis) * moonStartV).eulerAngles;
			moonLightT.localEulerAngles = eulerAngles2;
			float num8 = fogLightScale * moonBright;
			float t = GamePrefs.GetFloat(EnumGamePrefs.OptionsGfxBrightness) * 2f;
			num8 *= Utils.FastLerp(0.2f, 1f, t);
			moonLight.intensity = num8;
			moonLight.color = moonLightColor;
			moonLight.shadowStrength = 1f;
			moonLight.shadows = ((num8 > 0f) ? LightShadows.Soft : LightShadows.None);
		...
		if (!GameManager.IsDedicatedServer && (bool)mainCamera)
		{
			...
			if ((bool)moonSpriteT)
			{
				...
				float num9 = 6857.143f;
				if (IsBloodMoonVisible())
				{
					num9 *= 1.3f;
				}
				moonSpriteT.localScale = new Vector3(num9, num9, num9);
			}
	...
	public void Update()
	{
		...
		if (triggerLightning)
		{
			if (--lightningFrameCount >= 0)
			{
				...
				vector2.x = random.RandomRange(-80, 80);
				vector2.y = random.RandomRange(-50, 50);
				vector2.z = random.RandomRange(-80, 80);
				vector2.y += 300f;
				Transform obj = light.transform;
				obj.position = lightningPosition + vector2;
				obj.LookAt(GameManager.Instance.World.GetPrimaryPlayer().transform);
				light.color = Color.white;
				light.intensity = 1.1f;
				light.shadows = LightShadows.Hard;
				light.shadowStrength = 1f;
				light.enabled = true;
				cloudsSphereMtrl.SetFloat("_Lightning", 1f);
				float f2 = (lightningDir + (float)random.RandomRange(-40, 40)) * (MathF.PI / 180f);
				Vector3 vector3 = default(Vector3);
				vector3.x = Mathf.Cos(f2);
				vector3.y = 0f;
				vector3.z = Mathf.Sin(f2);
				cloudsSphereMtrl.SetVector("_LightningDir", vector3);
			}
			else
			{
				...
				else if (Time.time >= lightningStartTime + lightningTimes[lightningIndex])
				{
					lightningIndex++;
					lightningFrameCount = 2;
				}
	...
	public static void SetSunIntensity(float i)
	{
		...
		if (sunLight != null)
		{
			sunLight.intensity = sunIntensity * fogLightScale;
		}
	}
