...
public class TriggerEffectManager : IDisposable
{
	...
	public void Update()
	{
		...
		if (Enabled && !inUI)
		{
			TriggerEffectDualsensePC.PCTriggerUpdate(_stateChanged, _triggerSetLeft, _triggerSetRight, _currentEffectLeft.DualsenseEffect, _currentEffectRight.DualsenseEffect, _controllersConnected);
		...
		else if (_stateChanged && (!Enabled || inUI))
		{
			...
			TriggerEffectDualsensePC.DualsensePCSetEffectToOff(triggerSetLeft: true, triggerSetRight: true);
			_stateChanged = true;
		}
