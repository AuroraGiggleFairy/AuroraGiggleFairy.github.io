...
public class AutoTurretFireController : MonoBehaviour
{
	...
	public void Init(DynamicProperties _properties, AutoTurretController _atc)
	{
		...
		{
			muzzleFireParticle = _properties.Values["ParticlesMuzzleFire"];
		}
		else
		{
			muzzleFireParticle = "nozzleflashuzi";
		}
		if (_properties.Values.ContainsKey("ParticlesMuzzleSmoke"))
		{
			muzzleSmokeParticle = _properties.Values["ParticlesMuzzleSmoke"];
		}
		else
		{
			muzzleSmokeParticle = "nozzlesmokeuzi";
		}
	...
	public void Fire()
	{
		...
		for (int i = 0; i < rayCount; i++)
		{
			...
			forward *= -1f;
			forward = Quaternion.Euler(gameRandom.RandomRange(spread.x, spread.y), gameRandom.RandomRange(spread.x, spread.y), 0f) * forward;
			...
			Voxel.Raycast(GameManager.Instance.World, ray, maxDistance, -538750981, 8, 0f);
			GameManager.Instance.GetPersistentPlayerList().GetPlayerData(TileEntity.GetOwner());
			ItemActionAttack.Hit(Voxel.voxelRayHitInfo.Clone(), TileEntity.OwnerEntityID, EnumDamageTypes.Bashing, blockDamage, entityDamage, 1f, 1f, 0.5f, 0.05f, "bullet", damageMultiplier, buffActions, new ItemActionAttack.AttackHitInfo(), 3, 0, 0f, null, null, ItemActionAttack.EnumAttackMode.RealNoHarvesting, null, -2, TileEntity.blockValue.ToItemValue());
