...
public class XUiView
{
	...
	public virtual void UpdateData()
	{
		if (positionDirty)
		{
			uiTransform.localPosition = new Vector3(position.x, position.y, uiTransform.localPosition.z);
			positionDirty = false;
		}
	...
	{
		if (PlatformManager.NativePlatform.Input.CurrentInputStyle == PlayerInputManager.InputStyle.Keyboard && xuiHoverSound != null && soundPlayOnHover && enabled && GameManager.Instance.GameIsFocused)
		{
	...
	public virtual void SetDefaults(XUiController _parent)
	{
		...
		RepeatContent = false;
		RepeatCount = 1;
		Size = _parent?.ViewComponent?.Size ?? new Vector2i(0, 0);
	...
	public void parseNavigationTargets()
	{
		...
		XUiView findView(string _name)
		{
			...
			return findHierarchyClosestView(_name) ?? throw new ArgumentException("Invalid navigation target, view component with name '" + _name + "' not found.\nOn: " + controller.GetXuiHierarchy());
		}
	}
	...
	public virtual bool ParseAttribute(string _attribute, string _value, XUiController _parent)
	{
		...
		case "width":
		{
			...
			if (_value.Contains("%"))
			{
				...
				{
					result2 = (int)((float)result2 / 100f) * _parent.ViewComponent.Size.x;
				}
			}
			else
			{
				int.TryParse(_value, out result2);
			}
			Size = new Vector2i(result2, y);
			break;
		}
		case "height":
		{
			...
			if (_value.Contains("%"))
			{
				...
				{
					result3 = (int)((float)result3 / 100f) * _parent.ViewComponent.Size.y;
				}
			}
			else
			{
		...
		case "depth":
		{
			...
			Depth = result + (_parent.ViewComponent?.Depth ?? 0);
			break;
		}
		case "value":
			Value = _value;
