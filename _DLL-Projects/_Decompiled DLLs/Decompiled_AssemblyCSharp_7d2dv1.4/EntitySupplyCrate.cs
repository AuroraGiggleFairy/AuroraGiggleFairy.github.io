...
public class EntitySupplyCrate : EntityAlive
{
	...
	{
		if (GameStats.GetBool(EnumGameStats.AirDropMarker) && EntityClass.list[entityClass].NavObject != "")
		{
	...
	{
		base.OnEntityUnload();
		if (unloadReason == EnumRemoveEntityReason.Killed)
		{
			AIAirDrop.RemoveSupplyCrate(entityId);
	...
	public override void MoveEntityHeaded(Vector3 _direction, bool _isDirAbsolute)
	{
		...
		{
			if (((EModelSupplyCrate)emodel).parachute.gameObject.activeSelf && !IsInWater())
			{
				motion.y += ScalePhysicsAddConstant(world.Gravity * 0.95f);
			}
			if (isCollidedVertically && ChunkObserver != null)
			{
				world.GetGameManager().RemoveChunkObserver(ChunkObserver);
				ChunkObserver = null;
			}
		}
	}
	...
	public override void OnUpdateEntity()
	{
		...
		if (onGround && !wasOnGround)
		{
			...
			{
				GameManager.Instance.World.aiDirector.GetComponent<AIDirectorAirDropComponent>().SetSupplyCratePosition(entityId, World.worldToBlockPos(position));
			}
		}
