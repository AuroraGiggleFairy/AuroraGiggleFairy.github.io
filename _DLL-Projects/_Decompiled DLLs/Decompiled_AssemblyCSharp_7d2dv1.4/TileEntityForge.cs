...
public class TileEntityForge : TileEntity
{
	...
	public void updateLightState(World world)
	{
		...
		if (fuelInStorageInTicks + fuelInForgeInTicks == 0 && block.meta != 0)
		{
			block.meta = 0;
			world.SetBlockRPC(ToWorldPos(), block);
		}
		else if (fuelInStorageInTicks + fuelInForgeInTicks != 0 && block.meta == 0)
		{
			block.meta = 1;
			world.SetBlockRPC(ToWorldPos(), block);
		}
