...
public class EntityVulture : EntityFlying
{
	...
	public void Init()
	{
		...
		isRadiated = EntityClass.list[entityClass].Properties.Values.ContainsKey("Radiated");
		battleFatigueSeconds = 30f + rand.RandomFloat * 60f;
	}
		...
		else
		{
			...
			case State.Stun:
				if (flag)
				{
					...
					ModelTransform.GetComponentInChildren<Animator>().enabled = false;
					return;
				}
				ModelTransform.GetComponentInChildren<Animator>().enabled = true;
				SetState(State.WanderStart);
				break;
			case State.WanderStart:
			{
				...
				if (!isWithinHomeDistanceCurrentPosition())
				{
					SetState(State.Home);
					homeSeekDelay = 0;
					waypoint = getHomePosition().position.ToVector3();
					break;
				}
				...
				{
					float num4 = 10f + rand.RandomFloat * 20f;
					if (IsSleeper)
					{
						num4 *= 0.4f;
					}
			...
			{
				moveUpdateDelay = 5 + rand.RandomRange(5);
			...
			if ((bool)currentTarget && (bool)currentTarget.AttachedToEntity)
			{
			...
			if ((bool)currentTarget)
			{
				...
				headPosition += currentTarget.GetVelocityPerSecond() * 0.2f;
				...
				else if (state != State.AttackReposition)
				{
					...
					{
						if (sqrMagnitude2 < 0.64000005f && position.y >= currentTarget.position.y && position.y < headPosition.y + 0.1f)
						{
					...
					if (itemActionDataVomit != null && attack2Delay <= 0 && sqrMagnitude2 >= 9f)
					{
						...
						if (sqrMagnitude2 < range * range && Utils.FastAbs(Mathf.DeltaAngle(num5, rotation.y)) < 20f && Utils.FastAbs(Vector3.SignedAngle(vector6, base.transform.forward, Vector3.right)) < 25f)
						{
							flag3 = true;
						}
					}
	...
	public void StartAttackReposition()
	{
		...
		{
			ClearTarget();
			battleDuration = 60f + rand.RandomFloat * 120f;
	...
	{
		if (Attack(_bAttackReleased: false))
		{
			Attack(_bAttackReleased: true);
			attackDelay = 18;
			isCircling = false;
			if ((bool)currentTarget.AttachedToEntity)
			{
