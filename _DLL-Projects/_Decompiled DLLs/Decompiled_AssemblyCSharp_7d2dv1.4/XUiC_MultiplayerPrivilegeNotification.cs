...
public class XUiC_MultiplayerPrivilegeNotification : XUiController
{
	...
	public bool ResolvePrivilegesWithDialog(EUserPerms _permissionsWithPrompt, Action<bool> _resolutionComplete, EUserPerms _permissionsSilent = (EUserPerms)0, float _delayDisplay = -1f, bool _usingProgressWindow = false, Action _cancellationCleanupAction = null)
	{
		...
		ThreadManager.StartCoroutine(ResolvePrivilegesCoroutine(_permissionsWithPrompt, _permissionsSilent, _resolutionComplete, _usingProgressWindow));
		return true;
	}
	...
	public IEnumerator ResolvePrivilegesCoroutine(EUserPerms _permissionsWithPrompt, EUserPerms _permissionsSilent, Action<bool> _resolutionComplete, bool _usingProgressWindow)
	{
		...
		if (_permissionsWithPrompt != 0)
		{
			yield return PermissionsManager.ResolvePermissions(_permissionsWithPrompt, _canPrompt: true, cancellationToken);
		}
		if (_permissionsSilent != 0)
		{
			yield return PermissionsManager.ResolvePermissions(_permissionsSilent, _canPrompt: false, cancellationToken);
		}
		resolving = false;
		...
		bool flag = eulaAccepted && (PermissionsManager.GetPermissions() & _permissionsWithPrompt) == _permissionsWithPrompt;
		if (_usingProgressWindow)
		{
			if (flag)
			{
			...
			{
				string text = ((!eulaAccepted) ? Localization.Get("uiPermissionsEula") : (PermissionsManager.GetPermissionDenyReason(_permissionsWithPrompt) ?? Localization.Get("lblInvalidPrivileges")));
				text = text + "\n\n[FFFFFF]" + Utils.GetCancellationMessage();
				XUiC_ProgressWindow.Open(LocalPlayerUI.primaryUI, text, [PublicizedFrom(EAccessModifier.Private)] () =>
				{
					CloseWindow(_success: false);
		...
		{
			lblResolvingPrivileges.IsVisible = false;
			string text2 = (eulaAccepted ? PermissionsManager.GetPermissionDenyReason(_permissionsWithPrompt) : Localization.Get("uiPermissionsEula"));
		...
		bool obj = eulaAccepted && (PermissionsManager.GetPermissions() & allPermissions) == allPermissions;
		_resolutionComplete?.Invoke(obj);
	}
