	...
	public static void ShareQuestWithParty(Quest _selectedQuest, EntityPlayerLocal _localPlayer, bool _showTooltips)
	{
		...
		else
		{
			...
			for (int i = 0; i < _localPlayer.Party.MemberList.Count; i++)
			{
				...
				GameManager.Instance.QuestShareServer(_selectedQuest.QuestCode, _selectedQuest.ID, _selectedQuest.GetPOIName(), _selectedQuest.GetLocation(), _selectedQuest.GetLocationSize(), pos, _localPlayer.entityId, entityPlayer.entityId, _selectedQuest.QuestGiverID);
				num++;
				if (AutoShare)
				{
