...
public class NGuiWdwDebugPanels : MonoBehaviour
{
	...
	public int showDebugPanel_Player(int x, int y)
	{
		...
		for (EGuiState eGuiState = EGuiState.CalcSize; eGuiState < EGuiState.Count; eGuiState++)
		{
			...
			if (eGuiState == EGuiState.Draw)
			{
				LabelWithOutline(x, y, $"DM Threat Lvl: {playerEntity.ThreatLevel.Category.ToStringCached()} : {playerEntity.ThreatLevel.Numeric:0.##}");
			}
			y += 16;
			if (eGuiState == EGuiState.Draw)
			{
				LabelWithOutline(x, y, $"DM Zeds: {ThreatLevelUtility.Zombies}, Targeting: {ThreatLevelUtility.Targeting}");
			}
			y += 16;
			if (eGuiState == EGuiState.Draw)
			{
				LabelWithOutline(x, y, $"Outside Temperature (F): {Mathf.FloorToInt(entityPlayer.Stats.GetOutsideTemperature())}");
			}
			y += 16;
			if (eGuiState == EGuiState.Draw)
			{
				LabelWithOutline(x, y, string.Format("Feels Like Temperature (F): {0} ({1})", Mathf.FloorToInt(entityPlayer.Buffs.GetCustomVar("_coretemp")) + 70, Mathf.FloorToInt(entityPlayer.Buffs.GetCustomVar("_coretemp"))));
			}
			y += 16;
			if (eGuiState == EGuiState.Draw)
			{
				LabelWithOutline(x, y, string.Format("Degrees Absorbed (F): {0}", Mathf.FloorToInt(entityPlayer.Buffs.GetCustomVar("_degreesabsorbed"))));
			}
			y += 16;
			num = y;
		}
	...
	public int showDebugPanel_PlayerEffectInfo(int x, int y)
	{
		...
		y += 21;
		HeaderLabel(EGuiState.Draw, x, y, entityAlive.EntityName + " CVars (" + entityAlive.Buffs.CVars.Count + ")");
		...
		{
			if (entityAlive.Buffs.CVars[key] == 0f || !key.ContainsCaseInsensitive(filterCVar))
			{
				continue;
			}
			LabelWithOutline(x, y, $"{key} : {entityAlive.Buffs.CVars[key]}");
			if (num5 % 25 == 0)
			{
				x += 220;
				if (num == -1)
	...
	public int showDebugPanel_FocusedBlock(int x, int y, bool forceFocusedBlock = false)
	{
		...
		if (num == 0)
		{
			num = GameUtils.FindPaintIdForBlockFace(block, rotatedBlockFace, out _name);
		}
		else
		{
			_name = ((num < 0 || num >= BlockTextureData.list.Length) ? string.Empty : (BlockTextureData.list[num]?.Name ?? "N/A"));
		}
		bool flag = false;
		if (num >= 0 && num < BlockTextureData.list.Length && block2.MeshIndex == 0)
		{
			int num2 = BlockTextureData.list[num]?.TextureID ?? 0;
			...
			{
				switch (block2.GetUVMode((int)rotatedBlockFace))
				{
		...
		{
			y = num4;
			PanelBoxWithHeader(eGuiState, x, ref y, boxWidth, num3 - num4 + 5, "Focused Block");
			if (eGuiState == EGuiState.Draw)
			{
				int y3 = y;
			...
			if (eGuiState == EGuiState.Draw)
			{
				LabelWithOutline(x, y, $"Paint Id: {num} ({_name})");
			}
			y += 16;
			if (eGuiState == EGuiState.Draw)
			{
				...
				{
					LabelWithOutline(x, y, "Prefab: " + blockShapeModelEntity.modelName);
				}
				else
				{
					LabelWithOutline(x, y, string.Format("Shape: {0} V={1} T={2} UV: {3}", shape.GetName(), shape.GetVertexCount(), shape.GetTriangleCount(), flag ? "G" : "L"));
				}
			}
			y += 16;
			if (eGuiState == EGuiState.Draw)
