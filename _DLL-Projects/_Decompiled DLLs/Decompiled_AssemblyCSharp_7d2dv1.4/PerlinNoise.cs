...
public class PerlinNoise
{
	...
	{
		_random = GameRandomManager.Instance.CreateGameRandom(seed);
		InitGradients();
	}

	public double Noise01(double x, double z)
	{
		return (Noise(x, 0.0, z) + 1.0) * 0.5;
	}
	...
	public void InitGradients()
	{
		for (int i = 0; i < 256; i++)
		{
			double num = 1.0 - 2.0 * _random.RandomDouble;
			double num2 = Math.Sqrt(1.0 - num * num);
			double num3 = Math.PI * 2.0 * _random.RandomDouble;
