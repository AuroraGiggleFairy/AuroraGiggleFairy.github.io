...
public abstract class ItemAction : XMLData.Item.ItemActionData
{
	...
	public virtual void CancelReload(ItemActionData _actionData)
	{
	...
	public virtual void ReadFrom(DynamicProperties _props)
	{
		...
		{
			Delay = StringParsers.ParseFloat(_props.Values["Delay"]);
		}
		...
		{
			Sound_in_head = StringParsers.ParseBool(_props.Values["Sound_in_head"]);
		}
		...
		{
			ActionExp = int.Parse(_props.Values["ActionExp"]);
		}
		else
		{
			ActionExp = 2;
		...
		{
			ActionExpBonusMultiplier = StringParsers.ParseFloat(_props.Values["ActionExpBonusMultiplier"]);
		}
		else
		{
		...
		{
			UseAnimation = StringParsers.ParseBool(_props.Values["UseAnimation"]);
		}
