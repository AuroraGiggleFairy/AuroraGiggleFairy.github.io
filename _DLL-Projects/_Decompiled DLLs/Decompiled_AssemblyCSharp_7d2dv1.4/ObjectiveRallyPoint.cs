...
public class ObjectiveRallyPoint : BaseObjective
{
	...
	public override void RemoveObjectives()
	{
		...
		if (RallyStartType == RallyStartTypes.Find)
		{
			if (world.GetChunkFromWorldPos(rallyPos) is Chunk chunk)
			{
				...
				{
					blockEntity.transform.GetChild(0).gameObject.SetActive(value: false);
				}
			}
		}
		...
		{
			SetRallyPointVisibility(visible: false);
		}
	}
	...
	public override void RemoveHooks()
	{
		...
		if (rallyPos != Vector3i.zero)
		{
			if (RallyStartType == RallyStartTypes.Find)
			{
				...
				SetRallyPointVisibility(rallyPointVisibility);
			}
			else if (base.OwnerQuest.SharedOwnerID == -1)
			{
	...
	public bool SetRallyPointVisibility(bool visible)
	{
		...
		if (world.GetChunkFromWorldPos(rallyPos) is Chunk chunk)
		{
			...
			world.SetBlockRPC(chunk.ClrIdx, rallyPos, Block.GetBlockValue("questRallyMarker"));
		}
		return false;
	}
	...
	public void HandleParty()
	{
		...
		if (ownerPlayer.Party != null)
		{
			if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
			{
				SingletonMonoBehaviour<ConnectionManager>.Instance.SendPackage(NetPackageManager.GetPackage<NetPackagePartyQuestChange>().Setup(ownerPlayer.entityId, base.OwnerQuest.GetObjectiveIndex(this), _isComplete: true, base.OwnerQuest.QuestCode));
			}
			else
			{
	...
	public Vector3 HandleRallyPoint()
	{
			...
			else
			{
				...
				if ((rallyPos = GetRallyPosition(world, prefabPosition, new Vector3i(pos))) != Vector3i.zero)
				{
					...
					if (world.GetChunkFromWorldPos(rallyPos) is Chunk chunk)
					{
						...
						if (blockEntity == null || !(blockEntity.transform != null))
						{
							world.SetBlockRPC(chunk.ClrIdx, rallyPos, block);
							return Vector3.zero;
						}
		...
		else
		{
			...
			else
			{
				...
				{
					world2.SetBlockRPC(chunk2.ClrIdx, rallyPos, block2);
					return Vector3.zero;
				}
				blockEntity2.transform.GetChild(0).gameObject.SetActive(value: true);
				positionSet = PositionSetTypes.RallyMarkerPosition;
			}
		}
	...
	public override void UpdateState_Update()
	{
		...
		if (positionSet == PositionSetTypes.RallyMarkerPosition)
		{
			SetRallyPointVisibility(base.OwnerQuest.OwnerJournal.ActiveQuest == null);
		}
	...
	{
		SetRallyPointVisibility(visible: true);
	}
}
