...
public abstract class EModelBase : MonoBehaviour
{
	...
	public bool IsRagdollActive => ragdollState != ERagdollState.Off;
	...
	public void LookAtUpdate()
	{
		...
		else if (!lookAtIsPos)
		{
			...
			if ((bool)attackTargetLocal && entityAlive.CanSee(attackTargetLocal))
			{
		...
		{
			lookAtFullChangeTime = 1.3f + 2.7f * entityAlive.rand.RandomFloat;
			lookAtFullBlendPer = 0.2f + 1.5f * entityAlive.rand.RandomFloat;
	...
	public virtual void LateUpdate()
	{
		...
		if (lookAtEnabled && !IsRagdollActive)
		{
			LookAtUpdate();
		}
	}
	...
	public void DoRagdoll(float stunTime, EnumBodyPartHit bodyPart, Vector3 forceVec, Vector3 forceWorldPos, bool isRemote)
	{
		...
		else
		{
			...
			{
				Vector3 vector = (entity.isEntityRemote ? (forceVec * 0.95f) : forceVec);
	...
	public void ClothSimInit()
	{
		...
		{
			isClothSimOn = true;
			ClothSimOn(!entity.AttachedToEntity);
		}
	}

	public void ClothSimOn(bool _on)
	{
		if (clothSim != null && isClothSimOn != _on)
		{
			isClothSimOn = _on;
			for (int num = clothSim.Length - 1; num >= 0; num--)
			{
				clothSim[num].gameObject.SetActive(_on);
			}
		}
	}
	...
	public virtual void SwitchModelAndView(bool _bFPV, bool _bMale)
	{
		...
		if (avatarController != null)
		{
			...
			avatarController.GetActiveModelRoot();
		}
	...
	public virtual void createModel(World _world, EntityClass _ec)
	{
		...
		if (_ec.AltMatNames != null)
		{
			...
			if (num2 >= 0)
			{
				Material material = (AltMaterial = DataLoader.LoadAsset<Material>(_ec.AltMatNames[num2]));
		...
		Renderer[] componentsInChildren4 = GetComponentsInChildren<Renderer>(includeInactive: true);
		foreach (Renderer renderer in componentsInChildren4)
		{
	...
	{
		if (entity is EntityPlayerLocal && _ec.Properties.Values.ContainsKey(EntityClass.PropLocalAvatarController))
		{
			avatarController = base.transform.gameObject.GetComponent(Type.GetType(_ec.Properties.Values[EntityClass.PropLocalAvatarController])) as AvatarController;
			if (avatarController == null)
			{
				avatarController = base.transform.gameObject.AddComponent(Type.GetType(_ec.Properties.Values[EntityClass.PropLocalAvatarController])) as AvatarController;
			}
		}
		else if (_ec.Properties.Values.ContainsKey(EntityClass.PropAvatarController))
		{
			avatarController = base.transform.gameObject.GetComponent(Type.GetType(_ec.Properties.Values[EntityClass.PropAvatarController])) as AvatarController;
			if (avatarController == null)
			{
				avatarController = base.transform.gameObject.AddComponent(Type.GetType(_ec.Properties.Values[EntityClass.PropAvatarController])) as AvatarController;
			}
		}
	}
