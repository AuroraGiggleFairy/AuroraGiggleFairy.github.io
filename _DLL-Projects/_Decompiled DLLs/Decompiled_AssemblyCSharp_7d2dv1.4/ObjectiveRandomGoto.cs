...
public class ObjectiveRandomGoto : BaseObjective
{
	...
	public virtual Vector3 GetPosition()
	{
		...
		float _result = 50f;
		if (Value != null && Value != "" && !StringParsers.TryParseFloat(Value, out _result) && Value.Contains("-"))
		{
		...
		{
			Vector3i vector3i = CalculateRandomPoint(ownerPlayer.entityId, _result, base.OwnerQuest.ID, canBeWithinPOI: false, biomeFilterType, biomeFilter);
		...
		{
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageQuestTreasurePoint>().Setup(ownerPlayer.entityId, _result, 1, base.OwnerQuest.QuestCode));
			base.CurrentValue = 1;
		}
		return Vector3.zero;
	}
