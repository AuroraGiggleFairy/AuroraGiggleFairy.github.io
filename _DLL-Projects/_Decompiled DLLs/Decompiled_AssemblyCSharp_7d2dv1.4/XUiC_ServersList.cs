...
public class XUiC_ServersList : XUiC_List<XUiC_ServersList.ListEntry>
{
	...
	public class ListEntry : XUiListEntry
	{
		...
		{
			if (!(_otherEntry is ListEntry listEntry))
			{
				return 1;
			}
			return gameServerInfo.LastPlayedLinux.CompareTo(listEntry.gameServerInfo.LastPlayedLinux);
		}
	...
	public override void Init()
	{
		...
		for (int i = 0; i < 5; i++)
		{
			...
			{
				sortButtons[i].ViewComponent.Value = i.ToString();
		...
		for (int j = 0; j < listEntryControllers.Length; j++)
		{
			...
			obj2.Value = j.ToString();
			obj2.Controller.OnPress += [PublicizedFrom(EAccessModifier.Private)] (XUiController _sender, int _args) =>
			{
				int num = StringParsers.ParseSInt32(_sender.ViewComponent.Value);
				XUiC_ListEntry<ListEntry> obj3 = listEntryControllers[num];
	...
	public override void OnOpen()
	{
		...
		ThreadManager.StartThread("ServerBrowserListUpdater", null, currentListUpdateThread, null, System.Threading.ThreadPriority.BelowNormal);
	}
