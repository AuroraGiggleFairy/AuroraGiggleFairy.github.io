...
public class AIAirDrop
{
			...
			while (num < flightPaths.Count)
			{
				...
				if (flightPath2.Delay <= 0f)
				{
					...
					while (num2 < flightPath2.Crates.Count)
					{
						...
						{
							EntitySupplyCrate entitySupplyCrate = controller.SpawnSupplyCrate(supplyCrateSpawn2.SpawnPos);
							...
							{
								world.GetGameManager().RemoveChunkObserver(supplyCrateSpawn2.ChunkRef);
	...
	public static void RemoveSupplyCrate(int id)
	{
		if (controller != null)
		{
			controller.RemoveSupplyCrate(id);
		}
	}
}
