...
public class PlayAndCleanup
{
	...
	public PlayAndCleanup(GameObject _go, AudioSource source, float _occlusion = 0f, float delay = 0f, bool isLooping = false, bool hasLoopingAnalog = false)
	{
		...
		float num2 = Mathf.Abs(Manager.currentListenerPosition.y - go.transform.position.y);
		num2 = Mathf.Clamp01(num2 / 30f);
		src.volume *= 1f - num2;
		src.volume *= num;
		if (num < 0.95f)
		{
			go.AddComponent<AudioLowPassFilter>().cutoffFrequency = Mathf.Lerp(10f, 5000f, Mathf.Pow(num, 2f));
		}
		if (delay > 0f)
		{
