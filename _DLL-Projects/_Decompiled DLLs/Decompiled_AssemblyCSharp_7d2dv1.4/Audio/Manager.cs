...
public class Manager : IDisposable
{
	...
	public static void Play(Vector3 position, string soundGroupName, int entityId = -1)
	{
		...
		if (!CheckGlobalPlayRequirements(soundGroupName) || !audioData.TryGetValue(soundGroupName, out var value) || !value.Update())
		{
			return;
		}
	...
	public static Handle Play(Entity entity, string soundGroupName, float volumeScale = 1f, bool wantHandle = false)
	{
		...
		SignalAI(entity, entity.position, soundGroupName, volumeScale);
		...
		bool flag = true;
		if (entity != null && playingOnEntity.TryGetValue(entity.entityId, out var value2))
		{
			List<int> list = null;
			if (value.channel == XmlData.Channel.Environment)
			{
				if (value2.environment != null && value2.environment.dict.TryGetValue(soundGroupName, out var value3))
				{
					...
					{
						if (value3[i] == null || !value3[i].isPlaying)
						{
							if (list == null)
							{
								list = new List<int>();
					...
					if (value.maxVoicesPerEntity < value3.Count)
					{
						flag = false;
					}
					if (list != null)
					{
						...
						{
							if (value3[list[num]] != null)
							{
								RemovePlayingAudioSource(value3[list[num]]);
							}
							value3.RemoveAt(list[num]);
						}
					}
					if (value3.Count == 0)
					{
						...
						{
							playingOnEntity.Remove(entity.entityId);
						}
					}
				}
			}
			else if (value2.mouth != null)
			{
				...
				{
					if (value2.mouth[j] == null || !value2.mouth[j].isPlaying)
					{
						if (list == null)
						{
							list = new List<int>();
				...
				if (value2.mouth.Count > 0)
				{
					...
					{
						for (int k = 0; k < value2.mouth.Count; k++)
						{
							StopSource(value2.mouth[k]);
							RemovePlayingAudioSource(value2.mouth[k]);
						}
						value2.mouth.Clear();
					}
					else
			...
			if (playingOnEntity.ContainsKey(entity.entityId))
			{
				playingOnEntity[entity.entityId] = value2;
			}
		}
		if (!flag)
		{
		...
		if (value.distantFadeStart >= 0f)
		{
			float magnitude = (entity.position - Origin.position - currentListenerPosition).magnitude;
		...
		if (flag3)
		{
			audioSource = LoadAudio(randomClip.forceLoop, (entity.position - LocalPlayer().position).magnitude, randomClip.clipName, randomClip.audioSourceName);
			if (audioSource == null)
			{
				return null;
			}
		...
		if (flag2)
		{
			audioSource2 = LoadAudio(randomClip.forceLoop, (entity.position - LocalPlayer().position).magnitude, randomClip.clipName_distant, (randomClip.audioSourceName_distant.Length > 0) ? randomClip.audioSourceName_distant : randomClip.audioSourceName);
			if (audioSource2 == null)
			{
				return null;
			}
		...
		if ((bool)entityAlive)
		{
			...
			if (audioSource != null)
			{
				float num3 = audioSource.volume * volumeScale;
				...
				SetPitch(audioSource, value, shift);
			...
			if (audioSource2 != null)
			{
				float num4 = audioSource2.volume * volumeScale;
				if (entityAlive.IsCrouching)
				{
				...
				audioSource2.volume = num4;
				SetPitch(audioSource2, value, shift);
			}
		}
		if (((bool)audioSource && audioSource.loop) || ((bool)audioSource2 && audioSource2.loop))
		{
			if (loopingOnEntity.TryGetValue(entity.entityId, out var value4))
			{
			...
			if (value4 == null)
			{
				...
				loopingOnEntity.Add(entity.entityId, value4);
			}
		...
		else
		{
			if (entity != null)
			{
				if (!playingOnEntity.TryGetValue(entity.entityId, out var value6))
				{
					value6 = default(Channels);
					playingOnEntity.Add(entity.entityId, value6);
				}
				if (value.channel == XmlData.Channel.Environment)
				{
				...
				playingOnEntity[entity.entityId] = value6;
			}
			if (audioSource != null)
			{
				new PlayAndCleanup(gameObject, audioSource, (entity == null) ? 0f : entity.CalculateAudioOcclusion());
	...
	public static void StopAllSequencesOnEntity(Entity entity)
	{
		...
		foreach (KeyValuePair<string, SequenceGOs> item in value)
		{
			...
			if (!value4.TryGetValue(item.Key, out var value5))
			{
	...
	public static void StopSequence(Entity entity, string soundGroupName)
	{
		...
		if (value2.nearEnd != null || value2.farEnd != null)
		{
			...
			if (audioSource != null || audioSource2 != null)
			{
				...
				if (!value4.TryGetValue(soundGroupName, out var value5))
				{
					List<GameObject> list = new List<GameObject>();
					if (value2.nearStart != null)
					{
	...
	public static string GetFormattedSubtitle(string subtitleId)
	{
		...
		if (subtitleData != null)
		{
			...
			{
				string text = Localization.Get(subtitleData.speakerLocId);
				...
				result = $"{text}    {Localization.Get(subtitleData.contentLocId)}";
			}
			else
			{
