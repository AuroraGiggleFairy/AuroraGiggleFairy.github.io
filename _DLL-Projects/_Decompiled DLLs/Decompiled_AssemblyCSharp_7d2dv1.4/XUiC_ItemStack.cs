...
public class XUiC_ItemStack : XUiC_SelectableEntry
{
	...
	public void updateLockTypeIcon()
	{
		...
		if (itemClass is ItemClassBlock itemClassBlock && ((!itemClassBlock.GetBlock().SelectAlternates && itemStack.itemValue.Texture > 0) || itemStack.itemValue.TextureAllSides > 0))
		{
		...
		if (itemClass is ItemClassModifier itemClassModifier)
		{
			...
			if (base.xui.AssembleItem.CurrentItem != null)
			{
				...
				{
					flashLockTypeIcon = true;
				}
				else
				{
					setLockTypeIconColor(Color.grey);
					flashLockTypeIcon = false;
				}
			}
			else
			{
	...
	public override bool GetBindingValue(ref string _value, string _bindingName)
	{
		...
		case "itemtypeicon":
			...
			if (!itemStack.IsEmpty())
			{
				...
				if (itemClass2 != null)
				{
					if (itemClass2.IsBlock() && itemStack.itemValue.TextureAllSides == 0)
					{
