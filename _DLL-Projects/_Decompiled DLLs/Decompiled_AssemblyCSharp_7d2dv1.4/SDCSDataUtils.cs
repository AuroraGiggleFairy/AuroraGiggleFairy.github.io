...
public static class SDCSDataUtils
{
	...
	public static void ParseRaceVariantFromResources(bool isMale)
	{
		...
		foreach (FileInfo fileInfo in files)
		{
			...
			if (!(value == ""))
			{
				...
				GenderKey key = new GenderKey(text3, isMale);
				if (!VariantData.ContainsKey(key))
				{
					VariantData.Add(key, new List<int>());
				}
				VariantData[key].Add(StringParsers.ParseSInt32(value));
			}
		}
	}
	...
	public static List<string> GetHairNamesFromResources(bool isMale, HairTypes hairType)
	{
		List<string> list = new List<string>();
		string text = (isMale ? "Male" : "Female");
		DirectoryInfo[] directories = new DirectoryInfo(Application.dataPath + "/Resources/Entities/Player/" + text + "/HairMorphMatrix/" + hairType.ToString() + "/").GetDirectories();
	...
	{
		List<string> list = new List<string>();
		FileInfo[] files = new DirectoryInfo(Application.dataPath + "/Resources/Entities/Player/Eyes/").GetFiles();
		foreach (FileInfo fileInfo in files)
		{
	...
	{
		List<string> list = new List<string>();
		FileInfo[] files = new DirectoryInfo(Application.dataPath + "/Resources/" + baseHairColorLoc + "/").GetFiles();
		foreach (FileInfo fileInfo in files)
		{
			if (!fileInfo.Name.EndsWith(".meta"))
			{
