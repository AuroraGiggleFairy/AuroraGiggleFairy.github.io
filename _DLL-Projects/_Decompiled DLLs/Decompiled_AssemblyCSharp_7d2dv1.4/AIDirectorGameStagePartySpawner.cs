...
public class AIDirectorGameStagePartySpawner
{
	...
	{
		gsScaling = Utils.FastLerp(1f, 2.8f, (_scaling - 1f) / 3f);
	}
	...
	public void SetPartyLevel(int _partyLevel)
	{
		...
		Log.Out("Party of {0}, game stage {1}, scaling {2}, enemy max {3}, bonus every {4}", members.Count, partyLevel, gsScaling, stageSpawnMax, bonusLootEvery);
	...
	public void SetupGroup()
	{
		...
		if (spawnGroup != null)
		{
			interval = spawnGroup.interval;
			nextStageTime = ((spawnGroup.duration != 0) ? (world.worldTime + spawnGroup.duration * 1000) : 0);
