...
public static class Utils
{
	...
	public static (int, int, bool) FindNextBbCode(string _input, int _startIndex, bool _allowUrls = false)
	{
		...
		while (num < length)
		{
			...
			{
				string text2 = _input.Substring(num, 6).ToLower();
				if (text2 == "[/sub]" || text2 == "[/sup]")
				{
					return (num, 6, false);
				}
			}
			if (_allowUrls && num + 5 < length && _input.Substring(num, 5).ToLower() == "[url=")
			{
			...
			if (!_allowUrls && num + 6 <= length && _input.Substring(num, 6).ToLower() == "[/url]")
			{
				return (num, 6, false);
			}
