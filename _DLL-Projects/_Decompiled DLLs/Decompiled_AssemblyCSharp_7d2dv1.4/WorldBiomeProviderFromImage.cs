...
public class WorldBiomeProviderFromImage : IBiomeProvider
{
	...
	public IEnumerator InitData()
	{
		...
		int num = 0;
		foreach (KeyValuePair<uint, BiomeDefinition> item in m_Biomes.GetBiomeMap())
		{
			num = Utils.FastMax(item.Value.subbiomes.Count, num);
		}
		noises = new PerlinNoise[num];
		for (int i = 0; i < noises.Length; i++)
		{
			noises[i] = new PerlinNoise(i);
		}
		yield return null;
		...
		else if (SdFile.Exists(text + ".png"))
		{
		...
		if (texture2D.width <= 512 && texture2D.height <= 512)
		{
			...
			if (texture2D.format == TextureFormat.RGBA32)
			{
				...
				{
					radiationMapSmall[j] = ProcessColor(nativeArray[j]);
				}
			}
			else
			{
				...
				{
					radiationMapSmall[k] = ProcessColor(nativeArray2[k]);
				}
			}
		}
		else
		{
			radiationTilesX = radiationMapSize / 512;
			radiationTilesZ = radiationMapSize / 512;
			radiationTileAreaConfig = new TileAreaConfig
			{
				tileStart = new Vector2i(-radiationTilesX / 2, -radiationTilesZ / 2),
				tileEnd = new Vector2i(Utils.FastMax(0, radiationTilesX / 2 - 1), Utils.FastMax(0, radiationTilesZ / 2 - 1)),
			...
			if (PlatformOptimizations.FileBackedRadiationTiles)
			{
				...
				radiationMap = LoadRadiationMapToFile(texture2D, radiationTileAreaConfig);
			}
			else
			{
				radiationMap = LoadRadiationMap(texture2D, radiationTileAreaConfig);
			}
		}
	...
	public int GetSubBiomeIdxAt(BiomeDefinition bd, int _x, int _y, int _z)
	{
		for (int i = 0; i < bd.subbiomes.Count; i++)
		{
			...
			if ((float)noises[i].Noise01((float)_x * biomeDefinition.freq, (float)_z * biomeDefinition.freq) < biomeDefinition.prob)
			{
				return i;
			}
		}
	...
	public float GetRadiationAt(int x, int z)
	{
		...
		int worldX = x / radiationMapScale;
		int worldZ = z / radiationMapScale;
		worldCoordsToTileCoords(worldX, worldZ, radiationTileAreaConfig, out var tileX, out var tileZ, out var posX, out var posZ);
		if (tileX < radiationTileAreaConfig.tileStart.x || tileZ < radiationTileAreaConfig.tileStart.y || tileX > radiationTileAreaConfig.tileEnd.x || tileZ > radiationTileAreaConfig.tileEnd.y || posX < 0 || posZ < 0 || posX > 512 || posZ > 512)
		{
			return 0f;
		}
		return (int)radiationMap[tileX, tileZ][posX, posZ];
	}
