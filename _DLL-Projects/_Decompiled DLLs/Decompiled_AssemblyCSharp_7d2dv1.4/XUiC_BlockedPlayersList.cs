...
public class XUiC_BlockedPlayersList : XUiController
{
	...
	public bool IsVisible
	{
		get
		{
			return mainRect.IsVisible;
		}
		set
		{
			mainRect.IsVisible = value;
		}
	}
	...
	{
		base.Init();
		mainRect = (XUiV_Rect)GetChildById("blockedPlayersRect").ViewComponent;
	...
	public void DisplayMessage(string _header, string _message)
	{
		...
		XUiC_MessageBoxWindowGroup.ShowMessageBox(base.xui, _header, _message, XUiC_MessageBoxWindowGroup.MessageBoxTypes.Ok, DisableNoClick, DisableNoClick, _openMainMenuOnClose: false, _bCloseAllOpenWindows: false);
		[PublicizedFrom(EAccessModifier.Private)]
		void DisableNoClick()
		{
			noClick.Enabled = false;
