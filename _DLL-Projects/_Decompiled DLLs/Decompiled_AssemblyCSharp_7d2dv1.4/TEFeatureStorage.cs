...
public class TEFeatureStorage : TEFeatureAbs, ITileEntityLootable, ITileEntity, IInventory
{
	...
	public void migrateItemsFromOtherContainer(ITileEntityLootable _other)
	{
		...
		{
			Log.Error(string.Format("{0}.UpgradeDowngradeFrom to smaller container, discarding overflow: other.size={1}, other.length={2}, this.size={3}, this.length={4}", "TEFeatureStorage", _other.GetContainerSize(), _other.items.Length, containerSize, items.Length));
		}
	}
