...
public class ItemActionEntryUse : BaseItemActionEntry
{
	...
	public ItemActionEntryUse(XUiController controller, ConsumeType consumeType)
		: base(controller, "", "", GamepadShortCut.DPadLeft)
	{
	...
	public override void OnActivated()
	{
		...
		if (!stackControl.ItemStack.itemValue.ItemClass.CanExecuteAction(0, base.ItemController.xui.playerUI.entityPlayer, stackControl.ItemStack.itemValue))
		{
		...
		RefreshEnabled();
		oldToolbeltFocusID = inventory.GetFocusedItemIdx();
		int actionIdx = 0;
		if (stackControl.ItemStack.itemValue.ItemClass != null)
		{
			for (int i = 0; i < stackControl.ItemStack.itemValue.ItemClass.Actions.Length; i++)
			{
				...
				case ConsumeType.Open:
					...
					else if (stackControl.ItemStack.itemValue.ItemClass.Actions[i] is ItemActionOpenLootBundle)
					{
						flag = true;
					}
					break;
