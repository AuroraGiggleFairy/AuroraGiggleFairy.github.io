...
public class Inventory
{
	...
	{
		if (!entity.IsDead() && entity.IsSpawned())
		{
		...
		if (entity is EntityPlayer && entity.emodel != null && entity.emodel.avatarController != null)
		{
	...
	public IEnumerator delayedShowHideHeldItem(bool show, float waitTime)
	{
		...
		if ((bool)entity)
		{
			...
			entity.MinEventContext.ItemValue = holdingItemItemValue;
			if (!show && (bool)entity.emodel && (bool)entity.emodel.avatarController)
			{
				entity.emodel.avatarController.TriggerEvent(AvatarController.itemHasChangedTriggerHash);
			}
			if (entity is EntityPlayerLocal { bFirstPersonView: not false } entityPlayerLocal)
			{
	...
	public virtual void setHeldItemByIndex(int _inventoryIdx, bool _applyHolsterTime)
	{
		...
		if (previousHeldItemValue != holdingItemItemValue && !holdingItem.Equals(bareHandItem) && _inventoryIdx != DUMMY_SLOT_IDX && m_HoldingItemIdx != _inventoryIdx)
		{
			previousHeldItemValue = holdingItemItemValue;
			previousHeldItemSlotIdx = m_HoldingItemIdx;
		}
		...
		ShowHeldItem(show: false, 0f);
		ShowHeldItem(show: true, (!_applyHolsterTime) ? 0f : 0.2f);
		if (flag)
		{
	...
	public virtual int GetItemCount(ItemValue _itemValue, bool _bConsiderTexture = false, int _seed = -1, int _meta = -1, bool _ignoreModdedItems = true)
	{
		...
		{
			if ((!_ignoreModdedItems || !slots[i].itemValue.HasModSlots || !slots[i].itemValue.HasMods()) && slots[i].itemStack.itemValue.type == _itemValue.type && (!_bConsiderTexture || slots[i].itemStack.itemValue.Texture == _itemValue.Texture) && (_seed == -1 || _seed == slots[i].itemValue.Seed) && (_meta == -1 || _meta == slots[i].itemValue.Meta))
			{
	...
	public virtual Transform createHeldItem(int _idx, ItemValue _itemValue)
	{
		...
		Transform transform = _itemValue.ItemClass.CloneModel(entity.world, _itemValue, entity.GetPosition(), inactiveItems, BlockShape.MeshPurpose.Hold, 0L);
		if (transform != null)
		{
			transform.gameObject.SetActive(value: false);
		}
	...
	{
		holdingItemItemValue.FireEvent(_eventType, _eventParms);
	}
	...
	{
		if (previousHeldItemValue == null || previousHeldItemSlotIdx < 0)
		{
			return false;
		}
		if (previousHeldItemValue.Equals(_itemValue))
		{
			return slots[previousHeldItemSlotIdx].Equals(_itemValue);
		}
		return false;
	}
	...
	{
		if (previousHeldItemSlotIdx == -1 || previousHeldItemValue == null)
		{
			return -1;
		}
		if (slots[previousHeldItemSlotIdx].itemValue.Equals(previousHeldItemValue))
		{
			return previousHeldItemSlotIdx;
		}
		for (int i = 0; i < slots.Length; i++)
		{
			if (slots[i].itemValue.Equals(previousHeldItemValue))
			{
				return i;
			}
		}
