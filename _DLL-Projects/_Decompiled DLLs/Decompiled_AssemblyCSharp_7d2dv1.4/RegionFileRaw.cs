...
public class RegionFileRaw : RegionFile
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public unsafe static void WriteBytes<T>(Stream stream, T[] buf, int offset, int length) where T : unmanaged
	{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public unsafe static void ReadBytes<T>(Stream stream, T[] buf, int offset, int length) where T : unmanaged
	{
		fixed (T* ptr = &buf[offset])
		{
			void* pointer = ptr;
