...
public class EntityPlayer : EntityAlive
{
	...
	public int GetLootStage(float containerMod, float containerBonus)
	{
		...
		if (biomeStandingOn != null)
		{
			return Mathf.FloorToInt(EffectManager.GetValue(PassiveEffects.LootStage, null, (float)Progression.Level * (1f + num + biomeStandingOn.LootStageMod + containerMod) + (num2 + biomeStandingOn.LootStageBonus + containerBonus), this));
		}
	...
	public virtual void onNewPrefabEntered(PrefabInstance _prefabInstance)
	{
		if (_prefabInstance != null && _prefabInstance.prefab.bTraderArea && this is EntityPlayerLocal entityPlayerLocal)
		{
			...
			waypoint.lastKnownVehiclePositionEntityId = -1;
	...
	public override void onNewBiomeEntered(BiomeDefinition _biome)
	{
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			if (biomeStandingOn != null)
			{
				biomeStandingOn.WeatherChanged -= BiomeStandingOn_WeatherChanged;
				if (biomeStandingOn.Buff != null)
				{
			...
			{
				biomeStandingOn.WeatherChanged += BiomeStandingOn_WeatherChanged;
				if (EntityStats.NewWeatherSurvivalEnabled && biomeStandingOn.Buff != null)
				{
					Buffs.AddBuff(biomeStandingOn.Buff);
				}
			}
		}
		else
	...
	public void AddKillXP(EntityAlive killedEntity, float xpModifier = 1f)
	{
		...
		if (!isEntityRemote)
		{
			Progression.AddLevelExp(experienceValue, "_xpFromKill", Progression.XPTypes.Kill);
