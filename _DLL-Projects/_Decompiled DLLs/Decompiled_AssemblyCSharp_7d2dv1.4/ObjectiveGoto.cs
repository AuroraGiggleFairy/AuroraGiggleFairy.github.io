...
public class ObjectiveGoto : BaseObjective
{
	...
	public virtual Vector3 GetPosition(EntityNPC ownerNPC = null, EntityPlayer entityPlayer = null, List<Vector2> usedPOILocations = null, int entityIDforQuests = -1)
	{
		...
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			...
			if (prefabInstance == null)
			{
				prefabInstance = GameManager.Instance.World.ChunkClusters[0].ChunkProvider.GetDynamicPrefabDecorator().GetClosestPOIToWorldPos(fastTags, new Vector2(entityAlive.position.x, entityAlive.position.z), usedPOILocations, -1, flag, biomeFilterType, biomeFilter);
				if (prefabInstance == null)
				{
					prefabInstance = GameManager.Instance.World.ChunkClusters[0].ChunkProvider.GetDynamicPrefabDecorator().GetClosestPOIToWorldPos(fastTags, new Vector2(entityAlive.position.x, entityAlive.position.z), usedPOILocations, -1, flag);
				}
			}
			if (prefabInstance == null)
			{
