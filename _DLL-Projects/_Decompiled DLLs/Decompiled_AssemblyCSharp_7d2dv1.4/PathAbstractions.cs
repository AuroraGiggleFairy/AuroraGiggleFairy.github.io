...
public static class PathAbstractions
{
	...
	public class SearchDefinition
	{
		...
		public AbstractedLocation GetLocation(string _name, string _worldName = null, string _gameName = null)
		{
		...
		public List<AbstractedLocation> GetAvailablePathsList(Regex _nameMatch = null, string _worldName = null, string _gameName = null, bool _ignoreDuplicateNames = false)
		{
			...
			foreach (SearchPath path in paths)
			{
				if (path.CanMatch)
				{
					path.GetAvailablePathsList(list, _nameMatch, _worldName, _gameName, _ignoreDuplicateNames);
				}
			}
			return list;
		}

		public void GetAvailablePathsList(List<AbstractedLocation> _resultList, Regex _nameMatch = null, string _worldName = null, string _gameName = null, bool _ignoreDuplicateNames = false)
		{
			if (_resultList == null)
			{
	...
	public abstract class SearchPath
	{
		...
		public void getAvailablePathsSingleBase(List<AbstractedLocation> _targetList, EAbstractedLocationType _locationType, string _basePath, Regex _nameMatch, string _worldName, string _gameName, bool _ignoreDuplicateNames, Mod _containingMod, string _subfolder = null)
		{
			...
			{
				if ((Owner.Extension == null || sdFileSystemInfo.Name.EndsWith(Owner.Extension, StringComparison.Ordinal)) && (_nameMatch == null || _nameMatch.IsMatch(sdFileSystemInfo.Name)))
				{
					string filename = (Owner.RemoveExtension ? GameIO.RemoveExtension(sdFileSystemInfo.Name, Owner.Extension) : sdFileSystemInfo.Name);
					if (!_ignoreDuplicateNames || !_targetList.Exists([PublicizedFrom(EAccessModifier.Internal)] (AbstractedLocation _location) => _location.Name.Equals(filename)))
					{
	...
	{
		for (int i = 0; i < allSearchDefs.Count; i++)
		{
			allSearchDefs[i].InvalidateCache();
		}
	}
}
