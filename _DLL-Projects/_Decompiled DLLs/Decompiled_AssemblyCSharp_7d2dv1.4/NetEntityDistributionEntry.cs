...
public class NetEntityDistributionEntry
{
	...
	public void updatePlayerEntity(EntityPlayer _ep)
	{
		...
		if (num >= (float)(-trackingDistanceThreshold) && num <= (float)trackingDistanceThreshold && num2 >= (float)(-trackingDistanceThreshold) && num2 <= (float)trackingDistanceThreshold)
		{
	...
	public void updatePlayerList(List<EntityPlayer> _playerList)
	{
		...
		if (flag)
		{
			...
			if (netPackage != null)
			{
				int range = ((trackingDistanceThreshold != int.MaxValue) ? trackingDistanceThreshold : 192);
				SendToPlayers(netPackage, -1, inRangeOnly, range);
			}
			EntityAlive entityAlive = trackedEntity as EntityAlive;
