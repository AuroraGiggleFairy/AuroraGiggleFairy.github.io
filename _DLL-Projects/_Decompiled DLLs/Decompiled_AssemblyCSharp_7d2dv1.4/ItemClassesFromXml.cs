...
public class ItemClassesFromXml
{
	...
	{
		foreach (XElement item in _node.Elements("smell"))
		{
			if (!item.HasAttribute("name"))
			{
			...
			string attribute = item.GetAttribute("name");
			if (!item.HasAttribute("range"))
			{
				throw new Exception("Attribute 'range' missing in smell name='" + attribute + "'");
			}
			float range = StringParsers.ParseFloat(item.GetAttribute("range"));
			if (!item.HasAttribute("belt_range"))
			{
				throw new Exception("Attribute 'belt_range' missing in smell name='" + attribute + "'");
			}
			float beltRange = StringParsers.ParseFloat(item.GetAttribute("belt_range"));
			float heatMapStrength = 0f;
			if (item.HasAttribute("heat_map_strength"))
			{
				heatMapStrength = StringParsers.ParseFloat(item.GetAttribute("heat_map_strength"));
