...
public class Waypoint
{
	...
	public int lastKnownVehiclePositionEntityId = -1;
	...
	public Waypoint Clone()
	{
		return new Waypoint
		{
			...
			bTracked = bTracked,
			ownerId = ownerId,
			lastKnownVehiclePositionEntityId = lastKnownVehiclePositionEntityId,
			navObject = navObject,
	...
	public void Read(BinaryReader _br, int version = 6)
	{
		...
		if (version > 1)
		{
			...
			lastKnownVehiclePositionEntityId = _br.ReadInt32();
		}
		if (version > 3)
		{
	...
	public void Write(BinaryWriter _bw)
	{
		...
		_bw.Write(lastKnownVehiclePositionEntityId);
	...
	public override bool Equals(object obj)
	{
		if (obj is Waypoint waypoint)
		{
			...
			{
				return waypoint.lastKnownVehiclePositionEntityId == lastKnownVehiclePositionEntityId;
			}
			return false;
		}
		return false;
	...
	{
		return ((((17 * 31 + pos.GetHashCode()) * 31 + icon.GetHashCode()) * 31 + name.Text.GetHashCode()) * 31 + (ownerId?.GetHashCode() ?? 0)) * 31 + lastKnownVehiclePositionEntityId.GetHashCode();
	}

	public bool CanBeViewedBy(PlatformUserIdentifierAbs _userIdentifier)
	{
		if (lastKnownVehiclePositionEntityId != -1)
		{
	...
	{
		return "Waypoint name:" + name?.ToString() + " icon:" + icon + " Entity ID:" + lastKnownVehiclePositionEntityId;
	}
}
