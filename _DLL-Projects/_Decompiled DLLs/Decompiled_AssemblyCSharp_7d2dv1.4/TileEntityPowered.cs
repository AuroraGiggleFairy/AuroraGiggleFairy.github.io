...
public class TileEntityPowered : TileEntity, IPowered
{
	...
	public Transform BlockTransform
	{
		...
		set
		{
			...
			{
				Vector3 wireOffset2 = block.Block.shape.GetRotation(block) * StringParsers.ParseVector3(block.Block.Properties.Values["WireOffset"]);
	...
	public void RemoveParentWithWiringTool(int wiringEntityID)
	{
		...
		else
		{
			...
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageWireActions>().Setup(NetPackageWireActions.WireActions.RemoveParent, ToWorldPos(), new List<Vector3i>(), wiringEntityID));
		}
		SetModified();
	}
