...
public class ItemActionMelee : ItemActionAttack
{
	...
	public virtual void hitTheTarget(InventoryDataMelee _actionData, WorldRayHitInfo hitInfo, float damageScale)
	{
		...
		float value = _actionData.invData.item.CritChance.Value;
		value = Mathf.Clamp01(value * (holdingEntity.Stamina / holdingEntity.Stats.Stamina.Max));
		value = EffectManager.GetValue(PassiveEffects.CriticalChance, itemValue, value, holdingEntity);
		...
		ItemActionAttack.Hit(hitInfo, holdingEntity.entityId, (DamageType == EnumDamageTypes.None) ? EnumDamageTypes.Bashing : DamageType, blockDamage, entityDamage, holdingEntity.Stats.Stamina.ValuePercent, weaponCondition, value, ItemAction.GetDismemberChance(_actionData, hitInfo), item.MadeOfMaterial.SurfaceCategory, damageMultiplier, getBuffActions(_actionData), _actionData.attackDetails, num, ActionExp, ActionExpBonusMultiplier, this, ToolBonuses, _actionData.bHarvesting ? EnumAttackMode.RealAndHarvesting : EnumAttackMode.RealNoHarvesting);
