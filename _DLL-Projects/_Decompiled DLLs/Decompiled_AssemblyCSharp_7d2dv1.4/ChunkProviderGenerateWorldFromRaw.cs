...
public class ChunkProviderGenerateWorldFromRaw : ChunkProviderGenerateWorld
{
	...
	public override IEnumerator Init(World _world)
	{
		...
		if (!SdFile.Exists(dtmFilename + ".raw"))
		{
			if (SdFile.Exists(dtmFilename + ".tga"))
			{
				...
				Log.Out("Converting tga to dtm took " + ms.ElapsedMilliseconds + "ms");
				ms.ResetAndRestart();
			}
			else
			{
		...
		{
			loadDTM();
			Log.Out("Loading dtm raw file took " + ms.ElapsedMilliseconds + "ms");
			ms.ResetAndRestart();
		}
		yield return GCUtils.UnloadAndCollectCo();
		...
		Log.Out("Loading and creating biomes took " + ms.ElapsedMilliseconds + "ms");
		ms.ResetAndRestart();
		yield return GCUtils.UnloadAndCollectCo();
		...
		Log.Out("Loading and creating shader control textures took " + ms.ElapsedMilliseconds + "ms");
		ms.ResetAndRestart();
		yield return GCUtils.UnloadAndCollectCo();
		...
		Log.Out("Loading and parsing of generator took " + ms.ElapsedMilliseconds + "ms");
		ms.ResetAndRestart();
	...
	public IEnumerator processFiles(string _worldPath, ProcessFilesCallback _callback)
	{
		...
		GetDynamicPrefabDecorator().CopyPrefabHeightsIntoHeightMap(heightMapWidth, heightMapHeight, heightData, heightMapScale, topTexMap);
		...
		if (flag)
		{
			...
			for (int num = 0; num < topTexMap.Length; num++)
			{
				TextureUtils.ColorARGB32 value = rawTextureData[num];
				TextureUtils.ColorARGB32 value2 = default(TextureUtils.ColorARGB32);
				if (splat4Loaded)
				{
					value2 = splat4Cols[num];
				}
				switch (topTexMap[num])
				{
				case 10:
					value.r = byte.MaxValue;
					break;
				...
				rawTextureData[num] = value;
				splat4Cols[num] = value2;
			}
		}
		else
		{
			NativeArray<Color32> rawTextureData2 = splat3Tex.GetRawTextureData<Color32>();
			for (int num2 = 0; num2 < topTexMap.Length; num2++)
			{
				Color32 value3 = rawTextureData2[num2];
				TextureUtils.ColorARGB32 value4 = default(TextureUtils.ColorARGB32);
				if (splat4Loaded)
				{
					value4 = splat4Cols[num2];
				}
				switch (topTexMap[num2])
				{
				case 10:
				...
				rawTextureData2[num2] = value3;
				splat4Cols[num2] = value4;
			}
		}
		yield return null;
		if (heightMap == null)
		...
		int num3 = colors.width * colors.height;
		for (int num4 = 0; num4 < num3; num4++)
		{
			if (colors.GetValue(num4) < 5)
			{
				continue;
			}
			TextureUtils.ColorARGB32 value5 = splat4Cols[num4];
			if (Mathf.FloorToInt(heightMap.GetAt(num4)) <= (byte)(colors.GetValue(num4) - 5) + 1)
			{
				value5.g = byte.MaxValue;
				if (colors.GetValue(num4) > 5)
				{
					value5.b = (byte)(colors.GetValue(num4) - 5);
				}
			}
			splat4Cols[num4] = value5;
		}
		yield return null;
