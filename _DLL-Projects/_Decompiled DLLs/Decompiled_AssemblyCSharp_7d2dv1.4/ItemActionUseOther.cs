...
public class ItemActionUseOther : ItemAction
{
	...
	public override void ExecuteAction(ItemActionData _actionData, bool _bReleased)
	{
		...
		_actionData.invData.holdingEntity.FireEvent(MinEventTypes.onSelfHealedOther);
		_actionData.invData.holdingEntity.FireEvent((_actionData.indexInEntityOfAction == 0) ? MinEventTypes.onSelfPrimaryActionEnd : MinEventTypes.onSelfSecondaryActionEnd);
		...
		if (Consume)
		{
			if (_actionData.invData.itemValue.MaxUseTimes > 0 && _actionData.invData.itemValue.UseTimes + 1f < (float)_actionData.invData.itemValue.MaxUseTimes)
			{
				...
				itemValue.UseTimes += EffectManager.GetValue(PassiveEffects.DegradationPerUse, feedInventoryData.invData.itemValue, 1f, _actionData.invData.holdingEntity, null, _actionData.invData.itemValue.ItemClass.ItemTags);
		...
		if (CreateItem != null && CreateItemCount > 0)
		{
			...
			{
				_actionData.invData.holdingEntity.world.gameManager.ItemDropServer(itemStack, _actionData.invData.holdingEntity.GetPosition(), Vector3.zero);
			}
		}
		feedInventoryData.bFeedingStarted = false;
		feedInventoryData.TargetEntity = null;
	...
	{
		FeedInventoryData feedInventoryData = (FeedInventoryData)_actionData;
		if (feedInventoryData == null || !canShowOverlay(feedInventoryData) || !(feedInventoryData.invData.holdingEntity is EntityPlayerLocal))
		{
			return;
		}
