...
public abstract class AvatarMultiBodyController : AvatarController
{
	...
	{
		timeHarestingAnimationPlaying = 0.3f;
	...
	public override void Update()
	{
		...
		{
			_value = Mathf.Lerp(_value, targetSpeedForward, deltaTime * forwardSpeedLerpMultiplier);
			_value2 = Mathf.Lerp(_value2, targetSpeedStrafe, deltaTime * strafeSpeedLerpMultiplier);
		}
		float x = entity.rotation.x;
		bool flag = entity.IsDead();
		bool flag2 = Mathf.Abs(_value) > 0.01f || Mathf.Abs(_value2) > 0.01f;
		if (flag2)
		{
			idleTime = 0f;
		}
		...
		_setFloat(AvatarController.rotationPitchHash, x, _netsync: false);
		if (!flag)
		{
			if (_value2 >= 1234f)
			{
			...
			{
				float num2 = _value * _value + _value2 * _value2;
	...
	public override void _setTrigger(int _propertyHash, bool _netsync = true)
	{
		...
		{
			ChangedAnimationParameters[_propertyHash] = new AnimParamData(_propertyHash, AnimParamData.ValueTypes.Trigger, _value: true);
		}
		if (changed)
		{
