...
public class GameServerInfo
{
	...
	public static readonly Dictionary<GameInfoInt, int[]> DefaultIntRanges = new Dictionary<GameInfoInt, int[]>
	{
		...
		},
		{
			...
			new int[9] { 25, 50, 75, 100, 125, 150, 175, 200, 300 }
		},
		{
			GameInfoInt.DayNightLength,
			new int[8] { 10, 20, 30, 40, 50, 60, 90, 120 }
		},
		{
			GameInfoInt.DayLightLength,
			new int[4] { 12, 14, 16, 18 }
		},
		{
		...
		},
		{
			...
			new int[9] { 0, 1, 2, 3, 4, 7, 10, 14, 20 }
		},
		{
		...
		},
		{
			...
			new int[9] { 4, 6, 8, 10, 12, 16, 24, 32, 64 }
		},
		{
		...
		},
		{
			...
			new int[11]
			{
				25, 33, 50, 67, 75, 100, 125, 150, 175, 200,
				300
			}
		},
		{
		...
		},
		{
			...
			new int[8] { 25, 33, 50, 67, 75, 100, 150, 200 }
		},
		{
			GameInfoInt.LootRespawnDays,
			new int[9] { -1, 5, 7, 10, 15, 20, 30, 40, 50 }
		},
		{
			GameInfoInt.MaxChunkAge,
			new int[8] { -1, 10, 20, 30, 40, 50, 75, 100 }
		},
		{
		...
		},
		{
			...
			new int[5] { 21, 31, 41, 51, 71 }
		},
		{
			GameInfoInt.LandClaimDeadZone,
			new int[8] { 0, 5, 10, 15, 20, 30, 40, 50 }
		},
		{
			GameInfoInt.LandClaimExpiryTime,
			new int[8] { 1, 2, 3, 4, 5, 7, 10, 30 }
		},
		{
		...
		},
		{
			...
			new int[10] { 0, 1, 2, 4, 8, 16, 32, 64, 128, 256 }
		},
		{
		...
		},
		{
			...
			new int[6] { 3, 7, 15, 30, 45, 60 }
		},
		{
			GameInfoInt.PartySharedKillRange,
			new int[6] { 0, 100, 500, 1000, 5000, 10000 }
		}
	};
	...
	public static readonly GameInfoInt[] IntInfosInGameTags = new GameInfoInt[43]
	{
		GameInfoInt.GameDifficulty,
		GameInfoInt.DayNightLength,
		GameInfoInt.DeathPenalty,
	...
	public static readonly GameInfoBool[] BoolInfosInGameTags = new GameInfoBool[12]
	{
	...
	public void RefreshServerDisplayTexts(GameServerInfo gsi)
	{
		...
		{
			platformUserIdentifierAbs = PlatformUserIdentifierAbs.FromCombinedString(value2, _logErrors: false);
		...
		if (!string.IsNullOrEmpty(value3) && string.IsNullOrEmpty(ServerDisplayName.Text))
		{
			ServerDisplayName.Update(value3, platformUserIdentifierAbs);
		}
		if (!string.IsNullOrEmpty(value4) && string.IsNullOrEmpty(ServerWorldName.Text))
		{
			ServerWorldName.Update(value4, platformUserIdentifierAbs);
		}
		if (!string.IsNullOrEmpty(value5) && string.IsNullOrEmpty(ServerDescription.Text))
		{
			ServerDescription.Update(value5.Replace("\\n", "\n"), platformUserIdentifierAbs);
		}
		if (!string.IsNullOrEmpty(value6) && string.IsNullOrEmpty(ServerURL.Text))
		{
			ServerURL.Update(value6, platformUserIdentifierAbs);
		}
		if (!string.IsNullOrEmpty(value7) && string.IsNullOrEmpty(ServerLoginConfirmationText.Text))
		{
			ServerLoginConfirmationText.Update(value7, platformUserIdentifierAbs);
		}
	}
	...
	{
		foreach (KeyValuePair<GameInfoInt, int[]> defaultIntRange in DefaultIntRanges)
		{
			if (!Ints.TryGetValue(defaultIntRange.Key, out var value))
			{
				continue;
			}
			bool flag = false;
			for (int i = 0; i < defaultIntRange.Value.Length; i++)
			{
				if (defaultIntRange.Value[i] == value)
				{
					flag = true;
					break;
				}
			...
			if (!flag)
			{
				Log.Warning("Using non default value {0} for int {1}", value, defaultIntRange.Key);
				return false;
			}
		}
		return true;
	...
	public static int[] GetDefaultIntValues(EnumGamePrefs gamePref)
	{
		...
		foreach (KeyValuePair<GameInfoInt, int[]> defaultIntRange in DefaultIntRanges)
		{
			if (defaultIntRange.Key.ToString().Equals(text))
			{
				return defaultIntRange.Value;
			}
		}
