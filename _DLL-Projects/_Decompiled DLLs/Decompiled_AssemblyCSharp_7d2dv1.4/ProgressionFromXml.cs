	...
	public static void parseProgressionItem(XElement childElement, int max_level, int min_level, int base_cost, float cost_multiplier_per_level, float max_level_ratio_to_parent, ref float order)
	{
		...
		foreach (XElement item2 in childElement.Elements())
		{
			if (item2.Name == "level_requirements")
			{
				...
				{
					foreach (XElement item3 in item2.Elements("requirement"))
					{
						IRequirement requirement = RequirementBase.ParseRequirement(item3);
						if (requirement != null)
						{
			...
			else
			{
				...
				string[] array = item2.GetAttribute("unlock_level").Split(',');
				...
				{
					array2[i] = StringParsers.ParseSInt32(array[i]);
				}
				...
				{
					array3 = item2.GetAttribute("icon").Split(',');
				}
				if (array3 != null)
				{
					customIconTint = ((!item2.HasAttribute("icontint")) ? new string[array3.Length] : item2.GetAttribute("icontint").Split(','));
				}
				...
				{
					string[] array4 = item2.GetAttribute("name_key").Split(',');
					for (int j = 0; j < array4.Length; j++)
					{
						array4[j] = Localization.Get(array4[j]);
					}
					customName = array4;
				}
				ProgressionClass.DisplayData displayData = progressionClass.AddDisplayData(item, array2, array3, customIconTint, customName, customHasQuality);
				...
				foreach (XElement item4 in item2.Elements("unlock_entry"))
				{
					int unlockTier = 0;
					if (item4.HasAttribute("unlock_tier"))
					{
						unlockTier = StringParsers.ParseSInt32(item4.GetAttribute("unlock_tier")) - 1;
					}
					string[] recipeList = null;
					if (item4.HasAttribute("recipes"))
					{
						recipeList = item4.GetAttribute("recipes").Split(',');
					}
					if (item4.HasAttribute("item"))
					{
						string[] array5 = item4.GetAttribute("item").Split(',');
						for (int k = 0; k < array5.Length; k++)
						{
							displayData.AddUnlockData(array5[k], unlockTier, recipeList);
						}
					}
				}
			}
