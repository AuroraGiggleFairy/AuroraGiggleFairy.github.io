...
public class AvatarLocalPlayerController : AvatarCharacterController
{
	...
	public override void SwitchModelAndView(string _modelName, bool _bFPV, bool _bMale)
	{
		...
		if (fpsArms == null)
		{
			...
			if (transform != null)
			{
				Transform transform2 = transform.Find((entity.emodel is EModelSDCS) ? "baseRigFP" : (isMale ? "maleArms_fp" : "femaleArms_fp"));
				if (transform2 != null)
				{
	...
	public void TPVResetAnimPose()
	{
		if ((bool)anim)
		{
			tpvDisableInFrames = 1;
			anim.enabled = true;
			anim.Play("Locomotion", 0);
			anim.Play("UpperBodyLocomotion", 2);
			anim.Play("New State", 4);
		}
	}
	...
	{
		base.LateUpdate();
		if (tpvDisableInFrames > 0 && --tpvDisableInFrames == 0 && (bool)anim)
		{
			anim.enabled = false;
		}
	}
