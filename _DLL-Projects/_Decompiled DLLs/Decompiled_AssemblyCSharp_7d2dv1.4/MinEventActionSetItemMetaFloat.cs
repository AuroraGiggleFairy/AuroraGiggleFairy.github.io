...
public class MinEventActionSetItemMetaFloat : MinEventActionBase
{
	...
	public override void Execute(MinEventParams _params)
	{
		...
		{
			itemValue.SetMetadata(metaKey, 0f, "float");
		}
		...
		if (metadata is float)
		{
			if (relative)
			{
				itemValue.SetMetadata(metaKey, (float)metadata + change, "float");
			}
			else
			{
				itemValue.SetMetadata(metaKey, change, "float");
			}
			if ((float)metadata < 0f)
			{
				itemValue.SetMetadata(metaKey, 0, "float");
			}
		}
	}
