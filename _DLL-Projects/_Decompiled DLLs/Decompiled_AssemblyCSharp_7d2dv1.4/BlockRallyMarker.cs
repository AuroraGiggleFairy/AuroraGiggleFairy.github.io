...
public class BlockRallyMarker : Block
{
	...
	public override void OnBlockEntityTransformAfterActivated(WorldBase _world, Vector3i _blockPos, int _cIdx, BlockValue _blockValue, BlockEntityData _ebcd)
	{
		...
		if (!_world.IsEditor())
		{
			_ebcd.transform.GetChild(0).gameObject.SetActive(value: false);
		}
	}
	...
	public void ShowRallyMarker(bool _show, WorldBase _world, Vector3i _blockPos, int _clrIdx)
	{
		...
		if (chunkCluster != null)
		{
			((Chunk)chunkCluster.GetChunkFromWorldPos(_blockPos))?.GetBlockEntity(_blockPos).transform.GetChild(0).gameObject.SetActive(_show);
		}
	}
}
