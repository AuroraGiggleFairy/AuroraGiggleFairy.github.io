...
public class XUiM_PlayerInventory : XUiModel, IInventory
{
		...
		if (!flag)
		{
			...
			if (itemStack.count > num)
			{
				for (int num2 = itemStack.count; num2 > 0; num2 -= num)
				{
					...
					if (itemStack.itemValue.ItemClass is ItemClassArmor itemClassArmor && !xui.PlayerEquipment.IsEquipmentTypeWorn(itemClassArmor.EquipSlot) && localPlayer.equipment.ReturnItem(itemStack2))
					{
						flag = true;
						flag2 = true;
			...
			else if (itemStack.itemValue.ItemClass is ItemClassArmor itemClassArmor2 && !xui.PlayerEquipment.IsEquipmentTypeWorn(itemClassArmor2.EquipSlot) && localPlayer.equipment.ReturnItem(itemStack))
			{
				flag = true;
	...
	public int CountAvailabileSpaceForItem(ItemValue itemValue)
	{
		...
		for (int i = 0; i < slots.Length; i++)
		{
			...
			if (itemStack.CanStackWith(slots[i]))
			{
		...
		for (int j = 0; j < toolbelt.PUBLIC_SLOTS; j++)
		{
			...
			if (itemStack.CanStackWith(slots2[j]))
			{
