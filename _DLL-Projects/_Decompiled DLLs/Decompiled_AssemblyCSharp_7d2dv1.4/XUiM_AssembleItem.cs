	...
	public bool AddPartToItem(ItemStack partStack, out ItemStack resultStack)
	{
		...
		if (itemClassModifier != null)
		{
			...
			else
			{
				...
				{
					if (CurrentItem.itemValue.Modifications[j] != null && CurrentItem.itemValue.Modifications[j].ItemClass != null && !itemClassModifier.HasAnyTags(EntityDrone.cStorageModifierTags) && itemClassModifier.HasAnyTags(CurrentItem.itemValue.Modifications[j].ItemClass.ItemTags))
					{
						resultStack = partStack;
						return false;
					}
			...
			if (itemClassModifier.HasAnyTags(ItemClassModifier.CosmeticModTypes))
			{
				if (CurrentItem.itemValue.CosmeticMods != null)
				{
					for (int k = 0; k < CurrentItem.itemValue.CosmeticMods.Length; k++)
					{
						...
						{
							float num = 1f - CurrentItem.itemValue.PercentUsesLeft;
							...
							{
								CurrentItem.itemValue.UseTimes = (int)(num * (float)CurrentItem.itemValue.MaxUseTimes);
							}
