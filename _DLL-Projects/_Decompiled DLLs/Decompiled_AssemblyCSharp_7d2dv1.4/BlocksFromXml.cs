...
public class BlocksFromXml
{
	...
	public static Block CreateBlock(bool _bEditMode, string blockName, DynamicProperties properties)
	{
		...
		else if (properties.Values.TryGetValue("Model", out _value) && (_value.EqualsCaseInsensitive("cube") || _value.EqualsCaseInsensitive("cube_glass") || _value.EqualsCaseInsensitive("cube_frame")))
		{
			block.PathType = 1;
		}
		...
		if (text6.Length > 0)
		{
			try
			{
				if (text6.Contains(","))
				{
					string[] sideTextureId = text6.Split(',');
					block.SetSideTextureId(sideTextureId);
				}
				else
				{
					int sideTextureId2 = int.Parse(text6);
					block.SetSideTextureId(sideTextureId2);
				}
			}
			catch (Exception)
			{
				throw new Exception("Error parsing texture id '" + text6 + "' in block with name=" + blockName);
			}
		}
	...
	public static HashSet<int> GetTextureIdsForMesh(XmlFile _xmlFile, string _meshName)
	{
		...
		try
		{
			...
			foreach (XElement item3 in root.Elements(XNames.block))
			{
				...
				{
					throw new Exception("Error parsing texture id '" + text3 + "' in block " + item3.GetAttribute("id"));
				}
			}
		}
		catch (Exception ex3)
