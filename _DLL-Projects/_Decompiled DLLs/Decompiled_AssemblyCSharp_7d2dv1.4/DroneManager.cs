...
public class DroneManager
{
		...
		if (_reason == EnumRemoveEntityReason.Unloaded)
		{
			...
			if ((bool)owner)
			{
				...
				if (ownedEntity != null)
				{
					...
					NavObjectManager.Instance.RegisterNavObject(entityClass.NavObject, ownedEntity.LastKnownPosition);
				}
			}
	...
	public string LogUnloadedDronesForPlayer(EntityPlayer player)
	{
		...
		{
			if (dronesUnloaded[i].belongsPlayerId == player.entityId)
			{
				text = text + dronesUnloaded[i].clientEntityId + Environment.NewLine;
			}
		}
		return text;
	}
	...
	public void Save()
	{
		if (saveThread == null || !ThreadManager.ActiveThreads.ContainsKey("droneDataSave"))
		{
			...
			saveThread = ThreadManager.StartThread("droneDataSave", null, SaveThread, null, System.Threading.ThreadPriority.Normal, pooledExpandableMemoryStream, null, _useRealThread: false, _isSilent: true);
		}
	}
