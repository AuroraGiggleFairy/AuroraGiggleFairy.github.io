...
public abstract class ItemActionAttack : ItemAction
{
	...
	public override void ReadFrom(DynamicProperties _props)
	{
		...
		{
			particlesMuzzleFire = _props.Values["Particles_muzzle_fire"];
		}
		if (_props.Values.ContainsKey("Particles_muzzle_fire_fpv"))
		{
			particlesMuzzleFireFpv = _props.Values["Particles_muzzle_fire_fpv"];
		}
			...
			if (_attackMode != EnumAttackMode.Simulate)
			{
				...
				if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer && (entityAlive as EntityPlayer == null || !entityAlive.isEntityRemote) && entity.isEntityRemote && rangeCheckedAction != null)
				{
					...
					if (entityPlayer != null)
					{
						...
						text2 = ((!entityAlive2 || entityAlive2.RecordedDamage.ArmorDamage <= entityAlive2.RecordedDamage.Strength) ? EntityClass.list[entity.entityClass].Properties.Values["SurfaceCategory"] : "metal");
						text = text2;
