...
public class XUiC_ServerBrowser : XUiController
{
	...
	public void BtnConnect_OnPressed(XUiController _sender, int _mouseButton)
	{
		...
		EUserPerms permissionsWithPrompt = ((!gsi.AllowsCrossplay) ? EUserPerms.Multiplayer : (EUserPerms.Multiplayer | EUserPerms.Crossplay));
		wdwMultiplayerPrivileges?.ResolvePrivilegesWithDialog(permissionsWithPrompt, [PublicizedFrom(EAccessModifier.Internal)] (bool result) =>
		{
			if (result)
			{
