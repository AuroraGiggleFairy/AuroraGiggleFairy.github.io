...
public class BlockToolSelection : ISelectionBoxCallback, IBlockTool
{
	...
	public BlockToolSelection()
	{
		...
		nGuiAction4.SetClickActionDelegate([PublicizedFrom(EAccessModifier.Private)] () =>
		{
			...
			{
				BeginUndo(0);
				BlockTools.CubeRPC(GameManager.Instance, SelectionClrIdx, SelectionStart, SelectionEnd, BlockValue.Air, MarchingCubes.DensityAir, 0, 0L);
		...
		nGuiAction5.SetClickActionDelegate([PublicizedFrom(EAccessModifier.Private)] () =>
		{
			...
			if (!blockValue.isair)
			{
				...
				BlockTools.CubeRPC(GameManager.Instance, SelectionClrIdx, m_selectionStartPoint, m_SelectionEndPoint, blockValue, blockValue.Block.shape.IsTerrain() ? MarchingCubes.DensityTerrain : MarchingCubes.DensityAir, 0, holdingItemItemValue.Texture);
				EndUndo(0);
			}
		});
		...
		foreach (NGuiAction action in actions)
		{
		...
		case KeyCode.V:
			...
			if (!flag2 && SelectionLockMode != 2)
			{
				if (SelectionActive && clipboard.size.Equals(Vector3i.one) && !SelectionSize.Equals(clipboard.size))
				{
					...
					long texture = clipboard.GetTexture(0, 0, 0);
	...
	public void EndUndo(int _clrIdx, bool _bRedo = false)
	{
		...
		if (undoChanges.Count > 0)
		{
			...
			if (!_bRedo)
			{
				undoQueue.Add(undoChanges);
			}
			else
			{
				redoQueue.Add(undoChanges);
			}
		}
		if (undoQueue.Count > 100)
		{
		...
		if (redoQueue.Count > 100)
		{
			redoQueue.RemoveAt(0);
		}
		undoChanges = null;
	}
	...
	public void blockUndo()
	{
		...
		{
			List<BlockChangeInfo> list = undoQueue[undoQueue.Count - 1];
			if (redoQueue.Count > 100)
			{
				redoQueue.RemoveAt(0);
			}
			BeginUndo(list[0].clrIdx);
			GameManager.Instance.SetBlocksRPC(list);
			undoQueue.RemoveAt(undoQueue.Count - 1);
			EndUndo(list[0].clrIdx, _bRedo: true);
		}
	}
	...
	public void blockRedo()
	{
		...
		{
			List<BlockChangeInfo> list = redoQueue[redoQueue.Count - 1];
			BeginUndo(list[0].clrIdx);
			GameManager.Instance.SetBlocksRPC(list);
			redoQueue.RemoveAt(redoQueue.Count - 1);
			EndUndo(list[0].clrIdx);
		}
	}
