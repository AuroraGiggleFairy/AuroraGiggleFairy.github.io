...
public abstract class NetConnectionAbs : INetConnection
{
	...
	public NetConnectionAbs(int _channel, ClientInfo _clientInfo, INetworkClient _netClient, string _uniqueId)
	{
		...
		IAntiCheatEncryption antiCheatEncryption;
		if (!isServer)
		{
			IAntiCheatEncryption antiCheatClient = PlatformManager.MultiPlatform.AntiCheatClient;
			antiCheatEncryption = antiCheatClient;
		}
		else
		{
			IAntiCheatEncryption antiCheatClient = PlatformManager.MultiPlatform.AntiCheatServer;
			antiCheatEncryption = antiCheatClient;
		}
		encryptionModule = antiCheatEncryption;
	...
	{
		IAntiCheatEncryption antiCheatEncryption = encryptionModule;
	...
	public virtual bool Encrypt(MemoryStream _stream, long _offset)
	{
		...
		{
			bool result = encryptionModule.EncryptStream(cInfo, _stream, _offset);
			_stream.Position = 0L;
			return result;
		}
		return false;
	...
	public virtual bool Decrypt(bool _bEncrypted, MemoryStream _stream, long _offset)
	{
		...
		bool result = encryptionModule.DecryptStream(cInfo, _stream, _offset);
		_stream.Position = 0L;
		return result;
	}
