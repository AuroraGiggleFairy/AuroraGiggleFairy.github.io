...
public class XUiC_OptionsControls : XUiController
{
	...
	public void createControlsEntries()
	{
		...
		foreach (KeyValuePair<PlayerActionData.ActionTab, SortedDictionary<PlayerActionData.ActionGroup, List<PlayerAction>>> item in sortedDictionary)
		{
			...
			List<XUiC_KeyboardBindingEntry> list2 = new List<XUiC_KeyboardBindingEntry>((tabs.GetTabRect(num).Controller.GetChildById("controlsGrid").ViewComponent as XUiV_Grid).Controller.GetChildrenByType<XUiC_KeyboardBindingEntry>());
			int num2 = 0;
			int num3 = 0;
