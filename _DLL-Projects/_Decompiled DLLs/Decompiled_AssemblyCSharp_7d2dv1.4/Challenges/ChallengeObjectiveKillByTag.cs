...
public class ChallengeObjectiveKillByTag : BaseChallengeObjective
{
	...
	public override string DescriptionText
	{
		get
		{
			...
			return string.Format(Localization.Get("challengeObjectiveKillIn"), Localization.Get(targetName), biome);
		}
	}
	...
	{
		if (entityTags.Test_AnySet(killedEntity.EntityClass.Tags) && (!(biome != "") || !(Owner.Owner.Player.biomeStandingOn.m_sBiomeName != biome)) && (isTwitchSpawn <= -1 || ((isTwitchSpawn != 0 || killedEntity.spawnById == -1) && (isTwitchSpawn != 1 || killedEntity.spawnById != -1))) && (killerHasBuffTag.IsEmpty || killedBy.Buffs.HasBuffByTag(killerHasBuffTag)) && (killedHasBuffTag.IsEmpty || killedEntity.Buffs.HasBuffByTag(killedHasBuffTag)))
		{
			base.Current++;
