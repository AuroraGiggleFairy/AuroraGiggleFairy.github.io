...
public class DecoManager
{
	...
	public IEnumerator OnWorldLoaded(int _worldWidth, int _worldHeight, World _world, IChunkProvider _chunkProvider)
	{
		...
		resourceNoise = new TS_PerlinNoise(_world.Seed);
		resourceNoise.setOctaves(1);
		yield return null;
		...
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer && !fileLoaded)
		{
			...
			if (PlatformOptimizations.FileBackedArrays)
			{
				fileBackedOccupiedMap.CopyDecoChunkRow(chunkEndZ - 1, occupiedMap.GetData());
				occupiedMap = null;
			}
		}
		yield return null;
	...
	public int decorateChunkRandom(DecoChunk _decoChunk, GameRandom rnd)
	{
		...
		for (int i = 0; i < 1000; i++)
		{
			...
			int num9 = 3;
			for (int num10 = biomeAt.m_DistantDecoBlocks.Count - 1; num10 >= 0; num10--)
			{
				BiomeBlockDecoration biomeBlockDecoration = biomeAt.m_DistantDecoBlocks[num10];
	...
	public void AddDecorationAt(World _world, BlockValue _blockValue, Vector3i _blockPos, bool _bForceBlockYPos = false)
	{
		...
		{
			BlockValue block = world.GetBlock(World.toBlock(_blockPos) - new Vector3i(0, 1, 0));
	...
	{
		occupiedMap?.SaveAsTexture(_filename);
	}
