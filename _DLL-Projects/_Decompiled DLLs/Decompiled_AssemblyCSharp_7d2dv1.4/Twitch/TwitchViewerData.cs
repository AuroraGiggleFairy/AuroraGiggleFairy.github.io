...
public class TwitchViewerData
{
		...
		for (int num = SubEntries.Count - 1; num >= 0; num--)
		{
			if (SubEntries[num].Update(deltaTime))
			{
				...
				if (num2 > 0)
				{
					...
					XUiC_ChatOutput.AddMessage(Owner.LocalPlayerXUi, EnumGameMessages.PlainTextLocal, EChatType.Global, message, -1, EMessageSender.Server);
				}
	...
	public void AddGiftSubEntry(string userName, int userID, TwitchSubEventEntry.SubTierTypes tier)
	{
		...
		SubEntries.Add(new GiftSubEntry(userName, userID, tier));
	}
