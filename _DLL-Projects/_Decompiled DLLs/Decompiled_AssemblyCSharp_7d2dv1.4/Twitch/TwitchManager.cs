...
public class TwitchManager
{
	...
	public void StopTwitchIntegration(InitStates initState = InitStates.None)
	{
		...
		if (Authentication != null && (DeviceFlags.Current & (DeviceFlag.StandaloneWindows | DeviceFlag.StandaloneLinux | DeviceFlag.StandaloneOSX)) == DeviceFlags.Current)
		{
			Authentication.StopListener();
		}
		InitState = initState;
			...
			if (!XUi.InGameMenuOpen && AllowActions && ExtensionCheckTime < 0f)
			{
				...
				ExtensionManager.CheckExtensionInstalled([PublicizedFrom(EAccessModifier.Private)] (bool IsInstalled) =>
				{
					...
					else
					{
						...
						XUiC_ChatOutput.AddMessage(uIForPlayer.xui, EnumGameMessages.PlainTextLocal, EChatType.Global, Localization.Get("TwitchChat_ExtensionNotInstalled"), -1, EMessageSender.None);
						StopTwitchIntegration();
			...
			if (LocalPlayer.IsAlive() && CooldownType != CooldownTypes.Time)
			{
				for (int num8 = 0; num8 < QueuedActionEntries.Count; num8++)
				{
					if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsClient)
					{
						...
						{
							SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageGameEventRequest>().Setup(QueuedActionEntries[num8].Action.EventName, QueuedActionEntries[num8].Target.entityId, _isTwitchEvent: true, Vector3.zero, QueuedActionEntries[num8].UserName, "action", AllowCrateSharing));
	...
	public void HandleInGameChatQueue()
	{
		if (inGameChatQueue.Count > 0 && LocalPlayer != null && LocalPlayer.IsAlive())
		{
			...
			XUiC_ChatOutput.AddMessage(LocalPlayerXUi, EnumGameMessages.PlainTextLocal, EChatType.Global, twitchMessageEntry.Message, -1, EMessageSender.Server);
	...
	{
		int num = 22;
		int num2 = GameStats.GetInt(EnumGameStats.DayLightLength);
		if (num2 > 22)
		{
			num = Mathf.Clamp(num2, 0, 23);
		}
		BMCooldownStart = num - CurrentCooldownPreset.BMStartOffset;
		BMCooldownEnd = Mathf.Clamp(num - num2, 0, 23) + CurrentCooldownPreset.BMEndOffset;
	}
	...
	public void PubSub_OnSubscriptionRedeemed(object sender, PubSubSubscriptionRedemptionMessage e)
	{
		...
		if (e.is_gift)
		{
			...
			{
				ViewerData.AddGiftSubEntry(e.user_name, StringParsers.ParseSInt32(e.user_id), subTier);
			}
			return;
		}
		...
		if (num > 0)
		{
			...
			ircClient.SendChannelMessage(string.Format(chatOutput_Subscribed, e.user_name, viewerEntry.CombinedPoints, GetTierName(subTier), num), useQueue: true);
			string msg = string.Format(ingameOutput_Subscribed, e.user_name, GetTierName(subTier), num);
			AddToInGameChatQueue(msg);
		}
		HandleSubEvent(e.user_name, e.cumulative_months, subTier);
	}
	...
	public void EventSub_OnSubscriptionRedeemed(SubscriptionEventBase e)
	{
		if (e.UserName != null)
		{
			...
			ViewerEntry viewerEntry = ViewerData.GetViewerEntry(e.UserName);
			viewerEntry.UserID = StringParsers.ParseSInt32(e.UserId);
			int num = ViewerData.GetSubTierPoints(subTier) * SubPointModifier;
			if (num > 0)
			{
				viewerEntry.SpecialPoints += num;
				ircClient.SendChannelMessage(string.Format(chatOutput_Subscribed, e.UserName, viewerEntry.CombinedPoints, GetTierName(subTier), num), useQueue: true);
				string msg = string.Format(ingameOutput_Subscribed, e.UserName, GetTierName(subTier), num);
				AddToInGameChatQueue(msg);
			}
			if (e is SubscriptionMessageEvent subscriptionMessageEvent)
			{
				HandleSubEvent(subscriptionMessageEvent.UserName, subscriptionMessageEvent.CumulativeMonths, subTier);
			}
			else
			{
				HandleSubEvent(e.UserName, 1, subTier);
			}
		}
	}
	...
	public void EventSub_OnSubGifted(SubscriptionGiftEvent e)
	{
		...
		{
			ViewerData.AddGiftSubEntry(e.UserName, StringParsers.ParseSInt32(e.UserId), TwitchSubEventEntry.GetSubTier(e.Tier));
		}
	}
	...
	public void EventSub_OnBitsRedeemed(BitsUsedEvent e)
	{
		if (e.UserName != null)
		{
			...
			viewerEntry.UserID = StringParsers.ParseSInt32(e.UserId);
			viewerEntry.SpecialPoints += num;
			ircClient.SendChannelMessage(string.Format(chatOutput_DonateBits, e.UserName, viewerEntry.CombinedPoints, e.Bits, num), useQueue: true);
			string msg = string.Format(ingameOutput_DonateBits, e.UserName, e.Bits, num);
			AddToInGameChatQueue(msg);
			HandleBitRedeem(e.UserName, e.Bits, viewerEntry);
		}
	}
	...
	{
		HandleChannelPointsRedeem(e.Reward.Title, e.UserName.ToLower());
	}
	...
	public void SaveViewerData()
	{
		if (dataSaveThreadInfo == null || !ThreadManager.ActiveThreads.ContainsKey("viewerDataSave"))
		{
			...
			dataSaveThreadInfo = ThreadManager.StartThread("viewerDataSave", null, saveViewerDataThreaded, null, System.Threading.ThreadPriority.Normal, new PooledExpandableMemoryStream[2] { pooledExpandableMemoryStream, pooledExpandableMemoryStream2 }, null, _useRealThread: false, _isSilent: true);
		}
	}
	...
	public void SetupAvailableCommandsWithOutput(int lastGameStage, bool displayMessage)
	{
		...
		if (displayMessage && stringBuilder != null && AllowActions)
		{
