...
public class EntityCreationData
{
	...
	public void ApplyToEntity(Entity _e)
	{
		...
		if ((bool)entityAlive)
		{
			if (stats != null)
			{
				stats.CopyBuffChangedDelegates(entityAlive.Stats);
				entityAlive.Stats = stats;
				entityAlive.Stats.Entity = entityAlive;
			}
			else
			{
				entityAlive.Stats.InitWithOldFormatData(health, stamina, sickness, gassiness);
			}
	...
	public void read(PooledBinaryReader _br, bool _bNetworkRead)
	{
		...
		if (b >= 8)
		{
			if (_br.ReadBoolean())
			{
				...
				{
					stats.ReadBeforeEmbeddedVersion(_br);
				}
			}
		}
		else
		{
			health = _br.ReadInt16();
			stamina = _br.ReadInt16();
			if (b >= 7)
			{
				sickness = _br.ReadInt16();
				gassiness = _br.ReadInt16();
			}
		}
