...
public class Entity : MonoBehaviour
{
	...
	public EntityClass EntityClass
	{
		get
		{
			if (!EntityClass.list.ContainsKey(entityClass))
			{
				return null;
			}
			return EntityClass.list[entityClass];
		}
	}
	...
	public float width => scaledExtent.x * 2f;

	public float depth => scaledExtent.z * 2f;

	public float height => scaledExtent.y * 2f;
	...
	public void SetupBounds()
	{
		...
		else if (TryGetComponent<CharacterController>(out component2))
		{
			...
			scaledExtent = new Vector3(radius * localScale2.x, component2.height * localScale2.y * 0.5f, radius * localScale2.z);
			boundingBox = BoundsUtils.BoundsForMinMax(-scaledExtent, scaledExtent);
		}
		else
		{
	...
	public virtual void AddCharacterController()
	{
		...
		if (flag2)
		{
			...
			if ((bool)boxCollider && linkCapsuleSizeToBoundingBox)
			{
	...
	public virtual AttachedToEntitySlotExit FindValidExitPosition(List<AttachedToEntitySlotExit> candidatePositions)
	{
		...
		float radius = m_characterController.GetRadius();
		float num = m_characterController.GetHeight() - radius * 2f;
		...
		for (int i = 0; i < candidatePositions.Count; i++)
		{
			for (float num2 = 0f; num2 < 0.75f; num2 += 0.24f)
			{
				...
				for (float num4 = (0f - radius) * 0.5f; num4 < num; num4 += 0.2f)
				{
					origin.y = vector2.y + num4;
