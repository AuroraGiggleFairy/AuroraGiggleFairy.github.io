...
public class EAIManager
{
	...
	public void Update()
	{
		interestDistance = Mathf.MoveTowards(interestDistance, 10f, 0.004166667f);
	...
	public string MakeDebugName(EntityPlayer player)
	{
		...
		text += string.Format("\nHealth {0} / {1}, PCost {2}, InterestD {3}", entity.Health, entity.GetMaxHealth(), pathCostScale.ToCultureInvariantString(".00"), interestDistance.ToCultureInvariantString("0.000"));
		string text2 = string.Format("\n{0}{1}", entity.IsAlert ? string.Format("Alert {0}, ", ((float)entity.GetAlertTicks() / 20f).ToCultureInvariantString("0.00")) : "", entity.HasInvestigatePosition ? string.Format("Investigate {0}, ", ((float)entity.GetInvestigatePositionTicks() / 20f).ToCultureInvariantString("0.00")) : "");
		if (text2.Length > 1)
		{
			text += text2;
		}
		string text3 = string.Format("\n{0}{1}{2}{3}{4}{5}", moveHelper.IsActive ? string.Format("Move {0} {1},", entity.GetMoveSpeedAggro().ToCultureInvariantString(".00"), entity.GetSpeedModifier().ToCultureInvariantString(".00")) : "", moveHelper.IsBlocked ? string.Format("Blocked {0},", moveHelper.BlockedTime.ToCultureInvariantString("0.00")) : "", moveHelper.CanBreakBlocks ? "CanBrk, " : "", moveHelper.IsUnreachableAbove ? "UnreachAbove, " : "", moveHelper.IsUnreachableSide ? "UnreachSide, " : "", moveHelper.IsUnreachableSideJump ? "UnreachSideJump" : "");
		if (text3.Length > 1)
		{
			text += text3;
		}
		...
		if (entity.IsSleeping)
		{
			...
			string text4 = $"\nLight {player.Stealth.lightLevel.ToCultureInvariantString():0} groan{groan.ToCultureInvariantString():0} wake{wake.ToCultureInvariantString():0}, Noise {entity.noisePlayerVolume.ToCultureInvariantString():0} groan{entity.noiseGroan.ToCultureInvariantString():0} wake{entity.noiseWake.ToCultureInvariantString():0}";
			text += text4;
		}
		else
		{
		...
		return text + entity.MakeDebugNameInfo();
	}
