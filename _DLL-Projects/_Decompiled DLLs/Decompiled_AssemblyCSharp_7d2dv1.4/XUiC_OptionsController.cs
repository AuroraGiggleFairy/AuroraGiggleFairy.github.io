...
public class XUiC_OptionsController : XUiController
{
	...
	public override void Init()
	{
		...
		comboSprintLock = GetChildById("SprintLock").GetChildByType<XUiC_ComboBoxBool>();
	...
	public void BtnDefaults_OnOnPressed(XUiController _sender, int _mouseButton)
	{
		...
		case 0:
			...
			comboSprintLock.Value = (bool)GamePrefs.GetDefault(EnumGamePrefs.OptionsControlsSprintLock);
			if (comboTriggerEffects != null)
			{
		...
		case 1:
			...
			break;
		case 2:
			foreach (PlayerAction item2 in actionTabGroups["inpTabVehicle"])
			{
				item2.ResetBindings();
			}
			break;
	...
	{
		PlayerActionsBase[] obj = new PlayerActionsBase[2]
		{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			{
				if (controllerRebindableAction.UserData is PlayerActionData.ActionUserData actionUserData)
				{
		...
		{
			XUiV_Grid xUiV_Grid = (XUiV_Grid)tabs.GetTabRect(num).Controller.GetChildById("controlsGrid").ViewComponent;
	...
	public void updateOptions()
	{
		...
		comboSprintLock.Value = GamePrefs.GetBool(EnumGamePrefs.OptionsControlsSprintLock);
		if (comboTriggerEffects != null)
		{
	...
	public void applyChanges()
	{
		...
		GamePrefs.Set(EnumGamePrefs.OptionsControlsSprintLock, comboSprintLock.Value);
		if (comboIconStyle != null)
		{
