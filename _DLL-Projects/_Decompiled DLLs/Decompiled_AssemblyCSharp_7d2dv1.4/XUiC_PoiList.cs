...
public class XUiC_PoiList : XUiC_List<XUiC_PoiList.PoiListEntry>
{
	...
	public class PoiListEntry : XUiListEntry
	{
		...
		{
			if (!(_otherEntry is PoiListEntry poiListEntry))
			{
				return 1;
			}
			int num = string.Compare(prefabInstance.name, poiListEntry.prefabInstance.name, StringComparison.OrdinalIgnoreCase);
			if (num != 0)
			{
				return num;
			}
			num = prefabInstance.boundingBoxPosition.x - poiListEntry.prefabInstance.boundingBoxPosition.x;
			if (num != 0)
			{
				return num;
			}
			return prefabInstance.boundingBoxPosition.z - poiListEntry.prefabInstance.boundingBoxPosition.z;
		}
		...
		public override bool GetBindingValue(ref string _value, string _bindingName)
		{
			...
			case "localizedname":
				...
				return true;
			case "coords":
				_value = "(" + ValueDisplayFormatters.WorldPos(prefabInstance.boundingBoxPosition.ToVector3()) + ")";
				return true;
			default:
				return false;
			}
		...
		public override bool MatchesSearch(string _searchString)
		{
			...
			if (!prefab.PrefabName.ContainsCaseInsensitive(_searchString))
			{
				return prefab.LocalizedName.ContainsCaseInsensitive(_searchString);
			}
			return true;
		}
