...
public class XUiC_NewsWindow : XUiController
{
	...
	public override void Init()
	{
		...
		XUiController childById4 = GetChildById("newsImage");
		if (childById4 != null)
		{
			bannerTexture = childById4.ViewComponent as XUiV_Texture;
		}
		selector = GetChildByType<XUiC_ComboBoxFloat>();
		if (selector != null)
		{
			selector.OnValueChanged += Selector_OnValueChanged;
			maxNews = Mathf.RoundToInt((float)selector.Max);
		}
	...
	public void newsUpdated(NewsManager _newsManager)
	{
		...
		for (int i = 0; i < maxNews && i < entries.Count; i++)
		{
	...
	public override void Update(float _dt)
	{
		...
		if (IsDirty)
		{
			...
			{
				NewsManager.NewsEntry currentEntry = CurrentEntry;
				bannerTexture.Texture = currentEntry?.Image;
			}
			IsDirty = false;
		}
	}
