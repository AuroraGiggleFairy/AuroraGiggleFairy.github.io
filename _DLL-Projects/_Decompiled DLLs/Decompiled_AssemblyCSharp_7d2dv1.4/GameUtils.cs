...
public class GameUtils
{
	...
	{
		var (num, num2, _) = WorldTimeToElements(_worldTime);
		if (num == _bmDay)
		{
			if (num2 >= _duskDawnTimes.duskHour)
			{
				return true;
			}
		}
		else if (num > 1 && num == _bmDay + 1 && num2 < _duskDawnTimes.dawnHour)
		{
			return true;
		}
		return false;
	...
	{
		ThreadManager.StartCoroutine(ForceDisconnectRoutine(0.5f));
	}
	...
	public static IEnumerator ForceDisconnectRoutine(float delay)
	{
		...
		if (!GameManager.IsDedicatedServer)
		{
			...
			KickPlayerData kickData = new KickPlayerData
			{
				reason = EKickReason.InternalNetConnectionError
			};
	...
	public static EPlayerHomeType CheckForAnyPlayerHome(World world, Vector3i BoxMin, Vector3i BoxMax)
	{
		...
		int num4 = GamePrefs.GetInt(EnumGamePrefs.LandClaimSize);
		int num5 = num4 / 2;
