		...
		if (quest != null)
		{
			for (int i = 0; i < quest.Rewards.Count; i++)
			{
				if ((quest.Rewards[i] is RewardItem || quest.Rewards[i] is RewardLootItem) && !quest.Rewards[i].isChosenReward)
				{
					...
					int num = Mathf.FloorToInt(EffectManager.GetValue(PassiveEffects.QuestBonusItemReward, null, itemStack.count, player));
					text = ((count != num) ? (text + string.Format(rewardBonusItemFormat, num, itemStack.itemValue.ItemClass.GetLocalizedItemName(), num - count, Localization.Get("bonus")) + ", ") : (text + string.Format(rewardItemFormat, num, itemStack.itemValue.ItemClass.GetLocalizedItemName()) + ", "));
				}
			}
			if (text != "")
			{
			...
			for (int i = 0; i < quest.Rewards.Count; i++)
			{
				if (quest.Rewards[i].isChosenReward == isChosen)
				{
					...
					if (quest.Rewards[i] is RewardItem)
					{
						...
						int num2 = Mathf.FloorToInt(EffectManager.GetValue(PassiveEffects.QuestBonusItemReward, null, rewardItem.Item.count, player));
						text = ((count != num2) ? (text + string.Format(rewardItemBonusFormat, num2, rewardItem.Item.itemValue.ItemClass.GetLocalizedItemName(), num2 - count, Localization.Get("bonus")) + ", ") : (text + string.Format(rewardItemFormat, num2, rewardItem.Item.itemValue.ItemClass.GetLocalizedItemName()) + ", "));
					}
					...
					else if (quest.Rewards[i] is RewardExp)
					{
						...
						int num3 = 0;
						int num4 = Convert.ToInt32(obj.Value) * GameStats.GetInt(EnumGameStats.XPMultiplier) / 100;
						num3 += Mathf.FloorToInt(EffectManager.GetValue(PassiveEffects.PlayerExpGain, null, num4, player, null, QuestTag));
						text = text + string.Format(rewardNumberFormat, num3, Localization.Get("RewardXP_keyword")) + ", ";
					}
					else if (quest.Rewards[i] is RewardSkillPoints)
					{
						int num5 = Convert.ToInt32((quest.Rewards[i] as RewardSkillPoints).Value);
						text = text + string.Format(rewardNumberFormat, num5, Localization.Get("RewardSkillPoints_keyword")) + ", ";
					}
				}
			}
			if (text != "")
			...
			for (int i = 0; i < quest.Rewards.Count; i++)
			{
				if (!quest.Rewards[i].isChosenReward && quest.Rewards[i].isChainReward && quest != quest3)
				{
					if (quest.Rewards[i] is RewardItem)
					{
						RewardItem rewardItem = quest.Rewards[i] as RewardItem;
						int count = rewardItem.Item.count;
						int num = Mathf.FloorToInt(EffectManager.GetValue(PassiveEffects.QuestBonusItemReward, null, rewardItem.Item.count, player));
						text = ((count != num) ? (text + string.Format(rewardItemBonusFormat, num, rewardItem.Item.itemValue.ItemClass.GetLocalizedItemName(), num - count, Localization.Get("bonus")) + ", ") : (text + string.Format(rewardItemFormat, num, rewardItem.Item.itemValue.ItemClass.GetLocalizedItemName()) + ", "));
					}
					else if (quest.Rewards[i] is RewardExp)
					{
						RewardExp obj = quest.Rewards[i] as RewardExp;
