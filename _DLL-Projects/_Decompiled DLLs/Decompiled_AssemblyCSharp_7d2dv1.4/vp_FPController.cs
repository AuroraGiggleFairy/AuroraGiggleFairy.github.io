...
public class vp_FPController : vp_Component
{
	...
	public virtual void UpdateThrottleWalk()
	{
		...
		m_MotorThrottle += ((Player.InputMoveVector.Get().y > 0f) ? Player.InputMoveVector.Get().y : (Player.InputMoveVector.Get().y * MotorBackwardsSpeed)) * (base.Transform.TransformDirection(Vector3.forward * (MotorAcceleration * 0.1f) * m_MotorAirSpeedModifier) * m_SlopeFactor);
		m_MotorThrottle += Player.InputMoveVector.Get().x * MotorSidewaysSpeed * (base.Transform.TransformDirection(Vector3.right * (MotorAcceleration * 0.1f) * m_MotorAirSpeedModifier) * m_SlopeFactor);
	...
	public virtual void UpdateCollisions()
	{
		if (m_GroundHit.transform != null && m_GroundHit.transform != m_LastGroundHit.transform)
		{
			if (m_LastGroundHit.transform == null)
			{
				...
				float num = base.Transform.position.y + Origin.position.y;
				float num2 = m_MaxHeight - num;
				if (!MotorFreeFly && num2 >= 0f)
				{
					float num3 = Math.Abs(m_GravityForce);
					m_FallImpact = (float)Math.Sqrt(Math.Pow(m_MaxHeightInitialFallSpeed, 2.0) + (double)(2f * num3 * num2)) * 0.9f;
				}
				else
				{
