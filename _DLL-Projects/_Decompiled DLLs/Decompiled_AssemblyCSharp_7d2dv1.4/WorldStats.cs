...
public class WorldStats
{
	...
	public WorldStats(MeshStats _blockEntities, MeshStats[] _chunkMeshes)
	{
	...
	public static WorldStats CaptureWorldStats()
	{
		...
		WorldStats result = new WorldStats(new MeshStats(_verts, _tris), array5);
	...
	public static void calcComplexityGameObject(GameObject _go, ref int _verts, ref int _tris, bool _onlyActive = true, Chunk _onChunk = null)
	{
			...
			for (int j = 0; j < lODs.Length; j++)
			{
				...
				foreach (Renderer renderer in renderers)
				{
					...
					if (!mesh.isReadable)
					{
						...
						if (component2 != null && component2.TryGetMeshData(mesh.name, out var data))
						{
		...
		foreach (MeshRenderer meshRenderer2 in componentsInChildren2)
		{
			...
			if (!mesh2.isReadable)
			{
				...
				if (component4 != null && component4.TryGetMeshData(mesh2.name, out var data2))
				{
