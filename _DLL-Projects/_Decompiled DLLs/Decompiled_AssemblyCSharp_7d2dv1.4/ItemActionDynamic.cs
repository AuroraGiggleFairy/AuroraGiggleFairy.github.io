...
public class ItemActionDynamic : ItemAction
{
	...
	public WorldRayHitInfo[] GetExecuteActionGrazeTarget(ItemActionData _actionData, float normalizedClipTime = -1f)
	{
		...
		if (SwingAngle != 0f)
		{
			lookRay.direction = Quaternion.AngleAxis(SwingAngle, (holdingEntity as EntityPlayerLocal).cameraTransform.forward) * lookRay.direction;
		}
		...
		_ = itemActionDynamicData.invData.holdingEntity.GetLookRay().direction;
		lastModelLayer = holdingEntity.GetModelLayer();
		holdingEntity.SetModelLayer(2);
		Entity entity = null;
		Ray lookRay2 = itemActionDynamicData.invData.holdingEntity.GetLookRay();
		lookRay2.origin -= lookRay2.direction * SphereRadius;
		if (Voxel.Raycast(itemActionDynamicData.invData.world, lookRay2, num3, -538750981, (hitmaskOverride == 0) ? 128 : hitmaskOverride, SphereRadius))
		{
		...
		while (num5 < num4)
		{
			...
			_ = lookRay.origin + lookRay.direction * (num3 + num2);
			EntityAlive entityAlive = null;
			Color color = Color.red;
			if (Voxel.Raycast(itemActionDynamicData.invData.world, lookRay, num3, -538750981, (hitmaskOverride == 0) ? 128 : hitmaskOverride, num))
			{
				...
				if (entityAlive != null && entityAlive.IsAlive() && !(entityAlive is EntityVehicle) && holdingEntity is EntityPlayerLocal entityPlayerLocal && (entityPlayerLocal.HitInfo.transform == null || !entityPlayerLocal.HitInfo.transform.IsChildOf(entityAlive.ModelTransform)) && Vector3.Angle(holdingEntity.transform.forward, entityAlive.transform.position - holdingEntity.transform.position) <= 30f)
				{
			...
			if (ShowDebugSwing)
			{
				...
				DebugDisplayHits[DebugDisplayHits.Count - 1].transform.position = lookRay.origin + lookRay.direction * (num2 + 0.2f) + lookRay.direction * ((num3 - (num2 + 0.2f)) * 0.5f);
				DebugDisplayHits[DebugDisplayHits.Count - 1].transform.LookAt(lookRay.origin);
