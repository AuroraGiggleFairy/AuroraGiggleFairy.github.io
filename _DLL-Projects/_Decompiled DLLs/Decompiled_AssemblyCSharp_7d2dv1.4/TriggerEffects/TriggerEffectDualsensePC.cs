...
public static class TriggerEffectDualsensePC
{
	...
	public static void DualsensePCSetEffectToOff(bool triggerSetLeft, bool triggerSetRight)
	{
		...
		for (int i = 1; i < 5; i++)
		{
			if (triggerSetLeft && !ControllerExt.ControllerExtSetTriggerEffectOff(i, 0) && ControllerExt.GetErrorString(out var error))
			{
				Debug.LogWarning($"Controller {i} ControllerExtTriggerEffectApply returned false {error}");
			}
			if (triggerSetRight && !ControllerExt.ControllerExtSetTriggerEffectOff(i, 1) && ControllerExt.GetErrorString(out var error2))
			{
				Debug.LogWarning($"Controller {i} ControllerExtTriggerEffectApply returned false {error2}");
			}
		}
