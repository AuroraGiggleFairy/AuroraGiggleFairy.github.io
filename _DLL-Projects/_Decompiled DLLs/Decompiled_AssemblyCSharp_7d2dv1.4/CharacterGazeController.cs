...
public class CharacterGazeController : MonoBehaviour
{
	...
	public void UpdateLookAtTarget()
	{
		...
		for (int num2 = instances.Count - 1; num2 >= 0; num2--)
		{
			...
			else if (characterGazeController2.enabled && characterGazeController2 != this)
			{
				...
				float num3 = Vector3.Angle(headTransform.forward, to);
		...
		else
		{
			...
			float num4 = Vector3.Angle(headTransform.forward, to2);
			float magnitude2 = to2.magnitude;
			if (num4 < eyeLookAtTargetAngle && magnitude2 <= maxLookAtDistance)
			{
				lookAtCamera = true;
	...
	public void UpdateHeadRotation()
	{
		...
		Vector3 normalized2 = (normalized + headTransform.forward).normalized;
		if (Vector3.Angle(headTransform.forward, normalized) < headLookAtTargetAngle && !isDead)
		{
			if (shouldSnapHeadNextUpdate)
			{
				currentHeadRotation = Quaternion.FromToRotation(headTransform.forward, normalized2);
				shouldSnapHeadNextUpdate = false;
			}
			else
			{
				currentHeadRotation = Quaternion.Slerp(currentHeadRotation, Quaternion.FromToRotation(headTransform.forward, normalized2), headRotationSpeed * Time.deltaTime);
			}
		}
		...
		headTransform.rotation = currentHeadRotation * headTransform.rotation;
	}
