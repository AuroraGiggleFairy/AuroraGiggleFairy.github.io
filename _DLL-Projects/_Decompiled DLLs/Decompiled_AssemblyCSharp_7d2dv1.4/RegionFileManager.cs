...
public class RegionFileManager : WorldChunkCache
{
	...
	public RegionFileManager(string _loadDirectory, string _saveDirectory, int _maxChunksInCache, bool _bSaveOnChunkDrop)
	{
		...
		LoadResetRequests();
		taskInfoThreadSaveChunks = ThreadManager.StartThread("SaveChunks " + (string.IsNullOrEmpty(saveDirectory) ? loadDirectory : saveDirectory), null, thread_SaveChunks, null, System.Threading.ThreadPriority.Normal, null, null, _useRealThread: true);
	}
	...
	public void MainThreadCacheProtectedPositions()
	{
		...
		using (s_UpdateProtectionLevels.Auto())
		{
			lock (ppdPositionCache)
			{
				...
				foreach (EntityCreationData vehicle in VehicleManager.Instance.GetVehicleList())
				{
					ppdPositionCache.vendingMachines.Add(World.worldToBlockPos(vehicle.pos));
				}
	...
	public void UpdateChunkProtectionLevels()
	{
		...
		void EvaluateSingleChunkProtectionLevels()
		{
			...
			lock (ppdPositionCache)
			{
				ProcessPotectedPositionList(ppdPositionCache.bedrolls, halfSize2, ChunkProtectionLevel.Bedroll, 1, ChunkProtectionLevel.NearBedroll);
				ProcessPotectedPositionList(ppdPositionCache.lpBlocks, halfSize, ChunkProtectionLevel.LandClaim, 1, ChunkProtectionLevel.NearLandClaim);
				ProcessPotectedPositionList(ppdPositionCache.offlinePlayers, 0, ChunkProtectionLevel.OfflinePlayer, 1, ChunkProtectionLevel.NearOfflinePlayer);
				ProcessPotectedPositionList(ppdPositionCache.backpacks, 0, ChunkProtectionLevel.DroppedBackpack, 1, ChunkProtectionLevel.NearDroppedBackpack);
				ProcessPotectedPositionList(ppdPositionCache.quests, halfSize, ChunkProtectionLevel.QuestObjective, 1, ChunkProtectionLevel.NearQuestObjective);
				ProcessPotectedPositionList(ppdPositionCache.vendingMachines, halfSize, ChunkProtectionLevel.Trader, 1, ChunkProtectionLevel.Trader);
				ProcessPotectedPositionList(ppdPositionCache.vehicles, halfSize, ChunkProtectionLevel.Vehicle, 1, ChunkProtectionLevel.NearVehicle);
				ProcessPotectedPositionList(ppdPositionCache.supplyCrates, halfSize, ChunkProtectionLevel.SupplyCrate, 1, ChunkProtectionLevel.NearSupplyCrate);
			}
		}
		[PublicizedFrom(EAccessModifier.Private)]
		void ProcessPotectedPositionList(List<Vector3i> centerPositions, int halfSize, ChunkProtectionLevel innerProtectionLevel, int margin, ChunkProtectionLevel marginProtectionLevel)
		{
	...
	public static void RemovePersistentDataForChunks(ICollection<long> _chunks)
	{
		...
		foreach (EntityCreationData vehicle in VehicleManager.Instance.GetVehicleList())
		{
			Vector3i vector3i = World.worldToBlockPos(vehicle.pos);
			...
			foreach (long _chunk2 in _chunks)
			{
				if (_chunk2 == num)
				{
					VehicleManager.Instance.RemoveUnloadedVehicle(vehicle.id);
				}
			}
		}
