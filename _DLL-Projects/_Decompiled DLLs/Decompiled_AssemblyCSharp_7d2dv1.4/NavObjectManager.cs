...
public class NavObjectManager
{
	...
	public NavObject RegisterNavObject(string className, Vector3 trackedPosition, string overrideSprite = "", bool hiddenOnCompass = false, Entity ownerEntity = null)
	{
		for (int i = 0; i < NavObjectList.Count; i++)
		{
			if (NavObjectList[i].IsTrackedPosition(trackedPosition) && NavObjectList[i].NavObjectClass != null && className == NavObjectList[i].NavObjectClass.NavObjectClassName && (ownerEntity == null || ownerEntity == NavObjectList[i].OwnerEntity))
			{
				return NavObjectList[i];
			}
		}
	...
	public NavObject RegisterNavObject(string className, Entity trackedEntity, string overrideSprite = "", bool hiddenOnCompass = false)
	{
		for (int i = 0; i < NavObjectList.Count; i++)
		{
			if (NavObjectList[i].IsTrackedEntity(trackedEntity) && (NavObjectList[i].NavObjectClass == null || NavObjectList[i].NavObjectClass.NavObjectClassName == className))
			{
				...
				NavObjectList[i].RestoreEntityUpdate();
				return NavObjectList[i];
			}
		}
		NavObject navObject = null;
	...
	public void UnRegisterNavObjectByPosition(Vector3 position, string navObjectClass)
	{
		for (int num = NavObjectList.Count - 1; num >= 0; num--)
		{
			...
			if (navObject.IsTrackedPosition(position) && navObject.NavObjectClass != null && NavObjectList[num].NavObjectClass.NavObjectClassName == navObjectClass)
			{
				NavObjectList.RemoveAt(num);
				RemoveNavObjectTag(navObject);
				if (!removedNavObjectPool.Contains(navObject))
				{
					removedNavObjectPool.Add(navObject);
	...
	public void UnRegisterNavObjectByOwnerEntity(Entity ownerEntity, string navObjectClass)
	{
		...
		{
			NavObject navObject = NavObjectList[num];
			if (navObject.OwnerEntity == ownerEntity && navObject.NavObjectClass != null && NavObjectList[num].NavObjectClass.NavObjectClassName == navObjectClass)
			{
				NavObjectList.RemoveAt(num);
				RemoveNavObjectTag(navObject);
				if (!removedNavObjectPool.Contains(navObject))
	...
	public void UnRegisterNavObjectByClass(string className)
	{
		for (int num = NavObjectList.Count - 1; num >= 0; num--)
		{
			...
			if (navObject.NavObjectClass != null && navObject.NavObjectClass.NavObjectClassName == className)
			{
				...
				this.OnNavObjectRemoved?.Invoke(navObject);
			}
		}
	}
