...
public class DayTimeTracker : AbstractDayTimeTracker, INotifiableFilter<MusicActionType, SectionType>, INotifiable<MusicActionType>, IFilter<SectionType>
{
	...
	{
		dawnTime = duskTime - (float)GamePrefs.GetInt(EnumGamePrefs.DayNightLength) * ((float)GamePrefs.GetInt(EnumGamePrefs.DayLightLength) / 24f);
	}

	[PublicizedFrom(EAccessModifier.Protected)]
	public override void SetDuskTime()
	{
		duskTime = 11f / 12f * (float)GamePrefs.GetInt(EnumGamePrefs.DayNightLength);
	}
