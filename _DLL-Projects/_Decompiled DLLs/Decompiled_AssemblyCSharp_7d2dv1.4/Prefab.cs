...
public class Prefab : INeighborBlockCache, IChunkAccess
{
	...
	public class PrefabChunk : IChunk
	{
		...
		public int GetBlockFaceTexture(int _x, int _y, int _z, BlockFace _blockFace)
		{
			...
			return (int)((prefab.GetTexture(_x, _y, _z) >> (int)_blockFace * 6) & 0x3F);
		}
		...
		public long GetTextureFull(int _x, int _y, int _z)
		{
			...
			return prefab.GetTexture(_x, _y, _z);
		}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public Cells<long> textureCells;
	...
	public static PathAbstractions.AbstractedLocation LocationForNewPrefab(string _name, string _prefabsSubfolder = null)
	{
		...
		{
			Mod mod = ModManager.GetMod(launchArgument, _onlyLoaded: true);
			if (mod != null)
			{
				return new PathAbstractions.AbstractedLocation(PathAbstractions.EAbstractedLocationType.Mods, _name, mod.Path + "/Prefabs" + ((_prefabsSubfolder != null) ? ("/" + _prefabsSubfolder) : ""), _prefabsSubfolder, _name, ".tts", _isFolder: false, mod);
			}
			Log.Warning("Argument -newprefabsmod given but mod with name '" + launchArgument + "' not found, ignoring!");
		}
		return new PathAbstractions.AbstractedLocation(PathAbstractions.EAbstractedLocationType.UserDataPath, _name, GameIO.GetUserGameDataDir() + "/LocalPrefabs" + ((_prefabsSubfolder != null) ? ("/" + _prefabsSubfolder) : ""), _prefabsSubfolder, _name, ".tts", _isFolder: false);
	}
	...
	public void InitData()
	{
		...
		textureCells = new Cells<long>(size.y, 0L);
	...
	public bool readBlockData(PooledBinaryReader _br, uint _version, int[] _blockIdMapping, bool _fixChildblocks)
	{
		...
		if (_version >= 10)
		{
			...
			{
				_data.m_Textures[num3] = _br.ReadInt64();
			}
		}
		entities.Clear();
		...
		for (int i = 0; i < size.y; i++)
		{
			for (int num = size.z - 1; num >= 0; num--)
			{
				for (int num2 = size.x - 1; num2 >= 0; num2--)
				{
					...
					long num5 = _data.m_Textures[num3];
					if (num5 != 0L)
					{
						textureCells.AllocCell(num2, i, num).Set(num2, num, num5);
					}
					WaterValue value = _data.m_Water[num3];
					if (value.HasMass())
					{
						waterCells.AllocCell(num2, i, num).Set(num2, num, value);
					}
				}
			}
		}
		...
		for (int i = 0; i < size.x; i++)
		{
			for (int j = 0; j < size.z; j++)
			{
				for (int k = 0; k < size.y; k++)
				{
					...
					long num = GetTexture(i, k, j);
					for (int l = 0; l < 6; l++)
					{
						if (((num >> l * 8) & 0xFF) == _searchPaintId)
						{
							num &= ~(255L << l * 8);
							num |= (long)_replacePaintId << l * 8;
						}
					}
					SetTexture(i, k, j, num);
				}
			}
		}
	}
	...
	public bool hasTexture(long fulltexture, int textureIdx)
	{
		for (int i = 0; i < 6; i++)
		{
			if (((fulltexture >> i * 8) & 0xFF) == textureIdx)
			{
				return true;
			}
		}
	...
	public void CopySleeperVolumes(WorldBase _world, Chunk _chunk, Vector3i _offset)
	{
		...
		for (int i = 0; i < SleeperVolumes.Count; i++)
		{
			...
			for (int j = num3; j <= num4; j++)
			{
				...
				{
					((Chunk)_world.GetChunkSync(j, 0, k))?.AddSleeperVolumeId(num2);
				}
			}
		}
		for (int l = 0; l < TriggerVolumes.Count; l++)
		{
			...
			for (int m = num9; m <= num10; m++)
			{
				...
				{
					((Chunk)_world.GetChunkSync(m, 0, n))?.AddTriggerVolumeId(num8);
				}
			}
		}
		for (int num13 = 0; num13 < WallVolumes.Count; num13++)
		{
			...
			for (int num20 = num16; num20 <= num17; num20++)
			{
				...
				{
					((Chunk)_world.GetChunkSync(num20, 0, num21))?.AddWallVolumeId(num15);
				}
			}
		}
	}
	...
	public void CopyIntoLocal(ChunkCluster _cluster, Vector3i _destinationPos, bool _bOverwriteExistingBlocks, bool _bSetChunkToRegenerate, FastTags<TagGroup.Global> _questTags)
	{
		...
		for (int num7 = 0; num7 < size.z; num7++)
		{
			...
			for (int num11 = 0; num11 < size.x; num11++)
			{
				...
				bool flag3 = false;
				for (int num15 = 0; num15 < size.y; num15++)
				{
					WaterValue water = GetWater(num11, num15, num7);
					BlockValue targetBV = GetBlock(num11, num15, num7);
					...
					bool flag4 = false;
					if (!flag && targetBV.Block.IsSleeperBlock)
					{
						flag4 = true;
						targetBV = BlockValue.Air;
					}
					sbyte b = GetDensity(num11, num15, num7);
					if (!flag)
					{
						...
						{
							ProcessMultiBlock(ref targetBV, chunkSync, new Vector3i(num11, num15, num7), new Vector3i(num14, num16, num10), _questTags, _bOverwriteExistingBlocks);
						}
						else if (BlockPlaceholderMap.Instance.IsReplaceableBlockType(targetBV))
						{
							byte meta = targetBV.meta;
							targetBV = BlockPlaceholderMap.Instance.Replace(targetBV, GameManager.Instance.World.GetGameRandom(), chunkSync, num14, num16, num10, _questTags, _bOverwriteExistingBlocks);
							targetBV.meta = meta;
						}
					}
					if (b == 0)
					...
					if (!flag4)
					{
						long texture = GetTexture(num11, num15, num7);
						chunkSync.SetTextureFull(num14, num16, num10, texture);
					}
					chunkSync.SetBlock(world, num14, num16, num10, targetBV, _notifyAddChange: true, _notifyRemove: true, !_questTags.IsEmpty, _poiOwned: true);
					...
					if (targetBV.Block.shape.IsTerrain() && chunkSync.GetTerrainHeight(num14, num10) < num16)
					{
			...
			while (num4 <= vector3i2.x)
			{
				...
				while (num6 <= vector3i2.z)
				{
					...
					if (!bv.ischild)
					{
						...
						SetTexture(num3, num, num5, _world.GetTexture(num4, num2, num6));
	...
	public Vector3i CopyFromWorldWithEntities(World _world, Vector3i _posStart, Vector3i _posEnd, ICollection<int> _entityIds)
	{
		...
		for (int i = num; i <= num3; i++)
		{
			...
			{
				((Chunk)_world.GetChunkSync(i, 0, j))?.GetEntitiesInBounds(typeof(Entity), bb, list);
			}
		}
		indexedBlockOffsets.Clear();
		triggerData.Clear();
		for (int k = num; k <= num3; k++)
		{
			...
			{
				Chunk chunk = (Chunk)_world.GetChunkSync(k, 0, l);
				if (chunk == null)
				{
					continue;
				}
			...
			for (int j = 0; j < num3; j++)
			{
				...
				for (int k = 0; k < size.y; k++)
				{
					...
					if (targetBV.type == terrainFillerType)
					{
						if (!flag2)
						{
							...
							if (blockValue.isair)
							{
								...
								{
									int terrainHeight2 = _chunk.GetTerrainHeight(num10, num9);
					...
					if (!flag2)
					{
						...
						{
							targetBV = BlockPlaceholderMap.Instance.Replace(targetBV, GameManager.Instance.World.GetGameRandom(), _chunk, num10, num12, num9, FastTags<TagGroup.Global>.none, _bForceOverwriteBlocks);
							block = targetBV.Block;
						}
					}
					...
					if (b == 0)
					{
						b = (sbyte)(block.shape.IsTerrain() ? MarchingCubes.DensityTerrain : ((block.shape.IsSolidCube && k + _prefabTargetPos.y <= num11) ? 1 : MarchingCubes.DensityAir));
					}
					if (yOffset == 0)
					{
						sbyte density2 = _chunk.GetDensity(num10, num12, num9);
						if ((b >= 0 && density2 >= 0 && (density2 != MarchingCubes.DensityAir / 2 || (block.IsTerrainDecoration && !bCopyAirBlocks))) || (b < 0 && density2 < 0 && density2 != MarchingCubes.DensityTerrain / 2))
						{
							b = density2;
						}
					}
					...
					if (flag && !block.shape.IsTerrain() && IsInsidePrefab(blockPos.x, blockPos.y, blockPos.z))
					{
					...
					if (!_bForceOverwriteBlocks && !block6.Block.shape.IsTerrain() && !block6.isair && (block6.ischild || block6.type == targetBV.type))
					{
					...
					if (!flag4)
					{
						long texture = GetTexture(j + num2, k, i + num6);
	...
	public long mirrorTexture(EnumMirrorAlong _axis, BlockShape _shape, int _sourceRot, int _targetRot, long _tex)
	{
		...
		for (int i = 0; i < 6; i++)
		{
			...
			long num2 = (_tex >> 8 * (int)_sourceFace) & 0xFF;
			num2 <<= 8 * (int)_targetFace;
			num |= num2;
		}
		return num;
	}
