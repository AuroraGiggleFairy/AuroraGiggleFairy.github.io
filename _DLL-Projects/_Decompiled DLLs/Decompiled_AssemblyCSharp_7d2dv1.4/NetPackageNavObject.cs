...
public class NetPackageNavObject : NetPackage
{
	...
	public override void ProcessPackage(World _world, GameManager _callbacks)
	{
		...
		if (isAdd)
		{
			NavObject navObject = NavObjectManager.Instance.RegisterNavObject(navObjectClass, position);
			navObject.name = name;
			navObject.usingLocalizationId = usingLocalizationId;
			if (useOverrideColor)
			{
				navObject.UseOverrideColor = true;
				navObject.OverrideColor = overrideColor;
			}
		}
		else if (entityId != -1)
		{
