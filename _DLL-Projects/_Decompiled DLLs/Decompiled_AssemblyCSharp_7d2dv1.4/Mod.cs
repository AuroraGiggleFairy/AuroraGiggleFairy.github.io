...
public class Mod
{
	...
	public EModLoadState LoadAssemblies()
	{
		...
		foreach (string text in array)
		{
			...
			try
			{
				allAssemblies.Add(Assembly.LoadFrom(text));
			}
			catch (Exception e)
			{
			...
			foreach (Assembly allAssembly in allAssemblies)
			{
				try
				{
					...
					foreach (Type type in types)
					{
						...
						{
							Log.Out("[MODS]     Found ModAPI in " + System.IO.Path.GetFileName(allAssembly.Location) + ", creating instance");
							IModApi modApi = (IModApi)Activator.CreateInstance(type);
							try
							{
								modApi.InitMod(this);
								Log.Out("[MODS]     Initialized code in mod '" + Name + "' from DLL '" + System.IO.Path.GetFileName(allAssembly.Location) + "'");
							}
							catch (Exception e)
							{
								Log.Error("[MODS]     Failed initializing ModAPI instance on mod '" + Name + "' from DLL '" + System.IO.Path.GetFileName(allAssembly.Location) + "'");
								Log.Exception(e);
							}
							flag = true;
						}
				...
				{
					Log.Warning("[MODS]     Failed iterating types in DLL " + System.IO.Path.GetFileName(allAssembly.Location));
				}
				catch (Exception e2)
				{
					Log.Error("[MODS]     Failed creating ModAPI instance from DLL " + System.IO.Path.GetFileName(allAssembly.Location));
					Log.Exception(e2);
					return false;
				}
			}
