...
public class GameEventManager
{
	...
	public bool HandleAction(string name, EntityPlayer requester, Entity entity, bool twitchActivated, Vector3 targetPosition, string extraData = "", string tag = "", bool crateShare = false, bool allowRefunds = true, string sequenceLink = "", GameEventActionSequence ownerSeq = null)
	{
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsClient)
		{
			return HandleActionClient(name, entity, twitchActivated, targetPosition, extraData, tag, crateShare, allowRefunds, sequenceLink);
		}
		if (GameEventSequences.ContainsKey(name))
		{
			...
			if (gameEventActionSequence.CanPerform(entity))
			{
				if (gameEventActionSequence.SingleInstance)
				{
					...
					{
						if (ActionSequenceUpdates[i].Name == name)
						{
							return false;
						}
					}
	...
	{
		return HandleActionClient(name, entity, twitchActivated, Vector3.zero, extraData, tag, crateShare, allowRefunds, sequenceLink);
	}
	...
	{
		SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageGameEventRequest>().Setup(name, entity ? entity.entityId : (-1), twitchActivated, targetPosition, extraData, tag, crateShare, allowRefunds, sequenceLink));
		return true;
	}
	...
	public void HandleGameEventSequenceItemForClient(string gameEventID, int index)
	{
		...
		GameEventSequences[gameEventID].HandleClientPerform(player, index);
	}
