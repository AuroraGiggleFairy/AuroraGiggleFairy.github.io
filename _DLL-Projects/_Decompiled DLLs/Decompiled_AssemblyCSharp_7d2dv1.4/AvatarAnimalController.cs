...
public class AvatarAnimalController : AvatarController
{
	...
	public bool rightArmDismembered
	{
		...
		get
		{
			return rightUpperArmDismembered;
		}
	}
	...
	public bool leftArmDismembered
	{
		...
		get
		{
			return leftUpperArmDismembered;
		}
	}
	...
	{
		Transform transform = bipedTransform.FindInChilds("Hips");
		if ((bool)transform)
		{
			head = bipedTransform.FindInChilds("Head");
			leftUpperLeg = bipedTransform.FindInChilds("LeftUpLeg");
			rightUpperLeg = bipedTransform.FindInChilds("RightUpLeg");
			leftUpperArm = bipedTransform.FindInChilds("LeftArm");
			rightUpperArm = bipedTransform.FindInChilds("RightArm");
	...
	public void MakeDismemberedPart(uint bodyDamageFlag, EnumDamageTypes damageType, Transform partT, bool isLeft, bool restoreState)
	{
		...
		{
			Transform transform = SpawnLimbGore(partT, isLeft, dismemberedPartData.prefabPath, restoreState);
			if ((bool)transform)
			{
				ProcDismemberedPart(transform, partT, dismemberedPartData);
			}
		}
	}
	...
	public void ProcDismemberedPart(Transform t, Transform partT, DismemberedPartData part)
	{
		...
		if (!entity.HasAnyTags(DismembermentManager.radiatedTag))
		{
			return;
		}
	...
	public override void SwitchModelAndView(string _modelName, bool _bFPV, bool _bMale)
	{
		...
		{
			AvatarRootMotion avatarRootMotion = bipedTransform.GetComponent<AvatarRootMotion>();
	...
	{
		if (!(forceAttackPlaying > 0f))
		{
			if (!anim.IsInTransition(0))
			{
				return currentBaseState.tagHash == AvatarController.attackHash;
			}
			return false;
		}
		return true;
	...
	{
		forceAttackPlaying -= Time.deltaTime;
		if ((!m_bVisible && (entity == null || !entity.RootMotion || entity.isEntityRemote)) || bipedTransform == null || !bipedTransform.gameObject.activeInHierarchy || anim == null || !anim.enabled)
		{
			return;
		}
		if (anim.avatar == null)
		{
			Log.Error(anim.gameObject.name + " has no avatar on the animation controller!");
		}
		else
		{
			...
			if (isInDeathAnim && currentBaseState.tagHash == AvatarController.deathHash && currentBaseState.normalizedTime >= 1f && !anim.IsInTransition(0))
			{
				isInDeathAnim = false;
