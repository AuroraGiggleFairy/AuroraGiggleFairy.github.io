...
public class Path
{
	...
	public Path(WorldBuilder _worldBuilder, Vector2i _startPosition, Vector2i _endPosition, int _lanes, bool _isCountryRoad, bool _validityTest = false)
	{
		...
		validityTestOnly = _validityTest;
		CreatePath();
	}
	...
	public Path(WorldBuilder _worldBuilder, bool _isCountryRoad = false, float _radius = 8f, bool _validityTest = false)
	{
		worldBuilder = _worldBuilder;
		isCountryRoad = _isCountryRoad;
		FinalPathPoints = new List<Vector2>();
		radius = _radius;
		validityTestOnly = _validityTest;
	}
	...
	{
		int num = (isCountryRoad ? 2 : 4);
		for (int i = 0; i < num; i++)
		{
			roundOffCorners();
		}
		float num2 = worldBuilder.WorldSize;
		float num3 = 0f;
		for (int j = 0; j < FinalPathPoints.Count; j++)
		{
			...
			if (j > 0)
			{
				num3 += Vector2.Distance(FinalPathPoints[j - 1], FinalPathPoints[j]);
			}
			pathPoints3d.Add(new Vector3(x, worldBuilder.GetHeight((int)x, (int)y), y));
		}
		Cost = Mathf.RoundToInt(num3);
		float[] heights = new float[pathPoints3d.Count];
		if (isCountryRoad)
		{
			for (int k = 0; k < 4; k++)
			{
		...
		{
			float num4 = 0f;
			for (int l = 0; l < pathPoints3d.Count; l++)
			{
				num4 += pathPoints3d[l].y;
			}
			num4 /= (float)pathPoints3d.Count;
			num4 += 8f;
			for (int m = 0; m < pathPoints3d.Count; m++)
			{
				Vector3 value = pathPoints3d[m];
				if (value.y > num4)
				{
					value.y = num4 * 0.3f + value.y * 0.7f;
		...
		for (int num5 = 0; num5 < pathPoints3d.Count; num5++)
		{
			zero.x = (int)(pathPoints3d[num5].x + 0.5f);
			zero.y = (int)(pathPoints3d[num5].z + 0.5f);
			FinalPathPoints.Add(zero);
		}
	}
	...
	public void DrawPathToRoadIds(byte[] ids)
	{
		...
		for (int i = 0; i < FinalPathPoints.Count - 1; i++)
		{
			...
			for (int j = v3; j < v4; j++)
			{
				...
				for (int k = v; k < v2; k++)
				{
					point.x = k;
					Vector2 pointOnLine;
					float num6 = GetPointToLineDistance2(point, FinalPathPoints[i], FinalPathPoints[i + 1], out pointOnLine);
					float num7;
					if (num6 < num4)
					{
					...
					else
					{
						...
						if (i > 0)
						{
							...
							if (num6 >= num9 || GetPointToLineDistance2(point, FinalPathPoints[i - 1], FinalPathPoints[i], out var _) < num4)
							{
								continue;
							}
						}
					...
					if (!isRiver)
					{
						v5 = Utils.FastMax(worldBuilder.GetWater(k, j), worldBuilder.WaterHeight) + 1;
					}
	...
	{
		if (StartPosition.x < 8 || StartPosition.y < 8)
		{
			Log.Error("Start position oob");
			return;
		}
		if (EndPosition.x < 8 || EndPosition.y < 8)
		{
			Log.Error("End position oob");
			return;
		}
		List<Vector2i> path = worldBuilder.PathingUtils.GetPath(this, StartPosition, EndPosition);
		...
		FinalPathPoints = new List<Vector2>(16);
		Vector2 item = new Vector2(EndPosition.x, EndPosition.y);
		FinalPathPoints.Add(item);
		for (int i = 1; i < path.Count; i++)
		{
			if (!(distanceSqr(path[i], path[i - 1]) < 64f) && !(distanceSqr(path[i], StartPosition) < 64f) && !(distanceSqr(path[i], EndPosition) < 64f))
			{
				item.x = path[i].x;
				item.y = path[i].y;
				FinalPathPoints.Add(item);
	...
	public void commitPathingMapData()
	{
		...
		for (int i = 0; i < FinalPathPoints.Count - 1; i++)
		{
			...
			PathTile pathTileWorld = getPathTileWorld(vector2i);
			if (pathTileWorld != null)
			{
				pathTileWorld.TileState = (isCountryRoad ? PathTile.PathTileStates.Country : PathTile.PathTileStates.Highway);
				pathTileWorld.PathRadius = (byte)radius;
			}
			if (!isCountryRoad)
			{
	...
	{
		return getPathTileWorld(worldPosition.x, worldPosition.y);
	}
	...
	public PathTile getPathTileWorld(int x, int y)
	{
		x /= 10;
		y /= 10;
		if (x < 0 || x >= worldBuilder.PathingGrid.GetLength(0))
		{
			return null;
		}
		if (y < 0 || y >= worldBuilder.PathingGrid.GetLength(1))
		{
			return null;
		}
		if (worldBuilder.PathingGrid[x, y] == null)
		{
			worldBuilder.PathingGrid[x, y] = new PathTile();
		}
		return worldBuilder.PathingGrid[x, y];
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void roundOffCorners()
	{
		for (int i = 2; i < FinalPathPoints.Count - 2; i++)
		{
			Vector2 normalized = (FinalPathPoints[i] - FinalPathPoints[i - 1]).normalized;
			Vector2 normalized2 = (FinalPathPoints[i + 1] - FinalPathPoints[i]).normalized;
			if (normalized != normalized2)
			{
				FinalPathPoints[i] = (FinalPathPoints[i] + FinalPathPoints[i - 1] * 0.5f + FinalPathPoints[i + 1] * 0.5f) / 2f;
			}
		}
	}
	...
	{
		for (int i = 0; i < pathPoints3d.Count; i++)
		{
			heights[i] = pathPoints3d[i].y;
		}
		for (int j = 1; j < pathPoints3d.Count - 1; j++)
		{
