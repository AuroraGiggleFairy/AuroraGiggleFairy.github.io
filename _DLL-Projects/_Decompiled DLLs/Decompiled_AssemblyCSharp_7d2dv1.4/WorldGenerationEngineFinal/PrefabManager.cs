...
public class PrefabManager
{
	...
	public PrefabData GetPrefabWithDistrict(District _district, FastTags<TagGroup.Poi> _markerTags, Vector2i minSize, Vector2i maxSize, Vector2i center, float densityPointsLeft, float _distanceScale)
	{
		...
		for (int i = 0; i < prefabManagerData.prefabDataList.Count; i++)
		{
			...
			if (prefabData.DensityScore > densityPointsLeft || prefabData.Tags.Test_AnySet(prefabManagerData.PartsAndTilesTags) || (flag && !prefabData.Tags.Test_AllSet(_district.tag)) || (flag2 && !prefabData.Tags.Test_AnySet(_markerTags)) || !isSizeValid(prefabData, minSize, maxSize))
			{
				continue;
			}
	...
	public static void Shuffle<T>(int seed, ref List<T> list)
	{
		...
		{
			num--;
			int index = gameRandom.RandomRange(0, num);
			T value = list[index];
			list[index] = list[num];
			list[num] = value;
		}
