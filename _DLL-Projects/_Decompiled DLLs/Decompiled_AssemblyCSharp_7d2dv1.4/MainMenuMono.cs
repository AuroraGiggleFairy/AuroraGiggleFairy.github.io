...
public class MainMenuMono : MonoBehaviour
{
	...
	public bool CheckLogin()
	{
		...
		else
		{
			...
			{
				throw new Exception($"Could not find Login window for platform: {DeviceFlags.Current}");
			}
	...
	public void Update()
	{
		...
		if (!GameManager.Instance.bStaticDataLoaded)
		{
			...
			nguiWindowManager.SetLabelText(EnumNGUIWindow.Loading, loadingText + " " + GameManager.Instance.CurrentLoadAction + "...", _toUpper: false);
			return;
		}
		if (!GameManager.Instance.gameStateManager.IsGameStarted() && nguiWindowManager.IsShowing(EnumNGUIWindow.Loading))
		{
		...
		if (IsQuickContinue)
		{
			...
			if (networkConnectionError != NetworkConnectionError.NoError)
			{
				((XUiC_MessageBoxWindowGroup)((XUiWindowGroup)windowManager.GetWindow(XUiC_MessageBoxWindowGroup.ID)).Controller).ShowNetworkError(networkConnectionError);
			}
		}
		else
		{
