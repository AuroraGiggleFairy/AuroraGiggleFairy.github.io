...
public static class BlockUtilityNavIcon
{
	...
	public static void UpdateNavIcon(bool _shouldShow, Vector3i _blockPos)
	{
		...
		if (!flag && _shouldShow && scBlockTypes.Contains(block.Block.GetBlockName()) && primaryPlayer != null)
		{
			PrefabInstance prefabAtPosition = GameManager.Instance.GetDynamicPrefabDecorator().GetPrefabAtPosition(_blockPos);
			GameManager.Instance.GetDynamicPrefabDecorator().GetPrefabAtPosition(primaryPlayer.position);
			bool flag2 = false;
			foreach (Quest quest in primaryPlayer.QuestJournal.quests)
			{
				...
				if (!flag3 && quest.RallyMarkerActivated && quest.CurrentState == Quest.QuestState.InProgress && quest.GetPositionData(out var pos, Quest.PositionDataTypes.POIPosition) && GameManager.Instance.GetDynamicPrefabDecorator().GetPrefabsFromWorldPosInside(pos, quest.QuestTags).Contains(prefabAtPosition))
				{
					flag2 = true;
					break;
				}
