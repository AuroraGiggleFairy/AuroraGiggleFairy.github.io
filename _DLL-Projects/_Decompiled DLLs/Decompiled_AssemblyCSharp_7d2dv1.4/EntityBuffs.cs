...
public class EntityBuffs
{
	...
	public void ModifyValue(PassiveEffects _effect, ref float _value, ref float _perc_val, FastTags<TagGroup.Global> _tags)
	{
		for (int i = 0; i < ActiveBuffs.Count; i++)
		{
			...
			if (buffClass != null && !ActiveBuffs[i].Paused)
			{
	...
	public void FireEvent(MinEventTypes _eventType, MinEventParams _params)
	{
		for (int i = 0; i < ActiveBuffs.Count; i++)
		{
			BuffClass buffClass = ActiveBuffs[i].BuffClass;
			if (buffClass != null && !ActiveBuffs[i].Paused)
			{
	...
	public BuffStatus AddBuff(string _name, Vector3i _instigatorPos, int _instigatorId = -1, bool _netSync = true, bool _fromElectrical = false, float _buffDuration = -1f)
	{
		...
		for (int i = 0; i < ActiveBuffs.Count; i++)
		{
			...
			if (_netSync)
			{
				int senderId = GameManager.Instance.persistentLocalPlayer?.EntityId ?? (-1);
				AddBuffNetwork(senderId, _name, _buffDuration, _instigatorPos, _instigatorId);
			}
		...
		if (_netSync)
		{
			int senderId2 = GameManager.Instance.persistentLocalPlayer?.EntityId ?? (-1);
			AddBuffNetwork(senderId2, _name, _buffDuration, _instigatorPos, _instigatorId);
		}
		return BuffStatus.Added;
	}
	...
	public void AddBuffNetwork(int _senderId, string _name, float _duration, Vector3i _instigatorPos, int _instigatorId = -1)
	{
		...
		{
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendPackage(NetPackageManager.GetPackage<NetPackageAddRemoveBuff>().Setup(parent.entityId, _senderId, _name, _duration, _adding: true, _instigatorId, _instigatorPos), _onlyClientsAttachedToAnEntity: false, -1, -1, parent.entityId);
		}
		else
		{
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageAddRemoveBuff>().Setup(parent.entityId, _senderId, _name, _duration, _adding: true, _instigatorId, _instigatorPos));
		}
	}
	...
	public void RemoveBuffNetwork(string _name)
	{
		...
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendPackage(NetPackageManager.GetPackage<NetPackageAddRemoveBuff>().Setup(parent.entityId, senderId, _name, -1f, _adding: false, -1, Vector3i.zero), _onlyClientsAttachedToAnEntity: false, -1, -1, parent.entityId);
		}
		else
		{
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageAddRemoveBuff>().Setup(parent.entityId, senderId, _name, -1f, _adding: false, -1, Vector3i.zero));
		}
	}
	...
	{
		if (!CVars.TryGetValue(_name, out var value) || value != _value)
		{
			CVars[_name] = _value;
			if (_netSync)
			{
				SetCustomVarNetwork(_name, _value);
			}
		}
	}
	...
	{
		if (parent.isEntityRemote && _name[0] != '.' && _name[0] != '_')
		{
			if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
			{
				SingletonMonoBehaviour<ConnectionManager>.Instance.SendPackage(NetPackageManager.GetPackage<NetPackageModifyCVar>().Setup(parent, _name, _value));
			}
			else
			{
				SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageModifyCVar>().Setup(parent, _name, _value));
			}
		}
	}
	...
	public void IncrementCustomVar(string _name, float _amount)
	{
		...
		SetCustomVar(_name, num + _amount);
	}
	...
	public bool HasImmunity(BuffClass _buffClass)
	{
		...
		return parent.rand.RandomFloat <= Mathf.Clamp01(EffectManager.GetValue(PassiveEffects.BuffResistance, null, 0f, parent, null, _buffClass.NameTag));
	}
	...
	public void Read(BinaryReader _br)
	{
		...
		else
		{
			...
			if (num4 > 0)
			{
				...
				{
					CVars[_br.ReadString()] = _br.ReadSingle();
				}
			}
		}
		AddCustomVar("_difficulty", GameStats.GetInt(EnumGameStats.GameDifficulty));
