...
public class XUiC_Radial : XUiController
{
	...
	public override void OnOpen()
	{
		...
		if (!showingCallouts)
		{
			...
			base.xui.calloutWindow.AddCallout(radialButtonIsLeftSide(base.xui.playerUI.playerInput) ? UIUtils.ButtonIcon.RightStick : UIUtils.ButtonIcon.LeftStick, "igcoRadialHighlight", XUiC_GamepadCalloutWindow.CalloutType.Menu);
			base.xui.calloutWindow.AddCallout(selectionControllerButton, "igcoRadialSelect", XUiC_GamepadCalloutWindow.CalloutType.Menu);
	...
	{
		if (!_actionSet.Activate.IsPressed && !_actionSet.PermanentActions.Activate.IsPressed && !_actionSet.Reload.IsPressed && !_actionSet.PermanentActions.Reload.IsPressed && !_actionSet.ToggleFlashlight.IsPressed && !_actionSet.PermanentActions.ToggleFlashlight.IsPressed && !_actionSet.Inventory.IsPressed && !_actionSet.VehicleActions.Inventory.IsPressed && !_actionSet.PermanentActions.Inventory.IsPressed && !_actionSet.Swap.IsPressed && !_actionSet.PermanentActions.Swap.IsPressed)
		{
			if (!_actionSet.InventorySlotLeft.IsPressed)
			{
				return _actionSet.InventorySlotRight.IsPressed;
			}
			return true;
		}
		return true;
	...
	public void SetCurrentBlockData(WorldBase _world, Vector3i _blockPos, int _cIdx, BlockValue _blockValue, EntityPlayerLocal _entityFocusing)
	{
		...
		SetCommonData(UIUtils.GetButtonIconForAction(base.xui.playerUI.playerInput.Activate), handleBlockCommand, new RadialContextBlock(_blockPos, _cIdx, _blockValue, _entityFocusing, blockActivationCommands));
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void handleBlockCommand(XUiC_Radial _sender, int _commandIndex, RadialContextAbs _context)
	{
		if (_context is RadialContextBlock { BlockValue: var blockValue } radialContextBlock)
		{
			blockValue.Block.OnBlockActivated(radialContextBlock.Commands[_commandIndex].text, GameManager.Instance.World, radialContextBlock.ClusterIdx, radialContextBlock.BlockPos, radialContextBlock.BlockValue, radialContextBlock.EntityFocusing);
		}
	}
	...
	public void SetCurrentEntityData(WorldBase _world, Entity _entity, ITileEntity _te, EntityAlive _entityFocusing)
	{
		...
		SetCommonData(UIUtils.GetButtonIconForAction(base.xui.playerUI.playerInput.Activate), handleEntityCommand, new RadialContextEntity(vector3i, _entityFocusing, _entity));
	}
	...
	public void SetupToolbeltMenu(int _direction)
	{
		...
		for (int i = 0; i < Mathf.Min(slots.Length, menuItem.Length); i++)
		{
			if (i != entityPlayer.inventory.DUMMY_SLOT_IDX)
			{
				...
				{
					CreateRadialEntry(i, slots[i].itemValue.ItemClass.GetIconName(), slots[i].itemValue.ItemClass.GetIconTint(slots[i].itemValue), "ItemIconAtlas", slots[i].itemValue.ItemClass.CanStack() ? slots[i].count.ToString() : "", slots[i].itemValue.ItemClass.GetLocalizedItemName(), _highlighted: true);
				}
			}
		}
