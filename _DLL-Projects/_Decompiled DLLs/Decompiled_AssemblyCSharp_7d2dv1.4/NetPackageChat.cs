...
public class NetPackageChat : NetPackage
{
	...
	{
		chatType = (EChatType)_br.ReadInt32();
		senderEntityId = _br.ReadInt32();
		msg = _br.ReadString();
		msgSender = (EMessageSender)_br.ReadInt32();
	...
	{
		base.write(_bw);
		_bw.Write((int)chatType);
		_bw.Write(senderEntityId);
		_bw.Write(msg);
		_bw.Write((int)msgSender);
	...
	public override void ProcessPackage(World _world, GameManager _callbacks)
	{
		if (_world != null)
		{
			if (!_world.IsRemote())
			{
				GameManager.Instance.ChatMessageServer(base.Sender, chatType, senderEntityId, msg, recipientEntityIds, msgSender);
			}
			else
			{
				GameManager.Instance.ChatMessageClient(chatType, senderEntityId, msg, null, msgSender);
			}
		}
	}
