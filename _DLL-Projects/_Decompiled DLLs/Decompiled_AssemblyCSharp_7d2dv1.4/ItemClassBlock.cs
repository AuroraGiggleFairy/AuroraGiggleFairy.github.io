...
public class ItemClassBlock : ItemClass
{
	...
	{
		return CreateMesh(_go, _world, _blockValue, _vertices, _position, _parent, _purpose, _textureFull);
	}
	...
	{
		return CreateMesh(null, _world, GetBlockValueFromItemValue(_itemValue), null, _position, _parent, _purpose, _textureFull);
	}
	...
	public static Transform CreateMesh(GameObject _go, World _world, BlockValue _blockValue, Vector3[] _vertices, Vector3 _worldPos, Transform _parent, BlockShape.MeshPurpose _purpose, long _textureFull = 0L)
	{
		...
		Transform transform2 = CreateMeshOfType(_go, _world, _blockValue, _vertices, _worldPos, _parent, _purpose, _textureFull, meshIndex);
		if (meshIndex == 0 && (_purpose == BlockShape.MeshPurpose.Preview || _purpose == BlockShape.MeshPurpose.Local))
		{
			Transform transform3 = CreateMeshOfType(_go, _world, _blockValue, _vertices, _worldPos, _parent, _purpose, _textureFull, 2);
			if ((bool)transform3)
			{
				if ((bool)transform2)
				{
	...
	public static Transform CreateMeshOfType(GameObject _go, World _world, BlockValue _blockValue, Vector3[] _vertices, Vector3 _worldPos, Transform _parent, BlockShape.MeshPurpose _purpose, long _textureFull, int _meshIndex)
	{
		...
		_blockValue.Block.shape.renderFull(vector3i, _blockValue, renderOffsetV, _vertices, new LightingAround(sunLight, blockLight, 0), _textureFull, array, _purpose);
