...
public class LootContainer
{
	...
	public static ItemStack GetRewardItem(string lootGroup, float questDifficulty)
	{
		...
		int slotsLeft = 1;
		SpawnItemsFromGroup(GameManager.Instance.lootManager.Random, lootGroups[lootGroup], 1, 1f, list, ref slotsLeft, questDifficulty, 0f, lootGroups[lootGroup].lootQualityTemplate, null, FastTags<TagGroup.Global>.none, uniqueItems: true, ignoreLootProb: true, _forceStacking: false);
		if (list.Count == 0)
		{
			return ItemStack.Empty.Clone();
		}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static void SpawnItem(GameRandom random, LootEntry template, ItemValue lootItemValue, int countToSpawn, List<ItemStack> spawnedItems, ref int slotsLeft, float gameStage, string lootQualityTemplate, EntityPlayer player, FastTags<TagGroup.Global> containerTags, bool _forceStacking)
	{
	...
	public static void SpawnAllItemsFromList(GameRandom random, List<LootEntry> itemSet, float abundance, List<ItemStack> spawnedItems, ref int slotsLeft, float playerLevelPercentage, float rareLootChance, string lootQualityTemplate, EntityPlayer player, FastTags<TagGroup.Global> containerTags, bool uniqueItems, bool ignoreLootProb, bool _forceStacking)
	{
		for (int i = 0; i < itemSet.Count; i++)
		{
			...
			if (lootEntry.group != null)
			{
				...
				{
					SpawnItemsFromGroup(random, lootEntry.group, num, abundance, spawnedItems, ref slotsLeft, playerLevelPercentage, rareLootChance, lootQualityTemplate, player, containerTags, uniqueItems, ignoreLootProb, _forceStacking);
				}
			}
			else
			{
				SpawnItem(random, lootEntry, lootEntry.item.itemValue, num, spawnedItems, ref slotsLeft, playerLevelPercentage, lootQualityTemplate, player, containerTags, _forceStacking);
			}
		}
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public static void SpawnItemsFromGroup(GameRandom random, LootGroup group, int numToSpawn, float abundance, List<ItemStack> spawnedItems, ref int slotsLeft, float gameStage, float rareLootChance, string lootQualityTemplate, EntityPlayer player, FastTags<TagGroup.Global> containerTags, bool uniqueItems, bool ignoreLootProb, bool _forceStacking)
	{
		for (int i = 0; i < numToSpawn; i++)
		{
			...
			SpawnLootItemsFromList(random, group.items, RandomSpawnCount(random, group.minCount, group.maxCount, 1f), abundance, spawnedItems, ref slotsLeft, gameStage, rareLootChance, lootQualityTemplate, player, containerTags, uniqueItems, ignoreLootProb, _forceStacking);
		}
	}

	public static void SpawnLootItemsFromList(GameRandom random, List<LootEntry> itemSet, int numToSpawn, float abundance, List<ItemStack> spawnedItems, ref int slotsLeft, float lootStage, float rareLootChance, string lootQualityTemplate, EntityPlayer player, FastTags<TagGroup.Global> containerTags, bool uniqueItems, bool ignoreLootProb, bool _forceStacking)
	{
		if (numToSpawn < 1)
		{
			if (numToSpawn == -1)
			{
				SpawnAllItemsFromList(random, itemSet, abundance, spawnedItems, ref slotsLeft, lootStage, rareLootChance, lootQualityTemplate, player, containerTags, uniqueItems, ignoreLootProb, _forceStacking);
			}
			return;
		}
		float num = 0f;
		...
		for (int j = 0; j < numToSpawn; j++)
		{
			...
			for (int k = 0; k < itemSet.Count; k++)
			{
				...
				if (lootEntry2.group != null)
				{
					...
					{
						SpawnItemsFromGroup(random, lootEntry2.group, num3, abundance, spawnedItems, ref slotsLeft, lootStage, rareLootChance, lootQualityTemplate, player, containerTags, uniqueItems, ignoreLootProb, _forceStacking);
					}
				}
				else
				{
					SpawnItem(random, lootEntry2, lootEntry2.item.itemValue, num3, spawnedItems, ref slotsLeft, lootStage, lootQualityTemplate, player, containerTags, _forceStacking);
				}
				break;
			}
		}
	...
	public IList<ItemStack> Spawn(GameRandom random, int _maxItems, float playerLevelPercentage, float rareLootChance, EntityPlayer player, FastTags<TagGroup.Global> containerTags, bool uniqueItems, bool ignoreLootProb)
	{
		...
		SpawnLootItemsFromList(random, itemsToSpawn, numToSpawn, abundance, list, ref _maxItems, playerLevelPercentage, rareLootChance, lootQualityTemplate, player, containerTags, uniqueItems, ignoreLootProb, _forceStacking: false);
		return list;
	}
}
