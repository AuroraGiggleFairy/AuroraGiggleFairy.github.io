...
public class XUiC_MapArea : XUiController
{
	...
	{
		base.xui.LoadData("Textures/UI/fow_chunkMask", [PublicizedFrom(EAccessModifier.Private)] (Texture2D o) =>
		{
	...
	public void RefreshVehiclePositionWaypoint(EntityVehicle _vehicle, bool _unloaded)
	{
		...
		waypoint.lastKnownVehiclePositionEntityId = _vehicle.entityId;
		EntityPlayer entityPlayer = base.xui.playerUI.entityPlayer;
		if (!entityPlayer.Waypoints.ContainsLastKnownPositionWaypointForVehicle(_vehicle))
		{
	...
	{
		if (base.xui.playerUI.entityPlayer.Waypoints.TryRemoveVehicleLastKnownWaypoint(_vehicle))
		{
			((XUiC_MapWaypointList)base.Parent.GetChildById("waypointList")).UpdateWaypointsList();
		}
	}
