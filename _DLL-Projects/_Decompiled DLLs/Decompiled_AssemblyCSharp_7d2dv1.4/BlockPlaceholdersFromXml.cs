...
public class BlockPlaceholdersFromXml
{
	public static IEnumerator Load(XmlFile _xmlFile)
	{
		...
		foreach (XElement item in root.Elements())
		{
			...
			BlockValue placeholderBlockValue = ItemClass.GetItem(attribute).ToBlockValue();
			foreach (XElement item2 in item.Elements("block"))
			{
				...
				{
					BlockPlaceholderMap.Instance.AddQuestResetPlaceholder(placeholderBlockValue, targetValue, _result, biome, randomRotation, questTags);
				}
				else
				{
					BlockPlaceholderMap.Instance.AddPlaceholder(placeholderBlockValue, targetValue, _result, biome, randomRotation);
				}
			}
		}
		yield break;
