...
public class PowerManager
{
	...
	public void SavePowerManager()
	{
		if (dataSaveThreadInfo == null || !ThreadManager.ActiveThreads.ContainsKey("powerDataSave"))
		{
			...
			dataSaveThreadInfo = ThreadManager.StartThread("powerDataSave", null, savePowerDataThreaded, null, System.Threading.ThreadPriority.Normal, pooledExpandableMemoryStream, null, _useRealThread: false, _isSilent: true);
		}
	}
	...
	{
		int num = 0;
		Span<Vector3i> span = stackalloc Vector3i[PowerItemDictionary.Count];
		foreach (KeyValuePair<Vector3i, PowerItem> item in PowerItemDictionary)
		{
			long num2 = WorldChunkCache.MakeChunkKey(World.toChunkXZ(item.Key.x), World.toChunkXZ(item.Key.z));
		...
		for (int i = 0; i < num; i++)
		{
			if (PowerItemDictionary.TryGetValue(span[i], out var value))
			{
				RemovePowerNode(value);
			}
		}
	}
