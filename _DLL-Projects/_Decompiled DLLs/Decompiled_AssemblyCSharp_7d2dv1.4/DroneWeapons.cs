...
public class DroneWeapons
{
	...
	public class HealBeamWeapon(EntityAlive _entity) : Weapon(_entity)
	{
		...
		[PublicizedFrom(EAccessModifier.Private)]
		public string[] _supportedHealingItems = new string[2] { "bandage", "medicalFirstAidKit" };
		...
		public override void Fire(EntityAlive _target)
		{
			...
			ItemStack healingItemStack = getHealingItemStack();
			...
			ItemActionData actionData = entity.inventory.holdingItemData.actionData[1];
			itemAction?.ExecuteAction(actionData, _bReleased: true);
		...
		{
			if (!_hasItem(_healTypeToString(HealItemType.Bandage)))
			{
				return _hasItem(_healTypeToString(HealItemType.FirstAidKit));
			}
			return true;
		}
		...
		{
			if (_target.IsAlive() && !_target.Buffs.HasBuff("buffHealHealth") && _target.Health < _target.GetMaxHealth())
			{
				return true;
			}
			return false;
		...
		public bool targetNeedsHealing(EntityAlive _target)
		{
			...
			if (targetCanBeHealed(_target) && ((num == modifiedMax && (float)_target.Health < num - HealDamageThreshold) || (float)_target.Health < modifiedMax * 0.67f))
			{
				return true;
			}
			return false;
		...
		[PublicizedFrom(EAccessModifier.Private)]
		public string _healTypeToString(HealItemType healType)
		{
			return _supportedHealingItems[(int)healType];
		}

		[PublicizedFrom(EAccessModifier.Private)]
		public ItemStack getHealingItemStack()
		{
			HealItemType healType = HealItemType.Bandage;
			bool num = _hasItem(_healTypeToString(HealItemType.Bandage));
			bool flag = _hasItem(_healTypeToString(HealItemType.FirstAidKit));
			if (!num && flag)
			{
				healType = HealItemType.FirstAidKit;
			}
			...
			{
				if (array[i] != null && array[i].itemValue != null && array[i].itemValue.ItemClass != null && array[i].itemValue.ItemClass.HasAnyTags(healingItemTags) && array[i].count > 0 && _isItem(array[i].itemValue, _healTypeToString(healType)))
				{
		...
		public bool _hasItem(string itemGroupOrName)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				if (array[i] != null && array[i].itemValue != null && array[i].itemValue.ItemClass != null && array[i].itemValue.ItemClass.HasAnyTags(healingItemTags) && array[i].itemValue.ItemClass.Name.ContainsCaseInsensitive(itemGroupOrName))
				{
					return true;
				}
			}
		...
		{
			if (iv.ItemClass.Name.ContainsCaseInsensitive(itemGroupOrName))
			{
				return true;
			}
			return false;
