...
public class XUiC_SaveDirtyPrefab : XUiController
{
	...
	public void CloseWith(ESelectedAction _action)
	{
		...
		if (_action == ESelectedAction.Save)
		{
			...
			{
				GameManager.ShowTooltip(GameManager.Instance.World.GetLocalPlayers()[0], string.Format(Localization.Get("xuiPrefabsPrefabSaved"), PrefabEditModeManager.Instance.LoadedPrefab.Name));
			}
			else
			{
				GameManager.ShowTooltip(GameManager.Instance.World.GetLocalPlayers()[0], Localization.Get("xuiPrefabsPrefabSavingError"));
