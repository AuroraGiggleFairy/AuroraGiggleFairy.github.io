...
public class XUiC_PopupToolTip : XUiController
{
	...
	public void QueueTooltipInternal(string _text, string[] _args, string _alertSound, ToolTipEvent _eventHandler, bool _showImmediately)
	{
		...
		{
			_text = Localization.Get(_text);
			if (_args != null && _args.Length != 0)
			{
				_text = string.Format(_text, _args);
			}
			Tooltip item = new Tooltip(_text, _alertSound, _eventHandler);
			if (_showImmediately)
			{
				immediateTip = item;
				DisplayTooltipText();
	...
	{
		GetInstance(_xui)?.QueueTooltipInternal(_text, _args, _alertSound, _eventHandler, _showImmediately);
	}
