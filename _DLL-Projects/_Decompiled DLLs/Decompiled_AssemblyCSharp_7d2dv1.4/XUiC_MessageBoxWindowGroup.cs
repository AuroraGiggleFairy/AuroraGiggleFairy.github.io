...
public class XUiC_MessageBoxWindowGroup : XUiController
{
	...
	public void ShowUrlConfirmationDialog(string _url, string _displayUrl, bool _modal = false, Func<string, bool> _browserOpenMethod = null, string _title = null, string _text = null)
	{
		if (!string.IsNullOrEmpty(_url) && Utils.IsValidWebUrl(ref _url))
		{
			...
			ShowMessage(_title, _text, MessageBoxTypes.OkCancel, openPage, cancelOpenPage, _openMainMenuOnClose: false, _modal);
	...
	public static void ShowMessageBox(XUi _xuiInstance, string _title, string _text, MessageBoxTypes _messageBoxType = MessageBoxTypes.Ok, Action _onOk = null, Action _onCancel = null, bool _openMainMenuOnClose = true, bool _bCloseAllOpenWindows = true)
	{
		returnNavigationTarget = _xuiInstance.playerUI.CursorController.navigationTarget;
		((XUiC_MessageBoxWindowGroup)_xuiInstance.FindWindowGroupByName(ID)).ShowMessage(_title, _text, _messageBoxType, _onOk, _onCancel, _openMainMenuOnClose, _modal: true, _bCloseAllOpenWindows);
	}
	...
	{
		returnNavigationTarget = _xuiInstance.playerUI.CursorController.navigationTarget;
		((XUiC_MessageBoxWindowGroup)_xuiInstance.FindWindowGroupByName(ID)).ShowMessage(_title, _text, MessageBoxTypes.Ok, null, _onOk, _openMainMenuOnClose);
	}
