...
public class GUIWindowScreenshotText : GUIWindow
{
				...
				if (LocalPlayerUI.GetUIForPrimaryPlayer() != null)
				{
					...
					if (entityPlayer != null)
					{
						...
						if (!confirmed)
						{
							if (i == 1)
							{
								savePerks = GUI.Toggle(new Rect(xMin + 5f, num2, width - 10f, lineHeight), savePerks, "Save Perks", checkboxStyle);
							}
							num2 += (float)lineHeight;
						}
					}
	...
	public void StorePlayerStats(string _filenameBase)
	{
		...
		foreach (KeyValuePair<int, ProgressionValue> item in primaryPlayer.Progression.GetDict())
		{
		...
		foreach (ProgressionValue item2 in list)
		{
			...
			{
				stringBuilder.AppendLine();
				stringBuilder.AppendLine($"{progressionClass.Name},{item2.Level},{item2.CalculatedLevel(primaryPlayer)}");
			}
		...
		foreach (ProgressionValue item3 in list)
		{
			...
			{
				stringBuilder.AppendLine($" - {progressionClass2.Name},{item3.Level},{item3.CalculatedLevel(primaryPlayer)}");
			}
		}
		stringBuilder.AppendLine();
		...
		foreach (ProgressionValue item4 in list)
		{
			...
			{
				stringBuilder.AppendLine($" - {progressionClass3.Name},{item4.Level},{item4.CalculatedLevel(primaryPlayer)}");
			}
		}
		stringBuilder.AppendLine();
