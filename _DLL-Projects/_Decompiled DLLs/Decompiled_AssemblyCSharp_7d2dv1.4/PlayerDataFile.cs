...
public class PlayerDataFile
{
	...
	public void ToPlayer(EntityPlayer _player)
	{
		...
		if (ecd.stats != null)
		{
			ecd.stats.CopyBuffChangedDelegates(_player.Stats);
			_player.Stats = ecd.stats;
			_player.Stats.Entity = _player;
		}
		else
		{
			_player.Stats.InitWithOldFormatData(ecd.health, ecd.stamina, ecd.sickness, ecd.gassiness);
		}
		...
		_player.bag.LockedSlots = (bool[])bagLockedSlots?.Clone();
		...
		_player.trackedFriendEntityIds = trackedFriendEntityIds;
	...
	public void FromPlayer(EntityPlayer _player)
	{
		...
		bagLockedSlots = (bool[])_player.bag.LockedSlots?.Clone();
		...
		trackedFriendEntityIds = new List<int>(_player.trackedFriendEntityIds);
	...
	public void Save(string _dir, string _playerId)
	{
		try
		{
			...
			using (Stream baseStream = SdFile.Open(text + ".tmp", FileMode.CreateNew, FileAccess.Write, FileShare.Read))
			{
				...
				pooledBinaryWriter.Write((byte)55);
	...
	public void Write(PooledBinaryWriter _bw)
	{
		...
		bool[] array = bagLockedSlots;
		_bw.Write((ushort)((array != null) ? ((uint)array.Length) : 0u));
		if (bagLockedSlots != null)
		{
			bool[] array2 = bagLockedSlots;
		...
		_bw.Write((ushort)trackedFriendEntityIds.Count);
		for (int j = 0; j < trackedFriendEntityIds.Count; j++)
		{
			_bw.Write(trackedFriendEntityIds[j]);
		}
		...
		for (int k = 0; k < favoriteCreativeStacks.Count; k++)
		{
			_bw.Write(favoriteCreativeStacks[k]);
		}
		_bw.Write((ushort)favoriteShapes.Count);
		for (int l = 0; l < favoriteShapes.Count; l++)
		{
			_bw.Write(favoriteShapes[l]);
		}
		_bw.Write((ushort)ownedEntities.Count);
		for (int m = 0; m < ownedEntities.Count; m++)
		{
			ownedEntities[m].Write(_bw);
		}
