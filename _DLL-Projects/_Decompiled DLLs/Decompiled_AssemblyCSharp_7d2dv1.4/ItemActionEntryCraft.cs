...
public class ItemActionEntryCraft : BaseItemActionEntry
{
	...
	public ItemActionEntryCraft(XUiController controller, XUiC_RecipeCraftCount recipeCraftCount, int craftingTier)
		: base(controller, "lblContextActionCraft", "ui_game_symbol_hammer", GamepadShortCut.DPadUp)
	{
	...
	public bool HasItems(XUi xui, Recipe recipe)
	{
		...
		for (int i = 0; i < recipe.ingredients.Count; i++)
		{
			...
			{
				num = (int)EffectManager.GetValue(PassiveEffects.CraftingIngredientCount, null, recipe.ingredients[i].count, xui.playerUI.entityPlayer, recipe, FastTags<TagGroup.Global>.Parse(recipe.ingredients[i].itemValue.ItemClass.GetItemName()), calcEquipment: true, calcHoldingItem: true, calcProgression: true, calcBuffs: true, calcChallenges: true, craftingTier);
			}
			if (!recipe.ingredients[i].itemValue.HasQuality)
			{
				tempIngredientList.Add(new ItemStack(recipe.ingredients[i].itemValue, num));
				continue;
			}
			int num2 = ((num == 0) ? 1 : num);
			tempIngredientList.Add(new ItemStack(recipe.ingredients[i].itemValue.Clone(), num2));
		...
		flag |= xui.PlayerInventory.HasItems(tempIngredientList, craftCountControl.Count);
		XUiC_WorkstationInputGrid childByType = craftCountControl.WindowGroup.Controller.GetChildByType<XUiC_WorkstationInputGrid>();
		if (childByType != null)
		{
