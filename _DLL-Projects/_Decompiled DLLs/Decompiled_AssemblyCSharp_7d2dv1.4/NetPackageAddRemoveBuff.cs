...
public class NetPackageAddRemoveBuff : NetPackage
{
	...
	{
		if (_world == null || (senderId != -1 && senderId == GameManager.Instance?.persistentLocalPlayer?.EntityId))
		{
			return;
		}
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendPackage(NetPackageManager.GetPackage<NetPackageAddRemoveBuff>().Setup(entityId, senderId, buffName, duration, adding, instigatorId, instigatorPos), _onlyClientsAttachedToAnEntity: false, -1, -1, entityId);
		}
