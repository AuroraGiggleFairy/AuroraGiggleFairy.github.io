...
public class GameSparksManager : MonoBehaviour
{
	...
	public IEnumerator Start()
	{
		...
		GS.GameSparksAvailable = (Action<bool>)Delegate.Combine(GS.GameSparksAvailable, (Action<bool>)([PublicizedFrom(EAccessModifier.Internal)] (bool _gsAvailable) =>
		{
			if (_gsAvailable)
			{
		...
		yield return new WaitUntil([PublicizedFrom(EAccessModifier.Internal)] () => GS.Available);
	...
	public void DeviceAuth(OnPlayerAuthenticated _onPlayerAuthSuccess, OnError _onAuthError)
	{
		...
		new DeviceAuthenticationRequest().SetDurable(durable: true).Send([PublicizedFrom(EAccessModifier.Internal)] (AuthenticationResponse _response) =>
		{
			...
			else
			{
				...
				_onAuthError?.Invoke(_response.Errors.JSON);
			}
		});
	}
	...
	public void PrepareAndSendRequest(GSRequestData _data, string _eventKey)
	{
		...
		SendRequest(request, _eventKey);
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public void SendRequest(LogEventRequest _request, string _eventKey)
	{
		_request.Send([PublicizedFrom(EAccessModifier.Internal)] (LogEventResponse _response) =>
		{
