...
public class ConsoleCmdPrefab : ConsoleCmdAbstract
{
	...
	public override void Execute(List<string> _params, CommandSenderInfo _senderInfo)
	{
		...
		case "load":
			...
			{
				GameManager.Instance.prefabEditModeManager.LoadVoxelPrefab(PathAbstractions.PrefabsSearchPaths.GetLocation(_params[1]));
			}
			break;
		case "save":
		...
		case "thumbnail":
			...
			else if (_params.Count == 2)
			{
				...
				{
					GameManager.Instance.prefabEditModeManager.LoadVoxelPrefab(PathAbstractions.PrefabsSearchPaths.GetLocation(_params[1]), _bBulk: true, _bIgnoreExcludeImposterCheck: true);
		...
		case "restore":
			...
			break;
		case "stats":
			SdFile.WriteAllText(PrefabStatsFilename, "Prefab,TotalVerts,TotalTris\n", Encoding.UTF8);
	...
	public void GetPrefabStats(PathAbstractions.AbstractedLocation _path, Prefab _prefab)
	{
		...
		SdFile.AppendAllText(PrefabStatsFilename, $"{_path.Name},{worldStats.TotalVertices},{worldStats.TotalTriangles}\n");
	}
	...
	public void thumbnailBulk()
	{
		...
		while (prefabsToThumbnail.Count != 0)
		{
			...
			if (GameManager.Instance.prefabEditModeManager.LoadVoxelPrefab(location, _bBulk: true, _bIgnoreExcludeImposterCheck: true))
			{
				break;
			}
		}
	...
	public void convertBulk()
	{
		...
		while (prefabsToConvert.Count != 0)
		{
			...
			if (GameManager.Instance.prefabEditModeManager.LoadVoxelPrefab(abstractedLocation, _bBulk: true))
			{
				break;
			}
		}
	...
	public void convertBulkInsideOutside()
	{
		...
		while (prefabsToConvert.Count != 0)
		{
			...
			prefabsToConvert.RemoveAt(0);
			if (GameManager.Instance.prefabEditModeManager.LoadVoxelPrefab(location, _bBulk: true) && !GameManager.Instance.prefabEditModeManager.VoxelPrefab.bExcludePOICulling)
			{
				break;
			}
		}
