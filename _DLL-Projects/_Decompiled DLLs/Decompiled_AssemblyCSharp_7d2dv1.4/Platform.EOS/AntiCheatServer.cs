...
public class AntiCheatServer : IAntiCheatServer, IAntiCheatEncryption
{
	...
	public void apiInitialized()
	{
		...
		lock (AntiCheatCommon.LockObject)
		{
			antiCheatInterface.AddNotifyMessageToClient(ref options, null, handleMessageToClient);
		}
		AddNotifyClientActionRequiredOptions options2 = default(AddNotifyClientActionRequiredOptions);
		lock (AntiCheatCommon.LockObject)
		{
			antiCheatInterface.AddNotifyClientActionRequired(ref options2, null, handleClientAction);
		}
		AddNotifyClientAuthStatusChangedOptions options3 = default(AddNotifyClientAuthStatusChangedOptions);
		lock (AntiCheatCommon.LockObject)
		{
			antiCheatInterface.AddNotifyClientAuthStatusChanged(ref options3, null, handleClientAuthStateChange);
		}
	}
