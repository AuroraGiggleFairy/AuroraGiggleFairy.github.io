...
public class ItemActionTextureBlock : ItemActionRanged
{
	...
	public int getCurrentPaintIdx(ChunkCluster _cc, Vector3i _blockPos, BlockFace _blockFace, BlockValue _blockValue)
	{
		...
		string _name;
		return GameUtils.FindPaintIdForBlockFace(_blockValue, _blockFace, out _name);
	}

	[PublicizedFrom(EAccessModifier.Private)]
	public EPaintResult paintFace(ChunkCluster _cc, int _entityId, ItemActionTextureBlockData _actionData, Vector3i _blockPos, BlockFace _blockFace, BlockValue _blockValue)
	{
		int currentPaintIdx = getCurrentPaintIdx(_cc, _blockPos, _blockFace, _blockValue);
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public EPaintResult paintBlock(World _world, ChunkCluster _cc, int _entityId, ItemActionTextureBlockData _actionData, Vector3i _blockPos, BlockFace _blockFace, BlockValue _blockValue, PersistentPlayerData _lpRelative)
	{
		...
		{
			return paintFace(_cc, _entityId, _actionData, _blockPos, _blockFace, _blockValue);
		}
		int num = 0;
		for (int i = 0; i <= 5; i++)
		{
			_blockFace = (BlockFace)i;
			EPaintResult ePaintResult = paintFace(_cc, _entityId, _actionData, _blockPos, _blockFace, _blockValue);
			switch (ePaintResult)
			{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public void floodFill(World _world, ChunkCluster _cc, int _entityId, ItemActionTextureBlockData _actionData, PersistentPlayerData _lpRelative, int _sourcePaint, Vector3 _hitPosition, Vector3 _hitFaceNormal, Vector3 _dir1, Vector3 _dir2)
	{
		...
		while (positionsToCheck.Count > 0)
		{
			...
			if (!flag)
			{
				...
				if (getCurrentPaintIdx(_cc, blockPos, blockFaceFromHitInfo, blockValue) != _sourcePaint)
				{
					visitedPositions.Add(blockPos, value: false);
					continue;
				}
				EPaintResult ePaintResult = paintBlock(_world, _cc, _entityId, _actionData, blockPos, blockFaceFromHitInfo, blockValue, _lpRelative);
	...
	public IEnumerator fireShotLater(int _shotIdx, ItemActionDataRanged _actionData)
	{
		...
		if (itemActionTextureBlockData.bReplacePaintNextTime)
		{
			...
			int num = chunkCluster.GetBlockFaceTexture(blockPos, blockFace);
			...
			if (num == 0)
			{
				num = GameUtils.FindPaintIdForBlockFace(bv, blockFace, out _name);
			}
			if (num != itemActionTextureBlockData.idx)
			{
				...
				{
					replacePaintInCurrentSelection(blockPos, blockFace, num, itemActionTextureBlockData.idx);
				}
				else
				{
					replacePaintInCurrentPrefab(blockPos, blockFace, num, itemActionTextureBlockData.idx);
				}
			}
			yield break;
		}
		...
		case EnumPaintMode.Single:
			paintBlock(world, chunkCluster, holdingEntity.entityId, itemActionTextureBlockData, blockPos, blockFace, bv, playerDataFromEntityID);
			break;
		case EnumPaintMode.Fill:
		{
			...
			int num5 = chunkCluster.GetBlockFaceTexture(blockPos, blockFace);
			...
			if (num5 == 0)
			{
				num5 = GameUtils.FindPaintIdForBlockFace(bv, blockFace, out _name);
			}
			if (itemActionTextureBlockData.idx == num5)
			{
				yield break;
			}
			floodFill(world, chunkCluster, holdingEntity.entityId, itemActionTextureBlockData, playerDataFromEntityID, num5, hitInfo.hit.pos, normalized2, vector3, vector4);
			break;
		}
	...
	public void CopyTextureFromWorld(ItemActionDataRanged _actionData)
	{
		if (_actionData.invData.holdingEntity is EntityPlayerLocal)
		{
			...
			case 0:
			{
				num = GameUtils.FindPaintIdForBlockFace(bv, blockFace, out var _);
				break;
			}
			}
			ItemActionTextureBlockData itemActionTextureBlockData = (ItemActionTextureBlockData)_actionData;
			...
			for (int j = prefabInstance.boundingBoxPosition.z; j <= prefabInstance.boundingBoxPosition.z + prefabInstance.boundingBoxSize.z; j++)
			{
				for (int k = 0; k < 256; k++)
				{
					...
					for (int l = 0; l < 6; l++)
					{
						...
						if (num2 == 0)
						{
							num2 = GameUtils.FindPaintIdForBlockFace(block, (BlockFace)l, out var _);
						}
