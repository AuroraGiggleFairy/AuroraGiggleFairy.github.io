...
public abstract class EntityAlive : Entity
{
	...
	public void InitPostCommon()
	{
		...
		if ((entityFlags & (EntityFlags.Zombie | EntityFlags.Animal | EntityFlags.Bandit)) != EntityFlags.None)
		{
	...
	public override void CopyPropertiesFromEntityClass()
	{
		...
		entityClass.Properties.ParseString(EntityClass.PropSoundSleeperGroan, ref soundSleeperGroan);
		...
		entityClass.Properties.ParseString(EntityClass.PropSoundFootstepModifier, ref soundFootstepModifier);
		...
		{
			string text2 = entityClass.Properties.GetString(EntityClass.PropItemsOnEnterGame + "." + gameModeForId.GetTypeName());
			if (text2.Length > 0)
			{
				...
				foreach (string text3 in array)
				{
					...
					{
						throw new Exception("Item with name '" + text3 + "' not found in class " + EntityClass.list[base.entityClass].entityClassName);
					}
		...
		{
			string text4 = array2[j].ToStringCached();
			...
			if (dictionarySave2.TryGetValue("difficulty", out _value2))
			{
				...
				{
					Log.Error("Expected 'difficulty' parameter as range(min,min-max) " + text4 + ", skipping");
					continue;
				}
				difficulty2 = _result9;
			}
			_destroyBlockBehaviors.Add(new DestroyBlockBehavior(text4, array2[j], _result6, ragePer2, rageTime2, difficulty2));
		}
	}
	...
	public override void OnUpdateEntity()
	{
		...
		{
			attackingTime--;
			if (attackingTime == 0 && attackTarget != null)
			{
	...
	{
		if (!SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer || GameManager.Instance.IsEditMode() || IsGodMode.Value || !(this is EntityPlayer) || !(Time.time - lastTimeTraderStationChecked > 2f))
		{
			return;
		}
		...
		if (traderAreaAt != null)
		{
			...
			bool flag = false;
			Vector3 vector2 = traderAreaAt.ProtectPosition + traderAreaAt.ProtectSize * 0.5f;
			if ((bool)entityPlayer && world.IsWorldEvent(World.WorldEvent.BloodMoon))
			{
				flag = true;
			}
			if (((bool)entityPlayer || this is EntityHuman) && traderAreaAt.IsWithinTeleportArea(vector, out var tpVolume))
			{
				flag = traderAreaAt.IsClosed;
				if (!flag && (bool)entityPlayer && EffectManager.GetValue(PassiveEffects.NoTrader, null, 0f, this) == 1f)
				{
					flag = true;
					vector2 = world.GetPOIAtPosition(vector3i).boundingBoxPosition + tpVolume.startPos + tpVolume.size * 0.5f;
				}
			}
			if (flag)
			{
				traderTeleportStreak++;
				Vector3 normalized = (GetPosition() - vector2).normalized;
				normalized.y = 0f;
				Vector3 vector3 = GetPosition() + normalized * 5f * traderTeleportStreak;
				vector3 = GetTeleportPosition(vector3, normalized);
				if (isEntityRemote)
				{
					SingletonMonoBehaviour<ConnectionManager>.Instance.Clients.ForEntityId(entityId).SendPackage(NetPackageManager.GetPackage<NetPackageTeleportPlayer>().Setup(vector3));
				}
				else if ((bool)entityPlayer)
				{
					entityPlayer.Teleport(vector3);
				}
				else if (AttachedToEntity != null)
				{
					AttachedToEntity.SetPosition(vector3);
				}
				else
				{
					SetPosition(vector3);
	...
	public void ClearInvestigatePosition()
	{
		...
		investigatePositionTicks = 0;
		ResetDespawnTime();
		int num = rand.RandomRange(12, 25) * 20;
	...
	public void internalPlayStepSound()
	{
		...
		string text;
		if (!blockValue.isair && (materialForSide = block2.GetMaterialForSide(blockValue, BlockFace.Top)) != null && materialForSide.stepSound != null && (text = materialForSide.stepSound.name).Length > 0)
		{
			if (WeatherManager.GetCurrentSnowValue() > 0.25f && GetLightBrightness() > 0.75f)
			{
				text = "snow";
			}
			if (EffectManager.GetValue(PassiveEffects.SilenceBlockSteps, null, 0f, this, null, block2.Tags) <= 0f)
			{
				string stepSound = "step" + text;
				playStepSound(stepSound);
			}
		}
	}
	...
	public virtual void updateStepSound(float _distX, float _distZ)
	{
		...
		else
		{
			...
			distanceWalked += num;
			stepDistanceToPlaySound -= num;
			if (stepDistanceToPlaySound <= 0f)
			{
				stepDistanceToPlaySound = getNextStepSoundDistance();
				internalPlayStepSound();
			}
		}
	}
	...
	public virtual void playStepSound(string stepSound)
	{
		if (this is EntityPlayerLocal)
		{
			...
			if (soundFootstepModifier != null)
			{
				Manager.BroadcastPlay(this, soundFootstepModifier);
			}
		}
		else
		{
			...
			if (soundFootstepModifier != null)
			{
				Manager.Play(this, soundFootstepModifier);
			}
		}
	}
	...
	public float GetDismemberChance(ref DamageResponse _dmResponse, float damagePer)
	{
		...
		float num = dismemberChance * damagePer * originalValue;
		...
		{
			Log.Out("GetDismemberChance {0}, primary {1}, damage {2}, chance {3} * damage% {4} * multiplier {5} = {6}", hitBodyPart, hitBodyPart.ToPrimary(), _dmResponse.Strength, dismemberChance.ToCultureInvariantString(), damagePer.ToCultureInvariantString(), originalValue.ToCultureInvariantString(), num.ToCultureInvariantString());
		}
		return num;
	}
	...
	public void ApplyLocalBodyDamage(DamageResponse _dmResponse)
	{
		...
		if (_dmResponse.Dismember)
		{
			...
			if (partData != null && partData.isLinked)
			{
				...
				bodyDamage.bodyPartHit = bodyPartHit;
	...
	public void HeadHeightFixedUpdate()
	{
		...
		origin.y += num3;
		RaycastHit hitInfo;
		bool flag = Physics.SphereCast(origin, num2, Vector3.up, out hitInfo, maxDistance, 1083277312);
		if (flag)
		{
			...
			{
				num = num3 + hitInfo.distance - 0.16f;
			}
		}
		...
		{
			num = Mathf.MoveTowards(physicsHeight, num, 1f / 70f);
		}
		SetHeight(num);
		if (flag && num <= physicsBaseHeight * 0.75f)
		{
			int num4 = 22;
			if (walkType != num4 && walkType != 21)
			{
