	...
	public void ActivateObject(bool _bHeightAndNormalOnly)
	{
		...
		while (num11 < cMDataVerticesHeight.Length)
		{
			...
			if (BaseChunkMap != null && BaseChunkMap.wcd != null)
			{
				...
				int sideTextureId = block.GetSideTextureId(new BlockValue((uint)waterPlaneBlockId), BlockFace.Top);
				int sideTextureId2 = block.GetSideTextureId(new BlockValue((uint)waterPlaneBlockId), BlockFace.South);
		...
		for (int m = 0; m < CurTri.Length; m += 3)
		{
			...
			CellMeshData.VoxelMesh.GetColorForTextureId(num17, CellMeshData.TextureId[num14], _bTopSoil: true, out CellMeshData.Colors[num14], out var _uv, out var _uv2, out var _uv3, out var _uv4);
			CellMeshData.VoxelMesh.GetColorForTextureId(num17, CellMeshData.TextureId[num15], _bTopSoil: true, out CellMeshData.Colors[num14], out _uv, out _uv2, out _uv3, out _uv4);
			CellMeshData.VoxelMesh.GetColorForTextureId(num17, CellMeshData.TextureId[num16], _bTopSoil: true, out CellMeshData.Colors[num14], out _uv, out _uv2, out _uv3, out _uv4);
		}
	}
