...
public static class GameOptionsManager
{
	...
	{
		int num = GamePrefs.GetInt(EnumGamePrefs.OptionsGfxAA);
		QualitySettings.antiAliasing = (int)((num == 0 || num > 3) ? 0f : Mathf.Pow(2f, num));
		float num2 = (QualitySettings.streamingMipmapsMemoryBudget = GameOptionsPlatforms.GetStreamingMipmapBudget());
		Log.Out("ApplyAllOptions streaming budget {0} MB", num2);
