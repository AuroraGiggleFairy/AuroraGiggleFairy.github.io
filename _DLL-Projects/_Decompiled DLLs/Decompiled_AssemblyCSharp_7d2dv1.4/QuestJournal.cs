...
public class QuestJournal
{
	...
	public bool AddSharedQuestEntry(int questCode, string questID, string poiName, Vector3 position, Vector3 size, Vector3 returnPos, int sharerID, int questGiverID)
	{
		for (int i = 0; i < sharedQuestEntries.Count; i++)
		{
			...
			{
				Log.Out($"Ignoring received quest, already have a sharedquest with the questCode={questCode}, id={questID}, sharer={sharerID}, giver={questGiverID}:");
		...
		Log.Out($"Received shared quest: questCode={questCode}, id={questID}, POI {poiName}");
