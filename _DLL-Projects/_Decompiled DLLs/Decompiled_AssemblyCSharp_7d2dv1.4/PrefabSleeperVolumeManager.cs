...
public class PrefabSleeperVolumeManager
{
	...
	public void AddSleeperVolumeServer(Vector3i _hitPointBlockPos)
	{
		if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
		{
			...
			if (dynamicPrefabDecorator != null)
			{
				...
				if (prefabInstance != null)
				{
					...
					int num = prefabInstance.prefab.AddSleeperVolume(prefabInstance.name, prefabInstance.boundingBoxPosition, _hitPointBlockPos - prefabInstance.boundingBoxPosition - new Vector3i(size.x / 2, 0, size.z / 2), size, 0, "GroupGenericZombie", 5, 6);
		...
		{
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendToServer(NetPackageManager.GetPackage<NetPackageEditorAddSleeperVolume>().Setup(_hitPointBlockPos));
		}
	}
