...
public class ItemActionDynamicMelee : ItemActionDynamic
{
	...
	{
		if (!(_actionData is ItemActionDynamicMeleeData))
		{
			return false;
		}
		return (_actionData as ItemActionDynamicMeleeData).Attacking;
	}
	...
	{
		if (_actionData is ItemActionDynamicMeleeData { Attacking: not false } itemActionDynamicMeleeData)
		{
			if (!UsePowerAttackTriggers)
			{
	...
	public bool canStartAttack(ItemActionDynamicMeleeData _actionData)
	{
		...
		EntityAlive holdingEntity = _actionData.invData.holdingEntity;
		float num = EffectManager.GetValue(PassiveEffects.AttacksPerMinute, _actionData.invData.itemValue, 60f, _actionData.invData.holdingEntity, null, tags);
		if (num == 0f)
		{
			num = 0.0001f;
		}
		...
		_actionData.StaminaUsage = EffectManager.GetValue(PassiveEffects.StaminaLoss, _actionData.invData.itemValue, 2f, _actionData.invData.holdingEntity, null, _actionData.ActionTags);
		if (_actionData.invData.holdingEntity.Stats.Stamina.Value < _actionData.StaminaUsage)
		{
			if (_actionData.HasReleased)
			{
				Manager.Play(_actionData.invData.holdingEntity, _actionData.invData.holdingEntity.IsMale ? "player1stamina" : "player2stamina");
				GameManager.ShowTooltip(_actionData.invData.holdingEntity as EntityPlayerLocal, "ttOutOfStamina");
			}
			return false;
		}
		return true;
