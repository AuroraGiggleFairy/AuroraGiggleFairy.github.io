...
public class Block
{
	...
	public virtual void Init()
	{
		...
		if (Properties.Values.ContainsKey(PropCanMobsSpawnOn))
		{
			CanMobsSpawnOn = StringParsers.ParseBool(Properties.Values[PropCanMobsSpawnOn]);
		}
		else
		{
			CanMobsSpawnOn = false;
		}
		if (Properties.Values.ContainsKey(PropCanPlayersSpawnOn))
		{
			CanPlayersSpawnOn = StringParsers.ParseBool(Properties.Values[PropCanPlayersSpawnOn]);
		}
		else
		{
		...
		if (text3.Length > 0)
		{
			...
			if (!text3.Contains(","))
			{
				...
				for (int num9 = 0; num9 < cUVModeSides; num9++)
				{
					UVModesPerSide |= (uint)uVMode << num9 * cUVModeBits;
				}
			}
			else
			{
	...
	public UVMode GetUVMode(int side)
	{
		return (UVMode)((UVModesPerSide >> side * cUVModeBits) & cUVModeMask);
	}
	...
	{
		int sideTextureId = GetSideTextureId(_blockValue, _side);
		if (sideTextureId < 0)
		{
	...
	{
		singleTextureId = _textureId;
		bTextureForEachSide = false;
	}

	public void SetSideTextureId(string[] _texIds)
	{
		sideTextureIds = new int[_texIds.Length];
		for (int i = 0; i < _texIds.Length; i++)
		{
			sideTextureIds[i] = int.Parse(_texIds[i]);
		}
		bTextureForEachSide = true;
	}

	public int GetSideTextureId(BlockValue _blockValue, BlockFace _side)
	{
		if (bTextureForEachSide)
		{
			int num = shape.MapSideAndRotationToTextureIdx(_blockValue, _side);
			if (num >= sideTextureIds.Length)
			{
				num = 0;
			}
			return sideTextureIds[num];
		}
		return singleTextureId;
	}
	...
	public int GetUiBackgroundTextureId(BlockValue _blockValue, BlockFace _side)
	{
		...
		{
			return GetSideTextureId(_blockValue, _side);
		}
	...
	{
		shape.renderDecorations(_worldPos, _blockValue, _drawPos, _vertices, _lightingAround, _fulltexture, _meshes, _nBlocks);
	}
