...
public class ChunkManager : IChunkProviderIndicator
{
	...
	public void Init(World _world)
	{
		...
		threadInfoRegenerating = ThreadManager.StartThread("ChunkRegeneration", thread_RegeneratingInit, thread_Regenerating, null, System.Threading.ThreadPriority.Normal, null, null, _useRealThread: true);
		threadInfoCalc = ThreadManager.StartThread("ChunkCalc", null, thread_Calc, null, System.Threading.ThreadPriority.Normal, null, null, _useRealThread: true);
		BakeInit();
	}
	...
	public int thread_Regenerating(ThreadManager.ThreadInfo _threadInfo)
	{
		if (!_threadInfo.TerminationRequested())
		{
			if (VoxelMeshLayer.InstanceCount - MemoryPools.poolVML.GetPoolSize() > MaxQueuedMeshLayers)
			{
				return 20;
			}
				...
				if (SingletonMonoBehaviour<ConnectionManager>.Instance.IsServer)
				{
					...
					if (chunkCache != null && !chunkCache.IsFixedSize)
					{
						lock (chunksToUnload)
						{
							...
							{
								if (!m_AllChunkPositions.Contains(item) && !DynamicMeshThread.ChunksToProcess.Contains(item) && !DynamicMeshThread.ChunksToLoad.Contains(item))
								{
	...
	{
		groundAlignBlocks.Add(_data);
	}

	public void GroundAlignFrameUpdate()
	{
		int count = groundAlignBlocks.Count;
		for (int i = 0; i < count; i++)
		{
			BlockEntityData blockEntityData = groundAlignBlocks[i];
	...
	{
		bakeThreadInfo = ThreadManager.StartThread("ChunkMeshBake", null, BakeThread, null, System.Threading.ThreadPriority.Normal, null, null, _useRealThread: true);
