...
public class TitleStorageOverridesManager
{
	...
	public IEnumerator RequestDataCo()
	{
		...
		try
		{
			...
			if (storage == null || PlatformManager.NativePlatform.User.UserStatus != EUserStatus.LoggedIn)
			{
				yield break;
			}
		...
		void fileDownloadedCallback(IRemoteFileStorage.EFileDownloadResult _result, string _errorDetails, byte[] _data)
		{
			try
			{
				...
				{
					Log.Warning("Retrieving title storage overrides file failed: " + _result.ToStringCached() + " (" + _errorDetails + ")");
					ClearOverrides();
				}
				else
				{
					...
					if (_data != null && _data.Length != 0)
					{
						...
						{
							Log.Error("Failed loading title storage overrides XML:");
							Log.Exception(e);
							ClearOverrides();
							return;
						}
					...
					if (xElement == null)
					{
						Log.Error("Failed to load root from title storage override XML.");
					}
					else
					{
						Log.Error("Could not parse local platform/network for title storage overrides.");
					}
					ClearOverrides();
				}
			}
