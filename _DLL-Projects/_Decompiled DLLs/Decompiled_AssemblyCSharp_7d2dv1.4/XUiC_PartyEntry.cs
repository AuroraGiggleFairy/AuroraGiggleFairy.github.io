...
public class XUiC_PartyEntry : XUiController
{
	...
	public override void Update(float _dt)
	{
		...
		if (voiceMemberState != voiceState)
		{
			voiceState = voiceMemberState;
			IsDirty = true;
		}
	...
	public override bool GetBindingValue(ref string value, string bindingName)
	{
		...
		case "arrowcolor":
		{
			...
			Color32 v = Constants.TrackedFriendColors[num % Constants.TrackedFriendColors.Length];
			value = arrowcolorFormatter.Format(v);
			return true;
		}
		case "icon1":
		...
		case "voicevisible":
			value = (GamePrefs.GetBool(EnumGamePrefs.OptionsVoiceChatEnabled) && voiceState != IPartyVoice.EVoiceMemberState.Disabled).ToString();
			return true;
		case "voicemuted":
