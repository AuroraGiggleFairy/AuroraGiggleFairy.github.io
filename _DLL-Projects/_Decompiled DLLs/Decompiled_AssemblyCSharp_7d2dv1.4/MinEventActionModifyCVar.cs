...
public class MinEventActionModifyCVar : MinEventActionTargetedBase
{
	...
	public override void Execute(MinEventParams _params)
	{
		...
		for (int i = 0; i < targets.Count; i++)
		{
			...
			float num3 = targets[i].Buffs.GetCustomVar(cvarName);
			switch (operation)
			{
			...
			targets[i].Buffs.SetCustomVar(cvarName, num3, (targets[i].isEntityRemote && !_params.Self.isEntityRemote) || _params.IsLocal);
		}
	}
