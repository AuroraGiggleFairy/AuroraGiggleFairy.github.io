...
public class NetPackageSimpleChat : NetPackage
{
	...
	public override void ProcessPackage(World _world, GameManager _callbacks)
	{
			...
			}
			{
				foreach (int recipientEntityId in recipientEntityIds)
				{
					if (_world.GetEntity(recipientEntityId) is EntityPlayerLocal entityPlayer)
					{
						...
						{
							XUiC_ChatOutput.AddMessage(uIForPlayer.xui, EnumGameMessages.PlainTextLocal, EChatType.Global, msg, -1, EMessageSender.Server);
						}
					}
					else
					{
		...
		for (int i = 0; i < localPlayers.Count; i++)
		{
			...
			{
				XUiC_ChatOutput.AddMessage(uIForPlayer2.xui, EnumGameMessages.PlainTextLocal, EChatType.Global, msg, -1, EMessageSender.Server);
			}
		}
	}
