...
public class Chunk : IChunk, IBlockAccess, IMemoryPoolableObject
{
	...
	public int NeedsRegenerationAt
	{
		...
		set
		{
			lock (this)
			{
				m_NeedsRegenerationAtY |= 1 << value / 16;
			}
		}
	}
	...
	public Chunk()
	{
		...
		chnTextures = new ChunkBlockChannel(0L, 6);
	...
	public void read(PooledBinaryReader _br, uint _version, bool _bNetworkRead)
	{
		...
		if (_version >= 35)
		{
			chnTextures.Read(_br, _version, _bNetworkRead);
		}
		if (_version >= 46)
		{
		...
		if (!_bNetworkRead && _version > 37)
		{
			...
			for (int num12 = 0; num12 < num11; num12++)
			{
				...
				if (num13 < 0)
				{
					Log.Error("chunk sleeper volumeId invalid {0}", num13);
				}
				else
				{
					AddSleeperVolumeId(num13);
				}
			}
		}
		...
		{
			triggerVolumes.Clear();
			int num14 = _br.ReadByte();
			for (int num15 = 0; num15 < num14; num15++)
			{
				int num16 = _br.ReadInt32();
				if (num16 < 0)
				{
					Log.Error("chunk trigger volumeId invalid {0}", num16);
				}
				else
				{
					AddTriggerVolumeId(num16);
				}
			}
		}
		if (_version >= 45)
		{
			wallVolumes.Clear();
			int num17 = _br.ReadByte();
			for (int num18 = 0; num18 < num17; num18++)
			{
				int num19 = _br.ReadInt32();
				if (num19 < 0)
				{
					Log.Error("chunk wall volumeId invalid {0}", num19);
				}
				else
				{
					AddWallVolumeId(num19);
				}
			}
		}
		if (_bNetworkRead)
		...
		lock (tickedBlocks)
		{
			...
			{
				ChunkBlockLayer chunkBlockLayer2 = m_BlockLayers[num20];
		...
		if (_version > 39)
		{
			int num22 = _br.ReadInt16();
			insideDevices.Capacity = num22;
			byte x2 = 0;
			byte z2 = 0;
			int num23 = 0;
	...
	public void write(PooledBinaryWriter _bw, bool _bNetworkWrite)
	{
		...
		chnTextures.Write(_bw, _bNetworkWrite, array);
		chnWater.Write(_bw, _bNetworkWrite, array);
		_bw.Write(NeedsLightCalculation);
		int num2 = 0;
		for (int l = 0; l < 16; l++)
		{
			List<Entity> list = entityLists[l];
		...
		for (int n = 0; n < 16; n++)
		{
			List<Entity> list2 = entityLists[n];
		...
		for (int num4 = 0; num4 < tileEntities.list.Count; num4++)
		{
			_bw.Write((int)tileEntities.list[num4].GetTileEntityType());
			tileEntities.list[num4].write(_bw, _bNetworkWrite ? TileEntity.StreamModeWrite.ToClient : TileEntity.StreamModeWrite.Persistency);
		}
		_bw.Write(false);
		if (!_bNetworkWrite)
		{
		...
		{
			if (list3.Count > 254 || num8 != insideDevice.x || num9 != insideDevice.z)
			{
				if (list3.Count > 0)
				{
					_bw.Write((byte)num8);
		...
		if (!_bNetworkWrite)
		{
			...
			{
				StreamUtils.Write(_bw, triggerData.list[num12].LocalChunkPos);
				triggerData.list[num12].Write(_bw);
			}
		}
	...
	public byte GetLight(int x, int y, int z, LIGHT_TYPE type)
	{
		x &= 0xF;
		z &= 0xF;
		int num = (int)chnLight.Get(x, y, z);
		num = ((type != LIGHT_TYPE.SUN) ? ((num >> 4) & 0xF) : (num & 0xF));
		return (byte)num;
	}
	...
	{
		return chnTextures.Get(_x, _y, _z);
	}

	public long SetTextureFull(int _x, int _y, int _z, long _texturefull)
	{
		long set = chnTextures.GetSet(_x, _y, _z, _texturefull);
		isModified = true;
		return set;
	}
	...
	{
		if (!WaterUtils.CanWaterFlowThrough(GetBlock(_x, _y, _z)))
		{
			_data.SetMass(0);
		}
		...
		if (_notifyAddChange)
		{
			...
			if (block != null)
			{
				if (blockValue.type != _blockValue.type)
				{
					...
					{
						block.OnBlockAdded(_world, this, vector3i, _blockValue);
					}
				}
				else
				{
	...
	public int GetUsedMem()
	{
		...
		TotalMemory += chnTextures.GetUsedMem();
