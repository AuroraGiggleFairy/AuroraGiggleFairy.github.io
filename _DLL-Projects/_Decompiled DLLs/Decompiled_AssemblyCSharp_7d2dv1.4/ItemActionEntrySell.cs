...
public class ItemActionEntrySell : BaseItemActionEntry
{
	...
	public ItemActionEntrySell(XUiController controller)
		: base(controller, "lblContextActionSell", "ui_game_symbol_coin", GamepadShortCut.None, "")
	{
	...
	public override void RefreshEnabled()
	{
		...
		else if (!xUiC_ItemStack.ItemStack.IsEmpty())
		{
			...
			bool flag = true;
			flag = ((!itemClass.IsBlock()) ? itemClass.SellableToTrader : Block.list[itemStack2.itemValue.type].SellableToTrader);
			if (!isOwner)
			{
				if (!flag)
				{
	...
	public override void OnActivated()
	{
		try
		{
			...
			int num = 1;
			num = ((!forId.IsBlock()) ? forId.EconomicBundleSize : Block.list[itemValue.type].EconomicBundleSize);
	...
	public ItemStack HandleRemoveAmmo(ItemStack stack)
	{
		if (stack.itemValue.Meta > 0)
		{
			...
			for (int i = 0; i < forId.Actions.Length; i++)
			{
				...
				if (stack.itemValue.SelectedAmmoTypeIndex < itemActionRanged.MagazineItemNames.Length)
				{
