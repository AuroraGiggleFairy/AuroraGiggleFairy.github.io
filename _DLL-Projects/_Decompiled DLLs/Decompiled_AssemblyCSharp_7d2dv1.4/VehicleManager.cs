...
public class VehicleManager
{
	...
	public void UpdateVehicleWaypointsForPlayer(int entityId)
	{
		...
		{
			SingletonMonoBehaviour<ConnectionManager>.Instance.SendPackage(NetPackageManager.GetPackage<NetPackageVehiclePositions>().Setup(list), _onlyClientsAttachedToAnEntity: false, entityId);
		}
	}
	...
	public void Save()
	{
		if (saveThread == null || !ThreadManager.ActiveThreads.ContainsKey("vehicleDataSave"))
		{
			...
			saveThread = ThreadManager.StartThread("vehicleDataSave", null, SaveThread, null, System.Threading.ThreadPriority.Normal, pooledExpandableMemoryStream, null, _useRealThread: false, _isSilent: true);
		}
	}
	...
	public void write(PooledBinaryWriter _bw)
	{
		...
		_bw.Write((byte)0);
		_bw.Write((byte)1);
		List<EntityCreationData> vehicles = GetVehicles();
