...
public class EntityVehicle : EntityAlive, ILockable
{
	...
	public void PhysicsFixedUpdate()
	{
		...
		if (wheels.Length != 0)
		{
			if (movementInput != null)
			{
				float num16 = Mathf.Pow(magnitude * 0.1f, 2f);
				float num17 = Mathf.Clamp(1f - num16, 0.15f, 1f);
				wheelMotor = movementInput.moveForward;
				float steerAngleMax = vehicle.SteerAngleMax;
				float num18 = vehicle.SteerRate * num17 * deltaTime;
				if (isTurnTowardsLook)
				{
					...
					if (!Input.GetMouseButton(1))
					{
						...
						forward2.y = 0f;
						num19 = Vector3.SignedAngle(forward, forward2, Vector3.up);
						if (num15 < -0.02f)
						{
							if (Mathf.Abs(num19) > 90f)
							{
								num19 += 180f;
								if (num19 > 180f)
								{
					...
					float num20 = num18 * 1.2f;
					if ((wheelDir < 0f && wheelDir < num19) || (wheelDir > 0f && wheelDir > num19))
					{
						num20 *= 3f;
					}
					wheelDir = Mathf.MoveTowards(wheelDir, num19, num20);
					wheelDir = Mathf.Clamp(wheelDir, 0f - steerAngleMax, steerAngleMax);
				}
				else if (movementInput.lastInputController)
				{
					wheelDir = Mathf.MoveTowards(wheelDir, movementInput.moveStrafe * steerAngleMax, num18 * 1.5f);
				}
				else
				{
			...
			SetWheelsForces(motorTorque, num7, brakeTorque);
		...
		if (hasDriver || (bool)GetFirstAttached())
		{
			if (vehicle.TiltUpForce > 0f)
			{
				...
				Mathf.Abs(right.y);
				float num22 = Mathf.Asin(right.y) * 57.29578f;
				float num23 = wheelDir / vehicle.SteerAngleMax;
				num23 *= 2f;
				num23 = Mathf.LerpUnclamped(0f, num23, Mathf.Pow(magnitude * 0.1f, 2f));
				float tiltAngleMax = vehicle.TiltAngleMax;
				num23 = Mathf.Clamp(num23 * tiltAngleMax, 0f - tiltAngleMax, tiltAngleMax);
				float f2 = num22 + num23;
				float num24 = Mathf.Abs(f2);
				if (num24 > vehicle.TiltThreshold)
				{
					float value = (num24 - vehicle.TiltThreshold) * Mathf.Sign(f2) * 0.01f * (0f - vehicle.TiltUpForce);
					value = Mathf.Clamp(value, -4f, 4f);
					vehicleRB.AddRelativeTorque(0f, 0f, value, ForceMode.VelocityChange);
				}
				if (num24 < vehicle.TiltDampenThreshold)
				{
					...
					if (magnitude2 > 0f)
					{
						...
						float num25 = Mathf.Abs(Vector3.Dot(base.transform.forward, rhs));
						vehicleRB.angularVelocity -= angularVelocity * (0.02f + vehicle.TiltDampening * num25);
					}
				}
			}
			if (vehicle.UpForce > 0f)
			{
				...
				float num26 = Mathf.Abs(Mathf.Acos(up.y) * 57.29578f) - vehicle.UpAngleMax;
				if (num26 > 0f)
				{
					float num27 = num26 / 90f;
					Vector3 torque = Vector3.Cross(up, Vector3.up) * (num27 * num27 * vehicle.UpForce);
	...
	public override void Update()
	{
		...
		if ((bool)vehicleRB)
		{
			...
			if (!isEntityRemote)
			{
				pos2 = PhysicsTransform.position + Origin.position;
				SetPosition(pos2, _bUpdatePhysics: false);
				pos2 -= Origin.position;
				qrotation = PhysicsTransform.rotation;
				rotation = qrotation.eulerAngles;
				ModelTransform.rotation = qrotation;
			}
			...
			if ((bool)attachedPlayerLocal)
			{
				...
				{
					Vector3 forward = base.transform.forward;
					cameraAngleTarget = Vector2.SignedAngle(to: new Vector2(forward.x, forward.z), from: cameraStartVec);
					...
					num -= cameraAngle;
					vp_FPCamera2.Yaw += num;
				}
				float magnitude = vehicleRB.velocity.magnitude;
				float num3 = (0f - Mathf.Lerp(vehicle.CameraDistance.x, vehicle.CameraDistance.y, magnitude / vehicle.VelocityMaxForward)) * cameraDistScale - cameraDist;
				if (num3 < 0f)
				{
				...
				pos2.y += 1.8f;
				pos2.y += lastRBVel.y * 0.2f;
				cameraPos.x = pos2.x;
				cameraPos.z = pos2.z;
				float num4 = pos2.y - cameraPos.y;
				if ((num4 < 0f && cameraVelY > 0f) || (num4 > 0f && cameraVelY < 0f))
				{
					cameraVelY *= 0.98f;
				}
				cameraVelY += num4 * 0.25f * deltaTime;
				cameraVelY *= 0.94f;
				cameraPos.y += cameraVelY;
				num4 = pos2.y - cameraPos.y;
				if (num4 > 2.5f)
				{
					cameraPos.y = pos2.y - 2.5f;
					cameraVelY = 0f;
				}
				else if (num4 < -2.5f)
				{
				...
				{
					cameraStartBlend += deltaTime * 1.2f;
					vp_FPCamera2.DrivingPosition = Vector3.Lerp(cameraStartPos, cameraPos, cameraStartBlend);
					float z = Mathf.Lerp(-0.0001f, cameraDist, cameraStartBlend);
					vp_FPCamera2.Position3rdPersonOffset = new Vector3(0f, 0f, z);
				}
				else
				{
					vp_FPCamera2.DrivingPosition = cameraPos;
					vp_FPCamera2.Position3rdPersonOffset = new Vector3(0f, 0f, cameraDist);
				}
			}
		}
		UpdateAttachment();
	...
	public void CameraInit()
	{
		...
		if ((bool)attachedPlayerLocal)
		{
			...
			cameraDist = 0f - (cameraPos - cameraStartPos).magnitude;
		}
	}
	...
	public override void ProcessDamageResponseLocal(DamageResponse _dmResponse)
	{
		...
		float num = 0f;
		if ((bool)base.AttachedMainEntity)
		{
			num = vehicle.GetPlayerDamagePercent();
			if (!isEntityRemote && world.IsWorldEvent(World.WorldEvent.BloodMoon))
			{
				velocityMax *= 0.6f;
		...
		ApplyDamage((int)num2);
		if ((bool)base.AttachedMainEntity && source.bIsDamageTransfer)
		{
	...
	{
		DamageSource damageSource = new DamageSource(EnumDamageSource.Internal, EnumDamageTypes.VehicleInside, bTransferToRider: true);
	...
	public void DropItemsAsBackpack()
	{
		...
		pos.y += 0.9f;
		EntityLootContainer entityLootContainer = GameManager.Instance.DropContentInLootContainerServer(-1, "DroppedVehicleContainer", pos, list.ToArray());
		if ((bool)entityLootContainer)
		{
			Vector3 vel = rand.RandomOnUnitSphere * 16f;
			vel.y = Utils.FastAbs(vel.y);
			vel.y += 8f;
			entityLootContainer.AddVelocity(vel);
		}
	}
