...
public class AIDirectorBloodMoonParty
{
	...
	public void CalcBestDir(Vector3 basePos)
	{
		...
		for (int i = 0; i < 16; i++)
		{
			...
			for (int j = 0; j < 9; j++)
			{
				if (spawnWorld.GetRandomSpawnPositionMinMaxToPosition(basePos + spawnDirectionV, 0, 10, 30, _checkBedrolls: false, out var _, _isPlayer: false, _checkWater: true, 30))
				{
					num3++;
				}
			}
	...
	{
		int num = GameStats.GetInt(EnumGameStats.BloodMoonEnemyCount) * partySpawner.partyMembers.Count;
		enemyActiveMax = Utils.FastMin(30, num);
		partySpawner.SetScaling(Utils.FastMax(1f, (float)num / (float)enemyActiveMax));
		partySpawner.ResetPartyLevel();
