...
public class EnvironmentAudioManager : MonoBehaviour, IGamePrefsChangedListener
{
	...
	public static IEnumerator CreateNewInstance()
	{
		...
		if (loadedPrefab == null)
		{
			...
			loadedPrefab = prefabLoading.asset;
		}
	...
	public void FixedUpdate()
	{
		...
		UpdateRainAudio();
		UpdateValueTrigger(WeatherManager.Instance.GetCurrentSnowfallValue(), AudioObject.Trigger.Snow);
		...
		if (thunderTriggered != 0)
		{
			List<AudioObject> sound2 = toBiomeLoops.triggers[2].sound;
			if (sound2.Count > 0 && world.GetWorldTime() > thunderTriggered)
			{
				thunderTriggered = 0uL;
				if (world.GetPrimaryPlayer() != null)
				{
					...
					Vector3 vector = lightningPos - position;
					thunderTimer = Time.time + vector.magnitude / 343f;
				}
			}
		}
		if (Time.time > thunderTimer)
		{
			...
			for (int i = 0; i < toBiomeLoops.triggers[2].sound.Count; i++)
			{
				AudioObject audioObject2 = toBiomeLoops.triggers[2].sound[i];
				thunderPlaying = true;
				audioObject2.Play();
			}
		}
	...
	public void IncrementRainVolumes(float inc0, float inc1, float inc2)
	{
		...
		currentRainVolume[0] = Mathf.Clamp01(currentRainVolume[0] + inc0);
		rainAudioSources[0].volume = Mathf.Clamp01(currentRainVolume[0] * num);
		currentRainVolume[1] = Mathf.Clamp01(currentRainVolume[1] + inc1);
		rainAudioSources[1].volume = Mathf.Clamp01(currentRainVolume[1] * num);
		currentRainVolume[2] = Mathf.Clamp01(currentRainVolume[2] + inc2);
		rainAudioSources[2].volume = Mathf.Clamp01(currentRainVolume[2] * num);
	}
	...
	public void TriggerThunder(ulong worldTimeToPlay, Vector3 position)
	{
		...
		thunderTriggered = worldTimeToPlay;
		thunderTimer = float.PositiveInfinity;
	}
