...
public class ItemClass : ItemData
{
	...
	public virtual void Init()
	{
		...
		if (Properties.Classes.ContainsKey("SDCS"))
		{
			...
			if (Properties.Values.ContainsKey("SDCS.Prefab"))
			{
				...
				if (SDCSData.PrefabName.Contains("*"))
				{
					DataLoader.PreloadBundle(SDCSData.PrefabName.Replace("*", "female"));
					DataLoader.PreloadBundle(SDCSData.PrefabName.Replace("*", "male"));
				}
				else
				{
					DataLoader.PreloadBundle(SDCSData.PrefabName);
				}
			}
			...
			if (Properties.Values.ContainsKey("SDCS.CullDistFPV"))
			{
				SDCSData.CullDistance = StringParsers.ParseFloat(Properties.Values["SDCS.CullDistFPV"]);
			}
			...
			if (Properties.Values.ContainsKey("SDCS.HeadGearName"))
			{
				SDCSData.HeadGearName = Properties.Values["SDCS.HeadGearName"];
			}
		}
	...
	{
		if (Properties.Values.ContainsKey(PropSmell))
		{
			AIDirectorData.FindSmell(Properties.Values[PropSmell], out Smell);
		}
		if (HasQuality)
		{
			Stacknumber.Value = 1;
	...
	{
		return CloneModel(_world, _blockValue.ToItemValue(), _position, _parent, _purpose, _textureFull);
	}
	...
	{
		bool flag = true;
		ItemAction itemAction = Actions[actionIdx];
		if (itemAction != null)
		{
	...
	public virtual void ExecuteAction(int _actionIdx, ItemInventoryData _data, bool _bReleased, PlayerActionsLocal _playerActions)
	{
		...
		else
		{
			...
			if (curAction is ItemActionActivate && _bReleased)
			{
				...
				{
					_data.holdingEntity.MinEventContext.ItemValue = _data.itemValue;
					for (int num = 0; num < executionRequirements2.Count; num++)
					{
						flag2 &= executionRequirements2[num].IsValid(_data.holdingEntity.MinEventContext);
						if (!flag2)
						{
							GameManager.ShowTooltip(_data.holdingEntity as EntityPlayerLocal, Localization.Get("ttCannotUseAtThisTime"), string.Empty, "ui_denied");
							actionData.HasExecuted = true;
	...
	public virtual void GetIronSights(ItemInventoryData _invData, out float _fov)
	{
		...
		_fov = GamePrefs.GetInt(EnumGamePrefs.OptionsGfxFOV);
	}
