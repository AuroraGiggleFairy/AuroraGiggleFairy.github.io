...
public class NetConnectionSteam : NetConnectionAbs
{
		...
		: base(_channel, _clientInfo, _netClient, _uniqueId)
	{
		...
		readerThreadInfo = ThreadManager.StartThread("NCSteam_Reader_" + connectionIdentifier, Task_CommReader, ThreadPriority.Normal, null, null, _useRealThread: true);
		writerThreadInfo = ThreadManager.StartThread("NCSteam_Writer_" + connectionIdentifier, Task_CommWriter, ThreadPriority.Normal, null, null, _useRealThread: true);
	}
	...
	public void Task_CommReader(ThreadManager.ThreadInfo _threadInfo)
	{
		try
		{
			while (!bDisconnected)
			{
				...
				Decrypt(bEncrypted, receiveStreamCompressed, 0L);
