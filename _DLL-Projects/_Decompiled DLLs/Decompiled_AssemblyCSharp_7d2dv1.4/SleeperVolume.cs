...
public class SleeperVolume
{
	...
	public int FindSpawnIndex(World _world)
	{
		...
		for (int num2 = spawnsAvailable.Count; num2 > 0; num2--)
		{
			...
			{
				pos.y--;
				if (!_world.GetBlock(pos).isair && SpawnPointIsHidden(_world, num3))
				{
					spawnsAvailable.RemoveAt(num);
					return num3;
				}
	...
	public EntityAlive Spawn(World _world, int entityClass, int spawnIndex, BlockSleeper block)
	{
		...
		if (_world.GetTileEntity(0, spawnPoint.pos) is TileEntitySleeper tileEntitySleeper)
		{
			entityAlive.SetSleeperSight(tileEntitySleeper.GetSightAngle(), tileEntitySleeper.GetSightRange());
	...
	public void Write(BinaryWriter _bw)
	{
		...
		_bw.Write((byte)((groupCountList != null) ? ((uint)groupCountList.Count) : 0u));
		if (groupCountList != null)
		{
