...
public class TextEllipsisAnimator
{
	...
	{
		if (totalEllipsis != 0 && baseString != null && label != null)
		{
			ellipsisTimer += _dt;
	...
	{
		if (_input == null || label == null || !label.SupportBbCode || _mode == AnimationMode.Off)
		{
			baseString = null;
			return;
		}
		...
		{
			label.UpdateData();
			if (label.Text == label.Label.processedText)
			{
				break;
			}
			bool hadEastern = false;
			int j2 = 0;
			baseString = ellipsisBreakPatternEastern.Replace(label.Label.processedText, [PublicizedFrom(EAccessModifier.Internal)] (Match m) =>
			{
			...
			j2 = 0;
			baseString = ellipsisBreakPattern.Replace(label.Label.processedText, [PublicizedFrom(EAccessModifier.Internal)] (Match m) =>
			{
				if (j2 == processedMatches)
				{
					int num2 = processedMatches;
	...
	public void UpdateLabel()
	{
		if (totalEllipsis > 0)
		{
			...
			label.Text = animatedStrings[currentEllipsisState];
			label.UpdateData();
		}
		else
		{
