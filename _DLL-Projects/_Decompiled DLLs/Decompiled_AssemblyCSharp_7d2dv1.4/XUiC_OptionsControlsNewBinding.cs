...
public class XUiC_OptionsControlsNewBinding : XUiController
{
	...
	public override void OnOpen()
	{
		...
		if (forController)
		{
			lblAction.Text = string.Format(Localization.Get("xuiNewBindingCurrent_Controller"), ((PlayerActionData.ActionUserData)action.UserData).LocalizedName, action.GetBindingString(_forController: true, PlatformManager.NativePlatform.Input.CurrentControllerInputStyle));
		}
		else
		{
		...
		string text = ((!forController) ? string.Format(arg0: (PlatformManager.NativePlatform.Input.CurrentControllerInputStyle != PlayerInputManager.InputStyle.PS4) ? "ESC / [sp=XB_Button_Back]" : "ESC / [sp=PS5_Button_Options]", format: Localization.Get("xuiNewBindingAbort_Controller")) : string.Format(arg0: (PlatformManager.NativePlatform.Input.CurrentControllerInputStyle != PlayerInputManager.InputStyle.PS4) ? "[sp=XB_Button_Back] / ESC" : "[sp=PS5_Button_Options] / ESC", format: Localization.Get("xuiNewBindingAbort_Controller")));
		lblAbort.Text = text;
