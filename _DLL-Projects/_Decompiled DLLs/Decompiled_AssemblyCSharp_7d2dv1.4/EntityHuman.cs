...
public class EntityHuman : EntityEnemy
{
	...
	public override float GetMoveSpeedAggro()
	{
		...
		float num2 = moveSpeeds[num];
		if (moveSpeedRagePer > 1f)
		{
			num2 = moveSuperRageSpeeds[num];
		}
		else if (moveSpeedRagePer > 0f)
		{
			float num3 = moveRageSpeeds[num];
			num2 = num2 * (1f - moveSpeedRagePer) + num3 * moveSpeedRagePer;
		}
		num2 = ((!(num2 < 1f)) ? (moveSpeedAggroMax * num2) : (moveSpeedAggro * (1f - num2) + moveSpeedAggroMax * num2));
		return EffectManager.GetValue(PassiveEffects.RunSpeed, null, num2, this);
	}
