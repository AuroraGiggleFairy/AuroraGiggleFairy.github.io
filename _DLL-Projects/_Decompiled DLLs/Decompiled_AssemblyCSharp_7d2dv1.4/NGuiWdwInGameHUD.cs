...
public class NGuiWdwInGameHUD : MonoBehaviour
{
	...
	public void Start()
	{
		...
		nGuiAction.SetClickActionDelegate([PublicizedFrom(EAccessModifier.Private)] () =>
		{
			...
			{
				playerEntity.SwitchFirstPersonView(_bLerpPosition: true);
			}
		});
		nGuiAction.SetIsCheckedDelegate([PublicizedFrom(EAccessModifier.Private)] () => playerEntity != null && playerEntity.bFirstPersonView);
		nGuiAction.SetIsEnabledDelegate([PublicizedFrom(EAccessModifier.Private)] () => playerEntity != null && playerEntity.Spawned && !playerEntity.IsDead() && GamePrefs.GetBool(EnumGamePrefs.DebugMenuEnabled));
		...
		nGuiAction3.SetIsEnabledDelegate([PublicizedFrom(EAccessModifier.Internal)] () => GamePrefs.GetBool(EnumGamePrefs.DebugMenuEnabled));
		...
		playerUI.windowManager.AddGlobalAction(nGuiAction4);
	}
