...
public class TileEntityWorkstation : TileEntity
{
	...
	public override void OnSetLocalChunkPosition()
	{
		...
		if (block.Properties.Values.ContainsKey("Workstation.Modules"))
		{
			...
			array2 = ((!text2.Contains(",")) ? new string[1] { text2 } : text2.Replace(" ", "").Split(','));
	...
	public void HandleRecipeQueue(float _timePassed)
	{
		...
		while (recipeQueueItem.CraftingTimeLeft < 0f && hasRecipeInQueue())
		{
			if (recipeQueueItem.Multiplier > 0)
			{
				...
				AddCraftComplete(recipeQueueItem.StartingEntityId, itemValue, recipeQueueItem.Recipe.GetName(), recipeQueueItem.Recipe.craftExpGain, recipeQueueItem.Recipe.count);
	...
	public override void write(PooledBinaryWriter _bw, StreamModeWrite _eStreamMode)
	{
		...
		case StreamModeWrite.Persistency:
		{
			...
			writeRecipeStackArray(_bw, 48);
			writeCraftCompleteData(_bw, 48);
			_bw.Write(isBurning);
	...
	public void AddCraftComplete(int crafterEntityID, ItemValue itemCrafted, string recipeName, int craftExpGain, int craftedCount)
	{
		...
		for (int i = 0; i < CraftCompleteList.Count; i++)
		{
			if (CraftCompleteList[i].CraftedItemStack.itemValue.GetItemId() == itemCrafted.GetItemId())
			{
		...
		CraftCompleteList.Add(new CraftCompleteData(crafterEntityID, new ItemStack(itemCrafted, craftedCount), recipeName, craftExpGain, 1));
		setModified();
	}
