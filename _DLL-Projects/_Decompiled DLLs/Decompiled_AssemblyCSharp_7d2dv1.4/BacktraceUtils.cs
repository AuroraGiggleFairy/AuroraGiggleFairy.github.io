...
public static class BacktraceUtils
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static readonly string s_svncommit = "75320";
	...
	public static void BacktraceUserLoggedIn(IPlatform platform)
	{
		if (!(s_BacktraceClient == null))
		{
			...
			s_BacktraceClient.SetAttributes(new Dictionary<string, string> { { "gamestats.platformuserid", text } });
	...
	public static void SendBugReport(string message, string screenshotPath = null, SaveInfoProvider.SaveEntryInfo saveEntry = null, Action<BacktraceResult> callback = null)
	{
		if (BugReportFeature)
		{
			...
			if (BugReportAttachSaveFeature && saveEntry != null)
			{
				bool flag = false;
				string archivePath;
	...
	public static void InitializeConfiguration()
	{
		...
		s_platformString = DeviceFlags.Current.ToString().ToUpper();
		Reset();
		lock (_lock)
		{
			s_PlayerLogAttachmentPath = Path.Join(Application.temporaryCachePath, "Player.log");
			Log.LogCallbacks += Backtrace_LogCallbacks;
	...
	public static void Backtrace_LogCallbacks(string _msg, string _trace, LogType _type)
	{
		try
		{
			lock (_lock)
			{
				using StreamWriter streamWriter = new StreamWriter(s_PlayerLogAttachmentPath, append: true);
	...
	public static void ParsePlatformElement(XElement _element)
	{
		...
		else if (_element.Name == enabledversions)
		{
			_element.TryGetAttribute("value", out var _result2);
			s_VersionEnabled = _result2.ContainsCaseInsensitive(Constants.cVersionInformation.SerializableString);
		}
