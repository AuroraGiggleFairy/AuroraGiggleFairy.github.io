...
public static class XUiFromXml
{
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public static DebugLevel debugXuiLoading;
		...
		foreach (XElement item in root.Elements("ruleset"))
		{
			...
			foreach (XElement item2 in item.Elements("window_group"))
			{
				...
				foreach (XElement item3 in item2.Elements("window"))
				{
					...
					{
						flag = true;
						XUiV_Window xUiV_Window = (XUiV_Window)parseViewComponents(item3, xUiWindowGroup, xUiWindowGroup.Controller);
						if (xUiV_Window == null)
						{
							continue;
						}
						if (item3.HasAttribute("anchor"))
						{
							xUiV_Window.Anchor = item3.GetAttribute("anchor");
						}
						if (item3.HasAttribute("pos"))
						{
							xUiV_Window.Position = StringParsers.ParseVector2i(item3.GetAttribute("pos"));
						}
						else if (item3.HasAttribute("position"))
						{
							xUiV_Window.Position = StringParsers.ParseVector2i(item3.GetAttribute("position"));
						}
					}
					if (flag)
					{
					...
					if (windowData.ContainsKey(text2))
					{
						usedWindows[text2]++;
						XUiV_Window xUiV_Window2 = (XUiV_Window)parseViewComponents(windowData[text2], xUiWindowGroup, xUiWindowGroup.Controller);
						if (xUiV_Window2 != null)
						{
							if (item3.HasAttribute("anchor"))
							{
								xUiV_Window2.Anchor = item3.GetAttribute("anchor");
							}
							if (item3.HasAttribute("pos"))
							{
								xUiV_Window2.Position = StringParsers.ParseVector2i(item3.GetAttribute("pos"));
							}
							else if (item3.HasAttribute("position"))
							{
								xUiV_Window2.Position = StringParsers.ParseVector2i(item3.GetAttribute("position"));
							}
	...
	{
		foreach (XElement item in _xmlFile.XmlDoc.Root.Elements("window"))
		{
			string text = "";
			...
			{
				if (!windowData.ContainsKey(text))
				{
					windowData[text] = item;
					usedWindows[text] = 0;
				}
				else if (debugXuiLoading != DebugLevel.Off)
				{
	...
	public static void loadControls(XmlFile _xmlFile)
	{
		foreach (XElement item in _xmlFile.XmlDoc.Root.Elements())
		{
			...
			{
				dictionary[item2.Name.LocalName] = item2.Value;
			}
			int num = item.Elements().Count();
			XElement value = item.Elements().First();
			if (num > 1)
			{
	...
	public static XUiView parseViewComponents(XElement _node, XUiWindowGroup _windowGroup, XUiController _parent = null, string nodeNameOverride = "", Dictionary<string, object> _controlParams = null)
	{
		...
		{
			parseControlParams(_node, _controlParams);
		}
		XUiView xUiView = localName switch
		{
			...
			_ => createFromTemplate(localName, text, _node, _parent, _windowGroup, ref _parseChildren, ref _parseControllerAndAttributes), 
		};
		if (_parseControllerAndAttributes)
		{
			xUiView.xui = xui;
			parseController(_node, xUiView, _parent);
			parseAttributes(_node, xUiView, _parent, _controlParams);
		}
		xUiView.Controller.WindowGroup = _windowGroup;
		if (xUiView.RepeatContent)
		{
			...
			else
			{
				...
				for (int i = 0; i < repeatCount; i++)
				{
					...
					parseViewComponents(new XElement(other), _windowGroup, xUiView.Controller, i.ToString(), _controlParams);
				}
			}
	...
	public static void parseAttributes(XElement _node, XUiView _viewComponent, XUiController _parent, Dictionary<string, object> _controlParams = null)
	{
		...
		foreach (XAttribute item in _node.Attributes())
		{
			...
			if (IsStyleRef(value5))
			{
				string text3 = value5.Substring(1, value5.Length - 2);
				if (!styles["global"].StyleEntries.TryGetValue(text3, out var value6))
				{
					logForNode(LogType.Error, _node, "[XUi] Global style key '" + text3 + "' not found!");
					continue;
				}
				value5 = value6.Value;
			}
			string attributeNameLower = localName2.ToLower();
			parseAttribute(_viewComponent, attributeNameLower, value5, _parent, _controlParams);
		}
	}
