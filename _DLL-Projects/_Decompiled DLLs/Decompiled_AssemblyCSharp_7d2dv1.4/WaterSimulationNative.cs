...
public class WaterSimulationNative
{
	...
	public struct ChunkHandle
	{
		...
		public void WakeNeighbours(int _x, int _y, int _z)
		{
			if (IsValid)
			{
				sim.voxelsToWakeup.Add(chunkKey, new int3(_x, _y, _z));
	...
	public void Init(ChunkCluster _cc)
	{
		...
		{
			if (waterDataHandles.IsCreated || modifiedChunks.IsCreated || voxelsToWakeup.IsCreated)
			{
			...
			voxelsToWakeup = new UnsafeParallelMultiHashMap<ChunkKey, int3>(1000, AllocatorManager.Persistent);
	...
	public string GetMemoryStats()
	{
		...
		num3 += ProfilerUtils.CalculateUnsafeParallelMultiHashMapBytes(voxelsToWakeup);
