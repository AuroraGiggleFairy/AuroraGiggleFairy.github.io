...
public class EntityTrader : EntityNPC
{
	...
	public void ActivateTrader(bool traderIsOpen)
	{
		...
		case 2:
		{
			Waypoint entityVehicleWaypoint = entityPlayerLocal.Waypoints.GetEntityVehicleWaypoint(entityId);
	...
	public void SetupActiveQuestsForPlayer(EntityPlayer player, int overrideFactionPoints = -1)
	{
		...
		QuestEventManager.Current.SetupQuestList(entityId, player.entityId, activeQuests);
	}
	...
	public bool UpdateLocations(int tier, List<Vector2> pois)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			num += QuestEventManager.Current.GetPrefabsForTrader(traderArea, tier, i, world.GetGameRandom())?.Count ?? 0;
		}
	...
	public List<Quest> PopulateActiveQuests(EntityPlayer player, int currentTier = -1, int questFactionPoints = -1)
	{
		...
		for (int i = 1; i <= currentTier; i++)
		{
			...
			if (UpdateLocations(i, usedPOILocations))
			{
				list3 = null;
				if (traderData != null)
				{
			...
			if (list3 != null)
			{
				usedPOILocations.AddRange(list3);
			}
			list2.Clear();
			for (int j = 0; j < questDictionary[i].Count; j++)
			{
				QuestEntry questEntry = questDictionary[i][j];
			...
			for (int k = 0; k < 100; k++)
			{
				...
				if (!(rand.RandomFloat < questEntry2.Prob))
				{
					continue;
				}
	...
	public override void OnEntityTargeted(EntityAlive target)
	{
		...
		if (!isEntityRemote && GetSpawnerSource() != EnumSpawnerSource.Dynamic && (object)target != null)
		{
