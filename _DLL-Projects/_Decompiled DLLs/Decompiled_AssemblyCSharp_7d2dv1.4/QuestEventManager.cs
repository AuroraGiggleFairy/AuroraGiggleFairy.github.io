...
public class QuestEventManager
{
	...
	public class PrefabListData
	{
		...
		{
			if (TierData.ContainsKey(difficulty))
			{
				List<PrefabInstance> list = TierData[difficulty];
	...
	public void Update()
	{
		...
		for (int j = 0; j < removeSleeperDataList.Count; j++)
		{
			SleeperVolumeUpdateDictionary.Remove(removeSleeperDataList[j]);
		}
	...
	public void SetupQuestList(int npcEntityID, int playerEntityID, List<Quest> questList)
	{
		...
		{
			npcQuestData.Add(npcEntityID, new NPCQuestData());
		}
		if (!npcQuestData[npcEntityID].PlayerQuestList.ContainsKey(playerEntityID))
		{
			npcQuestData[npcEntityID].PlayerQuestList.Add(playerEntityID, new NPCQuestData.PlayerQuestData(questList));
		}
		else
		{
			npcQuestData[npcEntityID].PlayerQuestList[playerEntityID].QuestList = questList;
		}
	}
