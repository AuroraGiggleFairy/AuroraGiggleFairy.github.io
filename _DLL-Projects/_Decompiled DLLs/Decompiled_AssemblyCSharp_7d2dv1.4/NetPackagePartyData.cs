		...
		if (localPlayers != null && localPlayers.Count > 0)
		{
				...
				if (changedEntityID != -1)
				{
					...
					case PartyActions.Disconnected:
						if (entityPlayer != entityPlayerLocal)
						{
							...
							GameManager.ShowTooltip(entityPlayerLocal, string.Format(Localization.Get("ttPartyDisconnectedFromParty"), entityPlayer.PlayerDisplayName));
					...
					case PartyActions.KickFromParty:
						...
						{
							Manager.PlayInsidePlayerHead("party_member_leave");
							GameManager.ShowTooltip(entityPlayerLocal, string.Format(Localization.Get("ttPartyOtherKickedFromParty"), entityPlayer.PlayerDisplayName));
							entityPlayerLocal.QuestJournal.RemovePlayerFromSharedWiths(entityPlayer);
						}
						break;
					case PartyActions.LeaveParty:
						...
						{
							Manager.PlayInsidePlayerHead("party_member_leave");
							GameManager.ShowTooltip(entityPlayerLocal, string.Format(Localization.Get("ttPartyOtherLeftParty"), entityPlayer.PlayerDisplayName));
						}
						break;
			...
			else if (entityPlayerLocal == entityPlayer)
			{
				...
				case PartyActions.KickFromParty:
					...
					GameManager.ShowTooltip(entityPlayerLocal, Localization.Get("ttPartyKickedFromParty"));
					break;
				case PartyActions.LeaveParty:
					Manager.PlayInsidePlayerHead("party_leave");
					break;
