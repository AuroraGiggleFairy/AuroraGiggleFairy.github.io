...
public class ConsoleCmdScreamerAlert : ConsoleCmdAbstract
{
    ...
    public override string getHelp()
    {
        return "Usage:\n"
            ...
            + "  agf-screameralert <off|on|numbers|cycle>\n"
            + "  agf-screameralert admin <entityId> <off|on|numbers|cycle>\n"
            + "Modes:\n"
            + "  1) off: hide all Screamer Alert UI text\n"
            + "  2) on: show text only (no counts)\n"
            + "  3) numbers: show text and counts\n"
    ...
    private static void HandleAdmin(List<string> args, int senderEntityId)
    {
        ...
        if (args.Count < 3)
        {
            SdtdConsole.Instance.Output("[ScreamerAlert] Usage: agf-screameralert admin <entityId> <off|on|numbers|cycle>");
            return;
        }
