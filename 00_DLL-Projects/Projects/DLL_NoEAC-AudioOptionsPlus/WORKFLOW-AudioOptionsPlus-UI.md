...
- Build and deploy DLL from project root output:
  - Build: `dotnet build -c Release` in `00_DLL-Projects/Projects/DLL_NoEAC-AudioOptionsPlus`
...
- 2026-07-16:
  ...
  - Synced `_x2.6` tab label key `xuiOptionsAudioAOPVolumeProfiles` english text from `Volume Profiles` to `Custom Volumes` to match live mod localization and observed in-game header.
  - Updated non-English translations for `xuiOptionsAudioAOPVolumeProfiles` in both live and `_x2.6` localization files so the tab label meaning matches `Custom Volumes` across languages.
  - Fixed first-render Sound Swap animal dropdown labels showing raw localization keys by updating `XUiC_AudioOptionsPlusSoundSwap` to use current token values (`Default`/`Swapped`) and normalize legacy key-based values on load.
  - Built and deployed updated `AudioOptionsPlus.dll` to the live mod path after clearing a file lock from the running game process.
  - Reduced Sound Swap animal option control widths by setting explicit per-control `width="500"` for all animal swap rows; `grid cell_width` alone did not constrain control render width.
  - Fixed AOP option dirty-state behavior so a single changed option marks only that option (plus menu/apply), instead of all AOP options across both tabs showing changed.
  - Implemented per-option `IsChangedDelegate` binding via `HasPendingChangeForOption(instance, name)` and deployed rebuilt live DLL.
