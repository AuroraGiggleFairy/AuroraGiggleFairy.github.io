...
Outputs (written self-contained under GeneratedFiles/ next to this generator):
- GeneratedFiles/Config/Localization.csv
- GeneratedFiles/Config/XUi_InGame/windows.xml
- GeneratedFiles/Config/XUi_InGame/templates.xml (or legacy xui.xml)
- GeneratedFiles/Links/*.xml

Pushing to a live/draft mod folder is a separate, explicit copy step;
this generator no longer writes directly into a mod's Config/Links folders.
"""
...
# Generated outputs are self-contained inside the generator's own folder.
# Pushing them into a live/draft mod folder is a separate, explicit copy step.
GENERATED_FILES_DIR = GENERATOR_DIR / "GeneratedFiles"
DEFAULT_LINKS_DIR = GENERATED_FILES_DIR / "Links"
DEFAULT_LOCALIZATION_FILE = GENERATED_FILES_DIR / "Config" / "Localization.csv"
DEFAULT_WINDOWS_FILE = GENERATED_FILES_DIR / "Config" / "XUi_InGame" / "windows.xml"
DEFAULT_XUI_FILE = GENERATED_FILES_DIR / "Config" / "XUi_InGame" / "templates.xml"
DEFAULT_STYLES_FILE = GENERATED_FILES_DIR / "Config" / "XUi_InGame" / "styles.xml"
DEFAULT_LEGACY_XUI_FILE = GENERATED_FILES_DIR / "Config" / "XUi_InGame" / "xui.xml"
...
<grid name="optionsList" pos="20,-140" depth="10" rows="6" cols="1" cell_width="300" cell_height="100" arrangement="vertical">
    <conditional>
        <if cond="mod_loaded('AGF-NoEAC-ScreamerAlert')">
            <entry name="screamerAlert">
                <rect name="agfOptionsScreamer" controller="ScreamerAlertOptions, ScreamerAlert" pos="0,0" width="280" height="120" disableautobackground="true">
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Localization file to merge generated keys into (default: Config/Localization.csv).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "XUi templates file used for admin window registration "
            "(default: Config/XUi_InGame/templates.xml; falls back to legacy xui.xml if present)."
        ),
    )
    parser.add_argument(
        "--styles-file",
        default=str(DEFAULT_STYLES_FILE),
        help="XUi styles file used for color style_entry updates (default: Config/XUi_InGame/styles.xml).",
    )
    parser.add_argument(
        "--windows-link-id",
        default="discord",
...
def _large_tab_button_xml(*, is_hv: bool) -> str:
    ...
    return (
        '<simplebutton name="tabButton" depth="220" pos="0,0" width="170" height="36" '
        ...
        'foregroundlayer="true" caption="{tab_name_localized}"/>'
    )
...
def apply_text_overrides(source: dict, texts_path: Path) -> tuple[dict, int]:
    ...
    if isinstance(colors_overrides, dict):
        ...
        color_aliases = {
            ...
            "linkBackgroundColor": "linkBackground",
            "optionsBackgroundColor": "optionsBackground",
            "panelBorderColor": "panelBorder",
            "borderColor": "panelBorder",
            "headerTitleColor": "headerTitle",
            "headerMottoColor": "headerMotto",
            "pageTabButtonBackgroundColor": "pageTabButtonBackground",
            "pageTabButtonBorderColor": "pageTabButtonBorder",
            "sectionButtonBorderColor": "pageTabButtonBorder",
            "pageTabButtonSelectedColor": "pageTabButtonSelected",
            "tabButtonSelectedColor": "pageTabButtonSelected",
            "pageTabButtonTextColor": "pageTabButtonText",
            "pageButtonTextColor": "pageTabButtonText",
            "pageTitleColor": "pageTitle",
            "pageBodyColor": "pageBody",
            "textBoldingColor": "textBolding",
            "textHighlightColor": "textHighlight",
            "linkButtonBackgroundColor": "linkButtonBackground",
            "linkButtonBorderColor": "linkButtonBorder",
            "linkButtonTextColor": "linkButtonText",
            "linkButtonHoverColor": "linkButtonHover",
        }
        for key, value in colors_overrides.items():
            if key in {
                ...
                canonical_key = color_aliases.get(key, key)
                theme_color[canonical_key] = str(value)
                # Keep the text-config key mirrored for backward compatibility reads.
...
def build_localization_rows(source: dict, use_hv_dev_overrides: bool = False) -> list[list[str]]:
    ...
    tracker_title = labels.get("optionsTrackerTitle", "Visual Entity Tracker")
    ...
    rows: list[list[str]] = [
        ...
        [
            "windowESC_options_tracker_tooltip_off",
            "UI",
            "Hides entity tracking on your compass.",
        ],
        [
            "windowESC_options_tracker_tooltip_on",
            "UI",
            "Shows entities on your compass within 20m.",
        ],
    ...
    color_rows.extend(
        [
            ...
            ["windowESC_color_options_tracker_title", "UI", tracker_title],
            [
                "windowESC_color_options_tracker_off",
                "UI",
                _colorize_text("Off", theme_color["pageTabButtonText"]),
            ],
            [
                "windowESC_color_options_tracker_on",
                "UI",
                _colorize_text("On", theme_color["pageTabButtonText"]),
            ],
        ]
    ...
    high_rows.extend(
        [
            ...
            ["windowESC_highvisibility_options_tracker_title", "UI", tracker_title],
            [
                "windowESC_highvisibility_options_tracker_off",
                "UI",
                _colorize_text("Off", theme_hv["pageTabButtonText"]),
            ],
            [
                "windowESC_highvisibility_options_tracker_on",
                "UI",
                _colorize_text("On", theme_hv["pageTabButtonText"]),
            ],
        ]
        ...
        for tab in tab_models:
            ...
            if len(body_columns) <= 1:
                if scrollable_body:
                    pages.extend(
                        [
                            f'\t\t\t<rect name="{body_name}_scroll" depth="10" pos="{single_body_pos}" width="{single_body_width}" height="{PAGE_BODY_HEIGHT}" on_scroll="true" disableautobackground="true" createuipanel="true" clipping="SoftClip" clippingsoftness="4,4" clippingsize="{single_body_width},{PAGE_BODY_HEIGHT}" clippingcenter="{single_body_width // 2},-{PAGE_BODY_HEIGHT // 2}">',
                ...
                for column_idx, (pos_value, width_value) in enumerate(body_layout, start=1):
                    ...
                    if scrollable_body:
                        pages.extend(
                            [
                                f'\t\t\t<rect name="{body_name_column}_scroll" depth="10" pos="{body_col_pos}" width="{body_col_width}" height="{PAGE_BODY_HEIGHT}" on_scroll="true" disableautobackground="true" createuipanel="true" clipping="SoftClip" clippingsoftness="4,4" clippingsize="{body_col_width},{PAGE_BODY_HEIGHT}" clippingcenter="{body_col_width // 2},-{PAGE_BODY_HEIGHT // 2}">',
                                _build_body_label_xml(
                                    indent="\t\t\t\t",
        ...
        if len(body_columns) <= 1:
            if scrollable_body:
                lines.extend(
                    [
                        (
                            f'\t\t<rect name="{body_name}_scroll" depth="10" pos="{single_body_pos}" width="{single_body_width}" '
            ...
            for column_idx, (pos_value, width_value) in enumerate(body_layout, start=1):
                ...
                if scrollable_body:
                    lines.extend(
                        [
                            (
                                f'\t\t<rect name="{body_name_column}_scroll" depth="10" pos="{body_col_pos}" width="{body_col_width}" '
...
def _localization_fixed_translations() -> dict[str, dict[str, str]]:
    return {
        ...
        "windowESC_color_options_screamer_title": {
            "german": "Schreier-Alarm",
            "spanish": "Alerta de chilladora",
            "french": "Alerte de hurleuse",
            "italian": "Allarme strillatrice",
            "japanese": "スクリーマー警報",
            "koreana": "스크리머 경보",
            "polish": "Alarm krzykaczki",
            "brazilian": "Alerta de gritadora",
            "russian": "Тревога крикуна",
            "turkish": "Ciglikci Alarmi",
            "schinese": "尖叫者警报",
            "tchinese": "尖叫者警報",
        },
        "windowESC_highvisibility_options_screamer_off": {
        ...
        "windowESC_highvisibility_options_screamer_title": {
            "german": "Schreier-Alarm",
            "spanish": "Alerta de chilladora",
            "french": "Alerte de hurleuse",
            "italian": "Allarme strillatrice",
        ...
        "windowESC_color_options_tracker_title": {
            "german": "Visueller Entitaten-Tracker",
            "spanish": "Rastreador visual de entidades",
            "french": "Suivi visuel des entites",
            "italian": "Tracciatore visivo entita",
            "japanese": "視覚エンティティトラッカー",
            "koreana": "시각적 엔티티 추적기",
            "polish": "Wizualny tracker encji",
            "brazilian": "Rastreador visual de entidades",
            "russian": "Визуальный трекер сущностей",
            "turkish": "Gorsel Varlik Izleyici",
            "schinese": "视觉实体追踪器",
            "tchinese": "視覺實體追蹤器",
        },
        "windowESC_highvisibility_options_tracker_off": {
        ...
        "windowESC_highvisibility_options_tracker_title": {
            "german": "Visueller Entitaten-Tracker",
            "spanish": "Rastreador visual de entidades",
            "french": "Suivi visuel des entites",
            "italian": "Tracciatore visivo entita",
        ...
        "windowESC_options_tracker_tooltip_off": {
            "german": "Blendet die Entitatenverfolgung auf deinem Kompass aus.",
            "spanish": "Oculta el seguimiento de entidades en tu brujula.",
            "french": "Masque le suivi des entites sur votre boussole.",
            "italian": "Nasconde il tracciamento delle entita sulla bussola.",
            "japanese": "コンパス上のエンティティ追跡を非表示にします。",
            "koreana": "나침반에서 엔티티 추적을 숨깁니다.",
            "polish": "Ukrywa sledzenie encji na kompasie.",
            "brazilian": "Oculta o rastreamento de entidades na sua bussola.",
            "russian": "Скрывает отслеживание сущностей на вашем компасе.",
            "turkish": "Pusulanizdaki varlik takibini gizler.",
            "schinese": "在指南针上隐藏实体追踪。",
            "tchinese": "在指南針上隱藏實體追蹤。",
        },
        "windowESC_options_tracker_tooltip_on": {
            "german": "Zeigt nahe Entitaten innerhalb von 20 m auf deinem Kompass an.",
            "spanish": "Muestra entidades cercanas en tu brujula dentro de 20 m.",
            "french": "Affiche les entites proches sur votre boussole dans un rayon de 20 m.",
            "italian": "Mostra le entita vicine sulla bussola entro 20 m.",
            "japanese": "20m以内の近くのエンティティをコンパスに表示します。",
            "koreana": "20m 이내의 주변 엔티티를 나침반에 표시합니다.",
            "polish": "Pokazuje pobliskie encje na kompasie w zasiegu 20 m.",
            "brazilian": "Mostra entidades proximas na sua bussola em um raio de 20 m.",
            "russian": "Показывает ближайшие сущности на вашем компасе в радиусе 20 м.",
            "turkish": "20 m icindeki yakin varliklari pusulanizda gosterir.",
            "schinese": "在指南针上显示20米范围内的附近实体。",
            "tchinese": "在指南針上顯示20公尺範圍內的附近實體。",
        },
        ...
        "windowESCAdmin_mode_highvisibility_tab": {
            "german": "Hoher Kontrast",
            "spanish": "Alto contraste",
            "french": "Contraste eleve",
            "italian": "Alto contrasto",
        ...
        "windowESCAdmin_player_view_tab": {
            "german": "Spieleransicht",
            "spanish": "Vista de jugador",
            "french": "Vue joueur",
            "italian": "Vista giocatore",
            "japanese": "プレイヤービュー",
            "koreana": "플레이어 보기",
            "polish": "Widok gracza",
            "brazilian": "Visao do jogador",
            "russian": "Вид игрока",
            "turkish": "Oyuncu Gorunumu",
            "schinese": "玩家视图",
            "tchinese": "玩家視圖",
        },
        "windowESCAdmin_admin_view_tab": {
            "german": "Admin-Ansicht",
            "spanish": "Vista de admin",
            "french": "Vue admin",
            "italian": "Vista admin",
            "japanese": "管理者ビュー",
            "koreana": "관리자 보기",
            "polish": "Widok administratora",
            "brazilian": "Visao de admin",
            "russian": "Вид администратора",
            "turkish": "Yonetici Gorunumu",
            "schinese": "管理员视图",
            "tchinese": "管理員視圖",
        },
        ...
        "windowESC_color_join_title": {
            "german": "Verlassen oder Beitreten",
            "spanish": "Salir o unirse",
            "french": "Quitter ou rejoindre",
            "italian": "Esci o entra",
            "japanese": "退出または参加",
            "koreana": "나가기 또는 참가",
            "polish": "Wyjdz lub dolacz",
            "brazilian": "Sair ou entrar",
            "russian": "Выйти или войти",
            "turkish": "Ayril veya Katil",
            "schinese": "离开或加入",
            "tchinese": "離開或加入",
        },
        "windowESC_highvisibility_join_title": {
            "german": "Verlassen oder Beitreten",
            "spanish": "Salir o unirse",
            "french": "Quitter ou rejoindre",
            "italian": "Esci o entra",
...
def _write_localization_full_csv(rows: list[list[str]], out_file: Path) -> Path:
    ...
    english_col = full_header_lookup.get("english", 6)
    ...
    for idx, row in enumerate(ordered_rows):
        ...
        full_row[0] = key
        type_col = full_header_lookup.get("type")
        context_col = full_header_lookup.get("context / alternate text")
        if type_col is not None:
            full_row[type_col] = "UI"
        if english_col is not None:
            full_row[english_col] = english
        if context_col is not None:
            full_row[context_col] = context
...
def merge_localization(
    ...
    if target_file.exists():
        # utf-8-sig safely handles files that include a BOM without polluting the first header key.
...
def _build_options_inner_xml(is_hv: bool) -> str:
    ...
    replacements = {
        ...
        'text_key="windowESC_color_options_screamer_title"': 'text_key="windowESC_highvisibility_options_screamer_title"',
        'caption_key="windowESC_color_options_screamer_off"': 'caption_key="windowESC_highvisibility_options_screamer_off"',
        'caption_key="windowESC_color_options_screamer_on"': 'caption_key="windowESC_highvisibility_options_screamer_on"',
        'caption_key="windowESC_color_options_screamer_on_num"': 'caption_key="windowESC_highvisibility_options_screamer_on_num"',
        'text_key="windowESC_color_options_tracker_title"': 'text_key="windowESC_highvisibility_options_tracker_title"',
        'caption_key="windowESC_color_options_tracker_off"': 'caption_key="windowESC_highvisibility_options_tracker_off"',
        'caption_key="windowESC_color_options_tracker_on"': 'caption_key="windowESC_highvisibility_options_tracker_on"',
    }
    updated = inner
...
def update_windows_options_visibility(windows_file: Path, source: dict) -> dict[str, int]:
    ...
    def _ensure_links_rect_blocks(xml_text: str) -> tuple[str, int]:
        ...
        def _collect_named_tag_spans(xml: str, tag_name: str, target_name: str) -> list[tuple[int, int]]:
            ...
            for match in tag_pattern.finditer(xml):
                ...
                if not is_close:
                    attrs = match.group(2)
                    name_match = name_pattern.search(attrs)
                    elem_name = name_match.group(1).strip() if name_match else ""
                    if tag_text.rstrip().endswith("/>"):
                        continue
                    stack.append((elem_name, match.start()))
                    continue

                if not stack:
                    continue

                elem_name, start_idx = stack.pop()
                if elem_name == target_name:
                    found.append((start_idx, match.end()))

            return found

        spans = _collect_named_tag_spans(xml_text, "panel", "core")
        if not spans:
            spans = _collect_named_tag_spans(xml_text, "rect", "core")
        ...
        for start_idx, end_idx in sorted(spans, reverse=True):
            ...
            container_line_match = re.search(r'(?m)^(?P<indent>[ \t]*)<(?:panel|rect)\b', block)
            container_indent = container_line_match.group("indent") if container_line_match else ""
            ...
            if options_match:
                ...
                container_close_match = re.search(r'(?m)^[ \t]*</(?:panel|rect)>\s*$', block)
                if not container_close_match:
                    continue
                insert_at = container_close_match.start()
...
def sync_windows_copied_variants(
    ...
    def _extract_first_named_panel_block(block: str, panel_name: str) -> tuple[int, int, str] | None:
        open_match = re.search(
            rf'<panel\b[^>]*\bname="{re.escape(panel_name)}"[^>]*>',
            block,
            re.IGNORECASE,
        )
        if not open_match:
        ...
        depth = 0
        for tag in re.finditer(r'</?panel\b[^>]*>', block[scan_start:], re.IGNORECASE):
    ...
    def _convert_core_panels_to_rect(block: str) -> str:
        updated = block
        while True:
            panel_range = _extract_first_named_panel_block(updated, "core")
            if not panel_range:
                break
            start, end, panel_block = panel_range
            converted_block = re.sub(r'<panel\b', '<rect', panel_block, count=1, flags=re.IGNORECASE)
            converted_block = re.sub(r'</panel>(\s*)$', r'</rect>\1', converted_block, count=1, flags=re.IGNORECASE)
            updated = updated[:start] + converted_block + updated[end:]
        return updated
    ...
    source_window = _convert_core_panels_to_rect(source_window)
    ...
    def _ensure_pages_headers(block: str, target_context: str) -> str:
        # Keep page headers unchanged.
        # Prior regex-based normalization could cross root/nested TabSelector boundaries,
        # creating malformed duplicated tabsContents/page blocks.
        return block
    ...
    def _normalize_large_mode_selector(block: str, is_hv: bool = False) -> str:
        pattern = re.compile(
            r'<simplebutton\s+name="tabButton"\s+depth="\d+"\s+pos="0,0"\s+width="170"\s+height="36"\s+font_size="30"[^>]*/>',
            re.IGNORECASE,
        )
        # Main window top mode tabs must remain on the normal interaction layer.
        main_mode_button = (
            '<simplebutton name="tabButton" depth="12" pos="0,0" width="170" height="36" '
            'font_size="30" sprite="ui_game_header_fill" '
            'bordercolor="[pageTabButtonBorderColor]" defaultcolor="[mediumGrey]" '
            'selectedsprite="menu_empty3px" selectedcolor="[lightGrey]" '
            'foregroundlayer="false" caption="{tab_name_localized}"/>'
        )
        return pattern.sub(main_mode_button, block, count=1)

    def _reorder_root_mode_tabs_for_overlay(block: str) -> str:
        """No-op guard: root mode tabs order is managed by source template structure."""
        return block
    ...
    source_window = _reorder_root_mode_tabs_for_overlay(source_window)
    ...
    server_block = _reorder_root_mode_tabs_for_overlay(server_block)
    ...
    admin_block = _reorder_root_mode_tabs_for_overlay(admin_block)
...
def update_xui_admin_window_binding(xui_file: Path, enabled: bool) -> int:
    """Ensure templates/xui file includes admin window registration only when enabled."""
    ...
    if block_match:
        ...
        block = (
            "\n\t<append xpath=\"/xui/window_group[@name='ingameDebugMenu']\">\n"
...
def normalize_xui_v3_append_paths(xui_file: Path) -> int:
    """Normalize legacy /xui/ruleset append paths to v3 /xui paths."""
    if not xui_file.exists():
        return 0

    original = xui_file.read_text(encoding="utf-8")
    updated = re.sub(
        r'(<append\s+xpath=["\'])/xui/ruleset(?P<suffix>[^"\']*)(["\'])',
        r'\1/xui\g<suffix>\3',
        original,
        flags=re.IGNORECASE,
    )
    if updated != original:
        xui_file.write_text(updated, encoding="utf-8")
        return 1
    return 0


def update_styles_theme_values(
    styles_file: Path,
    source: dict,
    use_hv_dev_overrides: bool = False,
    texts_path: Path | None = None,
    update_hv_entries: bool = False,
) -> dict[str, int]:
    """Sync styles.xml style_entry RGB values from the current generator themes."""
    ...
    theme_color = get_theme_colors(source, "color")
    theme_hv = (
        get_theme_colors(source, "highVisibility", use_hv_dev_overrides=use_hv_dev_overrides)
        if update_hv_entries
        else {}
    )

    # Guarantee txt-config colors flow into styles.xml even when source JSON keeps legacy keys.
    if texts_path is not None and texts_path.exists():
        ...
        if isinstance(raw_colors, dict):
            ...
            for key, value in raw_colors.items():
                canonical = text_color_aliases.get(str(key).strip())
                if canonical:
                    theme_color[canonical] = str(value)
    ...
    def _normalize_rgb_triplet(value: str, fallback: str) -> str:
        rgb = _parse_rgb_value(str(value or ""))
        if rgb is None:
            rgb = _parse_rgb_value(str(fallback or ""))
        if rgb is None:
            rgb = (255, 255, 255)
        return f"{rgb[0]},{rgb[1]},{rgb[2]}"

    def _update_style_entry_value(xml_text: str, style_name: str, new_value: str) -> tuple[str, int]:
        pattern = re.compile(
            rf'(<style_entry\b[^>]*\bname="{re.escape(style_name)}"[^>]*\bvalue=")(.*?)("[^>]*/?>)',
            re.IGNORECASE,
        )
        updates = 0
        ...
        return pattern.sub(_replace, xml_text), updates

    updated = text
    for style_name, theme_key in style_map.items():
        base_value = _normalize_rgb_triplet(
            theme_color.get(theme_key, DEFAULT_THEME_COLORS[theme_key]),
            DEFAULT_THEME_COLORS[theme_key],
        )
        updated, count = _update_style_entry_value(updated, style_name, base_value)
        if count > 0:
            changed_by_key[style_name] = changed_by_key.get(style_name, 0) + count

        if update_hv_entries:
            hv_name = f"{style_name}HV"
            hv_fallback = DEFAULT_AUTO_HIGH_VISIBILITY_COLORS.get(theme_key, DEFAULT_THEME_COLORS[theme_key])
            hv_value = _normalize_rgb_triplet(theme_hv.get(theme_key, hv_fallback), hv_fallback)
            updated, count_hv = _update_style_entry_value(updated, hv_name, hv_value)
            if count_hv > 0:
                changed_by_key[hv_name] = changed_by_key.get(hv_name, 0) + count_hv

    if updated != text:
        styles_file.write_text(updated, encoding="utf-8")

    return changed_by_key


def update_mode_selector_visibility(windows_file: Path) -> dict[str, int]:
    """Raise top mode selector tab-buttons so they are not obscured by headers."""
    if not windows_file.exists():
        return {}

    text = windows_file.read_text(encoding="utf-8")
    window_pattern = re.compile(
        r'(?P<window><window\b[^>]*\bname="windowESC"[^>]*>.*?</window>)',
        re.IGNORECASE | re.DOTALL,
    )
    ...
    window_block = window_match.group("window")
    updates = 0

    pattern = re.compile(
        r'(<grid\s+name="tabButtons"\s+pos=")(?P<x>-?\d+),(?P<y>-?\d+)("\s+depth=")(?P<depth>\d+)("\s+rows="2"\s+cols="1"\s+cell_width="170"\s+cell_height="46"\s+repeat_content="true"\s+arrangement="horizontal">)',
        re.IGNORECASE,
    )
    ...
    def _replace(match: re.Match[str]) -> str:
        ...
        # Keep root mode selector at canonical placement and interaction depth.
        new_y = 438
        new_depth = 12
        if new_y == y and new_depth == depth:
            return match.group(0)

        updates += 1
        return f'{match.group(1)}{x},{new_y}{match.group(4)}{new_depth}{match.group(6)}'

    updated_window = pattern.sub(_replace, window_block)
    if updated_window == window_block:
        return {}

    updated = text[: window_match.start()] + updated_window + text[window_match.end() :]
    if updates > 0:
        windows_file.write_text(updated, encoding="utf-8")
        return {"modeSelectorTabs": updates}
    return {}
...
def main() -> None:
    ...
    windows_mode_selector_updates: dict[str, int] = {}
    styles_updates: dict[str, int] = {}
    windows_admin_comment_updates = 0
    xui_path_updates = 0
    xui_admin_updates = 0
    windows_pretty_updates = 0
    xui_pretty_updates = 0
    styles_pretty_updates = 0
    if args.update_windows_sources or args.update_windows_links_layout:
        ...
        if not xui_file.exists() and Path(args.xui_file) == DEFAULT_XUI_FILE and DEFAULT_LEGACY_XUI_FILE.exists():
            xui_file = DEFAULT_LEGACY_XUI_FILE
            print(f"Templates file not found, using legacy xui file: {xui_file}")
    ...
    if windows_file is not None and (args.update_windows_links_layout or args.update_windows_sources):
        ...
        windows_mode_selector_updates = update_mode_selector_visibility(windows_file)
        if xui_file is not None:
            xui_path_updates = normalize_xui_v3_append_paths(xui_file)
            xui_admin_updates = update_xui_admin_window_binding(
                xui_file,
                feature_toggles["showAdminVersion"],
            )
        if styles_file is not None:
            styles_updates = update_styles_theme_values(
                styles_file,
                source,
                use_hv_dev_overrides=args.use_hv_dev_overrides,
    ...
    if windows_file is not None and (args.update_windows_links_layout or args.update_windows_sources):
        ...
        if styles_file is not None and styles_file.exists():
            styles_pretty_updates = pretty_print_xml_file(styles_file)
    ...
    if windows_file is not None:
        ...
        total_grid_updates = (
            ...
            + sum(windows_mode_selector_updates.values())
        )
        print(
        ...
        if windows_mode_selector_updates:
            details = ", ".join(f"{k}={v}" for k, v in sorted(windows_mode_selector_updates.items()))
            print(f"Windows mode selector updates: {details}")
        if xui_file is not None and xui_path_updates > 0:
            print(f"XUi v3 path normalization updates: {xui_path_updates} in {xui_file}")
        if xui_file is not None and xui_admin_updates > 0:
            print(f"XUi admin registration updates: {xui_admin_updates} in {xui_file}")
        if styles_file is not None and styles_updates:
            details = ", ".join(f"{k}={v}" for k, v in sorted(styles_updates.items()))
            print(f"Styles color updates: {details} in {styles_file}")
        ...
        if styles_file is not None and styles_pretty_updates > 0:
            print(f"XML pretty print applied: {styles_file}")
