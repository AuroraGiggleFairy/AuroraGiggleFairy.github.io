...
4. Install Scope & EAC Requirement
- Server-Side (EAC Off): EAC off required; server install works for all joining players.
...
- Works Standalone
- Sends private chat alerts when Screamers or Screamer hordes spawn nearby (120m).
- Players can toggle alerts with chat commands: /agfsa on and /agfsa off.
- Commands README is available for players and admins.
- If a player also has EnhancedAGF installed, they get live on-screen alerts.
...
- Sends private chat alerts when qualifying enemies spawn within 120 meters.
- If a player also has EnhancedAGF installed, alerts can show live on-screen.
- EnhancedAGF can also show live detected counts for Screamers and Screamer horde zombies.
...
- agf-sa default : Set the default mode for new joining players.
- agf-sa set  : Set mode for one online player or all online players.
- agf-sa list: List online players with entityId, mode, and EnhancedAGF capability state.
...
v2.2.0
- Updated to work with WindowESC mod.
- Updated to work with DishongTowerChallenge mod.

v2.1.2
- Forgot to update the short description in modinfo.xml.

v2.1.1
- Removed the "BETA" call.

v2.1.0
- Reworked to function as a Server Side mod.
- Player chat commands available.
- Admin chat commands available.
- Commands ReadMe is available.
- Set up to if EnhancedAGF is installed, you get UI support instead of a chat alert.
